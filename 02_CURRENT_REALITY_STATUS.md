...
- 真实支付、生产数据、生产部署、公网暴露。
- Windows 宿主 Testcontainers 仍不可用；工程门禁以 **WSL Linux Docker** 为准。
- 冒烟镜像为缓存 `sub2api:local`（可能早于 tip）；未强制重建。
- 浏览器 UI 截图：未补；不阻止工程「通过」。

## 事实边界

mock 成功不等于生产可用；`result_url` 存在不等于资产已持久交付；工程「通过」不等于产品生产 READY。
