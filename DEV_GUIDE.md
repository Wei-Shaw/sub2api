...
**当前反转 KPI**（`rg -l 'internal/service"' backend/internal/repository --type go -g '!*_test.go' | wc -l`）：**0**（**目标达成：repo→service 反转彻底清零**）。本会话从 69 降至此——account_repo domain 换包、RoleAdmin、port/cache + port/oauthclient 两批基础设施端口，六个成体 BC，一批 Tier-1 小 BC 扫描，account_repo 最终残留清零，单接口端口 sweep，以及 shared-infra tail（Phase 1-5）：
...
- **shared-infra tail**（Phase 1-5，−8，收尾）：Phase 1 `port/concurrency`（ConcurrencyCache 17 方法）+ 补搬 `ImageStorageFactory`→`port/imagestorage`，`concurrency_cache.go`+`wire.go` drop；Phase 2 `UserRPMCache`/`GatewayCache`/`CyberSessionBlockStore`→扩展 `port/cache`（`gateway_service.go` 用 `portcache` 别名避 `cache` 字段冲突）；Phase 3 `HTTPUpstream`+Profile（含 4 个 ctx helpers）→`port/upstream`，`ClaudeUsageFetcher`→`port/claudeusage`；Phase 5 `UsageLogCreateError`+markers→`domain`；Phase 4 APIKey 认证快照簇（4 struct，引用类型已在 domain）→`domain/api_key_auth_cache.go` + `APIKeyCache`+`With*/NotifyAuthCacheSubscriptionReady` ctx pattern→`port/apikeycache`。

**repo→service 反转已彻底清零** —— `internal/repository/` 下无任何非测试文件 import `internal/service`。handler 仍依赖 `service.OpenAIGatewayService` 等应用服务（设计如此，非反转）；若要进一步解耦 handler，可对这些应用服务做接口提取（handler-facing port），但那是另一个目标，不在本次 BC 提取范围内。
