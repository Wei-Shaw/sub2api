...
**当前反转 KPI**（`rg -l 'internal/service"' backend/internal/repository --type go -g '!*_test.go' | wc -l`）：**29**。本会话从 69 降至此：account_repo domain 换包、RoleAdmin、port/cache + port/oauthclient 两批基础设施端口，以及三个成体 BC——
...
- **BatchImage BC**：`domain/batch_image.go`（~12 实体/DTO + 56 错误 + 16 常量 + 5 纯函数）+ `port/batchimage`（BatchImageRepository 32 方法 + Queue + JobLock/Refresher + DownloadLimiter/Permit）；3 个 batch_image repo 文件 drop service。`image_task_store.go` 属独立 image-task BC（imgtask_ 命名空间），defer。

剩余 repo 文件多为未提取 BC 的实体/读模型（image-task、scheduler-outbox、gemini-quota、account_repo 残留的 Grok/Group/Scheduler 符号等）以及若干单接口阻塞（SecretEncryptor/ImageStorage/DBDumper，分属 auth/image/backup 等待提取 BC）——这些应随各自 BC 成体拆迁，而非提前单接口提取。
