...
- 提示词记录默认保存完整请求体和请求头，支持通过“请求头”和“提示词”两个独立开关控制新记录的保存内容；关闭“提示词”后不保存请求体和提示词正文，历史记录不受影响。
- 提示词记录开启“过滤预设”后，保存的请求体仅保留用户与助手的交互正文及多模态输入，并移除系统与开发者指令、工具过程、推理、客户端元数据、请求参数和可识别的代理预设区块。
