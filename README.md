...
- **Multi-Account Management** - Support multiple upstream account types (OAuth, API Key, Codex Agent Identity)
...
- **Codex Agent Identity** - Import `auth.json` files generated by `codex-agent-identity` directly from the admin dashboard; Responses HTTP/WebSocket requests use per-request AgentAssertion signatures and share Codex quota monitoring
