...
- 提示词记录支持保留天数、自动到期清理和游标翻页；后台处理增加字节预算、响应就绪回填及正常停机排空，页面可以查看处理统计，保留天数设置与总开关并排显示以收紧列表上方空间。
- 修复带路径的 AGENTS.md 预设过滤和大响应截断状态，降低大请求记录开销；响应保存覆盖 HTTP JSON/SSE，WebSocket 目前只保存各轮请求。
- 提示词记录默认保存请求头、提示词和响应，支持分别控制三类内容；关闭“响应”后不再缓存或提取响应文本，设置只影响新记录。
- 提示词记录开启“过滤预设”后，仅保留用户与助手的交互正文，移除 Claude 内部推理及签名，并可分别过滤用户的 AGENTS.md/CLAUDE.md 预设和已注册技能信息；两个子过滤开关默认开启。
- 完整请求体支持一键复制；图片、音频、视频和文件等多模态大文件内容始终不会写入提示词记录。
