...
- `force_priority`：将匹配请求强制设置为 `priority`，会按 Priority/Fast 价格计费。`all` 匹配所有 tier，包括省略 `service_tier` 的请求；`missing` 仅匹配省略该字段的请求。非 OpenAI 平台不会执行缺省 tier 注入。这种方式可以让请求实际使用 Fast，但 Codex UI 仍可能不显示 Fast 状态。
