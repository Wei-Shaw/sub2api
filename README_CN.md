...
- 管理后台从 `clansty/sub2api` 的 `clansty` 分支检查更新，不再使用官方 Sub2API 的版本结果。
...
官方发行版可以直接在管理后台进行二进制在线升级。本 Fork 使用源码构建，管理后台会读取 `clansty/sub2api` 的 `FORK_VERSION` 检测新版本，并提示使用 `git pull` 后重新构建；不会在线替换二进制。
