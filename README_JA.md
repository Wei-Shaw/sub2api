...
## Kiro チャネル対応（本 fork の追加機能）

本 fork は公式アップストリームに Kiro チャネル対応を取り込んでいます（出典:
[nianzs/sub2api](https://github.com/nianzs/sub2api)、アップストリームでクローズされた
[PR #5058](https://github.com/Wei-Shaw/sub2api/pull/5058) に相当）:

- OAuth / AWS Builder ID / トークンインポートによる Kiro アカウント、および API Key 互換の上流接続。
- Kiro トラフィック向けの Anthropic プロンプトキャッシュ利用量エミュレーション（グループ単位で切替・比率調整可）。
- Kiro のスティッキーセッション、credits 利用量集計、専用エラー分類。

連携の詳細、アップストリーム / fork の同期手順、既知の制約は [docs/KIRO.md](docs/KIRO.md) を参照。

> ⚠️ アップストリームは DMCA テイクダウンのリスクを理由に本コードの取り込みを明確に拒否しています。
> Kiro のリバースプロキシは AWS の利用規約に違反する可能性があり、リスクは利用者自身が負います。
