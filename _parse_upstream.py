...
with open(r"C:\Users\16790\xwechat_files\wxid_8tc8tfooo5rs22_fef8\msg\file\2026-04\asakifeng_consume.txt", 'r', encoding='utf-8') as f:
    for line in f:
        m = re.match(r'\[INFO\] (\d{4REDACTED/\d{2REDACTED/\d{2REDACTED - \d{2REDACTED:\d{2REDACTED:\d{2REDACTED) \|.*params=(\{.*\REDACTED)\s*$', line.strip())
        if not m: continue
        try: p = json.loads(m.group(2))
        except Exception: continue
        tn = p.get('token_name', '')
        model = p.get('model_name', '')
        other = p.get('other') or {REDACTED
...
OUR_PRICE = {
    'REDACTED':  {'input': 1e-6, 'output': 5e-6, 'cc5m': 1.25e-6, 'cr': 1e-7REDACTED,
    'claude-sonnet-4-6':          {'input': 3e-6, 'output': 1.5e-5, 'cc5m': 3.75e-6, 'cr': 3e-7REDACTED,
    'claude-sonnet-4-5-20250929': {'input': 3e-6, 'output': 1.5e-5, 'cc5m': 3.75e-6, 'cr': 3e-7REDACTED,
    'claude-opus-4-6':            {'input': 5e-6, 'output': 2.5e-5, 'cc5m': 6.25e-6, 'cr': 5e-7REDACTED,
    'claude-opus-4-5-20251101':   {'input': 5e-6, 'output': 2.5e-5, 'cc5m': 6.25e-6, 'cr': 5e-7REDACTED,
    'claude-opus-4-7':            {'input': 5e-6, 'output': 2.5e-5, 'cc5m': 6.25e-6, 'cr': 5e-7REDACTED,  # 我们回退到 opus-4-6 价
REDACTED
...
for (tn, model), d in sorted(by_key.items()):
    ...
    if d['flat_price_reqs']:
        note = f"flat_price {d['flat_price_reqs']REDACTED/{d['count']REDACTED"
