...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	balanceOperationRepository := repository.NewBalanceOperationRepository(client)
	openAPIService := service.NewOpenAPIService(userRepository, balanceOperationRepository, userService, apiKeyService, usageService)
	openAPIHandler := handler.NewOpenAPIHandler(openAPIService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, openAPIHandler, idempotencyCoordinator, idempotencyCleanupService)
