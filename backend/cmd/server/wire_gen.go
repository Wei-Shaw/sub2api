...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	privateGroupProvisioner := service.NewPrivateGroupProvisioner(groupRepository, userRepository, subscriptionService, settingService, billingCacheService)
	...
	authService := service.NewAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, privateGroupProvisioner)
	...
	privacyClientFactoryEarly := providePrivacyClientFactory()
	userAccountService := service.NewUserAccountService(accountRepository, groupRepository, privateGroupProvisioner, settingService, client, privacyClientFactoryEarly)
	...
	rateLimitService := service.ProvideRateLimitService(accountRepository, groupRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, settingService, compositeTokenCacheInvalidator)
	...
	oAuthRefreshAPI := service.ProvideOAuthRefreshAPI(accountRepository, groupRepository, geminiTokenCache)
	...
	shareRevenueRepo := repository.NewShareRevenueRepository(client)
	gatewayService.SetShareRevenueDeps(shareRevenueRepo, shareRevenueRepo)
	shareRevenueService := service.NewShareRevenueService(shareRevenueRepo, settingService)
	shareRevenueHandler := handler.NewShareRevenueHandler(shareRevenueService)
	openAIOAuthClient := repository.NewOpenAIOAuthClient()
	privacyClientFactory := privacyClientFactoryEarly
	...
	openAIGatewayService.SetShareRevenueDeps(shareRevenueRepo, shareRevenueRepo)
	...
	adminService := service.NewAdminService(userRepository, adminGroupRepository, adminAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, affiliateService, compositeModelRouteRepository, compositeRouteResolver, privateGroupProvisioner)
	// auditLogService 提前构造，供 User/Setting handler 强制审计写入
	auditLogRepository := repository.NewAuditLogRepository(db)
	auditLogService := service.ProvideAuditLogService(auditLogRepository, settingService)
	adminUserHandler := handler.ProvideAdminUserHandler(adminService, concurrencyService, serviceUserPlatformQuotaRepository, billingCache, totpService, userService, settingService, privateGroupProvisioner, auditLogService)
	...
	userAccountHandler.SetUsageService(accountUsageService)
	...
	userOAuthHandler := handler.NewUserOAuthHandler(settingService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService)
	...
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, privateGroupProvisioner, auditLogService)
