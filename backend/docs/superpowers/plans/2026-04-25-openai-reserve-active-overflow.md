...
- Modify: `backend/internal/service/openai_account_scheduler.go`
  - 调整 load-balance、sticky、previous_response 路径对 reserve 的接纳/拒绝逻辑。
...
Expected: FAIL。当前旧 binding 仍可能把 `affinity_domain=exhausted` 当成 active/any 下的拒绝理由。
...
```bash
git add backend/internal/service/openai_sticky_compat.go backend/internal/service/openai_ws_forwarder.go backend/internal/service/openai_ws_account_sticky_test.go backend/internal/service/openai_account_scheduler_test.go
...
```bash
C:\Users\34404\Documents\GitHub\workbench\toolchains\go\bin\go.exe test -tags unit ./internal/service -run "Test(SelectByLoadBalance_.*Reserve.*ProjectionMiss|PreviousResponseReserveStillFailsClosedOnUnknownModelMiss|StickyReserveStillFailsClosedOnCacheNotReady)" -count=1
```
...
```bash
git add backend/internal/service/openai_gateway_service_test.go backend/internal/service/openai_account_scheduler_test.go backend/internal/service/openai_ws_account_sticky_test.go backend/internal/service/openai_gateway_service.go backend/internal/service/openai_account_scheduler.go
...
Expected: FAIL，如果当前快照/观测仍把 active/any 命中的 reserve 写成 `selected_group=active` 或改写了 target。

- [ ] **Step 3: 最小实现观测修正**

```go
// 命中 reserve 的所有请求：selected_group=reserve
// 但 routing_target_group 保持请求原始语义
```
...
```bash
git add backend/internal/service/openai_routing_observability.go backend/internal/handler/openai_gateway_handler_test.go
...
```bash
C:\Users\34404\Documents\GitHub\workbench\toolchains\go\bin\go.exe test -tags unit ./internal/service -run "TestReserveSemanticShift_.*|TestOpenAIGatewayService_.*GPT55.*|TestSelectByLoadBalance_.*GPT55.*|Test(Sticky|PreviousResponse|WSContinuation).*Reserve.*" -count=1
```
