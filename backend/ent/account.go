...
type Account struct {
	...
	// UserBillingRateMultiplier holds the value of the "user_billing_rate_multiplier" field.
	UserBillingRateMultiplier *float64 `json:"user_billing_rate_multiplier,omitempty"`
	// UserBillingModelPricing holds the value of the "user_billing_model_pricing" field.
	UserBillingModelPricing jsontext.Value `json:"user_billing_model_pricing,omitempty"`
...
func (*Account) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case account.FieldCredentials, account.FieldExtra, account.FieldUserBillingModelPricing:
			values[i] = new([]byte)
		case account.FieldAutoPauseOnExpired, account.FieldSchedulable:
			values[i] = new(sql.NullBool)
		case account.FieldRateMultiplier, account.FieldUserBillingRateMultiplier:
...
func (_m *Account) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case account.FieldUserBillingRateMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field user_billing_rate_multiplier", values[i])
			} else if value.Valid {
				_m.UserBillingRateMultiplier = new(float64)
				*_m.UserBillingRateMultiplier = value.Float64
			}
		case account.FieldUserBillingModelPricing:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field user_billing_model_pricing", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.UserBillingModelPricing); err != nil {
					return fmt.Errorf("unmarshal field user_billing_model_pricing: %w", err)
				}
			}
...
func (_m *Account) String() string {
	...
	builder.WriteString(", ")
	if v := _m.UserBillingRateMultiplier; v != nil {
		builder.WriteString("user_billing_rate_multiplier=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	builder.WriteString("user_billing_model_pricing=")
	builder.WriteString(fmt.Sprintf("%v", _m.UserBillingModelPricing))
	builder.WriteString(", ")
