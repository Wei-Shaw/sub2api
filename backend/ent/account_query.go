...
func (_q *AccountQuery) QueryProxyGroup() *ProxyGroupQuery {
...
func (_q *AccountQuery) WithProxyGroup(opts ...func(*ProxyGroupQuery)) *AccountQuery {
	query := (&ProxyGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withProxyGroup; query != nil {
		if err := _q.loadProxyGroup(ctx, query, nodes, nil,
			func(n *Account, e *ProxyGroup) { n.Edges.ProxyGroup = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadProxyGroup(ctx context.Context, query *ProxyGroupQuery, nodes []*Account, init func(*Account), assign func(*Account, *ProxyGroup)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Account)
	for i := range nodes {
...
func (_q *AccountQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProxyGroup != nil {
			_spec.Node.AddColumnOnce(account.FieldProxyGroupID)
		}
