...
REDACTED
	if len(nodes) == 0 {
		return nil, &NotFoundError{announcement.LabelREDACTED
REDACTED
	return nodes[0], nil
REDACTED
...
REDACTED
	if len(ids) == 0 {
		err = &NotFoundError{announcement.LabelREDACTED
		return
REDACTED
	return ids[0], nil
REDACTED
...
REDACTED
	...
	case 0:
		return nil, &NotFoundError{announcement.LabelREDACTED
	default:
		return nil, &NotSingularError{announcement.LabelREDACTED
REDACTED
REDACTED
...
REDACTED
	...
	case 0:
		err = &NotFoundError{announcement.LabelREDACTED
	default:
		err = &NotSingularError{announcement.LabelREDACTED
REDACTED
	return
REDACTED
...
REDACTED
	return &AnnouncementQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]announcement.OrderOption{REDACTED, _q.order...),
		inters:     append([]Interceptor{REDACTED, _q.inters...),
		predicates: append([]predicate.Announcement{REDACTED, _q.predicates...),
...
func (_q *AnnouncementQuery) GroupBy(field string, fields ...string) *AnnouncementGroupBy {
	...
	grbuild := &AnnouncementGroupBy{build: _qREDACTED
...
func (_q *AnnouncementQuery) Select(fields ...string) *AnnouncementSelect {
	...
	sbuild := &AnnouncementSelect{AnnouncementQuery: _qREDACTED
...
REDACTED
	_spec.Assign = func(columns []string, values []any) error {
		node := &Announcement{config: _q.configREDACTED
...
REDACTED
	if query := _q.withReads; query != nil {
		if err := _q.loadReads(ctx, query, nodes,
			func(n *Announcement) { n.Edges.Reads = []*AnnouncementRead{REDACTED REDACTED,
			func(n *Announcement, e *AnnouncementRead) { n.Edges.Reads = append(n.Edges.Reads, e) REDACTED); err != nil {
			return nil, err
	REDACTED
REDACTED
	return nodes, nil
