...
func (_q *APIKeyQuery) QueryChatSessions() *ChatSessionQuery {
...
func (_q *APIKeyQuery) WithChatSessions(opts ...func(*ChatSessionQuery)) *APIKeyQuery {
	query := (&ChatSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withChatSessions; query != nil {
		if err := _q.loadChatSessions(ctx, query, nodes,
			func(n *APIKey) { n.Edges.ChatSessions = []*ChatSession{} },
			func(n *APIKey, e *ChatSession) { n.Edges.ChatSessions = append(n.Edges.ChatSessions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadChatSessions(ctx context.Context, query *ChatSessionQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *ChatSession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chatsession.FieldAPIKeyID)
	}
	query.Where(predicate.ChatSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.ChatSessionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
