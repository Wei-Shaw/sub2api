...
type APIKeyQuery struct {
	...
	withPlatformUsages *APIKeyPlatformUsageQuery
...
// QueryPlatformUsages chains the current query on the "platform_usages" edge.
func (_q *APIKeyQuery) QueryPlatformUsages() *APIKeyPlatformUsageQuery {
	query := (&APIKeyPlatformUsageClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withPlatformUsages: _q.withPlatformUsages.Clone(),
...
// WithPlatformUsages tells the query-builder to eager-load the nodes that are connected to
// the "platform_usages" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithPlatformUsages(opts ...func(*APIKeyPlatformUsageQuery)) *APIKeyQuery {
	query := (&APIKeyPlatformUsageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPlatformUsages = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withPlatformUsages; query != nil {
		if err := _q.loadPlatformUsages(ctx, query, nodes,
			func(n *APIKey) { n.Edges.PlatformUsages = []*APIKeyPlatformUsage{} },
			func(n *APIKey, e *APIKeyPlatformUsage) { n.Edges.PlatformUsages = append(n.Edges.PlatformUsages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadPlatformUsages(ctx context.Context, query *APIKeyPlatformUsageQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIKeyPlatformUsage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikeyplatformusage.FieldAPIKeyID)
	}
	query.Where(predicate.APIKeyPlatformUsage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.PlatformUsagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
