...
func (_q *APIKeyQuery) QueryBillingPool() *BillingPoolQuery {
...
func (_q *APIKeyQuery) WithBillingPool(opts ...func(*BillingPoolQuery)) *APIKeyQuery {
	query := (&BillingPoolClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withBillingPool; query != nil {
		if err := _q.loadBillingPool(ctx, query, nodes, nil,
			func(n *APIKey, e *BillingPool) { n.Edges.BillingPool = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadBillingPool(ctx context.Context, query *BillingPoolQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *BillingPool)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*APIKey)
	for i := range nodes {
		if nodes[i].BillingPoolID == nil {
			continue
		}
...
func (_q *APIKeyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withBillingPool != nil {
			_spec.Node.AddColumnOnce(apikey.FieldBillingPoolID)
		}
	}
