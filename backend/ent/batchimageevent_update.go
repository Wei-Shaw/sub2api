...
func (_u *BatchImageEventUpdate) SetPayload(v map[string]interface{REDACTED) *BatchImageEventUpdate {
...
func (_u *BatchImageEventUpdate) check() error {
	if v, ok := _u.mutation.JobID(); ok {
		if err := batchimageevent.JobIDValidator(v); err != nil {
			return &ValidationError{Name: "job_id", err: fmt.Errorf(`ent: validator failed for field "BatchImageEvent.job_id": %w`, err)REDACTED
	REDACTED
REDACTED
	if v, ok := _u.mutation.EventType(); ok {
		if err := batchimageevent.EventTypeValidator(v); err != nil {
			return &ValidationError{Name: "event_type", err: fmt.Errorf(`ent: validator failed for field "BatchImageEvent.event_type": %w`, err)REDACTED
	REDACTED
REDACTED
	if v, ok := _u.mutation.EventHash(); ok {
		if err := batchimageevent.EventHashValidator(v); err != nil {
			return &ValidationError{Name: "event_hash", err: fmt.Errorf(`ent: validator failed for field "BatchImageEvent.event_hash": %w`, err)REDACTED
	REDACTED
REDACTED
	return nil
REDACTED
...
REDACTED
	if _node, err = sqlgraph.UpdateNodes(ctx, _u.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{batchimageevent.LabelREDACTED
...
// SetPayload sets the "payload" field.
func (_u *BatchImageEventUpdateOne) SetPayload(v map[string]interface{REDACTED) *BatchImageEventUpdateOne {
	_u.mutation.SetPayload(v)
	return _u
REDACTED
...
REDACTED
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "BatchImageEvent.id" for update`)REDACTED
REDACTED
...
REDACTED
	if _u.mutation.EventHashCleared() {
		_spec.ClearField(batchimageevent.FieldEventHash, field.TypeString)
REDACTED
	_node = &BatchImageEvent{config: _u.configREDACTED
