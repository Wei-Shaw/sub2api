...
func (_q *ChatMessageQuery) QueryUsageLog() *UsageLogQuery {
...
func (_q *ChatMessageQuery) WithUsageLog(opts ...func(*UsageLogQuery)) *ChatMessageQuery {
	query := (&UsageLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ChatMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ChatMessage, error) {
	...
	if query := _q.withUsageLog; query != nil {
		if err := _q.loadUsageLog(ctx, query, nodes, nil,
			func(n *ChatMessage, e *UsageLog) { n.Edges.UsageLog = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChatMessageQuery) loadUsageLog(ctx context.Context, query *UsageLogQuery, nodes []*ChatMessage, init func(*ChatMessage), assign func(*ChatMessage, *UsageLog)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*ChatMessage)
	for i := range nodes {
...
func (_q *ChatMessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUsageLog != nil {
			_spec.Node.AddColumnOnce(chatmessage.FieldUsageLogID)
		}
	}
