...
func (c *CustomDomainUserClient) QueryCustomDomain(_m *CustomDomainUser) *CustomDomainQuery {
	return c.Query().
		Where(customdomainuser.UserID(_m.UserID), customdomainuser.CustomDomainID(_m.CustomDomainID)).
		QueryCustomDomain()
}
...
func (c *UserClient) QueryAuthorizedCustomDomains(_m *User) *CustomDomainQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(customdomain.Table, customdomain.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, user.AuthorizedCustomDomainsTable, user.AuthorizedCustomDomainsPrimaryKey...),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
