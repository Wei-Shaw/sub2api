...
type Group struct {
	...
	// Whether this group accepts matching public user-owned accounts as a share pool.
	IsSharePool bool `json:"is_share_pool,omitempty"`
	...
	// 上游订阅档位 code，来自系统设置 group_upstream_plans
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldName, group.FieldDescription, group.FieldPeakStart, group.FieldPeakEnd, group.FieldStatus, group.FieldDuplicateOperationID, group.FieldPlatform, group.FieldUpstreamPlan, group.FieldSubscriptionType, group.FieldDefaultMappedModel, group.FieldMaxReasoningEffort:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldIsSharePool:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_share_pool", values[i])
			} else if value.Valid {
				_m.IsSharePool = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("is_share_pool=")
	builder.WriteString(fmt.Sprintf("%v", _m.IsSharePool))
	builder.WriteString(", ")
