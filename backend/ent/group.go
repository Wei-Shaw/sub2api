...
type Group struct {
	...
	// Group request routing mode: fixed or auto_lowest_cost
	RoutingMode string `json:"routing_mode,omitempty"`
	// Candidate balance groups for auto_lowest_cost routing
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldAutoCandidateGroupIds, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
		...
		case group.FieldName, group.FieldDescription, group.FieldPeakStart, group.FieldPeakEnd, group.FieldStatus, group.FieldDuplicateOperationID, group.FieldRoutingMode, group.FieldPlatform, group.FieldSubscriptionType, group.FieldDefaultMappedModel, group.FieldMaxReasoningEffort:
	...
	for i := range columns {
		...
		case group.FieldAutoCandidateGroupIds:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field auto_candidate_group_ids", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.AutoCandidateGroupIds); err != nil {
					return fmt.Errorf("unmarshal field auto_candidate_group_ids: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("auto_candidate_group_ids=")
	builder.WriteString(fmt.Sprintf("%v", _m.AutoCandidateGroupIds))
	builder.WriteString(", ")
