...
type Group struct {
	...
	// 智能路由成员分组配置：group_id/priority/weight
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldReasoningEffortMappings, group.FieldSmartRoutingMembers:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldSmartRoutingMembers:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field smart_routing_members", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.SmartRoutingMembers); err != nil {
					return fmt.Errorf("unmarshal field smart_routing_members: %w", err)
				}
			}
		default:
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("smart_routing_members=")
	builder.WriteString(fmt.Sprintf("%v", _m.SmartRoutingMembers))
	builder.WriteByte(')')
