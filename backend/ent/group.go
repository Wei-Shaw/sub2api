...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldGroupExtra:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldGroupExtra:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field group_extra", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.GroupExtra); err != nil {
					return fmt.Errorf("unmarshal field group_extra: %w", err)
				}
			}
		default:
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("group_extra=")
	builder.WriteString(fmt.Sprintf("%v", _m.GroupExtra))
	builder.WriteByte(')')
