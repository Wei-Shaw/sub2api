...
type GroupQuery struct {
	...
	withPoolGrants        *UserPoolGroupGrantQuery
...
func (_q *GroupQuery) QueryUserPools() *UserPoolQuery {
...
// QueryPoolGrants chains the current query on the "pool_grants" edge.
func (_q *GroupQuery) QueryPoolGrants() *UserPoolGroupGrantQuery {
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withPoolGrants:        _q.withPoolGrants.Clone(),
...
func (_q *GroupQuery) WithUserPools(opts ...func(*UserPoolQuery)) *GroupQuery {
	query := (&UserPoolClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithPoolGrants tells the query-builder to eager-load the nodes that are connected to
// the "pool_grants" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithPoolGrants(opts ...func(*UserPoolGroupGrantQuery)) *GroupQuery {
	query := (&UserPoolGroupGrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPoolGrants = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withUserPools; query != nil {
		if err := _q.loadUserPools(ctx, query, nodes,
			func(n *Group) { n.Edges.UserPools = []*UserPool{} },
			func(n *Group, e *UserPool) { n.Edges.UserPools = append(n.Edges.UserPools, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withPoolGrants; query != nil {
		if err := _q.loadPoolGrants(ctx, query, nodes,
			func(n *Group) { n.Edges.PoolGrants = []*UserPoolGroupGrant{} },
			func(n *Group, e *UserPoolGroupGrant) { n.Edges.PoolGrants = append(n.Edges.PoolGrants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadUserPools(ctx context.Context, query *UserPoolQuery, nodes []*Group, init func(*Group), assign func(*Group, *UserPool)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Group)
	nids := make(map[int64]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.UserPoolsTable)
		s.Join(joinT).On(s.C(userpool.FieldID), joinT.C(group.UserPoolsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.UserPoolsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.UserPoolsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *GroupQuery) loadPoolGrants(ctx context.Context, query *UserPoolGroupGrantQuery, nodes []*Group, init func(*Group), assign func(*Group, *UserPoolGroupGrant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userpoolgroupgrant.FieldGroupID)
	}
	query.Where(predicate.UserPoolGroupGrant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.PoolGrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
