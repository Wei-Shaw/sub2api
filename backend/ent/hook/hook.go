...
// The APIKeyPlatformUsageFunc type is an adapter to allow the use of ordinary
// function as APIKeyPlatformUsage mutator.
type APIKeyPlatformUsageFunc func(context.Context, *ent.APIKeyPlatformUsageMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f APIKeyPlatformUsageFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.APIKeyPlatformUsageMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.APIKeyPlatformUsageMutation", m)
}
