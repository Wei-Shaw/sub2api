...
// The UserUsageLimitRuleFunc type is an adapter to allow the use of ordinary
// function as UserUsageLimitRule mutator.
type UserUsageLimitRuleFunc func(context.Context, *ent.UserUsageLimitRuleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserUsageLimitRuleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserUsageLimitRuleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserUsageLimitRuleMutation", m)
}
