...
REDACTED
	if _node, err = sqlgraph.UpdateNodes(ctx, _u.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{idempotencyrecord.LabelREDACTED
...
REDACTED
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "IdempotencyRecord.id" for update`)REDACTED
REDACTED
...
REDACTED
	if value, ok := _u.mutation.ExpiresAt(); ok {
		_spec.SetField(idempotencyrecord.FieldExpiresAt, field.TypeTime, value)
REDACTED
	_node = &IdempotencyRecord{config: _u.configREDACTED
