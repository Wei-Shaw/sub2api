...
// The AccountWindowUsageHistoryFunc type is an adapter to allow the use of ordinary function as a Querier.
type AccountWindowUsageHistoryFunc func(context.Context, *ent.AccountWindowUsageHistoryQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AccountWindowUsageHistoryFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AccountWindowUsageHistoryQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AccountWindowUsageHistoryQuery", q)
}

// The TraverseAccountWindowUsageHistory type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAccountWindowUsageHistory func(context.Context, *ent.AccountWindowUsageHistoryQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAccountWindowUsageHistory) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAccountWindowUsageHistory) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AccountWindowUsageHistoryQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AccountWindowUsageHistoryQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AccountWindowUsageHistoryQuery:
		return &query[*ent.AccountWindowUsageHistoryQuery, predicate.AccountWindowUsageHistory, accountwindowusagehistory.OrderOption]{typ: ent.TypeAccountWindowUsageHistory, tq: q}, nil
