...
// The BalanceOperationFunc type is an adapter to allow the use of ordinary function as a Querier.
type BalanceOperationFunc func(context.Context, *ent.BalanceOperationQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f BalanceOperationFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.BalanceOperationQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.BalanceOperationQuery", q)
}

// The TraverseBalanceOperation type is an adapter to allow the use of ordinary function as Traverser.
type TraverseBalanceOperation func(context.Context, *ent.BalanceOperationQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseBalanceOperation) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseBalanceOperation) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.BalanceOperationQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.BalanceOperationQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.BalanceOperationQuery:
		return &query[*ent.BalanceOperationQuery, predicate.BalanceOperation, balanceoperation.OrderOption]{typ: ent.TypeBalanceOperation, tq: q}, nil
