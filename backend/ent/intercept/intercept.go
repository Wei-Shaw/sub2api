...
// The APIKeyPlatformUsageFunc type is an adapter to allow the use of ordinary function as a Querier.
type APIKeyPlatformUsageFunc func(context.Context, *ent.APIKeyPlatformUsageQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f APIKeyPlatformUsageFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.APIKeyPlatformUsageQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.APIKeyPlatformUsageQuery", q)
}

// The TraverseAPIKeyPlatformUsage type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAPIKeyPlatformUsage func(context.Context, *ent.APIKeyPlatformUsageQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAPIKeyPlatformUsage) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAPIKeyPlatformUsage) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.APIKeyPlatformUsageQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.APIKeyPlatformUsageQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.APIKeyPlatformUsageQuery:
		return &query[*ent.APIKeyPlatformUsageQuery, predicate.APIKeyPlatformUsage, apikeyplatformusage.OrderOption]{typ: ent.TypeAPIKeyPlatformUsage, tq: q}, nil
