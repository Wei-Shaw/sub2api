...
// The CompositeRouteSchemeFunc type is an adapter to allow the use of ordinary function as a Querier.
type CompositeRouteSchemeFunc func(context.Context, *ent.CompositeRouteSchemeQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f CompositeRouteSchemeFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.CompositeRouteSchemeQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.CompositeRouteSchemeQuery", q)
}

// The TraverseCompositeRouteScheme type is an adapter to allow the use of ordinary function as Traverser.
type TraverseCompositeRouteScheme func(context.Context, *ent.CompositeRouteSchemeQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseCompositeRouteScheme) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseCompositeRouteScheme) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.CompositeRouteSchemeQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.CompositeRouteSchemeQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.CompositeRouteSchemeQuery:
		return &query[*ent.CompositeRouteSchemeQuery, predicate.CompositeRouteScheme, compositeroutescheme.OrderOption]{typ: ent.TypeCompositeRouteScheme, tq: q}, nil
