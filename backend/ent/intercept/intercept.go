...
// The ProxyGroupFunc type is an adapter to allow the use of ordinary function as a Querier.
type ProxyGroupFunc func(context.Context, *ent.ProxyGroupQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ProxyGroupFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ProxyGroupQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ProxyGroupQuery", q)
}

// The TraverseProxyGroup type is an adapter to allow the use of ordinary function as Traverser.
type TraverseProxyGroup func(context.Context, *ent.ProxyGroupQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseProxyGroup) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseProxyGroup) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ProxyGroupQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ProxyGroupQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ProxyGroupQuery:
		return &query[*ent.ProxyGroupQuery, predicate.ProxyGroup, proxygroup.OrderOption]{typ: ent.TypeProxyGroup, tq: q}, nil
