...
REDACTED
	if _node, err = sqlgraph.UpdateNodes(ctx, _u.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{paymentauditlog.LabelREDACTED
...
REDACTED
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "PaymentAuditLog.id" for update`)REDACTED
REDACTED
...
REDACTED
	if value, ok := _u.mutation.Operator(); ok {
		_spec.SetField(paymentauditlog.FieldOperator, field.TypeString, value)
REDACTED
	_node = &PaymentAuditLog{config: _u.configREDACTED
