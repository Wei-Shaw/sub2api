...
REDACTED
	if len(nodes) == 0 {
		return nil, &NotFoundError{paymentorder.LabelREDACTED
REDACTED
	return nodes[0], nil
REDACTED
...
REDACTED
	...
	case 0:
		return nil, &NotFoundError{paymentorder.LabelREDACTED
	default:
		return nil, &NotSingularError{paymentorder.LabelREDACTED
REDACTED
REDACTED
...
REDACTED
	...
	case 0:
		err = &NotFoundError{paymentorder.LabelREDACTED
	default:
		err = &NotSingularError{paymentorder.LabelREDACTED
REDACTED
	return
REDACTED
...
REDACTED
	return &PaymentOrderQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]paymentorder.OrderOption{REDACTED, _q.order...),
		inters:     append([]Interceptor{REDACTED, _q.inters...),
		predicates: append([]predicate.PaymentOrder{REDACTED, _q.predicates...),
...
func (_q *PaymentOrderQuery) GroupBy(field string, fields ...string) *PaymentOrderGroupBy {
	...
	grbuild := &PaymentOrderGroupBy{build: _qREDACTED
...
func (_q *PaymentOrderQuery) Select(fields ...string) *PaymentOrderSelect {
	...
	sbuild := &PaymentOrderSelect{PaymentOrderQuery: _qREDACTED
...
REDACTED
	_spec.Assign = func(columns []string, values []any) error {
		node := &PaymentOrder{config: _q.configREDACTED
...
REDACTED
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *PaymentOrder, e *User) { n.Edges.User = e REDACTED); err != nil {
			return nil, err
	REDACTED
REDACTED
	return nodes, nil
