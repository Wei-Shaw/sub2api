...
REDACTED
	if len(nodes) == 0 {
		return nil, &NotFoundError{paymentproviderinstance.LabelREDACTED
REDACTED
	return nodes[0], nil
REDACTED
...
REDACTED
	if len(ids) == 0 {
		err = &NotFoundError{paymentproviderinstance.LabelREDACTED
		return
REDACTED
	return ids[0], nil
REDACTED
...
REDACTED
	...
	case 0:
		return nil, &NotFoundError{paymentproviderinstance.LabelREDACTED
	default:
		return nil, &NotSingularError{paymentproviderinstance.LabelREDACTED
REDACTED
REDACTED
...
REDACTED
	...
	case 0:
		err = &NotFoundError{paymentproviderinstance.LabelREDACTED
	default:
		err = &NotSingularError{paymentproviderinstance.LabelREDACTED
REDACTED
	return
REDACTED
...
REDACTED
	return &PaymentProviderInstanceQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]paymentproviderinstance.OrderOption{REDACTED, _q.order...),
		inters:     append([]Interceptor{REDACTED, _q.inters...),
		predicates: append([]predicate.PaymentProviderInstance{REDACTED, _q.predicates...),
...
func (_q *PaymentProviderInstanceQuery) GroupBy(field string, fields ...string) *PaymentProviderInstanceGroupBy {
	...
	grbuild := &PaymentProviderInstanceGroupBy{build: _qREDACTED
...
func (_q *PaymentProviderInstanceQuery) Select(fields ...string) *PaymentProviderInstanceSelect {
	...
	sbuild := &PaymentProviderInstanceSelect{PaymentProviderInstanceQuery: _qREDACTED
...
func (_q *PaymentProviderInstanceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentProviderInstance, error) {
	var (
		nodes = []*PaymentProviderInstance{REDACTED
		_spec = _q.querySpec()
	)
...
REDACTED
	_spec.Assign = func(columns []string, values []any) error {
		node := &PaymentProviderInstance{config: _q.configREDACTED
