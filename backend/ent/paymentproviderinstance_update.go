...
REDACTED
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "PaymentProviderInstance.id" for update`)REDACTED
REDACTED
...
REDACTED
	if value, ok := _u.mutation.UpdatedAt(); ok {
		_spec.SetField(paymentproviderinstance.FieldUpdatedAt, field.TypeTime, value)
REDACTED
	_node = &PaymentProviderInstance{config: _u.configREDACTED
