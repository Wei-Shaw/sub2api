...
func (_q *PendingAuthSessionQuery) QueryAdoptionDecision() *IdentityAdoptionDecisionQuery {
	query := (&IdentityAdoptionDecisionClient{config: _q.configREDACTED).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
	REDACTED
...
REDACTED
	if len(nodes) == 0 {
		return nil, &NotFoundError{pendingauthsession.LabelREDACTED
REDACTED
	return nodes[0], nil
REDACTED
...
REDACTED
	...
	case 0:
		return nil, &NotFoundError{pendingauthsession.LabelREDACTED
	default:
		return nil, &NotSingularError{pendingauthsession.LabelREDACTED
REDACTED
REDACTED
...
REDACTED
	...
	case 0:
		err = &NotFoundError{pendingauthsession.LabelREDACTED
	default:
		err = &NotSingularError{pendingauthsession.LabelREDACTED
REDACTED
	return
REDACTED
...
REDACTED
	return &PendingAuthSessionQuery{
		config:               _q.config,
		ctx:                  _q.ctx.Clone(),
		order:                append([]pendingauthsession.OrderOption{REDACTED, _q.order...),
		inters:               append([]Interceptor{REDACTED, _q.inters...),
		predicates:           append([]predicate.PendingAuthSession{REDACTED, _q.predicates...),
...
func (_q *PendingAuthSessionQuery) GroupBy(field string, fields ...string) *PendingAuthSessionGroupBy {
	...
	grbuild := &PendingAuthSessionGroupBy{build: _qREDACTED
...
func (_q *PendingAuthSessionQuery) Select(fields ...string) *PendingAuthSessionSelect {
	...
	sbuild := &PendingAuthSessionSelect{PendingAuthSessionQuery: _qREDACTED
...
func (_q *PendingAuthSessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PendingAuthSession, error) {
	var (
		nodes       = []*PendingAuthSession{REDACTED
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
...
REDACTED
	_spec.Assign = func(columns []string, values []any) error {
		node := &PendingAuthSession{config: _q.configREDACTED
...
REDACTED
	if query := _q.withTargetUser; query != nil {
		if err := _q.loadTargetUser(ctx, query, nodes, nil,
			func(n *PendingAuthSession, e *User) { n.Edges.TargetUser = e REDACTED); err != nil {
			return nil, err
	REDACTED
REDACTED
	if query := _q.withAdoptionDecision; query != nil {
		if err := _q.loadAdoptionDecision(ctx, query, nodes, nil,
			func(n *PendingAuthSession, e *IdentityAdoptionDecision) { n.Edges.AdoptionDecision = e REDACTED); err != nil {
			return nil, err
	REDACTED
REDACTED
	return nodes, nil
