...
REDACTED
	if len(nodes) == 0 {
		return nil, &NotFoundError{promocode.LabelREDACTED
REDACTED
	return nodes[0], nil
REDACTED
...
REDACTED
	if len(ids) == 0 {
		err = &NotFoundError{promocode.LabelREDACTED
		return
REDACTED
	return ids[0], nil
REDACTED
...
REDACTED
	...
	case 0:
		return nil, &NotFoundError{promocode.LabelREDACTED
	default:
		return nil, &NotSingularError{promocode.LabelREDACTED
REDACTED
REDACTED
...
REDACTED
	...
	case 0:
		err = &NotFoundError{promocode.LabelREDACTED
	default:
		err = &NotSingularError{promocode.LabelREDACTED
REDACTED
	return
REDACTED
...
REDACTED
	return &PromoCodeQuery{
		config:           _q.config,
		ctx:              _q.ctx.Clone(),
		order:            append([]promocode.OrderOption{REDACTED, _q.order...),
		inters:           append([]Interceptor{REDACTED, _q.inters...),
		predicates:       append([]predicate.PromoCode{REDACTED, _q.predicates...),
...
func (_q *PromoCodeQuery) GroupBy(field string, fields ...string) *PromoCodeGroupBy {
	...
	grbuild := &PromoCodeGroupBy{build: _qREDACTED
...
func (_q *PromoCodeQuery) Select(fields ...string) *PromoCodeSelect {
	...
	sbuild := &PromoCodeSelect{PromoCodeQuery: _qREDACTED
...
REDACTED
	_spec.Assign = func(columns []string, values []any) error {
		node := &PromoCode{config: _q.configREDACTED
...
REDACTED
	if query := _q.withUsageRecords; query != nil {
		if err := _q.loadUsageRecords(ctx, query, nodes,
			func(n *PromoCode) { n.Edges.UsageRecords = []*PromoCodeUsage{REDACTED REDACTED,
			func(n *PromoCode, e *PromoCodeUsage) { n.Edges.UsageRecords = append(n.Edges.UsageRecords, e) REDACTED); err != nil {
			return nil, err
	REDACTED
REDACTED
	return nodes, nil
