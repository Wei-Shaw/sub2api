...
func (Account) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "accounts"REDACTED,
REDACTED
REDACTED
...
func (Account) Fields() []ent.Field {
	return []ent.Field{
		...
		// 结构取决于 type 字段：
		// - api_key: {"api_key": "sk-xxx"REDACTED
		// - oauth: {"access_token": "...", "refresh_token": "...", "expires_at": "..."REDACTED
		// - cookie: {"session_key": "..."REDACTED
		field.JSON("credentials", map[string]any{REDACTED).
		...
		field.JSON("extra", map[string]any{REDACTED).
			Default(func() map[string]any { return map[string]any{REDACTED REDACTED).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED),
