...
func (BatchImageItem) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "batch_image_items"REDACTED,
REDACTED
REDACTED
...
func (BatchImageItem) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("prompt_preview").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "text"REDACTED),
		...
		field.String("error_message").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "text"REDACTED),
		field.Float("billed_amount").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"REDACTED),
		field.Time("created_at").Immutable().Default(time.Now).SchemaType(map[string]string{dialect.Postgres: "timestamptz"REDACTED),
		field.Time("indexed_at").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "timestamptz"REDACTED),
REDACTED
REDACTED
