...
func (ChannelMonitor) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "channel_monitors"REDACTED,
REDACTED
REDACTED
...
func (ChannelMonitor) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("primary_model").
			NotEmpty().
			MaxLen(200),
		field.JSON("extra_models", []string{REDACTED).
		...
		field.JSON("extra_headers", map[string]string{REDACTED).
			Default(map[string]string{REDACTED),
		...
		field.JSON("body_override", map[string]any{REDACTED).
			Optional(),
REDACTED
REDACTED
