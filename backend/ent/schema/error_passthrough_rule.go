...
func (ErrorPassthroughRule) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "error_passthrough_rules"REDACTED,
REDACTED
REDACTED
...
func (ErrorPassthroughRule) Fields() []ent.Field {
	return []ent.Field{
		...
		field.JSON("error_codes", []int{REDACTED).
			Optional().
		...
		// 关键词匹配不区分大小写
		field.JSON("keywords", []string{REDACTED).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED),
		...
		// 空列表表示适用于所有平台
		field.JSON("platforms", []string{REDACTED).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED),
