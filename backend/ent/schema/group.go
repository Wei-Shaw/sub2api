...
func (Group) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "groups"REDACTED,
REDACTED
REDACTED
...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("rate_multiplier").
			SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"REDACTED).
			Default(1.0),
		...
		field.Float("video_price_1080p").
			...
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,8)"REDACTED),
		field.JSON("video_model_prices", map[string]map[string]float64{REDACTED).
			Optional().
		...
		field.JSON("model_pricing", json.RawMessage{REDACTED).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED).
		...
		field.JSON("model_routing", map[string][]int64{REDACTED).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED).
		...
		field.JSON("supported_model_scopes", []string{REDACTED).
			Default([]string{"claude", "gemini_text", "gemini_image"REDACTED).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED).
		...
		field.JSON("messages_dispatch_model_config", domain.OpenAIMessagesDispatchModelConfig{REDACTED).
			Default(domain.OpenAIMessagesDispatchModelConfig{REDACTED).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED).
			Comment("OpenAI Messages 调度模型配置：按 Claude 系列/精确模型映射到目标 GPT 模型"),
		field.JSON("models_list_config", domain.GroupModelsListConfig{REDACTED).
			Default(domain.GroupModelsListConfig{REDACTED).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED).
		...
		field.JSON("reasoning_effort_mappings", []domain.ReasoningEffortMapping{REDACTED).
			Default([]domain.ReasoningEffortMapping{REDACTED).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED).
