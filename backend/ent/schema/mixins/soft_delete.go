...
type softDeleteKey struct{REDACTED
...
func SkipSoftDelete(parent context.Context) context.Context {
	return context.WithValue(parent, softDeleteKey{REDACTED, true)
REDACTED
...
func (d SoftDeleteMixin) Interceptors() []ent.Interceptor {
	return []ent.Interceptor{
		intercept.TraverseFunc(func(ctx context.Context, q intercept.Query) error {
			// 检查是否需要跳过软删除过滤
			if skip, _ := ctx.Value(softDeleteKey{REDACTED).(bool); skip {
				return nil
		REDACTED
...
// 这是软删除过滤的核心实现。
func (d SoftDeleteMixin) applyPredicate(w interface{ WhereP(...func(*sql.Selector)) REDACTED) {
	w.WhereP(
...
REDACTED
	...
	results := mutateMethod.Call([]reflect.Value{reflect.ValueOf(ctx), reflect.ValueOf(m)REDACTED)
