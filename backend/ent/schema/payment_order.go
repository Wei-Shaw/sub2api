...
func (PaymentOrder) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "payment_orders"REDACTED,
REDACTED
REDACTED
...
func (PaymentOrder) Fields() []ent.Field {
	return []ent.Field{
		...
		// 金额信息
		field.Float("amount").
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,2)"REDACTED),
		...
		field.String("provider_key").
			...
			Nillable().
			MaxLen(30),
		field.JSON("provider_snapshot", map[string]any{REDACTED).
			Optional().
