...
func (UserAttributeDefinition) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "user_attribute_definitions"REDACTED,
REDACTED
REDACTED
...
func (UserAttributeDefinition) Fields() []ent.Field {
	return []ent.Field{
		...
		// Format: [{"value": "xxx", "label": "XXX"REDACTED, ...]
		field.JSON("options", []map[string]any{REDACTED).
			Default([]map[string]any{REDACTED).
		...
		// Format: {"min_length": 1, "max_length": 100, "min": 0, "max": 100, "pattern": "^[a-z]+$", "message": "..."REDACTED
		field.JSON("validation", map[string]any{REDACTED).
			Default(map[string]any{REDACTED).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"REDACTED),
