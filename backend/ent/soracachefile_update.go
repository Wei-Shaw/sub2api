...
func (_u *SoraCacheFileUpdate) check() error {
	if v, ok := _u.mutation.TaskID(); ok {
		if err := soracachefile.TaskIDValidator(v); err != nil {
			return &ValidationError{Name: "task_id", err: fmt.Errorf(`ent: validator failed for field "SoraCacheFile.task_id": %w`, err)REDACTED
	REDACTED
REDACTED
	if v, ok := _u.mutation.MediaType(); ok {
		if err := soracachefile.MediaTypeValidator(v); err != nil {
			return &ValidationError{Name: "media_type", err: fmt.Errorf(`ent: validator failed for field "SoraCacheFile.media_type": %w`, err)REDACTED
	REDACTED
REDACTED
	return nil
REDACTED
...
REDACTED
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "SoraCacheFile.id" for update`)REDACTED
REDACTED
...
REDACTED
	if value, ok := _u.mutation.CreatedAt(); ok {
		_spec.SetField(soracachefile.FieldCreatedAt, field.TypeTime, value)
REDACTED
	_node = &SoraCacheFile{config: _u.configREDACTED
