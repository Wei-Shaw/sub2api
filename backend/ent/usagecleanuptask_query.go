...
REDACTED
	if len(nodes) == 0 {
		return nil, &NotFoundError{usagecleanuptask.LabelREDACTED
REDACTED
	return nodes[0], nil
REDACTED
...
REDACTED
	if len(ids) == 0 {
		err = &NotFoundError{usagecleanuptask.LabelREDACTED
		return
REDACTED
	return ids[0], nil
REDACTED
...
REDACTED
	...
	case 0:
		return nil, &NotFoundError{usagecleanuptask.LabelREDACTED
	default:
		return nil, &NotSingularError{usagecleanuptask.LabelREDACTED
REDACTED
REDACTED
...
REDACTED
	...
	case 0:
		err = &NotFoundError{usagecleanuptask.LabelREDACTED
	default:
		err = &NotSingularError{usagecleanuptask.LabelREDACTED
REDACTED
	return
REDACTED
...
REDACTED
	return &UsageCleanupTaskQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]usagecleanuptask.OrderOption{REDACTED, _q.order...),
		inters:     append([]Interceptor{REDACTED, _q.inters...),
		predicates: append([]predicate.UsageCleanupTask{REDACTED, _q.predicates...),
...
func (_q *UsageCleanupTaskQuery) GroupBy(field string, fields ...string) *UsageCleanupTaskGroupBy {
	...
	grbuild := &UsageCleanupTaskGroupBy{build: _qREDACTED
...
func (_q *UsageCleanupTaskQuery) Select(fields ...string) *UsageCleanupTaskSelect {
	...
	sbuild := &UsageCleanupTaskSelect{UsageCleanupTaskQuery: _qREDACTED
...
func (_q *UsageCleanupTaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageCleanupTask, error) {
	var (
		nodes = []*UsageCleanupTask{REDACTED
		_spec = _q.querySpec()
	)
...
REDACTED
	_spec.Assign = func(columns []string, values []any) error {
		node := &UsageCleanupTask{config: _q.configREDACTED
