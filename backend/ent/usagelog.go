...
type UsageLog struct {
	...
	// CachePolicyTrace holds the value of the "cache_policy_trace" field.
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldCachePolicyTrace:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field cache_policy_trace", values[i])
			} else if value.Valid {
				_m.CachePolicyTrace = new(string)
				*_m.CachePolicyTrace = value.String
			}
		case usagelog.FieldCreatedAt:
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.CachePolicyTrace; v != nil {
		builder.WriteString("cache_policy_trace=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")
