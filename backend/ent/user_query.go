...
func (_q *UserQuery) loadAuthorizedCustomDomains(ctx context.Context, query *CustomDomainQuery, nodes []*User, init func(*User), assign func(*User, *CustomDomain)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(customdomain.FieldID), joinT.C(user.AuthorizedCustomDomainsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.AuthorizedCustomDomainsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.AuthorizedCustomDomainsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
