...
REDACTED
	if _node, err = sqlgraph.UpdateNodes(ctx, _u.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{userallowedgroup.LabelREDACTED
...
REDACTED
	...
	if id, ok := _u.mutation.UserID(); !ok {
		return nil, &ValidationError{Name: "user_id", err: errors.New(`ent: missing "UserAllowedGroup.user_id" for update`)REDACTED
REDACTED else {
		_spec.Node.CompositeID[0].Value = id
REDACTED
	if id, ok := _u.mutation.GroupID(); !ok {
		return nil, &ValidationError{Name: "group_id", err: errors.New(`ent: missing "UserAllowedGroup.group_id" for update`)REDACTED
REDACTED else {
...
REDACTED
	...
	REDACTED
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
REDACTED
	_node = &UserAllowedGroup{config: _u.configREDACTED
