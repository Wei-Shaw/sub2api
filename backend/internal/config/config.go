...
func NormalizeForwardedClientIPHeaders(headers []string) ([]string, error) {
	...
	seen := make(map[string]struct{REDACTED, len(headers))
...
func (c *Config) ForwardedClientIPSettings() ForwardedClientIPSettings {
	if c == nil {
		return ForwardedClientIPSettings{Headers: []string{REDACTEDREDACTED
REDACTED
...
REDACTED
	...
	if c.Security.forwardedClientIPSettingsLive == nil {
		c.Security.forwardedClientIPSettingsLive = &atomic.Pointer[ForwardedClientIPSettings]{REDACTED
REDACTED
...
type GatewayConfig struct {
	...
	// 模板渲染后会直接覆盖最终 instructions；若需要保留客户端 system 转换结果，请在模板中显式引用 {{ .ExistingInstructions REDACTEDREDACTED。
...
REDACTED
	...
	if m := cfg.Gateway.UserMessageQueue.Mode; m != "" && m != UMQModeSerialize && m != UMQModeThrottle {
		slog.Warn("invalid user_message_queue mode, disabling",
			"mode", m,
			"valid_modes", []string{UMQModeSerialize, UMQModeThrottleREDACTED)
...
func setDefaults() {
	...
	// CORS
	viper.SetDefault("cors.allowed_origins", []string{REDACTED)
	...
	viper.SetDefault("webauthn.rp_origins", []string{REDACTED)
...
REDACTED)
	viper.SetDefault("security.url_allowlist.crs_hosts", []string{REDACTED)
	...
	viper.SetDefault("security.response_headers.additional_allowed", []string{REDACTED)
	viper.SetDefault("security.response_headers.force_remove", []string{REDACTED)
	...
	viper.SetDefault("batch_image.vertex_managed_gcs_prefix", "batch-image/{envREDACTED/{batch_idREDACTED")
	...
	viper.SetDefault("plugins.trusted_publishers", map[string]string{REDACTED)
...
func setEnvReachableDefaults() {
	...
	for _, provider := range []string{"github_oauth", "google_oauth"REDACTED {
...
REDACTED
	...
	viper.SetDefault("dingtalk_connect.attribute_sync_fields", []string{REDACTED)
...
REDACTED
	...
	REDACTED
		if !strings.Contains(c.BatchImage.VertexManagedGCSPrefix, "{batch_idREDACTED") {
			return fmt.Errorf("batch_image.vertex_managed_gcs_prefix must contain {batch_idREDACTED")
	REDACTED
...
func isWeakJWTSecret(secret string) bool {
	...
	if lower == "" {
		return true
REDACTED
	weak := map[string]struct{REDACTED{
		"change-me-in-production": {REDACTED,
		"changeme":                {REDACTED,
		"secret":                  {REDACTED,
		"password":                {REDACTED,
		"123456":                  {REDACTED,
