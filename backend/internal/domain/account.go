// Account BC: the Account aggregate, its value types/errors, and its pure
// domain methods. Lifted from internal/service in Phase 3.
//
// Precedent note: this is the first domain file to import internal/pkg/xai.
// xai is a leaf package (no domain/service/config/ent deps), so the dependency
// is acyclic and matches the established domain-purity contract (stdlib +
// internal/pkg/errors + leaf internal/pkg/*). Other BCs (proxy, group) already
// import leaf internal/pkg packages such as timezone/ip.
package domain
...
// Account-domain errors used by persistence and application layers. Service
// re-exports these (internal/service/account.go) so existing callers and test
...
func applyAntigravityGemini31ProAliases(mapping map[string]string) {
	target := strings.TrimSpace(mapping[AntigravityGemini31ProAgentModel])
	if target == "" {
		return
	}
...
const (
	...
	// Exported aliases for service _test.go files that reference the original
	// unexported names via the service re-export.
	DefaultPoolModeRetryCount = defaultPoolModeRetryCount
	MaxPoolModeRetryCount     = maxPoolModeRetryCount
)
...
// OpenAIEndpointCapabilitySet is exported because the (impure)
// SupportsOpenAIEndpointCapability gate now lives in the service layer as a
// free function and must call back into this pure accessor on domain.Account.
...
func (a *Account) GetUserMsgQueueMode() string {
	...
	if mode, ok := a.Extra["user_msg_queue_mode"].(string); ok && mode != "" {
		if mode == UMQModeSerialize || mode == UMQModeThrottle {
			return mode
		}
...
// OpenAIImagesCapability classifies an OpenAI image-generation request for
// account capability gating. Lifted from internal/service/openai_images.go so
// the SupportsOpenAIImageCapability domain method can reference it; service
// re-exports the type and consts via aliases.
...
// pointers for application-layer projections (DTO mapping). Lifted from
// internal/service/group.go now that both Account and Group live in domain.
// The domain persistence edge remains AccountGroupLink (group.go); the
// ToLink/FromLink converters stay in the service layer.
...
// Moved from service with the type; the application-layer FromLink/FromLinks
// converters stay in service and call this method through the alias.
func (ag AccountGroup) ToLink() AccountGroupLink {
...
// --- Pure helpers lifted from other service files ---
//
// These four helpers are required by domain methods that moved with the
// Account entity (IsModelSupported, GetGrokMediaBaseURL). They are also still
// called by service-layer code that did not move, so each is exposed through
// an exported alias at the bottom of this file and re-exported from service.
...
// Exported aliases over unexported helpers so the service layer (which still
// has callers that did not move with the Account entity) can reach the
// canonical implementations now living in domain. Service re-exports each
// under its original unexported name (internal/service/account.go).
var (
	LastOpenAIModelSegment              = lastOpenAIModelSegment
	IsGrokCLIProxyTarget                = isGrokCLIProxyTarget
	ParseExtraFloat64                   = parseExtraFloat64
	ParseExtraTime                      = parseExtraTime
	ParseExtraInt                       = parseExtraInt
	NormalizeAccountNotes               = normalizeAccountNotes
	IsOpenAIPersonalAccessTokenAuthMode = isOpenAIPersonalAccessTokenAuthMode
	MatchWildcard                       = matchWildcard
	StringMappingFromRaw                = stringMappingFromRaw
	NextFixedDailyReset                 = nextFixedDailyReset
	LastFixedDailyReset                 = lastFixedDailyReset
	NextFixedWeeklyReset                = nextFixedWeeklyReset
	LastFixedWeeklyReset                = lastFixedWeeklyReset
	NormalizeOpenAIWSIngressDefaultMode = normalizeOpenAIWSIngressDefaultMode
	NormalizeOpenAIWSIngressMode        = normalizeOpenAIWSIngressMode
	NormalizeOpenAICompactMode          = normalizeOpenAICompactMode
	IsOpenAIOAuthServableModel          = isOpenAIOAuthServableModel
	MatchWildcardMappingResult          = matchWildcardMappingResult
)

// Exported const aliases over unexported credential/extra key consts, for the
// same reason as the helper aliases above.
const (
	OpenAILongContextBillingEnabledKey = openAILongContextBillingEnabledKey
