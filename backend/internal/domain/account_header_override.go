...
//
// Lifted from internal/service in Phase 3 (Account BC hybrid) so the methods
// can live on domain.Account (type-alias migration forbids service methods).
const (
	...
	// Exported aliases for service _test.go files.
	MaxHeaderOverrideEntries     = maxHeaderOverrideEntries
	MaxHeaderOverrideNameLength  = maxHeaderOverrideNameLength
	MaxHeaderOverrideValueLength = maxHeaderOverrideValueLength
)

// CredKeyHeaderOverrideEnabled / CredKeyHeaderOverrides are exported for the
// service/admin save path that still references the credential keys by name.
const (
	CredKeyHeaderOverrideEnabled = credKeyHeaderOverrideEnabled
	CredKeyHeaderOverrides       = credKeyHeaderOverrides
)
...
// Lifted from service/header_util.go so ApplyHeaderOverrides can call
// resolveWireCasing without a service→domain reverse dep. The rest of
// header_util (setHeaderRaw etc.) stays in service and re-exports this via
// an alias.
...
// ResolveWireCasing maps a Go-canonical header key to the wire casing used by
// the real Claude CLI. Missing keys are returned unchanged. Exported so the
// service-layer header_util helpers can re-export it under the original name.
...
func resolveWireCasing(key string) string { return ResolveWireCasing(key) }
...
// Moved to domain with the Account methods so the admin save path can keep calling
// service.NormalizeHeaderOverrideCredentials via a thin re-export.
