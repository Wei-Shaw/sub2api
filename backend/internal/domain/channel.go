...
// Channel-domain errors used by persistence and application layers.
var (
...
// Feature-config keys shared by Channel.FeaturesConfig (and, for
// FeatureKeyWebSearchEmulation, Account.Extra). Canonical home is domain so the
// Channel aggregate methods can read FeaturesConfig without importing service.
const (
	// FeatureKeyWebSearchEmulation is the key used in Channel.FeaturesConfig and Account.Extra.
...
// FeatureKeyCodexImageGenerationBridge is the key used in Channel.FeaturesConfig
// (and Account.Extra) for the Codex image_generation bridge override.
...
// boolOverridePtr boxes a bool into a fresh pointer.
func boolOverridePtr(v bool) *bool {
	return &v
}

// platformBoolOverride reads a per-platform bool override from a features map:
// a top-level bool wins; otherwise a platform-keyed nested map is consulted.
