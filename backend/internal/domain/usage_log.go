// UsageLog BC: the UsageLog entity, its RequestType/BillingType values, domain
// errors, and pure methods. Lifted from internal/service in Phase 3.
//
// Nested associations (User, APIKey, Account, Group, UserSubscription) are
// already domain types, so UsageLog can live here without layer inversion.
package domain
...
// UsageLog is a single recorded API request for billing and analytics.
...
// TotalTokens returns the sum of input, output, and cache tokens.
func (u *UsageLog) TotalTokens() int {
	return u.InputTokens + u.OutputTokens + u.CacheCreationTokens + u.CacheReadTokens
}

// EffectiveRequestType returns the typed request classification, falling back to
// legacy stream/openai_ws_mode flags when RequestType is unknown.
...
// SyncRequestTypeAndLegacyFields normalizes RequestType and rewrites the legacy
// stream/openai_ws_mode flags so they stay consistent with the typed value.
