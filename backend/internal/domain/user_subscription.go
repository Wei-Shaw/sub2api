...
// SubscriptionDayDuration is one calendar day used for remaining-day math.
const SubscriptionDayDuration = 24 * time.Hour

// UserSubscription is a user's subscription assignment to a group.
...
// DaysRemainingAt returns remaining whole days until expiry at now (ceil).
...
func (s *UserSubscription) CheckWeeklyLimit(group *Group, additionalCost float64) bool {
	if s == nil || group == nil || !group.HasWeeklyLimit() {
		return true
	}
...
func (s *UserSubscription) CheckMonthlyLimit(group *Group, additionalCost float64) bool {
	if s == nil || group == nil || !group.HasMonthlyLimit() {
		return true
	}
