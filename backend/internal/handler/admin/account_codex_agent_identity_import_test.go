...
func TestBuildCodexAgentIdentityKeysUseChatGPTAccountOnly(t *testing.T) {
	...
	require.Equal(t, []string{"account:team-a"REDACTED, keys)
REDACTED
...
REDACTED, []codexImportEntry{{Index: 1, Value: firstREDACTED, {Index: 2, Value: secondREDACTEDREDACTED)
REDACTED
...
REDACTED
	...
	REDACTED,
REDACTED
	svc := newCodexImportMemoryAdminService([]service.Account{existingREDACTED)
	handler := NewAccountHandler(svc, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)

	result, err := handler.importCodexSessions(context.Background(), CodexSessionImportRequest{
		SkipDefaultGroupBind: boolPtr(true),
REDACTED, []codexImportEntry{{Index: 1, Value: secondREDACTEDREDACTED)
REDACTED
