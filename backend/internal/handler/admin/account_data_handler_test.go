...
REDACTED
	...
	REDACTED"token": "secret"REDACTED,
			Extra:       map[string]any{"note": "x"REDACTED,
			ProxyID:     &proxyID,
			Concurrency: 3,
			Priority:    50,
	...
	REDACTED,
		{
			...
			Platform:        service.PlatformOpenAI,
			Type:            service.AccountTypeOAuth,
			Credentials:     map[string]any{REDACTED, // 影子恒空凭据
...
func TestExportDataPassesAccountFiltersAndSort(t *testing.T) {
	router, adminSvc := setupAccountDataRouter()
	adminSvc.accounts = []service.Account{
		{ID: 1, Name: "acc-1", Status: service.StatusActiveREDACTED,
REDACTED
			...
			"accounts": []map[string]any{
				{
					...
					"platform":    service.PlatformOpenAI,
					"type":        service.AccountTypeOAuth,
					"credentials": map[string]any{"token": "x"REDACTED,
					"proxy_key":   "socks5|1.2.3.4|1080|u|p",
					"concurrency": 3,
					"priority":    50,
			REDACTED,
