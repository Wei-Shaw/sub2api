...
//
// 仅对 platform=openai / 国产供应商 / ollama_cloud && type=apikey 账号生效；其他账号
// 无操作（国产与 ollama_cloud 在探针内按协议直接落标，不发网络请求）。
...
func (h *AccountHandler) scheduleOpenAIResponsesProbe(account *service.Account) {
	if account == nil || account.Type != service.AccountTypeAPIKey ||
		(account.Platform != service.PlatformOpenAI && !service.IsCNProvider(account.Platform) && !account.IsOllamaCloud()) {
		return
	}
