...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	if req.Type == service.AccountTypeAPIKeyChatCompletions {
		if err := validateAPIKeyChatCompletionsCredentials(req.Credentials); err != nil {
...
func (h *AccountHandler) Update(c *gin.Context) {
	...
	if req.Type == service.AccountTypeAPIKeyChatCompletions && len(req.Credentials) > 0 {
		if err := validateAPIKeyChatCompletionsCredentialsUpdate(req.Credentials); err != nil {
			response.BadRequest(c, err.Error())
			return
		}
	}
