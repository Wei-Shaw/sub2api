...
REDACTED
	...
	if h.concurrencyService != nil {
		if counts, err := h.concurrencyService.GetAccountConcurrencyBatch(ctx, []int64{account.IDREDACTED); err == nil {
...
REDACTED
	...
	REDACTED
		...
		if h.sessionLimitCache != nil && account.GetMaxSessions() > 0 {
			...
			idleTimeouts := map[int64]time.Duration{account.ID: idleTimeoutREDACTED
			if sessions, err := h.sessionLimitCache.GetActiveSessionCountBatch(ctx, []int64{account.IDREDACTED, idleTimeouts); err == nil {
...
REDACTED
	if h.ollamaCloudUsage != nil {
		if err := h.ollamaCloudUsage.ResolveAccounts(c.Request.Context(), []*service.Account{accountREDACTED); err != nil {
			response.ErrorFrom(c, err)
			return
	REDACTED
REDACTED
...
REDACTED
	...
	if len(req.GroupIDs) == 0 {
		response.Success(c, gin.H{"has_risk": falseREDACTED)
		return
REDACTED
...
REDACTED
	...
	result, err := executeAdminIdempotent(
		...
		struct {
			AccountID int64 `json:"account_id"`
	REDACTED{AccountID: accountIDREDACTED,
...
REDACTED else if account.Platform == service.PlatformGemini {
	...
	REDACTED
		...
		REDACTED
	REDACTED
REDACTED else if account.Platform == service.PlatformAntigravity {
	...
	REDACTED
		...
		REDACTED
	REDACTED
REDACTED else if account.Platform == service.PlatformGrok {
...
REDACTED
	...
	for _, u := range updates {
		updateInput := &service.UpdateAccountInput{Credentials: u.CredentialsREDACTED
...
REDACTED
	...
	if state == nil || state.UntilUnix <= time.Now().Unix() {
		response.Success(c, gin.H{"active": falseREDACTED)
		return
REDACTED
...
REDACTED
	...
	response.Success(c, gin.H{"message": "Temp unschedulable cleared successfully"REDACTED)
REDACTED
...
REDACTED
	...
	payload := gin.H{"stats": statsREDACTED
...
REDACTED
	...
	if len(accountIDs) == 0 {
		response.Success(c, gin.H{
			"usage":  map[string]any{REDACTED,
			"errors": map[string]string{REDACTED,
	REDACTED)
		return
REDACTED
...
REDACTED
	...
	response.Success(c, gin.H{"models": modelsREDACTED)
REDACTED
