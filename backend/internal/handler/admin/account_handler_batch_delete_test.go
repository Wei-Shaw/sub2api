...
func (s *batchDeleteAdminService) GetAccountsByIDs(_ context.Context, ids []int64) ([]*service.Account, error) {
	...
	for _, id := range ids {
		...
		REDACTED
			continue
	REDACTED
		accounts = append(accounts, &service.Account{ID: idREDACTED)
REDACTED
	return accounts, nil
REDACTED
...
REDACTED
	...
	req := httptest.NewRequest(
		...
		bytes.NewBufferString(`{"account_ids":[5,4,3,2,1,2,0,-1]REDACTED`),
	)
...
REDACTED
	...
	require.Equal(t, []int64{1, 2, 4, 5REDACTED, payload.Data.SuccessIDs)
	require.Equal(t, []int64{3REDACTED, payload.Data.FailedIDs)
...
func TestAccountHandlerBatchDeleteDoesNotRaceSelectedShadowWithParent(t *testing.T) {
	...
	adminSvc := &batchDeleteAdminService{
		...
		accountsByID: map[int64]*service.Account{
			1: {ID: 1REDACTED,
			2: {ID: 2, ParentAccountID: &parentIDREDACTED,
			3: {ID: 3REDACTED,
	REDACTED,
REDACTED
	router := setupAccountBatchDeleteRouter(adminSvc)
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/api/v1/admin/accounts/batch-delete",
		bytes.NewBufferString(`{"account_ids":[1,2,3]REDACTED`),
	)
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
...
REDACTED
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &payload))
	require.Equal(t, []int64{1, 2, 3REDACTED, payload.Data.SuccessIDs)
	require.Empty(t, payload.Data.FailedIDs)
	require.ElementsMatch(t, []int64{1, 3REDACTED, adminSvc.deletedIDs)
REDACTED
...
REDACTED
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/api/v1/admin/accounts/batch-delete",
		bytes.NewBufferString(`{"account_ids":[0,-1]REDACTED`),
	)
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
