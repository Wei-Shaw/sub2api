...
func setupAccountPerformanceRouter(t *testing.T, repo service.UsageLogRepository) *gin.Engine {
	...
	accountPerformanceBatchCache = newBoundedSnapshotCache(30*time.Second, maxAccountPerformanceSnapshots)
	t.Cleanup(func() {
		stopSnapshotCacheCleanup(accountPerformanceBatchCache)
		accountPerformanceBatchCache = originalCache
	})
...
func TestAccountPerformanceHandlerCacheSnapshot(t *testing.T) {
	...
	require.Equal(t, maxAccountPerformanceSnapshots, accountPerformanceBatchCache.maxEntries)
