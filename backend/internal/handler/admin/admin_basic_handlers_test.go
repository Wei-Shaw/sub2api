...
func setupAdminRouter() (*gin.Engine, *stubAdminService) {
	...
	router.POST("/api/v1/admin/users/auth-identity-migration-reports/:id/resolve", func(c *gin.Context) {
		c.Set(string(servermiddleware.ContextKeyUser), servermiddleware.AuthSubject{UserID: 99REDACTED)
...
func TestUserHandlerEndpoints(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{"resolution_note": "resolved by manual bind"REDACTED)
	rec = httptest.NewRecorder()
...
REDACTED
	...
	createBody := map[string]any{"email": "new@example.com", "password": "pass123", "balance": 1, "concurrency": 2REDACTED
	...
	updateBody := map[string]any{"email": "updated@example.com"REDACTED
	...
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/users/1/balance", bytes.NewBufferString(`{"balance":1,"operation":"add"REDACTED`))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
...
func TestUserHandlerBindAuthIdentityMapsRequest(t *testing.T) {
	...
	body, err := json.Marshal(map[string]any{
		...
		"metadata":         map[string]any{"report_id": 12REDACTED,
REDACTED)
REDACTED
...
func TestGroupHandlerEndpoints(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{"name": "new", "platform": "anthropic", "subscription_type": "standard"REDACTED)
	rec = httptest.NewRecorder()
	...
	body, _ = json.Marshal(map[string]any{"name": "update"REDACTED)
	rec = httptest.NewRecorder()
...
func TestProxyHandlerEndpoints(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{"name": "proxy", "protocol": "http", "host": "localhost", "port": 8080REDACTED)
	rec = httptest.NewRecorder()
	...
	body, _ = json.Marshal(map[string]any{"name": "proxy2"REDACTED)
	rec = httptest.NewRecorder()
	...
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/proxies/batch-delete", bytes.NewBufferString(`{"ids":[1,2]REDACTED`))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
...
func TestRedeemHandlerEndpoints(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{"count": 1, "type": "balance", "value": 10REDACTED)
	rec = httptest.NewRecorder()
	...
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/redeem-codes/batch-delete", bytes.NewBufferString(`{"ids":[1,2]REDACTED`))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
