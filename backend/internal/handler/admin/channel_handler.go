...
func NewChannelHandler(channelService *service.ChannelService, billingService *service.BillingService, pricingService *service.PricingService) *ChannelHandler {
	return &ChannelHandler{channelService: channelService, billingService: billingService, pricingService: pricingServiceREDACTED
REDACTED
...
REDACTED
	if resp.ModelMapping == nil {
		resp.ModelMapping = map[string]map[string]string{REDACTED
REDACTED
...
REDACTED
	...
	REDACTED
		if ruleResp.GroupIDs == nil {
			ruleResp.GroupIDs = []int64{REDACTED
	REDACTED
		if ruleResp.AccountIDs == nil {
			ruleResp.AccountIDs = []int64{REDACTED
	REDACTED
...
REDACTED
	...
	response.Success(c, gin.H{"message": "Channel deleted successfully"REDACTED)
REDACTED
...
func (h *ChannelHandler) GetModelDefaultPricing(c *gin.Context) {
	...
	if model == "" {
		response.ErrorFrom(c, infraerrors.BadRequest("MISSING_PARAMETER", "model parameter is required").
			WithMetadata(map[string]string{"param": "model"REDACTED))
		return
REDACTED
	...
	if err != nil {
		// 模型不在定价列表中
		response.Success(c, gin.H{"found": falseREDACTED)
		return
REDACTED
...
func (h *ChannelHandler) SyncPricingModels(c *gin.Context) {
	...
	if platform == "" {
		response.ErrorFrom(c, infraerrors.BadRequest("MISSING_PARAMETER", "platform parameter is required").
			WithMetadata(map[string]string{"param": "platform"REDACTED))
		return
REDACTED
