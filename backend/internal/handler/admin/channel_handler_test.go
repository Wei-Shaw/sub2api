...
func float64Ptr(v float64) *float64 { return &v REDACTED
...
func TestChannelToResponse_FullChannel(t *testing.T) {
	...
	ch := &service.Channel{
		...
		GroupIDs:           []int64{1, 2, 3REDACTED,
		ModelPricing: []service.ChannelModelPricing{
			{
				ID:              10,
				Platform:        "openai",
				Models:          []string{"gpt-4"REDACTED,
	...
	REDACTED,
		ModelMapping: map[string]map[string]string{
			"anthropic": {"claude-3-haiku": "claude-haiku-3"REDACTED,
	REDACTED,
REDACTED
	...
	require.Equal(t, []int64{1, 2, 3REDACTED, resp.GroupIDs)
	...
	require.Equal(t, []string{"gpt-4"REDACTED, p.Models)
		...
		REDACTED,
			{
				ID:              2,
				Platform:        "openai",
				Models:          []string{"gpt-4", "gpt-4o"REDACTED,
				BillingMode:     service.BillingModePerRequest,
...
REDACTED
	...
	require.Equal(t, "anthropic", resp.ModelPricing[0].Platform)
	require.Equal(t, []string{"claude-sonnet-4"REDACTED, resp.ModelPricing[0].Models)
	require.Equal(t, "token", resp.ModelPricing[0].BillingMode)

	require.Equal(t, int64(2), resp.ModelPricing[1].ID)
	require.Equal(t, "openai", resp.ModelPricing[1].Platform)
	require.Equal(t, []string{"gpt-4", "gpt-4o"REDACTED, resp.ModelPricing[1].Models)
	...
	require.Equal(t, []string{"gemini-2.5-pro"REDACTED, resp.ModelPricing[2].Models)
...
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := pricingRequestToService([]channelModelPricingRequest{tt.reqREDACTED, true)
...
REDACTED
	...
	require.Equal(t, []string{"gpt-4", "gpt-4o"REDACTED, r.Models)
...
func TestPricingRequestToService_EmptySlice(t *testing.T) {
	result := pricingRequestToService([]channelModelPricingRequest{REDACTED, true)
...
REDACTED
	...
	got := pricingRequestToService([]channelModelPricingRequest{reqREDACTED, true)
...
func TestPricingRequestToService_TimePricingNil(t *testing.T) {
	got := pricingRequestToService([]channelModelPricingRequest{{Models: []string{"gpt-5"REDACTEDREDACTEDREDACTED, true)
...
REDACTED
	...
	allowed := pricingRequestToService([]channelModelPricingRequest{reqREDACTED, true)
	...
	dropped := pricingRequestToService([]channelModelPricingRequest{reqREDACTED, false)
...
func TestPricingToResponse_TimePricingNil(t *testing.T) {
	got := pricingToResponse(&service.ChannelModelPricing{REDACTED)
...
func setupSyncPricingModelsRouter(pricingSvc *service.PricingService) *gin.Engine {
	...
	router := gin.New()
	h := &ChannelHandler{pricingService: pricingSvcREDACTED
...
func TestSyncPricingModels_ValidPlatform_EmptyService(t *testing.T) {
	...
	for _, platform := range []string{"anthropic", "openai", "gemini", "antigravity", "grok", "kimi", "zhipu", "deepseek"REDACTED {
