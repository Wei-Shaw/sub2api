...
type duplicateChannelMonitorHandlerEncryptor struct{REDACTED
...
REDACTED
	...
	repo := &duplicateChannelMonitorHandlerRepoStub{
		source: &service.ChannelMonitor{
			...
			ExtraModels:      []string{"gpt-5.4"REDACTED,
			Enabled:          true,
			IntervalSeconds:  60,
			BodyOverrideMode: service.MonitorBodyOverrideModeOff,
	REDACTED,
REDACTED
	monitorService := service.NewChannelMonitorService(repo, duplicateChannelMonitorHandlerEncryptor{REDACTED)
