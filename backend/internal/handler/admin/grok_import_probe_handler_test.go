...
type grokImportOAuthClientStub struct{REDACTED

func (grokImportOAuthClientStub) ExchangeCode(context.Context, string, string, string, string, string) (*xai.TokenResponse, error) {
	return &xai.TokenResponse{AccessToken: "access-token", RefreshToken: "refresh-token", ExpiresIn: 3600REDACTED, nil
REDACTED
...
func TestGrokSSOBatchImportKeepsCreatedAccountsWhenOneAutomaticProbeFails(t *testing.T) {
	...
	oauthService := service.NewGrokOAuthService(nil, grokImportOAuthClientStub{REDACTED)
	...
	request := httptest.NewRequest(
		...
		strings.NewReader(`{"sso_tokens":["sso-one","sso-two","sso-three"]REDACTED`),
	)
...
REDACTED
	...
	require.Equal(t, map[int64]int{501: 1, 502: 1, 503: 1REDACTED, calls)
REDACTED
...
func TestAccountCreateWithoutAutomaticGrokProbeServiceStillSucceeds(t *testing.T) {
	...
	request := httptest.NewRequest(
		...
		strings.NewReader(`{"name":"grok-rt","platform":"grok","type":"oauth","credentials":{"refresh_token":"secret"REDACTEDREDACTED`),
	)
	request.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(recorder, request)
