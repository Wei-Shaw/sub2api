...
type CreateGroupRequest struct {
	...
	// JSHandlerScriptIDs ordered on_before_request scripts for this group.
	JSHandlerScriptIDs []string `json:"jshandler_script_ids"`
	// 从指定分组复制账号（创建后自动绑定）
...
type UpdateGroupRequest struct {
	...
	RPMLimit *int `json:"rpm_limit"`
	// JSHandlerScriptIDs nil=不修改；非 nil（含空数组）=替换 on_before_request 绑定
	JSHandlerScriptIDs *[]string `json:"jshandler_script_ids"`
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		JSHandlerScriptIDs:              req.JSHandlerScriptIDs,
