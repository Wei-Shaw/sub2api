...
func duplicateGroupHandlerFixture() *service.Group {
	return &service.Group{
		...
		ModelRouting:         map[string][]int64{"claude-*": {7REDACTEDREDACTED,
REDACTED
REDACTED

func TestDuplicateGroupHandlerReturnsAdminDTOWithoutOperationMetadata(t *testing.T) {
	svc := &duplicateGroupAdminServiceStub{group: duplicateGroupHandlerFixture()REDACTED
...
func TestDuplicateGroupHandlerRejectsInvalidID(t *testing.T) {
	for _, id := range []string{"not-a-number", "0", "-1"REDACTED {
		t.Run(id, func(t *testing.T) {
			svc := &duplicateGroupAdminServiceStub{REDACTED
			router := setupDuplicateGroupRouter(t, svc)
			recorder := httptest.NewRecorder()
