...
type storeUnavailableRepoStub struct{REDACTED
...
func TestExecuteAdminIdempotentJSONFailCloseOnStoreUnavailable(t *testing.T) {
	...
	service.SetDefaultIdempotencyCoordinator(service.NewIdempotencyCoordinator(storeUnavailableRepoStub{REDACTED, service.DefaultIdempotencyConfig()))
	t.Cleanup(func() {
...
REDACTED)
	...
	router.POST("/idempotent", func(c *gin.Context) {
		executeAdminIdempotentJSON(c, "admin.test.high", map[string]any{"a": 1REDACTED, time.Minute, func(ctx context.Context) (any, error) {
			executed++
...
REDACTED)
	...
	router := gin.New()
	router.POST("/idempotent", func(c *gin.Context) {
		executeAdminIdempotentJSONFailOpenOnStoreUnavailable(c, "admin.test.medium", map[string]any{"a": 1REDACTED, time.Minute, func(ctx context.Context) (any, error) {
			executed++
			return gin.H{"ok": trueREDACTED, nil
	REDACTED)
REDACTED)
...
REDACTED)
	...
	router := gin.New()
	router.POST("/idempotent", func(c *gin.Context) {
		executeAdminIdempotentJSON(c, "admin.test.concurrent", map[string]any{"a": 1REDACTED, time.Minute, func(ctx context.Context) (any, error) {
			executed.Add(1)
