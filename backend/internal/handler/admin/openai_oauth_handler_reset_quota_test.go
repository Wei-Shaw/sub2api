...
func (s *openAIAccountStateRecovererStub) RecoverAccountState(ctx context.Context, accountID int64, options service.AccountRecoveryOptions) (*service.SuccessfulTestRecoveryResult, error) {
	...
	return &service.SuccessfulTestRecoveryResult{REDACTED, s.err
REDACTED
	...
	REDACTED,
		queryResult: &service.OpenAIQuotaUsage{
			...
			RateLimitResetCredits: &service.OpenAIRateLimitResetCredits{
				AvailableCount: 0,
				Credits:        []service.OpenAIRateLimitResetCreditDetail{REDACTED,
		REDACTED,
	REDACTED,
REDACTED
REDACTED
...
func TestOpenAIResetQuota_ResetFailureStopsWorkflow(t *testing.T) {
	quota := &openAIQuotaWorkflowStub{resetErr: errors.New("upstream reset failed")REDACTED
	recoverer := &openAIAccountStateRecovererStub{REDACTED
	handler := &OpenAIOAuthHandler{
		adminService:     &openAIResetAdminServiceStub{REDACTED,
		quotaService:     quota,
...
func TestOpenAIResetQuota_RecoveryFailureStopsWorkflow(t *testing.T) {
	quota := successfulOpenAIQuotaWorkflowStub()
	recoverer := &openAIAccountStateRecovererStub{err: errors.New("recovery failed")REDACTED
	adminService := recoveredAccountStub()
	handler := &OpenAIOAuthHandler{
		adminService:     adminService,
		quotaService:     quota,
...
func TestOpenAIResetQuota_AccountRefreshFailureReportsRecoveredState(t *testing.T) {
	quota := successfulOpenAIQuotaWorkflowStub()
	recoverer := &openAIAccountStateRecovererStub{REDACTED
	adminService := &openAIResetAdminServiceStub{err: errors.New("account refresh failed")REDACTED
	handler := &OpenAIOAuthHandler{
		adminService:     adminService,
		quotaService:     quota,
		rateLimitService: recoverer,
...
func TestOpenAIRefreshQuota_QueryFailureIsReported(t *testing.T) {
	quota := &openAIQuotaWorkflowStub{queryErr: errors.New("upstream query failed")REDACTED
	handler := &OpenAIOAuthHandler{
		adminService: &openAIResetAdminServiceStub{REDACTED,
		quotaService: quota,
REDACTED
...
func TestNewOpenAIOAuthHandlerKeepsNilQuotaCapabilitiesGuarded(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		path   string
REDACTED{
		{http.MethodGet, "/openai/accounts/42/quota"REDACTED,
		{http.MethodPost, "/openai/accounts/42/quota/refresh"REDACTED,
		{http.MethodPost, "/openai/accounts/42/reset-quota"REDACTED,
REDACTED {
		recorder := httptest.NewRecorder()
