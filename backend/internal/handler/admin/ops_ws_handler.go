...
REDACTED,
	...
	Subprotocols: []string{"sub2api-admin"REDACTED,
REDACTED
...
REDACTED
	...
	REDACTED
		...
		c.cancel = cancel
		c.done = make(chan struct{REDACTED)
		done = c.done
		c.running = true
		c.mu.Unlock()
...
func (h *OpsHandler) QPSWSHandler(c *gin.Context) {
	...
	if h == nil || h.opsService == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "ops service not initialized"REDACTED)
		return
REDACTED
	...
	if !h.opsService.IsRealtimeMonitoringEnabled(c.Request.Context()) {
		...
		if err != nil {
			c.JSON(http.StatusNotFound, gin.H{"error": "ops realtime monitoring is disabled"REDACTED)
			return
	REDACTED
...
func requestPeerIP(r *http.Request) (netip.Addr, bool) {
	if r == nil {
		return netip.Addr{REDACTED, false
REDACTED
