...
func (h *PaymentHandler) CreateChannel(c *gin.Context) {
	var req service.CreateChannelRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	ch, err := h.configService.CreateChannel(c.Request.Context(), req)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// UpdateChannel updates an existing payment channel.
// PUT /api/v1/admin/payment/channels/:id
func (h *PaymentHandler) UpdateChannel(c *gin.Context) {
	...
	ch, err := h.configService.UpdateChannel(c.Request.Context(), id, req)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// DeleteChannel deletes a payment channel.
// DELETE /api/v1/admin/payment/channels/:id
func (h *PaymentHandler) DeleteChannel(c *gin.Context) {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid channel ID")
		return
	}
	if err := h.configService.DeleteChannel(c.Request.Context(), id); err != nil {
		response.ErrorFrom(c, err)
		return
	}
