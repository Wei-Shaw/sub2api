...
func TestDiffSettings_DetectsGlobalPlatformQuotaChange(t *testing.T) {
	...
	before := &service.SystemSettings{
		DefaultPlatformQuotas: map[string]*service.DefaultPlatformQuotaSetting{
			"anthropic": {DailyLimitUSD: &fiveREDACTED,
	REDACTED,
REDACTED
...
REDACTED
	...
	changed := diffSettings(before, after, nil, nil, UpdateSettingsRequest{REDACTED)
	found := false
...
REDACTED{
		...
		{&five, nil, falseREDACTED,
		{nil, &five, falseREDACTED,
		{&five, &five2, trueREDACTED,
		{&five, &ten, falseREDACTED,
REDACTED
	for _, c := range cases {
...
REDACTED
	...
	changed := appendAuthSourceDefaultChanges([]string{REDACTED, before, after)
