...
REDACTED
	...
	repo := &settingHandlerRepoStub{values: storedREDACTED
...
func TestUpdateSettingsEnableStepUpRejectsWithoutSession(t *testing.T) {
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{REDACTED)

	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": trueREDACTED, nil)
...
func TestUpdateSettingsEnableStepUpRejectsAdminAPIKey(t *testing.T) {
	h, _ := newStepUpSwitchTestHandler(t, map[string]string{REDACTED)

	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": trueREDACTED, func(c *gin.Context) {
...
func TestUpdateSettingsEnableStepUpFailsClosedWithoutUserService(t *testing.T) {
	...
	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": trueREDACTED, func(c *gin.Context) {
		c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 1REDACTED)
REDACTED)
...
REDACTED)
	...
	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": falseREDACTED, nil)
...
REDACTED)
	...
	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": falseREDACTED, func(c *gin.Context) {
		c.Set("auth_method", service.AuditAuthMethodAdminAPIKey)
REDACTED)
...
REDACTED)
	...
	rec := doUpdateSettings(t, h, map[string]any{"registration_enabled": trueREDACTED, nil)
...
REDACTED)
	...
	preserved := doUpdateSettings(t, h, map[string]any{"registration_enabled": trueREDACTED, nil)
	...
	cleared := doUpdateSettings(t, h, map[string]any{"forwarded_client_ip_headers": []string{REDACTEDREDACTED, nil)
...
func TestUpdateSettingsMalformedForwardedClientIPHeadersRemainFailClosedWhenOmitted(t *testing.T) {
	cfg := &config.Config{Default: config.DefaultConfig{UserConcurrency: 5REDACTEDREDACTED
	repo := &settingHandlerRepoStub{values: map[string]string{
		service.SettingKeyAPIKeyACLTrustForwardedIP: "true",
		service.SettingKeyForwardedClientIPHeaders:  `{"not":"an array"REDACTED`,
REDACTEDREDACTED
...
REDACTED)
	...
	rec := doUpdateSettings(t, h, map[string]any{
		"forwarded_client_ip_headers": []string{"X Invalid"REDACTED,
REDACTED, nil)
