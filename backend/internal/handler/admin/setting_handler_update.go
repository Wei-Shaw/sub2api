...
func buildSettingKeyByJSONName() map[string]string {
	t := reflect.TypeOf(UpdateSettingsRequest{REDACTED)
...
func omittedSettingKeys(sentFields map[string]json.RawMessage) service.OmittedSettingKeys {
	...
	for jsonName, settingKey := range settingKeyByJSONName {
		if _, sent := sentFields[jsonName]; !sent {
			omitted[settingKey] = struct{REDACTED{REDACTED
	REDACTED
REDACTED
	return omitted
REDACTED
...
REDACTED
	...
	for _, enabled := range []bool{turnstileEnabled, tencentCaptchaEnabled, aliyunCaptchaEnabledREDACTED {
		if enabled {
...
REDACTED
	...
	REDACTED
		...
		REDACTED else if req.WeChatConnectMobileEnabled {
...
REDACTED
	...
	REDACTED
		...
		REDACTED
			...
			REDACTED
				items[i].ID = id
		REDACTED else if len(item.ID) > maxMenuItemIDLen {
				response.BadRequest(c, "Custom menu item ID is too long (max 32 characters)")
				return
		REDACTED else if !menuItemIDPattern.MatchString(item.ID) {
	...
	REDACTED
		...
		for _, item := range items {
			if _, exists := seen[item.ID]; exists {
				...
				return
		REDACTED
			seen[item.ID] = struct{REDACTED{REDACTED
	REDACTED
...
REDACTED else if fastPolicy != nil {
