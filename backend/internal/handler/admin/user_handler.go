...
//   - attr[{idREDACTED]: filter by custom attribute value, e.g. attr[1]=company
...
// Format: attr[{attributeIDREDACTED]=value, e.g. attr[1]=company&attr[2]=developer
func parseAttributeFilters(c *gin.Context) map[int64]string {
	...
	for key, values := range c.Request.URL.Query() {
		if len(values) == 0 || values[0] == "" {
			continue
	REDACTED
		// Check if key matches pattern attr[{idREDACTED]
...
REDACTED
	...
	response.Success(c, gin.H{"message": "User deleted successfully"REDACTED)
REDACTED
...
REDACTED
	...
	if req.All {
		...
		const pageSize = 500
		for {
			users, _, err := h.adminService.ListUsers(c.Request.Context(), page, pageSize, service.UserListFilters{REDACTED, "id", "asc")
			if err != nil {
				response.ErrorFrom(c, err)
				return
		REDACTED
...
REDACTED
	...
	if len(userIDs) == 0 {
		response.Success(c, gin.H{"affected": 0REDACTED)
		return
REDACTED
	...
	if err != nil {
		...
		return
REDACTED
	response.Success(c, gin.H{"affected": affectedREDACTED)
REDACTED
...
REDACTED
	if h.userPlatformQuotaRepo == nil {
		response.Success(c, map[string]any{"platform_quotas": []any{REDACTEDREDACTED)
		return
REDACTED
...
REDACTED
	response.Success(c, map[string]any{"platform_quotas": outREDACTED)
REDACTED
...
REDACTED
	...
	if len(req.Quotas) > 4 {
		...
		return
REDACTED
	seen := make(map[string]struct{REDACTED, len(req.Quotas))
	...
	REDACTED
		if _, dup := seen[q.Platform]; dup {
			...
			return
	REDACTED
		seen[q.Platform] = struct{REDACTED{REDACTED
		...
		for _, f := range []struct {
			...
			val  *float64
	REDACTED{
			{"daily_limit_usd", q.DailyLimitUSDREDACTED,
			{"weekly_limit_usd", q.WeeklyLimitUSDREDACTED,
			{"monthly_limit_usd", q.MonthlyLimitUSDREDACTED,
	REDACTED {
			if f.val == nil {
				continue
		REDACTED
...
REDACTED
	afterPlatforms := make(map[string]struct{REDACTED, len(records))
	for _, r := range records {
		afterPlatforms[r.Platform] = struct{REDACTED{REDACTED
REDACTED
...
var allowedWindowsForQuotaReset = map[string]struct{REDACTED{
	"daily":   {REDACTED,
	"weekly":  {REDACTED,
	"monthly": {REDACTED,
REDACTED
