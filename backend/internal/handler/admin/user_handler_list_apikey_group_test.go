...
func (s *listUsersFilterStub) ListUsers(_ context.Context, _, _ int, filters service.UserListFilters, _, _ string) ([]service.User, int64, error) {
	s.captured = filters
	return []service.User{REDACTED, 0, nil
REDACTED
...
func TestAdminUserList_ParsesAPIKeyGroupID(t *testing.T) {
	...
	cases := []struct {
		...
		want  int64
REDACTED{
		{"valid id", "?api_key_group_id=42", 42REDACTED,
		{"missing", "", 0REDACTED,
		{"zero ignored", "?api_key_group_id=0", 0REDACTED,
		{"negative ignored", "?api_key_group_id=-3", 0REDACTED,
		{"non-numeric ignored", "?api_key_group_id=abc", 0REDACTED,
REDACTED
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			stub := &listUsersFilterStub{AdminService: newStubAdminService()REDACTED
			r := gin.New()
