...
REDACTED
	...
	identityKey := service.PendingAuthIdentityKey{ProviderType: "dingtalk", ProviderKey: "dingtalk", ProviderSubject: unionIDREDACTED
...
REDACTED
	...
	REDACTED
		if err := h.createOAuthPendingSession(c, oauthPendingSessionPayload{
			...
			UpstreamIdentityClaims: upstreamClaims,
			CompletionResponse:     map[string]any{"redirect": redirectToREDACTED,
	REDACTED); err != nil {
...
REDACTED
	...
	REDACTED
		...
		if err := h.createOAuthPendingSession(c, oauthPendingSessionPayload{
			...
			UpstreamIdentityClaims: upstreamClaims,
			CompletionResponse:     map[string]any{"redirect": redirectTo, "synthetic_email": syntheticEmailREDACTED,
	REDACTED); err != nil {
			redirectOAuthError(c, frontendCallback, "session_error", infraerrors.Reason(err), infraerrors.Message(err))
			return
	REDACTED
...
REDACTED
	...
	REDACTED
		if username != "" && h.userService != nil {
			if _, err := h.userService.UpdateProfile(ctx, userID, service.UpdateProfileRequest{Username: &usernameREDACTED); err != nil {
...
REDACTED
	...
	if cfg.SyncDisplayName && strings.TrimSpace(staff.Name) != "" {
		fields = append(fields, syncField{cfg.SyncDisplayNameAttrKey, strings.TrimSpace(staff.Name)REDACTED)
REDACTED
	if cfg.SyncCorpEmail && strings.TrimSpace(staff.Email) != "" {
		fields = append(fields, syncField{cfg.SyncCorpEmailAttrKey, strings.TrimSpace(staff.Email)REDACTED)
REDACTED
	...
	REDACTED else {
			...
			fields = append(fields, syncField{cfg.SyncDeptAttrKey, pathREDACTED)
	REDACTED
REDACTED
...
func dingTalkStaffFromClaims(claims map[string]any) *DingTalkStaffInfo {
	if claims == nil {
		return &DingTalkStaffInfo{REDACTED
REDACTED
	staff := &DingTalkStaffInfo{REDACTED
...
REDACTED
	...
	case int64:
		if v > 0 {
			staff.DeptIDs = []int64{vREDACTED
	REDACTED
	case float64:
		if id := int64(v); id > 0 {
			staff.DeptIDs = []int64{idREDACTED
	REDACTED
REDACTED
	return staff
REDACTED
...
REDACTED
	if err := h.userAttributeService.UpdateUserAttributes(ctx, userID, []service.UpdateUserAttributeInput{
		{AttributeID: def.ID, Value: valueREDACTED,
REDACTED); err != nil {
		return err
REDACTED
...
func (h *AuthHandler) resolveDingTalkDeptPath(ctx context.Context, client *DingTalkClient, deptID int64) (string, error) {
	...
	REDACTED
		if strings.TrimSpace(info.Name) != "" {
			parts = append([]string{strings.TrimSpace(info.Name)REDACTED, parts...)
	REDACTED
