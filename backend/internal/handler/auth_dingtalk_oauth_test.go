...
// B/C: Step 3/4 失败降级时 staff = &DingTalkStaffInfo{REDACTED，claims 不应有 nil。
func TestBuildDingTalkUpstreamClaims_EmptyStaff(t *testing.T) {
	staff := &DingTalkStaffInfo{REDACTED
...
func TestCheckDingTalkCorpAllowed_CrossOrgPolicy(t *testing.T) {
	cfg := config.DingTalkConnectConfig{CorpRestrictionPolicy: "none"REDACTED
...
func TestDecideDingTalkStep34Strategy_PolicyNone(t *testing.T) {
	step3Err := &DingTalkAPIError{Code: "60011", Message: "not in directory", HTTP: 403REDACTED
...
func TestDecideDingTalkStep34Strategy_PolicyNoneEmpty(t *testing.T) {
	stepErr := &DingTalkAPIError{Code: "60011", Message: "not in directory", HTTP: 403REDACTED
...
func TestDecideDingTalkStep34Strategy_NoError(t *testing.T) {
	for _, policy := range []string{"none", "internal_only", ""REDACTED {
...
func TestBuildDingTalkUpstreamClaims_SubjectEqualsUnionID(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "user123", Name: "张三", Email: "zhangsan@corp.com"REDACTED
...
func TestBuildDingTalkUpstreamClaims_CrossOrgEmptyCorpUserID(t *testing.T) {
	// 跨组织降级路径：staff = &DingTalkStaffInfo{REDACTED（所有字段为零值）
	staff := &DingTalkStaffInfo{REDACTED
...
func TestBuildDingTalkUpstreamClaims_PrimaryDeptIDInClaims(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "u1", Name: "张三", Email: "a@b.com", DeptIDs: []int64{42, 99REDACTEDREDACTED
...
func TestBuildDingTalkUpstreamClaims_NoDeptIDs(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "u2", Name: "李四"REDACTED
...
func TestDingTalkStaffFromClaims_RoundTrip(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "u3", Name: "王五", Email: "ww@corp.com", DeptIDs: []int64{55REDACTEDREDACTED
	...
	require.Equal(t, []int64{55REDACTED, recovered.DeptIDs)
REDACTED
...
func TestResolveDingTalkDeptPath_SingleLevel(t *testing.T) {
	...
	responses := map[string]string{
		"42": `{"errcode":0,"result":{"dept_id":42,"name":"研发部","parent_id":1REDACTEDREDACTED`,
		"1":  `{"errcode":0,"result":{"dept_id":1,"name":"公司","parent_id":0REDACTEDREDACTED`,
REDACTED
	...
	REDACTED else {
			_, _ = w.Write([]byte(`{"errcode":60003,"errmsg":"not found"REDACTED`))
	REDACTED
REDACTED))
	defer server.Close()
...
func TestResolveDingTalkDeptPath_MultiLevel(t *testing.T) {
	...
	responses := map[string]string{
		"42": `{"errcode":0,"result":{"dept_id":42,"name":"AI研发","parent_id":10REDACTEDREDACTED`,
		"10": `{"errcode":0,"result":{"dept_id":10,"name":"研发部","parent_id":1REDACTEDREDACTED`,
		"1":  `{"errcode":0,"result":{"dept_id":1,"name":"公司","parent_id":0REDACTEDREDACTED`,
REDACTED
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
