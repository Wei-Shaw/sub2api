...
func TestEmailOAuthCallbackRequiresPendingRegistrationWhenInvitationEnabled(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: emailOAuthStateCookieName, Value: encodeCookieValue(state)REDACTED)
	req.AddCookie(&http.Cookie{Name: emailOAuthRedirectCookie, Value: encodeCookieValue("/dashboard")REDACTED)
	req.AddCookie(&http.Cookie{Name: emailOAuthProviderCookie, Value: encodeCookieValue("github")REDACTED)
	c.Request = req
...
REDACTED, "/auth/oauth/callback", "/dashboard", profile)
...
REDACTED
	...
	handler.emailOAuthCallbackWithProfile(c, "google", config.EmailOAuthProviderConfig{
		...
		FrontendRedirectURL: "/auth/oauth/callback",
REDACTED, "/auth/oauth/callback", "/dashboard", &emailOAuthProfile{
		Subject:       "google-123",
...
REDACTED)
	...
	req.AddCookie(&http.Cookie{Name: emailOAuthAffiliateCookie, Value: encodeCookieValue("AFF123")REDACTED)
	c.Request = req
...
func TestCompleteEmailOAuthRegistrationUsesAffiliateCodeFromPendingSession(t *testing.T) {
	affiliateRepo := newOAuthEmailAffiliateRepoStub(map[string]int64{"AFF456": 2002REDACTED)
	handler, client := newOAuthPendingFlowTestHandlerWithDependencies(t, oauthPendingFlowTestHandlerOptions{
...
func newOAuthEmailAffiliateRepoStub(codeOwners map[string]int64) *oauthEmailAffiliateRepoStub {
	return &oauthEmailAffiliateRepoStub{codeOwners: codeOwnersREDACTED
REDACTED
...
func (r *oauthEmailAffiliateRepoStub) EnsureUserAffiliate(_ context.Context, userID int64) (*service.AffiliateSummary, error) {
	...
	return &service.AffiliateSummary{UserID: userID, AffCode: "SELF"REDACTED, nil
REDACTED
...
REDACTED
	return &service.AffiliateSummary{UserID: userID, AffCode: strings.ToUpper(strings.TrimSpace(code))REDACTED, nil
REDACTED

func (r *oauthEmailAffiliateRepoStub) BindInviter(_ context.Context, userID, inviterID int64) (bool, error) {
	r.bindCalls = append(r.bindCalls, oauthEmailAffiliateBindCall{userID: userID, inviterID: inviterIDREDACTED)
	return true, nil
REDACTED
