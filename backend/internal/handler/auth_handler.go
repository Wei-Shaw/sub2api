...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if err := h.authService.VerifyCaptchaForRegister(c.Request.Context(), proof, ip.ExactClientIP(c), req.VerifyCode); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AuthHandler) SendVerifyCode(c *gin.Context) {
	...
	proof := captchaProof(req.TurnstileToken, req.TencentCaptchaTicket, req.TencentCaptchaRandstr)
	if err := h.authService.VerifyCaptcha(c.Request.Context(), proof, ip.ExactClientIP(c)); err != nil {
		response.ErrorFrom(c, err)
		return
	}
