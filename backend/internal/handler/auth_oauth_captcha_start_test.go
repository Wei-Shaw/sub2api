...
REDACTED
	return value, nil
REDACTED
func (r *oauthCaptchaSettingRepo) Set(context.Context, string, string) error { return nil REDACTED
...
func (r *oauthCaptchaSettingRepo) GetAll(context.Context) (map[string]string, error) {
	return r.values, nil
REDACTED
func (r *oauthCaptchaSettingRepo) Delete(context.Context, string) error { return nil REDACTED
...
REDACTED
	...
	settings := service.NewSettingService(&oauthCaptchaSettingRepo{values: valuesREDACTED, cfg)
	verifier := &oauthCaptchaVerifier{REDACTED
	...
	return &AuthHandler{authService: authService, settingSvc: settings, cfg: cfgREDACTED, verifier
REDACTED

func oauthStartHandlers() map[string]func(*AuthHandler, *gin.Context) {
	return map[string]func(*AuthHandler, *gin.Context){
		"github":   func(h *AuthHandler, c *gin.Context) { h.GitHubOAuthStart(c) REDACTED,
		"google":   func(h *AuthHandler, c *gin.Context) { h.GoogleOAuthStart(c) REDACTED,
		"linuxdo":  func(h *AuthHandler, c *gin.Context) { h.LinuxDoOAuthStart(c) REDACTED,
		"dingtalk": func(h *AuthHandler, c *gin.Context) { h.DingTalkOAuthStart(c) REDACTED,
		"wechat":   func(h *AuthHandler, c *gin.Context) { h.WeChatOAuthStart(c) REDACTED,
		"oidc":     func(h *AuthHandler, c *gin.Context) { h.OIDCOAuthStart(c) REDACTED,
REDACTED
REDACTED
	...
	for provider := range oauthStartHandlers() {
		t.Run(provider, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(
				...
				strings.NewReader(`{"tencent_captcha_ticket":"ticket-value","tencent_captcha_randstr":"@rand-value"REDACTED`),
			)
...
func TestOAuthStartPostRequiresTencentProofWhenEnabled(t *testing.T) {
	...
	for provider := range oauthStartHandlers() {
		t.Run(provider, func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(recorder)
			c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/auth/oauth/"+provider+"/start", strings.NewReader(`{REDACTED`))
			c.Request.Header.Set("Content-Type", "application/json")
