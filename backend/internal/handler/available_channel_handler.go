...
REDACTED
	...
	if !h.featureEnabled(c) {
		response.Success(c, []userAvailableChannel{REDACTED)
		return
REDACTED
...
REDACTED
	...
	if len(compositeGroups) > 0 {
		modelPlatforms := make(map[string]struct{REDACTED, len(ch.SupportedModels))
		for i := range ch.SupportedModels {
			if platform := ch.SupportedModels[i].Platform; platform != "" {
				modelPlatforms[platform] = struct{REDACTED{REDACTED
		REDACTED
	REDACTED
...
REDACTED
	...
	if intervals == nil {
		...
		intervals = []userPricingIntervalDTO{REDACTED
REDACTED
