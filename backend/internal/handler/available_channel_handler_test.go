...
REDACTED
	...
	for _, key := range []string{"id", "name", "platform", "subscription_type", "rate_multiplier", "peak_rate_enabled", "peak_start", "peak_end", "peak_rate_multiplier", "is_exclusive"REDACTED {
		...
		SupportedModels: []service.SupportedModel{
			{
				...
				Pricing:  &service.ChannelModelPricing{InputPrice: &anthropicPriceREDACTED,
		REDACTED,
			{
				Name:     "gpt-5",
				Platform: service.PlatformOpenAI,
				Pricing:  &service.ChannelModelPricing{InputPrice: &openAIPriceREDACTED,
		REDACTED,
	REDACTED,
REDACTED
	visible := []userAvailableGroup{
		{ID: 9, Name: "composite", Platform: service.PlatformCompositeREDACTED,
REDACTED
...
func TestBuildPlatformSections_OrdinaryGroupRemainsPlatformIsolated(t *testing.T) {
	ch := service.AvailableChannel{
		SupportedModels: []service.SupportedModel{
			{Name: "claude-sonnet-4-6", Platform: service.PlatformAnthropicREDACTED,
			{Name: "gpt-5", Platform: service.PlatformOpenAIREDACTED,
	REDACTED,
REDACTED
	visible := []userAvailableGroup{
		{ID: 1, Name: "anthropic-only", Platform: service.PlatformAnthropicREDACTED,
REDACTED
...
REDACTED
	...
	require.Len(t, sections, 2)
	require.Equal(t, service.PlatformAnthropic, sections[0].Platform)
	require.Equal(t, []int64{1, 9REDACTED, []int64{
...
REDACTED
	...
	sections := buildPlatformSections(service.AvailableChannel{
		SupportedModels: []service.SupportedModel{{Name: "invalid-without-platform"REDACTEDREDACTED,
REDACTED, visible)
