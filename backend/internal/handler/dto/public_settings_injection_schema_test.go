...
func TestPublicSettingsInjectionPayload_SchemaDoesNotDrift(t *testing.T) {
	injection := jsonTags(reflect.TypeOf(service.PublicSettingsInjectionPayload{REDACTED))
	dtoKeys := jsonTags(reflect.TypeOf(PublicSettings{REDACTED))
...
func jsonTags(t reflect.Type) map[string]struct{REDACTED {
	out := make(map[string]struct{REDACTED)
	...
	REDACTED
		...
		if name == "" {
			continue
	REDACTED
		out[name] = struct{REDACTED{REDACTED
REDACTED
	return out
REDACTED
