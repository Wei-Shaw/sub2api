...
type FailoverState struct {
	SwitchCount           int
	MaxSwitches           int
	FailedAccountIDs      map[int64]struct{REDACTED
	...
	profitVetoedAccountIDs map[int64]struct{REDACTED
...
func NewFailoverState(maxSwitches int, hasBoundSession bool) *FailoverState {
	return &FailoverState{
		...
		profitVetoedAccountIDs: make(map[int64]struct{REDACTED),
REDACTED
REDACTED
...
func (s *FailoverState) RecordProfitVeto(accountID int64) FailoverAction {
	s.FailedAccountIDs[accountID] = struct{REDACTED{REDACTED
	if s.profitVetoedAccountIDs == nil {
		s.profitVetoedAccountIDs = make(map[int64]struct{REDACTED)
REDACTED
	s.profitVetoedAccountIDs[accountID] = struct{REDACTED{REDACTED
	s.profitVetoCount++
...
REDACTED
	if s.FailedAccountIDs == nil {
		s.FailedAccountIDs = make(map[int64]struct{REDACTED)
REDACTED
	s.FailedAccountIDs[accountID] = struct{REDACTED{REDACTED
...
func (s *FailoverState) ProfitVetoCount() int { return s.profitVetoCount REDACTED
...
REDACTED
	...
	for _, excludedAccountID := range failoverErr.ExcludedAccountIDs {
		if excludedAccountID > 0 {
			s.FailedAccountIDs[excludedAccountID] = struct{REDACTED{REDACTED
	REDACTED
REDACTED
...
REDACTED
	...
	REDACTED
		...
		for id := range s.profitVetoedAccountIDs {
			s.FailedAccountIDs[id] = struct{REDACTED{REDACTED
	REDACTED
		return FailoverContinue
REDACTED
	return FailoverExhausted
