...
REDACTED
	res := profitVetoLoopResult{REDACTED
...
func TestProfitVetoAfter503DoesNotLivelock(t *testing.T) {
	...
	fs.SwitchCount = 1
	fs.FailedAccountIDs[1] = struct{REDACTED{REDACTED

	start := time.Now()
	res := runProfitVetoLoop(t, fs, []int64{1REDACTED, map[int64]bool{1: trueREDACTED, 50)
...
func TestProfitVetoKeepsBackoffUsefulForHealthyAccount(t *testing.T) {
	...
	res := runProfitVetoLoop(t, fs, []int64{2, 1REDACTED, map[int64]bool{2: trueREDACTED, 50)
