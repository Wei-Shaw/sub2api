...
func (m *mockTempUnscheduler) TempUnscheduleRetryableError(_ context.Context, accountID int64, failoverErr *service.UpstreamFailoverError) {
	m.calls = append(m.calls, tempUnscheduleCall{accountID: accountID, failoverErr: failoverErrREDACTED)
REDACTED

func TestSameAccountRetryDelayFor(t *testing.T) {
	capacityErr := &service.UpstreamFailoverError{RequestScopedTransient: trueREDACTED
	...
	for _, tc := range []struct {
		...
		want       time.Duration
REDACTED{
		{name: "first retry", retryCount: 1, want: 500 * time.MillisecondREDACTED,
		{name: "second retry", retryCount: 2, want: time.SecondREDACTED,
		{name: "third retry", retryCount: 3, want: 2 * time.SecondREDACTED,
		{name: "fourth retry", retryCount: 4, want: 4 * time.SecondREDACTED,
		{name: "fifth retry", retryCount: 5, want: 8 * time.SecondREDACTED,
		{name: "capped retry", retryCount: 10, want: 8 * time.SecondREDACTED,
REDACTED {
...
REDACTED
	...
	t.Run("non request scoped errors keep fixed delay", func(t *testing.T) {
		require.Equal(t, 500*time.Millisecond, sameAccountRetryDelayFor(&service.UpstreamFailoverError{REDACTED, 10))
REDACTED)
...
REDACTED)
	...
	t.Run("explicit oauth delay wins", func(t *testing.T) {
		err := &service.UpstreamFailoverError{SameAccountRetryDelay: 3 * time.SecondREDACTED
...
func TestEffectiveSameAccountRetryLimitHonorsErrorCapAndDisabledAccount(t *testing.T) {
	account := &service.Account{Type: service.AccountTypeAPIKey, Credentials: map[string]any{"pool_mode": true, "pool_mode_retry_count": float64(3)REDACTEDREDACTED
	require.Equal(t, 1, effectiveSameAccountRetryLimit(&service.UpstreamFailoverError{SameAccountRetryMax: 1REDACTED, account))
	account.Credentials["pool_mode_retry_count"] = float64(0)
	require.Equal(t, 0, effectiveSameAccountRetryLimit(&service.UpstreamFailoverError{SameAccountRetryMax: 1REDACTED, account))
REDACTED
...
REDACTED)
	...
	t.Run("503且未耗尽_等待后返回Continue并清除失败列表", func(t *testing.T) {
		...
		fs.FailedAccountIDs[100] = struct{REDACTED{REDACTED
		fs.SwitchCount = 1
