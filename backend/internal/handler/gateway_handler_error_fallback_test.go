...
// 而不是 silent EOF。非 /responses 路径走 legacy data:{"type":"error"REDACTED 分支。
...
REDACTED)
	...
	// handler 不得再追加 data:{"type":"error"REDACTED 帧，否则响应被污染成「JSON + 一行 data:」。
	t.Run("upstream 400 json passthrough via c.Data", func(t *testing.T) {
		w := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(w)
		c.Request = httptest.NewRequest(http.MethodPost, EndpointMessages, nil)
		before := c.Writer.Size()
		upstreamBody := []byte(`{"type":"error","error":{"type":"upstream_error","message":"Your Claude Code version (2.1.39) is below the minimum required version (2.1.81). Please update: npm update -g @anthropic-ai/claude-code"REDACTEDREDACTED`)
