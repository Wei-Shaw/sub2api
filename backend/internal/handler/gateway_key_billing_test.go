...
func TestGatewayHandlerKeyBillingInfoUsesUserOverride(t *testing.T) {
	...
	apiKey := &service.APIKey{
		UserID:  11,
		GroupID: &groupID,
		Group:   &service.Group{ID: groupID, RateMultiplier: 0.75REDACTED,
REDACTED
	c, w := newKeyBillingContext(apiKey)
	repo := &keyBillingUserGroupRateRepo{rate: &userRateREDACTED
...
REDACTED)
	...
	t.Run("ungrouped API key", func(t *testing.T) {
		c, w := newKeyBillingContext(&service.APIKey{REDACTED)
		newKeyBillingHandler(nil).KeyBillingInfo(c)
...
REDACTED)
	...
	t.Run("missing billing service", func(t *testing.T) {
		...
		c, w := newKeyBillingContext(&service.APIKey{
			UserID:  11,
			GroupID: &groupID,
			Group:   &service.Group{ID: groupID, RateMultiplier: 1REDACTED,
	REDACTED)
		(&GatewayHandler{REDACTED).KeyBillingInfo(c)
...
REDACTED)
	...
	t.Run("rate lookup failure matches billing fallback", func(t *testing.T) {
		...
		c, w := newKeyBillingContext(&service.APIKey{
			...
			Group:   &service.Group{ID: groupID, RateMultiplier: 1REDACTED,
	REDACTED)
		newKeyBillingHandler(&keyBillingUserGroupRateRepo{err: errors.New("database password leaked")REDACTED).KeyBillingInfo(c)
		require.Equal(t, http.StatusOK, w.Code)
		var got keyBillingInfoResponse
		require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
...
func TestGatewayHandlerKeyBillingInfoSharesBillingResolverCacheByPlatform(t *testing.T) {
	for _, tc := range []struct {
		...
		openAI   bool
REDACTED{
		{name: "anthropic", platform: service.PlatformAnthropicREDACTED,
		{name: "openai", platform: service.PlatformOpenAI, openAI: trueREDACTED,
		{name: "grok", platform: service.PlatformGrok, openAI: trueREDACTED,
REDACTED {
		t.Run(tc.name, func(t *testing.T) {
			groupID := int64(7)
			oldRate, newRate := 0.5, 1.8
			repo := &keyBillingUserGroupRateRepo{rate: &oldRateREDACTED
			gatewayService := newKeyBillingGatewayService(repo)
			openAIGatewayService := newKeyBillingOpenAIGatewayService(repo)
			h := &GatewayHandler{
