...
func TestGatewayModels_GeminiGroupFallsBackToGeminiModels(t *testing.T) {
	...
	h := newGatewayModelsHandlerForTest(
		&gatewayModelsAccountRepoStub{
			byGroup: map[int64][]service.Account{
				groupID: {
					{ID: 1, Platform: service.PlatformGeminiREDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
	)
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{ID: groupID, Platform: service.PlatformGeminiREDACTED,
REDACTED)
...
func TestGatewayModels_Grok45AdvertisesReasoningEffortForGrokBuild(t *testing.T) {
	...
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/models", nil)
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{ID: groupID, Platform: service.PlatformGrokREDACTED,
REDACTED)
...
REDACTED, model.ReasoningEfforts)
REDACTED
...
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"gemini-2.5-flash"REDACTED, modelIDsForTest(got.Data))
REDACTED
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: false,
				Models:  []string{"gpt-5.5"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"gpt-5.4", "gpt-5.5"REDACTED, modelIDsForTest(got.Data))
REDACTED
...
func TestGatewayModels_CustomModelsListFiltersAndOrdersMappedModels(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"gpt-5.5", "missing-model", "gpt-5.4"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"gpt-5.5", "gpt-5.4"REDACTED, modelIDsForTest(got.Data))
REDACTED
...
func TestGatewayModels_CompositeCustomModelsListFiltersAcrossConcretePlatforms(t *testing.T) {
	...
	h := newGatewayModelsHandlerForTest(
		&gatewayModelsAccountRepoStub{
			byGroup: map[int64][]service.Account{
				...
				REDACTED
							"model_mapping": map[string]any{"kimi-custom": "kimi-upstream"REDACTED,
					REDACTED,
				REDACTED,
					{
						ID:       5,
				...
				REDACTED
							"model_mapping": map[string]any{"glm-custom": "glm-upstream"REDACTED,
					REDACTED,
				REDACTED,
					{
						ID:       6,
				...
				REDACTED
							"model_mapping": map[string]any{"deepseek-custom": "deepseek-upstream"REDACTED,
					REDACTED,
				REDACTED,
			REDACTED,
		REDACTED,
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"gemini-2.5-flash", "missing-model", "ag-custom-model", "gpt-5.5", "kimi-custom", "glm-custom", "deepseek-custom"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"gemini-2.5-flash", "ag-custom-model", "gpt-5.5", "kimi-custom", "glm-custom", "deepseek-custom"REDACTED, modelIDsForTest(got.Data))
REDACTED
...
func TestGatewayModels_CompositeUnmappedAccountsFallbackToLinkedPlatformsOnly(t *testing.T) {
	...
	h := newGatewayModelsHandlerForTest(
		&gatewayModelsAccountRepoStub{
			byGroup: map[int64][]service.Account{
				groupID: {
					{ID: 1, Platform: service.PlatformOpenAIREDACTED,
					{ID: 2, Platform: service.PlatformGrokREDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
	)
	...
	h := newGatewayModelsHandlerForTest(
		&gatewayModelsAccountRepoStub{
			byGroup: map[int64][]service.Account{
				groupID: {
					{ID: 1, Platform: service.PlatformOpenAIREDACTED,
					{ID: 2, Platform: service.PlatformKimiREDACTED,
					{ID: 3, Platform: service.PlatformZhipuREDACTED,
					{ID: 4, Platform: service.PlatformDeepseekREDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
	)
...
REDACTED
	for _, platform := range []string{service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseekREDACTED {
...
func TestGatewayModels_CustomModelsListKeepsConcreteModelAllowedByWildcardMapping(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"claude-sonnet-4-6"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"claude-sonnet-4-6"REDACTED, modelIDsForTest(got.Data))
REDACTED
...
func TestGatewayModels_AnthropicCustomModelsListIncludesOAuthClaudeAndMappedDeepSeek(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"claude-fable-5", "claude-opus-4-8", "deepseek-v4-pro"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"claude-fable-5", "claude-opus-4-8", "deepseek-v4-pro"REDACTED, modelIDsForTest(got.Data))
REDACTED
...
func TestGatewayModels_AnthropicCustomModelsListDisabledKeepsMappedModelList(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: false,
				Models:  []string{"claude-fable-5", "deepseek-v4-pro"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"deepseek-v4-pro"REDACTED, modelIDsForTest(got.Data))
REDACTED
...
func TestGatewayModels_AnthropicCustomModelsListIncludesOAuthClaudeWithoutMappings(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"claude-opus-4-6-thinking", "claude-sonnet-4-5"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"claude-opus-4-6-thinking", "claude-sonnet-4-5"REDACTED, modelIDsForTest(got.Data))
REDACTED
...
func TestGatewayModels_CustomModelsListFiltersDefaultFallbackModels(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"gpt-5.5", "legacy-gpt-2024", "gpt-5.4"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
...
func TestGatewayModels_OpenAICustomModelsListKeepsOpenAIResponseShapeForDefaultFallback(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"gpt-5.5", "gpt-5.4"REDACTED,
		REDACTED,
	REDACTED,
REDACTED)
