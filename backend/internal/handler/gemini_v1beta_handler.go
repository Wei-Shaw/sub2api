...
REDACTED
	...
	if err != nil {
		...
		return
REDACTED
	// URL 里的模型名最终会被拼进上游 /v1beta/models/{modelREDACTED:{actionREDACTED，
...
REDACTED
	...
	REDACTED else if sessionKey != "" && sessionBoundAccountID == 0 && !cleanedForUnknownBinding && bytes.Contains(body, []byte(`"thoughtSignature"`)) {
