...
func (r *toggleSettingRepo) Get(context.Context, string) (*service.Setting, error) { return nil, nil REDACTED
...
REDACTED
func (r *toggleSettingRepo) SetMultiple(context.Context, map[string]string) error { return nil REDACTED
func (r *toggleSettingRepo) GetAll(context.Context) (map[string]string, error) {
	return map[string]string{REDACTED, nil
REDACTED
func (r *toggleSettingRepo) Delete(context.Context, string) error { return nil REDACTED

type passthroughEncryptor struct{REDACTED

func (passthroughEncryptor) Encrypt(plaintext string) (string, error)  { return plaintext, nil REDACTED
func (passthroughEncryptor) Decrypt(ciphertext string) (string, error) { return ciphertext, nil REDACTED

type noopImageStorage struct{REDACTED
...
func TestAsyncImageEnablesWithoutRestart(t *testing.T) {
	...
	repo := &toggleSettingRepo{values: map[string]string{REDACTEDREDACTED
	backup := service.NewBackupService(repo, &config.Config{REDACTED, passthroughEncryptor{REDACTED, nil, nil)
	factory := func(context.Context, *config.ImageStorageConfig) (service.ImageStorage, error) {
		return noopImageStorage{REDACTED, nil
REDACTED
	settings := service.NewImageStorageSettingService(repo, passthroughEncryptor{REDACTED, backup, factory, config.ImageStorageConfig{REDACTED)

	store := &asyncImageMemoryStore{tasks: make(map[string]*service.ImageTaskRecord)REDACTED
	tasks := service.NewImageTaskServiceWithResolver(store, settings.Resolver(), time.Hour, time.Minute)

	h := &AsyncImageHandler{tasks: tasksREDACTED
	h.execute = func(_ string, c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"created": 1, "data": []gin.H{{"url": "https://upstream.test/i.png"REDACTEDREDACTEDREDACTED)
REDACTED
	...
	router.Use(func(c *gin.Context) {
		...
		c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
			...
			Group: &service.Group{ID: groupID, Platform: service.PlatformOpenAI, AllowImageGeneration: trueREDACTED,
	REDACTED)
		c.Next()
REDACTED)
	...
	submit := func() *httptest.ResponseRecorder {
		req := httptest.NewRequest(http.MethodPost, "/v1/images/generations/async",
			strings.NewReader(`{"model":"gpt-image-1","prompt":"a lighthouse"REDACTED`))
...
REDACTED
	...
	_, err = settings.Update(context.Background(), service.ImageStorageSettings{Enabled: falseREDACTED)
REDACTED
