...
func (h *ModelPlazaHandler) Get(c *gin.Context) {
	...
	var hiddenGroups map[int64]struct{}
	...
	if authed {
		...
		hiddenGroups, err = h.apiKeyService.GetUserHiddenGroupIDSet(c.Request.Context(), subject.UserID)
		if err != nil {
			response.ErrorFrom(c, err)
			return
		}
	...
	visible := filterPlazaVisibleGroups(groups, allowedGroups, restrictPublicGroups, hiddenGroups)
...
func filterPlazaVisibleGroups(
	...
	hiddenGroupSets ...map[int64]struct{},
) []service.PlazaGroup {
	var hiddenGroups map[int64]struct{}
	if len(hiddenGroupSets) > 0 {
		hiddenGroups = hiddenGroupSets[0]
	}
	visible := make([]service.PlazaGroup, 0, len(groups))
	for _, g := range groups {
		if _, hiddenForUser := hiddenGroups[g.ID]; hiddenForUser {
			continue
		}
