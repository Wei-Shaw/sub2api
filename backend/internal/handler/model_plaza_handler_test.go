...
func plazaGroups() []service.PlazaGroup {
	return []service.PlazaGroup{
		{ID: 1, Name: "public-standard", Platform: "anthropic", SubscriptionType: "standard", RateMultiplier: 1REDACTED,
		{ID: 2, Name: "exclusive-a", Platform: "anthropic", IsExclusive: true, RateMultiplier: 0.5REDACTED,
		{ID: 3, Name: "public-subscription", Platform: "openai", SubscriptionType: "subscription", RateMultiplier: 1REDACTED,
		{ID: 4, Name: "exclusive-b", Platform: "openai", IsExclusive: true, RateMultiplier: 0.8REDACTED,
REDACTED
REDACTED
...
func TestFilterPlazaVisibleGroups_AuthedSeesGrantedExclusive(t *testing.T) {
	...
	allowed := map[int64]struct{REDACTED{2: {REDACTEDREDACTED
	...
	for _, g := range visible {
		ids = append(ids, g.ID)
REDACTED
	require.ElementsMatch(t, []int64{1, 2, 3REDACTED, ids)
REDACTED
...
func TestFilterPlazaVisibleGroups_AuthedEmptySetSeesNoExclusive(t *testing.T) {
	...
	visible := filterPlazaVisibleGroups(plazaGroups(), map[int64]struct{REDACTED{REDACTED)
	require.Len(t, visible, 2)
REDACTED

func TestModelPlazaHandler_NilSettingServiceFailsClosed404(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := &ModelPlazaHandler{REDACTED // settingService == nil → fail-closed
...
REDACTED
	...
	dto := toModelPlazaGroupDTO(&g, map[int64]float64{2: 0.5REDACTED)
		...
		Models: []service.PlazaModel{{
			...
			Pricing: &service.ChannelModelPricing{
				...
				Intervals: []service.PricingInterval{
					{MinTokens: 0, MaxTokens: &maxTokens, TierLabel: "≤272K", InputPrice: testPtr(2.5e-6)REDACTED,
					{MinTokens: 272000, TierLabel: ">272K", InputPrice: testPtr(5e-6)REDACTED,
			REDACTED,
		REDACTED,
			OfficialPricing: &service.PlazaOfficialPricing{
				InputPrice: testPtr(2.5e-6),
...
func testPtr(v float64) *float64 { return &v REDACTED
...
func TestToModelPlazaGroupDTO_TimePricing(t *testing.T) {
	g := service.PlazaGroup{
		...
		Models: []service.PlazaModel{{
			Name:     "deepseek-chat",
			Platform: "deepseek",
			Pricing:  &service.ChannelModelPricing{BillingMode: service.BillingModeToken, InputPrice: testPtr(0.28e-6)REDACTED,
	...
	REDACTED, {
			...
			Platform: "deepseek",
			Pricing:  &service.ChannelModelPricing{BillingMode: service.BillingModeToken, InputPrice: testPtr(0.56e-6)REDACTED,
