...
func TestFilterPlazaVisibleGroups_RestrictionAndHiddenGroupsBothApply(t *testing.T) {
	allowed := map[int64]struct{}{1: {}, 2: {}}
	hidden := map[int64]struct{}{1: {}}

	visible := filterPlazaVisibleGroups(plazaGroups(), allowed, true, hidden)
	ids := make([]int64, 0, len(visible))
	for _, g := range visible {
		ids = append(ids, g.ID)
	}

	require.ElementsMatch(t, []int64{2}, ids)
}
