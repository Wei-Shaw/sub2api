...
func TestOpenAIBodyLimitFailoverExhausted_ReturnsRedactedJSON413(t *testing.T) {
	...
	(&OpenAIGatewayHandler{REDACTED).handleFailoverExhausted(c, bodyLimitFailoverTestError(), false)
...
func TestOpenAIBodyLimitFailoverExhausted_ReturnsRedactedResponsesSSE(t *testing.T) {
	...
	(&OpenAIGatewayHandler{REDACTED).handleFailoverExhausted(c, bodyLimitFailoverTestError(), true)
...
func bodyLimitFailoverTestError() *service.UpstreamFailoverError {
	return &service.UpstreamFailoverError{
		...
		ResponseBody:      []byte(`{"error":{"message":"proxy limit secret=must-not-leak"REDACTEDREDACTED`),
