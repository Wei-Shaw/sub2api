...
func (h *OpenAIGatewayHandler) Embeddings(c *gin.Context) {
	...
	channelMapping, _ := h.gatewayService.ResolveChannelMappingAndRestrict(c.Request.Context(), apiKey.GroupID, "openai", reqModel)
	...
	if err != nil {
		reqLog.Warn("openai_embeddings.user_slot_acquire_failed", zap.Error(err))
	...
	// Billing pre-check
	ticket, err := h.billingCacheService.PrepareBillingCheck(c.Request.Context(), apiKey.User, apiKey, apiKey.Group, subscription)
	if err != nil {
		reqLog.Info("openai_embeddings.billing_check_failed", zap.Error(err))
		status, code, message, metadata := billingErrorDetails(err)
		if retryAfter, ok := metadata["retry_after"]; ok && retryAfter != "" {
	...
	for {
		...
		if !account.SupportsOpenAIEndpointCapability(service.OpenAIEndpointCapabilityEmbeddings) {
			failedAccountIDs[account.ID] = struct{}{}
			if switchCount >= maxAccountSwitches {
				h.errorResponse(c, http.StatusServiceUnavailable, "api_error", "No accounts support embeddings")
				return
			}
			switchCount++
			continue
		...
		if selection.Acquired {
			...
			releaseFunc, fastAcquired, slotErr := h.concurrencyHelper.TryAcquireAccountSlot(
				c.Request.Context(), account.ID, selection.WaitPlan.MaxConcurrency)
			if slotErr != nil {
				reqLog.Warn("openai_embeddings.account_slot_acquire_failed",
					zap.Int64("account_id", account.ID), zap.Error(slotErr))
			...
			if fastAcquired {
				...
				waitRelease, waitErr := h.concurrencyHelper.AcquireAccountSlotWithWaitTimeout(
					c, account.ID, selection.WaitPlan.MaxConcurrency, selection.WaitPlan.Timeout, false, &streamStarted)
				if waitErr != nil {
					reqLog.Warn("openai_embeddings.account_slot_wait_failed",
						zap.Int64("account_id", account.ID), zap.Error(waitErr))
