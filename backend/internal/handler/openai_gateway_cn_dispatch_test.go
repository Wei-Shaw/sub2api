...
func TestAllowOpenAICompatibleMessagesDispatch_CNProvidersExempt(t *testing.T) {
	...
	for _, platform := range []string{service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformGrokREDACTED {
		apiKey := &service.APIKey{Group: &service.Group{Platform: platform, AllowMessagesDispatch: falseREDACTEDREDACTED
		require.True(t, allowOpenAICompatibleMessagesDispatch(nil, apiKey),
			"%s 分组必须豁免 allow_messages_dispatch 闸门", platform)
REDACTED

	// 非回归：openai 分组仍受开关控制。
	openaiOff := &service.APIKey{Group: &service.Group{Platform: service.PlatformOpenAI, AllowMessagesDispatch: falseREDACTEDREDACTED
	require.False(t, allowOpenAICompatibleMessagesDispatch(nil, openaiOff))
	openaiOn := &service.APIKey{Group: &service.Group{Platform: service.PlatformOpenAI, AllowMessagesDispatch: trueREDACTEDREDACTED
...
func TestAllowOpenAICompatibleMessagesDispatch_CompositeResolvedTargets(t *testing.T) {
	...
	newCompositeCtx := func(model string) (*gin.Context, *service.APIKey) {
		...
		apiKey := &service.APIKey{Group: &service.Group{Platform: service.PlatformComposite, AllowMessagesDispatch: falseREDACTEDREDACTED
		ensureCompositeTargetPlatform(c, apiKey, model)
		return c, apiKey
REDACTED

	// 解析到 grok/CN 目标：与对应独立分组同语义豁免。
	for _, model := range []string{"grok-4.3", "kimi-k2-thinking", "glm-5.2", "deepseek-v3.2"REDACTED {
...
REDACTED
	...
	require.False(t, allowOpenAICompatibleMessagesDispatch(cNone,
		&service.APIKey{Group: &service.Group{Platform: service.PlatformComposite, AllowMessagesDispatch: falseREDACTEDREDACTED))
REDACTED
...
func TestResolveOpenAIMessagesDispatchMappedModel_CompositeCNTargetsSkipGroupMapping(t *testing.T) {
	...
	for _, model := range []string{"kimi-k2-thinking", "glm-5.2", "deepseek-v3.2", "grok-4.3"REDACTED {
		c, _ := gin.CreateTestContext(httptest.NewRecorder())
		c.Request = httptest.NewRequest("POST", "/v1/messages", nil)
