...
func TestNormalizeOpenAIResponsesCompactRequest_BodySignalStreamTrueMarksClientStream(t *testing.T) {
	h := &OpenAIGatewayHandler{REDACTED
	body := []byte(`{"model":"gpt-5.5","stream":true,"input":[{"type":"compaction_trigger"REDACTED]REDACTED`)
	c := newCompactBodySignalTestContext(t, "/v1/responses", body)
...
func TestNormalizeOpenAIResponsesCompactRequest_BodySignalStreamFalseNotMarked(t *testing.T) {
	...
	for name, body := range map[string][]byte{
		"stream_false":  []byte(`{"model":"gpt-5.5","stream":false,"input":[{"type":"compaction_trigger"REDACTED]REDACTED`),
		"stream_absent": []byte(`{"model":"gpt-5.5","input":[{"type":"compaction_trigger"REDACTED]REDACTED`),
REDACTED {
		c := newCompactBodySignalTestContext(t, "/v1/responses", body)
		_, ok := h.normalizeOpenAIResponsesCompactRequest(c, zap.NewNop(), body)
