...
REDACTED
	...
	action := state.HandleFailoverError(context.Background(), &mockTempUnscheduler{REDACTED, 71, service.PlatformGrok, 0, stopErr)
	...
	(&GatewayHandler{REDACTED).handleCCFailoverExhausted(c, state.LastFailoverErr, false)
...
func TestGatewayChatAntigravityCredentialFailureReturnsActionableMessage(t *testing.T) {
	...
	(&GatewayHandler{REDACTED).handleCCFailoverExhausted(c, &service.UpstreamFailoverError{
		...
		ResponseBody:      []byte(`{"error":{"message":"Invalid bearer token","refresh_token":"must-not-leak"REDACTEDREDACTED`),
REDACTED, false)
...
func TestOpenAIAccessStateCredentialFailureUsesTypedSafeResponse(t *testing.T) {
	...
	(&OpenAIGatewayHandler{REDACTED).handleFailoverExhausted(c, &service.UpstreamFailoverError{
		...
		ResponseBody:      []byte(`{"error":{"message":"Your workspace is deactivated","token":"must-not-leak"REDACTEDREDACTED`),
REDACTED, false)
...
func TestOpenAICapacityFailoverExhaustionPreservesMessageAsServerError(t *testing.T) {
	...
	failoverErr := &service.UpstreamFailoverError{
		...
		ResponseBody:           []byte(`{"error":{"code":"server_is_overloaded","message":"` + message + `"REDACTEDREDACTED`),
...
REDACTED
	...
	t.Run("native_openai", func(t *testing.T) {
		recorder := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(recorder)
		(&OpenAIGatewayHandler{REDACTED).handleFailoverExhausted(c, failoverErr, false)
		require.Equal(t, http.StatusServiceUnavailable, recorder.Code)
...
REDACTED)
	...
	t.Run("responses_compat", func(t *testing.T) {
		recorder := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(recorder)
		(&GatewayHandler{REDACTED).handleResponsesFailoverExhausted(c, failoverErr, false)
		require.Equal(t, http.StatusServiceUnavailable, recorder.Code)
...
REDACTED)
	...
	t.Run("anthropic_compat", func(t *testing.T) {
		recorder := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(recorder)
		(&OpenAIGatewayHandler{REDACTED).handleAnthropicFailoverExhausted(c, failoverErr, false)
		require.Equal(t, http.StatusServiceUnavailable, recorder.Code)
...
REDACTED
	...
	(&GatewayHandler{REDACTED).handleResponsesFailoverExhausted(c, &service.UpstreamFailoverError{
		...
		ResponseBody: []byte(`{"error":{"message":"fallback failure"REDACTEDREDACTED`),
REDACTED, true)
	...
	(&GatewayHandler{REDACTED).handleResponsesFailoverExhausted(markerContext, &service.UpstreamFailoverError{
...
REDACTED
	...
	(&GatewayHandler{REDACTED).handleResponsesFailoverExhausted(heartbeatContext, &service.UpstreamFailoverError{
		StatusCode: http.StatusBadGateway,
REDACTED, true)
...
func TestGatewayChatInferenceExhaustionRestoresRetryAfter(t *testing.T) {
	...
	(&GatewayHandler{REDACTED).handleCCFailoverExhausted(c, &service.UpstreamFailoverError{
		StatusCode:      http.StatusTooManyRequests,
		ResponseHeaders: http.Header{"Retry-After": []string{"45"REDACTEDREDACTED,
REDACTED, false)
...
func TestInferenceFailoverExhaustionRestoresRetryAfter(t *testing.T) {
	...
	h.handleFailoverExhausted(c, &service.UpstreamFailoverError{
		StatusCode:      http.StatusTooManyRequests,
		ResponseHeaders: http.Header{"Retry-After": []string{"17"REDACTEDREDACTED,
REDACTED, false)
...
func TestFailoverExhaustionRejectsSecretBearingRetryAfter(t *testing.T) {
	...
	h.handleFailoverExhausted(c, &service.UpstreamFailoverError{
		StatusCode:      http.StatusTooManyRequests,
		ResponseHeaders: http.Header{"Retry-After": []string{"refresh_token=must-not-leak"REDACTEDREDACTED,
REDACTED, false)
...
func TestFailoverExhaustionRejectsFarFutureRetryAfterDate(t *testing.T) {
	...
	h.handleFailoverExhausted(c, &service.UpstreamFailoverError{
		...
		ResponseHeaders: http.Header{
			"Retry-After": []string{time.Now().Add(30 * 24 * time.Hour).UTC().Format(http.TimeFormat)REDACTED,
	REDACTED,
REDACTED, false)
...
func TestFailoverExhaustionAllowsBoundedRetryAfterDate(t *testing.T) {
	...
	h.handleFailoverExhausted(c, &service.UpstreamFailoverError{
		StatusCode:      http.StatusTooManyRequests,
		ResponseHeaders: http.Header{"Retry-After": []string{retryAfterREDACTEDREDACTED,
REDACTED, false)
...
func TestOpsClassificationTreatsCredentialFailureAsAuthNotInference(t *testing.T) {
	...
	c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{
		{Stage: string(service.GatewayFailureStageInference), UpstreamStatusCode: http.StatusForbidden, Message: "stale inference message", Detail: "stale inference detail"REDACTED,
		{
...
func TestOpsRecoveredCredentialFailoverDoesNotCreateRequestError(t *testing.T) {
	...
	router.GET("/openai/v1/responses", func(c *gin.Context) {
		c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{
			{Stage: string(service.GatewayFailureStageInference), UpstreamStatusCode: http.StatusForbidden, Message: "earlier inference failure"REDACTED,
			{
