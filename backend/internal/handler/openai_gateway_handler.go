...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	initialSessionHash := sessionHash
	sessionHash, promptCacheKey = resolveOpenAIMessagesMetadataSession(sessionHash, promptCacheKey, reqModel, body)
	if initialSessionHash == "" && sessionHash != "" && strings.TrimSpace(gjson.GetBytes(body, "metadata.user_id").String()) != "" {
...
func (h *OpenAIGatewayHandler) handleStreamingAwareError(c *gin.Context, status int, errType, message string, streamStarted bool) {
	if streamStarted {
		...
		flusher, ok := c.Writer.(http.Flusher)
		if ok {
			payload, err := json.Marshal(openAIErrorBody(c, errType, message))
			if err != nil {
				payload = []byte(`{"error":{"type":` + strconv.Quote(errType) + `,"message":` + strconv.Quote(message) + `}}`)
			}
