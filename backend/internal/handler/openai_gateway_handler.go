...
// body-signal 提升只允许发生在这里，避免误伤 /responses/{idREDACTED/... 形态的请求。
...
func recordOpenAIProfitVeto(failedAccountIDs map[int64]struct{REDACTED, accountID int64, vetoCount *int) bool {
	failedAccountIDs[accountID] = struct{REDACTED{REDACTED
	*vetoCount++
...
REDACTED
	...
	cyberTurnBodies := map[int][]byte{1: append([]byte(nil), firstMessage...)REDACTED
...
REDACTED
	...
	REDACTED
		...
		turnChannelMapping.Store(&openAIWSTurnChannelMappingSnapshot{turn: 1, mapping: channelMappingWSREDACTED)
		...
		REDACTED,
			...
			REDACTED
				turnChannelMapping.Store(&openAIWSTurnChannelMappingSnapshot{turn: turn, mapping: mappingREDACTED)
...
REDACTED
	...
	REDACTED
		...
		flusher, ok := c.Writer.(http.Flusher)
		if ok {
			errorObject := gin.H{"type": errType, "message": messageREDACTED
			if code != "" {
				errorObject["code"] = code
		REDACTED
			payload, err := json.Marshal(gin.H{"error": errorObjectREDACTED)
			if err != nil {
				payload = []byte(`{"error":{"type":"upstream_error","message":"Upstream request failed"REDACTEDREDACTED`)
		REDACTED
...
func buildCyberSessionBlockWritePlan(apiKeyID int64, c *gin.Context, body []byte) cyberSessionBlockWritePlan {
	plan := cyberSessionBlockWritePlan{REDACTED
