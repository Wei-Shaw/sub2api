...
func TestOpenAIForwardSucceededForScheduling(t *testing.T) {
	...
	require.True(t, openAIForwardSucceededForScheduling(&service.OpenAIForwardResult{REDACTED))
...
func TestOpenAIEnsureForwardErrorResponse_AfterDeltaAppendsSingleValidResponseFailed(t *testing.T) {
	...
	delta := `{"type":"response.output_text.delta","delta":"ok","sequence_number":1REDACTED`
...
REDACTED)
	...
	t.Run("uses_family_default_when_no_override", func(t *testing.T) {
		...
		require.Equal(t, "gpt-5.4-mini", resolveOpenAIMessagesDispatchMappedModel(nil, apiKey, "REDACTED"))
REDACTED)
	...
	t.Run("returns_empty_for_non_claude_or_missing_group", func(t *testing.T) {
		...
		require.Empty(t, resolveOpenAIMessagesDispatchMappedModel(nil, &service.APIKey{REDACTED, "claude-sonnet-4-5-20250929"))
		require.Empty(t, resolveOpenAIMessagesDispatchMappedModel(nil, &service.APIKey{Group: &service.Group{REDACTEDREDACTED, "gpt-5.4"))
REDACTED)

	t.Run("grok_group_maps_claude_cli_model_to_grok_default", func(t *testing.T) {
		original := xai.RuntimeModelMappingOptions()
		t.Cleanup(func() { xai.SetRuntimeModelMappingOptions(original) REDACTED)
		xai.SetRuntimeModelMappingOptions(xai.ModelMappingOptions{EnableCrossClientMap: trueREDACTED)
		apiKey := &service.APIKey{
			Group: &service.Group{
				Platform: service.PlatformGrok,
		REDACTED,
...
func TestOpenAIGatewayMessagesDispatchGateAllowsGrokGroups(t *testing.T) {
	...
	t.Run("openai_group_without_dispatch_flag_is_rejected", func(t *testing.T) {
		...
		c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", strings.NewReader(`{"model":"claude-sonnet-4-5","messages":[{"role":"user","content":"hi"REDACTED]REDACTED`))
		groupID := int64(4101)
		c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{
			ID:      5101,
			GroupID: &groupID,
			User:    &service.User{ID: 6101REDACTED,
			Group: &service.Group{
				ID:                    groupID,
				Platform:              service.PlatformOpenAI,
		...
		REDACTED,
	REDACTED)
		c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 6101, Concurrency: 1REDACTED)
...
REDACTED)
	...
	t.Run("grok_group_without_dispatch_flag_reaches_gateway_dependencies", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", strings.NewReader(`{"model":"grok-4.3","messages":[{"role":"user","content":"hi"REDACTED]REDACTED`))
		groupID := int64(4102)
		c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{
			ID:      5102,
			GroupID: &groupID,
			User:    &service.User{ID: 6102REDACTED,
			Group: &service.Group{
				ID:                    groupID,
				Platform:              service.PlatformGrok,
				AllowMessagesDispatch: false,
		REDACTED,
	REDACTED)
		c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 6102, Concurrency: 1REDACTED)
...
func TestOpenAIResponses_RejectsHTTPContinuationOwnedByAnotherUser(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(
		`{"model":"gpt-5.1","stream":false,"previous_response_id":"resp_other_tenant","input":"hello"REDACTED`,
	))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{
		...
		UserID:  2,
		GroupID: &groupID,
		User:    &service.User{ID: 2REDACTED,
REDACTED)
	c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 2, Concurrency: 1REDACTED)
...
func TestOpenAIResponses_RejectsUnownedHTTPContinuation(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(
		`{"model":"gpt-5.1","stream":false,"previous_response_id":"resp_unknown","input":"hello"REDACTED`,
	))
	c.Request.Header.Set("Content-Type", "application/json")

	groupID := int64(2)
	c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{ID: 101, UserID: 1, GroupID: &groupID, User: &service.User{ID: 1REDACTEDREDACTED)
	c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 1, Concurrency: 1REDACTED)
...
func TestOpenAIResponsesWebSocket_PassthroughTracksModelPerTurn(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload:  `{"type":"response.create","model":"sol","stream":falseREDACTED`,
		secondPayload: `{"type":"response.create","model":"terra","stream":falseREDACTED`,
		channelMapping: map[string]string{
			"sol":   "sol-channel",
			"terra": "terra-channel",
	REDACTED,
...
func TestOpenAIResponsesWebSocket_UnchangedChannelTargetOutsideAccountMappingKeysRemainsValid(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload:  `{"type":"response.create","model":"public-alias","stream":falseREDACTED`,
		secondPayload: `{"type":"response.create","stream":falseREDACTED`,
		channelMapping: map[string]string{
...
func TestOpenAIResponsesWebSocket_PassthroughKeepsTurnMappingSnapshot(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload:  `{"type":"response.create","model":"sol","stream":falseREDACTED`,
		secondPayload: `{"type":"response.create","model":"sol","stream":falseREDACTED`,
		channelMapping: map[string]string{
			"sol": "gpt-5.6-sol",
	REDACTED,
		...
		REDACTED
			_, err := channelSvc.Update(context.Background(), 7701, &service.UpdateChannelInput{
				ModelMapping: map[string]map[string]string{
					service.PlatformOpenAI: {"sol": "gpt-5.6-terra"REDACTED,
			REDACTED,
		REDACTED)
			return err
	REDACTED,
...
func TestOpenAIResponsesWebSocket_CtxPoolAppliesPerTurnMappingAndPreservesRequestedModel(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload:       `{"type":"response.create","model":"gpt-5.6-sol","stream":falseREDACTED`,
		secondPayload:      `{"type":"response.create","model":"gpt-5.6-terra","stream":falseREDACTED`,
...
REDACTED{
		{
			...
			mapping:          service.ChannelMappingResult{BillingModelSource: service.BillingModelSourceUpstreamREDACTED,
	...
	REDACTED,
		{
			...
			resultModel:      "gpt-image-2",
			mapping:          service.ChannelMappingResult{MappedModel: "gpt-5.6-sol", BillingModelSource: service.BillingModelSourceChannelMappedREDACTED,
			requestedModel:   "gpt-5.6-sol",
			upstreamModel:    "gpt-5.6-sol",
			wantBillingModel: "gpt-image-2",
	REDACTED,
		{
			...
			resultModel:      "gpt-image-2",
			mapping:          service.ChannelMappingResult{BillingModelSource: service.BillingModelSourceRequestedREDACTED,
	...
	REDACTED,
		{
			...
			resultModel:      "gpt-image-2",
			mapping:          service.ChannelMappingResult{MappedModel: "priced-channel-model", BillingModelSource: service.BillingModelSourceChannelMappedREDACTED,
			requestedModel:   "public-image-alias",
			upstreamModel:    "gpt-5.6-sol",
...
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := &service.OpenAIForwardResult{BillingModel: tt.resultModelREDACTED
...
REDACTED
			"model_mapping":         map[string]any{"public": "billing"REDACTED,
			"compact_model_mapping": map[string]any{"public": "compact-actual"REDACTED,
	REDACTED,
REDACTED

	reported := &service.OpenAIForwardResult{UpstreamModel: "observed-actual"REDACTED
...
func (u *openAIHTTPPassthroughAuthFailoverUpstream) Do(_ *http.Request, _ string, accountID int64, _ int) (*http.Response, error) {
	...
	if accountID == 9911 {
		return &http.Response{
			...
			Header:     http.Header{"Content-Type": []string{"application/json"REDACTEDREDACTED,
			Body:       io.NopCloser(strings.NewReader(`{"id":"resp_healthy","object":"response","model":"gpt-5.2","status":"completed","usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2REDACTEDREDACTED`)),
	REDACTED, nil
REDACTED
	return &http.Response{
		StatusCode: u.statusCode,
		Header:     http.Header{"Content-Type": []string{"application/json"REDACTEDREDACTED,
		Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"upstream credential rejected"REDACTEDREDACTED`)),
REDACTED, nil
REDACTED
...
func (u *openAIHTTPPassthroughSSERateLimitUpstream) Do(_ *http.Request, _ string, accountID int64, _ int) (*http.Response, error) {
	...
	body := strings.Join([]string{
		...
		`data: {"type":"response.created","response":{"id":"resp_rate_limited"REDACTEDREDACTED`,
		"",
		"event: response.failed",
		`data: {"type":"response.failed","response":{"id":"resp_rate_limited","status":"failed","error":{"code":"rate_limit_exceeded","message":"Concurrency limit exceeded for account, please retry later"REDACTEDREDACTEDREDACTED`,
		"",
REDACTED, "\n")
	return &http.Response{
		StatusCode: http.StatusOK,
...
func TestOpenAIResponses_APIKeyPassthroughPoolAuthFailureRetriesThenSwitchesToHealthyAccount(t *testing.T) {
	tests := []struct {
		...
		statusCode int
REDACTED{
		{name: "401", statusCode: http.StatusUnauthorizedREDACTED,
		{name: "403", statusCode: http.StatusForbiddenREDACTED,
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			REDACTED
						...
						"pool_mode":                    true,
						"pool_mode_retry_count":        float64(1),
						"pool_mode_retry_status_codes": []any{float64(tt.statusCode)REDACTED,
				REDACTED,
					Extra: map[string]any{"openai_passthrough": trueREDACTED,
			REDACTED,
				{
		...
		REDACTED
			...
			accountRepo := &openAIWSFailoverHandlerAccountRepoStub{accounts: accountsREDACTED
			upstream := &openAIHTTPPassthroughAuthFailoverUpstream{statusCode: tt.statusCodeREDACTED
...
REDACTED
	...
	accountRepo := &openAIWSFailoverHandlerAccountRepoStub{accounts: accountsREDACTED
	upstream := &openAIHTTPPassthroughSSERateLimitUpstream{REDACTED
	billingCacheSvc := service.NewBillingCacheService(nil, nil, nil, nil, nil, nil, cfg, nil)
	t.Cleanup(billingCacheSvc.Stop)
	gatewaySvc := service.NewOpenAIGatewayService(
		accountRepo,
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(`{"model":"gpt-5.6-sol","input":"hello","stream":trueREDACTED`))
	c.Request.Header.Set("Content-Type", "application/json")
	c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{
		ID: 1804, GroupID: &groupID,
		User:  &service.User{ID: 1704, Status: service.StatusActiveREDACTED,
		Group: &service.Group{ID: groupID, Platform: service.PlatformOpenAI, Status: service.StatusActiveREDACTED,
REDACTED)
	c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 1704, Concurrency: 0REDACTED)

	h.Responses(c)

	require.Equal(t, []int64{9912, 9912REDACTED, upstream.calls())
	...
	REDACTED()
		...
		REDACTED
			...
			response := fmt.Sprintf(
				`{"type":"response.completed","response":{"id":"resp_usage_e2e_%d","model":%q,"usage":{"input_tokens":2,"output_tokens":1REDACTEDREDACTEDREDACTED`,
				turn,
...
REDACTED
	...
	usageRepo := &openAIWSUsageHandlerUsageLogRepoStub{created: make(chan *service.UsageLog, turnCount)REDACTED
