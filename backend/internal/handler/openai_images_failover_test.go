...
func (u *openAIImagesFailoverHTTPUpstream) Do(_ *http.Request, _ string, accountID int64, _ int) (*http.Response, error) {
	...
	return &http.Response{
		...
		Header: http.Header{
			...
			"X-Request-Id": []string{"req_img_failover"REDACTED,
	REDACTED,
		Body: io.NopCloser(bytes.NewBufferString(
			"data: {\"type\":\"error\",\"error\":{\"type\":\"server_error\",\"code\":\"server_error\",\"message\":\"image backend unavailable\"REDACTEDREDACTED\n\n",
		)),
REDACTED, nil
REDACTED
...
REDACTED
	...
	upstream := &openAIImagesFailoverHTTPUpstream{REDACTED
	...
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","quality":"high","size":"1536x1024"REDACTED`)
...
REDACTED
	...
	require.Equal(t, []int64{1, 2REDACTED, upstream.calls())
