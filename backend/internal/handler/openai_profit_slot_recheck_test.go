...
func TestAcquireResponsesAccountSlotProfitRecheck(t *testing.T) {
	...
	gw := &service.OpenAIGatewayService{REDACTED
	groupID := int64(50)
...
REDACTED
	newSelection := func(account *service.Account) *service.AccountSelectionResult {
		return &service.AccountSelectionResult{
			Account:  account,
			Acquired: false,
			WaitPlan: &service.AccountWaitPlan{AccountID: account.ID, MaxConcurrency: 2, Timeout: time.Second, MaxWaiting: 2REDACTED,
	REDACTED
REDACTED

	t.Run("veto releases slot and requests reschedule without writing response", func(t *testing.T) {
		cache := &profitCountingConcurrencyCache{REDACTED
		h := newHandler(cache)
