...
REDACTED{
		{
			...
			path: "/v1/responses",
			body: `{"model":"gpt-5","stream":"true","input":"hello"REDACTED`,
			run: func(c *gin.Context) {
				(&GatewayHandler{REDACTED).Responses(c)
		REDACTED,
	REDACTED,
		{
			name: "gateway_responses_number_stream",
			path: "/v1/responses",
			body: `{"model":"gpt-5","stream":1,"input":"hello"REDACTED`,
			run: func(c *gin.Context) {
				(&GatewayHandler{REDACTED).Responses(c)
		REDACTED,
	REDACTED,
		{
			...
			body: `{"model":"gpt-5","stream":"true","messages":[{"role":"user","content":"hello"REDACTED]REDACTED`,
			run: func(c *gin.Context) {
				(&GatewayHandler{REDACTED).ChatCompletions(c)
		REDACTED,
	REDACTED,
		{
			name: "gateway_chat_completions_number_stream",
			path: "/v1/chat/completions",
			body: `{"model":"gpt-5","stream":1,"messages":[{"role":"user","content":"hello"REDACTED]REDACTED`,
			run: func(c *gin.Context) {
				(&GatewayHandler{REDACTED).ChatCompletions(c)
		REDACTED,
	REDACTED,
...
REDACTED{
		{
			...
			path: "/v1/responses",
			body: `{"model":"gpt-5","stream":false,"input":"hello"REDACTED`,
			run: func(c *gin.Context) {
				(&GatewayHandler{gatewayService: &service.GatewayService{REDACTEDREDACTED).Responses(c)
		REDACTED,
	REDACTED,
		{
			...
			body: `{"model":"gpt-5","stream":true,"messages":[{"role":"user","content":"hello"REDACTED]REDACTED`,
			run: func(c *gin.Context) {
				(&GatewayHandler{gatewayService: &service.GatewayService{REDACTEDREDACTED).ChatCompletions(c)
		REDACTED,
	REDACTED,
REDACTED
...
func newOpenAICompatibleStreamValidationContext(path, body string, claudeCodeOnly bool) (*gin.Context, *httptest.ResponseRecorder) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		ID:      11,
		GroupID: &groupID,
		Group:   &service.Group{ID: groupID, ClaudeCodeOnly: claudeCodeOnlyREDACTED,
		User:    &service.User{ID: 13REDACTED,
REDACTED)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 13, Concurrency: 1REDACTED)
