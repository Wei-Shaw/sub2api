...
// OpenAIVideoGeneration forwards OpenAI-compatible video creation requests to an OpenAI API-key account.
func (h *OpenAIGatewayHandler) OpenAIVideoGeneration(c *gin.Context) {
	h.handleOpenAIVideo(c, service.OpenAIVideoEndpointGenerations, "")
}

// OpenAIVideoStatus forwards OpenAI-compatible video status requests to the account used for creation when possible.
...
func (h *OpenAIGatewayHandler) handleOpenAIVideo(c *gin.Context, endpoint service.OpenAIVideoEndpoint, requestID string) {
	...
	requestInfo := service.ParseOpenAIVideoRequest(body)
	requestModel := strings.TrimSpace(requestInfo.Model)
	if endpoint.IsGenerationRequest() && requestModel == "" {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "model is required")
		return
	}
	if endpoint == service.OpenAIVideoEndpointStatus && strings.TrimSpace(requestID) == "" {
	...
	if endpoint == service.OpenAIVideoEndpointStatus {
		sessionHash = service.OpenAIVideoRequestSessionHash(requestID)
	}
	...
	for {
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardOpenAIVideo(requestCtx, c, account, endpoint, requestID, body, c.GetHeader("Content-Type"))
		}()
		...
		if endpoint == service.OpenAIVideoEndpointGenerations && result != nil && strings.TrimSpace(result.ResponseID) != "" {
			if err := h.gatewayService.BindOpenAIVideoRequestAccount(requestCtx, apiKey.GroupID, result.ResponseID, account.ID); err != nil {
				reqLog.Warn("openai.videos.bind_video_request_account_failed",
					zap.Int64("account_id", account.ID),
		...
		if endpoint.IsGenerationRequest() && requestModel != "" {
			recordOpenAIVideoUsage(c, h, reqLog, apiKey, subject, subscription, account, result, requestModel, body, requestID)
		}
