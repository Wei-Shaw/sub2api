...
func TestBuildGrokXSearchResponsesBodyAcceptsInputAlias(t *testing.T) {
	t.Parallel()
	body, err := buildGrokXSearchResponsesBody(grokStandaloneSearchRequest{Input: "latest posts from xAI"REDACTED, xai.DefaultTextModel)
REDACTED
	require.Contains(t, gjson.GetBytes(body, "input").String(), "latest posts from xAI")
REDACTED
...
func TestResolveGrokStandaloneSearchModelUsesRuntimeDefault(t *testing.T) {
	...
	xai.SetRuntimeModelMappingOptions(xai.ModelMappingOptions{DefaultText: "grok-4.6"REDACTED)

	model := resolveGrokStandaloneSearchModel()
	body, err := buildGrokXSearchResponsesBody(grokStandaloneSearchRequest{Query: "latest posts from xAI"REDACTED, model)
REDACTED
