...
func TestLooksLikeSystemKey(t *testing.T) {
	cases := []struct {
		...
		want bool
REDACTED{
		{"sk-abcdef0123456789", trueREDACTED,
		{"ABCdef_-0123456789", trueREDACTED,
		{"short", falseREDACTED,
		{"with space xxxxxxxxxx", falseREDACTED,
		{"汉字key1234567890", falseREDACTED,
		{"", falseREDACTED,
REDACTED
	for _, c := range cases {
...
REDACTED{
		{
			...
			headers: map[string]string{"Authorization": "Bearer sk-testkey0123456789"REDACTED,
	...
	REDACTED,
		{
			...
			headers: map[string]string{"x-api-key": "sk-xapikey0123456789"REDACTED,
			want:    "sk-xapikey0123456789",
	REDACTED,
		{
			name:    "x-goog-api-key header",
			headers: map[string]string{"x-goog-api-key": "sk-goog0123456789"REDACTED,
			want:    "sk-goog0123456789",
	REDACTED,
		{
			name:    "Authorization takes priority over x-api-key",
			headers: map[string]string{"Authorization": "Bearer sk-auth0123456789", "x-api-key": "sk-xapi0123456789"REDACTED,
			want:    "sk-auth0123456789",
	REDACTED,
		{
			name:    "x-api-key takes priority over x-goog-api-key",
			headers: map[string]string{"x-api-key": "sk-xapi0123456789", "x-goog-api-key": "sk-goog0123456789"REDACTED,
	...
	REDACTED,
		{
			...
			headers: map[string]string{"Authorization": "Bearer   sk-trimmed0123456789  "REDACTED,
	...
	REDACTED,
		{
			...
			headers: map[string]string{"Authorization": "junk-not-bearer", "x-api-key": "sk-realkey1234567"REDACTED,
