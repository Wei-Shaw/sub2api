...
REDACTED
	...
	opsErrorLogOnce = sync.Once{REDACTED
	opsErrorLogStopOnce = sync.Once{REDACTED
	opsErrorLogWorkersWg = sync.WaitGroup{REDACTED
	opsErrorLogMu = sync.RWMutex{REDACTED
	...
	opsErrorLogShutdownOnce = sync.Once{REDACTED
...
func TestEnqueueOpsErrorLog_QueueFullDrop(t *testing.T) {
	...
	opsErrorLogOnce.Do(func() {REDACTED)
	...
	entry := &service.OpsInsertErrorLogInput{ErrorPhase: "upstream", ErrorType: "upstream_error"REDACTED
...
func TestOpsErrorLoggerMiddleware_DedicatedCyberSessionBlockRecordsExactlyOnce(t *testing.T) {
	...
	gin.SetMode(gin.TestMode)
	ops := service.NewOpsService(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	h := &OpenAIGatewayHandler{opsService: opsREDACTED
	apiKey := &service.APIKey{ID: 41, Key: "sk-dedicated-test"REDACTED
	router := gin.New()
	router.Use(OpsErrorLoggerMiddleware(ops))
...
REDACTED
	...
	flushOpsErrorLogBatch([]opsErrorLogJob{jobREDACTED)
...
func TestOpsErrorLoggerMiddleware_IntermediateSkipMonitoringDoesNotHideFinalVisibleFailure(t *testing.T) {
	...
	router.POST("/v1/responses", func(c *gin.Context) {
		c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{
			{UpstreamStatusCode: http.StatusBadGateway, Message: "hidden retry", SkipMonitoring: trueREDACTED,
			{UpstreamStatusCode: http.StatusServiceUnavailable, Message: "visible final"REDACTED,
	REDACTED)
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": gin.H{"type": "upstream_error", "message": "visible final"REDACTEDREDACTED)
REDACTED)
...
func TestOpsErrorLoggerMiddleware_CapturesSplitResponsesFailedSSE(t *testing.T) {
	...
	router.POST("/v1/responses", func(c *gin.Context) {
		...
		_, _ = c.Writer.Write([]byte(`data: {"type":"response.failed","response":{"error":{"code":"rate_limit_exceeded","message":"Too many pending requests"REDACTEDREDACTEDREDACTED`))
...
REDACTED{
		{
			...
			suffix:    `failed","response":{"error":{"code":"server_is_overloaded","message":"busy"REDACTEDREDACTEDREDACTED`,
	...
	REDACTED,
		{
			...
			prefix:    `data:{"type":"response.`,
			suffix:    `failed","error":{"code":"rate_limit_exceeded","message":"slow down"REDACTEDREDACTED`,
	...
	REDACTED,
		{
			...
			prefix:    `data: {"type":"er`,
			suffix:    `ror","error":{"type":"invalid_request_error","code":"invalid_request","message":"bad input"REDACTEDREDACTED`,
	...
	REDACTED,
		{
			...
			prefix:    `data:{"type":"er`,
			suffix:    `ror","error":{"type":"authentication_error","code":"authentication_failed","message":"sign in"REDACTEDREDACTED`,
...
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			state := &opsCaptureWriterState{limit: opsCaptureWriterLimitREDACTED
...
func TestOpsErrorLoggerMiddleware_StreamFailureUsesTerminalErrorOverAttemptContext(t *testing.T) {
	...
	router.POST("/v1/responses", func(c *gin.Context) {
		...
		_, _ = c.Writer.WriteString(`data: {"type":"error","error":{"type":"invalid_request_error","code":"context_length_exceeded","message":"input exceeds the context window"REDACTEDREDACTED`)
...
func TestOpsErrorLoggerMiddleware_PrefersContextRequestID(t *testing.T) {
	...
	router.POST("/v1/responses", func(c *gin.Context) {
		...
		c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{"type": "invalid_request_error", "message": "bad input"REDACTEDREDACTED)
REDACTED)
...
func TestShouldSkipFinalOpsFailureUsesOnlyFinalAttemptRule(t *testing.T) {
	...
	c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{
		{UpstreamStatusCode: http.StatusBadGateway, Message: "hidden intermediate", SkipMonitoring: trueREDACTED,
		{UpstreamStatusCode: http.StatusServiceUnavailable, Message: "visible final"REDACTED,
REDACTED)
	require.False(t, shouldSkipFinalOpsFailure(c))

	c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{
		{UpstreamStatusCode: http.StatusBadGateway, Message: "visible intermediate"REDACTED,
		nil,
		{UpstreamStatusCode: http.StatusServiceUnavailable, Message: "hidden final", SkipMonitoring: trueREDACTED,
REDACTED)
...
REDACTED
	...
	unknown := []string{"<nil>", "null", "", "random_error", "some_new_type", "<nil>\u003e"REDACTED
...
REDACTED{
		...
		{"known invalid_request_error", "invalid_request_error", "", "invalid_request_error"REDACTED,
		{"known rate_limit_error", "rate_limit_error", "", "rate_limit_error"REDACTED,
		{"known upstream_error", "upstream_error", "", "upstream_error"REDACTED,

		// Unknown/garbage types are rejected and fall through to code-based or default.
		{"nil literal from upstream", "<nil>", "", "api_error"REDACTED,
		{"null string", "null", "", "api_error"REDACTED,
		{"random string", "something_weird", "", "api_error"REDACTED,

		// Unknown type but known code still maps correctly.
		{"nil with INSUFFICIENT_BALANCE code", "<nil>", "INSUFFICIENT_BALANCE", "billing_error"REDACTED,
		{"nil with USAGE_LIMIT_EXCEEDED code", "<nil>", "USAGE_LIMIT_EXCEEDED", "subscription_error"REDACTED,
...
REDACTED)
	...
	require.JSONEq(t, `{"error":{"type":"model_not_found","message":"Model \"gpt-missing\" is not supported by any configured account in this group"REDACTEDREDACTED`, w.Body.String())
	job := <-opsErrorLogQueue
	...
	REDACTED,
		{
			...
			message:     "beta feature REDACTED is not allowed",
			code:        "",
			status:      http.StatusBadRequest,
...
func TestParseOpsErrorResponsePreservesNestedStringCode(t *testing.T) {
	parsed := parseOpsErrorResponse([]byte(`{"error":{"type":"permission_error","code":"GROUP_DELETED","message":"API Key 所属分组已删除"REDACTEDREDACTED`))
...
REDACTED{
		{
			...
			body:     `{"type":"model_not_found","code":404,"message":"model unavailable"REDACTED`,
	...
	REDACTED,
		{
			name:     "string error",
			body:     `{"type":"service_unavailable","code":"temporarily_unavailable","error":"capacity exhausted"REDACTED`,
...
func TestApplyOpsUpstreamFieldsUsesLastNonNilAttempt(t *testing.T) {
	...
	c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{
		{UpstreamStatusCode: http.StatusTooManyRequests, Message: "first attempt", Detail: "first detail"REDACTED,
		nil,
		{UpstreamStatusCode: http.StatusServiceUnavailable, Message: "final attempt", Detail: "final detail"REDACTED,
		nil,
REDACTED)
	entry := &service.OpsInsertErrorLogInput{REDACTED
...
func TestApplyOpsUpstreamFieldsFinalStatuslessAttemptClearsStaleContext(t *testing.T) {
	...
	c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{
		{UpstreamStatusCode: http.StatusBadGateway, Message: "first response"REDACTED,
		{Kind: "request_error", Message: "final transport failure", Detail: "connection reset"REDACTED,
REDACTED)
	entry := &service.OpsInsertErrorLogInput{REDACTED
...
func TestOpsCaptureWriter_ProtocolLevelTerminalFrameDetection(t *testing.T) {
	...
	chunks := []string{
		...
		"data: { \"response\" : { \"error\" : { \"message\" : \"busy\", \"code\" : \"service_unavailable\" REDACTED REDACTED,",
		" \"type\" : \"response.failed\" REDACTED\r\n\r\n",
REDACTED
...
REDACTED{
		{
			...
			body:       "event: error\ndata: {\"message\":\"denied\",\"code\":\"permission_denied\",\"type\":\"error\"REDACTED\n\n",
			wantType:   "permission_error",
			wantStatus: http.StatusForbidden,
	REDACTED,
		{
			name:       "top-level unavailable",
			body:       "data: {\"message\":\"busy\",\"type\":\"error\",\"code\":\"service_unavailable\"REDACTED\n\n",
			wantType:   "service_unavailable_error",
			wantStatus: http.StatusServiceUnavailable,
	REDACTED,
		{
			name:       "unknown terminal",
			body:       "event: response.failed\ndata: {\"type\":\"response.failed\",\"error\":{\"code\":\"new_provider_code\",\"message\":\"failed\"REDACTEDREDACTED\n\n",
			wantType:   "upstream_error",
			wantStatus: http.StatusBadGateway,
	REDACTED,
		{
			name:       "explicit terminal status",
			body:       "event: error\ndata: {\"type\":\"error\",\"status_code\":429,\"code\":\"new_rate_code\",\"message\":\"slow down\"REDACTED\n\n",
			wantType:   "api_error",
...
func TestOpsCaptureWriter_OversizedNonTerminalFrameRemainsBounded(t *testing.T) {
	...
	state.captureResponseChunk([]byte("event: error\ndata: {\"type\":\"error\",\"code\":\"permission_denied\",\"message\":\"denied\"REDACTED\n\n"), http.StatusOK)
	require.True(t, state.sseCapturing)
...
func TestOpsCaptureWriter_TerminalMetadataSurvivesBodyCaptureTruncation(t *testing.T) {
	state := &opsCaptureWriterState{limit: opsCaptureWriterLimitREDACTED
	frame := "event: response.failed\ndata: {\"padding\":\"" + strings.Repeat("x", opsCaptureWriterLimit) + "\",\"type\":\"response.failed\",\"error\":{\"code\":\"service_unavailable\",\"message\":\"busy\"REDACTEDREDACTED\n\n"
...
func TestOpsErrorLoggerMiddleware_LargeTerminalFrameUsesEventFallback(t *testing.T) {
	...
	router.POST("/v1/responses", func(c *gin.Context) {
		...
		_, _ = c.Writer.WriteString("data: {\"authorization\":\"Bearer must-not-persist\",\"padding\":\"" + strings.Repeat("x", opsTerminalSSEFrameProbeLimit*2) + "\"REDACTED")
REDACTED)
...
func TestOpsErrorLoggerMiddleware_DetectsTerminalDataAtEOFWithoutBlankLine(t *testing.T) {
	...
	router.POST("/v1/responses", func(c *gin.Context) {
		c.Status(http.StatusOK)
		_, _ = c.Writer.WriteString(`data: {"message":"denied","code":"permission_denied","type":"error"REDACTED`)
REDACTED)
...
func TestOpsCaptureWriter_DetectsCROnlySSEFrame(t *testing.T) {
	state := &opsCaptureWriterState{limit: opsCaptureWriterLimitREDACTED
	state.captureResponseChunk([]byte("data: {\"type\":\"error\",\"code\":\"service_unavailable\",\"message\":\"busy\"REDACTED\r\r"), http.StatusOK)
...
func TestSanitizeOpsSSEDataForPersistence_DropsTruncatedJSONFragment(t *testing.T) {
	...
	require.Contains(t, sanitized, `data: {"payload_truncated":trueREDACTED`)
REDACTED
...
func TestGetOpsAPIKeyFallsBackToOpsFallbackKey(t *testing.T) {
	...
	apiKey := &service.APIKey{
		ID:      100,
		GroupID: &groupID,
		User:    &service.User{ID: 7REDACTED,
		Group:   &service.Group{ID: groupID, Platform: service.PlatformAnthropicREDACTED,
REDACTED
...
func TestGetOpsAPIKeyPrefersPrimaryContextKey(t *testing.T) {
	...
	primary := &service.APIKey{ID: 1REDACTED
	fallback := &service.APIKey{ID: 2REDACTED
