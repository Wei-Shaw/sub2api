...
//
// Channel management has been migrated out of the core into the
// channel-management plugin. The legacy ChannelService stub used to be
// injected here only so /api/v1/payment/channels could return an empty
// list, but no frontend page actually consumes that endpoint. The route
// and the stub dependency have been removed; if a future need arises to
// surface channel metadata to the payment page, the plugin should expose
// its own API instead of reaching back into core.
