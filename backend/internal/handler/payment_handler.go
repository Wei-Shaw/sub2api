...
func (h *PaymentHandler) GetChannels(c *gin.Context) {
	channels, _, err := h.channelService.List(c.Request.Context(), pagination.PaginationParams{Page: 1, PageSize: 1000REDACTED, "active", "")
	if err != nil {
		response.ErrorFrom(c, err)
		return
REDACTED
...
REDACTED
	...
	if err := h.paymentService.RequestRefund(c.Request.Context(), orderID, subject.UserID, req.Reason); err != nil {
		...
		return
REDACTED
	response.Success(c, gin.H{"message": "refund requested"REDACTED)
REDACTED
...
func (h *PaymentHandler) GetRefundEligibleProviders(c *gin.Context) {
	...
	if err != nil {
		...
		return
REDACTED
	response.Success(c, gin.H{"provider_instance_ids": idsREDACTED)
REDACTED
...
func requireAuth(c *gin.Context) (middleware2.AuthSubject, bool) {
	...
	if !ok {
		...
		return middleware2.AuthSubject{REDACTED, false
REDACTED
	return subject, true
REDACTED
...
func isMobile(c *gin.Context) bool {
	...
	for _, kw := range []string{"mobile", "android", "iphone", "ipad", "ipod"REDACTED {
