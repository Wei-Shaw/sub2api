...
func promptGuardDecision(kind securityaudit.DecisionKind) *securityaudit.Decision {
	decision := &securityaudit.Decision{Kind: kind, AllowNextStage: falseREDACTED
	switch kind {
...
func TestPromptGuardOpenAIAndClaudeErrorEnvelopesGolden(t *testing.T) {
	...
	REDACTED)
		...
		t.Run("responses_"+string(kind), func(t *testing.T) {
			c, recorder := securityAuditErrorTestContext(t)
			(&GatewayHandler{REDACTED).responsesSecurityAuditError(c, decision)
			require.Equal(t, decision.HTTPStatus, recorder.Code)
	...
	REDACTED)
		...
		t.Run("claude_"+string(kind), func(t *testing.T) {
			c, recorder := securityAuditErrorTestContext(t)
			(&GatewayHandler{REDACTED).anthropicSecurityAuditError(c, decision)
			require.Equal(t, decision.HTTPStatus, recorder.Code)
			payload := decodeErrorJSON(t, recorder)
...
func TestPromptGuardGeminiErrorEnvelopeGolden(t *testing.T) {
	...
	REDACTED
		...
		require.Equal(t, map[string]any{"request_id": "request-error-golden"REDACTED, metadata)
REDACTED
REDACTED
...
func TestLegacyModerationErrorKeepsExistingClientPriority(t *testing.T) {
	legacy := &securityaudit.Decision{
		...
		Legacy: &securityaudit.LegacyDecision{Blocked: true, StatusCode: http.StatusForbidden, ErrorCode: "content_policy_violation", Message: "legacy exact message"REDACTED,
		Prompt: &securityaudit.PromptDecision{Kind: securityaudit.DecisionBlock, ErrorCode: securityaudit.ErrorCodeBlockedREDACTED,
REDACTED
	c, recorder := securityAuditErrorTestContext(t)
	(&GatewayHandler{REDACTED).openAISecurityAuditError(c, legacy)
