...
func TestStoreMediaWithDegradation_S3SuccessSingleURL(t *testing.T) {
	...
	h := &SoraClientHandler{objectStorage: objectStorageREDACTED
...
REDACTED
	objectStorage := newS3StorageForHandler(fakeS3.URL)
	genService := service.NewSoraGenerationService(repo, nil, nil)
	h := &SoraClientHandler{genService: genService, objectStorage: objectStorageREDACTED
...
REDACTED
	quotaService := service.NewSoraQuotaService(userRepo, nil, nil)
	h := &SoraClientHandler{genService: genService, objectStorage: objectStorage, quotaService: quotaServiceREDACTED
