...
func TestOpenAIHandleStreamingAwareError_ResponsesStreamingReusesRequestID(t *testing.T) {
	...
	c.Request = c.Request.WithContext(
		context.WithValue(c.Request.Context(), ctxkey.RequestID, "REDACTED"),
	)
...
func TestOpenAIHandleStreamingAwareError_ResponsesStreamingJSONEscaping(t *testing.T) {
	cases := []struct {
		...
		message string
REDACTED{
		{"双引号", "server_error", `upstream returned "invalid" response`REDACTED,
		{"反斜杠", "server_error", `path C:\Users\test\file.txt not found`REDACTED,
		{"双引号+反斜杠", "upstream_error", `error parsing "key\value": unexpected token`REDACTED,
		{"换行与制表", "server_error", "line1\nline2\ttab"REDACTED,
		{"普通", "upstream_error", "Upstream service temporarily unavailable"REDACTED,
REDACTED
...
// Gateway handler: /v1/messages preserves the legacy data:{type:error,...REDACTED format
...
func TestInboundIsResponses_CoversAllRoutes(t *testing.T) {
	cases := []struct {
		...
		want  bool
REDACTED{
		{"/v1/responses", trueREDACTED,
		{"/v1/responses/compact", trueREDACTED,
		{"/responses", trueREDACTED, // <-- 用户 16 实际走这条
		{"/responses/compact", trueREDACTED,
		{"/backend-api/codex/responses", trueREDACTED,
		{"/backend-api/codex/responses/compact", trueREDACTED,
		{"/v1/chat/completions", falseREDACTED,
		{"/v1/messages", falseREDACTED,
		{"/", falseREDACTED,
		{"/responses-fake", falseREDACTED,
REDACTED
	for _, tc := range cases {
...
func TestMapResponsesErrorCode(t *testing.T) {
	cases := []struct{ in, out string REDACTED{
		{"rate_limit_error", "rate_limit_exceeded"REDACTED,
		{"invalid_request_error", "invalid_request"REDACTED,
		{"permission_error", "permission_denied"REDACTED,
		{"authentication_error", "authentication_failed"REDACTED,
		{"upstream_error", "upstream_error"REDACTED,
		{"server_error", "server_error"REDACTED,
		{"api_error", "server_error"REDACTED,
		{"", "server_error"REDACTED,
		{"custom_thing", "custom_thing"REDACTED,
REDACTED
	for _, tc := range cases {
