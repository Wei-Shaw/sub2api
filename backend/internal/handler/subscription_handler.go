...
// ResetDaily resets the current user's daily subscription quota and deducts 24 hours from the subscription term.
...
func (h *SubscriptionHandler) ResetDaily(c *gin.Context) {
	...
	sub, err := h.subscriptionService.ResetDailyUsageWithTimeDeduction(c.Request.Context(), subject.UserID, subscriptionID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
