...
REDACTED
	return []usagestats.TrendDataPoint{REDACTED, nil
REDACTED
...
func TestUserUsageListRequestTypePriority(t *testing.T) {
	repo := &userUsageRepoCapture{REDACTED
...
func TestUserUsageSnapshotUsesScopedFilters(t *testing.T) {
	repo := &userUsageRepoCapture{
		modelStats: []usagestats.ModelStat{{Model: "gpt-5", AccountCost: 0.07REDACTEDREDACTED,
		groupStats: []usagestats.GroupStat{{GroupID: 1, GroupName: "default", AccountCost: 0.06REDACTEDREDACTED,
REDACTED
	router := newUserUsageRequestTypeTestRouter(repo)
