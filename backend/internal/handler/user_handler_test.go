...
func (s *userHandlerRepoStub) BatchSetConcurrency(context.Context, []int64, int) (int, error) { return 0, nil REDACTED
func (s *userHandlerRepoStub) BatchAddConcurrency(context.Context, []int64, int) (int, error) { return 0, nil REDACTED
...
REDACTED
	...
	var resp struct {
		...
		Data struct {
			...
			REDACTED `json:"wechat"`
		REDACTED `json:"identities"`
	REDACTED `json:"data"`
REDACTED
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
...
func TestUserHandlerGetProfileReturnsLegacyCompatibilityFields(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/api/v1/user/profile", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 21REDACTED)
...
REDACTED
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/api/v1/user/profile", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 22REDACTED)
...
REDACTED
	...
	body := []byte(`{"email":"new@example.com","verify_code":"123456","password":"new-password"REDACTED`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/user/account-bindings/email", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	c.Params = gin.Params{{Key: "provider", Value: "email"REDACTEDREDACTED
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 11REDACTED)
...
REDACTED
	...
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 21REDACTED)
	c.Params = gin.Params{{Key: "provider", Value: "linuxdo"REDACTEDREDACTED

	handler.UnbindIdentity(c)

	require.Equal(t, http.StatusOK, recorder.Code)
	require.Equal(t, []string{"linuxdo"REDACTED, repo.unbound)
...
func TestUserHandlerUnbindIdentityRevokesAllUserSessionsWhenAuthServiceConfigured(t *testing.T) {
	...
	REDACTED,
REDACTED
	refreshTokenCache := &userHandlerRefreshTokenCacheStub{REDACTED
	cfg := &config.Config{
		JWT: config.JWTConfig{
			Secret:     "test-secret",
			ExpireHour: 1,
...
REDACTED
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodDelete, "/api/v1/user/account-bindings/linuxdo", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 23REDACTED)
	c.Params = gin.Params{{Key: "provider", Value: "linuxdo"REDACTEDREDACTED
...
REDACTED
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodDelete, "/api/v1/user/account-bindings/linuxdo", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 24REDACTED)
	c.Params = gin.Params{{Key: "provider", Value: "linuxdo"REDACTEDREDACTED
...
REDACTED
	...
	repo := &userHandlerRepoStub{user: userREDACTED
	emailCache := &userHandlerEmailCacheStub{
		data: &service.VerificationCodeData{
			Code:      "123456",
			CreatedAt: time.Now().UTC(),
...
REDACTED
	...
	body := []byte(`{"email":"new@example.com","verify_code":"123456","password":"wrong-password"REDACTED`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/user/account-bindings/email", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
...
REDACTED
	...
	body := []byte(`{"provider":"wechat","redirect_to":"/settings/profile"REDACTED`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
