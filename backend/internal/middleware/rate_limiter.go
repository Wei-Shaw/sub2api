...
return {current, repairedREDACTED
`)
...
var rateLimitRun = func(ctx context.Context, client *redis.Client, key string, windowMillis int64) (int64, bool, error) {
	values, err := rateLimitScript.Run(ctx, client, []string{keyREDACTED, windowMillis).Slice()
	if err != nil {
		return 0, false, err
REDACTED
	if len(values) < 2 {
...
func (r *RateLimiter) Allow(ctx context.Context, key string, limit int, window time.Duration) (AllowResult, error) {
	...
	if err != nil {
		return AllowResult{REDACTED, err
REDACTED
	if repaired {
		log.Printf("[RateLimit] ttl repaired: key=%s window_ms=%d", redisKey, windowMillis)
REDACTED

	result := AllowResult{Allowed: count <= int64(limit), Count: countREDACTED
	if !result.Allowed {
...
func (r *RateLimiter) Limit(key string, limit int, window time.Duration) gin.HandlerFunc {
	return r.LimitWithOptions(key, limit, window, RateLimitOptions{REDACTED)
REDACTED
