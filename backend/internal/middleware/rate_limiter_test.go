...
func TestRateLimiterHonorsForwardedIPSnapshot(t *testing.T) {
	...
	require.Contains(t, callCounts, "rate_limit:fwd:198.51.100.1/32")
	require.Contains(t, callCounts, "rate_limit:fwd:198.51.100.2/32")
}
...
func TestRateLimiterMissingClientIPHonorsFailureMode(t *testing.T) {
	gin.SetMode(gin.TestMode)

	limiter := NewRateLimiter(redis.NewClient(&redis.Options{Addr: "127.0.0.1:1"}))
	request := func(mode RateLimitFailureMode) *httptest.ResponseRecorder {
		router := gin.New()
		router.Use(limiter.LimitWithOptions("register", 1, time.Minute, RateLimitOptions{FailureMode: mode}))
	...
	require.Equal(t, http.StatusOK, request(RateLimitFailOpen).Code)
	failClosed := request(RateLimitFailClose)
	require.Equal(t, http.StatusTooManyRequests, failClosed.Code)
	require.Equal(t, "60", failClosed.Header().Get("Retry-After"))
}
