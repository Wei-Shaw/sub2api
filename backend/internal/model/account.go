...
// JSONB 用于存储JSONB数据
type JSONB map[string]interface{REDACTED
...
func (j *JSONB) Scan(value interface{REDACTED) error {
	if value == nil {
		*j = nil
		return nil
REDACTED
...
type Account struct {
	...
	Credentials  JSONB          `gorm:"type:jsonb;default:'{REDACTED'" json:"credentials"` // 凭证(加密存储)
	Extra        JSONB          `gorm:"type:jsonb;default:'{REDACTED'" json:"extra"`       // 扩展信息
...
REDACTED
	...
	if !ok || raw == nil {
		return nil
REDACTED
	// 处理map[string]interface{REDACTED类型
	if m, ok := raw.(map[string]interface{REDACTED); ok {
...
REDACTED
	...
	if !ok || raw == nil {
		return nil
REDACTED
	// 处理 []interface{REDACTED 类型（JSON反序列化后的格式）
	if arr, ok := raw.([]interface{REDACTED); ok {
