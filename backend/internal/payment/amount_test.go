...
REDACTED{
		// Normal values
		{name: "one yuan", input: "1.00", want: 100REDACTED,
		{name: "ten yuan fifty fen", input: "10.50", want: 1050REDACTED,
		{name: "one fen", input: "0.01", want: 1REDACTED,
		{name: "large amount", input: "99999.99", want: 9999999REDACTED,

		// Edge: zero
		{name: "zero no decimal", input: "0", want: 0REDACTED,
		{name: "zero with decimal", input: "0.00", want: 0REDACTED,

		// IEEE 754 precision edge case: 1.15 * 100 = 114.99999... in float64
		{name: "ieee754 precision 1.15", input: "1.15", want: 115REDACTED,

		// More precision edge cases
		{name: "ieee754 precision 0.1", input: "0.1", want: 10REDACTED,
		{name: "ieee754 precision 0.2", input: "0.2", want: 20REDACTED,
		{name: "ieee754 precision 33.33", input: "33.33", want: 3333REDACTED,

		// Large value
		{name: "hundred thousand", input: "100000.00", want: 10000000REDACTED,

		// Integer without decimal
		{name: "integer 5", input: "5", want: 500REDACTED,
		{name: "integer 100", input: "100", want: 10000REDACTED,

		// Single decimal place
		{name: "single decimal 1.5", input: "1.5", want: 150REDACTED,

		// Negative values
		{name: "negative one yuan", input: "-1.00", want: -100REDACTED,
		{name: "negative with fen", input: "-10.50", want: -1050REDACTED,

		// Invalid inputs
		{name: "empty string", input: "", wantErr: trueREDACTED,
		{name: "alphabetic", input: "abc", wantErr: trueREDACTED,
		{name: "double dot", input: "1.2.3", wantErr: trueREDACTED,
		{name: "spaces", input: "  ", wantErr: trueREDACTED,
		{name: "special chars", input: "$10.00", wantErr: trueREDACTED,
REDACTED
...
func TestFenToYuan(t *testing.T) {
	tests := []struct {
		...
		want float64
REDACTED{
		{name: "one yuan", fen: 100, want: 1.0REDACTED,
		{name: "ten yuan fifty fen", fen: 1050, want: 10.5REDACTED,
		{name: "one fen", fen: 1, want: 0.01REDACTED,
		{name: "zero", fen: 0, want: 0.0REDACTED,
		{name: "large amount", fen: 9999999, want: 99999.99REDACTED,
		{name: "negative", fen: -100, want: -1.0REDACTED,
		{name: "negative with fen", fen: -1050, want: -10.5REDACTED,
REDACTED
...
func TestPaymentCurrencyHelpers(t *testing.T) {
	tests := []struct {
		...
		wantBack  float64
REDACTED{
		{name: "hkd uses cents", currency: "hkd", amount: "12.34", wantMinor: 1234, wantBack: 12.34REDACTED,
		{name: "jpy has no minor unit", currency: "JPY", amount: "12", wantMinor: 12, wantBack: 12REDACTED,
		{name: "kwd uses three decimal minor units", currency: "KWD", amount: "12.345", wantMinor: 12345, wantBack: 12.345REDACTED,
		{name: "isk uses Stripe legacy two-decimal API amount", currency: "ISK", amount: "12", wantMinor: 1200, wantBack: 12REDACTED,
		{name: "ugx uses Stripe legacy two-decimal API amount", currency: "UGX", amount: "12.00", wantMinor: 1200, wantBack: 12REDACTED,
		{name: "empty currency defaults to cny", currency: "", amount: "1.23", wantMinor: 123, wantBack: 1.23REDACTED,
REDACTED
...
func TestFormatAmountForCurrency(t *testing.T) {
	tests := []struct {
		...
		want     string
REDACTED{
		{currency: "CNY", amount: 12.3, want: "12.30"REDACTED,
		{currency: "JPY", amount: 12, want: "12"REDACTED,
		{currency: "KWD", amount: 12.345, want: "12.345"REDACTED,
		{currency: "ISK", amount: 12, want: "12"REDACTED,
REDACTED
...
func TestThreeDecimalPaymentCurrencies(t *testing.T) {
	for _, currency := range []string{"BHD", "IQD", "JOD", "KWD", "LYD", "OMR", "TND"REDACTED {
