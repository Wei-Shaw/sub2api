...
REDACTED
	...
	require.Equal(t, []payment.PaymentType{payment.TypeAirwallexREDACTED, prov.SupportedTypes())
...
func TestAirwallexCreatePaymentUsesServerAmountAndStableRequestID(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/authentication/login":
			...
			_, _ = w.Write([]byte(`{"token":"token-1","expires_at":"2099-01-01T00:00:00Z"REDACTED`))
		...
		REDACTED
			...
			_, _ = w.Write([]byte(`{"id":"int_123","client_secret":"secret_123","amount":12.34,"currency":"CNY","merchant_order_id":"sub2_order","status":"REQUIRES_PAYMENT_METHOD"REDACTED`))
		default:
			http.NotFound(w, r)
	REDACTED
REDACTED))
...
func TestAirwallexCreatePaymentUsesConfiguredCurrency(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		REDACTED
			...
			_, _ = w.Write([]byte(`{"id":"int_123","client_secret":"secret_123","amount":12.34,"currency":"HKD","merchant_order_id":"sub2_order","status":"REQUIRES_PAYMENT_METHOD"REDACTED`))
		default:
			http.NotFound(w, r)
	REDACTED
REDACTED))
...
func TestAirwallexRequestsUseConfiguredAccountID(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/pa/payment_intents/int_123":
			paRequestCount++
			require.Equal(t, "acct_123", r.Header.Get("x-on-behalf-of"))
			_, _ = w.Write([]byte(`{"id":"int_123","amount":12.34,"currency":"CNY","merchant_order_id":"sub2_order","status":"SUCCEEDED"REDACTED`))
		case "/api/v1/pa/refunds/create":
			paRequestCount++
			require.Equal(t, "acct_123", r.Header.Get("x-on-behalf-of"))
			_, _ = w.Write([]byte(`{"id":"ref_123","payment_intent_id":"int_123","amount":12.34,"currency":"CNY","status":"SETTLED"REDACTED`))
		case "/api/v1/pa/payment_intents/int_123/cancel":
			paRequestCount++
			require.Equal(t, "acct_123", r.Header.Get("x-on-behalf-of"))
			_, _ = w.Write([]byte(`{"id":"int_123","amount":12.34,"currency":"CNY","merchant_order_id":"sub2_order","status":"CANCELLED"REDACTED`))
		default:
			http.NotFound(w, r)
	REDACTED
REDACTED))
...
func TestAirwallexRefundRejectsUnsettledStatus(t *testing.T) {
	...
	for _, status := range []string{"RECEIVED", "ACCEPTED", "FAILED"REDACTED {
		t.Run(status, func(t *testing.T) {
			...
			server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case "/api/v1/authentication/login":
					_, _ = w.Write([]byte(`{"token":"token-1","expires_at":"2099-01-01T00:00:00Z"REDACTED`))
				case "/api/v1/pa/refunds/create":
					_, _ = w.Write([]byte(`{"id":"ref_123","payment_intent_id":"int_123","amount":12.34,"currency":"CNY","status":"` + status + `"REDACTED`))
				default:
					http.NotFound(w, r)
			REDACTED
		REDACTED))
...
func TestAirwallexAuthErrorIncludesCredentialGuidance(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"code":"credentials_invalid","details":["Access Denied"],"message":"UNAUTHORIZED","source":""REDACTED`))
REDACTED))
	defer server.Close()
...
REDACTED
	...
	raw := `{"id":"evt_1","name":"payment_intent.succeeded","accountId":"acct_123","data":{"object":{"id":"int_123","merchant_order_id":"sub2_abc","amount":88.66,"currency":"CNY","status":"SUCCEEDED"REDACTEDREDACTEDREDACTED`
...
func TestAirwallexQueryOrderMapsSucceeded(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/authentication/login":
			_, _ = w.Write([]byte(`{"token":"token-1","expires_at":"2099-01-01T00:00:00Z"REDACTED`))
		case "/api/v1/pa/payment_intents/int_123":
			_, _ = w.Write([]byte(`{"id":"int_123","amount":99.01,"currency":"CNY","merchant_order_id":"sub2_order","status":"SUCCEEDED"REDACTED`))
		default:
			http.NotFound(w, r)
	REDACTED
REDACTED))
