...
func TestNormalizeEasyPayAPIBase(t *testing.T) {
	...
	tests := []struct {
		...
		want  string
REDACTED{
		{input: "https://zpayz.cn", want: "https://zpayz.cn"REDACTED,
		{input: "https://zpayz.cn/", want: "https://zpayz.cn"REDACTED,
		{input: "https://zpayz.cn/mapi.php", want: "https://zpayz.cn"REDACTED,
		{input: "https://zpayz.cn/submit.php", want: "https://zpayz.cn"REDACTED,
		{input: "https://zpayz.cn/api.php", want: "https://zpayz.cn"REDACTED,
		{input: "https://zpayz.cn/api.php?act=refund", want: "https://zpayz.cn"REDACTED,
REDACTED
...
func TestEasyPayRefundNormalizesAPIBaseAndSendsOutTradeNoOnly(t *testing.T) {
	...
	REDACTED
		...
		_, _ = w.Write([]byte(`{"code":1,"msg":"ok"REDACTED`))
REDACTED))
	defer server.Close()
...
func TestEasyPayRefundRetriesWithTradeNoWhenOutTradeNoNotFound(t *testing.T) {
	...
	REDACTED
		...
		w.Header().Set("Content-Type", "application/json")
		if len(gotForms) == 1 {
			_, _ = w.Write([]byte(`{"code":0,"msg":"订单编号不存在！"REDACTED`))
			return
	REDACTED
		_, _ = w.Write([]byte(`{"code":1,"msg":"ok"REDACTED`))
REDACTED))
...
func TestEasyPayRefundResponseErrors(t *testing.T) {
	...
	tests := []struct {
		...
		want       string
REDACTED{
		{name: "html response", statusCode: http.StatusOK, body: "<html>bad config</html>", want: "non-JSON response (HTTP 200): <html>bad config</html>"REDACTED,
		{name: "non json response", statusCode: http.StatusOK, body: "not json", want: "non-JSON response (HTTP 200): not json"REDACTED,
		{name: "non 2xx response", statusCode: http.StatusBadGateway, body: "bad gateway", want: "HTTP 502: bad gateway"REDACTED,
		{name: "empty response", statusCode: http.StatusOK, body: "", want: "empty response (HTTP 200): <empty>"REDACTED,
REDACTED
...
func TestEasyPayCustomMethodsUseConfiguredUpstreamType(t *testing.T) {
	...
	provider, err := NewEasyPay("test-instance", map[string]string{
		...
		"customMethods": `[{"type":"ldc","upstreamType":"epay","displayName":"LDC"REDACTED,{"type":"usdt_trc20","upstreamType":"usdt","displayName":"USDT-TRC20"REDACTED]`,
REDACTED)
	if err != nil {
...
func TestEasyPayCustomMethodsResolveCIDFromConfiguredUpstreamType(t *testing.T) {
	...
	provider, err := NewEasyPay("test-instance", map[string]string{
		...
		"cidWxpay":      "cid-wxpay",
		"customMethods": `[{"type":"ldc","upstreamType":"alipay","displayName":"LDC"REDACTED]`,
REDACTED)
	if err != nil {
		t.Fatalf("NewEasyPay: %v", err)
REDACTED
...
REDACTED
	...
	for _, want := range []string{"alipay", "wxpay", "ldc", "usdt_trc20"REDACTED {
