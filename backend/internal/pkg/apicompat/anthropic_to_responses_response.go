...
func anthToResHandleContentBlockStop(evt *AnthropicStreamEvent, state *AnthropicEventToResponsesState) []ResponsesStreamEvent {
	...
	case "message":
		...
		state.TextAccum = ""
		state.CurrentContent = append(state.CurrentContent, ResponsesContentPart{Type: "output_text", Text: textREDACTED)
		...
		REDACTED),
			makeResponsesEvent(state, "response.content_part.done", &ResponsesStreamEvent{
				...
				ContentIndex: state.ContentIndex,
				ItemID:       state.CurrentItemID,
				Part:         &ResponsesContentPart{Type: "output_text", Text: textREDACTED,
		REDACTED),
	REDACTED
REDACTED
...
func anthropicResponsesStreamTerminalState(stopReason string) (string, *ResponsesIncompleteDetails) {
	if stopReason == "max_tokens" {
		return "incomplete", &ResponsesIncompleteDetails{Reason: "max_output_tokens"REDACTED
REDACTED
...
REDACTED
	...
	// carry its content. Emitting only {type,id,statusREDACTED makes SDK-side
...
REDACTED
	...
	case "reasoning":
		if state.CurrentSummary != "" {
			item.Summary = []ResponsesSummary{{Type: "summary_text", Text: state.CurrentSummaryREDACTEDREDACTED
	REDACTED
REDACTED
...
REDACTED
	...
	if outputs == nil {
		outputs = []ResponsesOutput{REDACTED
REDACTED
