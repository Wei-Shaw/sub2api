...
func assembleToolUseBlocks(events []AnthropicStreamEvent) []assembledToolUse {
	blockByIdx := map[int]int{REDACTED // anthropic block index → position in out
	...
	for _, e := range events {
		...
		case "content_block_start":
			if e.ContentBlock != nil && e.ContentBlock.Type == "tool_use" && e.Index != nil {
				...
				out = append(out, assembledToolUse{ID: e.ContentBlock.ID, Name: e.ContentBlock.NameREDACTED)
		REDACTED
...
func TestChatCompletionsToAnthropicStreamState_ToolCallArgsArriveBeforeName(t *testing.T) {
	...
	events := collectAnthropicStreamEvents(t, []string{
		`{"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_early","function":{"arguments":"{\"city\":"REDACTEDREDACTED]REDACTEDREDACTED]REDACTED`,
		`{"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"function":{"arguments":"\"SF\""REDACTEDREDACTED]REDACTEDREDACTED]REDACTED`,
		`{"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"function":{"name":"get_weather","arguments":"REDACTED"REDACTEDREDACTED]REDACTEDREDACTED]REDACTED`,
		`{"choices":[{"index":0,"delta":{REDACTED,"finish_reason":"tool_calls"REDACTED],"usage":{"prompt_tokens":1,"completion_tokens":1,"total_tokens":2REDACTEDREDACTED`,
REDACTED)
	...
	require.JSONEq(t, `{"city":"SF"REDACTED`, tools[0].Input)

	// No delta may precede the block's content_block_start.
	started := map[int]bool{REDACTED
	for _, e := range events {
		switch e.Type {
		case "content_block_start":
...
func TestChatCompletionsToAnthropicStreamState_ToolCallNameNeverArrives(t *testing.T) {
	...
	events := collectAnthropicStreamEvents(t, []string{
		`{"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_anon","function":{"arguments":"{\"a\":1REDACTED"REDACTEDREDACTED]REDACTEDREDACTED]REDACTED`,
		`{"choices":[{"index":0,"delta":{REDACTED,"finish_reason":"tool_calls"REDACTED],"usage":{"prompt_tokens":1,"completion_tokens":1,"total_tokens":2REDACTEDREDACTED`,
REDACTED)
	...
	require.JSONEq(t, `{"a":1REDACTED`, tools[0].Input)
...
func TestChatCompletionsToAnthropicStreamState_EmptyArgsToolEmitsPlaceholderDelta(t *testing.T) {
	...
	// "{REDACTED" before its stop — the double-conversion path normalizes empty
	// arguments to "{REDACTED", and some clients assemble input only from deltas.
	events := collectAnthropicStreamEvents(t, []string{
		`{"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_empty","type":"function","function":{"name":"noop","arguments":""REDACTEDREDACTED]REDACTEDREDACTED]REDACTED`,
		`{"choices":[{"index":0,"delta":{REDACTED,"finish_reason":"tool_calls"REDACTED],"usage":{"prompt_tokens":1,"completion_tokens":1,"total_tokens":2REDACTEDREDACTED`,
REDACTED)
	...
	require.JSONEq(t, `{REDACTED`, tools[0].Input)
REDACTED
...
func TestAnthropicToChatCompletionsRequest_UserArrayContentFoldsToString(t *testing.T) {
	...
	req := &AnthropicRequest{
		...
		MaxTokens: 100,
		Messages: []AnthropicMessage{
			{Role: "user", Content: json.RawMessage(`[{"type":"text","text":"first"REDACTED,{"type":"text","text":"second"REDACTED]`)REDACTED,
	REDACTED,
REDACTED
...
func TestDirectBridge_RequestMatchesDoubleConversion_ArrayUserContent(t *testing.T) {
	...
	req := &AnthropicRequest{
		...
		Messages: []AnthropicMessage{
			{Role: "user", Content: json.RawMessage(`[{"type":"text","text":"first"REDACTED,{"type":"text","text":"second"REDACTED]`)REDACTED,
			{Role: "assistant", Content: json.RawMessage(`"ok"`)REDACTED,
			{Role: "user", Content: json.RawMessage(`[{"type":"text","text":"look"REDACTED,{"type":"image","source":{"type":"base64","media_type":"image/png","data":"aGVsbG8="REDACTEDREDACTED]`)REDACTED,
	REDACTED,
REDACTED
...
func TestAnthropicToChatCompletionsRequest_ToolChoiceUndeclaredDropped(t *testing.T) {
	...
	base := AnthropicRequest{
		...
		MaxTokens: 100,
		Tools: []AnthropicTool{
			{Name: "get_weather", InputSchema: json.RawMessage(`{"type":"object"REDACTED`)REDACTED,
			{Name: "web_search_20250305", Type: "web_search_20250305", InputSchema: json.RawMessage(`{"type":"object"REDACTED`)REDACTED,
	REDACTED,
		Messages: []AnthropicMessage{
			{Role: "user", Content: json.RawMessage(`"hi"`)REDACTED,
	REDACTED,
REDACTED
	...
	undeclared := base
	undeclared.ToolChoice = json.RawMessage(`{"type":"tool","name":"nonexistent"REDACTED`)
...
REDACTED
	...
	droppedServerTool.ToolChoice = json.RawMessage(`{"type":"tool","name":"web_search_20250305"REDACTED`)
...
REDACTED
	...
	unknownType := base
	unknownType.ToolChoice = json.RawMessage(`{"type":"mystery"REDACTED`)
...
REDACTED
	...
	declared := base
	declared.ToolChoice = json.RawMessage(`{"type":"tool","name":"get_weather"REDACTED`)
	out, err = AnthropicToChatCompletionsRequest(&declared)
REDACTED
	require.JSONEq(t, `{"type":"function","function":{"name":"get_weather"REDACTEDREDACTED`, string(out.ToolChoice))
REDACTED
...
func TestDirectBridge_NonStreamingMatchesDoubleConversion_EmptyChoices(t *testing.T) {
	...
	resp := &ChatCompletionsResponse{ID: "chatcmpl-empty", Model: "deepseek-v4-pro"REDACTED
		...
		Choices: []ChatChoice{{
			Message: ChatMessage{
				...
				ToolCalls: []ChatToolCall{{
					ID:       "call_cf",
					Type:     "function",
					Function: ChatFunctionCall{Name: "search", Arguments: `{"q":"x"REDACTED`REDACTED,
		REDACTED
		REDACTED,
