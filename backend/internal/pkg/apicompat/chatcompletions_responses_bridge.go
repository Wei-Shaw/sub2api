...
// input item shaped as {"type":"additional_tools","tools":[...]REDACTED instead of
...
func buildChatMessagesFromItems(messages []ChatMessage, rawItems []json.RawMessage, opts *ResponsesToChatOptions) ([]ChatMessage, toolOutputMediaByCallID, error) {
	...
	invalidFunctionCallIDs := make(map[string]struct{REDACTED)
...
REDACTED
	...
	REDACTED
		...
		REDACTED else if opts != nil && opts.ReasoningContentByID != nil {
		...
		REDACTED
			...
			if !json.Valid([]byte(arguments)) {
				...
				if callID != "" {
					invalidFunctionCallIDs[callID] = struct{REDACTED{REDACTED
			REDACTED
				pendingReasoning = ""
				continue
		REDACTED
	...
	REDACTED
		...
		if err != nil {
			return nil, nil, err
	REDACTED
		msg := ChatMessage{Role: role, Content: chatContentREDACTED
...
REDACTED
	...
	REDACTED
		...
		entry := NamespacedToolName{Namespace: tool.Name, Name: child.NameREDACTED
		if topLevel[flat] {
