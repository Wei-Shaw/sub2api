...
func TestResponsesChatBridge_MixedCustomAndNamespaceToolNames(t *testing.T) {
	req := &ResponsesRequest{
		...
		Tools: []ResponsesTool{
			{Type: "custom", Name: "exec", Description: "Runs a command"REDACTED,
			{Type: "namespace", Name: "functions", Tools: []ResponsesTool{
				{Type: "function", Name: "wait", Parameters: json.RawMessage(`{"type":"object"REDACTED`)REDACTED,
	REDACTED
	REDACTED,
REDACTED
...
REDACTED
	...
	resp := &ChatCompletionsResponse{Choices: []ChatChoice{{Message: ChatMessage{ToolCalls: []ChatToolCall{
		{ID: "call_exact", Function: ChatFunctionCall{Name: "exec", Arguments: `{"input":"pwd"REDACTED`REDACTEDREDACTED,
		{ID: "call_wait", Function: ChatFunctionCall{Name: "functions__wait", Arguments: `{"cell_id":"1"REDACTED`REDACTEDREDACTED,
		{ID: "call_alias", Function: ChatFunctionCall{Name: "functions__exec", Arguments: `not-json`REDACTEDREDACTED,
...
func TestChatCompletionsResponseToResponses_ExplicitFunctionOwnsCustomAliasCollision(t *testing.T) {
	resp := &ChatCompletionsResponse{Choices: []ChatChoice{{Message: ChatMessage{ToolCalls: []ChatToolCall{{
		ID: "call_function", Function: ChatFunctionCall{Name: "functions__exec", Arguments: `{"path":"/tmp"REDACTED`REDACTED,
REDACTEDREDACTEDREDACTEDREDACTEDREDACTEDREDACTED

	out := ChatCompletionsResponseToResponses(resp, "deepseek-test",
		map[string]bool{"exec": trueREDACTED, map[string]bool{"functions__exec": trueREDACTED, false,
		map[string]NamespacedToolName{"functions__wait": {Namespace: "functions", Name: "wait"REDACTEDREDACTED)

	require.Len(t, out.Output, 1)
	assert.Equal(t, "function_call", out.Output[0].Type)
	assert.Equal(t, "functions__exec", out.Output[0].Name)
	assert.Equal(t, `{"path":"/tmp"REDACTED`, out.Output[0].Arguments)
REDACTED
	...
	req := &ResponsesRequest{
		...
		Input: json.RawMessage(`[
			{"type":"additional_tools","role":"developer","tools":[
				{"type":"custom","name":"exec","description":"Run PowerShell","format":{"type":"text"REDACTEDREDACTED,
				{"type":"function","name":"wait","parameters":{"type":"object","properties":{REDACTEDREDACTEDREDACTED,
				{"type":"namespace","name":"collaboration","tools":[
					{"type":"function","name":"send_message","parameters":{"type":"object","properties":{REDACTEDREDACTEDREDACTED
				]REDACTED
			]REDACTED,
			{"type":"message","role":"user","content":[{"type":"input_text","text":"run Get-Location"REDACTED]REDACTED
		]`),
...
REDACTED
	...
	assert.Equal(t, NamespacedToolName{Namespace: "collaboration", Name: "send_message"REDACTED, NamespaceToolNames(effective)["collaboration__send_message"])
...
func TestEffectiveResponsesTools_SkipsStringInputItems(t *testing.T) {
	req := &ResponsesRequest{
		Input: json.RawMessage(`["plain input",{"type":"additional_tools","tools":[{"type":"custom","name":"exec"REDACTED]REDACTED]`),
REDACTED
...
REDACTED
	...
	out := ChatCompletionsResponseToResponses(resp, "glm-5.2", map[string]bool{"exec": trueREDACTED, nil, false, nil)
...
func TestChatCompletionsChunkToResponsesEvents_MixedCustomNamespaceAliasStream(t *testing.T) {
	...
	state.NamespaceTools = map[string]NamespacedToolName{
		"functions__wait": {Namespace: "functions", Name: "wait"REDACTED,
REDACTED

	idx := 0
	chunk := &ChatCompletionsChunk{Choices: []ChatChunkChoice{{Delta: ChatDelta{ToolCalls: []ChatToolCall{{
		Index: &idx, ID: "call_alias", Function: ChatFunctionCall{Name: "functions__exec", Arguments: `not-json`REDACTED,
REDACTEDREDACTEDREDACTEDREDACTEDREDACTEDREDACTED
	events := ChatCompletionsChunkToResponsesEvents(chunk, state)
	events = append(events, FinalizeChatCompletionsResponsesStream(state)...)
...
func TestChatCompletionsChunkToResponsesEvents_ExplicitFunctionOwnsCustomAliasCollision(t *testing.T) {
	state := NewChatCompletionsToResponsesStreamState("deepseek-test")
	state.CustomTools = map[string]bool{"exec": trueREDACTED
	state.FunctionTools = map[string]bool{"functions__exec": trueREDACTED
	state.NamespaceTools = map[string]NamespacedToolName{
		"functions__wait": {Namespace: "functions", Name: "wait"REDACTED,
REDACTED
	...
	idx := 0
	chunk := &ChatCompletionsChunk{Choices: []ChatChunkChoice{{Delta: ChatDelta{ToolCalls: []ChatToolCall{{
		Index: &idx, ID: "call_function", Function: ChatFunctionCall{Name: "functions__exec", Arguments: `{"path":"/tmp"REDACTED`REDACTED,
REDACTEDREDACTEDREDACTEDREDACTEDREDACTEDREDACTED
	events := ChatCompletionsChunkToResponsesEvents(chunk, state)
	events = append(events, FinalizeChatCompletionsResponsesStream(state)...)
...
REDACTED
	...
	assert.Equal(t, `{"path":"/tmp"REDACTED`, final.Response.Output[0].Arguments)
REDACTED
...
func TestResponsesToChatCompletionsRequest_DropsDeferredFlagWithToolSearch(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{"model":"glm-5.2","input":"hi","tools":[{"type":"tool_search"REDACTED,{"type":"function","name":"shell","defer_loading":trueREDACTED]REDACTED`), &req))
	...
	resp := &ChatCompletionsResponse{
		...
		Choices: []ChatChoice{{
			Message: ChatMessage{
				Role: "assistant",
				ToolCalls: []ChatToolCall{
					{ID: "call_s", Function: ChatFunctionCall{Name: "tool_search", Arguments: `{"query":"gmail","limit":2REDACTED`REDACTEDREDACTED,
			REDACTED,
		REDACTED,
REDACTED
REDACTED
	...
	resp := &ChatCompletionsResponse{
		Choices: []ChatChoice{{
			Message: ChatMessage{
				Role: "assistant",
				ToolCalls: []ChatToolCall{
					{ID: "call_s", Function: ChatFunctionCall{Name: "tool_search", Arguments: `{"query":"gmail"REDACTED`REDACTEDREDACTED,
			REDACTED,
		REDACTED,
REDACTED
REDACTED
		...
		Choices: []ChatChunkChoice{{
			Delta: ChatDelta{
				ToolCalls: []ChatToolCall{{
					Index:    &idx,
					ID:       "call_s",
					Function: ChatFunctionCall{Name: "tool_search", Arguments: `{"query":"gmail"REDACTED`REDACTED,
		REDACTED
		REDACTED,
REDACTED
REDACTED
...
REDACTED
	...
	assert.Contains(t, sse, `"arguments":{"query":"gmail"REDACTED`)
...
func TestHasToolSearchTool(t *testing.T) {
	assert.True(t, HasToolSearchTool([]ResponsesTool{{Type: "tool_search"REDACTEDREDACTED))
	assert.False(t, HasToolSearchTool([]ResponsesTool{{Type: "function", Name: "tool_search"REDACTEDREDACTED))
...
func TestNamespaceToolNames_MapsFlattenedNames(t *testing.T) {
	tools := []ResponsesTool{
		{Type: "namespace", Name: "gmail", Tools: []ResponsesTool{
			{Type: "function", Name: "send"REDACTED,
			{Type: "custom", Name: "skip_me"REDACTED,
REDACTED
		{Type: "namespace", Name: "crm", Children: []ResponsesTool{
			{Type: "function", Name: "query"REDACTED,
REDACTED
...
REDACTED
	...
	require.Len(t, m, 2)
	assert.Equal(t, NamespacedToolName{Namespace: "gmail", Name: "send"REDACTED, m["gmail__send"])
	assert.Equal(t, NamespacedToolName{Namespace: "crm", Name: "query"REDACTED, m["crm__query"])
	...
	m2 := NamespaceToolNames([]ResponsesTool{{
		...
		Tools: []ResponsesTool{{Type: "function", Name: longChildREDACTEDREDACTED,
REDACTEDREDACTED)
	assert.Equal(t, NamespacedToolName{Namespace: longNS, Name: longChildREDACTED,
...
func TestResponsesToChatCompletionsRequest_RejectsToolSearchNameConflict(t *testing.T) {
	...
	_, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Tools: []ResponsesTool{
			...
			{Type: "function", Name: "tool_search"REDACTED,
	REDACTED,
REDACTED)
	...
	_, err = ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Input: json.RawMessage(`"hi"`),
		Tools: []ResponsesTool{
			{Type: "custom", Name: "tool_search"REDACTED,
			{Type: "tool_search"REDACTED,
	REDACTED,
REDACTED)
	...
	out, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
		Model: "glm-5.2",
		Input: json.RawMessage(`"hi"`),
		Tools: []ResponsesTool{{Type: "tool_search"REDACTED, {Type: "tool_search"REDACTEDREDACTED,
REDACTED)
REDACTED
	require.Len(t, out.Tools, 1)
	assert.Equal(t, "tool_search", out.Tools[0].Function.Name)
...
func TestResponsesToChatCompletionsRequest_RejectsDuplicateTopLevelExecutableNames(t *testing.T) {
	for _, tools := range [][]ResponsesTool{
		{{Type: "custom", Name: "exec"REDACTED, {Type: "function", Name: "exec"REDACTEDREDACTED,
		{{Type: "function", Name: "exec"REDACTED, {Type: "function", Name: "exec"REDACTEDREDACTED,
		{{Type: "custom", Name: "exec"REDACTED, {Type: "custom", Name: "exec"REDACTEDREDACTED,
REDACTED {
		_, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
			Model: "glm-5.2",
			Input: json.RawMessage(`"hi"`),
...
func TestResponsesToChatCompletionsRequest_DropsToolChoiceForDroppedTool(t *testing.T) {
	...
	out, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Tools: []ResponsesTool{
			...
			{Type: "web_search"REDACTED,
	REDACTED,
		ToolChoice: json.RawMessage(`{"type":"web_search"REDACTED`),
REDACTED)
REDACTED
	require.Len(t, out.Tools, 1)
	...
	out, err = ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Tools: []ResponsesTool{
			{Type: "function", Name: "wait", Parameters: json.RawMessage(`{"type":"object","properties":{REDACTEDREDACTED`)REDACTED,
			{Type: "web_search"REDACTED,
			{Type: "x_search"REDACTED,
	REDACTED,
		ToolChoice: json.RawMessage(`{"type":"function","name":"web_search"REDACTED`),
REDACTED)
REDACTED
	require.Len(t, out.Tools, 2)
	...
	out, err = ResponsesToChatCompletionsRequest(&ResponsesRequest{
		Model:      "glm-5.2",
		Input:      json.RawMessage(`"hi"`),
		Tools:      []ResponsesTool{{Type: "function", Name: "wait"REDACTEDREDACTED,
		ToolChoice: json.RawMessage(`{"type":"function","name":"missing"REDACTED`),
REDACTED)
REDACTED
...
REDACTED
	...
	out, err = ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Input:      json.RawMessage(`"hi"`),
		Tools:      []ResponsesTool{{Type: "function", Name: "wait"REDACTEDREDACTED,
		ToolChoice: json.RawMessage(`{"type":"function","name":"wait"REDACTED`),
REDACTED)
REDACTED
	assert.JSONEq(t, `{"type":"function","function":{"name":"wait"REDACTEDREDACTED`, string(out.ToolChoice))
REDACTED
...
func TestResponsesToChatCompletionsRequest_ToolSearchToolChoiceMapsToProxy(t *testing.T) {
	out, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Input:      json.RawMessage(`"hi"`),
		Tools:      []ResponsesTool{{Type: "tool_search"REDACTEDREDACTED,
		ToolChoice: json.RawMessage(`{"type":"tool_search"REDACTED`),
REDACTED)
REDACTED
	assert.JSONEq(t, `{"type":"function","function":{"name":"tool_search"REDACTEDREDACTED`, string(out.ToolChoice))
...
func TestResponsesToChatCompletionsRequest_RejectsAmbiguousFlattenedNames(t *testing.T) {
	...
	_, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Input: json.RawMessage(`"hi"`),
		Tools: []ResponsesTool{
			{Type: "function", Name: "gmail__send"REDACTED,
			{Type: "namespace", Name: "gmail", Tools: []ResponsesTool{{Type: "function", Name: "send"REDACTEDREDACTEDREDACTED,
	REDACTED,
REDACTED)
	...
	_, err = ResponsesToChatCompletionsRequest(&ResponsesRequest{
		...
		Input: json.RawMessage(`"hi"`),
		Tools: []ResponsesTool{
			{Type: "namespace", Name: "a", Tools: []ResponsesTool{{Type: "function", Name: "b__c"REDACTEDREDACTEDREDACTED,
			{Type: "namespace", Name: "a__b", Tools: []ResponsesTool{{Type: "function", Name: "c"REDACTEDREDACTEDREDACTED,
	REDACTED,
REDACTED)
	...
	resp := &ChatCompletionsResponse{
		...
		Choices: []ChatChoice{{
			Message: ChatMessage{
				Role: "assistant",
				ToolCalls: []ChatToolCall{
					{ID: "call_n", Function: ChatFunctionCall{Name: "mcp__svc__echo", Arguments: `{"text":"hi"REDACTED`REDACTEDREDACTED,
					{ID: "call_9", Function: ChatFunctionCall{Name: "wait", Arguments: `{"cell_id": 3REDACTED`REDACTEDREDACTED,
			REDACTED,
		REDACTED,
REDACTED
REDACTED
	nsTools := map[string]NamespacedToolName{
		"mcp__svc__echo": {Namespace: "mcp__svc", Name: "echo"REDACTED,
REDACTED
	...
	assert.Equal(t, `{"text":"hi"REDACTED`, item.Arguments)
		...
		Choices: []ChatChunkChoice{{
			Delta: ChatDelta{
				ToolCalls: []ChatToolCall{{
					Index:    &idx,
					ID:       "call_n",
					Function: ChatFunctionCall{Name: "mcp__svc__echo", Arguments: `{"text":"hi"REDACTED`REDACTED,
		REDACTED
		REDACTED,
REDACTED
REDACTED
...
REDACTED
	...
	assert.Equal(t, `{"text":"hi"REDACTED`, itemDone.Item.Arguments)
...
REDACTED
	...
	idx := 0
	chunk1 := &ChatCompletionsChunk{Choices: []ChatChunkChoice{{Delta: ChatDelta{
		ToolCalls: []ChatToolCall{{Index: &idx, ID: "call_n", Function: ChatFunctionCall{Arguments: `{"te`REDACTEDREDACTEDREDACTED,
REDACTEDREDACTEDREDACTEDREDACTED
	chunk2 := &ChatCompletionsChunk{Choices: []ChatChunkChoice{{Delta: ChatDelta{
		ToolCalls: []ChatToolCall{{Index: &idx, Function: ChatFunctionCall{Name: "mcp__svc__echo", Arguments: `xt":"hi"REDACTED`REDACTEDREDACTEDREDACTED,
REDACTEDREDACTEDREDACTEDREDACTED
...
REDACTED
	assert.Equal(t, 1, addedCount, "工具调用只宣告一次")
	assert.Equal(t, `{"text":"hi"REDACTED`, deltas, "宣告前累积的参数需在宣告时补发")
REDACTED
