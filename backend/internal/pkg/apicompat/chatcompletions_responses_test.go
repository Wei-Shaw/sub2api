...
func TestUsageConversionsPreserveCacheWriteTokens(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{
		"input_tokens":1000,
		"output_tokens":50,
		"input_tokens_details":{"cached_tokens":100,"cache_write_tokens":200REDACTED
REDACTED`), &responsesUsage))
...
func TestChatCompletionsToResponses_SystemMessage(t *testing.T) {
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "system", Content: json.RawMessage(`"You are helpful."`)REDACTED,
...
func TestChatCompletionsToResponses_ToolCalls(t *testing.T) {
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(`"Call the function"`)REDACTED,
			{
				Role: "assistant",
				ToolCalls: []ChatToolCall{
					{
						...
						Function: ChatFunctionCall{
							Name:      "ping",
							Arguments: `{"host":"example.com"REDACTED`,
					REDACTED,
				REDACTED,
			REDACTED,
		REDACTED,
		...
		Tools: []ChatTool{
			{
				...
				Function: &ChatFunction{
					...
					Parameters:  json.RawMessage(`{"type":"object"REDACTED`),
			REDACTED,
		REDACTED,
	REDACTED,
REDACTED
...
func TestChatCompletionsToResponses_ToolStrict(t *testing.T) {
	...
	tests := []struct {
		...
		want   bool
REDACTED{
		{name: "defaults omitted strict to false", want: falseREDACTED,
		{name: "preserves explicit true", strict: &strictTrue, want: trueREDACTED,
		{name: "preserves explicit false", strict: &strictFalse, want: falseREDACTED,
REDACTED
...
REDACTED)
REDACTED
	assert.JSONEq(t, `{"type":"function","strict":falseREDACTED`, string(payload))
REDACTED
...
func TestChatCompletionsToResponses_EmptyBase64ImageURLSkipped(t *testing.T) {
	content := `[{"type":"text","text":"Describe this"REDACTED,{"type":"image_url","image_url":{"url":"data:image/png;base64,"REDACTEDREDACTED]`
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(content)REDACTED,
...
REDACTED{
		{"null content", json.RawMessage(`null`)REDACTED,
		{"empty array content", json.RawMessage(`[]`)REDACTED,
		{"only empty text part", json.RawMessage(`[{"type":"text","text":""REDACTED]`)REDACTED,
		{"only empty base64 image part", json.RawMessage(`[{"type":"image_url","image_url":{"url":"data:image/png;base64,"REDACTEDREDACTED]`)REDACTED,
REDACTED
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			req := &ChatCompletionsRequest{
				Model: "gpt-5.5",
				Messages: []ChatMessage{
					{Role: "user", Content: tc.contentREDACTED,
			REDACTED,
		REDACTED
			resp, err := ChatCompletionsToResponses(req)
		REDACTED
...
func TestChatCompletionsToResponses_SystemArrayContent(t *testing.T) {
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "system", Content: json.RawMessage(`[{"type":"text","text":"You are a careful visual assistant."REDACTED]`)REDACTED,
			{Role: "user", Content: json.RawMessage(`[{"type":"text","text":"Describe this image"REDACTED,{"type":"image_url","image_url":{"url":"data:image/png;base64,abc123"REDACTEDREDACTED]`)REDACTED,
	REDACTED,
REDACTED
...
func TestChatCompletionsToResponses_LegacyFunctions(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		FunctionCall: json.RawMessage(`{"name":"get_weather"REDACTED`),
REDACTED
...
func TestChatCompletionsToResponses_AssistantWithTextAndToolCalls(t *testing.T) {
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(`"Do something"`)REDACTED,
			{
				Role:    "assistant",
		...
		Output: []ResponsesOutput{
			{
				...
				Content: []ResponsesContentPart{
					{Type: "output_text", Text: "Hello, world!"REDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
...
REDACTED
	...
	assert.Equal(t, `{"city":"NYC"REDACTED`, msg.ToolCalls[0].Function.Arguments)
REDACTED
		...
		Output: []ResponsesOutput{
			{
				...
				Summary: []ResponsesSummary{
					{Type: "summary_text", Text: "I thought about it."REDACTED,
			REDACTED,
		REDACTED,
			{
				Type: "message",
				Content: []ResponsesContentPart{
					{Type: "output_text", Text: "The answer is 42."REDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
REDACTED
...
func TestChatCompletionsToResponses_ToolArrayContent(t *testing.T) {
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(`"Use the tool"`)REDACTED,
			{
				Role: "assistant",
				ToolCalls: []ChatToolCall{
					{
		...
		REDACTED,
			{
				...
				ToolCallID: "call_1",
				Content: json.RawMessage(
					`[{"type":"text","text":"image width: 100"REDACTED,{"type":"image_url","image_url":{"url":"data:image/png;base64,ignored"REDACTEDREDACTED,{"type":"text","text":"; image height: 200"REDACTED]`,
				),
		REDACTED,
	REDACTED,
REDACTED
		...
		Output: []ResponsesOutput{
			{
				Type: "message",
				Content: []ResponsesContentPart{
					{Type: "output_text", Text: "partial..."REDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
REDACTED
...
func TestResponsesToChatCompletions_CachedTokens(t *testing.T) {
	resp := &ResponsesResponse{
		...
		Output: []ResponsesOutput{
			{
				Type:    "message",
				Content: []ResponsesContentPart{{Type: "output_text", Text: "cached"REDACTEDREDACTED,
		REDACTED,
	REDACTED,
		Usage: &ResponsesUsage{
			InputTokens:  100,
...
func TestResponsesToChatCompletions_ReasoningTokens(t *testing.T) {
	resp := &ResponsesResponse{
		...
		Output: []ResponsesOutput{
			{
				Type:    "message",
				Content: []ResponsesContentPart{{Type: "output_text", Text: "ping"REDACTEDREDACTED,
		REDACTED,
	REDACTED,
		Usage: &ResponsesUsage{
			InputTokens:  24,
...
func TestResponsesToChatCompletions_AllTokenDetailsPassThrough(t *testing.T) {
	...
	resp := &ResponsesResponse{
		...
		Output: []ResponsesOutput{
			{
				Type:    "message",
				Content: []ResponsesContentPart{{Type: "output_text", Text: "x"REDACTEDREDACTED,
		REDACTED,
	REDACTED,
		Usage: &ResponsesUsage{
			InputTokens:  100,
...
func TestResponsesToChatCompletions_NoReasoningTokensWhenZero(t *testing.T) {
	...
	resp := &ResponsesResponse{
		...
		Output: []ResponsesOutput{
			{
				Type:    "message",
				Content: []ResponsesContentPart{{Type: "output_text", Text: "hi"REDACTEDREDACTED,
		REDACTED,
	REDACTED,
		Usage: &ResponsesUsage{
			InputTokens:  10,
		...
		Output: []ResponsesOutput{
			{
				...
				Action: &WebSearchAction{Type: "search", Query: "test"REDACTED,
		REDACTED,
			{
				Type:    "message",
				Content: []ResponsesContentPart{{Type: "output_text", Text: "search results"REDACTEDREDACTED,
		REDACTED,
	REDACTED,
REDACTED
...
func TestResponsesEventToChatChunks_ResponseDone(t *testing.T) {
	...
	chunks := ResponsesEventToChatChunks(&ResponsesStreamEvent{
		...
		Response: &ResponsesResponse{
			Status: "completed",
			Usage:  &ResponsesUsage{InputTokens: 13, OutputTokens: 7REDACTED,
	REDACTED,
REDACTED, state)
	require.Len(t, chunks, 2)
	require.NotNil(t, chunks[0].Choices[0].FinishReason)
...
func TestChatChunkToSSE(t *testing.T) {
	chunk := ChatCompletionsChunk{
		...
		Choices: []ChatChunkChoice{
			{
				Index:        0,
				Delta:        ChatDelta{Role: "assistant"REDACTED,
				FinishReason: nil,
		REDACTED,
	REDACTED,
REDACTED
...
func TestChatCompletionsStreamRoundTrip(t *testing.T) {
	...
	chunks := ResponsesEventToChatChunks(&ResponsesStreamEvent{
		Type:     "response.created",
		Response: &ResponsesResponse{ID: "resp_rt"REDACTED,
REDACTED, state)
	allChunks = append(allChunks, chunks...)

	// 2. text deltas
	for _, text := range []string{"Hello", ", ", "world", "!"REDACTED {
		chunks = ResponsesEventToChatChunks(&ResponsesStreamEvent{
			Type:  "response.output_text.delta",
			Delta: text,
	REDACTED, state)
...
func TestBufferedResponseAccumulator_TextOnly(t *testing.T) {
	...
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.output_text.delta", Delta: "Hello"REDACTED)
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.output_text.delta", Delta: ", world!"REDACTED)
...
REDACTED)
	...
	assert.Equal(t, `{"city":"NYC"REDACTED`, output[0].Arguments)
REDACTED

func TestBufferedResponseAccumulator_Reasoning(t *testing.T) {
	acc := NewBufferedResponseAccumulator()

	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.reasoning_summary_text.delta", Delta: "Step 1: "REDACTED)
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.reasoning_summary_text.delta", Delta: "think about it"REDACTED)
...
func TestBufferedResponseAccumulator_Mixed(t *testing.T) {
	...
	// Reasoning first
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.reasoning_summary_text.delta", Delta: "I thought about it."REDACTED)

	// Then text
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.output_text.delta", Delta: "The answer is 42."REDACTED)
...
func TestBufferedResponseAccumulator_NoSupplementWhenOutputExists(t *testing.T) {
	acc := NewBufferedResponseAccumulator()
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.output_text.delta", Delta: "from deltas"REDACTED)
		...
		Output: []ResponsesOutput{
			{
				Type: "message",
				Content: []ResponsesContentPart{
					{Type: "output_text", Text: "from terminal event"REDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
REDACTED
...
func TestBufferedResponseAccumulator_EmptyDeltas(t *testing.T) {
	...
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.output_text.delta", Delta: ""REDACTED)
	acc.ProcessEvent(&ResponsesStreamEvent{Type: "response.created"REDACTED)

	assert.False(t, acc.HasContent())

	resp := &ResponsesResponse{ID: "resp_3", Status: "completed"REDACTED
	acc.SupplementResponseOutput(resp)
