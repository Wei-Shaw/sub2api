...
func TestResponsesOutputUnmarshal_ToolSearchObjectArguments(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{
		...
		"call_id":"call_1",
		"execution":"client",
		"arguments":{"query":"gmail","limit":2REDACTED
REDACTED`), &item))
	require.Equal(t, "tool_search_call", item.Type)
	require.Equal(t, `{"query":"gmail","limit":2REDACTED`, item.Arguments)
...
func TestResponsesResponseUnmarshal_ToolSearchObjectArguments(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{
		...
		"output":[{
			...
			"id":"item_1",
			"call_id":"call_1",
			"arguments":{"query":"gmail"REDACTED
	REDACTED]
REDACTED`), &response))
	require.Len(t, response.Output, 1)
	require.Equal(t, `{"query":"gmail"REDACTED`, response.Output[0].Arguments)
REDACTED
...
REDACTED`), &event))
	...
	require.Equal(t, `{"query":"gmail"REDACTED`, event.Item.Arguments)
REDACTED
