...
REDACTED
	...
	require.NoError(t, json.Unmarshal([]byte(`{"model":"glm-5.2","input":`+input+`REDACTED`), &req))
...
REDACTED
	for i, m := range messages {
		...
		require.NotContains(t, []string{"", "null", `""`, "[]"REDACTED, raw,
...
func TestResponsesToAnthropic_ReasoningItemWithContentIsDropped(t *testing.T) {
	messages := responsesToAnthropicMessages(t, `[
		{"type":"message","role":"user","content":[{"type":"input_text","text":"run a shell command"REDACTED]REDACTED,
		{"type":"reasoning","id":"rs_1","summary":[],"content":[{"type":"reasoning_text","text":"let me think"REDACTED]REDACTED
	]`)
...
func TestResponsesToAnthropic_ReasoningItemSummaryOnlyStillDropped(t *testing.T) {
	messages := responsesToAnthropicMessages(t, `[
		...
		{"type":"reasoning","id":"rs_1","summary":[{"type":"summary_text","text":"s"REDACTED],"encrypted_content":"gAAAA"REDACTED
	]`)
...
func TestResponsesToAnthropic_UnknownItemTypeContentIsSanitized(t *testing.T) {
	messages := responsesToAnthropicMessages(t, `[
		{"type":"web_search_call","id":"ws_1","content":[{"type":"web_search_result","text":"payload"REDACTED]REDACTED
	]`)
...
func TestResponsesToAnthropic_UnknownItemTypeKeepsRecognizableText(t *testing.T) {
	messages := responsesToAnthropicMessages(t, `[
		{"type":"some_future_item","content":[
			{"type":"input_text","text":"keep me"REDACTED,
			{"type":"reasoning_text","text":"drop me"REDACTED
		]REDACTED
	]`)
...
func TestResponsesToAnthropic_UserMessageWithOnlyUnknownPartsIsDropped(t *testing.T) {
	messages := responsesToAnthropicMessages(t, `[
		{"type":"message","role":"user","content":[{"type":"input_file","file_id":"file_1"REDACTED]REDACTED
	]`)
...
func TestResponsesToAnthropic_AssistantMessageWithOnlyUnknownPartsIsDropped(t *testing.T) {
	messages := responsesToAnthropicMessages(t, `[
		{"type":"message","role":"user","content":[{"type":"input_text","text":"hi"REDACTED]REDACTED,
		{"type":"message","role":"assistant","content":[{"type":"refusal","refusal":"no"REDACTED]REDACTED
	]`)
...
func TestResponsesToAnthropic_CodexToolRoundStaysIntactAndSendable(t *testing.T) {
	messages := responsesToAnthropicMessages(t, `[
		{"type":"message","role":"user","content":[{"type":"input_text","text":"run ls"REDACTED]REDACTED,
		{"type":"reasoning","id":"rs_1","summary":[],"content":[{"type":"reasoning_text","text":"plan"REDACTED],"encrypted_content":"gAAAA"REDACTED,
		{"type":"function_call","id":"fc_1","call_id":"call_1","name":"shell","arguments":"{\"cmd\":\"ls\"REDACTED"REDACTED,
		{"type":"function_call_output","call_id":"call_1","output":"file1"REDACTED,
...
REDACTED{
		...
		{`  []  `, trueREDACTED,
		{`"hi"`, falseREDACTED,
		{`[{"type":"text","text":"hi"REDACTED]`, falseREDACTED,
REDACTED
...
func TestAnthropicContentIsOnlyBlankText(t *testing.T) {
	cases := []struct {
		...
		want bool
REDACTED{
		{`[{"type":"text","text":""REDACTED]`, trueREDACTED,
		{`[{"type":"text","text":"   "REDACTED]`, trueREDACTED,
		{`[{"type":"text","text":""REDACTED,{"type":"text","text":" "REDACTED]`, trueREDACTED,
		{`[{"type":"text","text":"hi"REDACTED]`, falseREDACTED,
		{`[{"type":"text","text":""REDACTED,{"type":"image","source":{REDACTEDREDACTED]`, falseREDACTED,
		{`[]`, falseREDACTED,
REDACTED
	for _, tc := range cases {
