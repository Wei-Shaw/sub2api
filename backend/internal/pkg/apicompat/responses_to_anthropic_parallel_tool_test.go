...
REDACTED
	...
	for _, chunk := range []*ChatCompletionsChunk{chatChunk1, chatChunk2, chatChunk3REDACTED {
...
func TestStreamingParallelToolUseSecondToolPackedArgsDone(t *testing.T) {
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		...
		Response: &ResponsesResponse{ID: "resp_par", Model: "glm-5.2"REDACTED,
REDACTED, state)
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		...
		OutputIndex: 0,
		Item:        &ResponsesOutput{Type: "function_call", CallID: "call_a", Name: "tool_a"REDACTED,
REDACTED, state)
...
REDACTED
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		Type:        "response.output_item.added",
		OutputIndex: 1,
		Item:        &ResponsesOutput{Type: "function_call", CallID: "call_b", Name: "tool_b"REDACTED,
REDACTED, state)
...
REDACTED, state)
	...
	for _, e := range eventsTool2Done {
		if e.Type == "content_block_delta" && e.Index != nil {
			...
			assert.Equal(t, `{"y":2REDACTED`, e.Delta.PartialJSON)
	REDACTED
		if e.Type == "content_block_stop" && e.Index != nil {
			sawStop = true
			stopIndex = *e.Index
...
func TestStreamingThreeParallelToolsAllPackedDone(t *testing.T) {
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		Type:     "response.created",
		Response: &ResponsesResponse{ID: "resp_3par", Model: "glm-5.2"REDACTED,
REDACTED, state)

	// Open three tool blocks at indices 0, 1, 2.
	for i, name := range []string{"tool_a", "tool_b", "tool_c"REDACTED {
		ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
			Type:        "response.output_item.added",
			OutputIndex: i,
			Item:        &ResponsesOutput{Type: "function_call", CallID: "call_" + name, Name: nameREDACTED,
	REDACTED, state)
REDACTED

	// Track started blocks.
	started := map[int]bool{0: true, 1: true, 2: trueREDACTED

	// All three .done events with packed arguments (no prior delta).
	for i, args := range []string{`{"a":1REDACTED`, `{"b":2REDACTED`, `{"c":3REDACTED`REDACTED {
