...
REDACTED
	...
	assert.JSONEq(t, `{"type":"object","properties":{REDACTEDREDACTED`, string(tool.InputSchema))
...
func TestResponsesToAnthropic_CustomToolPreservesSchemaParameters(t *testing.T) {
	tools := convertResponsesToAnthropicTools([]ResponsesTool{{
		...
		Parameters:  json.RawMessage(`{"type":"object","properties":{"patch":{"type":"string"REDACTEDREDACTED,"required":["patch"]REDACTED`),
REDACTEDREDACTED)
	...
	assert.JSONEq(t, `{"patch":{"type":"string"REDACTEDREDACTED`, string(schema["properties"]))
	...
	tools := convertResponsesToAnthropicTools([]ResponsesTool{
		{
			Type:       "function",
			Name:       "read_file",
			Parameters: json.RawMessage(`{"type":"object","properties":{"path":{"type":"string"REDACTEDREDACTEDREDACTED`),
	REDACTED,
		{
			Type: "custom",
			Name: "apply_patch",
...
REDACTED)
	...
	assert.JSONEq(t, `{"type":"object","properties":{REDACTEDREDACTED`, string(tools[1].InputSchema))
...
REDACTEDREDACTED)
	...
	assert.JSONEq(t, `{"type":"object","properties":{REDACTEDREDACTED`, string(tools[0].InputSchema))
REDACTED
