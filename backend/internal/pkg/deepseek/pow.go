// Package deepseek provides DeepSeek web API client with PoW solver.
// Ported from https://github.com/CJackHwang/ds2api
package deepseek
...
// PoW challenge response from DeepSeek API
...
// keccakF23 performs Keccak-f[1600] with rounds 1..23 (skipping round 0).
// This is the core of DeepSeekHashV1.
...
// deepseekHashV1 returns the 32-byte hash of data using DeepSeekHashV1 algorithm.
// This is equivalent to SHA3-256 but with Keccak-f[1600] rounds 1..23 (skipping round 0).
...
// buildPrefix builds the PoW prefix: "<salt>_<expire_at>_"
func buildPrefix(salt string, expireAt int64) string {
	return salt + "_" + strconv.FormatInt(expireAt, 10) + "_"
}

// solvePow searches for nonce in [0, difficulty) such that
// DeepSeekHashV1(prefix + str(nonce)) equals the challenge.
...
// BuildPowHeader serializes the PoW response as base64(JSON) for the x-ds-pow-response header.
func BuildPowHeader(c *PowChallenge, answer int64) (string, error) {
...
// SolveAndBuildHeader is the end-to-end function: Challenge → x-ds-pow-response header string.
func SolveAndBuildHeader(ctx context.Context, c *PowChallenge) (string, error) {
