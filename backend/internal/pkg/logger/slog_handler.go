...
func (h *slogZapHandler) WithAttrs(attrs []slog.Attr) slog.Handler {
	next := *h
	next.attrs = append(append([]slog.Attr{REDACTED, h.attrs...), attrs...)
	return &next
REDACTED
...
REDACTED
	next := *h
	next.groups = append(append([]string{REDACTED, h.groups...), name)
	return &next
REDACTED
...
func slogAttrToZapField(groups []string, attr slog.Attr) zap.Field {
	if len(groups) > 0 {
		attr.Key = strings.Join(append(append([]string{REDACTED, groups...), attr.Key), ".")
REDACTED
