...
func TestMatchClientEntries_WhitelistAND(t *testing.T) {
	wl := []AllowedClientEntry{{Originator: "opencode", UAContains: []string{"opencode/"REDACTEDREDACTEDREDACTED
	...
	require.False(t, MatchClientEntries("opencode/1.2", "opencode", []AllowedClientEntry{{Originator: "opencode"REDACTEDREDACTED), "空 UAContains 安全失败")
REDACTED

func TestDenyEntries_BlacklistOR(t *testing.T) {
	bl := []AllowedClientEntry{
		{Originator: "evilbot"REDACTED,
		{UAContains: []string{"badscan/"REDACTEDREDACTED,
REDACTED
	...
	require.False(t, MatchDenyEntries("x", "y", []AllowedClientEntry{{REDACTEDREDACTED), "全空条目安全忽略")
REDACTED
