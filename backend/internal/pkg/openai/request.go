...
REDACTED
	if strict {
		if matchCodexClientHeaderStrictPrefixes(ua, codexOfficialClientUAPrefixes) {
			return true
	REDACTED
REDACTED else if matchCodexClientHeaderPrefixes(ua, codexOfficialClientUAPrefixes) {
		return true
REDACTED
...
// of a codex-rs formatted User-Agent: `{origREDACTED/{verREDACTED ({osREDACTED; {archREDACTED) {termREDACTED ({nameREDACTED; {verREDACTED)`.
//
...
// CodexUserAgentVersion 提取 Codex UA 的完整版本段，即 `{clientREDACTED/{versionREDACTED (...` 中的 version。
...
// （客户端名、OS / 架构 / 终端指纹）原样保留；UA 不是 `{clientREDACTED/{versionREDACTED` 形态时返回空串，
// 由调用方决定整体回退。
//
...
REDACTED
	...
	if space := strings.IndexByte(rest, ' '); space >= 0 {
		tail = rest[space:]
REDACTED else if strings.TrimSpace(rest) == "" {
...
// `{originatorREDACTED/{X.Y.ZREDACTED (...)`，第一个 '/' 后、首个空格或 '(' 前的三段版本。
