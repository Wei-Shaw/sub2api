...
REDACTED{
		...
		{name: "cccc override → codex-tui", ua: "cccc/0.141.0 (mac os 14.6.1; arm64) apple_terminal/453 (codex-tui; 0.141.0)", want: "codex-tui"REDACTED,
		{name: "cccc override Ubuntu", ua: "cccc/0.139.0 (ubuntu 22.4.0; x86_64) screen (codex-tui; 0.139.0)", want: "codex-tui"REDACTED,
		// 官方客户端自报名称
		{name: "Codex Desktop 自报(小写后)", ua: "codex desktop/0.142.0 (mac os 26.0.1; arm64) unknown (codex desktop; 26.616.71553)", want: "codex desktop"REDACTED,
		{name: "codex-tui 自报", ua: "codex-tui/0.141.0 (mac os 15.5.0; arm64) ghostty/1.3.1 (codex-tui; 0.141.0)", want: "codex-tui"REDACTED,
		{name: "codex_exec 自报", ua: "codex_exec/0.141.0 (mac os 14.7.3; arm64) apple_terminal (codex_exec; 0.141.0)", want: "codex_exec"REDACTED,
		// 无括号/无尾部组
		{name: "curl 无括号", ua: "curl/8.0.1", want: ""REDACTED,
		{name: "空字符串", ua: "", want: ""REDACTED,
		// 非 codex 尾部
		{name: "非 codex 尾部不影响", ua: "evil/0.1.0 (linux; x86_64) bash (evil; 0.1.0)", want: "evil"REDACTED,
REDACTED
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
REDACTED{
		...
		{name: "codex_vscode_copilot 变体前缀", ua: "codex_vscode_copilot/0.140.0", want: trueREDACTED,
		...
		{name: "codex-tui 连字符前缀(真实流量占比最高)", ua: "codex-tui/0.141.0 (Mac OS 15.5.0; arm64) ghostty/1.3.1 (codex-tui; 0.141.0)", want: trueREDACTED,
		...
		{name: "cccc override Mac → 尾部兜底放行", ua: "cccc/0.141.0 (Mac OS 14.6.1; arm64) Apple_Terminal/453 (codex-tui; 0.141.0)", want: trueREDACTED,
		{name: "cccc override Ubuntu → 尾部兜底放行", ua: "cccc/0.139.0 (Ubuntu 22.4.0; x86_64) screen (codex-tui; 0.139.0)", want: trueREDACTED,
		{name: "cccc override iTerm → 尾部兜底放行", ua: "cccc/0.137.0 (Mac OS 26.1.0; arm64) iTerm.app/3.4.22 (codex-tui; 0.137.0)", want: trueREDACTED,
		// 非 codex 尾部不应放行
		{name: "完全伪造尾部应拒", ua: "evil/0.1.0 (Linux; x86_64) bash (evil; 0.1.0)", want: falseREDACTED,
		{name: "非 codex", ua: "curl/8.0.1", want: falseREDACTED,
		{name: "空字符串", ua: "", want: falseREDACTED,
REDACTED
...
REDACTED{
		...
		{name: "codex-tui 连字符(真实流量占比最高)", originator: "codex-tui", want: trueREDACTED,
		{name: "空白包裹", originator: "  codex_vscode  ", want: trueREDACTED,
		{name: "伪造含 codex_ 子串应拒(L2 收紧)", originator: "evil-codex_cli", want: falseREDACTED,
		{name: "codex_ 混入中段应拒(L2 收紧)", originator: "my_codex_thing", want: falseREDACTED,
...
REDACTED{
		// 前缀开头：与 lax 版一致放行
		{name: "codex_cli_rs 前缀开头", ua: "codex_cli_rs/0.141.0 (x)", want: trueREDACTED,
		{name: "codex_vscode 前缀开头", ua: "codex_vscode/1.0.0", want: trueREDACTED,
		{name: "codex_app 前缀开头", ua: "codex_app/2.1.0", want: trueREDACTED,
		{name: "Codex 家族前缀保留", ua: "Codex Desktop/1.2.3", want: trueREDACTED,
		{name: "大小写混合前缀开头", ua: "Codex_CLI_Rs/0.141.0", want: trueREDACTED,
		// UA 尾部兜底保留：cccc override 真实 codex-tui 仍放行
		{name: "cccc override 尾部兜底仍放行", ua: "cccc/0.141.0 (Mac OS 14.6.1; arm64) Apple_Terminal/453 (codex-tui; 0.141.0)", want: trueREDACTED,
		// N1 收紧：codex token 不在行首（子串）不再算官方——lax 版会因 Contains 误判 true
		{name: "浏览器前缀+中段 codex_app 收紧→拒", ua: "Mozilla/5.0 codex_app/0.141.0", want: falseREDACTED,
		{name: "中段 codex_cli_rs 收紧→拒", ua: "evilclient/1.0 codex_cli_rs/0.141.0", want: falseREDACTED,
		{name: "非 codex", ua: "curl/8.0.1", want: falseREDACTED,
		{name: "空字符串", ua: "", want: falseREDACTED,
REDACTED
	for _, tt := range tests {
...
REDACTED{
		...
		{name: "仅 originator 命中 codex-tui", originator: "codex-tui", want: trueREDACTED,
		{name: "仅 ua 命中 codex-tui", ua: "codex-tui/0.141.0 (Mac OS 15.5.0; arm64) ghostty/1.3.1", want: trueREDACTED,
		// cccc：originator 不命中精确集，但 UA 尾部兜底恢复真实 codex-tui
		{name: "cccc override → UA 尾部兜底放行(审计 5.3% 误杀场景)", ua: "cccc/0.141.0 (Mac OS 14.6.1; arm64) Apple_Terminal/453 (codex-tui; 0.141.0)", originator: "cccc", want: trueREDACTED,
