...
type dependencyModuleKey struct{REDACTED
...
REDACTED
	...
	if module == "" {
		return ctx
REDACTED
	return context.WithValue(ctx, dependencyModuleKey{REDACTED, module)
REDACTED
...
REDACTED
	if _, ok := base.(*timingRoundTripper); ok {
		return base
REDACTED
	return &timingRoundTripper{base: baseREDACTED
REDACTED
...
func dependencyModule(req *http.Request) string {
	if req != nil {
		if module, ok := req.Context().Value(dependencyModuleKey{REDACTED).(string); ok && module != "" {
			return module
	REDACTED
REDACTED
