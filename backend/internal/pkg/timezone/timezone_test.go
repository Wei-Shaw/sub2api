...
func TestStartOfWeek_Boundaries(t *testing.T) {
	if err := Init("Asia/Shanghai"); err != nil {
		t.Fatalf("Init: %v", err)
REDACTED
	t.Cleanup(func() { _ = Init("UTC") REDACTED)
	...
	cases := []struct {
		...
		in   time.Time
REDACTED{
		{"friday", time.Date(2026, 5, 22, 14, 30, 0, 0, loc)REDACTED,
		{"sunday", time.Date(2026, 5, 24, 10, 0, 0, 0, loc)REDACTED,
		{"monday-self", time.Date(2026, 5, 18, 9, 15, 30, 0, loc)REDACTED,
REDACTED
	for _, c := range cases {
