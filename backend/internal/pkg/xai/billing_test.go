...
func TestBuildBillingSummaryWeeklyAndMonthly(t *testing.T) {
	...
	weeklyBody := []byte(`{
		"config": {
			"currentPeriod": {"type":"WEEKLY","start":"2026-07-09T03:25:00Z","end":"2026-07-16T03:25:00Z"REDACTED,
			"creditUsagePercent": 2.0,
			"productUsage": [{"product":"Api","usagePercent":2.0REDACTED],
			"prepaidBalance": {"val": 12REDACTED,
			"onDemandCap": {"val": 100REDACTED,
			"onDemandUsed": {"val": 5REDACTED,
			"isUnifiedBillingUser": true
	REDACTED
REDACTED`)
	monthlyBody := []byte(`{
		"config": {
			"monthlyLimit": {"val": 15000REDACTED,
			"used": {"val": 78REDACTED,
...
func TestBuildBillingSummaryMonthlyOnlyKeepsWeeklyUsageEmpty(t *testing.T) {
	t.Parallel()
	payload, err := ParseBillingPayload([]byte(`{"config":{"monthlyLimit":{"val":15000REDACTED,"used":{"val":7500REDACTED,"billingPeriodStart":"2026-07-01T00:00:00Z","billingPeriodEnd":"2026-08-01T00:00:00Z"REDACTEDREDACTED`))
REDACTED
...
func TestBuildBillingSummaryWeeklyDoesNotInheritMonthlyPeriodEnd(t *testing.T) {
	...
	payload, err := ParseBillingPayload([]byte(`{
		"config": {
			...
			"productUsage": [{"product":"Api","usagePercent":95.0REDACTED],
			"billingPeriodStart": "2026-07-01T00:00:00Z",
			"billingPeriodEnd": "2026-08-01T00:00:00Z",
			"monthlyLimit": {"val": 15000REDACTED,
			"used": {"val": 1000REDACTED
	REDACTED
REDACTED`))
REDACTED
	summary := BuildBillingSummary(payload.Config)
...
REDACTED
	...
	require.Equal(t, []string{"weekly"REDACTED, merged.FailedWindows)
	require.Equal(t, []string{"monthly"REDACTED, previous.FailedWindows)
REDACTED
