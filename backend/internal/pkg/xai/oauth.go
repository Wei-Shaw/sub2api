...
var (
	...
	baseURLAllowedHosts = []string{"api.x.ai", "*.api.x.ai", "cli-chat-proxy.grok.com"REDACTED
)
...
type SessionStore struct {
	...
	localOnly map[string]struct{REDACTED
	stopOnce  sync.Once
...
func NewSessionStore() *SessionStore {
	store := &SessionStore{
		...
		localOnly: make(map[string]struct{REDACTED),
...
REDACTED
	...
	if remoteErr != nil {
		s.localOnly[sessionID] = struct{REDACTED{REDACTED
