...
func (c *ssoDeviceFakeClient) Do(req *http.Request) (*http.Response, error) {
	...
	case SSOAccountsURL:
		...
		return ssoDeviceResponse(http.StatusOK, http.Header{"Set-Cookie": {"session=web-session; Domain=x.ai; Path=/"REDACTEDREDACTED, `{REDACTED`), nil
	case SSODeviceURL:
		...
		return ssoDeviceResponse(http.StatusOK, http.Header{"Set-Cookie": {"csrf=csrf-token; Path=/"REDACTEDREDACTED, `{"device_code":"device-1","user_code":"USER-1","verification_uri_complete":"https://auth.x.ai/oauth2/device/complete","interval":1,"expires_in":60REDACTED`), nil
	...
	case SSOVerifyURL:
		...
		return ssoDeviceResponse(http.StatusFound, http.Header{"Location": {"/oauth2/device/consent"REDACTEDREDACTED, ``), nil
	...
	case SSOApproveURL:
		...
		return ssoDeviceResponse(http.StatusSeeOther, http.Header{"Location": {"/oauth2/device/done"REDACTEDREDACTED, ``), nil
	...
	case SSOTokenURL:
		...
		return ssoDeviceResponse(http.StatusOK, nil, `{"access_token":"access-token","refresh_token":"refresh-token","id_token":"id-token","token_type":"Bearer","expires_in":3600,"scope":"`+SSOBuildScope+`"REDACTED`), nil
	default:
...
func TestConvertSSOToBuildCompletesDeviceFlow(t *testing.T) {
	...
	client := &ssoDeviceFakeClient{t: tREDACTED
...
REDACTED
	flow := &ssoDeviceFlow{cookieJar: jarREDACTED
