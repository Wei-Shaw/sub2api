...
REDACTED
	...
	result, err := client.ExecContext(
		ctx,
		"UPDATE accounts SET extra = jsonb_set(COALESCE(extra, '{REDACTED'::jsonb), $1::text[], $2::jsonb, true), updated_at = NOW(), last_used_at = NOW() WHERE id = $3 AND deleted_at IS NULL",
		path,
		raw,
		id,
	)
