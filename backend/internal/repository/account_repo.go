...
func newAccountRepositoryWithSQL(client *dbent.Client, sqlq sqlExecutor, schedulerCache service.SchedulerCache) *accountRepository {
	return &accountRepository{client: client, sql: sqlq, schedulerCache: schedulerCacheREDACTED
REDACTED
...
REDACTED
	...
	if probeAccount {
		if enabled, ok, err := decodeAccountExtraJSON(currentEnabled); err != nil {
			return nil, err
	REDACTED else if value, isBool := enabled.(bool); ok && isBool {
			probeEnabled = value
...
REDACTED
	...
	REDACTED
		if explicitProbeEnabled != nil && !*explicitProbeEnabled {
			rateSyncEnabled = false
			rateSyncEnabledPresent = true
	REDACTED else if rateSyncEnabled {
			probeEnabled = true
			probeEnabledPresent = true
	REDACTED
		if !probeEnabled {
	...
	result, err := client.ExecContext(ctx, `
		...
		SET
			...
			extra = CASE
				...
				THEN COALESCE(extra, '{REDACTED'::jsonb)
					- 'upstream_billing_probe'
				...
				WHEN type = 'apikey'
					AND credentials IS DISTINCT FROM $1::jsonb
				THEN COALESCE(extra, '{REDACTED'::jsonb) - 'upstream_billing_probe'
				ELSE extra
			END,
			updated_at = NOW()
...
REDACTED
	...
	if sortOrder == pagination.SortOrderDesc {
		return []func(*entsql.Selector){dbent.Desc(field), dbent.Desc(dbaccount.FieldID)REDACTED
REDACTED
	if defaultOrder {
...
REDACTED
	if len(ids) == 0 {
		return &service.OAuthRefreshCandidatePage{Accounts: []service.Account{REDACTEDREDACTED, nil
REDACTED
...
REDACTED
	payload := map[string]any{"last_used": lastUsedPayloadREDACTED
...
func (r *accountRepository) AddToGroup(ctx context.Context, accountID, groupID int64, priority int) error {
	...
	if err != nil {
		return err
REDACTED
	payload := buildSchedulerGroupPayload([]int64{groupIDREDACTED)
...
func (r *accountRepository) ListSchedulableCapacityByGroupIDs(ctx context.Context, groupIDs []int64) ([]service.GroupAccountCapacityRow, error) {
	...
	if len(groupIDs) == 0 {
		return []service.GroupAccountCapacityRow{REDACTED, nil
REDACTED
	if r.sql == nil {
...
REDACTED
	...
	rows, err := r.sql.QueryContext(ctx, `
		SELECT
			...
			a.id AS account_id,
			a.concurrency,
			COALESCE(a.extra, '{REDACTED'::jsonb)::text AS extra,
...
REDACTED
	...
	result, err := client.ExecContext(
		...
		`UPDATE accounts SET 
			extra = jsonb_set(
				jsonb_set(COALESCE(extra, '{REDACTED'::jsonb), '{model_rate_limitsREDACTED'::text[], COALESCE(extra->'model_rate_limits', '{REDACTED'::jsonb), true),
...
func (r *accountRepository) ClearAntigravityQuotaScopes(ctx context.Context, id int64) error {
	...
	result, err := client.ExecContext(
		ctx,
		"UPDATE accounts SET extra = COALESCE(extra, '{REDACTED'::jsonb) - 'antigravity_quota_scopes', updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL",
		id,
	)
	if err != nil {
		return err
...
REDACTED
	...
	if len(accountIDs) > 0 {
		...
		payload := map[string]any{"account_ids": accountIDsREDACTED
...
func (r *accountRepository) updateUpstreamBillingProbeSnapshotInTx(
	...
	payload, err := json.Marshal(map[string]any{service.UpstreamBillingProbeExtraKey: snapshotREDACTED)
	if err != nil {
		return err
REDACTED
...
REDACTED
	result, err := client.ExecContext(ctx, `
		UPDATE accounts
		SET
			extra = COALESCE(extra, '{REDACTED'::jsonb) || $1::jsonb,
			rate_multiplier = CASE
				WHEN $10::numeric IS NOT NULL
					AND extra @> '{"upstream_billing_probe_enabled": trueREDACTED'::jsonb
					AND extra @> '{"upstream_billing_rate_sync_enabled": trueREDACTED'::jsonb
				THEN $10::numeric
...
REDACTED
	...
	REDACTED
		...
		setClauses = append(setClauses, "credentials = COALESCE(credentials, '{REDACTED'::jsonb) || "+credentialPlaceholder+"::jsonb")
...
func mergeGroupIDs(a []int64, b []int64) []int64 {
	seen := make(map[int64]struct{REDACTED, len(a)+len(b))
...
func buildSchedulerGroupPayload(groupIDs []int64) any {
	if len(groupIDs) == 0 {
		return nil
REDACTED
	return map[string]any{"group_ids": groupIDsREDACTED
REDACTED
	...
	accounts, err := r.client.Account.Query().
		Where(
			...
			func(s *entsql.Selector) {
				...
				switch v := value.(type) {
				case string:
					preds := []*entsql.Predicate{sqljson.ValueEQ(dbaccount.FieldExtra, v, path)REDACTED
...
REDACTED
	...
	rows, err := r.sql.QueryContext(ctx, `
		WITH candidates AS (
			SELECT
				id,
				extra #>> '{upstream_billing_probe,statusREDACTED' AS probe_status,
				extra #>> '{upstream_billing_probe,next_probe_atREDACTED' AS next_probe_at
			FROM accounts
			WHERE deleted_at IS NULL
				AND status = 'active'
				AND type = 'apikey'
			...
			SELECT
				...
				probe_status,
				next_probe_at,
				next_probe_at ~ '^[0-9]{4REDACTED-[0-9]{2REDACTED-[0-9]{2REDACTEDT[0-9]{2REDACTED:[0-9]{2REDACTED:[0-9]{2REDACTED(\.[0-9]+)?(Z|[+-][0-9]{2REDACTED:[0-9]{2REDACTED)$' AS rfc3339_shape,
				jsonb_path_query_first_tz(
					...
					'{REDACTED'::jsonb,
					true
				) #>> '{REDACTED' AS parsed_next_probe_at
			FROM candidates
		), normalized AS (
			SELECT
				id,
...
func (r *accountRepository) ResetQuotaUsed(ctx context.Context, id int64) error {
	_, err := r.sql.ExecContext(ctx,
		`UPDATE accounts SET extra = (
			COALESCE(extra, '{REDACTED'::jsonb)
			|| '{"quota_used": 0, "quota_daily_used": 0, "quota_weekly_used": 0REDACTED'::jsonb
