...
REDACTED
	...
	success := &service.Account{
		...
		Schedulable: false,
REDACTED"api_key": "secret"REDACTED,
		Extra:       map[string]any{REDACTED,
REDACTED
	require.NoError(t, repo.CreateWithAccountGroups(ctx, success, []service.AccountGroup{{GroupID: group.ID, Priority: 37REDACTEDREDACTED))
	t.Cleanup(func() {
...
REDACTED"api_key": "secret"REDACTED,
		Extra:       map[string]any{REDACTED,
REDACTED
	err = repo.CreateWithAccountGroups(ctx, failure, []service.AccountGroup{{GroupID: int64(^uint64(0) >> 1), Priority: 1REDACTEDREDACTED)
REDACTED
