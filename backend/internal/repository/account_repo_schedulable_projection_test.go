...
func TestListSchedulableAccountLoadsUsesSingleProjectionQuery(t *testing.T) {
	...
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(captureEntQueryMatcher{actual: &capturedSQLREDACTED))
REDACTED
	...
	mock.ExpectQuery("schedulable account load projection").
		WillReturnRows(sqlmock.NewRows([]string{"id", "concurrency", "load_factor"REDACTED).
