...
func (s *AccountRepoSuite) TestList_DefaultSortByNameAsc() {
	mustCreateAccount(s.T(), s.client, &service.Account{Name: "z-account"REDACTED)
	mustCreateAccount(s.T(), s.client, &service.Account{Name: "a-account"REDACTED)

	accounts, _, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10REDACTED)
...
func (s *AccountRepoSuite) TestListWithFilters_SortByPriorityDesc() {
	mustCreateAccount(s.T(), s.client, &service.Account{Name: "low-priority", Priority: 10REDACTED)
	mustCreateAccount(s.T(), s.client, &service.Account{Name: "high-priority", Priority: 90REDACTED)
	...
	makeAccount := func(name, status string, rate any) {
		...
		if rate != nil {
			extra[service.UpstreamBillingProbeExtraKey] = map[string]any{
				"status": status,
				"data":   map[string]any{"effective_rate_multiplier": rateREDACTED,
		REDACTED
	REDACTED
		mustCreateAccount(s.T(), s.client, &service.Account{Name: name, Extra: extraREDACTED)
REDACTED
	...
	for _, tc := range []struct {
		...
		want  []string
REDACTED{
		{order: "asc", want: []string{"low-rate", "high-rate", "missing-rate", "unsupported-with-retained-rate"REDACTEDREDACTED,
		{order: "desc", want: []string{"high-rate", "low-rate", "unsupported-with-retained-rate", "missing-rate"REDACTEDREDACTED,
REDACTED {
		accounts, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{
...
func (s *AccountRepoSuite) TestListWithFilters_SortByCurrentUpstreamBillingRateDuringPeak() {
	now := time.Now()
	locations := []string{"UTC", "Asia/Shanghai", "America/New_York", "Europe/London"REDACTED
	var timezone string
	var minute int
...
REDACTED)
	...
	for _, tc := range []struct {
		...
		want  []string
REDACTED{
		{order: "asc", want: []string{"current-off-peak-rate", "current-peak-rate"REDACTEDREDACTED,
		{order: "desc", want: []string{"current-peak-rate", "current-off-peak-rate"REDACTEDREDACTED,
REDACTED {
		accounts, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{
			Page: 1, PageSize: 10, SortBy: "upstream_billing_rate", SortOrder: tc.order,
	REDACTED, "", "", "", "", 0, "")
