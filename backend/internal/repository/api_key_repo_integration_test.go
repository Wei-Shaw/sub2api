...
REDACTED
	...
	err := s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{Name: true, Status: trueREDACTED)
...
REDACTED
	...
	key.GroupID = nil
	err := s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{GroupID: trueREDACTED)
	s.Require().NoError(err, "Update")
...
func (s *APIKeyRepoSuite) TestListByUserID() {
	...
	keys, page, err := s.repo.ListByUserID(s.ctx, user.ID, pagination.PaginationParams{Page: 1, PageSize: 10REDACTED, service.APIKeyListFilters{REDACTED)
...
REDACTED
	...
	keys, page, err := s.repo.ListByUserID(s.ctx, user.ID, pagination.PaginationParams{Page: 1, PageSize: 2REDACTED, service.APIKeyListFilters{REDACTED)
	s.Require().NoError(err)
	s.Require().Len(keys, 2)
...
func (s *APIKeyRepoSuite) TestCRUD_Search_ClearGroupID() {
	...
	key.Status = service.StatusDisabled
	key.GroupID = nil
	s.Require().NoError(s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{Name: true, Status: true, GroupID: trueREDACTED), "Update")
...
func (s *APIKeyRepoSuite) TestIncrementQuotaUsedAndGetState() {
	...
	key.Quota = 3
	key.QuotaUsed = 1
	s.Require().NoError(s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{Quota: true, QuotaUsed: trueREDACTED), "Update quota")
...
func (s *APIKeyRepoSuite) TestDeleteWithAudit_RepeatIsIdempotent() {
	...
	key := &service.APIKey{UserID: user.ID, Key: "sk-del-audit-2", Name: "K", Status: service.StatusActiveREDACTED
	s.Require().NoError(s.repo.Create(s.ctx, key))
