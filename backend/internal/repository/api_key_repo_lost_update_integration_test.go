...
REDACTED
	...
	stale.Name = "after"
	s.Require().NoError(
		s.repo.Update(s.ctx, stale, service.APIKeyUpdateFields{Name: trueREDACTED),
		"Update",
	)
...
REDACTED
	...
	s.Require().NoError(
		s.repo.Update(s.ctx, current, service.APIKeyUpdateFields{QuotaUsed: true, RateLimitUsage: trueREDACTED),
		"Update",
	)
