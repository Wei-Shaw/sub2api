...
func (c *billingCache) DeductUserBalance(ctx context.Context, userID int64, amount float64) error {
	key := billingBalanceKey(userID)
	_, err := deductBalanceScript.Run(ctx, c.rdb, []string{keyREDACTED, amount, int(jitteredTTL().Seconds())).Result()
...
func (c *billingCache) UpdateSubscriptionUsage(ctx context.Context, userID, groupID int64, cost float64) error {
	key := billingSubKey(userID, groupID)
	_, err := updateSubUsageScript.Run(ctx, c.rdb, []string{keyREDACTED, cost, int(jitteredTTL().Seconds())).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
...
REDACTED
	if len(result) == 0 {
		return nil, redis.Nil
REDACTED
	data := &service.APIKeyRateLimitCacheData{REDACTED
...
func (c *billingCache) UpdateAPIKeyRateLimitUsage(ctx context.Context, keyID int64, cost float64) error {
	key := billingRateLimitKey(keyID)
	now := time.Now().Unix()
	_, err := updateRateLimitUsageScript.Run(ctx, c.rdb, []string{keyREDACTED,
		cost,
...
func userPlatformQuotaDirtySetKey() string { return "billing:" + "upq:dirty" REDACTED
...
REDACTED
	_, err := c.rdb.Eval(ctx, updateUserPlatformQuotaUsageScript,
		[]string{userPlatformQuotaCacheKey(userID, platform), userPlatformQuotaDirtySetKey()REDACTED,
...
func parseUserPlatformQuotaDirtyMember(m string) (service.UserPlatformQuotaKey, bool) {
	...
	if len(parts) != 2 {
		return service.UserPlatformQuotaKey{REDACTED, false
REDACTED
	uid, err := strconv.ParseInt(parts[0], 10, 64)
	if err != nil {
		return service.UserPlatformQuotaKey{REDACTED, false
REDACTED
	return service.UserPlatformQuotaKey{UserID: uid, Platform: parts[1]REDACTED, true
REDACTED
