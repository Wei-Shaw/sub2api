...
const (
	...
	rateLimitCacheTTL         = 31 * 24 * time.Hour // Keep Redis data alive across the monthly window.
...
var (
	...
	// updateRateLimitUsageScript atomically increments all rate limit usage counters
	...
	//
	// ARGV: [1]=cost, [2]=ttl_seconds, [3]=now_unix, [4]=window_5h_seconds, [5]=window_1d_seconds, [6]=window_7d_seconds, [7]=window_1mo_seconds
	updateRateLimitUsageScript = redis.NewScript(`
		...
		update_window('usage_1mo', 'window_1mo', win1mo)
		redis.call('EXPIRE', KEYS[1], ARGV[2])
		return 1
	`)
)
...
func (c *billingCache) GetAPIKeyRateLimit(ctx context.Context, keyID int64) (*service.APIKeyRateLimitCacheData, error) {
	...
	if v, ok := result[rateLimitFieldUsage1mo]; ok {
		data.Usage1mo, _ = strconv.ParseFloat(v, 64)
	}
	...
	if v, ok := result[rateLimitFieldWindow1mo]; ok {
		data.Window1mo, _ = strconv.ParseInt(v, 10, 64)
	}
	return data, nil
}
...
func (c *billingCache) UpdateAPIKeyRateLimitUsage(ctx context.Context, keyID int64, cost float64) error {
	...
	_, err := updateRateLimitUsageScript.Run(ctx, c.rdb, []string{key},
		...
		int(rateLimitWindow1mo.Seconds()),
	).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
