...
REDACTED
	...
	restored := entToServiceMonitor(&dbent.ChannelMonitor{ExtraHeaders: persistedREDACTED)
	require.Equal(t, "operation-digest", restored.DuplicateOperationID)
	require.Equal(t, map[string]string{"User-Agent": "Codex"REDACTED, restored.ExtraHeaders)
