...
REDACTED"api_key": "sk-kimi", "account_mode": service.AccountModeCodingREDACTED,
REDACTED)
	t.Cleanup(func() {
	...
	rows := []*service.ChannelMonitorHistoryRow{
		{
			...
			Quota: &domain.MonitorQuotaSnapshot{
				...
				Tiers: []domain.MonitorQuotaTier{
					{Window: "5h", UsedPercent: 42.5, Used: 17, Limit: 40, ResetAt: now.Add(time.Hour).Format(time.RFC3339)REDACTED,
			REDACTED,
				FetchedAt: now,
		REDACTED,
	REDACTED,
...
REDACTED
	...
	byModel := map[string]*service.ChannelMonitorHistoryEntry{REDACTED
...
REDACTED
	...
	// 用户视图聚合：主模型最近一行带快照。
	latest, err := repo.ListLatestForMonitorIDs(ctx, []int64{monitor.IDREDACTED)
REDACTED
