...
func NewChannelMonitorRepository(client *dbent.Client, db *sql.DB) service.ChannelMonitorRepository {
	return &channelMonitorRepository{client: client, db: dbREDACTED
REDACTED
...
REDACTED
	...
	for rows.Next() {
		l := &service.ChannelMonitorLatest{REDACTED
...
// 集中实现避免 latency / ping 两处重复 if latency.Valid { v := int(...) ... REDACTED 模板。
...
func scanMonitorQuota(data []byte) *domain.MonitorQuotaSnapshot {
	if len(data) == 0 {
		return nil
REDACTED
	snapshot := &domain.MonitorQuotaSnapshot{REDACTED
...
func scanAvailabilityRow(rows interface{ Scan(...any) error REDACTED, windowDays int) (*service.ChannelMonitorAvailability, error) {
	row := &service.ChannelMonitorAvailability{WindowDays: windowDaysREDACTED
...
REDACTED
	...
	for rows.Next() {
		var monitorID int64
		entry := &service.ChannelMonitorHistoryEntry{REDACTED
		var latency, ping sql.NullInt64
...
func channelMonitorHeadersForPersistence(m *service.ChannelMonitor) map[string]string {
	if m == nil {
		return map[string]string{REDACTED
REDACTED
