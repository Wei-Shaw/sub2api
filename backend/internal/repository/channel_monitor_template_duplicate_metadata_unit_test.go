...
REDACTED
	...
	monitorIDs := []int64{41, 42REDACTED
	...
	mock.ExpectExec(`(?s)UPDATE channel_monitors\s+SET extra_headers = \$1::jsonb \|\| CASE\s+WHEN COALESCE\(extra_headers, '\{\REDACTED'::jsonb\) \? \(\$2::text\)\s+THEN jsonb_build_object\(\$2::text, COALESCE\(extra_headers, '\{\REDACTED'::jsonb\) -> \(\$2::text\)\)\s+ELSE '\{\REDACTED'::jsonb\s+END\s+WHERE template_id = \$3\s+AND id = ANY\(\$4\)\s+AND provider = \$5\s+AND api_mode = \$6`).
		WithArgs(
			`{"User-Agent":"template-client"REDACTED`,
...
REDACTED
	...
	repo := NewChannelMonitorRequestTemplateRepository(client, db)
	affected, err := repo.ApplyToMonitors(context.Background(), templateID, []int64{41, 42REDACTED)
...
func expectChannelMonitorTemplateForApply(mock sqlmock.Sqlmock, templateID int64) {
	...
	REDACTED).AddRow(
			...
			service.MonitorAPIModeResponses, "", []byte(`{"User-Agent":"template-client"REDACTED`),
