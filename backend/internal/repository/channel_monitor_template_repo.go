...
func NewChannelMonitorRequestTemplateRepository(client *dbent.Client, db *sql.DB) service.ChannelMonitorRequestTemplateRepository {
	return &channelMonitorRequestTemplateRepository{client: client, db: dbREDACTED
REDACTED
...
REDACTED
	result, err := client.ExecContext(ctx, `
		...
		SET extra_headers = $1::jsonb || CASE
			WHEN COALESCE(extra_headers, '{REDACTED'::jsonb) ? ($2::text)
			THEN jsonb_build_object($2::text, COALESCE(extra_headers, '{REDACTED'::jsonb) -> ($2::text))
			ELSE '{REDACTED'::jsonb
		END
		WHERE template_id = $3
		  AND id = ANY($4)
