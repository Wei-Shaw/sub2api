...
func (r *channelRepository) batchLoadAccountStatsModelPricing(ctx context.Context, ruleIDs []int64) (map[int64][]service.ChannelModelPricing, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, rule_id, platform, models, billing_mode, input_price, output_price,
		        cache_write_price, cache_read_price, image_output_price, per_request_price, time_pricing, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := unmarshalTimePricing(timePricingJSON, &p.TimePricing); err != nil {
			return nil, fmt.Errorf("unmarshal account stats time pricing: %w", err)
		}
...
func createAccountStatsModelPricingTx(ctx context.Context, tx *sql.Tx, ruleID int64, pricing *service.ChannelModelPricing) error {
	...
	err = tx.QueryRowContext(ctx,
		`INSERT INTO channel_account_stats_model_pricing (rule_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_output_price, per_request_price, time_pricing)
