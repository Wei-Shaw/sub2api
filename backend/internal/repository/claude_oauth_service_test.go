...
REDACTED{
		{
			...
			handler: func(w http.ResponseWriter, r *http.Request) {
				...
				_, _ = w.Write([]byte(`[{"uuid":"org-1"REDACTED]`))
		REDACTED,
			wantUUID: "org-1",
			validate: func(captured requestCapture) {
...
REDACTED{
		{
			...
			handler: func(w http.ResponseWriter, r *http.Request) {
				w.Header().Set("Content-Type", "application/json")
				_ = json.NewEncoder(w).Encode(oauth.TokenResponse{AccessToken: "at2", TokenType: "bearer", ExpiresIn: 3600REDACTED)
		REDACTED,
			wantResp: &oauth.TokenResponse{AccessToken: "at2"REDACTED,
			validate: func(captured requestCapture) {
				require.Equal(s.T(), http.MethodPost, captured.method, "expected POST")
