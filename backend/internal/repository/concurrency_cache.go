...
//
// 性能优化说明：
// 原实现使用 SCAN 命令遍历独立的槽位键（concurrency:account:{idREDACTED:{requestIDREDACTED），
...
const (
	// 并发槽位键前缀（有序集合）
	// 格式: concurrency:account:{accountIDREDACTED
	accountSlotKeyPrefix = "concurrency:account:"
	// 格式: concurrency:user:{userIDREDACTED
...
var (
	...
	// ARGV[2] = TTL（秒）
	// ARGV[3] = requestID
	// 返回 {是否成功, Redis 当前秒REDACTED，Go 侧复用同一时间源写活跃索引，省去额外 TIME 往返。
	...
	// 返回 {是否成功, Redis 当前秒REDACTED，供 Go 侧免额外 TIME 往返写活跃索引。
	...
	// 返回值同 incrementWaitScript：{是否成功, Redis 当前秒REDACTED。
	...
	// 返回 {清除数量, 剩余成员数REDACTED，Go 侧据剩余数决定索引 member 去留，无需再回读槽位。
	startupCleanupSlotScript = redis.NewScript(`
		...
		return {removed, remainingREDACTED
	`)
)
...
var (
	accountSlotIndex = slotIndexSpec{indexKey: accountActiveIndexKey, slotKey: accountSlotKey, waitKey: accountWaitKeyREDACTED
	userSlotIndex    = slotIndexSpec{indexKey: userActiveIndexKey, slotKey: userSlotKey, waitKey: waitQueueKeyREDACTED
)
...
func (c *concurrencyCache) readActiveLoadForKey(ctx context.Context, id int64, slotKey, waitKey string, now int64) (activeIndexLoad, error) {
	...
	if _, err := pipe.Exec(ctx); err != nil && !errors.Is(err, redis.Nil) {
		return activeIndexLoad{REDACTED, fmt.Errorf("pipeline exec: %w", err)
REDACTED
...
func (c *concurrencyCache) readIndexLoads(ctx context.Context, spec slotIndexSpec, members []string, now int64) ([]activeIndexLoad, []string, error) {
	...
	for _, member := range members {
		...
		if err != nil || id <= 0 {
			...
			continue
	REDACTED
		candidates = append(candidates, activeIndexLoad{id: id, member: memberREDACTED)
REDACTED
...
// runScriptInt64Pair 执行返回两元素整数数组的 Lua 脚本并解析（如 {result, nowREDACTED、{removed, remainingREDACTED）。
...
func (c *concurrencyCache) AcquireAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, requestID string) (bool, error) {
	...
	result, now, err := runScriptInt64Pair(ctx, c.rdb, acquireScript, []string{key, liveAccountSlotKey(accountID)REDACTED, maxConcurrency, c.slotTTLSeconds, requestID)
	if err != nil {
		return false, err
REDACTED
	if result == 1 {
...
func (c *concurrencyCache) GetAccountConcurrency(ctx context.Context, accountID int64) (int, error) {
	...
	result, err := getCountScript.Run(ctx, c.rdb, []string{key, liveAccountSlotKey(accountID)REDACTED, c.slotTTLSeconds).Int()
	if err != nil {
		return 0, err
REDACTED
	return result, nil
...
func (c *concurrencyCache) AcquireUserSlot(ctx context.Context, userID int64, maxConcurrency int, requestID string) (bool, error) {
	...
	// 时间戳在 Lua 脚本内使用 Redis TIME 命令获取，确保多实例时钟一致
	result, now, err := runScriptInt64Pair(ctx, c.rdb, acquireScript, []string{key, liveUserSlotKey(userID)REDACTED, maxConcurrency, c.slotTTLSeconds, requestID)
	if err != nil {
		return false, err
REDACTED
	if result == 1 {
...
func (c *concurrencyCache) GetUserConcurrency(ctx context.Context, userID int64) (int, error) {
	key := userSlotKey(userID)
	// 时间戳在 Lua 脚本内使用 Redis TIME 命令获取
	result, err := getCountScript.Run(ctx, c.rdb, []string{key, liveUserSlotKey(userID)REDACTED, c.slotTTLSeconds).Int()
	if err != nil {
		return 0, err
REDACTED
	return result, nil
...
func (c *concurrencyCache) TrackAPIKeySlot(ctx context.Context, apiKeyID int64, requestID string) error {
	...
	_, err := trackSlotScript.Run(ctx, c.rdb, []string{keyREDACTED, c.slotTTLSeconds, requestID).Result()
	return err
REDACTED
...
REDACTED, accountMax, userMax, liveLeaseTTLSeconds, leaseID, replacing).Int()
	return result == 1, err
REDACTED
...
REDACTED
	result, err := refreshLiveLeaseScript.Run(ctx, c.rdb, []string{
		...
		liveUserSlotKey(userID),
		liveAPIKeySlotKey(apiKeyID),
REDACTED, liveLeaseTTLSeconds, leaseID).Int()
	return result == 1, err
REDACTED
...
func (c *concurrencyCache) IncrementWaitCount(ctx context.Context, userID int64, maxWait int) (bool, error) {
	...
	result, now, err := runScriptInt64Pair(ctx, c.rdb, incrementWaitScript, []string{keyREDACTED, maxWait, c.waitQueueTTLSeconds)
	if err != nil {
		return false, err
REDACTED
	if result == 1 {
...
func (c *concurrencyCache) DecrementWaitCount(ctx context.Context, userID int64) error {
	key := waitQueueKey(userID)
	_, err := decrementWaitScript.Run(ctx, c.rdb, []string{keyREDACTED).Result()
	if err == nil {
		// 等待数减少后重新判断是否还需要保留索引。
		c.refreshUserActiveIndex(ctx, userID)
REDACTED
...
func (c *concurrencyCache) IncrementAccountWaitCount(ctx context.Context, accountID int64, maxWait int) (bool, error) {
	...
	result, now, err := runScriptInt64Pair(ctx, c.rdb, incrementAccountWaitScript, []string{keyREDACTED, maxWait, c.waitQueueTTLSeconds)
	if err != nil {
		return false, err
REDACTED
	if result == 1 {
...
func (c *concurrencyCache) CleanupExpiredAccountSlots(ctx context.Context, accountID int64) error {
	key := accountSlotKey(accountID)
	_, err := cleanupExpiredSlotsScript.Run(ctx, c.rdb, []string{keyREDACTED, c.slotTTLSeconds).Result()
	if err == nil {
...
REDACTED
	if exists > 0 {
		return nil
REDACTED
	for _, pattern := range []string{accountWaitKeyPrefix + "*", waitQueueKeyPrefix + "*"REDACTED {
		var cursor uint64
		for {
...
func (c *concurrencyCache) cleanupStaleProcessSlotsForIndex(
	...
	REDACTED
		...
		_, remaining, err := runScriptInt64Pair(ctx, c.rdb, startupCleanupSlotScript, []string{spec.slotKey(id)REDACTED, activeRequestPrefix, c.slotTTLSeconds)
		if err != nil {
