...
var (
	...
	// KEYS[1] = 有序集合键 (concurrency:account:{idREDACTED / concurrency:user:{idREDACTED)
...
func (c *concurrencyCache) AcquireAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, requestID string) (bool, error) {
	...
	result, now, err := runScriptInt64Pair(ctx, c.rdb, acquireScript, []string{keyREDACTED, maxConcurrency, c.slotTTLSeconds, requestID)
	if err != nil {
		return false, err
REDACTED
	if result == 1 {
...
func (c *concurrencyCache) GetAccountConcurrency(ctx context.Context, accountID int64) (int, error) {
	...
	result, err := getCountScript.Run(ctx, c.rdb, []string{keyREDACTED, c.slotTTLSeconds).Int()
	if err != nil {
		return 0, err
REDACTED
	return result, nil
