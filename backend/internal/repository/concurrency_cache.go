...
const (
	...
	// Format: concurrency:account:{accountIDREDACTED:{requestIDREDACTED
	accountSlotKeyPrefix = "concurrency:account:"
	// Format: concurrency:user:{userIDREDACTED:{requestIDREDACTED
	userSlotKeyPrefix = "concurrency:user:"
	// Wait queue keeps counter format: concurrency:wait:{userIDREDACTED
...
func NewConcurrencyCache(rdb *redis.Client) ports.ConcurrencyCache {
	return &concurrencyCache{rdb: rdbREDACTED
REDACTED
...
func (c *concurrencyCache) AcquireAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, requestID string) (bool, error) {
	...
	result, err := acquireScript.Run(ctx, c.rdb, []string{pattern, slotKeyREDACTED, maxConcurrency, int(slotTTL.Seconds())).Int()
	if err != nil {
		return false, err
REDACTED
	return result == 1, nil
...
func (c *concurrencyCache) GetAccountConcurrency(ctx context.Context, accountID int64) (int, error) {
	pattern := accountSlotPattern(accountID)
	result, err := getCountScript.Run(ctx, c.rdb, []string{patternREDACTED).Int()
	if err != nil {
		return 0, err
REDACTED
	return result, nil
...
func (c *concurrencyCache) IncrementWaitCount(ctx context.Context, userID int64, maxWait int) (bool, error) {
	...
	result, err := incrementWaitScript.Run(ctx, c.rdb, []string{keyREDACTED, maxWait, int(slotTTL.Seconds())).Int()
	if err != nil {
		return false, err
REDACTED
	return result == 1, nil
