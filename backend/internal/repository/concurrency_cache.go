...
var (
	...
	// KEYS[1] = concurrency:account:{accountIDREDACTED
	// ARGV[1] = TTL (seconds)
...
func (c *concurrencyCache) IncrementWaitCount(ctx context.Context, userID int64, maxWait int) (bool, error) {
	...
	result, err := incrementWaitScript.Run(
		ctx,
		c.rdb,
		[]string{waitQueueTotalKey, waitQueueUpdatedKey, waitQueueCountsKeyREDACTED,
		userKey,
		maxWait,
...
func (c *concurrencyCache) DecrementAccountWaitCount(ctx context.Context, accountID int64) error {
	key := accountWaitKey(accountID)
	_, err := decrementAccountWaitScript.Run(ctx, c.rdb, []string{keyREDACTED).Result()
	return err
REDACTED
...
REDACTED
	...
	args := []any{c.slotTTLSecondsREDACTED
	for _, acc := range accounts {
		args = append(args, acc.ID, acc.MaxConcurrency)
REDACTED

	result, err := getAccountsLoadBatchScript.Run(ctx, c.rdb, []string{REDACTED, args...).Slice()
	if err != nil {
		return nil, err
REDACTED
