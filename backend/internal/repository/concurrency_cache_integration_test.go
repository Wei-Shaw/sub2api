...
func (s *ConcurrencyCacheSuite) TestOpenAIWSIngressAPIKeySlot_ReapsCrashedLeaseWithoutDeletingLiveOtherInstance() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, key,
		redis.Z{Score: float64(now - openAIWSIngressLeaseTTLSeconds - 1), Member: "crashed-instance"REDACTED,
		redis.Z{Score: float64(now), Member: "live-other-instance"REDACTED,
	).Err())
...
func (s *ConcurrencyCacheSuite) TestAPIKeySlot_TrackReleaseAndBatchCount() {
	...
	counts, err := cache.GetAPIKeyConcurrencyBatch(s.ctx, []int64{apiKeyID, emptyAPIKeyIDREDACTED)
	require.NoError(s.T(), err)
	require.Equal(s.T(), map[int64]int{apiKeyID: 2, emptyAPIKeyID: 0REDACTED, counts)
	...
	counts, err = cache.GetAPIKeyConcurrencyBatch(s.ctx, []int64{apiKeyIDREDACTED)
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountKey,
		redis.Z{Score: float64(now), Member: "oldproc-1"REDACTED,
		redis.Z{Score: float64(now), Member: "keep-1"REDACTED,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userKey,
		redis.Z{Score: float64(now), Member: "oldproc-2"REDACTED,
		redis.Z{Score: float64(now), Member: "keep-2"REDACTED,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, unindexedAccountKey,
		redis.Z{Score: float64(now), Member: "oldproc-unindexed"REDACTED,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, apiKeyKey,
		redis.Z{Score: float64(now), Member: "oldproc-3"REDACTED,
		redis.Z{Score: float64(now), Member: "keep-3"REDACTED,
	).Err())
...
REDACTED).Err())
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"keep-1"REDACTED, accountMembers)

	userMembers, err := s.rdb.ZRange(s.ctx, userKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"keep-2"REDACTED, userMembers)

	// API Key 槽位（stats-only）不在启动清理范围内，靠分数裁剪与 key TTL 自愈。
	apiKeyMembers, err := s.rdb.ZRange(s.ctx, apiKeyKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.ElementsMatch(s.T(), []string{"keep-3", "oldproc-3"REDACTED, apiKeyMembers)
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"oldproc-unindexed"REDACTED, unindexedMembers)
...
func (s *ConcurrencyCacheSuite) TestGetAccountsLoadBatch() {
	...
	accounts := []service.AccountWithConcurrency{
		{ID: account1, MaxConcurrency: 3REDACTED,
		{ID: account2, MaxConcurrency: 2REDACTED,
		{ID: account3, MaxConcurrency: 1REDACTED,
REDACTED
...
func (s *ConcurrencyCacheSuite) TestGetAccountsLoadBatch_Empty() {
	...
	loadMap, err := s.cache.GetAccountsLoadBatch(s.ctx, []service.AccountWithConcurrency{REDACTED)
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupExpiredAccountSlots() {
	...
	err = s.rdb.ZAdd(s.ctx, slotKey, redis.Z{Score: float64(expiredTime), Member: "req1"REDACTED).Err()
	require.NoError(s.T(), err)
	err = s.rdb.ZAdd(s.ctx, slotKey, redis.Z{Score: float64(expiredTime), Member: "req2"REDACTED).Err()
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupExpiredAccountSlotKeys() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountKeyWithFresh,
		redis.Z{Score: float64(expiredTime), Member: "expired"REDACTED,
		redis.Z{Score: float64(now), Member: "fresh"REDACTED,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountKeyExpiredOnly,
		redis.Z{Score: float64(expiredTime), Member: "expired-only"REDACTED,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userKey,
		redis.Z{Score: float64(expiredTime), Member: "user-expired"REDACTED,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, unindexedAccountKey,
		redis.Z{Score: float64(expiredTime), Member: "unindexed-expired"REDACTED,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountActiveIndexKey,
		redis.Z{Score: float64(now), Member: "301"REDACTED,
		redis.Z{Score: float64(now), Member: "302"REDACTED,
	).Err())
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"fresh"REDACTED, accountMembers)
	...
	userMembers, err := s.rdb.ZRange(s.ctx, userKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"user-expired"REDACTED, userMembers)

	unindexedMembers, err := s.rdb.ZRange(s.ctx, unindexedAccountKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"unindexed-expired"REDACTED, unindexedMembers)
...
func (s *ConcurrencyCacheSuite) TestCleanupExpiredAccountSlotKeys_ReapsUserIndex() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userActiveIndexKey,
		redis.Z{Score: expiredScore, Member: "401"REDACTED,
		redis.Z{Score: expiredScore, Member: "402"REDACTED,
		redis.Z{Score: expiredScore, Member: "not-a-user-id"REDACTED,
	).Err())
...
REDACTED).Err())
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"activeproc-1"REDACTED, accountMembers)

	userMembers, err := s.rdb.ZRange(s.ctx, userSlotKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"activeproc-2"REDACTED, userMembers)
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_DeletesEmptySlotKeys() {
	...
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountSlotKey, redis.Z{Score: float64(now), Member: "oldproc-1"REDACTED).Err())
	require.NoError(s.T(), s.rdb.Expire(s.ctx, accountSlotKey, testSlotTTL).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountActiveIndexKey, redis.Z{
		Score:  float64(now + 60),
		Member: strconv.FormatInt(accountID, 10),
