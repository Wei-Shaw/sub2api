...
func NewContentModerationRepository(db *sql.DB) service.ContentModerationRepository {
	return &contentModerationRepository{db: dbREDACTED
REDACTED
...
REDACTED
	if params.PageSize > 100 {
		params.PageSize = 100
REDACTED
	queryArgs := append([]any{REDACTED, args...)
...
REDACTED
	...
	REDACTED
		if queueDelay.Valid {
			...
			item.QueueDelayMS = &v
	REDACTED
		item.CategoryScores = map[string]float64{REDACTED
		_ = json.Unmarshal(scoresRaw, &item.CategoryScores)
		item.ThresholdSnapshot = map[string]float64{REDACTED
...
func (r *contentModerationRepository) CleanupExpiredLogs(ctx context.Context, hitBefore time.Time, nonHitBefore time.Time) (*service.ContentModerationCleanupResult, error) {
	result := &service.ContentModerationCleanupResult{FinishedAt: time.Now()REDACTED
	if r == nil || r.db == nil {
		return result, nil
REDACTED
...
func buildContentModerationLogWhere(filter service.ContentModerationLogFilter) ([]string, []any) {
	where := []string{"l.id IS NOT NULL"REDACTED
	args := make([]any, 0)
