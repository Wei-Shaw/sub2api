...
func NewGatewayCache(rdb *redis.Client) service.GatewayCache {
	return &gatewayCache{rdb: rdbREDACTED
REDACTED
...
// 格式: client_affinity:{groupIDREDACTED:{clientIDREDACTED
...
// 格式: client_affinity_rev:{groupIDREDACTED:{accountIDREDACTED
...
func (c *gatewayCache) GetClientAffinityAccounts(ctx context.Context, groupID int64, clientID string, ttl time.Duration) ([]int64, error) {
	...
	result, err := getAffinityScript.Run(ctx, c.rdb, []string{keyREDACTED, expireThreshold).StringSlice()
	if err != nil {
		if err == redis.Nil {
			return nil, nil
	REDACTED
...
func (c *gatewayCache) UpdateClientAffinity(ctx context.Context, groupID int64, clientID string, accountID int64, ttl time.Duration) error {
	...
	return updateAffinityScript.Run(ctx, c.rdb, []string{fwdKey, revKeyREDACTED,
...
func (c *gatewayCache) GetAccountAffinityCountBatch(ctx context.Context, groupID int64, accountIDs []int64, ttl time.Duration) (map[int64]int64, error) {
	if len(accountIDs) == 0 {
		return map[int64]int64{REDACTED, nil
REDACTED
	...
	for i, accID := range accountIDs {
		...
		cmds[i] = getAffinityCountScript.Run(ctx, pipe, []string{keyREDACTED, expireThreshold)
REDACTED
	_, err := pipe.Exec(ctx)
...
func (c *gatewayCache) GetAccountAffinityClientsBatch(ctx context.Context, accountGroups map[int64][]int64, ttl time.Duration) (map[int64][]string, error) {
	if len(accountGroups) == 0 {
		return map[int64][]string{REDACTED, nil
REDACTED
...
REDACTED
	...
	for accID, groupIDs := range accountGroups {
		for _, gID := range groupIDs {
			queries = append(queries, queryItem{accountID: accID, groupID: gIDREDACTED)
	REDACTED
REDACTED
	...
	for i, q := range queries {
		...
		cmds[i] = getAffinityClientsScript.Run(ctx, pipe, []string{keyREDACTED, expireThreshold)
REDACTED
	_, err := pipe.Exec(ctx)
	if err != nil && err != redis.Nil {
		return nil, err
REDACTED
	...
	seen := make(map[int64]map[string]struct{REDACTED, len(accountGroups))
	for i, q := range queries {
	...
	REDACTED
		if seen[q.accountID] == nil {
			seen[q.accountID] = make(map[string]struct{REDACTED)
	REDACTED
		for _, clientID := range clients {
			if _, exists := seen[q.accountID][clientID]; !exists {
				seen[q.accountID][clientID] = struct{REDACTED{REDACTED
...
REDACTED
	...
	for i, gID := range groupIDs {
		...
		cmds[i] = getAffinityClientsWithScoresScript.Run(ctx, pipe, []string{keyREDACTED, expireThreshold)
REDACTED
	_, err := pipe.Exec(ctx)
	if err != nil && err != redis.Nil {
		return nil, err
...
REDACTED
	...
	for _, gID := range groupIDs {
		...
		clearAccountAffinityScript.Run(ctx, pipe, []string{revKeyREDACTED, gID, accountID)
REDACTED
	_, err := pipe.Exec(ctx)
	if err != nil && err != redis.Nil {
		return err
