...
// fingerprint：命中则先读其续期「前」的剩余 PTTL 用于排障，再用其存储的 TTL 值
// 滚动续期，返回 {1-based 下标, 续期前剩余PTTL毫秒}；未命中返回 {0, 0}。
var kiroCacheProbeScript = redis.NewScript(`
for i, key in ipairs(KEYS) do
  local ttl = redis.call('GET', key)
  if ttl then
    local remain = redis.call('PTTL', key)
    if remain == nil or remain < 0 then remain = 0 end
...
func (c *gatewayCache) KiroCacheProbe(ctx context.Context, cacheKey uint64, fingerprintsNewestFirst []string) (int, int64, error) {
	...
	res, err := kiroCacheProbeScript.Run(ctx, c.rdb, keys).Int64Slice()
	if err != nil {
		return 0, 0, err
	}
	var idx int
	var remainTTLms int64
	if len(res) > 0 {
		idx = int(res[0])
	}
	if len(res) > 1 {
