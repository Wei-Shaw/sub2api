...
REDACTED
	...
	REDACTED
		s.Require().NoError(scanSingleRow(
			...
			[]any{name, service.PlatformOpenAI, accountType, deletedAtREDACTED,
			&id,
		))
		return id
REDACTED
	...
	for _, binding := range []struct {
		accountID int64
		priority  int
REDACTED{{oauthID, 37REDACTED, {apiKeyID, 8REDACTED, {deletedID, 3REDACTEDREDACTED {
...
REDACTED
	...
	spy := &forbidSQLExecutor{REDACTED
...
func (s *GroupRepoSuite) TestList() {
	baseGroups, basePage, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10REDACTED)
...
func (s *GroupRepoSuite) TestListWithFilters_Platform() {
	baseGroups, _, err := s.repo.ListWithFilters(
		s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 10REDACTED,
...
REDACTED
	...
	s.Run("search_name_should_match", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50REDACTED, "", "", "name-target", nil)
		s.Require().NoError(err)
...
REDACTED)
	...
	s.Run("search_description_should_match", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50REDACTED, "", "", "desc-needle", nil)
		s.Require().NoError(err)
...
REDACTED)
	...
	s.Run("search_nonexistent_should_return_empty", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50REDACTED, "", "", search, nil)
		s.Require().NoError(err)
...
REDACTED)
	...
	s.Run("search_should_be_case_insensitive", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50REDACTED, "", "", "mixedcase-needle", nil)
		s.Require().NoError(err)
...
REDACTED)
	...
	s.Run("search_should_escape_like_wildcards", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50REDACTED, "", "", "100%", nil)
		s.Require().NoError(err)
		...
		groups, _, err = repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50REDACTED, "", "", "ab_cd", nil)
		s.Require().NoError(err)
...
REDACTED
	...
	s.Require().NoError(scanSingleRow(
		...
		[]any{"acc1", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&accountID,
	))
...
REDACTED
	...
	s.Require().NoError(scanSingleRow(
		...
		s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"a1", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&a1,
	))
	var a2 int64
	s.Require().NoError(scanSingleRow(
		...
		s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"a2", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&a2,
	))
...
REDACTED
	...
	insertAccount := func(name, status string, schedulable bool) int64 {
		...
		s.Require().NoError(scanSingleRow(
			...
			[]any{name, service.PlatformAnthropic, service.AccountTypeOAuth, status, schedulableREDACTED,
			&id,
		))
		return id
REDACTED
...
REDACTED
	...
	groups, _, err := s.repo.ListWithFilters(s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 100REDACTED,
...
REDACTED
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"acc-normal", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&normalID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-rate-limited", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&rateLimitedID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-overloaded", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&overloadedID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-temp-unschedulable", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&tempUnschedulableID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-expired", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&expiredID))
...
REDACTED
	...
	s.Require().NoError(scanSingleRow(
		...
		s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"acc-del", service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
		&accountID,
	))
...
REDACTED
	...
	insertAccount := func(name string) int64 {
		...
		s.Require().NoError(scanSingleRow(
			...
			s.tx,
			"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
			[]any{name, service.PlatformAnthropic, service.AccountTypeOAuthREDACTED,
			&id,
		))
		return id
REDACTED
...
REDACTED
	...
	// 获取删除前的列表数量
	listBefore, _, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 100REDACTED)
	s.Require().NoError(err)
	...
	// 验证列表中不再包含软删除的 group
	listAfter, _, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 100REDACTED)
	s.Require().NoError(err)
