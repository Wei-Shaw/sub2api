...
func (s *GroupRepoSuite) TestListWithAccountCountSort_AttachesActiveCount() {
	...
	gA := &service.Group{Name: "sort-count-a", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandardREDACTED
	// Group B: 1 total, 1 active
	gB := &service.Group{Name: "sort-count-b", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandardREDACTED
	...
	insertAccount := func(name, status string) int64 {
		...
		s.Require().NoError(scanSingleRow(s.ctx, s.tx,
			...
			[]any{name, service.PlatformAnthropic, service.AccountTypeOAuth, statusREDACTED,
			&id))
		return id
REDACTED
...
REDACTED, service.PlatformAnthropic, service.StatusActive, "", nil)
	s.Require().NoError(err)
...
func (s *GroupRepoSuite) TestList_DefaultSortBySortOrderAsc() {
	g1 := &service.Group{Name: "g1", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandard, SortOrder: 20REDACTED
	g2 := &service.Group{Name: "g2", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandard, SortOrder: 10REDACTED
	s.Require().NoError(s.repo.Create(s.ctx, g1))
	s.Require().NoError(s.repo.Create(s.ctx, g2))

	groups, _, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 100REDACTED)
	s.Require().NoError(err)
...
func (s *GroupRepoSuite) TestList_SortBySortOrderDesc() {
	g1 := &service.Group{Name: "g1", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandard, SortOrder: 40REDACTED
	g2 := &service.Group{Name: "g2", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandard, SortOrder: 50REDACTED
	s.Require().NoError(s.repo.Create(s.ctx, g1))
	s.Require().NoError(s.repo.Create(s.ctx, g2))
