...
func TestGroupUsageRollupTriggerInvalidatesCascadedHistoricalDelete(t *testing.T) {
	for _, partitioned := range []bool{false, trueREDACTED {
		name := "ordinary"
		if partitioned {
			name = "partitioned"
	REDACTED
...
REDACTED
	...
	defer func() { _ = syncTx.Rollback() REDACTED()
	var stateID int16
	...
	defer func() { _ = lateTx.Rollback() REDACTED()
...
REDACTED
	...
	defer func() { _ = insertTx.Rollback() REDACTED()
...
REDACTED{
		{name: "closed_rollup", closedBefore: "2026-08-14", includeYesterday: trueREDACTED,
		{name: "raw_tail", closedBefore: "2026-08-13", includeYesterday: falseREDACTED,
REDACTED
