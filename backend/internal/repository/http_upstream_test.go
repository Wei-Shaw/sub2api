...
func TestHTTPUpstreamDoAppliesGrokCLIIdentityBeforeOAuthRoundTrip(t *testing.T) {
	...
	for _, endpoint := range []string{"responses", "chat/completions"REDACTED {
			...
			if req.URL.Hostname() == grokCLIProxyHost {
				return &http.Response{
					...
					Body: io.NopCloser(strings.NewReader(
						`{"code":"permission_denied","error":"Access to the chat endpoint is denied. Please ensure you're using the correct credentials. If you believe this is a mistake, please contact support."REDACTED`,
					)),
					Request: req,
			REDACTED, nil
		REDACTED
...
REDACTED
	require.Equal(t, http.StatusOK, resp.StatusCode)
	require.NoError(t, resp.Body.Close())
	require.Equal(t, []string{grokCLIProxyHost, grokOfficialAPIHostREDACTED, hosts)
REDACTED
...
func TestIsGrokCLICompatibilityAccessDenied(t *testing.T) {
	tests := []struct {
		...
		want bool
REDACTED{
		{name: "legacy compatibility wording", body: `{"error":"Access denied"REDACTED`, want: trueREDACTED,
		{
			name: "observed chat endpoint permission denial",
			body: `{"code":"permission_denied","error":"Access to the chat endpoint is denied. Please ensure you're using the correct credentials. If you believe this is a mistake, please contact support."REDACTED`,
			want: true,
	REDACTED,
		{
			name: "entitlement denial using the same broad terms",
			body: `{"code":"permission_denied","error":"Access to the chat endpoint is denied because a subscription is required"REDACTED`,
			want: false,
	REDACTED,
		{
			name: "different permission denied endpoint",
			body: `{"code":"permission_denied","error":"Access to the billing endpoint is denied."REDACTED`,
			want: false,
	REDACTED,
		{
			name: "wrong structured error code",
			body: `{"code":"subscription_required","error":"Access to the chat endpoint is denied. Please ensure you're using the correct credentials. If you believe this is a mistake, please contact support."REDACTED`,
			want: false,
	REDACTED,
		{name: "malformed response", body: `permission_denied: chat endpoint denied`, want: falseREDACTED,
REDACTED
...
func TestIsGrokCLIAccessDeniedFallbackCandidateRequiresAuthenticatedReplayableCLI403(t *testing.T) {
	...
	REDACTED
		...
		return req
REDACTED
	newResponse := func() *http.Response { return &http.Response{StatusCode: http.StatusForbiddenREDACTED REDACTED
...
func (s *HTTPUpstreamSuite) TestCustomResponseHeaderTimeout() {
	s.cfg.Gateway = config.GatewayConfig{ResponseHeaderTimeout: 7REDACTED
	svc := s.newService()
	entry := mustGetOrCreateClient(s.T(), svc, "", 0, 0)
	transport, ok := entry.client.Transport.(*http.Transport)
	require.True(s.T(), ok, "expected *http.Transport")
...
REDACTED
	svc := s.newService()
	entry, err := svc.getClientEntryWithTLS("", 1, 1, &tlsfingerprint.Profile{Name: "test"REDACTED, service.HTTPUpstreamProfileOpenAI, false, false)
	require.NoError(s.T(), err)
	transport, ok := entry.client.Transport.(*http.Transport)
	require.True(s.T(), ok, "expected *http.Transport")
...
func (s *HTTPUpstreamSuite) TestOpenAIProfileHTTP2DisabledUsesHTTP1Transport() {
	s.cfg.Gateway = config.GatewayConfig{
		OpenAIHTTP2: config.GatewayOpenAIHTTP2Config{Enabled: falseREDACTED,
REDACTED
	svc := s.newService()
	entry, err := svc.getClientEntry("", 1, 1, service.HTTPUpstreamProfileOpenAI, false, false)
	require.NoError(s.T(), err)
...
func (s *HTTPUpstreamSuite) TestOpenAIHeaderTimeoutChangeRebuildsClient() {
	s.cfg.Gateway = config.GatewayConfig{
		OpenAIHTTP2: config.GatewayOpenAIHTTP2Config{Enabled: trueREDACTED,
REDACTED
	svc := s.newService()
...
REDACTED))
	...
	s.cfg.Gateway = config.GatewayConfig{ResponseHeaderTimeout: 1REDACTED
	up := NewHTTPUpstream(s.cfg)
...
func (s *HTTPUpstreamSuite) TestAccountIsolation_DifferentAccounts() {
	s.cfg.Gateway = config.GatewayConfig{ConnectionPoolIsolation: config.ConnectionPoolIsolationAccountREDACTED
	svc := s.newService()
	// 同一代理，不同账户
...
func (s *HTTPUpstreamSuite) TestAccountProxyIsolation_DifferentProxy() {
	s.cfg.Gateway = config.GatewayConfig{ConnectionPoolIsolation: config.ConnectionPoolIsolationAccountProxyREDACTED
	svc := s.newService()
	// 同一账户，不同代理
