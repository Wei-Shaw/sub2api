...
type migrationChecksumCompatibilityRule struct {
	fileChecksum       string
	acceptedDBChecksum map[string]struct{REDACTED
	acceptedChecksums  map[string]struct{REDACTED
REDACTED
...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	"112_add_payment_order_provider_key_snapshot.sql":         newMigrationChecksumCompatibilityRule("b75f8f56d39455682787696a3d92ad25b055444ca328fb7fca9a460a15d68d99", "REDACTED"),
...
REDACTED
	defer func() { _ = lockConn.Close() REDACTED()
...
func prepareNonTransactionalMigration(ctx context.Context, db migrationConnection, name string) error {
	...
	case usageLogsEffectiveModelIndexesMigration:
		for _, indexName := range []string{usageLogsEffectiveRequestedModelIndex, usageLogsEffectiveUpstreamModelIndexREDACTED {
...
func checksumSet(values ...string) map[string]struct{REDACTED {
	out := make(map[string]struct{REDACTED, len(values))
	for _, value := range values {
		out[value] = struct{REDACTED{REDACTED
REDACTED
	return out
REDACTED
...
func newMigrationChecksumCompatibilityRule(fileChecksum string, acceptedDBChecksums ...string) migrationChecksumCompatibilityRule {
	return migrationChecksumCompatibilityRule{
		...
		acceptedChecksums:  checksumSet(append([]string{fileChecksumREDACTED, acceptedDBChecksums...)...),
REDACTED
REDACTED
