...
func (s *OpenAIOAuthServiceSuite) setupServer(handler http.HandlerFunc) {
	...
	s.svc = &openaiOAuthService{tokenURL: s.srv.URLREDACTED
REDACTED
...
func (s *OpenAIOAuthServiceSuite) TestExchangeCode_DefaultRedirectURI() {
	...
	REDACTED
		...
		_, _ = io.WriteString(w, `{"access_token":"at","refresh_token":"rt","token_type":"bearer","expires_in":3600REDACTED`)
REDACTED))
...
func (s *OpenAIOAuthServiceSuite) TestRefreshToken_FormFields() {
	...
	REDACTED
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"access_token":"at2","refresh_token":"rt2","token_type":"bearer","expires_in":3600REDACTED`)
REDACTED))
...
REDACTED))
	...
	require.Equal(s.T(), []string{openai.ClientIDREDACTED, seenClientIDs)
REDACTED
...
func (s *OpenAIOAuthServiceSuite) TestRefreshToken_UseSoraClientID() {
	...
	REDACTED
		...
		if clientID == openai.SoraClientID {
			w.Header().Set("Content-Type", "application/json")
			_, _ = io.WriteString(w, `{"access_token":"at-sora","refresh_token":"rt-sora","token_type":"bearer","expires_in":3600REDACTED`)
			return
	REDACTED
		w.WriteHeader(http.StatusBadRequest)
REDACTED))
	...
	require.Equal(s.T(), []string{openai.SoraClientIDREDACTED, seenClientIDs)
REDACTED
...
func (s *OpenAIOAuthServiceSuite) TestRefreshToken_UseProvidedClientID() {
	...
	REDACTED
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"access_token":"at-custom","refresh_token":"rt-custom","token_type":"bearer","expires_in":3600REDACTED`)
REDACTED))
	...
	require.Equal(s.T(), []string{customClientIDREDACTED, seenClientIDs)
REDACTED
...
func (s *OpenAIOAuthServiceSuite) TestRequestError_ClosedServer() {
	s.setupServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {REDACTED))
	s.srv.Close()
...
func (s *OpenAIOAuthServiceSuite) TestExchangeCode_UsesProvidedRedirectURI() {
	...
	REDACTED
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"access_token":"at","token_type":"bearer","expires_in":1REDACTED`)
REDACTED))
