...
func NewOpsRepository(db *sql.DB) service.OpsRepository {
	return &opsRepository{db: dbREDACTED
REDACTED
...
REDACTED
	if filter == nil {
		filter = &service.OpsErrorLogFilter{REDACTED
REDACTED
...
func nullTime(t time.Time) sql.NullTime {
	if t.IsZero() {
		return sql.NullTime{REDACTED
REDACTED
	return sql.NullTime{Time: t, Valid: trueREDACTED
REDACTED

func nullBool(v *bool) sql.NullBool {
	if v == nil {
		return sql.NullBool{REDACTED
REDACTED
	return sql.NullBool{Bool: *v, Valid: trueREDACTED
REDACTED
...
WHERE id = $1`
	...
	at := sql.NullTime{REDACTED
	if resolvedAt != nil && !resolvedAt.IsZero() {
		at = sql.NullTime{Time: resolvedAt.UTC(), Valid: trueREDACTED
REDACTED else if resolved {
		now := time.Now().UTC()
		at = sql.NullTime{Time: now, Valid: trueREDACTED
REDACTED
...
REDACTED
	if filter == nil {
		filter = &service.OpsSystemLogFilter{REDACTED
REDACTED
...
SELECT
  ...
  COALESCE(l.platform, ''),
  COALESCE(l.model, ''),
  COALESCE(l.extra::text, '{REDACTED')
...
REDACTED
	...
	for rows.Next() {
		item := &service.OpsSystemLog{REDACTED
		var userID sql.NullInt64
		var accountID sql.NullInt64
		var extraRaw string
		if err := rows.Scan(
	...
	REDACTED
		...
		if extraRaw != "" && extraRaw != "null" && extraRaw != "{REDACTED" {
...
REDACTED
	if filter == nil {
		filter = &service.OpsSystemLogCleanupFilter{REDACTED
REDACTED
...
REDACTED else if filter.StatusCodesOther {
		// "Other" means: status codes not in the common list.
		known := []int{400, 401, 403, 404, 409, 422, 429, 500, 502, 503, 504, 529REDACTED
...
func opsNullString(v any) any {
	...
	case *string:
		if s == nil || strings.TrimSpace(*s) == "" {
			return sql.NullString{REDACTED
	REDACTED
		return sql.NullString{String: strings.TrimSpace(*s), Valid: trueREDACTED
	case string:
...
func opsNullInt(v any) any {
	...
	case *int:
		if n == nil || *n == 0 {
			return sql.NullInt64{REDACTED
	REDACTED
		return sql.NullInt64{Int64: int64(*n), Valid: trueREDACTED
	case *int64:
		if n == nil || *n == 0 {
			return sql.NullInt64{REDACTED
	REDACTED
		return sql.NullInt64{Int64: *n, Valid: trueREDACTED
	case int:
		if n == 0 {
			return sql.NullInt64{REDACTED
	REDACTED
