...
usage_agg AS (
  SELECT
    ...
    percentile_cont(0.50) WITHIN GROUP (ORDER BY duration_ms) FILTER (WHERE duration_ms IS NOT NULL) AS REDACTED,
...
SELECT
  ...
  ROUND(SUM(REDACTED::double precision * success_count) FILTER (WHERE REDACTED IS NOT NULL)
    / NULLIF(SUM(success_count) FILTER (WHERE REDACTED IS NOT NULL), 0))::int AS REDACTED,
