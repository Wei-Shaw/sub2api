...
REDACTED
func TestProxyExpirySuite(t *testing.T) { suite.Run(t, new(ProxyExpirySuite)) REDACTED
...
func (s *ProxyExpirySuite) mkProxy(name, mode string, expiresAt *time.Time, backupID *int64) int64 {
	p := &service.Proxy{Name: name, Protocol: "http", Host: "127.0.0.1", Port: 8080,
		...
		ExpiresAt: expiresAt, BackupProxyID: backupIDREDACTED
...
func (s *ProxyExpirySuite) mkAccountWithProxy(proxyID int64) int64 {
	...
	err := scanSingleRow(s.ctx, s.tx, `
		...
		VALUES ($1,'claude','api','{REDACTED','{REDACTED','active',$2,NOW(),NOW()) RETURNING id`,
		[]any{"acc-" + time.Now().Format("150405.000000"), proxyIDREDACTED, &id)
...
func (s *ProxyExpirySuite) accountProxyID(id int64) *int64 {
	var pid *int64
	err := scanSingleRow(s.ctx, s.tx, `SELECT proxy_id FROM accounts WHERE id=$1`, []any{idREDACTED, &pid)
	s.Require().NoError(err)
	return pid
REDACTED
...
func (s *ProxyExpirySuite) TestSweep_DirectMode() {
	...
	var origin *int64
	err = scanSingleRow(s.ctx, s.tx, `SELECT proxy_fallback_origin_id FROM accounts WHERE id=$1`, []any{aidREDACTED, &origin)
	s.Require().NoError(err)
...
func (s *ProxyExpirySuite) TestSweep_EnqueuesChangedAccountIDsWithoutFullRebuild() {
	...
	err = scanSingleRow(s.ctx, s.tx, `
		...
		WHERE event_type=$1
		ORDER BY id DESC
		LIMIT 1`, []any{service.SchedulerOutboxEventAccountBulkChangedREDACTED, &payloadRaw)
	s.Require().NoError(err)
...
REDACTED
	...
	s.Require().Equal([]int64{firstAccountID, secondAccountIDREDACTED, payload.AccountIDs)

	var fullRebuildCount int
	err = scanSingleRow(s.ctx, s.tx, `
		SELECT COUNT(*)
		FROM scheduler_outbox
		WHERE event_type=$1`, []any{service.SchedulerOutboxEventFullRebuildREDACTED, &fullRebuildCount)
	s.Require().NoError(err)
