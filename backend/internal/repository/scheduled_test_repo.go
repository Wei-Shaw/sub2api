...
func (r *scheduledTestPlanRepository) Create(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		INSERT INTO scheduled_test_plans (account_id, model_id, client, message, cron_expression, enabled, max_results, auto_recover, next_run_at, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW(), NOW())
		RETURNING id, account_id, model_id, client, message, cron_expression, enabled, max_results, auto_recover, last_run_at, next_run_at, created_at, updated_at
	`, plan.AccountID, plan.ModelID, plan.Client, plan.Message, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover, plan.NextRunAt)
	return scanPlan(row)
}

func (r *scheduledTestPlanRepository) GetByID(ctx context.Context, id int64) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT id, account_id, model_id, client, message, cron_expression, enabled, max_results, auto_recover, last_run_at, next_run_at, created_at, updated_at
...
func (r *scheduledTestPlanRepository) Update(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		SET model_id = $2, client = $3, message = $4, cron_expression = $5, enabled = $6, max_results = $7, auto_recover = $8, next_run_at = $9, updated_at = NOW()
		WHERE id = $1
		RETURNING id, account_id, model_id, client, message, cron_expression, enabled, max_results, auto_recover, last_run_at, next_run_at, created_at, updated_at
	`, plan.ID, plan.ModelID, plan.Client, plan.Message, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover, plan.NextRunAt)
	return scanPlan(row)
}
...
func scanPlan(row scannable) (*service.ScheduledTestPlan, error) {
	...
	if err := row.Scan(
		&p.ID, &p.AccountID, &p.ModelID, &p.Client, &p.Message, &p.CronExpression, &p.Enabled, &p.MaxResults, &p.AutoRecover,
