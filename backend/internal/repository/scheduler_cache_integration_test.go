...
func TestSchedulerCacheSnapshotUsesSlimMetadataButKeepsFullAccount(t *testing.T) {
	...
	bucket := service.SchedulerBucket{GroupID: 2, Platform: service.PlatformGemini, Mode: service.SchedulerModeSingleREDACTED
...
REDACTED
	...
	REDACTED,
		...
		GroupIDs:               []int64{bucket.GroupIDREDACTED,
		AccountGroups: []service.AccountGroup{
			{
				AccountID: 101,
				GroupID:   bucket.GroupID,
				Priority:  5,
				Group:     &service.Group{ID: bucket.GroupID, Name: "gemini-group"REDACTED,
		REDACTED,
	REDACTED,
REDACTED
...
REDACTED
	...
	require.Equal(t, []int64{bucket.GroupIDREDACTED, got.GroupIDs)
...
REDACTED, 5*time.Second, 20*time.Millisecond)
