...
REDACTED
	...
	REDACTED,
		Extra: map[string]any{"large": strings.Repeat("x", 4096)REDACTED,
REDACTED
...
REDACTED
	...
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: latestREDACTED))
...
func TestSchedulerCacheLastUsedSideKeyIsMonotonicAndRequiresAccount(t *testing.T) {
	ctx := context.Background()
	cache := newSchedulerCacheUnit(t)
	account := service.Account{ID: 9202, Platform: service.PlatformGrok, Type: service.AccountTypeOAuthREDACTED
	require.NoError(t, cache.SetAccount(ctx, &account))

	newer := time.Now().UTC().Truncate(time.Millisecond)
	older := newer.Add(-time.Minute)
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: newerREDACTED))
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: olderREDACTED))
...
REDACTED
	...
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{missingID: newerREDACTED))
...
func TestSchedulerCacheUpdateLastUsedChunksLargeBatches(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		id := int64(9300 + i)
		accounts = append(accounts, service.Account{ID: id, Platform: service.PlatformGrokREDACTED)
