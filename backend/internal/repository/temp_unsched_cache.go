...
var openAIAPIKeyHealthFailureScript = redis.NewScript(`
	...
	if count >= threshold then
		...
		return {count, 1REDACTED
	end
	return {count, 0REDACTED
`)
...
func NewTempUnschedCache(rdb *redis.Client) service.TempUnschedCache {
	return &tempUnschedCache{rdb: rdbREDACTED
REDACTED
...
REDACTED
	...
	_, err = tempUnschedSetScript.Run(ctx, c.rdb, []string{keyREDACTED, state.UntilUnix, string(stateJSON), ttlSeconds).Result()
	return err
REDACTED
...
func (c *tempUnschedCache) openAIAPIKeyHealthKey(accountID int64) string {
	...
	return fmt.Sprintf("%s{%dREDACTED:failures", openAIAPIKeyHealthFailurePrefix, accountID)
REDACTED
...
REDACTED
	if threshold < 1 {
		threshold = 1
REDACTED
	result, err := openAIAPIKeyHealthFailureScript.Run(ctx, c.rdb, []string{c.openAIAPIKeyHealthKey(accountID)REDACTED, windowMinutes, threshold).Slice()
	if err != nil {
