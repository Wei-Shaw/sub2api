...
func NewTurnstileVerifier() service.TurnstileVerifier {
	client, err := httpclient.GetClient(httpclient.Options{Timeout: 10 * time.Second, ValidateResolvedIP: true})
	if err != nil {
		client = &http.Client{Timeout: 10 * time.Second}
	}
	return &turnstileVerifier{httpClient: client, verifyURL: turnstileVerifyURL}
}
