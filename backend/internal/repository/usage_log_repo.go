...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			mustMarshalStringSlice(log.ImageURLs), // image_urls
			mustMarshalStringSlice(log.CosURLs),   // cos_url
...
// mustMarshalStringSlice 序列化字符串切片为 jsonb 入参；空切片/ nil 返回 nil（写 NULL）。
// json.Marshal 对 []string 不会失败，故忽略错误是安全的。
func mustMarshalStringSlice(s []string) any {
	v, _ := marshalStringSlice(s)
	return v
}
