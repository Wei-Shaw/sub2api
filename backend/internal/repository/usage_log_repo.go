...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			nullStringAnyMapJSON(log.Metadata), // metadata
			createdAt,
		},
	}
}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log.Metadata = stringAnyMapFromNullJSON(metadata)
...
// nullStringAnyMapJSON marshals caller-supplied request metadata into a JSON
// string for the jsonb metadata column, returning nil (SQL NULL) when empty.
...
// stringAnyMapFromNullJSON unmarshals the jsonb metadata column back into a map,
// returning nil for NULL/empty/invalid payloads.
