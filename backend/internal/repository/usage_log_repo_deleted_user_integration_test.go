...
func TestUsageLog_ListWithFilters_ResolvesSoftDeletedUser(t *testing.T) {
	...
	active := mustCreateUser(t, client, &service.User{Email: "active-listfilter@test.com"REDACTED)
	deleted := mustCreateUser(t, client, &service.User{Email: "deleted-listfilter@test.com"REDACTED)
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: deleted.ID, Key: "sk-del-1", Name: "k"REDACTED)
	apiKey2 := mustCreateApiKey(t, client, &service.APIKey{UserID: active.ID, Key: "sk-act-1", Name: "k"REDACTED)
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-listfilter"REDACTED)

	now := time.Now().UTC()
	for _, u := range []struct {
		uid int64
		kid int64
REDACTED{{deleted.ID, apiKey.IDREDACTED, {active.ID, apiKey2.IDREDACTEDREDACTED {
...
REDACTED
	...
	logs, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50REDACTED,
		usagestats.UsageLogFilters{ExactTotal: trueREDACTED)
REDACTED

	byUser := map[int64]service.UsageLog{REDACTED
	for _, l := range logs {
		byUser[l.UserID] = l
REDACTED
