...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	if log.RequestResult != nil {
		// UsageRequestResult contains only strings and integers, so marshaling cannot fail.
		encoded, _ := json.Marshal(log.RequestResult)
		requestResult = string(encoded)
	}
