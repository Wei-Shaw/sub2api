...
// --- GetGroupCacheHitRates7d ---

func TestUsageLogRepository_GetGroupCacheHitRates7d(t *testing.T) {
	ctx := context.Background()
	client := testEntClient(t)
	repo := newUsageLogRepositoryWithSQL(client, integrationDB)
	...
	user := mustCreateUser(t, client, &service.User{Email: fmt.Sprintf("cache-hit-rate-%d@example.com", now.UnixNano())})
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-cache-hit-rate-" + fmt.Sprintf("%d", now.UnixNano()), Name: "k"})
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-cache-hit-rate-" + fmt.Sprintf("%d", now.UnixNano())})

	// Group 10-equivalent: create a real group and use its ID for seeding.
	// We use unique group names to avoid collisions with other tests.
	group10 := mustCreateGroup(t, client, &service.Group{Name: fmt.Sprintf("g-chr-10-%d", now.UnixNano())})
	group20 := mustCreateGroup(t, client, &service.Group{Name: fmt.Sprintf("g-chr-20-%d", now.UnixNano())})
	group30 := mustCreateGroup(t, client, &service.Group{Name: fmt.Sprintf("g-chr-30-%d", now.UnixNano())})

	// Group 10: two logs within the last 7 days.
	// input=800, cache_read=400 → denom=1200, cache_read=400
	// input=200, cache_read=100 → denom=300, cache_read=100
	// total: cache_read=500, denom=1500 → ratio=500/1500≈0.333
	log1 := &service.UsageLog{
		UserID:          user.ID,
		APIKeyID:        apiKey.ID,
		AccountID:       account.ID,
		RequestID:       fmt.Sprintf("req-chr-10a-%d", now.UnixNano()),
		Model:           "claude-3",
		GroupID:         &group10.ID,
		InputTokens:     800,
		CacheReadTokens: 400,
	...
	_, err := repo.Create(ctx, log1)
	require.NoError(t, err, "create log1 for group10")

	log2 := &service.UsageLog{
		UserID:          user.ID,
		APIKeyID:        apiKey.ID,
		AccountID:       account.ID,
		RequestID:       fmt.Sprintf("req-chr-10b-%d", now.UnixNano()),
		Model:           "claude-3",
		GroupID:         &group10.ID,
		InputTokens:     200,
		CacheReadTokens: 100,
	...
	_, err = repo.Create(ctx, log2)
	require.NoError(t, err, "create log2 for group10")

	// Group 20: data older than 7 days → must NOT appear in result.
	log3 := &service.UsageLog{
		UserID:          user.ID,
		APIKeyID:        apiKey.ID,
		AccountID:       account.ID,
		RequestID:       fmt.Sprintf("req-chr-20-%d", now.UnixNano()),
		Model:           "claude-3",
		GroupID:         &group20.ID,
		InputTokens:     500,
		CacheReadTokens: 250,
	...
	_, err = repo.Create(ctx, log3)
	require.NoError(t, err, "create log3 for group20 (old)")

	// Group 30: input=0, cache_read=0 → HAVING filters denom=0, must NOT appear.
	log4 := &service.UsageLog{
		UserID:          user.ID,
		APIKeyID:        apiKey.ID,
		AccountID:       account.ID,
		RequestID:       fmt.Sprintf("req-chr-30-%d", now.UnixNano()),
		Model:           "claude-3",
		GroupID:         &group30.ID,
		InputTokens:     0,
		CacheReadTokens: 0,
	...
	_, err = repo.Create(ctx, log4)
	require.NoError(t, err, "create log4 for group30 (zero tokens)")

	rates, err := repo.GetGroupCacheHitRates7d(ctx)
	...
	// Group 10 must appear with ratio ≈ 0.333.
	r10, ok := rateMap[group10.ID]
	require.True(t, ok, "group10 should be present in results")
	wantRatio := 500.0 / 1500.0
	gotRatio := r10.CacheRead / r10.Denom
	require.InDelta(t, wantRatio, gotRatio, 0.001, "group10 cache hit ratio mismatch")

	// Group 20 must NOT appear (data outside 7-day window).
	_, ok = rateMap[group20.ID]
	require.False(t, ok, "group20 (old data) should NOT be present in results")

	// Group 30 must NOT appear (zero denominator filtered by HAVING).
	_, ok = rateMap[group30.ID]
	require.False(t, ok, "group30 (zero tokens) should NOT be present in results")
}
