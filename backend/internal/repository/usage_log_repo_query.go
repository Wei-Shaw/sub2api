...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	if requestResult.Valid {
		if err := json.Unmarshal([]byte(requestResult.String), &log.RequestResult); err != nil {
			return nil, fmt.Errorf("decode usage request result: %w", err)
		}
	}
	log.Stream = stream
