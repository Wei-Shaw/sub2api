...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullString{},
			sql.NullFloat64{},
			sql.NullInt64{Valid: true, Int64: 42}, // auto_group_id
			now,
		}})
		require.NoError(t, err)
		...
		require.NotNil(t, log.AutoGroupID)
		require.Equal(t, int64(42), *log.AutoGroupID)
	})
