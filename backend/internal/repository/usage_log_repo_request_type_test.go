...
func TestPrepareUsageLogInsert_PersistsImageSizeMetadata(t *testing.T) {
	...
	prepared := prepareUsageLogInsert(&service.UsageLog{
		...
		ImageSizeBreakdown: map[string]int{"1K": 1, "4K": 1REDACTED,
		CreatedAt:          time.Date(2025, 1, 6, 12, 0, 0, 0, time.UTC),
REDACTED)

	require.Equal(t, sql.NullString{String: imageSize, Valid: trueREDACTED, prepared.args[34])
...
func TestCoalesceTrimmedString(t *testing.T) {
	require.Equal(t, "fallback", coalesceTrimmedString(sql.NullString{REDACTED, "fallback"))
	require.Equal(t, "fallback", coalesceTrimmedString(sql.NullString{Valid: true, String: "   "REDACTED, "fallback"))
	require.Equal(t, "value", coalesceTrimmedString(sql.NullString{Valid: true, String: "value"REDACTED, "fallback"))
REDACTED
...
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, []string{tt.wantConditionREDACTED, conditions)
			require.Equal(t, []any{tt.billingModeREDACTED, args)
	REDACTED)
REDACTED
REDACTED
...
func TestAppendUsageLogBillingModeWhereConditionWithAlias(t *testing.T) {
	...
	require.Equal(t, []string{"(ul.billing_mode = $1 OR ((ul.billing_mode IS NULL OR ul.billing_mode = '') AND COALESCE(ul.image_count, 0) > 0))"REDACTED, conditions)
	require.Equal(t, []any{string(service.BillingModeImage)REDACTED, args)
REDACTED

func TestAppendUsageLogBillingModeQueryFilter(t *testing.T) {
	query, args := appendUsageLogBillingModeQueryFilter("SELECT * FROM usage_logs WHERE user_id = $1", []any{int64(42)REDACTED, string(service.BillingModeToken), "")
	...
	require.Equal(t, []any{int64(42), string(service.BillingModeToken)REDACTED, args)
REDACTED
...
REDACTED
	...
	mock.ExpectQuery("SELECT COUNT\\(\\*\\) FROM usage_logs WHERE \\(request_type = \\$1 OR \\(request_type = 0 AND openai_ws_mode = TRUE\\)\\)").
		...
		WillReturnRows(sqlmock.NewRows([]string{"count"REDACTED).AddRow(int64(0)))
...
func TestUsageLogRepositoryGetUsageTrendWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("AND \\(request_type = \\$3 OR \\(request_type = 0 AND stream = TRUE AND openai_ws_mode = FALSE\\)\\)").
		...
		WillReturnRows(sqlmock.NewRows([]string{"date", "requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens", "total_tokens", "cost", "actual_cost"REDACTED))
...
func TestUsageLogRepositoryGetModelStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("AND \\(request_type = \\$3 OR \\(request_type = 0 AND openai_ws_mode = TRUE\\)\\)").
		WithArgs(start, end, requestType).
		WillReturnRows(sqlmock.NewRows([]string{"model", "requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens", "total_tokens", "cost", "actual_cost", "account_cost"REDACTED))
...
func TestUsageLogRepositoryGetUserModelStatsUsesRequestedModel(t *testing.T) {
	...
	REDACTED).AddRow("gpt-5.5", int64(2), int64(10), int64(20), int64(0), int64(0), int64(30), 0.1, 0.08, 0.07))
...
REDACTED
	...
	mock.ExpectQuery("FROM usage_logs\\s+WHERE COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$1").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"total_account_cost",
			"avg_duration_ms",
	REDACTED).AddRow(int64(1), int64(2), int64(3), int64(4), int64(1), int64(3), 1.2, 1.0, 1.2, 20.0))
	mock.ExpectQuery("SELECT COALESCE\\(NULLIF\\(TRIM\\(inbound_endpoint\\), ''\\), 'unknown'\\) AS endpoint").
		...
		WillReturnRows(sqlmock.NewRows([]string{"endpoint", "requests", "total_tokens", "cost", "actual_cost"REDACTED))
...
func TestUsageLogRepositoryGetModelStatsWithUsageFiltersAppliesRequestedModelFilter(t *testing.T) {
	...
	filters := usagestats.UsageLogFilters{Model: "gpt-5"REDACTED
	...
	mock.ExpectQuery("AND COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$3").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"cache_creation_tokens", "cache_read_tokens", "total_tokens",
			"cost", "actual_cost", "account_cost",
	REDACTED).AddRow("gpt-5", int64(1), int64(10), int64(20), int64(0), int64(0), int64(30), 0.1, 0.08, 0.07))
...
func TestUsageLogRepositoryGetGroupStatsWithUsageFiltersAppliesRequestedModelFilter(t *testing.T) {
	...
	mock.ExpectQuery("AND COALESCE\\(NULLIF\\(TRIM\\(ul.requested_model\\), ''\\), ul.model\\) = \\$3").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			"group_id", "group_name", "requests", "total_tokens",
			"cost", "actual_cost", "account_cost",
	REDACTED).AddRow(int64(1), "default", int64(1), int64(30), 0.1, 0.08, 0.07))
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	filters := usagestats.UsageLogFilters{REDACTED
	...
	REDACTED).AddRow(int64(50), int64(1000), int64(2000), int64(100), int64(60), int64(40), 15.0, 12.5, 11.0, 100.0))
...
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, []any{tt.wantArgREDACTED, args)
	REDACTED)
REDACTED
REDACTED
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			int64(23),
			int64(33),
			sql.NullString{Valid: true, String: "req-image-metadata"REDACTED,
			"gpt-image-2",
			...
			sql.NullString{REDACTED,
			2,
			sql.NullString{Valid: true, String: "4K"REDACTED,
			sql.NullString{Valid: true, String: "1024x1024"REDACTED,
	...
	REDACTED
		...
		require.Equal(t, map[string]int{"4K": 2REDACTED, log.ImageSizeBreakdown)
REDACTED)
	...
	t.Run("request_type_ws_v2_overrides_legacy", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			0.9,               // actual_cost
			1.0,               // rate_multiplier
			sql.NullFloat64{REDACTED, // account_rate_multiplier
			int16(service.BillingTypeBalance),
			int16(service.RequestTypeWSV2),
			false, // legacy stream
			...
			0,
			sql.NullString{REDACTED,
			sql.NullString{REDACTED, // image_input_size
