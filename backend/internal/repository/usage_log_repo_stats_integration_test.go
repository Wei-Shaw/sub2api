...
func TestUsageLog_UpstreamModelMismatchFilterAndPartialIndex(t *testing.T) {
	...
	user := mustCreateUser(t, client, &service.User{Email: "model-audit@test.com"REDACTED)
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-model-audit", Name: "model-audit"REDACTED)
	account := mustCreateAccount(t, client, &service.Account{Name: "model-audit-account"REDACTED)
	now := time.Now().UTC()
	responseModel := "gpt-5.4"
	for _, mismatch := range []bool{true, falseREDACTED {
...
REDACTEDREDACTED, stats.Endpoints)
	require.Equal(t, []usagestats.EndpointStat{{
		Endpoint: "unknown", Requests: 1, TotalTokens: 2,
REDACTEDREDACTED, stats.UpstreamEndpoints)
	require.Equal(t, []usagestats.EndpointStat{{
		Endpoint: "unknown -> unknown", Requests: 1, TotalTokens: 2,
REDACTEDREDACTED, stats.EndpointPaths)
...
func TestUsageLog_GetStatsWithFilters_AggregatesAndEndpoints(t *testing.T) {
	...
	user := mustCreateUser(t, client, &service.User{Email: "stats@test.com"REDACTED)
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-stats-1", Name: "k"REDACTED)
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-stats"REDACTED)
...
REDACTED
	...
	stats, err := repo.GetStatsWithFilters(ctx, usagestats.UsageLogFilters{UserID: user.ID, StartTime: &start, EndTime: &endREDACTED)
REDACTED
