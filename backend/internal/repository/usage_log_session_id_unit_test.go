...
func TestUsageLogInsertQueries_IncludeSessionID(t *testing.T) {
	...
	batchQuery, batchArgs := buildUsageLogBatchInsertQuery([]string{keyREDACTED,
		map[string]usageLogInsertPrepared{key: preparedREDACTED)
	...
	bestEffortQuery, bestEffortArgs := buildUsageLogBestEffortInsertQuery([]usageLogInsertPrepared{preparedREDACTED)
