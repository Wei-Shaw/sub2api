...
func TestMaybeReset(t *testing.T) {
	...
	cases := []struct {
		...
		want      float64
REDACTED{
		{"nil prev start resets", 10, nil, start, 1.5, 1.5REDACTED,
		{"different start resets", 10, &other, start, 1.5, 1.5REDACTED,
		{"same start accumulates", 10, &start, start, 1.5, 11.5REDACTED,
REDACTED
	for _, c := range cases {
