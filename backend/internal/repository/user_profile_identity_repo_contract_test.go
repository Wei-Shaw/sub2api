...
func (s *UserProfileIdentityRepoSuite) mustCreatePendingAuthSession(key AuthIdentityKey) *dbent.PendingAuthSession {
	...
	session, err := s.client.PendingAuthSession.Create().
		...
		SetUpstreamIdentityClaims(map[string]any{"provider_subject": key.ProviderSubjectREDACTED).
		SetLocalFlowState(map[string]any{"step": "pending"REDACTED).
		Save(s.ctx)
	s.Require().NoError(err)
	return session
REDACTED
...
func (s *UserProfileIdentityRepoSuite) TestCreateAndLookupCanonicalAndChannelIdentity() {
	...
	REDACTED,
		...
		Metadata:        map[string]any{"unionid": "union-123"REDACTED,
		ChannelMetadata: map[string]any{"openid": "openid-123"REDACTED,
REDACTED)
	s.Require().NoError(err)
...
func (s *UserProfileIdentityRepoSuite) TestBindAuthIdentityToUser_IsIdempotentAndRejectsOtherOwners() {
	...
	REDACTED,
		Metadata:        map[string]any{"username": "first"REDACTED,
		ChannelMetadata: map[string]any{"scope": "read"REDACTED,
REDACTED)
	s.Require().NoError(err)
	...
	REDACTED,
		Channel: &AuthIdentityChannelKey{
			...
			ChannelSubject: "subject-1",
	REDACTED,
		Metadata:        map[string]any{"username": "second"REDACTED,
		ChannelMetadata: map[string]any{"scope": "write"REDACTED,
REDACTED)
	s.Require().NoError(err)
...
func (s *UserProfileIdentityRepoSuite) TestBindAuthIdentityToUser_ReusesLegacyWeChatAliasRecords() {
	...
	legacyIdentity, err := s.client.AuthIdentity.Create().
		...
		SetMetadata(map[string]any{"source": "legacy-alias"REDACTED).
		Save(s.ctx)
	s.Require().NoError(err)
	...
	legacyChannel, err := s.client.AuthIdentityChannel.Create().
		...
		SetMetadata(map[string]any{"scene": "legacy-alias"REDACTED).
		Save(s.ctx)
	s.Require().NoError(err)
	...
	REDACTED,
		Metadata:        map[string]any{"source": "canonical-bind"REDACTED,
		ChannelMetadata: map[string]any{"scene": "canonical-bind"REDACTED,
REDACTED)
	s.Require().NoError(err)
...
func (s *UserProfileIdentityRepoSuite) TestWithUserProfileIdentityTx_AllowsAvatarOnlyProfileUpdate() {
	...
	REDACTED
		...
		return s.repo.Update(txCtx, model, service.UserUpdateFields{REDACTED)
REDACTED)
	s.Require().NoError(err)
