...
func (r *userRepository) create(ctx context.Context, userIn *service.User, guardEmailAlias bool) error {
	...
	for attempt := 0; attempt < publicIDCreateAttempts; attempt++ {
		...
		err := r.createWithPublicID(ctx, userIn, guardEmailAlias)
		if err == nil {
			return nil
		}
...
func (r *userRepository) createWithPublicID(ctx context.Context, userIn *service.User, guardEmailAlias bool) error {
	...
	if strings.TrimSpace(userIn.Email) != "" {
		...
		if guardEmailAlias {
			// 别名变体的字面量不同，唯一索引无法兜底；串行化同一收件箱的并发注册。
