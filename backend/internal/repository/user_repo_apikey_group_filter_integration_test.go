...
func (s *UserRepoAPIKeyGroupFilterSuite) listByAPIKeyGroup(groupID int64) []service.User {
	...
	users, _, err := s.repo.ListWithFilters(
		s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 50REDACTED,
		service.UserListFilters{APIKeyGroupID: groupIDREDACTED,
	)
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestFiltersUsersByAPIKeyGroup() {
	...
	s.Require().Equal([]int64{hit.IDREDACTED, s.ids(s.listByAPIKeyGroup(g.ID)))
REDACTED
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestMultipleKeysAnyMatchDedup() {
	...
	s.Require().Equal([]int64{u.IDREDACTED, s.ids(s.listByAPIKeyGroup(g.ID)))
REDACTED
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestAPIKeyGroupAndStatusFilter() {
	...
	s.Require().Equal([]int64{active.IDREDACTED, s.ids(users), "only active user with matching key group should match")
REDACTED
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestZeroGroupIDNoFilter() {
	...
	users, _, err := s.repo.ListWithFilters(
		s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 50REDACTED,
		service.UserListFilters{APIKeyGroupID: 0REDACTED,
	)
	s.Require().NoError(err)
	s.Require().ElementsMatch([]int64{u1.ID, u2.IDREDACTED, s.ids(users))
REDACTED
