...
func TestUserRepository_DeleteUser_AtomicWithAPIKeys(t *testing.T) {
	...
	user := mustCreateUser(t, client, &service.User{REDACTED)
	key1 := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: fmt.Sprintf("sk-atomic-a-%d", user.ID)REDACTED)
	key2 := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: fmt.Sprintf("sk-atomic-b-%d", user.ID)REDACTED)
...
REDACTED)
	...
	listParams := pagination.PaginationParams{Page: 1, PageSize: 10REDACTED
	...
	keys, _, err := apiKeyRepo.ListByUserID(ctx, user.ID, listParams, service.APIKeyListFilters{REDACTED)
	...
	keysAfter, _, err := apiKeyRepo.ListByUserID(ctx, user.ID, listParams, service.APIKeyListFilters{REDACTED)
	require.NoError(t, err, "ListByUserID")
