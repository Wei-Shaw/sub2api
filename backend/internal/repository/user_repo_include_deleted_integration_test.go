...
func TestUserRepo_ListWithFilters_IncludeDeleted(t *testing.T) {
	...
	active := mustCreateUser(t, client, &service.User{Email: "shared-keyword-active@test.com"REDACTED)
	deleted := mustCreateUser(t, client, &service.User{Email: "shared-keyword-deleted@test.com"REDACTED)
	require.NoError(t, client.User.DeleteOneID(deleted.ID).Exec(ctx))

	params := pagination.PaginationParams{Page: 1, PageSize: 50, SortBy: "email", SortOrder: "asc"REDACTED

	// 默认（不含已删）：只返回活跃用户。
	usersDefault, resDefault, err := repo.ListWithFilters(ctx, params,
		service.UserListFilters{Search: "shared-keyword-"REDACTED)
REDACTED
	...
	usersAll, resAll, err := repo.ListWithFilters(ctx, params,
		service.UserListFilters{Search: "shared-keyword-", IncludeDeleted: trueREDACTED)
REDACTED
...
func TestUserRepo_GetByIDIncludeDeleted(t *testing.T) {
	...
	u := mustCreateUser(t, client, &service.User{Email: "getbyid-deleted@test.com"REDACTED)
