...
func (s *UserRepoSuite) TestCreateWithEmailAliasGuardAndDomainLimitConcurrent() {
	...
	users := []*service.User{
		{Email: "first@" + domain, PasswordHash: "hash", Role: service.RoleUser, Status: service.StatusActiveREDACTED,
		{Email: "second@" + domain, PasswordHash: "hash", Role: service.RoleUser, Status: service.StatusActiveREDACTED,
REDACTED
...
func (s *UserRepoSuite) TestUpdate() {
	...
	s.Require().NoError(s.repo.Update(s.ctx, got, service.UserUpdateFields{Username: trueREDACTED), "Update")
...
func (s *UserRepoSuite) TestUpdateIgnoresNoRowsFromConflictingEmailIdentityUpsert() {
	...
	s.Require().NoError(err)
	got.Username = "updated"
	s.Require().NoError(s.repo.Update(s.ctx, got, service.UserUpdateFields{Username: trueREDACTED), "Update should tolerate ON CONFLICT DO NOTHING returning no rows")
