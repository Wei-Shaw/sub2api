...
REDACTED)
	...
	s.Require().NoError(
		s.repo.Update(s.ctx, stale, service.UserUpdateFields{Username: trueREDACTED),
		"Update",
	)
...
REDACTED)
	...
	s.Require().NoError(
		s.repo.Update(s.ctx, banned, service.UserUpdateFields{Status: trueREDACTED),
		"ban",
	)
...
REDACTED)
	...
	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{user.IDREDACTED, &concurrency, &rpmLimit)
...
REDACTED)
	...
	got, err := s.repo.GetByID(s.ctx, user.ID)
	s.Require().NoError(err, "GetByID after update")
	s.Require().Equal([]int64{group.IDREDACTED, got.AllowedGroups, "granted group must not be reverted")
REDACTED

func (s *UserRepoSuite) TestAdjustBalance_AppliesDeltaAndReportsChange() {
	user := s.mustCreateUser(&service.User{Email: "adjust-balance@example.com", Balance: 10REDACTED)
...
func (s *UserRepoSuite) TestAdjustBalance_RefusesNegativeResult() {
	user := s.mustCreateUser(&service.User{Email: "adjust-balance-negative@example.com", Balance: 3REDACTED)
...
func (s *UserRepoSuite) TestSetBalance_ReplacesValueAndReportsPrevious() {
	user := s.mustCreateUser(&service.User{Email: "set-balance@example.com", Balance: 7REDACTED)
...
func (s *UserRepoSuite) TestSetBalance_RejectsNegativeValue() {
	user := s.mustCreateUser(&service.User{Email: "set-balance-negative@example.com", Balance: 7REDACTED)
