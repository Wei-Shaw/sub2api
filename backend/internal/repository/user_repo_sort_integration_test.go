...
func (s *UserRepoSuite) mustInsertUsageLog(userID int64, createdAt time.Time) {
	...
	account := mustCreateAccount(s.T(), s.client, &service.Account{Name: "usage-log-account"REDACTED)
	apiKey := mustCreateApiKey(s.T(), s.client, &service.APIKey{UserID: userIDREDACTED)
...
func (s *UserRepoSuite) TestUpdate_PersistsSignupSourceAndActivityTimestamps() {
	...
	s.Require().NoError(s.repo.Update(s.ctx, created, service.UserUpdateFields{SignupSource: true, LastLoginAt: true, LastActiveAt: trueREDACTED))
...
func (s *UserRepoSuite) TestGetLatestUsedAtByUserIDs_UsesUsageLogs() {
	...
	userWithUsage := s.mustCreateUser(&service.User{Email: "usage-source@example.com"REDACTED)
	userWithoutUsage := s.mustCreateUser(&service.User{Email: "usage-missing@example.com"REDACTED)
	s.mustInsertUsageLog(userWithUsage.ID, older)
	s.mustInsertUsageLog(userWithUsage.ID, newer)

	got, err := s.repo.GetLatestUsedAtByUserIDs(s.ctx, []int64{userWithUsage.ID, userWithoutUsage.IDREDACTED)
	s.Require().NoError(err)
...
func (s *UserRepoSuite) TestListWithFilters_SortByLastUsedAtDesc_UsesUsageLogsNotLastActiveAt() {
	...
	nilUsage := s.mustCreateUser(&service.User{Email: "nil-last-used@example.com"REDACTED)
	wrongSource := s.mustCreateUser(&service.User{
		Email:        "active-not-usage@example.com",
		LastActiveAt: &lastActiveVeryRecent,
REDACTED)
	rightSource := s.mustCreateUser(&service.User{Email: "usage-wins@example.com"REDACTED)
