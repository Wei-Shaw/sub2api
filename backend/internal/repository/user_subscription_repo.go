...
func NewUserSubscriptionRepository(client *dbent.Client) service.UserSubscriptionRepository {
	return &userSubscriptionRepository{client: clientREDACTED
REDACTED
...
REDACTED
	if platform != "" {
		groupPredicates := []predicate.Group{group.PlatformEQ(platform)REDACTED
...
REDACTED
	...
	assignedByID := map[int64]*service.User{REDACTED
