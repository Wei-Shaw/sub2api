...
REDACTED)
	...
	subs, pag, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10REDACTED, nil, nil, "", "", "", "")
	s.Require().NoError(err)
...
func (s *UserSubscriptionRepoSuite) TestList_FilterByRevokedStatus() {
	...
	subs, pag, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10REDACTED, nil, nil, service.SubscriptionStatusRevoked, "", "", "")
	s.Require().NoError(err)
	s.Require().Len(subs, 1)
