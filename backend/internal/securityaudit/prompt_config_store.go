...
func NewConfigManager(db *sql.DB, settings service.SettingRepository, redisClient *redis.Client, encryptor service.SecretEncryptor, cfg *config.Config) *ConfigManager {
	return &ConfigManager{
		db: db, settings: settings, redis: redisClient, encryptor: encryptor, clock: realClock{REDACTED,
...
func (m *ConfigManager) Public() (PublicConfig, error) {
	if m == nil {
		return PublicConfig{REDACTED, infraerrors.ServiceUnavailable(ErrorCodeConfigUnavailable, "提示词审计配置暂不可用")
REDACTED
	snapshot := m.snapshot.Load()
	if snapshot == nil {
		return PublicConfig{REDACTED, infraerrors.ServiceUnavailable(ErrorCodeConfigUnavailable, "提示词审计配置暂不可用")
	...
	REDACTED
		...
		case strings.TrimSpace(endpoint.Token) != "":
			if !m.encryptionKeyConfigured {
				return storageConfig{REDACTED, infraerrors.BadRequest(ErrorCodeEncryptionKeyRequired,
