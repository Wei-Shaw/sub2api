...
type PromptAdminHandler struct{ service PromptAdminService REDACTED

func NewPromptAdminHandler(service PromptAdminService) *PromptAdminHandler {
	return &PromptAdminHandler{service: serviceREDACTED
REDACTED

func (h *PromptAdminHandler) GetConfig(c *gin.Context) { response.Success(c, h.service.GetConfig()) REDACTED
...
REDACTED
	...
	if err != nil {
		setPromptAdminAudit(c, "failed", infraerrors.Reason(err), map[string]any{"event_id": idREDACTED)
		response.ErrorFrom(c, err)
		return
REDACTED
	setPromptAdminAudit(c, "success", "", deleteAuditFields(result, map[string]any{"event_id": idREDACTED))
	LogWarn(EventEventDeleted, map[string]any{"user_id": adminID(c), "event_id": id, "status": "deleted"REDACTED)
	response.Success(c, result)
REDACTED
...
REDACTED
	for _, id := range request.IDs {
		if id <= 0 {
			setPromptAdminAudit(c, "failed", "prompt_audit_invalid_event_id", map[string]any{"requested_count": len(request.IDs)REDACTED)
			response.ErrorFrom(c, infraerrors.BadRequest("prompt_audit_invalid_event_id", "事件 ID 无效"))
			return
	REDACTED
REDACTED
	...
	if err != nil {
		setPromptAdminAudit(c, "failed", infraerrors.Reason(err), map[string]any{"requested_count": len(request.IDs)REDACTED)
		response.ErrorFrom(c, err)
		return
REDACTED
	setPromptAdminAudit(c, "success", "", deleteAuditFields(result, map[string]any{"requested_count": len(request.IDs)REDACTED))
	LogWarn(EventEventsDeleted, map[string]any{"user_id": adminID(c), "status": "deleted"REDACTED)
	response.Success(c, result)
REDACTED
...
func eventFilterFromQuery(c *gin.Context) (EventFilter, error) {
	...
	if err != nil {
		return EventFilter{REDACTED, err
REDACTED
...
REDACTED
	if value := strings.TrimSpace(c.Query("start_at")); value != "" {
		...
		if filter.StartAt == nil {
			return EventFilter{REDACTED, infraerrors.BadRequest("prompt_audit_invalid_time", "开始时间无效")
	REDACTED
REDACTED
	if value := strings.TrimSpace(c.Query("end_at")); value != "" {
		filter.EndAt = parseTimeQuery(value)
		if filter.EndAt == nil {
			return EventFilter{REDACTED, infraerrors.BadRequest("prompt_audit_invalid_time", "结束时间无效")
	REDACTED
REDACTED
	return filter, nil
REDACTED
