...
func TestAtomicMetricsExposeCountsLatencyDistributionAndAsyncDelivery(t *testing.T) {
	...
	latencies := []time.Duration{10, 20, 30, 40, 100REDACTED
	kinds := []DecisionKind{DecisionAllow, DecisionFlag, DecisionBlock, DecisionUnavailable, DecisionInvalidREDACTED
...
REDACTED
	...
	require.Equal(t, AuditMetricsSnapshot{Enqueued: 1, Dropped: 1REDACTED, metrics.AuditSnapshot())
REDACTED
