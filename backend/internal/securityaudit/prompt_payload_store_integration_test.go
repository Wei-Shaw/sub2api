...
REDACTED
	client := redis.NewClient(&redis.Options{Addr: addressREDACTED)
	t.Cleanup(func() { require.NoError(t, client.Close()) REDACTED)
...
REDACTEDREDACTED
	...
	service.probes["guard-1"] = ProbeResult{OK: true, Status: "healthy", HTTPStatus: 200REDACTED
