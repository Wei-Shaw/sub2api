...
func TestListPromptRecordsFiltersByAPIKeyName(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })
	repository := NewPostgreSQLRepository(db)
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT COUNT(*) FROM prompt_records WHERE (expires_at IS NULL OR expires_at > NOW()) AND api_key_name_snapshot ILIKE $1")).
		WithArgs("%primary%").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
	mock.ExpectQuery("SELECT id, session_id, turn_no, stage.*api_key_name_snapshot.*FROM prompt_records WHERE .*api_key_name_snapshot ILIKE.* ORDER BY created_at DESC, id DESC").
		WithArgs("%primary%", 20, 0).
		WillReturnRows(sqlmock.NewRows([]string{
			"id", "session_id", "turn_no", "stage", "user_id", "username_snapshot", "user_email_snapshot",
			"api_key_id", "api_key_name_snapshot", "group_id", "group_name", "provider", "endpoint", "protocol",
			"model", "prompt_hash", "prompt_length", "message_count", "risk_status", "risk_checked_at", "created_at", "expires_at",
	...
	page, err := repository.ListPromptRecords(context.Background(), PromptRecordFilter{APIKeyName: " primary "}, 1, 20)
	require.NoError(t, err)
	require.Len(t, page.Items, 1)
	require.Equal(t, "primary", page.Items[0].APIKeyName)
	require.NoError(t, mock.ExpectationsWereMet())
}
