...
func TestSnapshotFullPromptKeepsUnredactedText(t *testing.T) {
	body := `{"messages":[{"role":"user","content":"PROMPT_CANARY_ABC123 email@example.com sk-secretvalue123"REDACTED]REDACTED`
	snapshot, err := ExtractPromptSnapshot(Request{Protocol: "openai_chat_completions", Body: []byte(body)REDACTED)
REDACTED
...
func TestFullPromptFromScanTextRestoresMultiSegmentLayout(t *testing.T) {
	scanText, metadataText := buildPrioritizedScanText([]string{"latest user", "system policy", "earlier user"REDACTED)
	require.Contains(t, scanText, promptAuditPrioritySeparator)
	require.Equal(t, metadataText, FullPromptFromScanText(scanText))

	singleScan, singleMeta := buildPrioritizedScanText([]string{"only"REDACTED)
...
func TestPromptSnapshotSeparatesAnthropicUserPromptFromHarnessBlocks(t *testing.T) {
	...
	body := []byte(`{"system":"system policy","messages":[{"role":"user","content":[` +
		`{"type":"text","text":` + string(mustJSON(t, agents)) + `REDACTED,` +
		`{"type":"text","text":` + string(mustJSON(t, environment)) + `REDACTED,` +
		`{"type":"text","text":` + string(mustJSON(t, latest)) + `REDACTED` +
		`]REDACTED]REDACTED`)

	snapshot, err := ExtractPromptSnapshot(Request{Protocol: "anthropic_messages", Body: bodyREDACTED)
REDACTED
