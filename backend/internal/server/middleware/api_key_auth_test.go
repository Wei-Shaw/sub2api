...
REDACTEDREDACTED
	...
	for _, headers := range []map[string]string{
		{"x-api-key": strings.Repeat("x", service.MaxAPIKeyCredentialBytes+1)REDACTED,
		{"Authorization": "Bearer " + strings.Repeat("x", service.MaxAPIKeyCredentialBytes+1)REDACTED,
		{"Authorization": strings.Repeat("x", maxAPIKeyAuthorizationHeaderBytes+1)REDACTED,
REDACTED {
		r := gin.New()
		r.Use(gin.HandlerFunc(NewAPIKeyAuthMiddleware(svc, nil, cfg)))
		r.GET("/t", func(c *gin.Context) { c.Status(http.StatusOK) REDACTED)
...
REDACTED
	...
	REDACTED
		...
		REDACTED,
			...
			activateWindow: func(ctx context.Context, id int64, dailyStart, periodicStart time.Time) error { return nil REDACTED,
	...
	REDACTED
		...
		REDACTED,
			getByID: func(context.Context, int64) (*service.UserSubscription, error) {
				...
				return &clone, nil
		REDACTED,
			resetDaily:   func(context.Context, int64, time.Time) error { return nil REDACTED,
			resetWeekly:  func(context.Context, int64, time.Time) error { return nil REDACTED,
			resetMonthly: func(context.Context, int64, time.Time) error { return nil REDACTED,
	REDACTED
		subscriptionService := service.NewSubscriptionService(nil, subscriptionRepo, nil, nil, cfg)
		router := newAuthTestRouter(apiKeyService, subscriptionService, cfg)
...
REDACTED
	...
	for _, tt := range tests {
		...
		REDACTED)
			router.Use(gin.HandlerFunc(NewAPIKeyAuthMiddleware(apiKeyService, nil, cfg)))
			router.GET("/t", func(c *gin.Context) { c.Status(http.StatusOK) REDACTED)
...
REDACTED
	apiKey := &service.APIKey{
		...
		Status:      service.StatusActive,
		User:        user,
		IPBlacklist: []string{"9.9.9.9"REDACTED,
REDACTED
...
REDACTED
	...
	apiKeyService := service.NewAPIKeyService(apiKeyRepo, nil, nil, nil, nil, nil, cfg)
	router := gin.New()
	require.NoError(t, router.SetTrustedProxies([]string{"9.9.9.9"REDACTED))
	router.Use(gin.HandlerFunc(NewAPIKeyAuthMiddleware(apiKeyService, nil, cfg)))
	router.GET("/t", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"ok": trueREDACTED)
REDACTED)
...
REDACTED
	apiKey := &service.APIKey{
		...
		Status:      service.StatusActive,
		User:        user,
		IPWhitelist: []string{"9.9.9.9"REDACTED,
REDACTED
...
func TestAPIKeyAuthOpenAIQuotaErrorFormat(t *testing.T) {
	...
	user := &service.User{ID: 11, Role: service.RoleUser, Status: service.StatusActive, Balance: 10REDACTED
	group := &service.Group{ID: 8, Platform: service.PlatformOpenAI, Status: service.StatusActiveREDACTED
	apiKey := &service.APIKey{
