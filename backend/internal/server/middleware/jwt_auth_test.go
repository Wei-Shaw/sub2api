...
REDACTED
	...
	userRepo := &stubJWTUserRepo{users: map[int64]*service.User{1: userREDACTEDREDACTED
	authSvc := service.NewAuthService(nil, userRepo, nil, nil, cfg, nil, nil, nil, nil, nil, nil, nil, nil)
	userSvc := service.NewUserService(userRepo, nil, nil, nil)
	toucher := &recordingActivityToucher{REDACTED
...
REDACTED
	...
	require.Equal(t, http.StatusOK, w.Code)
	require.Equal(t, []int64{1REDACTED, toucher.userIDs)
REDACTED
...
func TestJWTAuth_InvalidHeaderFormat(t *testing.T) {
	tests := []struct {
		...
		header string
REDACTED{
		{"无Bearer前缀", "Token abc123"REDACTED,
		{"缺少空格分隔", "Bearerabc123"REDACTED,
		{"仅有单词", "abc123"REDACTED,
REDACTED
	router, _ := newJWTTestEnv(nil)
...
REDACTED
	...
	router, authSvc := newJWTTestEnv(map[int64]*service.User{REDACTED)
...
REDACTED
	router, authSvc := newJWTTestEnv(map[int64]*service.User{1: userInDBREDACTED)
