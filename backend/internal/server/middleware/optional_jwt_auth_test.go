...
func newOptionalJWTTestEnv(users map[int64]*service.User) (*gin.Engine, *service.AuthService) {
	...
	userRepo := &stubJWTUserRepo{users: usersREDACTED
	...
	r.GET("/plaza", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"authed": authed, "user_id": subject.UserIDREDACTED)
REDACTED)
	return r, authSvc
REDACTED
...
func TestOptionalJWTAuth_ValidTokenSetsSubject(t *testing.T) {
	user := &service.User{
		...
		TokenVersion: 1,
REDACTED
	router, authSvc := newOptionalJWTTestEnv(map[int64]*service.User{7: userREDACTED)
