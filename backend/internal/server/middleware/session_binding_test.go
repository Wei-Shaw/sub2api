...
func TestSessionBindingContextFollowsForwardedIPSwitch(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		wantIP         string
REDACTED{
		{name: "enabled switch takes over raw headers", trustForwarded: true, wantIP: "1.2.3.4"REDACTED,
		{name: "disabled switch ignores untrusted headers", trustForwarded: false, wantIP: "127.0.0.1"REDACTED,
		{name: "disabled switch uses configured Gin proxy", trustForwarded: false, trustedProxies: []string{"127.0.0.1"REDACTED, wantIP: "1.2.3.4"REDACTED,
REDACTED {
...
func TestSessionBindingContextSnapshotsForwardedModeAndHeaders(t *testing.T) {
	...
	cfg := &config.Config{REDACTED
	cfg.SetForwardedClientIPSettings(true, []string{"X-Initial-IP"REDACTED)
	...
	r.GET("/t", func(c *gin.Context) {
		...
		cfg.SetForwardedClientIPSettings(false, []string{"X-Changed-IP"REDACTED)
...
REDACTED)
	...
	require.Equal(t, []string{"X-Changed-IP"REDACTED, runtimeSettings.Headers)
REDACTED
...
func TestRequestSessionBindingPrefersInjectedBinding(t *testing.T) {
	...
	r := gin.New()
	require.NoError(t, r.SetTrustedProxies([]string{"127.0.0.1"REDACTED))
	r.Use(SessionBindingContext(cfg))
	r.GET("/t", func(c *gin.Context) {
		issued := &service.SessionBinding{IP: "1.2.3.4", UserAgent: "test-agent"REDACTED
