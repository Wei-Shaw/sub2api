...
var defaultPoolModeRetryableStatusCodes = []int{401, 403, 429REDACTED
...
REDACTED
	...
	if !ok {
		return nil
REDACTED
	seen := make(map[int]struct{REDACTED, len(arr))
	...
	REDACTED
		if _, exists := seen[code]; exists {
			continue
	REDACTED
		seen[code] = struct{REDACTED{REDACTED
...
// （上游路径为 {baseREDACTED/v1/messages）。优先取凭证 base_url，缺失时按
...
REDACTED
	...
	case OpenAIEndpointCapabilityAlphaSearch:
		...
		// {base_urlREDACTED/v1/alpha/search（见 openAIAlphaSearchURL），两类账号
...
func (a *Account) IsShadow() bool { return a != nil && a.ParentAccountID != nil REDACTED

// IsCredentialShadow 语义别名，供「凭据消费者跳过影子」处使用（管理/后台 OAuth 路径）。
func (a *Account) IsCredentialShadow() bool { return a.IsShadow() REDACTED
