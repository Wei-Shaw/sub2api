...
REDACTEDREDACTED
	...
	updated, err := (&adminServiceImpl{accountRepo: repoREDACTED).UpdateAccount(context.Background(), accountID, &UpdateAccountInput{
		Extra: map[string]any{codexFingerprintModeExtraKey: "device"REDACTED,
REDACTED)
...
REDACTEDREDACTED
	...
	disabled, err := svc.UpdateAccount(context.Background(), accountID, &UpdateAccountInput{
		Extra: map[string]any{codexFingerprintModeExtraKey: "off"REDACTED,
REDACTED)
REDACTED
	...
	reenabled, err := svc.UpdateAccount(context.Background(), accountID, &UpdateAccountInput{
		Extra: map[string]any{codexFingerprintModeExtraKey: "session"REDACTED,
REDACTED)
REDACTED
...
REDACTEDREDACTED
	...
	err := (&adminServiceImpl{accountRepo: repoREDACTED).UpdateAccountExtra(context.Background(), accountID, map[string]any{
...
func TestBulkUpdateAccountsDoesNotPrewriteCodexSeed(t *testing.T) {
	...
	result, err := (&adminServiceImpl{accountRepo: repoREDACTED).BulkUpdateAccounts(context.Background(), &BulkUpdateAccountsInput{
		AccountIDs: []int64{301, 302REDACTED,
		Extra: map[string]any{
			codexFingerprintModeExtraKey: "session",
			codexFingerprintSeedExtraKey: userSuppliedCodexFingerprintSeed,
	REDACTED,
...
func TestDuplicateAccountDoesNotCopyCodexFingerprintSeed(t *testing.T) {
	...
	repo := &codexSeedDuplicateRepo{upstreamBillingProbeAccountRepo: &upstreamBillingProbeAccountRepo{accounts: make(map[int64]*Account)REDACTEDREDACTED
...
func TestAccountServiceCreateAndUpdateCodexSeedLifecycle(t *testing.T) {
	ctx := context.Background()
	repo := &upstreamBillingProbeAccountRepo{accounts: make(map[int64]*Account)REDACTED
