...
func TestOpenAITokenRefresherSkipsShadow(t *testing.T) {
	...
	require.False(t, r.CanRefresh(&Account{ID: 200, Platform: PlatformOpenAI, Type: AccountTypeOAuth, ParentAccountID: &pidREDACTED))
	// 普通账号：有 refresh_token → CanRefresh 应返回 true
	require.True(t, r.CanRefresh(&Account{ID: 100, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{"refresh_token": "RT"REDACTEDREDACTED))
REDACTED
...
func TestAccountTestServiceSkipsShadow(t *testing.T) {
	...
	shadow := &Account{
		...
		ParentAccountID: &pid,
REDACTED
	repo := &shadowSkipTestRepo{account: shadowREDACTED
...
REDACTED"access_token": "shadow-passthrough-token"REDACTED,
REDACTED
	privacyCalled := false
