...
var sensitiveCredentialKeySet = func() map[string]struct{REDACTED {
	m := make(map[string]struct{REDACTED, len(SensitiveCredentialKeys))
	for _, k := range SensitiveCredentialKeys {
		m[k] = struct{REDACTED{REDACTED
REDACTED
	return m
REDACTED()
