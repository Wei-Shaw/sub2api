...
func TestIsHeaderOverrideEligible(t *testing.T) {
	tests := []struct {
		...
		want     bool
REDACTED{
		{"anthropic apikey", PlatformAnthropic, AccountTypeAPIKey, trueREDACTED,
		{"openai apikey", PlatformOpenAI, AccountTypeAPIKey, trueREDACTED,
		{"kimi apikey", PlatformKimi, AccountTypeAPIKey, trueREDACTED,
		{"zhipu apikey", PlatformZhipu, AccountTypeAPIKey, trueREDACTED,
		{"deepseek apikey", PlatformDeepseek, AccountTypeAPIKey, trueREDACTED,
		...
		{"kimi oauth", PlatformKimi, AccountTypeOAuth, falseREDACTED,
		{"zhipu oauth", PlatformZhipu, AccountTypeOAuth, falseREDACTED,
		{"deepseek oauth", PlatformDeepseek, AccountTypeOAuth, falseREDACTED,
...
REDACTED)
	...
	require.False(t, headerOverrideTestAccount(PlatformAnthropic, AccountTypeAPIKey, map[string]any{
		credKeyHeaderOverrideEnabled: "true",
REDACTED).IsHeaderOverrideEnabled())
	require.False(t, headerOverrideTestAccount(PlatformAnthropic, AccountTypeAPIKey, map[string]any{
...
REDACTED, overrides)
	...
	disabled := headerOverrideTestAccount(PlatformOpenAI, AccountTypeAPIKey, map[string]any{
		credKeyHeaderOverrides: map[string]any{"user-agent": "x"REDACTED,
REDACTED)
	...
	empty := headerOverrideTestAccount(PlatformOpenAI, AccountTypeAPIKey, map[string]any{
		credKeyHeaderOverrideEnabled: true,
		credKeyHeaderOverrides:       map[string]any{"user-agent": ""REDACTED,
REDACTED)
	...
	REDACTED,
REDACTED)
	require.Equal(t, map[string]string{"x-ok": "ok"REDACTED, defensive.GetHeaderOverrides())
REDACTED
...
REDACTED)
	...
	h["anthropic-beta"] = []string{"claude-code-20250219,oauth-2025-04-20"REDACTED // 非 canonical 原样 key
	h.Set("Content-Type", "application/json")

	acc.ApplyHeaderOverrides(h)

	// user-agent 覆盖且只有一个值（已知头恢复 wire casing）
	require.Equal(t, []string{"override-agent/2.0"REDACTED, h["User-Agent"])
	// anthropic-beta：非 canonical 旧值被清除，写入 wire casing（小写）
	require.Equal(t, []string{"custom-beta-1"REDACTED, h["anthropic-beta"])
	require.Empty(t, h["Anthropic-Beta"])
	// 新增头（未知头以小写原样键写入，与转发链路 wire casing 约定一致）
	require.Equal(t, []string{"custom-value"REDACTED, h["x-custom"])
...
REDACTED
	...
	oauth := headerOverrideTestAccount(PlatformAnthropic, AccountTypeOAuth, map[string]any{
		credKeyHeaderOverrideEnabled: true,
		credKeyHeaderOverrides:       map[string]any{"user-agent": "hacked"REDACTED,
REDACTED)
	h := baseline()
...
REDACTED)
	...
	t.Run("missing keys no-op", func(t *testing.T) {
		creds := map[string]any{"api_key": "sk-xxx"REDACTED
...
REDACTED)
	...
	REDACTED
		...
		require.Equal(t, map[string]any{
			"user-agent": "my-agent",
			"x-app":      "",
	REDACTED, creds[credKeyHeaderOverrides])
REDACTED)

	t.Run("accepts map[string]string input", func(t *testing.T) {
		creds := map[string]any{
			credKeyHeaderOverrides: map[string]string{"X-App": "cli"REDACTED,
	REDACTED
		require.NoError(t, NormalizeHeaderOverrideCredentials(creds))
		require.Equal(t, map[string]any{"x-app": "cli"REDACTED, creds[credKeyHeaderOverrides])
REDACTED)
...
REDACTED)
	...
	t.Run("rejects non-object overrides", func(t *testing.T) {
		err := NormalizeHeaderOverrideCredentials(map[string]any{
			credKeyHeaderOverrides: []any{"user-agent"REDACTED,
	REDACTED)
	REDACTED
REDACTED)
	...
	t.Run("rejects non-string value", func(t *testing.T) {
		err := NormalizeHeaderOverrideCredentials(map[string]any{
			credKeyHeaderOverrides: map[string]any{"x-app": 123REDACTED,
	REDACTED)
	REDACTED
REDACTED)
	...
	t.Run("rejects invalid header name", func(t *testing.T) {
		for _, name := range []string{"bad name", "bad:name", "bad\nname", "值"REDACTED {
			err := NormalizeHeaderOverrideCredentials(map[string]any{
				credKeyHeaderOverrides: map[string]any{name: "v"REDACTED,
		REDACTED)
...
REDACTED)
	...
	t.Run("rejects empty name with value", func(t *testing.T) {
		err := NormalizeHeaderOverrideCredentials(map[string]any{
			credKeyHeaderOverrides: map[string]any{"  ": "v"REDACTED,
	REDACTED)
	REDACTED
REDACTED)
...
REDACTED)
	...
	t.Run("allows tab inside value", func(t *testing.T) {
		creds := map[string]any{
			credKeyHeaderOverrides: map[string]any{"x-app": "a\tb"REDACTED,
	REDACTED
		require.NoError(t, NormalizeHeaderOverrideCredentials(creds))
		require.Equal(t, map[string]any{"x-app": "a\tb"REDACTED, creds[credKeyHeaderOverrides])
REDACTED)
...
REDACTED)
	...
	REDACTED
		err := NormalizeHeaderOverrideCredentials(map[string]any{
			credKeyHeaderOverrides: map[string]any{"x-app": string(big)REDACTED,
	REDACTED)
	REDACTED
REDACTED)
REDACTED
