...
func TestOpenAIQuotaStrategySchedulable_ExpiredWindowTreatedAsReset(t *testing.T) {
	account := &Account{
		...
		Extra: map[string]any{
			...
			"codex_7d_reset_at":                   time.Now().Add(-1 * time.Hour).Format(time.RFC3339),
		},
	}

	require.True(t, account.IsOpenAIQuotaStrategySchedulable())
	remaining, ok := account.GetOpenAIQuotaRemainingPercentByStrategy()
	require.True(t, ok)
	require.Equal(t, 100.0, remaining)
}
