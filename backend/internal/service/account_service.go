...
func (s *AccountService) TestCredentials(ctx context.Context, id int64) error {
	...
	case PlatformGrok:
		...
		return nil
	case PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo, PlatformTypeSafe:
		// 国产 OpenAI 兼容供应商、OpenCode 与 typesafe：凭证为 base_url + API Key，
		// 实际可用性经额度/余额探测与转发路径验证（typesafe 待端点落地后补探测）。
		return nil
	default:
