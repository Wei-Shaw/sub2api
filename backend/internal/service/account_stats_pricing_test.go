...
REDACTED
	...
	tests := []struct {
		...
		want        float64
REDACTED{
		{name: "standard", serviceTier: "", want: 0.265REDACTED,
		{name: "priority", serviceTier: "priority", want: 0.53REDACTED,
		{name: "flex", serviceTier: "flex", want: 0.1325REDACTED,
REDACTED
...
func TestResolveAccountStatsCost_FallbackHonorsAnthropicFast(t *testing.T) {
	channel := &Channel{ID: 1, Status: StatusActiveREDACTED
	cs := newTestChannelServiceForStats(t, channel, 10, "anthropic")
	bs := newTestBillingServiceWithPrices(map[string]*ModelPricing{
		"claude-opus-5": {
...
REDACTED)
	...
	result := resolveAccountStatsCost(
		...
		1, 10, "claude-opus-5",
		UsageTokens{InputTokens: 1_000_000, OutputTokens: 1_000_000REDACTED,
		1, 0, "fast",
	)
	require.NotNil(t, result)
...
REDACTED)
	...
	usageLog := &UsageLog{ServiceTier: &serviceTierREDACTED

	applyAccountStatsCost(
		context.Background(), usageLog, cs, bs,
		1, 10, "gpt-5.6-sol", "gpt-5.6-sol",
		UsageTokens{InputTokens: 100, OutputTokens: 50REDACTED, 999,
	)
