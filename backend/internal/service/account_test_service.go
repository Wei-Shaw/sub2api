...
REDACTED
	...
	if err != nil {
		...
		s.sendEvent(c, TestEvent{Type: "info", Text: "Chat Completions API failed, trying Responses API..."REDACTED)
			...
			REDACTED
				...
				if delta, ok := choice["delta"].(map[string]any); ok {
					if content, ok := delta["content"].(string); ok && content != "" {
						s.sendEvent(c, TestEvent{Type: "content", Text: contentREDACTED)
				REDACTED
			REDACTED
		REDACTED
	REDACTED
