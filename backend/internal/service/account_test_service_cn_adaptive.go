...
// suffix). The naive {baseREDACTED/v1/messages join would 404 (e.g.
...
REDACTED
	return fmt.Sprintf(
		"API protocol is anthropic but base_url (%s) looks like an OpenAI-compatible endpoint; "+
			"requests would hit {baseREDACTED/v1/messages and 404. Set base_url to the provider's Anthropic endpoint "+
