...
func adaptiveCNAccountTestService(account *Account, responses ...*http.Response) (*AccountTestService, *httpUpstreamRecorder) {
	...
	REDACTED,
REDACTED
	upstream := &httpUpstreamRecorder{responses: responsesREDACTED
...
func adaptiveCNChatTestResponse() *http.Response {
	return &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(`data: {"choices":[{"delta":{"content":"chat ok"REDACTED,"finish_reason":"stop"REDACTED]REDACTED
...
func adaptiveCNAnthropicTestResponse() *http.Response {
	return &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"REDACTEDREDACTED,
		Body: io.NopCloser(strings.NewReader(`data: {"type":"content_block_delta","delta":{"text":"anthropic ok"REDACTEDREDACTED
...
func adaptiveCNResponsesTestResponse() *http.Response {
	return &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"REDACTEDREDACTED,
		Body: io.NopCloser(strings.NewReader(`data: {"type":"response.output_text.delta","delta":"responses ok"REDACTED
...
func TestAccountTestService_AdaptiveChatOnlyProvidersTestChatAndAnthropicEndpoints(t *testing.T) {
	for index, testCase := range []struct {
		...
		model    string
REDACTED{
		{name: "Kimi", platform: PlatformKimi, model: "kimi-k2.5"REDACTED,
		{name: "Zhipu", platform: PlatformZhipu, model: "glm-4.7"REDACTED,
REDACTED {
...
func TestAccountTestService_AdaptiveStopsAndNamesFailingEndpoint(t *testing.T) {
	...
	svc, upstream := adaptiveCNAccountTestService(
		account,
		adaptiveCNChatTestResponse(),
		newJSONResponse(http.StatusNotFound, `{"error":{"message":"missing messages route"REDACTEDREDACTED`),
	)
	c, recorder := newTestContext()
...
REDACTED)
	svc, _ := adaptiveCNAccountTestService(
		account,
		newJSONResponse(http.StatusUnauthorized, `{"error":{"message":"invalid key"REDACTEDREDACTED`),
	)
	c, _ := newTestContext()
