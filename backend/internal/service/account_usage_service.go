...
type ClaudeUsageResponse struct {
	FiveHour struct {
		Utilization float64 `json:"utilization"`
		ResetsAt    string  `json:"resets_at"`
REDACTED `json:"five_hour"`
	SevenDay struct {
		Utilization float64 `json:"utilization"`
		ResetsAt    string  `json:"resets_at"`
REDACTED `json:"seven_day"`
	SevenDaySonnet struct {
		Utilization float64 `json:"utilization"`
		ResetsAt    string  `json:"resets_at"`
REDACTED `json:"seven_day_sonnet"`
...
func (s *AccountUsageService) getOpenAIUsage(ctx context.Context, account *Account, force bool) (*UsageInfo, error) {
	now := time.Now()
	usage := &UsageInfo{UpdatedAt: &nowREDACTED
...
REDACTED
	...
	if account != nil && account.IsGrokOAuth() && s.grokQuotaService != nil && (force || grokBillingSnapshotNeedsRefresh(account, time.Now())) && s.shouldProbeGrokBilling(account.ID, time.Now(), force) {
		...
		if err == nil && result != nil && result.Billing != nil {
			...
			mergeAccountExtra(account, map[string]any{grokBillingExtraKey: result.BillingREDACTED)
	REDACTED else if err != nil && force {
			return nil, err
	REDACTED
REDACTED
...
REDACTED
	...
	REDACTED else if s.usageLogRepo != nil {
...
REDACTED
	...
	for _, accountID := range uniqueIDs {
		if _, ok := result[accountID]; !ok {
			result[accountID] = &WindowStats{REDACTED
	REDACTED
REDACTED
	return result, nil
REDACTED
...
REDACTED
	...
	progress := &UsageProgress{Utilization: parseExtraFloat64(usedRaw)REDACTED
...
REDACTED
	...
	REDACTED
REDACTED
	return time.Time{REDACTED, fmt.Errorf("unable to parse time: %s", s)
REDACTED
