...
func TestClaudeUsageResponse_FableWindowDecoding(t *testing.T) {
	t.Run("seven_day_overage_included", func(t *testing.T) {
		raw := `{
  "five_hour": {"utilization": 12.0, "resets_at": "2026-07-03T10:00:00Z"REDACTED,
  "seven_day": {"utilization": 34.0, "resets_at": "2026-07-08T00:00:00Z"REDACTED,
  "seven_day_overage_included": {"utilization": 56.0, "resets_at": "2026-07-08T03:00:00Z"REDACTED
REDACTED`
...
REDACTED)
	...
	t.Run("absent", func(t *testing.T) {
		raw := `{"five_hour": {"utilization": 12.0, "resets_at": "2026-07-03T10:00:00Z"REDACTEDREDACTED`
		var resp ClaudeUsageResponse
		require.NoError(t, json.Unmarshal([]byte(raw), &resp))
...
func TestBuildPassiveUsageWindow(t *testing.T) {
	...
	REDACTED, "passive_usage_7d_oi_utilization", "passive_usage_7d_oi_reset")
...
REDACTED)
	...
	t.Run("no data returns nil", func(t *testing.T) {
		...
		require.Nil(t, buildPassiveUsageWindow(map[string]any{REDACTED, "u", "r"))
REDACTED)
...
REDACTED)
	...
	t.Run("utilization only", func(t *testing.T) {
		window := buildPassiveUsageWindow(map[string]any{"u": 0.25REDACTED, "u", "r")
		require.NotNil(t, window)
...
func TestSyncActiveToPassive_WritesFableExtras(t *testing.T) {
	repo := &accountUsageCodexProbeRepo{updateExtraCh: make(chan map[string]any, 1)REDACTED
