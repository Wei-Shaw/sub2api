...
REDACTED
	...
	updated, err := svc.UpdateAccount(context.Background(), accountID, &UpdateAccountInput{Name: "after"REDACTED)
REDACTED
...
REDACTED)
	...
	t.Run("enabling sync in the same request rejects manual rate", func(t *testing.T) {
		accountID := int64(154)
		repo := newRepo(accountID, map[string]any{REDACTED)
		enable := true
...
REDACTED)
	...
	t.Run("sync disabled allows manual rate", func(t *testing.T) {
		accountID := int64(156)
		repo := newRepo(accountID, map[string]any{UpstreamBillingProbeEnabledExtraKey: trueREDACTED)
...
func TestUpdateAccountExtraDropsManagedBillingProbeFields(t *testing.T) {
	accountID := int64(153)
	repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{
		accountID: {ID: accountID, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyREDACTED,
REDACTEDREDACTED
