...
type duplicateGroupRepoStub struct {
	...
	names              map[string]struct{REDACTED
...
func newDuplicateGroupRepoStub(source *Group) *duplicateGroupRepoStub {
	repo := &duplicateGroupRepoStub{
		...
		names:           make(map[string]struct{REDACTED),
...
REDACTED
	if source != nil {
		...
		repo.names[source.Name] = struct{REDACTED{REDACTED
REDACTED
	return repo
REDACTED
...
REDACTED
	...
	r.names[group.Name] = struct{REDACTED{REDACTED
...
func groupDuplicateTestPointer[T any](value T) *T { return &value REDACTED
...
func TestDuplicateGroupCopiesConfigurationDeeplyAndResetsRuntimeState(t *testing.T) {
	...
	source := &Group{
		...
		VideoModelPrices: map[string]map[string]float64{
			VideoPriceFamilyGrokImagineVideo15: {VideoBillingResolution720P: 0.14REDACTED,
	REDACTED,
		...
		ModelRouting:                    map[string][]int64{"gpt-*": {13, 17REDACTEDREDACTED,
		ModelRoutingEnabled:             true,
		MCPXMLInject:                    true,
		SupportedModelScopes:            []string{"claude", "gemini_text"REDACTED,
		SortOrder:                       9,
		...
		MessagesDispatchModelConfig: OpenAIMessagesDispatchModelConfig{
			...
			ExactModelMappings: map[string]string{"claude-special": "gpt-special"REDACTED,
	REDACTED,
		ModelsListConfig:        GroupModelsListConfig{Enabled: true, Models: []string{"gpt-5.4", "gpt-5-mini"REDACTEDREDACTED,
		RPMLimit:                99,
		...
		AccountGroups:           []AccountGroup{{AccountID: 13, GroupID: 41, Priority: 37REDACTEDREDACTED,
REDACTED
	repo := newDuplicateGroupRepoStub(source)
	repo.sourceBindings[source.ID] = []AccountGroup{
		{AccountID: 13, GroupID: source.ID, Priority: 37REDACTED,
		{AccountID: 17, GroupID: source.ID, Priority: 8REDACTED,
REDACTED
	svc := &adminServiceImpl{groupRepo: repo, groupDuplicateRepo: repoREDACTED
...
REDACTED
	...
	require.Equal(t, []int64{source.IDREDACTED, repo.createdFromSources)
	require.Equal(t, []AccountGroup{
		{AccountID: 13, GroupID: duplicate.ID, Priority: 37REDACTED,
		{AccountID: 17, GroupID: duplicate.ID, Priority: 8REDACTED,
REDACTED, repo.createdBindings[duplicate.ID])
...
func TestDuplicateGroupRecoversSameOperationAndScopesByAdmin(t *testing.T) {
	source := &Group{ID: 9, Name: "team", Platform: PlatformAnthropic, Status: StatusActiveREDACTED
	repo := newDuplicateGroupRepoStub(source)
	svc := &adminServiceImpl{groupRepo: repo, groupDuplicateRepo: repoREDACTED
...
func TestDuplicateGroupAdvancesNameAndTruncatesUnicodeByRunes(t *testing.T) {
	source := &Group{ID: 12, Name: "team", Platform: PlatformAnthropic, Status: StatusActiveREDACTED
	repo := newDuplicateGroupRepoStub(source)
	repo.names["team (Copy)"] = struct{REDACTED{REDACTED
	svc := &adminServiceImpl{groupRepo: repo, groupDuplicateRepo: repoREDACTED
...
func TestDuplicateGroupAtomicCreateFailureReturnsNoCopy(t *testing.T) {
	source := &Group{ID: 15, Name: "team", Platform: PlatformAnthropic, Status: StatusActiveREDACTED
	repo := newDuplicateGroupRepoStub(source)
