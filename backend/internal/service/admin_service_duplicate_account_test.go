...
func TestDuplicateAccountCopiesConfigurationAndResetsRuntimeState(t *testing.T) {
	...
	svc := &adminServiceImpl{accountRepo: repo, accountDuplicateRepo: repoREDACTED
...
REDACTED
			"api_key": "secret",
			"nested":  map[string]any{"token": "source-token"REDACTED,
	REDACTED,
		Extra: map[string]any{
			"config":                          map[string]any{"region": "us-east-1"REDACTED,
			"items":                           []any{map[string]any{"enabled": trueREDACTEDREDACTED,
			"quota_limit":                     1000,
			"quota_used":                      450,
			"quota_daily_used":                25,
			"quota_daily_start":               "2026-07-15T00:00:00Z",
			"model_rate_limits":               map[string]any{"gpt-5": "2099-01-01T00:00:00Z"REDACTED,
			"codex_5h_used_percent":           80,
			"codex_cli_only":                  true,
			"grok_usage_snapshot":             map[string]any{"status_code": 429REDACTED,
			...
			"antigravity_credits_overages":    map[string]any{"enabled": trueREDACTED,
			"crs_account_id":                  "remote-42",
			"crs_kind":                        "openai-api-key",
			"crs_synced_at":                   "2026-07-15T00:00:00Z",
	REDACTED,
		GroupIDs:                []int64{7, 3REDACTED,
		AccountGroups:           []AccountGroup{{GroupID: 7, Priority: 50REDACTED, {GroupID: 3, Priority: 7REDACTEDREDACTED,
...
REDACTED, duplicate.Extra)
	...
	require.Equal(t, []AccountGroup{
		{AccountID: duplicate.ID, GroupID: 7, Priority: 50REDACTED,
		{AccountID: duplicate.ID, GroupID: 3, Priority: 7REDACTED,
REDACTED, repo.accountGroupsOf[duplicate.ID])
...
func TestDuplicateAccountRejectsRotatingOrUnknownCredentialTypes(t *testing.T) {
	for _, accountType := range []string{AccountTypeOAuth, AccountTypeSetupToken, "legacy-cookie"REDACTED {
		t.Run(accountType, func(t *testing.T) {
			...
			source := &Account{
				...
				Platform:    PlatformOpenAI,
				Type:        accountType,
		REDACTED"refresh_token": "shared-token"REDACTED,
		REDACTED
			require.NoError(t, repo.Create(ctx, source))
...
func TestDuplicateAccountAtomicCreateFailureLeavesNoOrphan(t *testing.T) {
	...
	source := &Account{
		...
		Platform:      PlatformAnthropic,
		Type:          AccountTypeAPIKey,
		Credentials:   map[string]any{"api_key": "secret"REDACTED,
		GroupIDs:      []int64{7REDACTED,
		AccountGroups: []AccountGroup{{GroupID: 7, Priority: 25REDACTEDREDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, source))
