...
func TestAdminService_CreateGroup_RejectsTimePricing(t *testing.T) {
	repo := &groupRepoStubForAdmin{createID: 51REDACTED
...
func TestAdminService_UpdateGroup_RejectsTimePricing(t *testing.T) {
	existing := &Group{ID: 1, Name: "existing", Platform: PlatformOpenAI, Status: StatusActiveREDACTED
	repo := &groupRepoStubForAdmin{getByID: existingREDACTED
	svc := &adminServiceImpl{groupRepo: repoREDACTED
...
REDACTEDREDACTED
	...
	_, err := svc.UpdateGroup(context.Background(), existing.ID, &UpdateGroupInput{ModelPricing: &pricingREDACTED)
...
func TestNormalizeGroupModelPricing_NormalizesEmptyTimePricing(t *testing.T) {
	pricing, err := normalizeGroupModelPricing(PlatformOpenAI, []ChannelModelPricing{{
		Models:      []string{"gpt-5"REDACTED,
		BillingMode: BillingModeToken,
		TimePricing: &ChannelTimePricing{Timezone: "Asia/Shanghai"REDACTED,
REDACTEDREDACTED)
...
func TestAdminService_ListGroups_PassesSortParams(t *testing.T) {
	repo := &groupRepoStubForAdmin{
		listWithFiltersGroups: []Group{{ID: 1, Name: "g1"REDACTEDREDACTED,
REDACTED
	svc := &adminServiceImpl{groupRepo: repoREDACTED
...
REDACTED
	require.Equal(t, pagination.PaginationParams{
		...
		SortOrder: "ASC",
REDACTED, repo.listWithFiltersParams)
REDACTED
...
REDACTED{
		{
			...
			input: func() *CreateGroupInput {
				v := -0.1
				return &CreateGroupInput{Name: "bad-discount", RateMultiplier: 1, BatchImageDiscountMultiplier: &vREDACTED
		REDACTED(),
	REDACTED,
		{
			name: "negative_hold",
			input: func() *CreateGroupInput {
				v := -0.1
				return &CreateGroupInput{Name: "bad-hold", RateMultiplier: 1, BatchImageHoldMultiplier: &vREDACTED
		REDACTED(),
	REDACTED,
REDACTED
...
REDACTED{
		{
			...
			input: &UpdateGroupInput{REDACTED,
			want:  []ReasoningEffortMapping{{From: "max", To: "xhigh"REDACTEDREDACTED,
	REDACTED,
		{
			name: "empty array clears mappings",
			input: func() *UpdateGroupInput {
				empty := []ReasoningEffortMapping{REDACTED
				return &UpdateGroupInput{ReasoningEffortMappings: &emptyREDACTED
		REDACTED(),
			want: []ReasoningEffortMapping{REDACTED,
	REDACTED,
		{
			name: "non empty array replaces and canonicalizes mappings",
			input: func() *UpdateGroupInput {
				replacement := []ReasoningEffortMapping{{From: " X-HIGH ", To: " high "REDACTEDREDACTED
				return &UpdateGroupInput{ReasoningEffortMappings: &replacementREDACTED
		REDACTED(),
			want: []ReasoningEffortMapping{{From: "xhigh", To: "high"REDACTEDREDACTED,
	REDACTED,
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			existing := &Group{
				...
				Platform:                PlatformOpenAI,
				Status:                  StatusActive,
				ReasoningEffortMappings: []ReasoningEffortMapping{{From: "max", To: "xhigh"REDACTEDREDACTED,
		REDACTED
			repo := &groupRepoStubForAdmin{getByID: existingREDACTED
			svc := &adminServiceImpl{groupRepo: repoREDACTED
...
func TestAdminService_UpdateGroup_RejectsInvalidReasoningEffortMappings(t *testing.T) {
	existing := &Group{
		...
		Status:           StatusActive,
REDACTED
	repo := &groupRepoStubForInvalidRequestFallback{groups: map[int64]*Group{existing.ID: existingREDACTEDREDACTED
	svc := &adminServiceImpl{groupRepo: repoREDACTED
...
REDACTED
	...
	_, err := svc.UpdateGroup(context.Background(), existing.ID, &UpdateGroupInput{Platform: PlatformAnthropicREDACTED)
...
REDACTED
	...
	REDACTED,
REDACTED, repo.created.MessagesDispatchModelConfig)
REDACTED
...
REDACTED
	...
	REDACTED,
REDACTED, repo.updated.MessagesDispatchModelConfig)
REDACTED
...
REDACTED
	...
	require.Equal(t, OpenAIMessagesDispatchModelConfig{REDACTED, repo.updated.MessagesDispatchModelConfig)
REDACTED
...
func TestAdminService_ListGroups_WithSearch(t *testing.T) {
	...
	t.Run("search 参数正常传递到 repository 层", func(t *testing.T) {
		repo := &groupRepoStubForAdmin{
			listWithFiltersGroups: []Group{{ID: 1, Name: "alpha"REDACTEDREDACTED,
	...
	REDACTED
		...
		require.Equal(t, []Group{{ID: 1, Name: "alpha"REDACTEDREDACTED, groups)

		require.Equal(t, 1, repo.listWithFiltersCalls)
		require.Equal(t, pagination.PaginationParams{Page: 1, PageSize: 20REDACTED, repo.listWithFiltersParams)
...
REDACTED)
	...
	t.Run("search 为空字符串时传递空字符串", func(t *testing.T) {
		repo := &groupRepoStubForAdmin{
			listWithFiltersGroups: []Group{REDACTED,
			listWithFiltersResult: &pagination.PaginationResult{Total: 0REDACTED,
	REDACTED
		svc := &adminServiceImpl{groupRepo: repoREDACTED
	...
	REDACTED
		...
		require.Equal(t, 1, repo.listWithFiltersCalls)
		require.Equal(t, pagination.PaginationParams{Page: 2, PageSize: 10REDACTED, repo.listWithFiltersParams)
...
REDACTED)
	...
	t.Run("search 与其他过滤条件组合使用", func(t *testing.T) {
		isExclusive := true
		repo := &groupRepoStubForAdmin{
			listWithFiltersGroups: []Group{{ID: 2, Name: "beta"REDACTEDREDACTED,
			listWithFiltersResult: &pagination.PaginationResult{Total: 42REDACTED,
	REDACTED
		svc := &adminServiceImpl{groupRepo: repoREDACTED
	...
	REDACTED
		...
		require.Equal(t, []Group{{ID: 2, Name: "beta"REDACTEDREDACTED, groups)

		require.Equal(t, 1, repo.listWithFiltersCalls)
		require.Equal(t, pagination.PaginationParams{Page: 3, PageSize: 50REDACTED, repo.listWithFiltersParams)
...
func TestAdminService_CreateGroup_InvalidRequestFallbackRejectsUnsupportedPlatform(t *testing.T) {
	...
	repo := &groupRepoStubForInvalidRequestFallback{
		groups: map[int64]*Group{
			fallbackID: {ID: fallbackID, Platform: PlatformAnthropic, SubscriptionType: SubscriptionTypeStandardREDACTED,
	REDACTED,
REDACTED
	svc := &adminServiceImpl{groupRepo: repoREDACTED
...
REDACTED{
		{
			name:        "openai_target",
			fallback:    &Group{ID: 10, Platform: PlatformOpenAI, SubscriptionType: SubscriptionTypeStandardREDACTED,
			wantMessage: "fallback group must be anthropic platform",
	REDACTED,
		{
			name:        "antigravity_target",
			fallback:    &Group{ID: 10, Platform: PlatformAntigravity, SubscriptionType: SubscriptionTypeStandardREDACTED,
			wantMessage: "fallback group must be anthropic platform",
	REDACTED,
		{
			name:        "subscription_group",
			fallback:    &Group{ID: 10, Platform: PlatformAnthropic, SubscriptionType: SubscriptionTypeSubscriptionREDACTED,
	...
	REDACTED,
		{
			...
			fallback: &Group{
				...
				Platform:                        PlatformAnthropic,
				SubscriptionType:                SubscriptionTypeStandard,
				FallbackGroupIDOnInvalidRequest: func() *int64 { v := int64(99); return &v REDACTED(),
		REDACTED,
...
func TestAdminService_CreateGroup_InvalidRequestFallbackNotFound(t *testing.T) {
	fallbackID := int64(10)
	repo := &groupRepoStubForInvalidRequestFallback{REDACTED
	svc := &adminServiceImpl{groupRepo: repoREDACTED
...
REDACTED
	repo := &groupRepoStubForInvalidRequestFallback{
		groups: map[int64]*Group{
			existing.ID: existing,
			fallbackID:  {ID: fallbackID, Platform: PlatformAnthropic, SubscriptionType: SubscriptionTypeSubscriptionREDACTED,
	REDACTED,
REDACTED
	svc := &adminServiceImpl{groupRepo: repoREDACTED
