...
REDACTED
	...
	// Test 1: 基本生成
	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "p-spark", Priority: 50REDACTED)
REDACTED
	...
	// Test 2: 一母一影 — 再作成は拒否
	_, err = svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "dup"REDACTED)
REDACTED
REDACTED
...
func TestCreateShadowInheritsParentEffectiveOpenAILongContextBillingValue(t *testing.T) {
	tests := []struct {
		...
		want        bool
REDACTED{
		{name: "missing parent value defaults enabled", want: trueREDACTED,
...
REDACTED
	...
	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{
		Name:     "p-spark",
		GroupIDs: []int64{testGroupIDREDACTED,
REDACTED)
REDACTED
	require.NotNil(t, shadow)
	require.Equal(t, []int64{testGroupIDREDACTED, shadow.GroupIDs, "CreateShadow should backfill GroupIDs into the returned shadow")
...
func TestCreateShadow_InheritsParentConcurrency(t *testing.T) {
	...
	REDACTED"chatgpt_account_id": "org-c"REDACTED,
	REDACTED
		require.NoError(t, repo.Create(ctx, parent))

		shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "conc-shadow"REDACTED)
	REDACTED
...
REDACTED)
	...
	t.Run("explicit_positive_kept", func(t *testing.T) {
		...
		parent := &Account{
			...
			Status: StatusActive, Concurrency: 3,
	REDACTED"chatgpt_account_id": "org-c2"REDACTED,
	REDACTED
		require.NoError(t, repo.Create(ctx, parent))

		shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "conc-shadow2", Concurrency: 2REDACTED)
	REDACTED
...
func TestCreateShadow_InheritsParentPriorityWhenOmitted(t *testing.T) {
	...
	REDACTED"chatgpt_account_id": "org-p"REDACTED,
	REDACTED
		require.NoError(t, repo.Create(ctx, parent))

		// 模拟前端一键创建:只传 name,priority 省略=0。
		shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "prio-shadow"REDACTED)
	REDACTED
...
REDACTED)
	...
	t.Run("explicit_positive_kept", func(t *testing.T) {
		...
		parent := &Account{
			...
			Status: StatusActive, Priority: 30,
	REDACTED"chatgpt_account_id": "org-p2"REDACTED,
	REDACTED
		require.NoError(t, repo.Create(ctx, parent))

		shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "prio-shadow2", Priority: 7REDACTED)
	REDACTED
...
func TestPersistAccountCredentials_SkipsShadow(t *testing.T) {
	...
	shadow := &Account{
		...
		Status: StatusActive, Credentials: map[string]any{REDACTED,
...
REDACTED
	...
	err := persistAccountCredentials(ctx, repo, shadow, map[string]any{"access_token": "LEAK", "refresh_token": "LEAK"REDACTED)
REDACTED
...
REDACTED"refresh_token": "RT"REDACTED,
REDACTED
...
REDACTEDREDACTED, ParentAccountID: &grandparent.ID, QuotaDimension: QuotaDimensionSpark,
REDACTED
...
REDACTEDREDACTED, ParentAccountID: &parentShadow.ID, QuotaDimension: QuotaDimensionSpark,
REDACTED
...
func TestPersistOpenAI429PlanType_SkipsShadow(t *testing.T) {
	ctx := context.Background()
	body := []byte(`{"error":{"type":"usage_limit_reached","plan_type":"pro"REDACTEDREDACTED`)
	parentID := int64(1)

	t.Run("shadow_skipped", func(t *testing.T) {
		repo := newSparkShadowRepoStub()
		shadow := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{REDACTED, ParentAccountID: &parentIDREDACTED
...
REDACTED)
	...
	t.Run("normal_account_writes", func(t *testing.T) {
		repo := newSparkShadowRepoStub()
		normal := &Account{ID: 9, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{REDACTEDREDACTED
...
func TestPersistOpenAICodexSnapshot_SkipsShadow(t *testing.T) {
	...
	t.Run("shadow_skipped", func(t *testing.T) {
		spy := &updateExtraSpyRepo{sparkShadowRepoStub: newSparkShadowRepoStub()REDACTED
		s := &RateLimitService{accountRepo: spyREDACTED
		shadow := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, ParentAccountID: &parentIDREDACTED
...
REDACTED)
	...
	t.Run("normal_account_writes", func(t *testing.T) {
		spy := &updateExtraSpyRepo{sparkShadowRepoStub: newSparkShadowRepoStub()REDACTED
		s := &RateLimitService{accountRepo: spyREDACTED
		normal := &Account{ID: 9, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
...
REDACTED"chatgpt_account_id": "o"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))
	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "rq-shadow"REDACTED)
REDACTED
...
func TestCreateShadow_DefaultGroupBinding(t *testing.T) {
	...
	groupRepo := &sparkShadowGroupRepoStub{
		groups: []Group{
			{ID: 99, Name: PlatformOpenAI + "-default"REDACTED,
			{ID: 7, Name: "some-other-group"REDACTED,
	REDACTED,
REDACTED
	svc := &adminServiceImpl{accountRepo: repo, groupRepo: groupRepoREDACTED

	parent := &Account{
		Name: "grp-parent", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
		Status: StatusActive, Credentials: map[string]any{"chatgpt_account_id": "org-g"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))

	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "grp-shadow"REDACTED)
REDACTED
	require.Equal(t, []int64{99REDACTED, repo.groupsOf[shadow.ID], "未指定分组应回落绑定 openai-default(id=99)")
REDACTED
...
func TestCreateShadow_InheritsParentGroups(t *testing.T) {
	...
	groupRepo := &sparkShadowGroupRepoStub{groups: []Group{{ID: 99, Name: PlatformOpenAI + "-default"REDACTEDREDACTEDREDACTED
	svc := &adminServiceImpl{accountRepo: repo, groupRepo: groupRepoREDACTED

	parent := &Account{
		Name: "grp-parent", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
		Status: StatusActive, GroupIDs: []int64{11, 22REDACTED,
REDACTED"chatgpt_account_id": "org-grp"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))

	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "grp-shadow"REDACTED)
REDACTED
	require.Equal(t, []int64{11, 22REDACTED, repo.groupsOf[shadow.ID], "未指定分组应继承母账号分组,而非 openai-default")
REDACTED
...
func TestCreateShadow_RejectsShadowAsParent(t *testing.T) {
	...
	parent := &Account{
		...
		Status: StatusActive, Credentials: map[string]any{"chatgpt_account_id": "org-x"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))
	firstShadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "first-shadow"REDACTED)
REDACTED

	// 把一级影子当母 → 必须被拒(400)。
	_, err = svc.CreateShadow(ctx, firstShadow.ID, ShadowOptions{Name: "second-shadow"REDACTED)
REDACTED
...
func TestCreateShadow_StructuredErrors(t *testing.T) {
	...
	t.Run("non_oauth_parent_400", func(t *testing.T) {
		repo := newSparkShadowRepoStub()
		svc := &adminServiceImpl{accountRepo: repoREDACTED
		parent := &Account{Name: "apikey-parent", Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActiveREDACTED
		require.NoError(t, repo.Create(ctx, parent))
		_, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "s"REDACTED)
	REDACTED
...
REDACTED)
	...
	t.Run("duplicate_409", func(t *testing.T) {
		repo := newSparkShadowRepoStub()
		svc := &adminServiceImpl{accountRepo: repoREDACTED
		parent := &Account{Name: "p", Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Credentials: map[string]any{"chatgpt_account_id": "o"REDACTEDREDACTED
		require.NoError(t, repo.Create(ctx, parent))
		_, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "s1"REDACTED)
	REDACTED
		_, err = svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "s2"REDACTED)
	REDACTED
...
func TestUpdateAccount_RejectsTypeChangeOnShadow(t *testing.T) {
	...
	parent := &Account{
		...
		Status: StatusActive, Credentials: map[string]any{"chatgpt_account_id": "org-t"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))
	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "type-shadow"REDACTED)
REDACTED

	// 试图把影子 type 改成 apikey → 必须被拒(400)。
	_, err = svc.UpdateAccount(ctx, shadow.ID, &UpdateAccountInput{Type: AccountTypeAPIKeyREDACTED)
REDACTED
	...
	_, err = svc.UpdateAccount(ctx, shadow.ID, &UpdateAccountInput{Type: AccountTypeOAuthREDACTED)
...
func TestBulkUpdateAccounts_RejectsCredentialWriteToShadow(t *testing.T) {
	...
	parent := &Account{
		...
		Status: StatusActive, Credentials: map[string]any{"chatgpt_account_id": "org-b", "access_token": "t"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))
	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "bulk-shadow"REDACTED)
REDACTED

	_, err = svc.BulkUpdateAccounts(ctx, &BulkUpdateAccountsInput{
		AccountIDs:  []int64{shadow.IDREDACTED,
REDACTED"access_token": "leaked"REDACTED,
REDACTED)
...
func TestDeleteAccount_CascadeToShadow(t *testing.T) {
	...
	parent := &Account{
		...
		Type:        AccountTypeOAuth,
		Status:      StatusActive,
REDACTED"chatgpt_account_id": "org-cascade"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))

	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "cascade-shadow"REDACTED)
REDACTED
	shadowID := shadow.ID
...
func TestUpdateAccount_PropagatesProxyToShadow(t *testing.T) {
	...
	parent := &Account{
		...
		Status:      StatusActive,
		ProxyID:     &oldProxy,
REDACTED"chatgpt_account_id": "org-proxy"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))

	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "proxy-shadow"REDACTED)
REDACTED
	shadowID := shadow.ID

	// Update parent's ProxyID.
	newProxy := int64(42)
	_, err = svc.UpdateAccount(ctx, parent.ID, &UpdateAccountInput{ProxyID: &newProxyREDACTED)
REDACTED
...
func TestUpdateAccount_RejectsCredentialWriteToShadow(t *testing.T) {
	...
	parent := &Account{
		...
		Type:        AccountTypeOAuth,
		Status:      StatusActive,
REDACTED"access_token": "parent-secret", "refresh_token": "parent-rt"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))

	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "cred-shadow"REDACTED)
REDACTED
...
REDACTED"access_token": "leaked", "refresh_token": "leaked-rt"REDACTED,
REDACTED)
	...
	newPriority := 5
	_, err = svc.UpdateAccount(ctx, shadow.ID, &UpdateAccountInput{Priority: &newPriorityREDACTED)
...
func TestBulkUpdateAccounts_PropagatesProxyToShadow(t *testing.T) {
	...
	parent := &Account{
		...
		Status:      StatusActive,
		ProxyID:     &oldProxy,
REDACTED"chatgpt_account_id": "org-bulk"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))
...
REDACTED
	...
	newProxy := int64(99)
	_, err = svc.BulkUpdateAccounts(ctx, &BulkUpdateAccountsInput{
		AccountIDs: []int64{parent.IDREDACTED,
		ProxyID:    &newProxy,
REDACTED)
REDACTED
...
func TestCreateShadow_DefaultsNameFromParent(t *testing.T) {
	...
	parent := &Account{
		...
		Status: StatusActive, Credentials: map[string]any{"chatgpt_account_id": "o"REDACTED,
REDACTED
	require.NoError(t, repo.Create(ctx, parent))

	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "   "REDACTED)
...
func TestCreateShadow_ConcurrentCreateReturns409(t *testing.T) {
	...
	repo := &raceCreateRepoStub{sparkShadowRepoStub: baseREDACTED
	svc := &adminServiceImpl{accountRepo: repoREDACTED
	parent := &Account{
...
func TestCreateShadow_InvalidGroupRejectedNoOrphan(t *testing.T) {
	ctx := context.Background()
	repo := newSparkShadowRepoStub()
	groupRepo := &sparkShadowValidatingGroupRepoStub{existing: map[int64]bool{7: trueREDACTEDREDACTED
	svc := &adminServiceImpl{accountRepo: repo, groupRepo: groupRepoREDACTED
	parent := &Account{
		Name: "p", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
		Status: StatusActive, Credentials: map[string]any{"chatgpt_account_id": "o"REDACTED,
REDACTED
	...
	_, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "s", GroupIDs: []int64{999REDACTEDREDACTED)
...
func TestCreateShadow_BindFailureRollsBackShadow(t *testing.T) {
	ctx := context.Background()
	base := newSparkShadowRepoStub()
	repo := &bindFailRepoStub{sparkShadowRepoStub: baseREDACTED
	groupRepo := &sparkShadowValidatingGroupRepoStub{existing: map[int64]bool{7: trueREDACTEDREDACTED
	svc := &adminServiceImpl{accountRepo: repo, groupRepo: groupRepoREDACTED
	parent := &Account{
		Name: "p", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
...
REDACTED
	...
	_, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "s", GroupIDs: []int64{7REDACTEDREDACTED)
...
REDACTED
	...
	_, err = svc.UpdateAccount(ctx, parent.ID, &UpdateAccountInput{Type: AccountTypeAPIKeyREDACTED)
REDACTED
	...
	_, err = svc.UpdateAccount(ctx, parent.ID, &UpdateAccountInput{Type: AccountTypeOAuthREDACTED)
...
REDACTED
	require.NoError(t, repo.Create(ctx, parent))
	shadow, err := svc.CreateShadow(ctx, parent.ID, ShadowOptions{Name: "s"REDACTED)
REDACTED
	...
	newProxy := int64(42)
	_, err = svc.UpdateAccount(ctx, shadow.ID, &UpdateAccountInput{ProxyID: &newProxyREDACTED)
...
REDACTED
	require.Equal(t, []int64{7REDACTED, repo.groupsOf[shadow.ID])
	require.Equal(t, map[string]any{"gpt-5.3-codex-spark": "gpt-5.3-codex-spark"REDACTED, updated.Credentials["model_mapping"])
...
REDACTED
	...
	_, err := svc.UpdateAccount(ctx, shadow.ID, &UpdateAccountInput{
REDACTED"access_token": "leak"REDACTED,
REDACTED)
...
func TestForceOpenAIPrivacy_SkipsShadow(t *testing.T) {
	svc := &adminServiceImpl{REDACTED
	pid := int64(1)
	shadow := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth, ParentAccountID: &pidREDACTED
