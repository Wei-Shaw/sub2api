...
func (s *adminServiceImpl) ensureNotLastAdmin(ctx context.Context) error {
	...
	_, result, err := s.userRepo.ListWithFilters(ctx,
		pagination.PaginationParams{Page: 1, PageSize: 1REDACTED,
		UserListFilters{Role: RoleAdmin, IncludeSubscriptions: &noSubsREDACTED,
	)
	if err != nil {
...
REDACTED
	...
	cleaned := make([]int64, 0, len(userIDs))
	seen := make(map[int64]struct{REDACTED, len(userIDs))
	...
	REDACTED
		if _, ok := seen[userID]; ok {
			continue
	REDACTED
		seen[userID] = struct{REDACTED{REDACTED
