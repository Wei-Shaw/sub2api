...
func TestResolveRebateRatePercent_PerUserOverride(t *testing.T) {
	t.Parallel()
	svc := &AffiliateService{REDACTED

	// nil exclusive rate → falls back to global default (20%)
	require.InDelta(t, AffiliateRebateRateDefault,
		svc.resolveRebateRatePercent(context.Background(), &AffiliateSummary{REDACTED), 1e-9)

	// exclusive rate set → overrides global
	rate := 50.0
	require.InDelta(t, 50.0,
		svc.resolveRebateRatePercent(context.Background(), &AffiliateSummary{AffRebateRatePercent: &rateREDACTED), 1e-9)

	// exclusive rate 0 → returns 0 (no rebate, intentional)
	zero := 0.0
	require.InDelta(t, 0.0,
		svc.resolveRebateRatePercent(context.Background(), &AffiliateSummary{AffRebateRatePercent: &zeroREDACTED), 1e-9)

	// exclusive rate above max → clamped to Max
	tooHigh := 250.0
	require.InDelta(t, AffiliateRebateRateMax,
		svc.resolveRebateRatePercent(context.Background(), &AffiliateSummary{AffRebateRatePercent: &tooHighREDACTED), 1e-9)

	// exclusive rate below min → clamped to Min
	tooLow := -5.0
	require.InDelta(t, AffiliateRebateRateMin,
		svc.resolveRebateRatePercent(context.Background(), &AffiliateSummary{AffRebateRatePercent: &tooLowREDACTED), 1e-9)
REDACTED
...
func TestValidateExclusiveRate_BoundaryAndInvalid(t *testing.T) {
	...
	for _, v := range []float64{0, 0.01, 50, 99.99, 100REDACTED {
		v := v
		require.NoError(t, validateExclusiveRate(&v), "value %v should be valid", v)
REDACTED

	for _, v := range []float64{-0.01, 100.01, -100, 200REDACTED {
		v := v
...
func TestIsValidAffiliateCodeFormat(t *testing.T) {
	...
	// 因此校验放宽到 [A-Z0-9_-]{4,32REDACTED（要求调用方先 ToUpper）。
	cases := []struct {
		name string
		in   string
		want bool
REDACTED{
		{"valid canonical 12-char", "ABCDEFGHJKLM", trueREDACTED,
		{"valid all digits 2-9", "234567892345", trueREDACTED,
		{"valid mixed", "A2B3C4D5E6F7", trueREDACTED,
		{"valid admin custom short", "VIP1", trueREDACTED,
		{"valid admin custom with hyphen", "NEW-USER", trueREDACTED,
		{"valid admin custom with underscore", "VIP_2026", trueREDACTED,
		{"valid 32-char max", "ABCDEFGHIJKLMNOPQRSTUVWXYZ012345", trueREDACTED,
		// Previously-excluded chars (I/O/0/1) are now allowed since admins may use them.
		{"letter I now allowed", "IBCDEFGHJKLM", trueREDACTED,
		{"letter O now allowed", "OBCDEFGHJKLM", trueREDACTED,
		{"digit 0 now allowed", "0BCDEFGHJKLM", trueREDACTED,
		{"digit 1 now allowed", "1BCDEFGHJKLM", trueREDACTED,
		{"too short (3 chars)", "ABC", falseREDACTED,
		{"too long (33 chars)", "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456", falseREDACTED,
		{"lowercase rejected (caller must ToUpper first)", "abcdefghjklm", falseREDACTED,
		{"empty", "", falseREDACTED,
		{"utf8 non-ascii", "ÄÄÄÄÄÄ", falseREDACTED, // bytes out of charset
