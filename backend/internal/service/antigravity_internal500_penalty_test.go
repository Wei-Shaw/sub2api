...
func (r *internal500AccountRepoStub) SetError(_ context.Context, id int64, errorMsg string) error {
	r.setErrorCalls = append(r.setErrorCalls, setErrorCall{accountID: id, reason: errorMsgREDACTED)
	return nil
REDACTED
...
func TestIsAntigravityInternalServerError(t *testing.T) {
	t.Run("匹配完整的 INTERNAL 500 body", func(t *testing.T) {
		body := []byte(`{"error":{"code":500,"message":"Internal error encountered.","status":"INTERNAL"REDACTEDREDACTED`)
...
REDACTED)
	...
	t.Run("body 中 message 不匹配", func(t *testing.T) {
		body := []byte(`{"error":{"code":500,"message":"Some other error","status":"INTERNAL"REDACTEDREDACTED`)
		require.False(t, isAntigravityInternalServerError(500, body))
REDACTED)

	t.Run("body 中 status 不匹配", func(t *testing.T) {
		body := []byte(`{"error":{"code":500,"message":"Internal error encountered.","status":"UNAVAILABLE"REDACTEDREDACTED`)
		require.False(t, isAntigravityInternalServerError(500, body))
REDACTED)

	t.Run("body 中 code 不匹配", func(t *testing.T) {
		body := []byte(`{"error":{"code":503,"message":"Internal error encountered.","status":"INTERNAL"REDACTEDREDACTED`)
		require.False(t, isAntigravityInternalServerError(500, body))
REDACTED)

	t.Run("空 body", func(t *testing.T) {
		require.False(t, isAntigravityInternalServerError(500, []byte{REDACTED))
...
REDACTED)
	...
	t.Run("其他 500 错误格式（不同 JSON 结构）", func(t *testing.T) {
		body := []byte(`{"message":"Internal Server Error","statusCode":500REDACTED`)
		require.False(t, isAntigravityInternalServerError(500, body))
REDACTED)
REDACTED
...
func TestApplyInternal500Penalty(t *testing.T) {
	t.Run("count=1 → SetTempUnschedulable 10 分钟", func(t *testing.T) {
		repo := &internal500AccountRepoStub{REDACTED
		svc := &AntigravityGatewayService{accountRepo: repoREDACTED
		account := &Account{ID: 1, Name: "acc-1"REDACTED
...
REDACTED)
	...
	t.Run("count=2 → SetTempUnschedulable 10 小时", func(t *testing.T) {
		repo := &internal500AccountRepoStub{REDACTED
		svc := &AntigravityGatewayService{accountRepo: repoREDACTED
		account := &Account{ID: 2, Name: "acc-2"REDACTED
...
REDACTED)
	...
	t.Run("count=3 → SetError 永久禁用", func(t *testing.T) {
		repo := &internal500AccountRepoStub{REDACTED
		svc := &AntigravityGatewayService{accountRepo: repoREDACTED
		account := &Account{ID: 3, Name: "acc-3"REDACTED
...
REDACTED)
	...
	t.Run("count=5 → SetError 永久禁用（>=3 都走永久禁用）", func(t *testing.T) {
		repo := &internal500AccountRepoStub{REDACTED
		svc := &AntigravityGatewayService{accountRepo: repoREDACTED
		account := &Account{ID: 5, Name: "acc-5"REDACTED
...
REDACTED)
	...
	t.Run("count=0 → 不调用任何方法", func(t *testing.T) {
		repo := &internal500AccountRepoStub{REDACTED
		svc := &AntigravityGatewayService{accountRepo: repoREDACTED
		account := &Account{ID: 10, Name: "acc-10"REDACTED
...
REDACTED)
	...
	REDACTED
		svc := &AntigravityGatewayService{
			...
			internal500Cache: cache,
	REDACTED
		account := &Account{ID: 4, Name: "acc-4"REDACTED
...
REDACTED)
	...
	t.Run("正常调用 → 调用 ResetInternal500Count", func(t *testing.T) {
		cache := &mockInternal500Cache{REDACTED
		svc := &AntigravityGatewayService{
			internal500Cache: cache,
	REDACTED
