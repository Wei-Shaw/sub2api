...
func (s *stubAntigravityAccountRepo) SetModelRateLimit(ctx context.Context, id int64, modelKey string, resetAt time.Time, reason ...string) error {
	s.modelRateLimitCalls = append(s.modelRateLimitCalls, modelRateLimitCall{accountID: id, modelKey: modelKey, resetAt: resetAtREDACTED)
	return nil
REDACTED

func (s *stubAntigravityAccountRepo) UpdateExtra(ctx context.Context, id int64, updates map[string]any) error {
	s.extraUpdateCalls = append(s.extraUpdateCalls, extraUpdateCall{accountID: id, updates: updatesREDACTED)
	return nil
REDACTED
...
func TestHandleUpstreamError_429_ModelRateLimit(t *testing.T) {
	...
	account := &Account{ID: 1, Name: "acc-1", Platform: PlatformAntigravityREDACTED
...
func TestHandleUpstreamError_429_NonModelRateLimit_UsesMappedModelKey(t *testing.T) {
	repo := &stubAntigravityAccountRepo{REDACTED
	svc := &AntigravityGatewayService{accountRepo: repoREDACTED
	account := &Account{ID: 20, Name: "acc-20", Platform: PlatformAntigravityREDACTED

	body := buildGeminiRateLimitBody("5s")

	result := svc.handleUpstreamError(context.Background(), "[test]", account, http.StatusTooManyRequests, http.Header{REDACTED, body, "claude-opus-4-6", 0, "", false)
...
func TestHandleUpstreamError_503_ModelCapacityExhausted(t *testing.T) {
	repo := &stubAntigravityAccountRepo{REDACTED
	svc := &AntigravityGatewayService{accountRepo: repoREDACTED
	account := &Account{ID: 3, Name: "acc-3", Platform: PlatformAntigravityREDACTED
...
func TestHandleUpstreamError_503_NonModelRateLimit(t *testing.T) {
	repo := &stubAntigravityAccountRepo{REDACTED
	svc := &AntigravityGatewayService{accountRepo: repoREDACTED
	account := &Account{ID: 4, Name: "acc-4", Platform: PlatformAntigravityREDACTED
	...
	body := []byte(`{
		"error": {
			...
			"details": [
				{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "reason": "SERVICE_UNAVAILABLE"REDACTED
			]
	REDACTED
REDACTED`)
...
func TestHandleUpstreamError_503_EmptyBody(t *testing.T) {
	repo := &stubAntigravityAccountRepo{REDACTED
	svc := &AntigravityGatewayService{accountRepo: repoREDACTED
	account := &Account{ID: 5, Name: "acc-5", Platform: PlatformAntigravityREDACTED
			...
			body: `{
				"error": {
					...
					"details": [
						{
							"@type": "type.googleapis.com/google.rpc.ErrorInfo",
							"metadata": {"model": "claude-sonnet-4-5"REDACTED,
	...
	REDACTED,
		{
			...
			body: `{
				"error": {
					...
					"status": "UNAVAILABLE",
					"details": [
						{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "metadata": {"model": "gemini-pro"REDACTED, "reason": "SERVICE_UNAVAILABLE"REDACTED,
	...
	REDACTED,
		{
			...
			body: `{
				"error": {
					...
					"details": [
						{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "3s"REDACTED
					]
			REDACTED
		REDACTED`,
			expectedNil: true,
	...
	REDACTED,
		{
			...
			body: `{
				"error": {
					...
					"status": "RESOURCE_EXHAUSTED",
					"details": [
						{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "metadata": {"model": "test-model"REDACTED, "reason": "RATE_LIMIT_EXCEEDED"REDACTED,
						{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "500ms"REDACTED
					]
			REDACTED
		REDACTED`,
			...
			body: `{
				"error": {
					...
					"details": [
						...
						{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "4m50s"REDACTED
					]
			REDACTED
		REDACTED`,
	...
	REDACTED,
		{
			...
			body: `{
				"error": {
					...
					"status": "RESOURCE_EXHAUSTED",
					"details": [
						{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "reason": "RATE_LIMIT_EXCEEDED"REDACTED,
						{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "3s"REDACTED
					]
			REDACTED
		REDACTED`,
...
func TestShouldTriggerAntigravitySmartRetry(t *testing.T) {
	...
	upstreamAccount := &Account{Type: AccountTypeUpstream, Platform: PlatformAntigravityREDACTED
	...
	REDACTED,
		{
			...
			body: `{
				"error": {
					...
					"details": [
						{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "metadata": {"model": "claude-sonnet-4-5"REDACTED, "reason": "RATE_LIMIT_EXCEEDED"REDACTED,
						{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "2s"REDACTED
					]
			REDACTED
		REDACTED`,
			expectedShouldRetry:     true,
	...
	REDACTED,
		{
			...
			body: `{
				"error": {
					"status": "RESOURCE_EXHAUSTED",
					"details": [
						{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "metadata": {"model": "test"REDACTED, "reason": "RATE_LIMIT_EXCEEDED"REDACTED,
						{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "0.5s"REDACTED
					]
			REDACTED
		REDACTED`,
	...
	REDACTED,
		{
			...
			body: `{
				"error": {
					...
					"status": "UNAVAILABLE",
					"details": [
						{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "metadata": {"model": "gemini-2.5-flash"REDACTED, "reason": "MODEL_CAPACITY_EXHAUSTED"REDACTED
					],
...
func TestSetAntigravityModelRateLimits_GeminiWritesFamilyScope(t *testing.T) {
	repo := &stubAntigravityAccountRepo{REDACTED
	svc := &AntigravityGatewayService{REDACTED
	account := &Account{ID: 789, Platform: PlatformAntigravityREDACTED
	resetAt := time.Now().Add(30 * time.Second)
...
func TestSetAntigravityModelRateLimits_ClaudeDoesNotWriteGeminiScope(t *testing.T) {
	repo := &stubAntigravityAccountRepo{REDACTED
	svc := &AntigravityGatewayService{REDACTED
	account := &Account{ID: 790, Platform: PlatformAntigravityREDACTED
	resetAt := time.Now().Add(30 * time.Second)
...
func TestUpdateAccountModelRateLimitInCache_UpdatesExtraAndCallsCache(t *testing.T) {
	cache := &stubSchedulerCache{REDACTED
	snapshotService := &SchedulerSnapshotService{cache: cacheREDACTED
...
REDACTED
	...
	account := &Account{ID: 1, Name: "test"REDACTED
...
func TestSchedulerSnapshotService_UpdateAccountInCache(t *testing.T) {
	t.Run("calls cache.SetAccount", func(t *testing.T) {
		cache := &stubSchedulerCache{REDACTED
		svc := &SchedulerSnapshotService{cache: cacheREDACTED

		account := &Account{ID: 123, Name: "test"REDACTED
...
REDACTED)
	...
	t.Run("returns nil when cache is nil", func(t *testing.T) {
		svc := &SchedulerSnapshotService{cache: nilREDACTED

		err := svc.UpdateAccountInCache(context.Background(), &Account{ID: 1REDACTED)
...
REDACTED)
	...
	t.Run("propagates cache error", func(t *testing.T) {
		...
		cache := &stubSchedulerCache{setAccountErr: expectedErrREDACTED
		svc := &SchedulerSnapshotService{cache: cacheREDACTED
