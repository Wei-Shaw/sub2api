...
const apiKeyAuthSnapshotVersion = 21 // v21: api key smart routing fields (force refresh pre-fix snapshots) // v20: group long-context and model pricing fields (force refresh of pre-fix snapshots)
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		SmartRoutingEnabled: apiKey.SmartRoutingEnabled,
		SmartRoutingConfig:  apiKey.SmartRoutingConfig,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		SmartRoutingEnabled: snapshot.SmartRoutingEnabled,
		SmartRoutingConfig:  snapshot.SmartRoutingConfig,
		User: &User{
			ID:                         snapshot.User.ID,
