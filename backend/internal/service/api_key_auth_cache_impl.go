...
const apiKeyAuthSnapshotVersion = 25 // v25: api key platform_limits (per-upstream-platform sub-limits)
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		PlatformLimits: apiKey.PlatformLimits,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		PlatformLimits: snapshot.PlatformLimits,
		User: &User{
			ID:                         snapshot.User.ID,
