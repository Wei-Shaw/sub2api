...
func TestAPIKeyAuthSnapshotGroupPricingRoundtrip(t *testing.T) {
	...
	apiKey := &APIKey{
		...
		User: &User{ID: 40, Status: StatusActiveREDACTED,
		Group: &Group{
...
REDACTED
	...
	payload, err := json.Marshal(&APIKeyAuthCacheEntry{Snapshot: svc.snapshotFromAPIKey(context.Background(), apiKey)REDACTED)
REDACTED
...
REDACTED
	...
	billing := &BillingService{fallbackPrices: map[string]*ModelPricing{
		"claude-sonnet-4": {InputPricePerToken: 3e-6, OutputPricePerToken: 15e-6REDACTED,
REDACTEDREDACTED
	resolver := NewModelPricingResolver(nil, billing)
	resolved := resolver.Resolve(context.Background(), PricingInput{Model: "claude-sonnet-4", Group: materialized.GroupREDACTED)
