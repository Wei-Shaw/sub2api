...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	payload, err := json.Marshal(&APIKeyAuthCacheEntry{Snapshot: snapshotREDACTED)
REDACTED
...
REDACTED
	...
	gwSvc := &OpenAIGatewayService{REDACTED
...
func TestAPIKeyAuthSnapshotOldVersionEvicted(t *testing.T) {
	...
	materialized, used, err := svc.applyAuthCacheEntry("sk-old", &APIKeyAuthCacheEntry{Snapshot: snapshotREDACTED)
REDACTED
