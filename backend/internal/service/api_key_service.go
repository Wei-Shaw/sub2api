...
func (f APIKeyUpdateFields) IsEmpty() bool {
	return f == APIKeyUpdateFields{REDACTED
REDACTED
...
type authCacheSubscriptionReadyKey struct{REDACTED

func withAuthCacheSubscriptionReady(ctx context.Context, ready func()) context.Context {
	return context.WithValue(ctx, authCacheSubscriptionReadyKey{REDACTED, ready)
REDACTED
...
func NotifyAuthCacheSubscriptionReady(ctx context.Context) {
	if ready, ok := ctx.Value(authCacheSubscriptionReadyKey{REDACTED).(func()); ok && ready != nil {
		ready()
REDACTED
REDACTED
...
func validateCreateAPIKeyRequest(req CreateAPIKeyRequest) error {
	for _, v := range []float64{req.Quota, req.RateLimit5h, req.RateLimit1d, req.RateLimit7dREDACTED {
...
type APIKeyService struct {
	...
	authLookupSlots           chan struct{REDACTED
...
func (s *APIKeyService) AuthLookupMetrics() APIKeyAuthLookupMetrics {
	if s == nil {
		return APIKeyAuthLookupMetrics{REDACTED
REDACTED
...
REDACTED
	svc.authLookupSlots = make(chan struct{REDACTED, lookupConcurrency)
...
func (s *APIKeyService) currentConcurrencyForAPIKey(ctx context.Context, apiKeyID int64) int {
	if s == nil || s.concurrencyService == nil || apiKeyID <= 0 {
		return 0
REDACTED
	counts, err := s.concurrencyService.GetAPIKeyConcurrencyBatch(ctx, []int64{apiKeyIDREDACTED)
	if err != nil {
		return 0
REDACTED
...
REDACTED
	if req.ClearExpiration {
		...
		if apiKey.Status == StatusAPIKeyExpired {
			apiKey.Status = StatusActive
	REDACTED
REDACTED else if req.ExpiresAt != nil {
...
func (s *APIKeyService) GetUserAllowedGroupIDSet(ctx context.Context, userID int64) (map[int64]struct{REDACTED, error) {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("get user: %w", err)
REDACTED
	allowed := make(map[int64]struct{REDACTED, len(user.AllowedGroups))
	for _, id := range user.AllowedGroups {
		allowed[id] = struct{REDACTED{REDACTED
REDACTED
	return allowed, nil
REDACTED
...
REDACTED
	...
	if apiKey.Quota > 0 && newQuotaUsed >= apiKey.Quota {
		...
		if err := s.apiKeyRepo.Update(ctx, apiKey, APIKeyUpdateFields{Status: trueREDACTED); err != nil {
