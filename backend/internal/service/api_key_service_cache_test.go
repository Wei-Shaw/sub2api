...
REDACTEDREDACTED
	repo := &authRepoStub{getByKeyForAuth: func(context.Context, string) (*APIKey, error) {
		...
		return nil, nil
REDACTEDREDACTED
	svc := NewAPIKeyService(repo, nil, nil, nil, nil, cache, &config.Config{APIKeyAuth: config.APIKeyAuthCacheConfig{L2TTLSeconds: 60REDACTEDREDACTED)

	for _, key := range []string{"", strings.Repeat("x", MaxAPIKeyCredentialBytes+1)REDACTED {
...
func TestAPIKeyService_GetByKeyAllowsMaximumLength(t *testing.T) {
	...
	repo := &authRepoStub{getByKeyForAuth: func(_ context.Context, got string) (*APIKey, error) {
		...
		return nil, ErrAPIKeyNotFound
REDACTEDREDACTED
	svc := NewAPIKeyService(repo, nil, nil, nil, nil, nil, &config.Config{REDACTED)
	_, err := svc.GetByKey(context.Background(), key)
	require.ErrorIs(t, err, ErrAPIKeyNotFound)
	require.Equal(t, int32(1), repoCalls.Load())
REDACTED
...
func TestAPIKeyService_AuthLookupBulkheadRejectsExcessMisses(t *testing.T) {
	...
	repo := &authRepoStub{getByKeyForAuth: func(context.Context, string) (*APIKey, error) {
		...
		return nil, ErrAPIKeyNotFound
REDACTEDREDACTED
	svc := NewAPIKeyService(repo, nil, nil, nil, nil, nil, &config.Config{APIKeyAuth: config.APIKeyAuthCacheConfig{LookupConcurrency: 1REDACTEDREDACTED)
