...
func TestAPIKeyService_TouchLastUsed_FirstTouchSucceeds(t *testing.T) {
	repo := &apiKeyRepoStub{REDACTED
	svc := &APIKeyService{apiKeyRepo: repoREDACTED

	err := svc.TouchLastUsed(context.Background(), 123)
REDACTED
	require.Equal(t, []int64{123REDACTED, repo.touchedIDs)
...
func TestAPIKeyService_TouchLastUsed_DebouncedWithinWindow(t *testing.T) {
	...
	require.Equal(t, []int64{123REDACTED, repo.touchedIDs, "second touch within debounce window should not hit repository")
REDACTED
...
REDACTED
	...
	require.Equal(t, []int64{456REDACTED, repo.touchedIDs, "debounced retry should not hit repository again")
REDACTED
...
func TestAPIKeyService_TouchLastUsed_ConcurrentFirstTouchDeduplicated(t *testing.T) {
	repo := &touchSingleflightRepo{
		apiKeyRepoStub: &apiKeyRepoStub{REDACTED,
		blockCh:        make(chan struct{REDACTED),
REDACTED
	svc := &APIKeyService{apiKeyRepo: repoREDACTED

	const workers = 20
	startCh := make(chan struct{REDACTED)
