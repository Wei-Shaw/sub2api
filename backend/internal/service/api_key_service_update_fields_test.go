...
func newUpdateFieldsAPIKeyService(key *APIKey) (*APIKeyService, *updateFieldsAPIKeyRepoStub) {
	repo := &updateFieldsAPIKeyRepoStub{key: keyREDACTED
	return &APIKeyService{apiKeyRepo: repoREDACTED, repo
REDACTED
...
func TestAPIKeyUpdate_OnlyDeclaresRequestedColumns(t *testing.T) {
	...
	quota := 500.0
	rateLimit := 42.0
	whitelist := []string{"10.0.0.1"REDACTED
...
REDACTED{
		{
			name: "name only",
			req:  UpdateAPIKeyRequest{Name: &nameREDACTED,
			want: APIKeyUpdateFields{Name: trueREDACTED,
	REDACTED,
		{
			name: "quota only",
			req:  UpdateAPIKeyRequest{Quota: &quotaREDACTED,
			want: APIKeyUpdateFields{Quota: trueREDACTED,
	REDACTED,
		{
			name: "rate limit threshold only",
			req:  UpdateAPIKeyRequest{RateLimit5h: &rateLimitREDACTED,
			want: APIKeyUpdateFields{RateLimits: trueREDACTED,
	REDACTED,
		{
			name: "ip whitelist only",
			req:  UpdateAPIKeyRequest{IPWhitelist: &whitelistREDACTED,
			want: APIKeyUpdateFields{IPRules: trueREDACTED,
	REDACTED,
REDACTED
	...
	for _, tt := range tests {
		...
		REDACTED
			require.Equal(t, []APIKeyUpdateFields{tt.wantREDACTED, repo.updateFields)
	REDACTED)
REDACTED
REDACTED
...
REDACTED)
REDACTED
	require.Equal(t, []APIKeyUpdateFields{{QuotaUsed: true, RateLimitUsage: trueREDACTEDREDACTED, repo.updateFields)
REDACTED
...
REDACTED)
	...
	_, err := svc.Update(context.Background(), 1, 7, UpdateAPIKeyRequest{Quota: &quotaREDACTED)
REDACTED
	require.Equal(t, []APIKeyUpdateFields{{Quota: true, Status: trueREDACTEDREDACTED, repo.updateFields)
REDACTED
...
REDACTEDREDACTED
	svc := &APIKeyService{apiKeyRepo: repoREDACTED

	require.NoError(t, svc.UpdateQuotaUsed(context.Background(), 1, 5))
	require.Equal(t, []APIKeyUpdateFields{{Status: trueREDACTEDREDACTED, repo.updateFields)
REDACTED
