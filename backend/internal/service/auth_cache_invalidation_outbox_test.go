...
func (*authInvalidationCacheStub) IncrementCreateAttemptCount(context.Context, int64) error {
	return nil
REDACTED
func (*authInvalidationCacheStub) DeleteCreateAttemptCount(context.Context, int64) error { return nil REDACTED
func (*authInvalidationCacheStub) IncrementDailyUsage(context.Context, string) error     { return nil REDACTED
...
func TestAuthCacheInvalidationWorker_FirstPassSchedulesSafetyPass(t *testing.T) {
	repo := &authInvalidationRepoStub{REDACTED
	cache := &authInvalidationCacheStub{REDACTED
	worker := NewAuthCacheInvalidationWorker(repo, cache)
	worker.processEvent(context.Background(), AuthCacheInvalidationEvent{ID: 7, CacheKey: "hash", Stage: 0REDACTED)
	require.Equal(t, []string{"hash"REDACTED, cache.deleted)
	require.Equal(t, []string{"hash"REDACTED, cache.published)
	require.Equal(t, []int64{7REDACTED, repo.scheduled)
...
func TestAuthCacheInvalidationWorker_SecondPassCleansEvent(t *testing.T) {
	...
	cache := &authInvalidationCacheStub{REDACTED
	worker := NewAuthCacheInvalidationWorker(repo, cache)
	worker.processEvent(context.Background(), AuthCacheInvalidationEvent{ID: 8, CacheKey: "hash", Stage: 1REDACTED)
	require.Equal(t, []int64{8REDACTED, repo.deleted)
...
func TestAuthCacheInvalidationWorker_RetriesRedisAndPublishFailures(t *testing.T) {
	for _, tc := range []struct {
		...
		published  int
REDACTED{
		{name: "redis down", deleteErr: errors.New("redis unavailable")REDACTED,
		{name: "publish failure after delete", publishErr: errors.New("publish failed"), published: 1REDACTED,
REDACTED {
		t.Run(tc.name, func(t *testing.T) {
			repo := &authInvalidationRepoStub{REDACTED
			cache := &authInvalidationCacheStub{
				deleteFn:  func(context.Context, string) error { return tc.deleteErr REDACTED,
				publishFn: func(context.Context, string) error { return tc.publishErr REDACTED,
		REDACTED
			worker := NewAuthCacheInvalidationWorker(repo, cache)
			worker.processEvent(context.Background(), AuthCacheInvalidationEvent{ID: 9, CacheKey: "hash"REDACTED)
			require.Equal(t, []int64{9REDACTED, repo.retried)
...
REDACTEDREDACTED
	worker := NewAuthCacheInvalidationWorker(repo, cache)
	started := time.Now()
	worker.processEvent(context.Background(), AuthCacheInvalidationEvent{ID: 10, CacheKey: "hash"REDACTED)
	require.Less(t, time.Since(started), 3*time.Second)
	require.Equal(t, []int64{10REDACTED, repo.retried)
...
REDACTEDREDACTED
	worker := NewAuthCacheInvalidationWorker(repo, &authInvalidationCacheStub{REDACTED)
...
func TestAuthCacheInvalidationWorker_ProcessesClaimedBatchConcurrently(t *testing.T) {
	...
	for i := range events {
		events[i] = AuthCacheInvalidationEvent{ID: int64(i + 1), CacheKey: "hash", Stage: 1REDACTED
REDACTED
	repo := &authInvalidationRepoStub{events: eventsREDACTED
...
func TestAuthCacheInvalidationWorker_LifecycleIsManagedAndIdempotent(t *testing.T) {
	worker := NewAuthCacheInvalidationWorker(&authInvalidationRepoStub{REDACTED, &authInvalidationCacheStub{REDACTED)
	worker.Start()
	require.Eventually(t, func() bool { return worker.Health(context.Background()).Running REDACTED, time.Second, 10*time.Millisecond)
	require.NotPanics(t, func() { worker.Stop(); worker.Stop() REDACTED)
...
func TestAuthCacheInvalidationSubscriber_RetriesInitialFailureAndStops(t *testing.T) {
	ready := make(chan struct{REDACTED)
	var calls int
...
REDACTEDREDACTED
	...
	localCache, err := ristretto.NewCache(&ristretto.Config{NumCounters: 10, MaxCost: 1, BufferItems: 64REDACTED)
REDACTED
...
REDACTED
	require.Eventually(t, func() bool { return svc.AuthCacheInvalidationSubscriberHealth().Connected REDACTED, time.Second, 10*time.Millisecond)
	require.Equal(t, uint64(1), svc.AuthCacheInvalidationSubscriberHealth().Failures)
	require.NotPanics(t, func() { svc.StopAuthCacheInvalidationSubscriber(); svc.StopAuthCacheInvalidationSubscriber() REDACTED)
REDACTED
