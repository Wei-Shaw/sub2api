...
CREATE TABLE IF NOT EXISTS user_provider_default_grants (
	...
	if emailCache != nil {
		...
		emailQueue = service.NewEmailQueueService(emailSvc, 1)
		t.Cleanup(func() { emailQueue.Stop() })
	}

	svc := service.NewAuthService(client, repo, nil, refreshTokenCache, cfg, settingSvc, emailSvc, nil, emailQueue, nil, defaultSubAssigner, nil, nil)
