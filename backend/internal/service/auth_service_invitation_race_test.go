...
func newRaceSafeUserRepo() *raceSafeUserRepo {
	return &raceSafeUserRepo{nextID: 1, byEmail: map[string]*User{REDACTED, byID: map[int64]*User{REDACTEDREDACTED
REDACTED
...
func TestAuthService_Register_InvitationCodeSingleUseUnderConcurrency(t *testing.T) {
	...
	redeemRepo := &raceSafeRedeemRepo{codes: map[string]*RedeemCode{
		code: {ID: 1, Code: code, Type: RedeemTypeInvitation, Status: StatusUnusedREDACTED,
REDACTEDREDACTED
...
REDACTED
	svc := newOAuthEmailFlowAuthService(
		...
		&userPlatformQuotaRepoStub{REDACTED,
	)
...
func TestAuthService_Register_InvitationCodeRejectedWhenAlreadyUsed(t *testing.T) {
	...
	userRepo := newRaceSafeUserRepo()
	redeemRepo := &raceSafeRedeemRepo{codes: map[string]*RedeemCode{
		code: {ID: 2, Code: code, Type: RedeemTypeInvitation, Status: StatusUsedREDACTED,
REDACTEDREDACTED
	settings := map[string]string{
		"registration_enabled":    "true",
		"invitation_code_enabled": "true",
...
func TestAuthService_Register_InvitationCodeMissingWhenEnabled(t *testing.T) {
	userRepo := newRaceSafeUserRepo()
	redeemRepo := &raceSafeRedeemRepo{codes: map[string]*RedeemCode{REDACTEDREDACTED
	settings := map[string]string{
		"registration_enabled":    "true",
		"invitation_code_enabled": "true",
REDACTED
