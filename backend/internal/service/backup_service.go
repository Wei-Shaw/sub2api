...
REDACTED
	if cfg == nil {
		return &BackupS3Config{REDACTED, nil
REDACTED
	// 脱敏返回
	cfg.SecretAccessKey = ""
	return cfg, nil
...
func (s *BackupService) GetSchedule(ctx context.Context) (*BackupScheduleConfig, error) {
	...
	if err != nil || raw == "" {
		return &BackupScheduleConfig{REDACTED, nil
REDACTED
...
REDACTED
	...
	if err != nil {
		...
		return record, err
REDACTED
	defer func() { _ = cleanupBackupFiles(archivePath) REDACTED()
...
REDACTED
	defer func() { _ = gzReader.Close() REDACTED()
...
REDACTED
	...
	sort.Slice(ordered, func(i, j int) bool { return ordered[i].Index < ordered[j].Index REDACTED)
...
REDACTED
	...
	if len(record.Parts) > 0 {
		...
		sort.Slice(parts, func(i, j int) bool { return parts[i].Index < parts[j].Index REDACTED)
...
REDACTED
	...
	seen := make(map[string]struct{REDACTED, len(record.Parts)+1)
