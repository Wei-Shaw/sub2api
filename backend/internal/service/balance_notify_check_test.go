...
func TestCheckBalanceAfterDeduction_UserNotifyDisabled(t *testing.T) {
	...
	u := &User{ID: 1, BalanceNotifyEnabled: falseREDACTED
...
func TestCheckBalanceAfterDeduction_GlobalDisabled(t *testing.T) {
	...
	u := &User{ID: 1, BalanceNotifyEnabled: trueREDACTED
	s.CheckBalanceAfterDeduction(context.Background(), u, 20, 15)
REDACTED
...
func TestCheckAccountQuotaAfterIncrement_ZeroCost(t *testing.T) {
	s, _ := newBalanceNotifyServiceForTest()
	a := &Account{ID: 1, Platform: PlatformAnthropic, Type: AccountTypeAPIKeyREDACTED
...
func TestCheckQuotaDimCrossings_NoDimensions(t *testing.T) {
	s, _ := newBalanceNotifyServiceForTest()
	account := &Account{ID: 1, Name: "test", Platform: PlatformAnthropicREDACTED
	// Empty dims → no crossing, no panic.
	s.checkQuotaDimCrossings(account, nil, 10, []string{"admin@example.com"REDACTED, "TestSite")
	s.checkQuotaDimCrossings(account, []quotaDim{REDACTED, 10, []string{"admin@example.com"REDACTED, "TestSite")
REDACTED
...
REDACTED
	...
	s.checkQuotaDimCrossings(account, dims, 50, []string{"admin@example.com"REDACTED, "TestSite")
REDACTED
