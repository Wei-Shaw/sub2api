...
func TestResolvedThreshold_FixedNormal(t *testing.T) {
	...
	d := quotaDim{threshold: 400, thresholdType: thresholdTypeFixed, limit: 1000REDACTED
...
func TestResolvedThreshold_FixedThresholdExceedsLimit(t *testing.T) {
	...
	d := quotaDim{threshold: 1200, thresholdType: thresholdTypeFixed, limit: 1000REDACTED
...
func TestResolvedThreshold_FixedThresholdEqualsLimit(t *testing.T) {
	...
	d := quotaDim{threshold: 1000, thresholdType: thresholdTypeFixed, limit: 1000REDACTED
...
func TestResolvedThreshold_PercentageNormal(t *testing.T) {
	...
	d := quotaDim{threshold: 30, thresholdType: thresholdTypePercentage, limit: 1000REDACTED
...
func TestResolvedThreshold_PercentageZeroPercent(t *testing.T) {
	...
	d := quotaDim{threshold: 0, thresholdType: thresholdTypePercentage, limit: 1000REDACTED
...
func TestResolvedThreshold_PercentageHundredPercent(t *testing.T) {
	...
	d := quotaDim{threshold: 100, thresholdType: thresholdTypePercentage, limit: 1000REDACTED
...
func TestResolvedThreshold_PercentageOverHundred(t *testing.T) {
	...
	d := quotaDim{threshold: 150, thresholdType: thresholdTypePercentage, limit: 1000REDACTED
...
func TestResolvedThreshold_ZeroLimit(t *testing.T) {
	...
	d := quotaDim{threshold: 100, thresholdType: thresholdTypeFixed, limit: 0REDACTED
	require.Equal(t, 0.0, d.resolvedThreshold())
REDACTED

func TestResolvedThreshold_NegativeLimit(t *testing.T) {
	// Negative limit treated as 0
	d := quotaDim{threshold: 100, thresholdType: thresholdTypeFixed, limit: -10REDACTED
	require.Equal(t, 0.0, d.resolvedThreshold())
REDACTED
...
func TestCollectBalanceNotifyRecipients_Empty(t *testing.T) {
	...
	u := &User{BalanceNotifyExtraEmails: nilREDACTED
...
func TestCollectBalanceNotifyRecipients_FiltersDisabledAndUnverified(t *testing.T) {
	...
	u := &User{
		BalanceNotifyExtraEmails: []NotifyEmailEntry{
			...
			{Email: "b@example.com", Verified: true, Disabled: trueREDACTED,   // disabled
			{Email: "c@example.com", Verified: false, Disabled: falseREDACTED, // unverified
			{Email: "d@example.com", Verified: true, Disabled: falseREDACTED,
	REDACTED,
REDACTED
	got := s.collectBalanceNotifyRecipients(u)
	require.Equal(t, []string{"a@example.com", "d@example.com"REDACTED, got)
REDACTED
...
func TestCollectBalanceNotifyRecipients_DeduplicatesCaseInsensitive(t *testing.T) {
	...
	u := &User{
		BalanceNotifyExtraEmails: []NotifyEmailEntry{
			{Email: "User@Example.com", Verified: trueREDACTED,
			{Email: "user@example.com", Verified: trueREDACTED,
			{Email: "USER@EXAMPLE.COM", Verified: trueREDACTED,
	REDACTED,
REDACTED
...
func TestCollectBalanceNotifyRecipients_SkipsEmpty(t *testing.T) {
	...
	u := &User{
		BalanceNotifyExtraEmails: []NotifyEmailEntry{
			{Email: "  ", Verified: trueREDACTED,
			{Email: "", Verified: trueREDACTED,
			{Email: "valid@example.com", Verified: trueREDACTED,
	REDACTED,
REDACTED
	got := s.collectBalanceNotifyRecipients(u)
	require.Equal(t, []string{"valid@example.com"REDACTED, got)
REDACTED
...
func TestCollectBalanceNotifyRecipients_TrimsWhitespace(t *testing.T) {
	...
	u := &User{
		BalanceNotifyExtraEmails: []NotifyEmailEntry{
			{Email: "  trimmed@example.com  ", Verified: trueREDACTED,
	REDACTED,
REDACTED
	got := s.collectBalanceNotifyRecipients(u)
	require.Equal(t, []string{"trimmed@example.com"REDACTED, got)
REDACTED
