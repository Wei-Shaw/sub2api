...
REDACTED)
	...
	t.Run("non owner returns not found", func(t *testing.T) {
		...
		_, err := svc.DeleteOutputsForOwner(ctx, BatchImageOwner{UserID: 11, APIKeyID: 999REDACTED, "imgbatch_cleanup")
...
func TestBatchImageCleanupService_InputOutputAndWorker(t *testing.T) {
	...
	REDACTED
		require.Equal(t, []CleanupTarget{CleanupTargetInputREDACTED, provider.cleanupTargets)
...
func TestBatchImageSettlementOutputExpiration(t *testing.T) {
	...
	svc := &BatchImageSettlementService{
		...
		Config:      &config.Config{BatchImage: config.BatchImageConfig{OutputRetentionAfterTerminalHours: 5REDACTEDREDACTED,
REDACTED
...
func TestBatchImageDownloadAfterOutputDeletedReturnsGone(t *testing.T) {
	...
	result, err := svc.StreamZip(context.Background(), testBatchImageOwner(), "imgbatch_download", BatchImageZipOptions{REDACTED, &out)
...
func newTestBatchImageCleanupService() (*BatchImageCleanupService, *fakeBatchImageRepository, *publicBatchImageProvider) {
	...
	provider := &publicBatchImageProvider{name: BatchImageProviderGeminiAPIREDACTED
	accountID := int64(101)
	svc := &BatchImageCleanupService{
		...
		Config:           &config.Config{BatchImage: config.BatchImageConfig{CleanupBatchSize: 10, InputRetentionAfterTerminalHours: 24REDACTEDREDACTED,
REDACTED
