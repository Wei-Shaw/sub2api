...
var bedrockCrossRegionPrefixes = []string{"us.", "eu.", "apac.", "jp.", "au.", "us-gov.", "global."REDACTED
...
//  4. 移除工具定义中的 custom 字段（Claude Code 会发送 custom: {defer_loading: trueREDACTED）
...
REDACTED
	...
	// 移除工具定义中的 custom 字段
	// Claude Code (v2.1.69+) 在 tool 定义中发送 custom: {defer_loading: trueREDACTED，
...
REDACTED else if content.Type == gjson.String {
		// content 是纯字符串，转换为数组格式
		originalText := content.String()
		body, _ = sjson.SetBytes(body, basePath, []map[string]string{
			{"type": "text", "text": originalTextREDACTED,
			{"type": "text", "text": string(schemaJSON)REDACTED,
	REDACTED)
REDACTED
...
// 支持 claude-{tierREDACTED-{majorREDACTED-{minorREDACTED 和 claude-{tierREDACTED-{majorREDACTED.{minorREDACTED 格式
