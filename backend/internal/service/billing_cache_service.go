...
REDACTED
	...
	REDACTED else if s.userGroupRateRepo != nil {
	...
	REDACTED else if group.RPMLimit > 0 {
			...
			if err != nil {
				...
				// fail-open
		REDACTED else if count > group.RPMLimit {
				return ErrGroupRPMExceeded
		REDACTED
	REDACTED
REDACTED
...
REDACTED else if previousFailures > 0 {
...
// 返回 nil = 允许；返回 ErrUserPlatform{Daily/Weekly/MonthlyREDACTEDQuotaExhausted = 拒绝（带 window_resets_at metadata）。
