...
REDACTED
	...
	// doubao-embedding-vision 图文向量化：上游 usage 回传 prompt_tokens_details.{text_tokens,image_tokensREDACTED，
...
func isGrokMediaFamilyModel(native string) bool {
	for _, marker := range []string{"imagine", "image", "video", "audio", "speech", "tts", "transcribe", "realtime"REDACTED {
...
REDACTED
	...
	if fallback != nil {
		...
		if _, seen := s.fallbackWarnSeen.LoadOrStore(model, struct{REDACTED{REDACTED); !seen {
