...
type CostBreakdown struct {
	...
	BillingTier               *string // 实际命中的显式定价档位；nil 表示未命中区间
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	pricing, billingTier := input.Resolver.GetIntervalPricingWithTier(resolved, totalContext)
	if pricing == nil {
	...
	breakdown.BillingTier = billingTier
