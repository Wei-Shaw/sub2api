...
func TestGetModelPricing_OpenAICompactAliasesFallback(t *testing.T) {
	...
	tests := []struct {
		...
		longContext int
REDACTED{
		{model: "gpt5.5", inputPrice: 5e-6, outputPrice: 30e-6, cacheRead: 0.5e-6, longContext: 272000REDACTED,
		{model: "openai/gpt5.4", inputPrice: 2.5e-6, outputPrice: 15e-6, cacheRead: 0.25e-6, longContext: 272000REDACTED,
		{model: "gpt5.4-mini", inputPrice: 7.5e-7, outputPrice: 4.5e-6, cacheRead: 7.5e-8, longContext: 0REDACTED,
		{model: "gpt5.3codexspark", inputPrice: 1.5e-6, outputPrice: 12e-6, cacheRead: 0.15e-6, longContext: 0REDACTED,
REDACTED
...
func TestCalculateCost_OpenAIGPT54LongContextMarkerRequiresActualCostIncrease(t *testing.T) {
	...
	cost, err := svc.calculateCostWithServiceTierPolicy(
		"gpt-5.4-2026-03-05",
		UsageTokens{InputTokens: 300000REDACTED,
		0,
		"",
		true,
	)
...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	floatPtr := func(v float64) *float64 { return &v REDACTED
...
REDACTED{
		{name: "empty model", model: "   ", expectNilPricing: trueREDACTED,
		{name: "claude opus 4.6", model: "claude-opus-4.6-20260201", expectedInput: 5e-6REDACTED,
		{name: "claude opus 4.5 alt separator", model: "claude-opus-4-5-20260101", expectedInput: 5e-6REDACTED,
		{name: "claude generic model fallback sonnet", model: "claude-foo-bar", expectedInput: 3e-6REDACTED,
		{name: "gemini explicit fallback", model: "gemini-3-1-pro", expectedInput: 2e-6REDACTED,
		{name: "gemini unknown no fallback", model: "gemini-2.0-pro", expectNilPricing: trueREDACTED,
		{name: "openai gpt5.4", model: "gpt-5.4", expectedInput: 2.5e-6REDACTED,
		{name: "openai gpt5.4 mini", model: "gpt-5.4-mini", expectedInput: 7.5e-7REDACTED,
		{name: "openai gpt5.3 codex", model: "gpt-5.3-codex", expectedInput: 1.5e-6REDACTED,
		{name: "openai gpt5.3 codex spark", model: "gpt-5.3-codex-spark", expectedInput: 1.5e-6REDACTED,
		{name: "openai legacy gpt5.1 falls back to gpt5.4", model: "gpt-5.1", expectedInput: 2.5e-6REDACTED,
		{name: "openai legacy gpt5.1 codex falls back to gpt5.3 codex", model: "gpt-5.1-codex", expectedInput: 1.5e-6REDACTED,
		{name: "openai legacy codex mini latest falls back to gpt5.3 codex", model: "codex-mini-latest", expectedInput: 1.5e-6REDACTED,
		{name: "openai unknown no fallback", model: "gpt-unknown-model", expectNilPricing: trueREDACTED,
		{
	...
	REDACTED,
		...
		// ---- 负向用例 ----
		{name: "qwen unknown no fallback", model: "qwen-max", expectNilPricing: trueREDACTED,
		// doubao-pro / doubao-embedding（纯文本）不在白名单，不回退；仅 doubao-embedding-vision 显式命中。
		{name: "doubao unknown no fallback", model: "doubao-pro", expectNilPricing: trueREDACTED,
		{name: "doubao text embedding no fallback", model: "doubao-embedding-text-240515", expectNilPricing: trueREDACTED,
		{name: "hunyuan unknown no fallback", model: "hunyuan-t1", expectNilPricing: trueREDACTED,
		{name: "moonshot v1 not covered", model: "moonshot-v1-8k", expectNilPricing: trueREDACTED,
		// bare k3 仅精确/后缀匹配：相似未知型号不得因含 "k3" 误命中。
		{name: "k3-like unknown no fallback", model: "foo-k3-bar", expectNilPricing: trueREDACTED,
		// 路径最后一段不是 /k3：foo-k3 不得因 HasSuffix("/k3") 或 Contains 误命中。
		{name: "path segment not bare k3 no fallback", model: "vendor/foo-k3", expectNilPricing: trueREDACTED,
		// kimi-k3 非 Contains：kimi-k30 / 内嵌 foo-kimi-k3-bar 不得误命中。
		{name: "kimi-k30 unknown no fallback", model: "kimi-k30", expectNilPricing: trueREDACTED,
		{name: "embedded kimi-k3 unknown no fallback", model: "foo-kimi-k3-bar", expectNilPricing: trueREDACTED,
		// kimi-k3[1m] 是 Claude Code 上下文选择语法，不是 Kimi API 模型 ID，不命中 fallback。
		{name: "kimi-k3[1m] not an API model id no fallback", model: "kimi-k3[1m]", expectNilPricing: trueREDACTED,
		{name: "path kimi-k3[1m] not an API model id no fallback", model: "moonshot/kimi-k3[1m]", expectNilPricing: trueREDACTED,
...
func TestCalculateCost_DoubaoEmbeddingVisionDifferentialInput(t *testing.T) {
	...
	mixed := UsageTokens{InputTokens: 1340, ImageInputTokens: 28REDACTED
...
REDACTED
	...
	textOnly := UsageTokens{InputTokens: 1340REDACTED
...
REDACTED
	...
	weird := UsageTokens{InputTokens: 10, ImageInputTokens: 50REDACTED
...
func TestCalculateVideoCostUsesSeparateConfig(t *testing.T) {
	...
	imagePrice := 0.4
	videoPrice := 0.08
	imageCost := svc.CalculateImageCost("grok-imagine-video", "2K", 1, &ImagePriceConfig{Price2K: &imagePriceREDACTED, 1.0)
	videoCost := svc.CalculateVideoCost("grok-imagine-video", "480p", 1, 10, &VideoPriceConfig{Price480P: &videoPriceREDACTED, 0.5)
...
func TestGetModelPricing_Grok45OfficialFallback(t *testing.T) {
	...
	for _, model := range []string{"grok-4.5", "grok-4.5-latest"REDACTED {
		model := model
...
func TestGetModelPricing_GrokBareAliasesUseGrok46(t *testing.T) {
	svc := newTestBillingService()
	for _, model := range []string{"grok", "grok-latest"REDACTED {
		pricing, err := svc.GetModelPricing(model)
	REDACTED
		require.InDelta(t, 2e-6, pricing.InputPricePerToken, 1e-12)
		require.InDelta(t, 0.5e-6, pricing.CacheReadPricePerToken, 1e-12)
...
func TestGetModelPricing_Grok46OfficialFallback(t *testing.T) {
	...
	for _, model := range []string{"grok-4.6", "grok-4.6-latest"REDACTED {
		model := model
		t.Run(model, func(t *testing.T) {
			pricing, err := svc.GetModelPricing(model)
		REDACTED
...
REDACTED{
		{"grok-4.3", 1.25e-6, 0.2e-6, 2.5e-6REDACTED,
		{"grok-4.20-0309-reasoning", 1.25e-6, 0.2e-6, 2.5e-6REDACTED,
		{"grok-build-0.1", 1e-6, 0.2e-6, 2e-6REDACTED,
REDACTED {
...
func TestCalculateCostUnified_GroupLongContextToggleUsesPresetLadder(t *testing.T) {
	...
	tokens := UsageTokens{InputTokens: 250000, OutputTokens: 1000REDACTED

	off := &Group{LongContextPricingEnabled: falseREDACTED
	disabled, err := svc.CalculateCostUnified(CostInput{
		Model: "grok-4.5", Group: off, Tokens: tokens, RateMultiplier: 1, Resolver: resolver,
REDACTED)
REDACTED

	on := &Group{LongContextPricingEnabled: trueREDACTED
...
REDACTED
	...
	for _, model := range []string{"grok-5", "grok-5-latest", "x-ai/grok-7", "grok-4.7-beta"REDACTED {
		pricing, err := svc.GetModelPricing(model)
...
REDACTED
	...
	for _, model := range []string{"grok-2-image-1212", "grok-2-audio", "grok-5-video", "x-ai/grok-6-image"REDACTED {
...
func TestCalculateCostWithServiceTier_OpenAIPriorityUsesPriorityPricing(t *testing.T) {
	svc := newTestBillingService()
	tokens := UsageTokens{InputTokens: 100, OutputTokens: 50, CacheReadTokens: 20REDACTED
...
func TestCalculateCostWithServiceTier_FlexAppliesHalfMultiplier(t *testing.T) {
	svc := newTestBillingService()
	tokens := UsageTokens{InputTokens: 100, OutputTokens: 50, CacheCreationTokens: 40, CacheReadTokens: 20REDACTED
...
func TestCalculateCostWithServiceTier_Gpt54MiniPriorityFallsBackToTierMultiplier(t *testing.T) {
	svc := newTestBillingService()
	tokens := UsageTokens{InputTokens: 120, OutputTokens: 30, CacheCreationTokens: 12, CacheReadTokens: 8REDACTED
