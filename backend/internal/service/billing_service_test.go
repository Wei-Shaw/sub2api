...
func TestGetModelPricing_OpenAICompactAliasesFallback(t *testing.T) {
	...
	tests := []struct {
		...
		longContext int
REDACTED{
		{model: "gpt5.5", inputPrice: 2.5e-6, outputPrice: 15e-6, cacheRead: 0.25e-6, longContext: 272000REDACTED,
...
func TestGetModelPricing_Grok45OfficialFallback(t *testing.T) {
	...
	for _, model := range []string{"grok", "grok-latest", "grok-4.5", "grok-4.5-latest"REDACTED {
		model := model
