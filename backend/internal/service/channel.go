...
const (
	...
	BillingModeMixed      BillingMode = "mixed"       // 用量记录同时包含 Token 与图片按次费用
)
...
func (m BillingMode) IsValidUsageFilter() bool {
	switch m {
	case BillingModeToken, BillingModePerRequest, BillingModeImage, BillingModeVideo, BillingModeMixed, "":
		return true
	}
	return false
}
