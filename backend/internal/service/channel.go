...
type Channel struct {
	...
	// 渠道级模型映射（按平台分组：platform → {src→dstREDACTED）
...
REDACTED
	if p.TimePricing != nil {
		cp.TimePricing = &ChannelTimePricing{Timezone: p.TimePricing.TimezoneREDACTED
...
func validateIntervalPrices(iv *PricingInterval, idx int) error {
	prices := []struct {
		...
		val  *float64
REDACTED{
		{"input_price", iv.InputPriceREDACTED,
		{"output_price", iv.OutputPriceREDACTED,
		{"cache_write_price", iv.CacheWritePriceREDACTED,
		{"cache_read_price", iv.CacheReadPriceREDACTED,
		{"per_request_price", iv.PerRequestPriceREDACTED,
REDACTED
...
REDACTED
	multipliers := []struct {
		...
		val  *float64
REDACTED{
		{"input_multiplier", iv.InputMultiplierREDACTED,
		{"output_multiplier", iv.OutputMultiplierREDACTED,
		{"cache_write_multiplier", iv.CacheWriteMultiplierREDACTED,
		{"cache_read_multiplier", iv.CacheReadMultiplierREDACTED,
REDACTED
...
REDACTED
	...
	type dedupKey struct {
		...
		name     string
REDACTED
	seen := make(map[dedupKey]struct{REDACTED)
...
REDACTED
	...
	add := func(platform, displayName string, pricing *ChannelModelPricing) {
		key := dedupKey{platform: platform, name: strings.ToLower(displayName)REDACTED
		if _, ok := seen[key]; ok {
			return
	REDACTED
