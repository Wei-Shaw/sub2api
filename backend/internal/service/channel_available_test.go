...
func (s *stubGroupRepoForAvailable) Create(ctx context.Context, group *Group) error { return nil REDACTED
...
func (s *stubGroupRepoForAvailable) GetByIDLite(ctx context.Context, id int64) (*Group, error) {
	return nil, nil
REDACTED
func (s *stubGroupRepoForAvailable) Update(ctx context.Context, group *Group) error { return nil REDACTED
func (s *stubGroupRepoForAvailable) Delete(ctx context.Context, id int64) error     { return nil REDACTED
...
REDACTEDREDACTED
	svc := newAvailableChannelService(channels, &stubGroupRepoForAvailable{REDACTED)
...
func TestListAvailable_InactiveGroupIDSilentlyDropped(t *testing.T) {
	...
	channels := []Channel{{
		...
		Name:     "chA",
		Status:   StatusActive,
		GroupIDs: []int64{1, 99REDACTED,
REDACTEDREDACTED
	groupRepo := &stubGroupRepoForAvailable{
		activeGroups: []Group{{ID: 1, Name: "g1", Platform: "anthropic"REDACTEDREDACTED,
REDACTED
...
func TestListAvailable_SortedByName(t *testing.T) {
	channels := []Channel{
		{ID: 1, Name: "beta"REDACTED,
		{ID: 2, Name: "Alpha"REDACTED,
		{ID: 3, Name: "charlie"REDACTED,
REDACTED
	svc := newAvailableChannelService(channels, &stubGroupRepoForAvailable{REDACTED)
	out, err := svc.ListAvailable(context.Background())
REDACTED
...
REDACTED
	groupRepo := &stubGroupRepoForAvailable{REDACTED
...
func TestListAvailable_ListActiveErrorPropagates(t *testing.T) {
	...
	svc := newAvailableChannelService(
		[]Channel{{ID: 1, Name: "chA"REDACTEDREDACTED,
...
func TestListAvailable_DefaultsEmptyBillingModelSource(t *testing.T) {
	...
	channels := []Channel{
		{ID: 1, Name: "empty", BillingModelSource: ""REDACTED,
		{ID: 2, Name: "explicit", BillingModelSource: BillingModelSourceUpstreamREDACTED,
REDACTED
	svc := newAvailableChannelService(channels, &stubGroupRepoForAvailable{REDACTED)
	out, err := svc.ListAvailable(context.Background())
REDACTED
...
func TestPricingNeedsFallback(t *testing.T) {
	tests := []struct {
		...
		want bool
REDACTED{
		{"nil", nil, trueREDACTED,
		{"empty struct", &ChannelModelPricing{BillingMode: BillingModeTokenREDACTED, trueREDACTED,
		{"all-empty intervals", &ChannelModelPricing{
			BillingMode: BillingModeImage,
			Intervals:   []PricingInterval{{TierLabel: "1K"REDACTED, {TierLabel: "2K"REDACTEDREDACTED,
	REDACTED, trueREDACTED,
		{"flat input set", &ChannelModelPricing{InputPrice: testPtrFloat64(3e-6)REDACTED, falseREDACTED,
		{"flat per_request set", &ChannelModelPricing{PerRequestPrice: testPtrFloat64(0.04)REDACTED, falseREDACTED,
		{"interval with price", &ChannelModelPricing{
			Intervals: []PricingInterval{{TierLabel: "1K", PerRequestPrice: testPtrFloat64(0.04)REDACTEDREDACTED,
	REDACTED, falseREDACTED,
REDACTED
...
REDACTED
	existing := &ChannelModelPricing{BillingMode: BillingModePerRequestREDACTED
...
func TestFillGlobalPricingFallback_NilPricing(t *testing.T) {
	pricingSvc := newStubPricingServiceFromMap(map[string]*LiteLLMModelPricing{
		"claude-opus-4-5": {Mode: "chat", InputCostPerToken: 5e-6REDACTED,
REDACTED)
	svc := &ChannelService{pricingService: pricingSvcREDACTED

	models := []SupportedModel{
		{Name: "claude-opus-4-5", Platform: "anthropic"REDACTED,
REDACTED
...
func TestFillGlobalPricingFallback_KeepsExistingPrice(t *testing.T) {
	// 渠道已经填了价格的条目不应被回落覆盖。
	pricingSvc := newStubPricingServiceFromMap(map[string]*LiteLLMModelPricing{
		"served-model": {Mode: "chat", InputCostPerToken: 1e-6REDACTED,
REDACTED)
	svc := &ChannelService{pricingService: pricingSvcREDACTED
...
REDACTED
	models := []SupportedModel{
		{Name: "served-model", Platform: "anthropic", Pricing: existingREDACTED,
REDACTED
	fillGlobalPricingFallback(svc.pricingService, models)
...
func newStubPricingServiceFromMap(data map[string]*LiteLLMModelPricing) *PricingService {
	return &PricingService{pricingData: dataREDACTED
REDACTED
