...
REDACTED
	...
	if err != nil {
		...
		latestMap = map[int64][]*ChannelMonitorLatest{REDACTED
REDACTED
	...
	if err != nil {
		...
		availMap = map[int64][]*ChannelMonitorAvailability{REDACTED
REDACTED
...
REDACTED
	if len(monitors) == 0 {
		return []*UserMonitorView{REDACTED, nil
REDACTED
...
func (s *ChannelMonitorService) batchLatest(ctx context.Context, ids []int64) map[int64][]*ChannelMonitorLatest {
	...
	if err != nil {
		...
		return map[int64][]*ChannelMonitorLatest{REDACTED
REDACTED
	return latestMap
REDACTED
...
func (s *ChannelMonitorService) batchTimeline(
	...
	if err != nil {
		...
		return map[int64][]*ChannelMonitorHistoryEntry{REDACTED
REDACTED
	return timelineMap
REDACTED
...
func (s *ChannelMonitorService) collectAvailabilityWindows(ctx context.Context, monitorID int64) (map[int]map[string]*ChannelMonitorAvailability, error) {
	...
	windows := []int{monitorAvailability7Days, monitorAvailability15Days, monitorAvailability30DaysREDACTED
...
func buildStatusSummary(
	...
	summary := MonitorStatusSummary{ExtraModels: make([]ExtraModelStatus, 0, len(extras))REDACTED
	if primary != "" {
...
REDACTED
	for _, model := range extras {
		entry := ExtraModelStatus{Model: modelREDACTED
...
func mergeModelDetails(
	...
	all := append([]string{m.PrimaryModelREDACTED, m.ExtraModels...)
	latestByModel := indexLatestByModel(latest)
	out := make([]ModelDetail, 0, len(all))
	for _, model := range all {
		d := ModelDetail{Model: modelREDACTED
		if l, ok := latestByModel[model]; ok {
