...
REDACTED
	return &http.Client{Timeout: timeout, Transport: servertiming.WrapRoundTripper(tr)REDACTED
REDACTED
...
REDACTED
	if statusCode < 200 || statusCode >= 300 {
		...
		// 会丢掉真正的上游错误信息，例如 `{"error":{"message":"No available accounts ..."REDACTEDREDACTED`）。
		res.Status = MonitorStatusError
...
var providerAdapters = map[string]providerAdapter{
	...
	MonitorProviderAnthropic: {
		buildPath: func(string) string { return providerAnthropicPath REDACTED,
		buildBody: func(model, prompt string) ([]byte, error) {
			return json.Marshal(map[string]any{
				"model":      model,
				"messages":   []map[string]string{{"role": "user", "content": promptREDACTEDREDACTED,
...
REDACTED,
	MonitorProviderGemini: {
		// Gemini 把 model 名写在 URL path 上：/v1beta/models/{modelREDACTED:generateContent
		buildPath: func(model string) string { return fmt.Sprintf(providerGeminiPathTemplate, model) REDACTED,
		buildBody: func(_, prompt string) ([]byte, error) {
			return json.Marshal(map[string]any{
				"contents": []map[string]any{
					{"parts": []map[string]any{{"text": promptREDACTEDREDACTEDREDACTED,
			REDACTED,
				"generationConfig": map[string]any{"maxOutputTokens": monitorChallengeMaxTokensREDACTED,
		REDACTED)
	REDACTED,
		// 使用 x-goog-api-key header 而不是 ?key= query，避免 *url.Error 把 key 回填到错误日志。
		buildHeaders: func(apiKey string) map[string]string {
			return map[string]string{"x-goog-api-key": apiKeyREDACTED
	REDACTED,
...
func newOpenAICompatibleChatAdapter(path string) providerAdapter {
	return providerAdapter{
		buildPath: func(string) string { return path REDACTED,
		buildBody: func(model, prompt string) ([]byte, error) {
			return json.Marshal(map[string]any{
				"model":      model,
				"messages":   []map[string]string{{"role": "user", "content": promptREDACTEDREDACTED,
	...
	REDACTED,
		buildHeaders: func(apiKey string) map[string]string {
			return map[string]string{"Authorization": "Bearer " + apiKeyREDACTED
	REDACTED,
...
var providerOpenAIResponsesAdapter = providerAdapter{
	buildPath: func(string) string { return providerOpenAIResponsesPath REDACTED,
	buildBody: func(model, prompt string) ([]byte, error) {
		return json.Marshal(map[string]any{
			"model":             model,
...
var bodyMergeKeyDenyList = map[string]map[string]bool{
	MonitorProviderOpenAI + ":" + MonitorAPIModeChatCompletions: {"model": true, "messages": true, "stream": trueREDACTED,
	MonitorProviderOpenAI + ":" + MonitorAPIModeResponses:       {"model": true, "instructions": true, "input": true, "stream": trueREDACTED,
	MonitorProviderGrok:      {"model": true, "messages": true, "stream": trueREDACTED,
	MonitorProviderAnthropic: {"model": true, "messages": trueREDACTED,
	MonitorProviderGemini:    {"contents": trueREDACTED,
	// 国产 3 家与 OpenAI Chat Completions 同构。
	MonitorProviderKimi:     {"model": true, "messages": true, "stream": trueREDACTED,
	MonitorProviderZhipu:    {"model": true, "messages": true, "stream": trueREDACTED,
	MonitorProviderDeepseek: {"model": true, "messages": true, "stream": trueREDACTED,
REDACTED
...
REDACTED{
	...
	{regexp.MustCompile(`sk-ant-[A-Za-z0-9_-]{20,REDACTED`), "sk-ant-***REDACTED***"REDACTED,
	// OpenAI / Anthropic 通用 sk-: sk-xxxxxxx
	{regexp.MustCompile(`sk-[A-Za-z0-9-]{20,REDACTED`), "sk-***REDACTED***"REDACTED,
	// xAI API Key：xai-xxxxxxx
	{regexp.MustCompile(`xai-[A-Za-z0-9_-]{6,REDACTED`), "xai-***REDACTED***"REDACTED,
	// Gemini / Google API Key：固定前缀 + 35 位
	{regexp.MustCompile(`AIza[A-Za-z0-9_-]{35REDACTED`), "AIza***REDACTED***"REDACTED,
	// JWT 三段式（Bearer 后常出现）：eyJxxx.eyJxxx.signature
	{regexp.MustCompile(`eyJ[A-Za-z0-9_-]{8,REDACTED\.eyJ[A-Za-z0-9_-]{8,REDACTED\.[A-Za-z0-9_-]{8,REDACTED`), "eyJ***REDACTED.JWT***"REDACTED,
REDACTED
...
func sanitizeErrorMessage(msg string) string {
	if msg == "" {
		return msg
REDACTED
	msg = monitorSensitiveQueryParamRegex.ReplaceAllString(msg, `${1REDACTEDREDACTED`)
