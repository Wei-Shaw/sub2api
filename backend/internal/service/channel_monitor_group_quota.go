...
// 账号多的分组（多个账号轮换）需要聚合口径：把组内全部 active 账号的
...
//
// 绝对值（Used/Limit）只有部分平台会给：Anthropic 有请求上限的窗口、
// Grok 有 limit 的窗口都有；Antigravity per-model 只有百分比。因此分两种
...
// 那种号确实用不了，由调用方单独计入耗尽。
//
// 同一 window 下的多档是可替换额度池（gemini 多档等）：
