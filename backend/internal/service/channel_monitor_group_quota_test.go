...
// grokUsageWithBonus 构造同时带 requests 与 tokens 两个日窗口的 Grok 用量
// （同 window 下的两档，互为可替换额度池）。
func grokUsageWithBonus(reqUsed, reqLimit, tokUsed, tokLimit float64) *UsageInfo {
	usage := grokUsage(tokUsed, tokLimit)
	rl, rr := int64(reqLimit), int64(reqLimit-reqUsed)
	if rr < 0 {
		rr = 0
	}
	usage.GrokRequestQuota = &xai.QuotaWindow{Limit: &rl, Remaining: &rr}
	return usage
}

// grokUsage 构造一份 Grok tokens 日窗口用量：已用 used / 上限 limit。
func grokUsage(used, limit float64) *UsageInfo {
	l, r := int64(limit), int64(limit-used)
	if r < 0 {
		r = 0
	}
	return &UsageInfo{GrokTokenQuota: &xai.QuotaWindow{Limit: &l, Remaining: &r}}
}
...
func groupQuotaTestAccounts(ids ...int64) []Account {
	accounts := make([]Account, 0, len(ids))
	for _, id := range ids {
		accounts = append(accounts, Account{ID: id, Platform: domain.PlatformOpenAI})
	}
	return accounts
}

// 耗尽的 Grok 账号能被 quotaDegradedHint 认出来（此前恒为 operational）。
func TestQuotaFetcher_ExhaustedAccountIsNotOperational(t *testing.T) {
	fetcher := newGroupQuotaFetcher(t, groupQuotaTestAccounts(1), &perAccountUsageSource{
		byAccount: map[int64]*UsageInfo{1: grokUsage(2000, 2000)},
	})
...
func TestFetchGroup_AggregatesCountsAndSumsTiers(t *testing.T) {
	fetcher := newGroupQuotaFetcher(t, groupQuotaTestAccounts(1, 2, 3), &perAccountUsageSource{
		byAccount: map[int64]*UsageInfo{
			1: grokUsage(500, 1000),  // 健康
			2: grokUsage(1000, 1000), // 耗尽
			3: grokUsage(0, 1000),    // 健康
		},
	})
	...
	require.Len(t, snapshot.Tiers, 1)
	require.Equal(t, "daily", snapshot.Tiers[0].Window)
	require.Equal(t, "tokens", snapshot.Tiers[0].Label)
...
// requests 窗口打满但 tokens 还有余的号必须算健康，否则组里「还有余额」会被误判耗尽。
func TestFetchGroup_SecondWindowRemainingIsHealthy(t *testing.T) {
	fetcher := newGroupQuotaFetcher(t, groupQuotaTestAccounts(1, 2), &perAccountUsageSource{
		byAccount: map[int64]*UsageInfo{
			1: grokUsageWithBonus(500, 500, 20, 200),
			// 两个账号都带两个窗口，聚合后 requests 档被稀释（500/1000）。
			2: grokUsageWithBonus(0, 500, 0, 500),
		},
	})
...
func TestFetchGroup_FetchFailuresCountAsUnknown(t *testing.T) {
	fetcher := newGroupQuotaFetcher(t, groupQuotaTestAccounts(1, 2), &perAccountUsageSource{
		byAccount: map[int64]*UsageInfo{1: grokUsage(100, 1000)},
...
func TestFetchGroup_EmptyGroupAndListErrorAreConfigProblems(t *testing.T) {
	...
	broken := newGroupQuotaFetcher(t, groupQuotaTestAccounts(1), &perAccountUsageSource{})
...
func TestMonitorAccountQuotaExhausted(t *testing.T) {
	...
	// 同 window 的 credits 档打满但 bonus 档还有余：这个号还能打，不能整号算耗尽。
	require.False(t, monitorAccountQuotaExhausted(&domain.MonitorQuotaSnapshot{
		Success: true,
		Tiers: []domain.MonitorQuotaTier{
...
func TestValidateMonitorModeFields_QuotaTargetBinding(t *testing.T) {
	...
	require.NoError(t, validateMonitorModeFields(&ChannelMonitor{
		Provider: MonitorProviderOpenAI, CheckMode: MonitorCheckModeQuota, GroupID: &groupID,
	}))
