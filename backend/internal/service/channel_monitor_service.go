...
func applyMonitorUpdate(existing *ChannelMonitor, p ChannelMonitorUpdateParams) error {
	...
	targetCheckMode := defaultCheckMode(existing.CheckMode)
	if p.CheckMode != nil {
		targetCheckMode = defaultCheckMode(*p.CheckMode)
	}
	if targetCheckMode != MonitorCheckModeQuota && p.Endpoint != nil {
		if err := validateMonitorEndpoint(targetProvider, targetEndpoint); err != nil {
			return err
		}
	} else if targetCheckMode != MonitorCheckModeQuota && p.Provider != nil {
		if err := validateStoredMonitorEndpointRoute(targetProvider, targetEndpoint); err != nil {
			return err
		}
	}
