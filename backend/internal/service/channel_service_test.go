	...
	REDACTED,
		{
			...
			pricingList: []ChannelModelPricing{
				{Platform: "anthropic", Models: []string{"claude-sonnet-4.5*"REDACTEDREDACTED,
				{Platform: "anthropic", Models: []string{"claude-sonnet-4-5-x"REDACTEDREDACTED,
		REDACTED,
			wantErr:     true,
			errContains: "conflict",
	REDACTED,
		{
			...
			pricingList: []ChannelModelPricing{
				{Platform: "openai", Models: []string{"gpt-5.6"REDACTEDREDACTED,
				{Platform: "openai", Models: []string{" gpt-5.6 "REDACTEDREDACTED,
		REDACTED,
			wantErr:     true,
			errContains: "conflict",
	...
	REDACTED,
		{
			...
			mapping: map[string]map[string]string{
				"anthropic": {"claude-sonnet-4.5": "a", "claude-sonnet-4-5": "b"REDACTED,
		REDACTED,
			wantErr: false,
	REDACTED,
		{
...
func TestGetChannelModelPricing_NormalizesDotsAndHyphens(t *testing.T) {
	ch := Channel{
		...
		GroupIDs: []int64{10REDACTED,
		ModelPricing: []ChannelModelPricing{
			{ID: 100, Platform: "anthropic", Models: []string{"claude-opus-4.8"REDACTED, BillingMode: BillingModePerRequest, PerRequestPrice: testPtrFloat64(0.007)REDACTED,
	REDACTED,
REDACTED
	repo := makeStandardRepo(ch, map[int64]string{10: "anthropic"REDACTED)
	svc := newTestChannelService(repo)
...
REDACTED{
		...
		{"composite matches kimi pricing", PlatformComposite, PlatformKimi, trueREDACTED,
		{"composite matches zhipu pricing", PlatformComposite, PlatformZhipu, trueREDACTED,
		{"composite matches deepseek pricing", PlatformComposite, PlatformDeepseek, trueREDACTED,
...
REDACTED{
		...
		{"composite returns concrete platforms", PlatformComposite, []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseekREDACTEDREDACTED,
REDACTED
...
func validTimePricingForTest() *ChannelTimePricing {
	return &ChannelTimePricing{Timezone: "Asia/Shanghai", Periods: []ChannelTimePricingPeriod{
		{StartTime: "09:00", EndTime: "12:00", Multiplier: 2REDACTED,
REDACTEDREDACTED
REDACTED

func TestValidatePricingTimePricing(t *testing.T) {
	token := []ChannelModelPricing{{BillingMode: BillingModeToken, TimePricing: validTimePricingForTest()REDACTEDREDACTED
	require.NoError(t, validatePricingTimePricing(token))

	implicitToken := []ChannelModelPricing{{TimePricing: validTimePricingForTest()REDACTEDREDACTED
	require.NoError(t, validatePricingTimePricing(implicitToken))

	image := []ChannelModelPricing{{BillingMode: BillingModeImage, TimePricing: validTimePricingForTest()REDACTEDREDACTED
	...
	invalid := []ChannelModelPricing{{
		...
		BillingMode: BillingModeToken,
		TimePricing: &ChannelTimePricing{Timezone: "UTC+8", Periods: validTimePricingForTest().PeriodsREDACTED,
REDACTEDREDACTED
...
REDACTEDREDACTED
	...
	empty := []ChannelModelPricing{{BillingMode: BillingModeToken, TimePricing: &ChannelTimePricing{Timezone: "Asia/Shanghai"REDACTEDREDACTEDREDACTED
