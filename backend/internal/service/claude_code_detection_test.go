...
func TestValidate_ClaudeCLIUserAgent(t *testing.T) {
	...
	tests := []struct {
		...
		want bool
REDACTED{
		{"标准版本号", "claude-cli/1.0.0", trueREDACTED,
		{"多位版本号", "claude-cli/12.34.56", trueREDACTED,
		{"大写开头", "Claude-CLI/1.0.0", trueREDACTED,
		{"非 claude-cli", "curl/7.64.1", falseREDACTED,
		{"空 User-Agent", "", falseREDACTED,
		{"部分匹配", "not-claude-cli/1.0.0", falseREDACTED,
		{"缺少版本号", "claude-cli/", falseREDACTED,
		{"版本格式不对", "claude-cli/1.0", falseREDACTED,
REDACTED
...
REDACTED{
		{"缺少 X-App", "X-App"REDACTED,
		{"缺少 anthropic-beta", "anthropic-beta"REDACTED,
		{"缺少 anthropic-version", "anthropic-version"REDACTED,
REDACTED
...
func TestValidate_MessagesPath_InvalidMetadataUserID(t *testing.T) {
	...
	tests := []struct {
		...
		metadata map[string]any
REDACTED{
		{"缺少 metadata", nilREDACTED,
		{"缺少 user_id", map[string]any{"other": "value"REDACTEDREDACTED,
		{"空 user_id", map[string]any{"user_id": ""REDACTEDREDACTED,
		{"格式错误", map[string]any{"user_id": "invalid-format"REDACTEDREDACTED,
		{"hex 长度不足", map[string]any{"user_id": "user_abc_account__session_uuid"REDACTEDREDACTED,
REDACTED
...
func TestValidate_MaxTokensOneHaikuBypass(t *testing.T) {
	...
	result := v.Validate(req, map[string]any{"model": "claude-3-haiku", "max_tokens": 1REDACTED)
...
func TestSystemPromptSimilarity(t *testing.T) {
	...
	tests := []struct {
		...
		want   bool
REDACTED{
		{"精确匹配", "You are Claude Code, Anthropic's official CLI for Claude.", trueREDACTED,
		{"带多余空格", "You  are  Claude  Code,  Anthropic's  official  CLI  for  Claude.", trueREDACTED,
		{"Agent SDK 模板", "You are a Claude agent, built on Anthropic's Claude Agent SDK.", trueREDACTED,
		{"文件搜索专家模板", "You are a file search specialist for Claude Code, Anthropic's official CLI for Claude.", trueREDACTED,
		{"对话摘要模板", "You are a helpful AI assistant tasked with summarizing conversations.", trueREDACTED,
		{"交互式 CLI 模板", "You are an interactive CLI tool that helps users", trueREDACTED,
		{"无关文本", "Write me a poem about cats", falseREDACTED,
		{"空文本", "", falseREDACTED,
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			body := map[string]any{
				"model": "claude-sonnet-4",
				"system": []any{
					map[string]any{"type": "text", "text": tt.promptREDACTED,
			REDACTED,
		REDACTED
...
func TestDiceCoefficient(t *testing.T) {
	tests := []struct {
		...
		tol  float64
REDACTED{
		{"相同字符串", "hello", "hello", 1.0, 0.001REDACTED,
		{"完全不同", "abc", "xyz", 0.0, 0.001REDACTED,
		{"空字符串", "", "hello", 0.0, 0.001REDACTED,
		{"单字符", "a", "b", 0.0, 0.001REDACTED,
REDACTED
