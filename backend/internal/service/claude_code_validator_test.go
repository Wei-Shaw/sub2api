...
const claudeCodeMetadataUserIDJSON = `{"device_id":"REDACTEDREDACTED","account_uuid":"","session_id":"aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"REDACTED`
...
REDACTED
	validBody := func(prompt string) map[string]any {
		return map[string]any{
			"model": "REDACTED",
			"system": []any{
				map[string]any{"type": "text", "text": promptREDACTED,
		REDACTED,
			"metadata": map[string]any{"user_id": claudeCodeMetadataUserIDJSONREDACTED,
	REDACTED
REDACTED
	...
	REDACTED,
		{
			...
			body: map[string]any{
				"model":  "REDACTED",
				"system": []any{map[string]any{"type": "text", "text": string(monitorPrompt)REDACTEDREDACTED,
		REDACTED,
	REDACTED,
		{
			...
			body: func() map[string]any {
				body := validBody(string(monitorPrompt))
				body["metadata"] = map[string]any{"user_id": "invalid"REDACTED
				return body
		REDACTED(),
	REDACTED,
		{
