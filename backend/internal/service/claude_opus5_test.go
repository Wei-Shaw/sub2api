...
func TestClaudeOpus5_FamilyFallbackDoesNotUseOpus4Rates(t *testing.T) {
	svc := NewBillingService(&config.Config{REDACTED, &PricingService{
		...
		REDACTED,
			...
			"claude-opus-4-1":        {InputCostPerToken: 15e-6, OutputCostPerToken: 75e-6REDACTED,
			"claude-opus-4-20250514": {InputCostPerToken: 15e-6, OutputCostPerToken: 75e-6REDACTED,
			"claude-3-opus-20240229": {InputCostPerToken: 15e-6, OutputCostPerToken: 75e-6REDACTED,
	REDACTED,
REDACTED)

	for _, model := range []string{"claude-opus-5", "us.anthropic.claude-opus-5-v1"REDACTED {
...
func TestClaudeOpus5_HardcodedFallbackPricing(t *testing.T) {
	...
	tests := []struct {
		...
		output float64
REDACTED{
		{"claude-opus-5", opus5InputPricePerToken, opus5OutputPricePerTokenREDACTED,
		{"us.anthropic.claude-opus-5-v1", opus5InputPricePerToken, opus5OutputPricePerTokenREDACTED,
		// 4.8 与 5 同价；修复前兜底表缺失，会掉到 claude-3-opus 的 $15/$75
		{"claude-opus-4-8", opus5InputPricePerToken, opus5OutputPricePerTokenREDACTED,
		// 相邻型号不能被 "opus-5" 误匹配
		{"claude-opus-4-5-20251101", 5e-6, 25e-6REDACTED,
		{"claude-opus-4-1-20250805", 15e-6, 75e-6REDACTED,
		{"claude-3-opus-20240229", 15e-6, 75e-6REDACTED,
REDACTED
...
func TestClaudeOpus5_BedrockCapabilityGates(t *testing.T) {
	tests := []struct {
		...
		opus47Newer   bool
REDACTED{
		{"claude-opus-5", true, true, trueREDACTED,
		{"us.anthropic.claude-opus-5-v1", true, true, trueREDACTED,
		{"eu.anthropic.claude-opus-5-v1", true, true, trueREDACTED,
		{"claude-sonnet-5", true, true, falseREDACTED,
		{"us.anthropic.claude-sonnet-5-v1", true, true, falseREDACTED,
		// 回归保护：旧模型不能因为 minor 可选而被误判为新版本
		{"anthropic.claude-opus-4-1-v1", false, false, falseREDACTED,
		{"anthropic.claude-sonnet-4-0-v1", false, false, falseREDACTED,
		{"anthropic.claude-3-opus-20240229-v1:0", false, false, falseREDACTED,
		{"us.anthropic.claude-opus-4-8-v1", true, true, trueREDACTED,
		// Haiku 不支持 tool search
		{"us.anthropic.REDACTED-v1:0", true, false, falseREDACTED,
REDACTED
	for _, tt := range tests {
...
func TestClaudeOpus5_BedrockThinkingConvertedToAdaptive(t *testing.T) {
	body := []byte(`{"thinking":{"type":"enabled","budget_tokens":10000REDACTEDREDACTED`)
	got := sanitizeBedrockThinking(body, "us.anthropic.claude-opus-5-v1")

	assert.JSONEq(t, `{"thinking":{"type":"adaptive"REDACTEDREDACTED`, string(got))
REDACTED
