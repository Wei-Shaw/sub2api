...
REDACTED
	...
	case PlatformKimi:
		...
		entries = append(entries, CNProviderBalanceEntry{Currency: "CNY", Balance: balanceREDACTED)
	...
	REDACTED
		...
		gjson.GetBytes(bodyBytes, "balance_infos").ForEach(func(_, info gjson.Result) bool {
			...
			if currency == "" {
				currency = "CNY"
		REDACTED
			entries = append(entries, CNProviderBalanceEntry{Currency: currency, Balance: balanceREDACTED)
			return true
	REDACTED)
		if len(entries) == 0 {
			entries = append(entries, CNProviderBalanceEntry{Currency: "CNY"REDACTED)
	REDACTED
REDACTED
