...
REDACTED
	...
	formatOnly, err := cnValidateProbeURL(&config.Config{
		Security: config.SecurityConfig{URLAllowlist: config.URLAllowlistConfig{Enabled: falseREDACTEDREDACTED,
REDACTED, "https://relay.attacker.example/v1/usages")
REDACTED
	require.Equal(t, "https://relay.attacker.example/v1/usages", formatOnly)
REDACTED

// recordingHTTPUpstream 断言探测被策略拒绝时没有任何上游请求发出。
type recordingHTTPUpstream struct{ calls int REDACTED
...
REDACTED
	...
	REDACTED,
REDACTEDREDACTED
	upstream := &recordingHTTPUpstream{REDACTED
