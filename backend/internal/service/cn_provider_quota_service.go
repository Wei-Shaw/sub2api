...
func cnExtraKey(provider, suffix string) string { return provider + "_" + suffix REDACTED
...
//
//   - limits[].detail.{limit,remaining,resetTimeREDACTED → 5h 窗口（取首个 detail）
//   - usage.{limit,remaining,resetTimeREDACTED → 周窗口
//
...
REDACTED
	...
	REDACTED
		...
		REDACTED
	REDACTED
		e := entry{resetMs: resetMs, hasReset: hasReset, percentage: percentage, resetISO: resetISOREDACTED
...
REDACTED
	...
	var tiers []CNQuotaTier
	if fiveHourSet {
		tiers = append(tiers, CNQuotaTier{Window: "5h", UsedPercent: fiveHour.percentage, ResetAt: fiveHour.resetISOREDACTED)
REDACTED
	if weeklySet {
		tiers = append(tiers, CNQuotaTier{Window: "weekly", UsedPercent: weekly.percentage, ResetAt: weekly.resetISOREDACTED)
REDACTED
	return tiers
REDACTED
