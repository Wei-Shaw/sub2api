...
func TestCNParseF64(t *testing.T) {
	...
	cases := []struct {
		...
		ok   bool
REDACTED{
		{"float64", float64(12.5), 12.5, trueREDACTED,
		{"int", 100, 100, trueREDACTED,
		{"numeric string", "33.3", 33.3, trueREDACTED,
		{"trim string", "  7  ", 7, trueREDACTED,
		{"non-numeric string", "abc", 0, falseREDACTED,
		{"nil", nil, 0, falseREDACTED,
REDACTED
...
func TestParseKimiUsageTiers(t *testing.T) {
	...
	body := []byte(`{
		"limits": [
			{"name": "5h", "detail": {"limit": 1000, "remaining": 600, "resetTime": "2026-08-14T15:00:00Z"REDACTEDREDACTED,
			{"name": "ignored-second-detail", "detail": {"limit": 999, "remaining": 0, "resetTime": "2026-08-14T20:00:00Z"REDACTEDREDACTED
		],
		"usage": {"limit": 10000, "remaining": 4000, "resetTime": "2026-08-18T00:00:00Z"REDACTED
REDACTED`)
...
func TestParseKimiUsageTiers_LimitZero(t *testing.T) {
	t.Parallel()
	body := []byte(`{"limits":[{"detail":{"limit":0,"remaining":0,"resetTime":"2026-08-14T15:00:00Z"REDACTEDREDACTED]REDACTED`)
	tiers := parseKimiUsageTiers(body)
...
func TestParseZhipuTokenTiers_UnitClassification(t *testing.T) {
	...
	data := gjson.Parse(`{
		"limits": [
			{"type":"TOKENS_LIMIT","unit":6,"percentage":70,"nextResetTime":1700000000000REDACTED,
			{"type":"TOKENS_LIMIT","unit":3,"percentage":20,"nextResetTime":1700000099999REDACTED
		]
REDACTED`)
...
func TestParseZhipuTokenTiers_SingleTierOldPlan(t *testing.T) {
	t.Parallel()
	data := gjson.Parse(`{"limits":[{"type":"TOKENS_LIMIT","unit":3,"percentage":15,"nextResetTime":1700000000000REDACTED]REDACTED`)
	tiers := parseZhipuTokenTiers(data)
	require.Len(t, tiers, 1)
	require.Equal(t, "5h", tiers[0].Window)
REDACTED
...
func TestParseZhipuTokenTiers_FallbackHeuristic(t *testing.T) {
	...
	data := gjson.Parse(`{
		"limits": [
			{"type":"TOKENS_LIMIT","percentage":50,"nextResetTime":1700000000000REDACTED,
			{"type":"TOKENS_LIMIT","percentage":10REDACTED
		]
REDACTED`)
	tiers := parseZhipuTokenTiers(data)
	require.Len(t, tiers, 2)
...
func TestParseZhipuTokenTiers_IgnoresNonTokenEntries(t *testing.T) {
	t.Parallel()
	data := gjson.Parse(`{"limits":[{"type":"OTHER_LIMIT","unit":3,"percentage":99REDACTED]REDACTED`)
...
func TestCNQuotaExtraUpdates(t *testing.T) {
	...
	tiers := []CNQuotaTier{
		{Window: "5h", UsedPercent: 40, ResetAt: "2026-08-14T15:00:00Z"REDACTED,
		{Window: "weekly", UsedPercent: 60, ResetAt: "2026-08-18T00:00:00Z"REDACTED,
REDACTED
...
func TestCNProviderResponseIndicatesInsufficientBalance(t *testing.T) {
	t.Parallel()
	positive := []string{
		`{"error":{"message":"余额不足"REDACTEDREDACTED`,
		`{"error":{"message":"Insufficient balance"REDACTEDREDACTED`,
		`{"code":"insufficient_credit"REDACTED`,
...
REDACTED
	negative := []string{
		`{"error":{"message":"rate limit exceeded"REDACTEDREDACTED`,
		`{"error":{"message":"quota exhausted"REDACTEDREDACTED`,
		``,
REDACTED
...
func TestCNBalanceURL(t *testing.T) {
	t.Parallel()
	kimi := &Account{Platform: PlatformKimiREDACTED
	require.Equal(t, "https://api.moonshot.cn/v1/users/me/balance", cnBalanceURL(kimi))

	deepseek := &Account{
		Platform:    PlatformDeepseek,
REDACTED"base_url": "https://api.deepseek.com"REDACTED,
REDACTED
...
REDACTED
	...
	partial := &Account{
		Platform: PlatformKimi,
		Extra:    map[string]any{"kimi_5h_reset_at": "2026-08-14T15:00:00Z"REDACTED, // 无 used_percent
REDACTED
	require.Empty(t, filterNil(cnProviderThresholdCandidates(partial, PlatformKimi)))

	// 空 Extra / nil account 安全返回。
	require.Empty(t, filterNil(cnProviderThresholdCandidates(&Account{Platform: PlatformKimiREDACTED, PlatformKimi)))
REDACTED
...
func TestEvaluateAccountSchedulingThreshold_KimiCodingPlan(t *testing.T) {
	...
	REDACTED,
REDACTED
	decision := EvaluateAccountSchedulingThreshold(account, map[string]int{PlatformKimi: 80REDACTED, now)
...
func TestEvaluateAccountSchedulingThreshold_CNWindowResetSkipped(t *testing.T) {
	...
	REDACTED,
REDACTED
	require.False(t, EvaluateAccountSchedulingThreshold(expired, map[string]int{PlatformZhipu: 80REDACTED, now).ShouldPause)
	...
	REDACTED,
REDACTED
	require.False(t, EvaluateAccountSchedulingThreshold(low, map[string]int{PlatformZhipu: 80REDACTED, now).ShouldPause)
REDACTED
...
REDACTED"account_mode": AccountModePayGREDACTED,
		Extra:       map[string]any{"kimi_5h_reset_at": future5h.Format(time.RFC3339)REDACTED,
REDACTED
...
REDACTED"api_key": "sk-kimi"REDACTED,
REDACTED
...
REDACTED"api_key": "sk-leak"REDACTED,
REDACTED
	...
	openai := &Account{
		...
		Type:        AccountTypeAPIKey,
REDACTED"api_key": "sk-openai"REDACTED,
REDACTED
...
func TestBuildUpstreamModelsRequest_CNProviders(t *testing.T) {
	...
	svc := &AccountTestService{cfg: &config.Config{REDACTEDREDACTED
	cases := []struct {
		name     string
		platform string
		mode     string
		wantURL  string
REDACTED{
		{"kimi default", PlatformKimi, "", "https://api.moonshot.cn/v1/models"REDACTED,
		{"kimi coding", PlatformKimi, AccountModeCoding, "https://api.kimi.com/coding/v1/models"REDACTED,
		{"zhipu default", PlatformZhipu, "", "https://open.bigmodel.cn/api/paas/v4/models"REDACTED,
		{"zhipu coding", PlatformZhipu, AccountModeCoding, "https://open.bigmodel.cn/api/coding/paas/v4/models"REDACTED,
		{"deepseek", PlatformDeepseek, "", "https://api.deepseek.com/v1/models"REDACTED,
REDACTED
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			creds := map[string]any{"api_key": "sk-test"REDACTED
			if tc.mode != "" {
				creds["account_mode"] = tc.mode
		REDACTED
			account := &Account{ID: 1, Platform: tc.platform, Type: AccountTypeAPIKey, Credentials: credsREDACTED
...
func TestGetAPIProtocol(t *testing.T) {
	...
	REDACTED
	REDACTEDPlatform: platform, Type: AccountTypeAPIKey, Credentials: credsREDACTED
REDACTED
	...
	require.Equal(t, APIProtocolChatCompletions, (&Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKeyREDACTED).GetAPIProtocol(), "非 CN 供应商恒为默认")
REDACTED
...
REDACTED{
		{"kimi payg", PlatformKimi, AccountModePayG, DefaultKimiPayGBaseURL, DefaultKimiPayGAnthropicBaseURL, DefaultKimiPayGBaseURLREDACTED,
		{"kimi coding", PlatformKimi, AccountModeCoding, DefaultKimiCodingBaseURL, DefaultKimiCodingAnthropicBaseURL, DefaultKimiCodingBaseURLREDACTED,
		{"zhipu payg", PlatformZhipu, AccountModePayG, DefaultZhipuPayGBaseURL, DefaultZhipuAnthropicBaseURL, DefaultZhipuPayGBaseURLREDACTED,
		{"zhipu coding", PlatformZhipu, AccountModeCoding, DefaultZhipuCodingBaseURL, DefaultZhipuAnthropicBaseURL, DefaultZhipuCodingBaseURLREDACTED,
		{"deepseek", PlatformDeepseek, AccountModePayG, DefaultDeepseekBaseURL, DefaultDeepseekAnthropicBaseURL, DefaultDeepseekBaseURLREDACTED,
REDACTED
...
REDACTED"api_protocol": APIProtocolAnthropicREDACTED,
REDACTED).GetAnthropicProtocolBaseURL())
	require.Equal(t, "https://api.kimi.com/coding", (&Account{
		Platform: PlatformKimi, Type: AccountTypeAPIKey,
REDACTED"api_protocol": APIProtocolAnthropic, "account_mode": AccountModeCodingREDACTED,
REDACTED).GetAnthropicProtocolBaseURL())
...
REDACTED).GetAnthropicProtocolBaseURL())
	...
	require.Equal(t, "https://custom.example.com/anthropic", (&Account{
		Platform: PlatformZhipu, Type: AccountTypeAPIKey,
REDACTED"api_protocol": APIProtocolAnthropic, "base_url": "https://custom.example.com/anthropic"REDACTED,
REDACTED).GetAnthropicProtocolBaseURL())

	// 非 Anthropic 协议返回空串
	require.Empty(t, (&Account{
		Platform: PlatformZhipu, Type: AccountTypeAPIKey,
REDACTED"base_url": "https://open.bigmodel.cn/api/paas/v4"REDACTED,
REDACTED).GetAnthropicProtocolBaseURL())
REDACTED
...
REDACTED
	...
	ccAccount := &Account{
		Platform: PlatformDeepseek, Type: AccountTypeAPIKey,
REDACTED"base_url": "https://ds-relay.example.com"REDACTED,
REDACTED
...
func TestNormalizeDeepSeekResponsesRequestBody(t *testing.T) {
	...
	deepseekResponses := &Account{
		Platform: PlatformDeepseek, Type: AccountTypeAPIKey,
REDACTED"api_protocol": APIProtocolResponsesREDACTED,
REDACTED
	body := []byte(`{"model":"deepseek-v4-pro","store":true,"previous_response_id":"resp_123","input":"hi"REDACTED`)
	...
	deepseekAdaptive := &Account{
		Platform: PlatformDeepseek, Type: AccountTypeAPIKey,
REDACTED"api_protocol": APIProtocolAdaptiveREDACTED,
REDACTED
	...
	deepseekCC := &Account{Platform: PlatformDeepseek, Type: AccountTypeAPIKeyREDACTED
	require.Equal(t, string(body), string(normalizeDeepSeekResponsesRequestBody(deepseekCC, body)))

	// openai 账号原样返回
	openai := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKeyREDACTED
...
REDACTED
	...
	zhipu.Extra = map[string]any{"anthropic_apikey_auth_scheme": "authorization_bearer"REDACTED
