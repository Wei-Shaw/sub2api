...
REDACTED)
	...
	for _, model := range []string{"deepseek-v4-flash", "deepseek-v4-pro", "deepseek-v4"REDACTED {
...
REDACTED)
	...
	for _, model := range []string{"deepseek-v4-flash", "deepseek-v4-pro"REDACTED {
		decision, err := resolver.Resolve(context.Background(), 7, model, CompositeRouteEndpointChatCompletions)
	REDACTED
		require.True(t, decision.Matched)
...
REDACTED)
	...
	tests := []struct {
		...
		wantUpstream string
REDACTED{
		{"all/gpt-5", CompositeRouteEndpointResponses, PlatformOpenAI, "gpt-5"REDACTED,
		{"all/claude-sonnet", CompositeRouteEndpointMessages, PlatformAnthropic, "claude-sonnet-4-6"REDACTED,
		{"all/gemini-pro", CompositeRouteEndpointGemini, PlatformGemini, "gemini-2.5-pro"REDACTED,
		{"all/grok", CompositeRouteEndpointResponses, PlatformGrok, "grok-4.3"REDACTED,
REDACTED
