...
type ConcurrencyCache interface {
	// 账号槽位管理
	// 键格式: concurrency:account:{accountIDREDACTED（有序集合，成员为 requestID）
	...
	// 用户槽位管理
	// 键格式: concurrency:user:{userIDREDACTED（有序集合，成员为 requestID）
...
// Format: {process_random_prefixREDACTED-{base36_counterREDACTED
...
func NewConcurrencyService(cache ConcurrencyCache) *ConcurrencyService {
	return &ConcurrencyService{cache: cacheREDACTED
REDACTED
...
func (s *ConcurrencyService) GetUsersLoadBatch(ctx context.Context, users []UserWithConcurrency) (map[int64]*UserLoadInfo, error) {
	if s.cache == nil {
		return map[int64]*UserLoadInfo{REDACTED, nil
REDACTED
