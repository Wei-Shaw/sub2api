...
REDACTED
	...
	svc.RecordCyberPolicyEvent(context.Background(), CyberPolicyRecordInput{
		...
		UpstreamBody:    `{"error":{"code":"cyber_policy"REDACTEDREDACTED`,
		UpstreamStatus:  400,
REDACTED)
...
REDACTED{
		{
			name:     "excluded group",
			config:   `{"all_groups":false,"group_ids":[8],"ban_threshold":1REDACTED`,
			groupID:  &groupID,
			model:    "gpt-5",
			wantLogs: 0,
	REDACTED,
		{
			...
			config:   `{"all_groups":false,"group_ids":[7],"ban_threshold":1REDACTED`,
			groupID:  nil,
			model:    "gpt-5",
			wantLogs: 0,
	REDACTED,
		{
			name:     "excluded model",
			config:   `{"all_groups":true,"model_filter":{"type":"include","models":["gpt-4o"]REDACTED,"ban_threshold":1REDACTED`,
			groupID:  &groupID,
			model:    "gpt-5",
			wantLogs: 0,
	REDACTED,
		{
			...
			config:     `{"enabled":false,"mode":"off","sample_rate":0,"all_groups":false,"group_ids":[7],"model_filter":{"type":"include","models":["gpt-5"]REDACTED,"ban_threshold":1REDACTED`,
			groupID:    &groupID,
			model:      "gpt-5",
			wantCalls:  []bool{falseREDACTED,
			wantLogs:   1,
			wantBanned: true,
	REDACTED,
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := &banCountArgsTestRepo{REDACTED
			userRepo := &contentModerationTestUserRepo{user: &User{ID: 1, Role: RoleUser, Status: StatusActiveREDACTEDREDACTED
			svc := NewContentModerationService(
				&contentModerationTestSettingRepo{values: map[string]string{
					SettingKeyRiskControlEnabled:      "true",
...
func TestRecordCyberPolicyEvent_RuntimeSnapshotRefreshFailureKeepsStaleScope(t *testing.T) {
	...
	settingRepo := &contentModerationRuntimeSettingRepo{values: map[string]string{
		SettingKeyRiskControlEnabled:      "true",
		SettingKeyContentModerationConfig: `{"all_groups":true,"model_filter":{"type":"include","models":["gpt-5"]REDACTEDREDACTED`,
REDACTEDREDACTED
	svc := NewContentModerationService(settingRepo, repo, nil, nil, nil, nil, nil, nil)
...
func TestRecordCyberPolicyEvent_CreateLogBeforeEmail(t *testing.T) {
	repo := &cyberOrderingTestRepo{REDACTED
	svc := NewContentModerationService(
		&contentModerationTestSettingRepo{values: map[string]string{
			SettingKeyRiskControlEnabled: "true",
REDACTED
...
func TestApplyFlaggedAccountSideEffects_PassesExcludeCyberFlag(t *testing.T) {
	...
	svc.applyFlaggedAccountSideEffects(context.Background(), cfgExclude, &ContentModerationLog{Flagged: true, UserID: &userIDREDACTED)

	cfgDefault := defaultContentModerationConfig() // 默认 false
	svc.applyFlaggedAccountSideEffects(context.Background(), cfgDefault, &ContentModerationLog{Flagged: true, UserID: &userIDREDACTED)

	require.Equal(t, []bool{true, falseREDACTED, repo.snapshotCountCalls(),
...
func TestRecordCyberPolicyEvent_ExcludeFromBanCount_SkipsBanJudgment(t *testing.T) {
	...
	svc := NewContentModerationService(
		&contentModerationTestSettingRepo{values: map[string]string{
			SettingKeyRiskControlEnabled:      "true",
			SettingKeyContentModerationConfig: `{"cyber_policy_exclude_from_ban_count":trueREDACTED`,
REDACTED
		repo, nil, nil, nil, nil, nil, nil,
	)
...
REDACTED)
	...
	require.Equal(t, []bool{falseREDACTED, repo.snapshotCountCalls(),
