...
func TestContentModerationKeywordMatcherMatchesLegacyBehavior(t *testing.T) {
	tests := []struct {
		...
		keywords []string
REDACTED{
		{name: "miss", text: "clean prompt", keywords: []string{"blocked", "secret"REDACTEDREDACTED,
		{name: "case insensitive", text: "contains SECRET value", keywords: []string{"secret"REDACTEDREDACTED,
		{name: "configured order wins", text: "early appears before later", keywords: []string{"later", "early"REDACTEDREDACTED,
		{name: "overlap uses configured order", text: "abc", keywords: []string{"bc", "abc"REDACTEDREDACTED,
		{name: "unicode", text: "这里包含敏感词和世界", keywords: []string{"世界", "敏感词"REDACTEDREDACTED,
		{name: "duplicates", text: "duplicate", keywords: []string{"duplicate", "DUPLICATE"REDACTEDREDACTED,
		{name: "empty entries", text: "blocked", keywords: []string{"", "blocked"REDACTEDREDACTED,
REDACTED
