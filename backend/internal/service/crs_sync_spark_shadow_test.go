...
func TestPropagateAccountProxyToShadows(t *testing.T) {
	...
	oldProxy := int64(11)
	mother := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, ProxyID: &oldProxyREDACTED
...
func TestGuardCRSShadowParentInvariant(t *testing.T) {
	...
	mother := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	require.NoError(t, repo.Create(ctx, mother))
	parentID := mother.ID
