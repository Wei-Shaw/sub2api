...
func (s *DashboardAggregationService) SetLeaderLockBackends(redisClient *redis.Client, db *sql.DB) {
	if s == nil {
		return
	}
...
func (s *DashboardAggregationService) runScheduledAggregation() {
	...
	release, ok := tryAcquireSingletonLeaderLock(ctx, s.redisClient, s.db, dashboardAggregationLeaderLockKey, s.instanceID, dashboardAggregationLeaderLockTTL)
	if !ok {
		return
	}
	defer release()
