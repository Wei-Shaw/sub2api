...
func TestSanitizeMustBeBeforeCCHSigning_HashConsistency(t *testing.T) {
	...
	body := []byte(`{"model":"claude-haiku-4-5","context_management":{"edits":[{"type":"clear_thinking_20251015"REDACTED]REDACTED,"system":[{"type":"text","text":"x-anthropic-billing-header: cc_version=2.1.92; cch=00000;"REDACTED],"messages":[]REDACTED`)

	// 最终发送场景：final beta 不含 context-management beta → sanitize 会 strip
	finalBeta := "oauth-2025-04-20,REDACTED"

	extractCCH := func(t *testing.T, b []byte) string {
	REDACTED
		m := regexp.MustCompile(`\bcch=([0-9a-fA-F]{5REDACTED)\b`).FindSubmatch(b)
...
REDACTED
	...
	REDACTED
		...
		restored := re.ReplaceAll(b, []byte("${1REDACTED00000${2REDACTED"))
