...
REDACTED
	if stats, ok := s.singleResult[accountID]; ok {
		return stats, nil
REDACTED
	return &usagestats.AccountStats{REDACTED, nil
REDACTED
...
REDACTED, 5*time.Second, time.Millisecond, "all callers must miss the cache before the loader is released")
	close(unblock)
	wg.Wait()
	...
	accounts := []Account{
		{
			...
			Extra:              map[string]any{"window_cost_limit": 100.0REDACTED,
...
REDACTED
	...
	REDACTED,
REDACTED
	repo := &usageLogWindowBatchRepoStub{REDACTED
	svc := &GatewayService{
		sessionLimitCache: cache,
		usageLogRepo:      repo,
REDACTED
...
REDACTED
	...
	cache := &sessionLimitCacheHotpathStub{REDACTED
	repo := &usageLogWindowBatchRepoStub{
		batchErr: errors.New("batch failed"),
		singleResult: map[int64]*usagestats.AccountStats{
			2: {StandardCost: 33.0REDACTED,
	REDACTED,
REDACTED
	svc := &GatewayService{
		sessionLimitCache: cache,
...
REDACTED
	...
	require.Equal(t, []string{"claude-3-5-haiku", "claude-3-5-sonnet"REDACTED, models1)
...
REDACTED
	...
	require.Equal(t, []string{"claude-3-5-haiku", "claude-3-5-sonnet"REDACTED, models3)
	require.Equal(t, int64(1), repo.listByGroupCalls.Load())
	...
	require.Equal(t, []string{"claude-3-7-sonnet"REDACTED, models4)
...
REDACTED
	...
	require.Equal(t, []string{"claude-3-5-sonnet", "gemini-2.5-pro"REDACTED, models)
...
REDACTED
	group9 := int64(9)
	group10 := int64(10)
	svc.modelsListCache.Set(modelsListCacheKey(&group9, PlatformAnthropic), []string{"a"REDACTED, time.Minute)
	svc.modelsListCache.Set(modelsListCacheKey(&group9, PlatformGemini), []string{"b"REDACTED, time.Minute)
	svc.modelsListCache.Set(modelsListCacheKey(&group10, PlatformAnthropic), []string{"c"REDACTED, time.Minute)
	svc.modelsListCache.Set("invalid-key", []string{"d"REDACTED, time.Minute)
...
REDACTED
	...
	repo := stubOpenAIAccountRepo{accounts: []Account{accountREDACTEDREDACTED
	concurrency := NewConcurrencyService(stubConcurrencyCache{REDACTED)
...
REDACTED
	...
	t.Run("without_prefetch_reads_cache_once", func(t *testing.T) {
		cache := &stickyGatewayCacheHotpathStub{stickyID: account.IDREDACTED
		svc := &GatewayService{
			accountRepo:        repo,
			cache:              cache,
			cfg:                cfg,
