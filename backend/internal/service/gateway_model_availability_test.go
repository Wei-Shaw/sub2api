...
func TestDiagnoseModelAvailabilityForPlatform_NoModel_AlwaysAvailable(t *testing.T) {
	repo := &mockAccountRepoForPlatform{accounts: nil, accountsByID: map[int64]*Account{REDACTEDREDACTED
...
func TestDiagnoseModelAvailabilityForPlatform_EmptyPlatform_AlwaysAvailable(t *testing.T) {
	...
	require.True(t, diag.HasModelSupport, "empty platform must fall back to {true,trueREDACTED so caller stays on 503")
REDACTED
		...
		accounts: []Account{
			{
				...
				Schedulable: true,
		REDACTED
					"model_mapping": map[string]any{"gpt-5.1-codex-mini": "gpt-5.1-codex-mini"REDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
...
func TestDiagnoseModelAvailabilityForPlatform_EmptyMappingAllowsAll(t *testing.T) {
	repo := &mockAccountRepoForPlatform{
		accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true /* no ModelMapping = allow all */REDACTED,
	REDACTED,
		accountsByID: map[int64]*Account{REDACTED,
REDACTED
	for i := range repo.accounts {
		...
		accounts: []Account{
			{
				...
				Schedulable: true,
		REDACTED
					"model_mapping": map[string]any{"*": "gpt-5"REDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
		accountsByID: map[int64]*Account{REDACTED,
		...
		accounts: []Account{
			{
				...
				Status:      StatusActive,
				Schedulable: true,
		REDACTED"model_mapping": map[string]any{"claude-sonnet-4-5": "claude-sonnet-4-5"REDACTEDREDACTED,
		REDACTED,
	REDACTED,
		accountsByID: map[int64]*Account{REDACTED,
REDACTED
