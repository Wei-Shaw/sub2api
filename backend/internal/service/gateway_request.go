...
var (
	...
	// Fast-path patterns for empty text blocks: {"type":"text","text":""REDACTED
	...
	sessionUserAgentVersionPattern = regexp.MustCompile(`\bv?\d+(?:\.\d+){1,3REDACTED\b`)
)
...
func NewRequestBodyRef(data []byte) *RequestBodyRef {
	return &RequestBodyRef{data: dataREDACTED
REDACTED
...
func missingJSONRange() jsonRange {
	return jsonRange{start: -1, end: -1REDACTED
REDACTED
...
REDACTED
	...
	if end < r.Index {
		return missingJSONRange()
REDACTED
	return jsonRange{start: r.Index, end: end, kind: r.TypeREDACTED
REDACTED
...
REDACTED
	...
	REDACTED
		if _, exists := seen[product]; exists {
			continue
	REDACTED
		seen[product] = struct{REDACTED{REDACTED
...
func ParseGatewayRequest(body *RequestBodyRef, protocol string) (*ParsedRequest, error) {
	parsed := &ParsedRequest{Body: bodyREDACTED
...
REDACTED
	...
	// Check for empty text blocks: {"type":"text","text":""REDACTED
...
REDACTED
	...
	REDACTED
		...
		REDACTED
			...
			// Strip empty text blocks: {"type":"text","text":""REDACTED
		...
		REDACTED
			...
			case "thinking":
				...
				if thinkingText != "" {
					newContent = append(newContent, map[string]any{"type": "text", "text": thinkingTextREDACTED)
			REDACTED
				continue
		...
		REDACTED
			...
			if blockType == "" {
				if rawThinking, hasThinking := blockMap["thinking"]; hasThinking {
					...
					case string:
						if v != "" {
							newContent = append(newContent, map[string]any{"type": "text", "text": vREDACTED)
					REDACTED
					default:
						if b, err := json.Marshal(v); err == nil && len(b) > 0 {
							newContent = append(newContent, map[string]any{"type": "text", "text": string(b)REDACTED)
					REDACTED
				REDACTED
					continue
			REDACTED
	...
	REDACTED
		...
		if newContent == nil {
			...
			REDACTED
				msgMap["content"] = []any{map[string]any{"type": "text", "text": placeholderREDACTEDREDACTED
		REDACTED
			continue
	REDACTED
...
REDACTED
	...
	REDACTED
		...
		REDACTED
			...
			REDACTED
				newContent = append(newContent, map[string]any{"type": "text", "text": textREDACTED)
				continue
			case "tool_result":
				modifiedThisMsg = true
		...
		if modifiedThisMsg {
			...
			if len(newContent) == 0 {
				...
				if role == "assistant" {
					placeholder = "(assistant content removed)"
			REDACTED
				newContent = append(newContent, map[string]any{"type": "text", "text": placeholderREDACTED)
		REDACTED
			msgMap["content"] = newContent
	REDACTED
...
// `thinking: {type: "enabled"REDACTED` 或 `thinking: {type: "adaptive"REDACTED` 表达启用。
