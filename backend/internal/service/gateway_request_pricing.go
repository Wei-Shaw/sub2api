...
type gatewayTokenRequestPricingAtCtxKey struct{REDACTED
type gatewayTokenRequestBillingGroupCtxKey struct{REDACTED
...
REDACTED
	...
	ctx = context.WithValue(ctx, gatewayTokenRequestPricingAtCtxKey{REDACTED, pricingAt)
	...
	if group, ok := ctx.Value(ctxkey.Group).(*Group); ok && IsGroupContextValid(group) {
		ctx = context.WithValue(ctx, gatewayTokenRequestBillingGroupCtxKey{REDACTED, group)
REDACTED
...
REDACTED
	pricingAt, ok := ctx.Value(gatewayTokenRequestPricingAtCtxKey{REDACTED).(time.Time)
...
func gatewayTokenRequestBillingGroupFromContext(ctx context.Context) *Group {
	if ctx == nil {
		return nil
REDACTED
	group, _ := ctx.Value(gatewayTokenRequestBillingGroupCtxKey{REDACTED).(*Group)
