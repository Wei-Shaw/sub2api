...
REDACTED
	for _, msg := range parsed.Messages {
		if m, ok := msg.(map[string]any); ok {
			...
			REDACTED
		REDACTED else if parts, ok := m["parts"].([]any); ok {
...
REDACTED
	...
	if billingErr != nil || ccErr != nil {
		...
		return body
REDACTED
	out, ok := setJSONRawBytes(body, "system", buildJSONArrayRaw([][]byte{billingBlock, ccPromptBlockREDACTED))
	if !ok {
...
REDACTED else {
	...
	REDACTED else if s.cfg != nil && s.cfg.Gateway.InjectBetaForAPIKey {
