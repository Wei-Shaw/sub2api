...
REDACTED
	...
	log.Printf(
		"[ClaudeMimicDebug] url=%s account=%d(%s) tokenType=%s mimic=%t meta.user_id=%q system.preview=%q headers={%sREDACTED",
		req.URL.String(),
		aid,
		aname,
		tokenType,
	...
	if tokenType == "oauth" {
		if mimicClaudeCode {
			...
			if requestHasTools(body) {
				requiredBetas = append([]string{claude.BetaClaudeCodeREDACTED, requiredBetas...)
		REDACTED
