				...
				if containsInt64(routingAccountIDs, stickyAccountID) && !isExcluded(stickyAccountID) {
					...
					if stickyAccount, ok := accountByID[stickyAccountID]; ok {
						...
						gatePass := s.isAccountSchedulableForSelection(stickyAccount) &&
							...
							s.isAccountSchedulableForOAuthPreemptivePause(ctx, stickyAccount) &&
	...
	if len(routingAccountIDs) == 0 && sessionHash != "" && stickyAccountID > 0 && !isExcluded(stickyAccountID) {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if ok {
				...
				oauthPauseOK := s.isAccountSchedulableForOAuthPreemptivePause(ctx, account)
				...
				slog.Debug("sticky.layer1_5_no_routing_checks",
					...
					"quota_ok", quotaOK,
					"oauth_pause_ok", oauthPauseOK,
					"window_cost_ok", windowCostOK,
					"rpm_ok", rpmOK,
				)

				if !clearSticky && platformOK && modelSupported && modelSchedulable && quotaOK && oauthPauseOK && windowCostOK && rpmOK && schedulable {
