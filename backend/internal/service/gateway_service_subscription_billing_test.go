	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p := &postUsageBillingParams{
				Cost:               &CostBreakdown{TotalCost: tt.totalCost, ActualCost: tt.actualCostREDACTED,
				User:               &User{ID: 1REDACTED,
				APIKey:             &APIKey{ID: 2, GroupID: &groupIDREDACTED,
				Account:            &Account{ID: 3REDACTED,
				Subscription:       &UserSubscription{ID: subIDREDACTED,
