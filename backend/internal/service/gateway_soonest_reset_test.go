...
func accWithWindowEnd(id int64, end *time.Time) accountWithLoad {
	...
	REDACTED,
		loadInfo: &AccountLoadInfo{AccountID: idREDACTED,
REDACTED
REDACTED
...
REDACTED
	...
	ids := map[int64]bool{got[0].account.ID: true, got[1].account.ID: trueREDACTED
...
func TestFilterBySoonestReset_SingleOrEmptyUnchanged(t *testing.T) {
	...
	single := []accountWithLoad{accWithWindowEnd(1, nil)REDACTED
