...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.ImageCount > 0 {
		if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey, account); resolved != nil && resolved.Mode == BillingModeToken {
			return s.calculateTokenCost(ctx, result, apiKey, account, billingModel, multiplier, pricingAt, opts)
		}
	...
	tokenCost := s.calculateTokenCost(ctx, result, apiKey, account, billingModel, multiplier, pricingAt, opts)
