...
func (e *grokCredentialFailureSnapshotError) Error() string { return e.cause.Error() REDACTED
func (e *grokCredentialFailureSnapshotError) Unwrap() error { return e.cause REDACTED
...
REDACTED
	...
	if errors.As(err, &existing) {
		return err
REDACTED
	return &grokCredentialFailureSnapshotError{cause: err, snapshot: grokCredentialMutationSnapshot(account)REDACTED
REDACTED
...
func grokCredentialFailureSnapshot(err error) (GrokCredentialMutationSnapshot, bool) {
	...
	if !errors.As(err, &snapshotErr) || snapshotErr == nil {
		return GrokCredentialMutationSnapshot{REDACTED, false
REDACTED
...
REDACTED
	...
	REDACTED
		...
		REDACTED
			...
			REDACTED
		REDACTED else if errors.Is(mutationErr, errOAuthRefreshAccountRereadFailed) {
				class = grokCredentialFailureClass{
					scope:   GatewayFailureScopeProvider,
...
REDACTED
	...
	case errors.Is(err, errGrokOAuthRefreshTokenMissing), errors.Is(err, errGrokOAuthAccessTokenMissing), errors.Is(err, errGrokOAuthAccessTokenExpired):
		return grokCredentialFailureClass{scope: GatewayFailureScopeAccount, reason: GrokCredentialReasonMissing, action: NextAccountRetry, permanent: true, message: "Grok OAuth credentials are missing or expired"REDACTED
	case contains("invalid_grant", "invalid_refresh_token", "token_expired", "refresh_token_reused", "refresh_token_invalidated", "app_session_terminated"):
		return grokCredentialFailureClass{scope: GatewayFailureScopeAccount, reason: GrokCredentialReasonRevoked, action: NextAccountRetry, permanent: true, message: "Grok OAuth credentials require account action"REDACTED
	case contains("spending limit", "run out of credits", "out of credits", "credits exhausted", "included free usage"):
		...
		return grokCredentialFailureClass{scope: GatewayFailureScopeAccount, reason: GrokCredentialReasonRefreshTransient, action: NextAccountRetry, transient: true, message: "Grok OAuth billing quota is temporarily exhausted"REDACTED
	case contains("grok_oauth_entitlement_denied", "entitlement_denied", "access_denied", "subscription required", "no active grok subscription"):
		return grokCredentialFailureClass{scope: GatewayFailureScopeAccount, reason: GrokCredentialReasonEntitlement, action: NextAccountRetry, permanent: true, message: "Grok OAuth entitlement requires account action"REDACTED
	case errors.Is(err, errGrokOAuthConfiguredProxyMiss), contains("grok_oauth_proxy_not_found"):
		return grokCredentialFailureClass{scope: GatewayFailureScopeAccount, reason: GrokCredentialReasonProxyInvalid, action: NextAccountRetry, permanent: true, message: "Grok OAuth account proxy configuration is invalid"REDACTED
	case errors.Is(err, errOAuthRefreshAccountRereadFailed):
		return grokCredentialFailureClass{scope: GatewayFailureScopeProvider, reason: GrokCredentialReasonProviderDown, action: NextAccountStop, message: "Grok OAuth account state is temporarily unavailable"REDACTED
	case errors.Is(err, errOAuthRefreshCredentialPersist):
		return grokCredentialFailureClass{scope: GatewayFailureScopeProvider, reason: GrokCredentialReasonProviderDown, action: NextAccountStop, message: "Grok OAuth shared credential state is temporarily unavailable"REDACTED
	case errors.As(err, &containmentErr):
		return grokCredentialFailureClass{scope: GatewayFailureScopeProvider, reason: GrokCredentialReasonProviderDown, action: NextAccountStop, message: "Grok OAuth provider state is temporarily unavailable"REDACTED
	case errors.Is(err, errOAuthRefreshAccountStateChanged):
		return grokCredentialFailureClass{scope: GatewayFailureScopeAccount, reason: GrokCredentialReasonAccountChanged, action: NextAccountRetry, message: "Grok OAuth account eligibility changed"REDACTED
	case errors.As(err, &providerConfigErr):
		return grokCredentialFailureClass{scope: GatewayFailureScopeProvider, reason: GrokCredentialReasonProviderConfig, action: NextAccountStop, message: "Grok OAuth provider configuration is unavailable"REDACTED
	...
	case contains("grok_oauth_proxy_lookup_failed"),
		...
		return grokCredentialFailureClass{scope: GatewayFailureScopeProvider, reason: GrokCredentialReasonProviderDown, action: NextAccountStop, message: "Grok OAuth provider is temporarily unavailable"REDACTED
	...
	case contains("status 429", "status 500", "status 502", "status 503", "status 504") && (account == nil || account.ProxyID == nil):
		return grokCredentialFailureClass{scope: GatewayFailureScopeProvider, reason: GrokCredentialReasonProviderDown, action: NextAccountStop, message: "Grok OAuth provider is temporarily unavailable"REDACTED
	default:
		return grokCredentialFailureClass{scope: GatewayFailureScopeAccount, reason: GrokCredentialReasonRefreshTransient, action: NextAccountRetry, transient: true, message: "Grok OAuth credential refresh is temporarily unavailable"REDACTED
REDACTED
REDACTED
...
REDACTED
	...
	REDACTED
		if ctx.Err() != nil {
			return "", ctx.Err()
	REDACTED
		rollbackRuntime := s.blockGrokCredentialRuntime(account, time.Time{REDACTED, string(class.reason))
	...
	REDACTED
		...
		if err != nil {
			...
			if requestErr != nil || errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
				if s.grokCredentialMutationCommitted(account.ID, class, time.Time{REDACTED) {
					updated = true
			REDACTED else if requestErr != nil {
					return "", requestErr
			REDACTED else {
					keepRuntimeBlock = true
...
REDACTED
	...
	if class.reason == GrokCredentialReasonProxyInvalid {
		if latest.ProxyID == nil || latest.Proxy != nil {
			return "", errOAuthRefreshAccountStateChanged
	REDACTED
REDACTED else if latest.ProxyID != nil && latest.Proxy == nil {
		return "", errOAuthRefreshAccountStateChanged
REDACTED
...
func grokCredentialMutationSnapshot(account *Account) GrokCredentialMutationSnapshot {
	if account == nil {
		return GrokCredentialMutationSnapshot{REDACTED
REDACTED
