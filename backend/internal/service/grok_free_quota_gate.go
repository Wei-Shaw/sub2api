...
func resolveGrokFreeQuotaGateSettings(cfg *config.Config) (grokFreeQuotaGateSettings, bool) {
	if cfg == nil || !cfg.Gateway.Grok.FreeQuotaSoftGateEnabled {
		return grokFreeQuotaGateSettings{REDACTED, false
REDACTED
...
var freeQuotaRefreshInFlight sync.Map // *sync.Map -> *sync.Map (accountID -> struct{REDACTED)
...
REDACTED
	...
	seenMissing := make(map[int64]struct{REDACTED)
	...
	REDACTED
		...
		if _, exists := seenMissing[account.ID]; !exists {
			seenMissing[account.ID] = struct{REDACTED{REDACTED
...
func scheduleGrokFreeQuotaStatsRefresh(
	...
	if usageLogRepo == nil || cache == nil || len(accountIDs) == 0 {
		return
REDACTED
	inFlightRoot, _ := freeQuotaRefreshInFlight.LoadOrStore(cache, &sync.Map{REDACTED)
...
REDACTED
	...
	for _, id := range accountIDs {
		if _, loaded := inFlight.LoadOrStore(id, struct{REDACTED{REDACTED); !loaded {
