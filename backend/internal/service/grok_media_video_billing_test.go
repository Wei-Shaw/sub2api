...
func TestIsGrokVideoStatusBillable(t *testing.T) {
	...
	require.True(t, IsGrokVideoStatusBillable([]byte(`{
		...
		"video":{"url":"https://vidgen.x.ai/x.mp4","duration":8,"respect_moderation":trueREDACTED
REDACTED`)))
	...
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"expired"REDACTED`)))
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"failed"REDACTED`)))
	// done without video.url is not billable
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"done"REDACTED`)))
	// URL alone (legacy/non-official shapes) is not enough
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"url":"https://example.com/v.mp4"REDACTED`)))
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"download_url":"/v1/videos/task/content"REDACTED`)))
	// "completed" is not the official enum value
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"completed","video":{"url":"https://vidgen.x.ai/x.mp4"REDACTEDREDACTED`)))
REDACTED
...
REDACTED
	...
	body := []byte(`{
		"status":"done",
		"model":"grok-imagine-video-1.5",
		"video":{"url":"https://vidgen.x.ai/signed.mp4","duration":12,"respect_moderation":trueREDACTED
REDACTED`)
...
REDACTED
	...
	body := []byte(`{"status":"done","video":{"url":"https://vidgen.x.ai/signed.mp4"REDACTEDREDACTED`)
...
func TestExtractGrokVideoBillingRejectsNonDoneStatus(t *testing.T) {
	t.Parallel()
	pending := &GrokVideoPendingBilling{Model: "m", VideoDurationSeconds: 8, VideoResolution: "720p"REDACTED
	require.Nil(t, ExtractGrokVideoBillingFromStatusBody(
		[]byte(`{"status":"pending","video":{"url":"https://vidgen.x.ai/x.mp4","duration":8REDACTEDREDACTED`),
		pending, "req",
	))
	require.Nil(t, ExtractGrokVideoBillingFromStatusBody(
		[]byte(`{"status":"completed","video":{"url":"https://vidgen.x.ai/x.mp4","duration":8REDACTEDREDACTED`),
		pending, "req",
	))
REDACTED
...
func TestGrokMediaUsageFromResponseVideoStatusBillsOnOfficialDone(t *testing.T) {
	...
	meta := grokMediaUsageFromResponse(
		...
		[]byte(`{"status":"done","model":"grok-imagine-video-1.5","video":{"url":"https://vidgen.x.ai/a.mp4","duration":9REDACTEDREDACTED`),
	)
	...
	completed := grokMediaUsageFromResponse(
		GrokMediaEndpointVideoStatus,
		GrokMediaRequestInfo{REDACTED,
		[]byte(`{"status":"completed","video":{"url":"https://vidgen.x.ai/a.mp4","duration":9REDACTEDREDACTED`),
	)
