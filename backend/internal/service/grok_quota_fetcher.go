...
REDACTED else if snapshot.StatusCode >= http.StatusBadRequest || usage.GrokLastStatusCode == 0 {
		usage.GrokLastStatusCode = snapshot.StatusCode
REDACTED
...
REDACTED
	if billing.UsagePercent != nil {
		seven := &UsageProgress{Utilization: *billing.UsagePercentREDACTED
...
REDACTED
	if monthlyUtil != nil {
		thirty := &UsageProgress{Utilization: *monthlyUtilREDACTED
