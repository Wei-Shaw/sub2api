...
func grokInt64PtrForTest(v int64) *int64 { return &v REDACTED
func grokIntPtrForTest(v int) *int       { return &v REDACTED

func TestGrokQuotaFetcherBuildUsageInfoUnknownUntilFirstSnapshot(t *testing.T) {
	t.Parallel()

	usage := NewGrokQuotaFetcher().BuildUsageInfo(&Account{Platform: PlatformGrok, Type: AccountTypeOAuthREDACTED)
	...
	REDACTED,
		Extra: map[string]any{
			grokQuotaSnapshotExtraKey: &xai.QuotaSnapshot{
				Requests:          &xai.QuotaWindow{Limit: &reqLimitREDACTED,
				Tokens:            &xai.QuotaWindow{Limit: &tokLimitREDACTED,
...
func TestGrokQuotaFetcherJWTBeatsAmbiguousSuperGrokProQuota(t *testing.T) {
	...
	account := &Account{
		...
		Type:     AccountTypeOAuth,
REDACTED
			"access_token":      makeGrokOAuthJWT(map[string]any{"tier": 1REDACTED),
			"subscription_tier": "SuperGrokPro",
	REDACTED,
		Extra: map[string]any{
			grokQuotaSnapshotExtraKey: &xai.QuotaSnapshot{
				Requests:          &xai.QuotaWindow{Limit: &heavyReqREDACTED,
				LastHeadersSeenAt: fresh,
				HeadersObserved:   true,
				UpdatedAt:         fresh,
		REDACTED,
...
func TestGrokQuotaFetcherClassifiesForbiddenAndReauth(t *testing.T) {
	...
	tests := []struct {
		...
		wantEntitle string
REDACTED{
		{name: "reauth", statusCode: http.StatusUnauthorized, wantReauth: true, wantCode: "unauthenticated"REDACTED,
		{name: "forbidden", statusCode: http.StatusForbidden, wantForbid: true, wantCode: "forbidden", wantEntitle: "forbidden"REDACTED,
REDACTED
