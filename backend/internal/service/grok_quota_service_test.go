...
REDACTEDREDACTED
	...
	cfg.Security.URLAllowlist.UpstreamHosts = []string{"allowed.example.test"REDACTED
	svc := &GrokQuotaService{accountRepo: repo, httpUpstream: upstream, cfg: cfgREDACTED
...
REDACTEDREDACTED
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`{"data":[{"id":"grok-4.5"REDACTED]REDACTED`)),
REDACTEDREDACTED
	svc := &GrokQuotaService{accountRepo: repo, httpUpstream: upstream, cfg: &config.Config{REDACTEDREDACTED
...
type grokHybridUpstream struct {
	...
	billingStarted       chan struct{REDACTED
	billingRelease       <-chan struct{REDACTED
...
REDACTED
	...
	REDACTED
		...
		if headers == nil {
			headers = http.Header{
				"X-Ratelimit-Limit-Tokens":     []string{"2000000"REDACTED,
				"X-Ratelimit-Remaining-Tokens": []string{"1500000"REDACTED,
		REDACTED
	REDACTED
		return &http.Response{StatusCode: status, Header: headers, Body: io.NopCloser(strings.NewReader(`{"id":"resp_probe"REDACTED`))REDACTED, nil
REDACTED
	if u.billingStarted != nil {
		u.billingStartOnce.Do(func() { close(u.billingStarted) REDACTED)
REDACTED
...
REDACTED
	if billingStatus != 0 && billingStatus != http.StatusOK {
		return &http.Response{
			...
			Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"billing limited"REDACTEDREDACTED`)),
	REDACTED, nil
REDACTED
	...
	REDACTED
		payload := `{"config":{"currentPeriod":{"type":"WEEKLY","start":"2026-07-09T03:25:00Z","end":"2026-07-16T03:25:00Z"REDACTED` + usage + `REDACTEDREDACTED`
		return &http.Response{StatusCode: http.StatusOK, Header: make(http.Header), Body: io.NopCloser(strings.NewReader(payload))REDACTED, nil
REDACTED
	monthlyLimit := ""
	if u.monthlyLimitCents != nil {
		monthlyLimit = `,"monthlyLimit":{"val":` + strconv.FormatFloat(*u.monthlyLimitCents, 'f', -1, 64) + `REDACTED`
REDACTED
	monthlyPayload := `{"config":{"billingPeriodStart":"2026-07-01T00:00:00Z","billingPeriodEnd":"2026-08-01T00:00:00Z"` + monthlyLimit + `REDACTEDREDACTED`
	return &http.Response{
		StatusCode: http.StatusOK,
		Header:     make(http.Header),
...
func TestGrokQuotaServiceFetchBillingRetries502ThenSucceeds(t *testing.T) {
	...
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{status: http.StatusBadGateway, body: `The origin web server returned an invalid or incomplete response to Cloudflare.`REDACTED,
		{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY","start":"2026-07-09T03:25:00Z","end":"2026-07-16T03:25:00Z"REDACTED,"creditUsagePercent":12REDACTEDREDACTED`REDACTED,
REDACTEDREDACTED
	svc := &GrokQuotaService{httpUpstream: upstreamREDACTED
...
func TestGrokQuotaServiceFetchBillingRetriesTransportErrorThenSucceeds(t *testing.T) {
	...
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{err: errors.New("temporary transport failure")REDACTED,
		{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY"REDACTED,"creditUsagePercent":8REDACTEDREDACTED`REDACTED,
REDACTEDREDACTED
	svc := &GrokQuotaService{httpUpstream: upstreamREDACTED
...
func TestGrokQuotaServiceFetchBillingStopsAfterSingleTransientRetry(t *testing.T) {
	...
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{status: http.StatusBadGateway, body: `cloudflare failure`REDACTED,
		{status: http.StatusBadGateway, body: `cloudflare failure`REDACTED,
		{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY"REDACTEDREDACTEDREDACTED`REDACTED,
REDACTEDREDACTED
	svc := &GrokQuotaService{httpUpstream: upstreamREDACTED
...
func TestGrokQuotaServiceFetchBillingDoesNotRetryNonTransientStatuses(t *testing.T) {
	tests := []struct {
		...
		wantErr bool
REDACTED{
		{name: "unauthorized", status: http.StatusUnauthorized, wantErr: trueREDACTED,
		{name: "forbidden", status: http.StatusForbidden, wantErr: trueREDACTED,
		{name: "rate limited", status: http.StatusTooManyRequests, wantErr: falseREDACTED,
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
				{status: tt.status, body: `{"error":{"message":"rejected"REDACTEDREDACTED`REDACTED,
				{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY"REDACTEDREDACTEDREDACTED`REDACTED,
		REDACTEDREDACTED
			svc := &GrokQuotaService{httpUpstream: upstreamREDACTED
...
func TestIsRetryableGrokBillingStatus(t *testing.T) {
	tests := []struct {
		...
		want   bool
REDACTED{
		{status: http.StatusBadGateway, want: trueREDACTED,
		{status: http.StatusServiceUnavailable, want: trueREDACTED,
		{status: http.StatusGatewayTimeout, want: trueREDACTED,
		{status: http.StatusUnauthorized, want: falseREDACTED,
		{status: http.StatusForbidden, want: falseREDACTED,
		{status: http.StatusTooManyRequests, want: falseREDACTED,
REDACTED
...
REDACTEDREDACTED
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{status: http.StatusBadGateway, body: `cloudflare failure`REDACTED,
		{status: http.StatusOK, body: `{"id":"unexpected_retry"REDACTED`REDACTED,
REDACTEDREDACTED
...
func TestGrokQuotaServiceProbeUsageStoresHeaders(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{
		mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{42: accountREDACTED,
	REDACTED,
REDACTED
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header: http.Header{
			...
			"X-Ratelimit-Reset-Requests":     []string{"2000000000"REDACTED,
			"X-Ratelimit-Limit-Tokens":       []string{"1000"REDACTED,
			"X-Ratelimit-Remaining-Tokens":   []string{"900"REDACTED,
	REDACTED,
...
REDACTED
	repo := &grokQuotaAccountRepo{
		mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{47: accountREDACTED,
	REDACTED,
REDACTED
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
...
func TestGrokQuotaServiceProbeUsageReportsProbeModelOnUpstreamError(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{
		mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{48: accountREDACTED,
	REDACTED,
REDACTED
	upstream := &httpUpstreamRecorder{resp: &http.Response{
...
func TestGrokQuotaServiceProbeUsageLoadsProxyWhenAccountEdgeMissing(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{
		mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{46: accountREDACTED,
	REDACTED,
REDACTED
...
func TestGrokQuotaServiceProbeUsageStoresNoHeadersState(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{
		mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{45: accountREDACTED,
	REDACTED,
...
func TestGrokQuotaServiceProbeUsageDoesNotOverwriteSnapshotOnUnauthorized(t *testing.T) {
	...
	previous := &xai.QuotaSnapshot{StatusCode: http.StatusOK, HeadersObserved: trueREDACTED
	account.Extra = map[string]any{grokQuotaSnapshotExtraKey: previousREDACTED
	repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
		accountsByID: map[int64]*Account{account.ID: accountREDACTED,
REDACTEDREDACTED
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{REDACTED,
		Body:       io.NopCloser(strings.NewReader(`{"error":"unauthorized"REDACTED`)),
REDACTEDREDACTED
	svc := NewGrokQuotaService(repo, nil, NewGrokTokenProvider(repo, nil), upstream, nil)
...
func TestGrokQuotaServiceProbeUsageReturnsRateLimitedSnapshot(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{
		mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{43: accountREDACTED,
	REDACTED,
REDACTED
	upstream := &httpUpstreamRecorder{resp: &http.Response{
...
func TestGrokLocalUsage24hUsesRollingUTCWindow(t *testing.T) {
	...
	t.Run("returns usage from exact rolling window", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{stats: &usagestats.AccountStats{Tokens: 1_250_000REDACTEDREDACTED
		stats := grokLocalUsage24h(context.Background(), repo, 57, now)

		require.NotNil(t, stats)
		require.EqualValues(t, 1_250_000, stats.Tokens)
		require.Equal(t, []time.Time{now.UTC().Add(-24 * time.Hour)REDACTED, repo.startTimes)
REDACTED)

	t.Run("query failure returns no stats", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{err: context.DeadlineExceededREDACTED
		stats := grokLocalUsage24h(context.Background(), repo, 57, now)
...
REDACTED)
	...
	t.Run("invalid account returns no stats without query", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{REDACTED
...
REDACTED
	...
	t.Run("free queries only rolling 24h", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{stats: &usagestats.AccountStats{Tokens: 500_000REDACTEDREDACTED
		...
		require.Equal(t, []time.Time{now.Add(-24 * time.Hour)REDACTED, repo.startTimes)
REDACTED)
...
REDACTED
	...
	t.Run("valid weekly window", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{stats: &usagestats.AccountStats{Tokens: 1_500_000REDACTEDREDACTED
...
REDACTED
	...
	go func() {
		...
		billingOutcomes <- probeOutcome{result: result, err: errREDACTED
REDACTED()
	<-billingStarted
	secondStarted := make(chan struct{REDACTED)
	go func() {
		close(secondStarted)
		result, err := svc.ProbeBilling(context.Background(), account.ID)
		billingOutcomes <- probeOutcome{result: result, err: errREDACTED
...
func TestGrokQuotaServiceBilling403PersistsMediaEligibilitySignal(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
		accountsByID: map[int64]*Account{account.ID: accountREDACTED,
REDACTEDREDACTED
	upstream := &grokHybridUpstream{billingStatus: http.StatusForbiddenREDACTED
	svc := NewGrokQuotaService(repo, nil, NewGrokTokenProvider(repo, nil), upstream, nil)
...
REDACTED
	...
	account.Extra = map[string]any{grokBillingExtraKey: billingREDACTED
...
REDACTED
	...
	account.Extra = map[string]any{grokBillingExtraKey: result.BillingREDACTED
	eligible, reason := account.GrokMediaGenerationEligibility()
	require.False(t, eligible)
	require.Equal(t, "billing_forbidden", reason)
REDACTED
...
func TestPreferBillingObservationStatus(t *testing.T) {
	...
	tests := []struct {
		...
		want          int
REDACTED{
		{name: "weekly forbidden wins", weeklyStatus: http.StatusForbidden, monthlyStatus: http.StatusBadGateway, want: http.StatusForbiddenREDACTED,
		{name: "monthly forbidden wins", weeklyStatus: http.StatusBadGateway, monthlyStatus: http.StatusForbidden, want: http.StatusForbiddenREDACTED,
		{name: "weekly observation otherwise wins", weeklyStatus: http.StatusTooManyRequests, monthlyStatus: http.StatusBadGateway, want: http.StatusTooManyRequestsREDACTED,
		{name: "monthly observation is fallback", monthlyStatus: http.StatusBadGateway, want: http.StatusBadGatewayREDACTED,
REDACTED
