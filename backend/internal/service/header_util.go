...
var headerWireOrderSet map[string]struct{REDACTED

func init() {
	headerWireOrderSet = make(map[string]struct{REDACTED, len(headerWireOrder))
	for _, k := range headerWireOrder {
		headerWireOrderSet[strings.ToLower(k)] = struct{REDACTED{REDACTED
REDACTED
REDACTED
...
REDACTED
	...
	seen := make(map[string]struct{REDACTED, len(h))
	...
	for _, wk := range headerWireOrder {
		...
		if actual, ok := present[lk]; ok {
			if _, dup := seen[lk]; !dup {
				...
				seen[lk] = struct{REDACTED{REDACTED
		REDACTED
	REDACTED
REDACTED
