...
func TestIsAcceptableFingerprintUserAgent(t *testing.T) {
	cases := []struct {
		...
		want bool
REDACTED{
		{"official_cli", "claude-cli/2.1.220 (external, cli)", trueREDACTED,
		{"official_cli_no_meta", "claude-cli/2.1.220", trueREDACTED,
		{"next_major_still_allowed", "claude-cli/4.0.0 (external, cli)", trueREDACTED,
		{"other_product_valid_form", "some-sdk/1.2.3 (node)", trueREDACTED,

		// 本地/开发构建：版本号后带后缀，正是 #5254 的毒化 UA 形态。
		{"local_build_suffix", "claude-cli/999.0.0-local (undefined, cli)", falseREDACTED,
		{"dev_build_suffix", "claude-cli/2.1.220-dev (external, cli)", falseREDACTED,
		{"build_metadata_suffix", "claude-cli/2.1.220+build1 (external, cli)", falseREDACTED,

		// 哨兵版本号：形态合法但主版本号远超 sub2api 自身伪装版本。
		{"sentinel_major", "claude-cli/999.0.0 (external, cli)", falseREDACTED,

		{"empty", "", falseREDACTED,
		{"no_version", "claude-cli (external, cli)", falseREDACTED,
		{"two_segment_version", "claude-cli/2.1 (external, cli)", falseREDACTED,
		// 浏览器 UA 是两段版本号，不符合三段 semver 形态：不适合作为账号身份。
		{"browser_ua_two_segment", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)", falseREDACTED,
		{"leading_junk", "x claude-cli/2.1.220 (external, cli)", falseREDACTED,
		{"too_long", "claude-cli/2.1.220 (" + strings.Repeat("a", 300) + ")", falseREDACTED,
REDACTED
...
func TestGetOrCreateFingerprintRejectsMalformedUserAgentOnCreate(t *testing.T) {
	cache := &stubIdentityCache{REDACTED
...
REDACTED
	cache := &stubIdentityCache{fingerprint: cachedREDACTED
	svc := NewIdentityService(cache)
...
func TestGetOrCreateFingerprintMissingUserAgentKeepsDefault(t *testing.T) {
	...
	fp, err := svc.GetOrCreateFingerprint(context.Background(), 1, http.Header{REDACTED)
