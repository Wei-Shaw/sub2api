...
// SetDownloadClient injects the SSRF-safe client used for upstream image URLs.
// Application wiring sets it before the resolver can be called.
func (s *ImageStorageSettingService) SetDownloadClient(client *http.Client) {
	if s == nil {
		return
	}
	s.mu.Lock()
	s.downloadClient = client
	s.resolved = false
	s.uploader = nil
	s.enabled = false
	s.mu.Unlock()
...
func (s *ImageStorageSettingService) resolve() (*ImageResultUploader, bool) {
	...
	uploader, err := NewImageResultUploader(storage, cfg.Prefix, cfg.MaxDownloadByte, s.downloadClient)
	if err != nil {
		logger.L().Error("image_storage.uploader_build_failed; async image tasks stay disabled", zap.Error(err))
		return nil, false
	}
	s.uploader = uploader
	s.enabled = true
