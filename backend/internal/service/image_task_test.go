...
func TestImageTaskServiceLifecycleAndOwnership(t *testing.T) {
	...
	owner := ImageTaskOwner{UserID: 7, APIKeyID: 9REDACTED
...
REDACTED
	...
	_, err = svc.Get(context.Background(), ImageTaskOwner{UserID: 7, APIKeyID: 10REDACTED, created.ID)
	require.ErrorIs(t, err, ErrImageTaskNotFound)

	result := json.RawMessage(`{"created":123,"data":[{"url":"https://example.test/image.png"REDACTED]REDACTED`)
...
func TestImageTaskServiceInvalidResultBecomesFailed(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), ImageTaskOwner{UserID: 1, APIKeyID: 2REDACTED)
REDACTED

	require.NoError(t, svc.Complete(context.Background(), created.ID, http.StatusOK, json.RawMessage(`not-json`)))
	got, err := svc.Get(context.Background(), ImageTaskOwner{UserID: 1, APIKeyID: 2REDACTED, created.ID)
REDACTED
...
func TestImageTaskServiceMapsStoreFailures(t *testing.T) {
	store := &imageTaskMemoryStore{saveErr: errors.New("redis down")REDACTED
	svc := NewImageTaskService(store)

	_, err := svc.Create(context.Background(), ImageTaskOwner{UserID: 1, APIKeyID: 2REDACTED)
