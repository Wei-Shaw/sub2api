...
REDACTED{
		{
			...
			body:       []byte(`{"error":{"message":"model not found"REDACTEDREDACTED`),
			want:       true,
	REDACTED,
		{
			name:       "404 model_not_found code",
			statusCode: http.StatusNotFound,
			body:       []byte(`{"error":{"code":"model_not_found","message":"The requested model was not found"REDACTEDREDACTED`),
			want:       true,
	REDACTED,
		{
			name:       "404 unknown model message",
			statusCode: http.StatusNotFound,
			body:       []byte(`{"error":{"message":"unknown model gpt-5.4"REDACTEDREDACTED`),
			want:       true,
	REDACTED,
		{
			name:       "404 endpoint not found is not model specific",
			statusCode: http.StatusNotFound,
			body:       []byte(`{"error":{"message":"endpoint not found"REDACTEDREDACTED`),
			want:       false,
	REDACTED,
		{
...
REDACTED{
		{
			...
			statusCode: http.StatusBadRequest,
			body:       []byte(`{"detail":"The 'gpt-5.6-sol' model is not supported when using Codex with a ChatGPT account."REDACTED`),
			want:       true,
	REDACTED,
		{
			name:       "400 codex plan gated error message payload",
			statusCode: http.StatusBadRequest,
			body:       []byte(`{"error":{"message":"The 'gpt-5.4' model is not supported when using Codex with a ChatGPT account."REDACTEDREDACTED`),
			want:       true,
	REDACTED,
		{
			name:       "400 unrelated invalid request does not match",
			statusCode: http.StatusBadRequest,
			body:       []byte(`{"error":{"message":"Invalid schema for response_format 'agentic_plan'"REDACTEDREDACTED`),
			want:       false,
	REDACTED,
		{
