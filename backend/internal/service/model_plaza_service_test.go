...
REDACTED
	return NewModelPlazaService(repo, &stubGroupRepoForAvailable{activeGroups: groupsREDACTED, pricing, nil, nil)
REDACTED
...
REDACTED
	...
	svc := newPlazaService([]Channel{priced, unpricedREDACTED, groups, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	require.Len(t, out, 1)
...
REDACTED
	svc := newPlazaService([]Channel{chREDACTED, groups, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	require.Len(t, out, 2)
...
REDACTED
	...
	out, err := newPlazaService([]Channel{chREDACTED, groups, nil).ListGroups(context.Background())
...
func TestListPlazaGroups_InactiveChannelSkipped(t *testing.T) {
	...
	groups := []Group{{ID: 10, Name: "g", Platform: "anthropic", RateMultiplier: 1REDACTEDREDACTED
	svc := newPlazaService([]Channel{inactiveREDACTED, groups, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	require.Empty(t, out)
REDACTED
...
REDACTED
	...
	svc.billingService = NewBillingService(&config.Config{REDACTED, pricingSvc)
...
REDACTED
	svc := NewModelPlazaService(repo, &stubGroupRepoForAvailable{REDACTED, nil, nil, nil)
	out, err := svc.ListGroups(context.Background())
	require.Nil(t, out)
...
REDACTED
	...
	return NewModelPlazaService(repo, &stubGroupRepoForAvailable{activeGroups: groupsREDACTED, catalog, bs, NewModelPricingResolver(cs, bs))
REDACTED
...
func TestListGroups_TokenLadderFollowsGroupToggle(t *testing.T) {
	...
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10, 20REDACTED,
		ModelPricing: []ChannelModelPricing{{Platform: PlatformOpenAI, Models: []string{"gpt-5.4"REDACTED, BillingMode: BillingModeTokenREDACTEDREDACTED,
REDACTEDREDACTED
	groups := []Group{
		{ID: 10, Name: "on", Platform: PlatformOpenAI, RateMultiplier: 1, LongContextPricingEnabled: trueREDACTED,
		{ID: 20, Name: "off", Platform: PlatformOpenAI, RateMultiplier: 2, LongContextPricingEnabled: falseREDACTED,
REDACTED
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformOpenAI, 20: PlatformOpenAIREDACTED, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	require.Len(t, out, 2)
	...
	for _, m := range []PlazaModel{onModel, offModelREDACTED {
...
func TestListGroups_GeminiLegacyRuleShownAsMarginal(t *testing.T) {
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10REDACTED,
		ModelMapping: map[string]map[string]string{PlatformGemini: {"gemini-2.5-pro": "gemini-2.5-pro"REDACTEDREDACTED,
REDACTEDREDACTED
	groups := []Group{{ID: 10, Name: "g", Platform: PlatformGemini, RateMultiplier: 1, LongContextPricingEnabled: trueREDACTEDREDACTED
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformGeminiREDACTED, geminiCatalogStub())
	out, err := svc.ListGroups(context.Background())
REDACTED
	require.Len(t, out, 1)
	m := out[0].Models[0]
...
func TestListGroups_GroupTokenCardOverridesChannelPricing(t *testing.T) {
	channels := []Channel{plazaPricedChannel(1, "ch", []int64{10REDACTED, PlatformAnthropic, "claude-sonnet-4")REDACTED
	groups := []Group{{
		ID: 10, Name: "g", Platform: PlatformAnthropic, RateMultiplier: 1, LongContextPricingEnabled: true,
		ModelPricing: []ChannelModelPricing{{Models: []string{"claude-sonnet-*"REDACTED, BillingMode: BillingModeToken, InputPrice: testPtrFloat64(1e-6)REDACTEDREDACTED,
REDACTEDREDACTED
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformAnthropicREDACTED, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	m := out[0].Models[0]
...
func TestListGroups_ImageModelKeepsTierSynthesisWithBilling(t *testing.T) {
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10REDACTED,
		ModelPricing: []ChannelModelPricing{{
			Platform: PlatformOpenAI, Models: []string{"gpt-image-2"REDACTED, BillingMode: BillingModeImage,
...
REDACTEDREDACTED
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformOpenAIREDACTED, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	m := out[0].Models[0]
...
func TestListGroups_CatalogMissingStillShowsChannelFlatPricing(t *testing.T) {
	...
	channels := []Channel{plazaPricedChannel(1, "ch", []int64{10REDACTED, PlatformAnthropic, "unknown-model-xyz")REDACTED
	groups := []Group{{ID: 10, Name: "g", Platform: PlatformAnthropic, RateMultiplier: 1, LongContextPricingEnabled: trueREDACTEDREDACTED
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformAnthropicREDACTED, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	m := out[0].Models[0]
...
func TestListGroups_TimePricingPassthrough(t *testing.T) {
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10REDACTED,
		ModelPricing: []ChannelModelPricing{{
			Platform: PlatformDeepseek, Models: []string{"deepseek-chat"REDACTED, BillingMode: BillingModeToken,
...
REDACTED
REDACTEDREDACTED
	groups := []Group{{ID: 10, Name: "cn", Platform: PlatformDeepseek, RateMultiplier: 1, LongContextPricingEnabled: trueREDACTEDREDACTED
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformDeepseekREDACTED, nil)
	out, err := svc.ListGroups(context.Background())
REDACTED
	m := out[0].Models[0]
