...
func TestCyberPolicyNoticeTemplateWrapsLongUpstreamMessages(t *testing.T) {
	...
	for _, locale := range []string{"en", "zh"REDACTED {
		preview, err := svc.PreviewTemplate(ctx, NotificationEmailPreviewInput{
	...
	REDACTED
		...
		require.NotContains(t, preview.HTML, "{{upstream_messageREDACTEDREDACTED")
REDACTED
REDACTED
...
REDACTED
	...
	REDACTED
		for _, placeholder := range requiredPlaceholders {
			...
			require.Contains(t, tmpl.HTML, "{{"+placeholder+"REDACTEDREDACTED")
	REDACTED
	...
	REDACTED
		...
		require.NotContains(t, preview.HTML, "{{report_total_requestsREDACTEDREDACTED")
REDACTED
REDACTED
...
func TestOpsScheduledReportRuntimeVariablesDoNotLeakPreviewSamples(t *testing.T) {
	...
	variables := svc.runtimeVariables(ctx, NotificationEmailEventOpsScheduledReport, "en", NotificationEmailSendInput{REDACTED)
...
REDACTED
	...
	rendered, err := renderNotificationEmail(
		...
		"Report",
		`<div style="display: {{report_detail_displayREDACTEDREDACTED;">{{report_htmlREDACTEDREDACTED</div>`,
		variables,
		nil,
	)
REDACTED
