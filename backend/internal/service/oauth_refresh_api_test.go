...
func (e *refreshAPIExecutorStub) CanRefresh(_ *Account) bool { return !e.cannotRefresh REDACTED
...
func TestRefreshIfNeeded_Success(t *testing.T) {
	...
	executor := &refreshAPIExecutorStub{
		needsRefresh: true,
		credentials:  map[string]any{"access_token": "new-token"REDACTED,
REDACTED
...
REDACTED
	...
	executor := &refreshAPIExecutorStub{
		needsRefresh: true,
		credentials:  map[string]any{"access_token": "safe-token"REDACTED,
REDACTED
...
func TestRefreshIfNeeded_LockErrorDegrades(t *testing.T) {
	...
	executor := &refreshAPIExecutorStub{
		needsRefresh: true,
		credentials:  map[string]any{"access_token": "degraded-token"REDACTED,
REDACTED
...
func TestRefreshIfNeeded_NoCacheNoLock(t *testing.T) {
	...
	executor := &refreshAPIExecutorStub{
		needsRefresh: true,
		credentials:  map[string]any{"access_token": "no-cache-token"REDACTED,
REDACTED
...
REDACTED
	...
	executor := &refreshAPIExecutorStub{
		needsRefresh: true,
		credentials:  map[string]any{"access_token": "fallback-token"REDACTED,
REDACTED
...
func TestRefreshIfNeeded_DBRereadInactiveFailsClosed(t *testing.T) {
	account := &Account{ID: 82, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActiveREDACTED
...
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			account := &Account{ID: 83, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActiveREDACTED
			freshAccount := &Account{ID: account.ID, Platform: tt.freshPlatform, Type: tt.freshType, Status: StatusActiveREDACTED
			repo := &refreshAPIAccountRepo{account: freshAccountREDACTED
...
REDACTED
	...
	REDACTED,
REDACTED
	freshAccount := &Account{ID: account.ID, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActiveREDACTED
	repo := &refreshAPIAccountRepo{account: freshAccountREDACTED
	executor := NewGrokTokenRefresher(nil)
...
func TestMergeCredentials_Basic(t *testing.T) {
	old := map[string]any{"a": "1", "b": "2", "c": "3"REDACTED
	new := map[string]any{"a": "new", "d": "4"REDACTED
...
func TestMergeCredentials_NilNew(t *testing.T) {
	old := map[string]any{"a": "1"REDACTED
...
func TestMergeCredentials_NilOld(t *testing.T) {
	new := map[string]any{"a": "1"REDACTED
...
func TestMergeCredentials_NewOverridesOld(t *testing.T) {
	old := map[string]any{"access_token": "old-token", "refresh_token": "old-refresh"REDACTED
	new := map[string]any{"access_token": "new-token"REDACTED
...
REDACTED
	repo := &refreshAPIAccountRepoWithRace{
		refreshAPIAccountRepo: refreshAPIAccountRepo{account: accountREDACTED,
...
func (e *dynamicRefreshExecutor) CanRefresh(_ *Account) bool { return e.canRefresh REDACTED
