...
func TestOpenAI429FastPath_KeepsOAuthAccountSchedulableDuringRetryWindow(t *testing.T) {
	repo := &oauth429RateLimitRepo{REDACTED
	...
	setupTokenAccount := &Account{ID: 44, Platform: PlatformOpenAI, Type: AccountTypeSetupTokenREDACTED
	grokOAuthAccount := &Account{ID: 45, Platform: PlatformGrok, Type: AccountTypeOAuthREDACTED
...
func TestOpenAI429FastPath_BlocksOAuthOnlyAfterRetryWindow(t *testing.T) {
	...
	account := &Account{ID: 420, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	svc.openaiOAuth429RetryStartedAt.Store(account.ID, time.Now().Add(-openAIOAuth429RetryWindow-time.Second))

	svc.markOpenAIOAuth429RateLimited(context.Background(), account, http.Header{REDACTED, nil)
...
func TestOpenAIStream429IgnoresSuccessfulQuotaSnapshotHeaders(t *testing.T) {
	...
	svc := &OpenAIGatewayService{rateLimitService: rateLimitsREDACTED
	rateLimits.SetAccountRuntimeBlocker(svc)
	account := &Account{ID: 421, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	svc.openaiOAuth429RetryStartedAt.Store(account.ID, time.Now().Add(-openAIOAuth429RetryWindow-time.Second))
...
func TestOpenAIHTTP429StillUsesQuotaResetHeaders(t *testing.T) {
	svc := &OpenAIGatewayService{rateLimitService: &RateLimitService{REDACTEDREDACTED
	account := &Account{ID: 422, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	svc.openaiOAuth429RetryStartedAt.Store(account.ID, time.Now().Add(-openAIOAuth429RetryWindow-time.Second))
	headers := http.Header{REDACTED
	headers.Set("x-codex-primary-used-percent", "37")
	headers.Set("x-codex-primary-reset-after-seconds", "604800")
...
func TestOpenAI429RetryDelayHonorsBoundedRetryAfter(t *testing.T) {
	...
	require.Equal(t, openAIOAuth429MaxRetryDelay, openAIOAuth429SameAccountRetryDelay(http.Header{"Retry-After": []string{"90"REDACTEDREDACTED, deadline))
REDACTED
...
REDACTED
			"pool_mode":                    true,
			"pool_mode_retry_status_codes": []any{float64(524)REDACTED,
	REDACTED,
REDACTED
	...
	for i := 0; i < 2; i++ {
		shouldDisable := gateway.handleOpenAIAccountUpstreamError(
			...
			[]byte(`{"error":{"message":"upstream timeout"REDACTEDREDACTED`),
			"gpt-5.4",
		)
		require.False(t, shouldDisable)
REDACTED
...
REDACTED
			"pool_mode":                    true,
			"pool_mode_retry_status_codes": []any{float64(http.StatusGatewayTimeout)REDACTED,
	REDACTED,
REDACTED
	...
	for i := 0; i < 2; i++ {
		shouldDisable := gateway.handleOpenAIAccountUpstreamError(
			...
			http.Header{REDACTED,
			[]byte(`{"error":{"message":"upstream unavailable"REDACTEDREDACTED`),
			"gpt-5.4",
		)
		require.False(t, shouldDisable)
REDACTED
