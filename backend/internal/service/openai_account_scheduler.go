...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if shouldClearStickySession(account, req.RequestedModel) || !account.IsOpenAICompatibleGateway() || !account.IsSchedulable() {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if !account.IsSchedulable() || !account.IsOpenAICompatibleGateway() {
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatible(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) bool {
	...
	if account.Platform != openAICompatibleGatewayPlatform(ctx) {
		return false
	}
