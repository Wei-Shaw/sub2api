...
type openAISelectionProbeBudget struct {
	acquires  int
	rechecks  int
	attempted map[int64]struct{REDACTED
	limited   bool
REDACTED

func newOpenAISelectionProbeBudget() *openAISelectionProbeBudget {
	return &openAISelectionProbeBudget{attempted: make(map[int64]struct{REDACTED)REDACTED
REDACTED
...
REDACTED
	if b.attempted == nil {
		b.attempted = make(map[int64]struct{REDACTED)
REDACTED
	b.acquires++
	b.attempted[accountID] = struct{REDACTED{REDACTED
	return true
REDACTED
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	REDACTED
		...
		if req.StickyWeighted {
			for _, stickyID := range []int64{req.StickyPreviousAccountID, req.StickyAccountIDREDACTED {
				if stickyID <= 0 {
					continue
			REDACTED
				for i, candidate := range ranked {
					if candidate.account != nil && candidate.account.ID == stickyID {
						primary = append([]openAIAccountCandidateScore{candidateREDACTED, ranked[:i]...)
	...
	REDACTED
		...
		selected := make(map[int64]struct{REDACTED, len(primary))
		for _, candidate := range primary {
			selected[candidate.account.ID] = struct{REDACTED{REDACTED
	REDACTED
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	if !req.StickyWeighted {
		return nil, nil
REDACTED
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountIDREDACTED {
		if accountID <= 0 {
			continue
	REDACTED
		if req.ExcludedIDs != nil {
...
REDACTED else if stickyFallback != nil {
...
REDACTED
	...
	REDACTED
		...
		lbTopKOverride := 0
		weightOverrides := map[string]float64{REDACTED
...
func openAIAdvancedSchedulerWeightOverrideSpecs() []openAIAdvancedSchedulerWeightOverrideSpec {
	return []openAIAdvancedSchedulerWeightOverrideSpec{
		{key: SettingKeyOpenAIAdvancedSchedulerWeightPriority, name: "priority"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightLoad, name: "load"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightQueue, name: "queue"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightErrorRate, name: "error_rate"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightTTFT, name: "ttft"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightReset, name: "reset"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightQuotaHeadroom, name: "quota_headroom"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightUpstreamCost, name: "upstream_cost"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightPreviousResponse, name: "previous_response"REDACTED,
		{key: SettingKeyOpenAIAdvancedSchedulerWeightSessionSticky, name: "session_sticky"REDACTED,
REDACTED
REDACTED
...
func parseOpenAIAdvancedSchedulerWeightOverrides(values map[string]string) map[string]float64 {
	overrides := map[string]float64{REDACTED
	for _, spec := range openAIAdvancedSchedulerWeightOverrideSpecs() {
...
type openAIGroupPrivacyRequirementContextKey struct{REDACTED
...
func (s *OpenAIGatewayService) withOpenAIGroupPrivacyRequirement(ctx context.Context, groupID *int64) context.Context {
	return context.WithValue(ctx, openAIGroupPrivacyRequirementContextKey{REDACTED, openAIGroupPrivacyRequirement{
...
func (s *OpenAIGatewayService) openAIGroupRequiresPrivacySet(ctx context.Context, groupID *int64) bool {
	if cached, ok := ctx.Value(openAIGroupPrivacyRequirementContextKey{REDACTED).(openAIGroupPrivacyRequirement); ok && cached.groupID == derefGroupID(groupID) {
...
REDACTED
	...
	if scheduler == nil {
		...
		REDACTED
			fallbackScheduler := &defaultOpenAIAccountScheduler{service: s, stats: newOpenAIAccountRuntimeStats()REDACTED
...
REDACTED
	...
	REDACTED else if len(observedErr) > 0 && observedErr[0] != nil {
...
func (s *RateLimitService) BuildOpenAIAccountSchedulerScoreSnapshot(
	...
	gateway := &OpenAIGatewayService{cfg: nil, rateLimitService: sREDACTED
	if s != nil {
		gateway.cfg = s.cfg
REDACTED
...
REDACTED
	...
	REDACTED
		...
		if rate, ok := openAISchedulingRate(account, now, oauthSchedulingRateMultiplier); ok {
			samples = append(samples, rateSample{accountID: account.ID, rate: rateREDACTED)
	REDACTED
REDACTED
...
func newOpenAILegacyUpstreamRateOrder(accounts []*Account, now time.Time, oauthSchedulingRateMultiplier float64) openAILegacyUpstreamRateOrder {
	...
	REDACTED
		if len(rates) == 0 {
			first = rate
	REDACTED else if rate != first {
			distinct = true
	REDACTED
		rates[account.ID] = rate
REDACTED
	return openAILegacyUpstreamRateOrder{enabled: len(rates) >= 2 && distinct, rates: ratesREDACTED
REDACTED
