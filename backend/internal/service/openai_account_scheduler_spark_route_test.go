...
func TestSparkRoutingByModel(t *testing.T) {
	...
	sparkCreds := map[string]any{"model_mapping": defaultSparkShadowModelMapping()REDACTED
	...
	newScheduler := func(snapshot map[int64]*Account) *defaultOpenAIAccountScheduler {
		return &defaultOpenAIAccountScheduler{service: &OpenAIGatewayService{
			schedulerSnapshot: &SchedulerSnapshotService{
				cache: &openAISnapshotCacheStub{accountsByID: snapshotREDACTED,
		REDACTED,
			cfg: &config.Config{REDACTED,
	REDACTEDREDACTED
REDACTED
	sparkReq := OpenAIAccountScheduleRequest{RequestedModel: sparkModel, Platform: PlatformOpenAIREDACTED
	normalReq := OpenAIAccountScheduleRequest{RequestedModel: normalModel, Platform: PlatformOpenAIREDACTED

	t.Run("normal_account_with_spark_mapping_accepts_spark", func(t *testing.T) {
		acc := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Credentials: sparkCredsREDACTED
...
REDACTED)
	...
	REDACTED"model_mapping": map[string]any{normalModel: normalModelREDACTEDREDACTEDREDACTED
...
REDACTED)
	...
	t.Run("shadow_with_spark_mapping_accepts_spark_rejects_non_spark", func(t *testing.T) {
		pid := int64(100)
		parent := &Account{ID: 100, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: trueREDACTED
		shadow := &Account{ID: 200, ParentAccountID: &pid, QuotaDimension: QuotaDimensionSpark,
			Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Credentials: sparkCredsREDACTED
		s := newScheduler(map[int64]*Account{100: parentREDACTED)
...
REDACTED)
	...
	t.Run("empty_model_shadow_is_eligible_under_a2", func(t *testing.T) {
		...
		shadow := &Account{ID: 200, ParentAccountID: &pid, QuotaDimension: QuotaDimensionSpark,
			Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Credentials: sparkCredsREDACTED
		emptyReq := OpenAIAccountScheduleRequest{RequestedModel: "", Platform: PlatformOpenAIREDACTED
		s := newScheduler(map[int64]*Account{100: parentREDACTED)
...
REDACTED
	...
	makeScheduler := func(parent *Account) *defaultOpenAIAccountScheduler {
		snapshotCache := &openAISnapshotCacheStub{
			accountsByID: map[int64]*Account{parent.ID: parentREDACTED,
	REDACTED
		snapshotSvc := &SchedulerSnapshotService{cache: snapshotCacheREDACTED
		svc := &OpenAIGatewayService{
			schedulerSnapshot: snapshotSvc,
			cfg:               &config.Config{REDACTED,
	REDACTED
		return &defaultOpenAIAccountScheduler{service: svcREDACTED
REDACTED
...
REDACTED
	...
	repo := schedulerTestOpenAIAccountRepo{accounts: []Account{parentREDACTEDREDACTED
	scheduler := &defaultOpenAIAccountScheduler{service: &OpenAIGatewayService{
		accountRepo: repo,
		schedulerSnapshot: &SchedulerSnapshotService{
			cache:       &openAISnapshotCacheStub{REDACTED,
			accountRepo: repo,
			cfg: &config.Config{
...
REDACTEDREDACTED
	...
	require.True(t, scheduler.isAccountRequestCompatible(ctx, shadow, OpenAIAccountScheduleRequest{
		...
		Platform:       PlatformOpenAI,
REDACTED), "快照缺失母账号且调度快照 DB fallback 关闭时，应回退 repo 解析健康母账号")
REDACTED
