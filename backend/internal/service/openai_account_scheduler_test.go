...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyOutsideGroupFallsBack(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10)
	accounts := []Account{
		{
	...
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{
			"openai:session_hash_cross_group": 2001,
		},
	}
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(2002), selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.False(t, decision.StickySessionHit)
	require.Equal(t, 1, cache.deletedSessions["openai:session_hash_cross_group"])
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}
