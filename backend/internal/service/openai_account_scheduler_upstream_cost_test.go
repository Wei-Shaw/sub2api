...
REDACTED
	...
	order := newOpenAILegacyUpstreamRateOrder([]*Account{grokCheap, anthropicExpensive, nilREDACTED, now, defaultOpenAIOAuthSchedulingRateMultiplier)
	require.False(t, order.enabled)
	require.Empty(t, order.rates)
	require.Zero(t, order.compare(grokCheap, anthropicExpensive))

	factors := openAIUpstreamCostFactors([]*Account{grokCheap, anthropicExpensiveREDACTED, now, defaultOpenAIOAuthSchedulingRateMultiplier)
	...
	mixed := newOpenAILegacyUpstreamRateOrder(
		[]*Account{grokCheap, openAICheap, anthropicExpensive, openAIExpensiveREDACTED,
