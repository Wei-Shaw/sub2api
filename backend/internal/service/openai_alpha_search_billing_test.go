...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, err := svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"REDACTED, 3.0, 1.0, 1.0, 2.0, UsageTokens{REDACTED, "", boolPtr(false), time.Time{REDACTED)
REDACTED
	require.Equal(t, string(BillingModePerRequest), cost.BillingMode)
	require.InDelta(t, 0.01, cost.TotalCost, 1e-12)
	...
	cost, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"REDACTED, 1.0, 1.0, 1.0, 1.0, UsageTokens{REDACTED, "", boolPtr(false), time.Time{REDACTED)
REDACTED
	...
	_, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"REDACTED, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10REDACTED, "", boolPtr(false), time.Time{REDACTED)
REDACTED
REDACTED
...
func TestAPIKeyService_SnapshotRoundTrip_PreservesWebSearchPricePerCall(t *testing.T) {
	...
	apiKey := &APIKey{
		...
		Key:     "k-websearch",
		Status:  StatusActive,
		User:    &User{ID: 2, Status: StatusActive, Role: RoleUserREDACTED,
		Group: &Group{
			ID:                    groupID,
			Name:                  "openai",
