...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, err := svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"REDACTED, 3.0, 1.0, 1.0, 2.0, UsageTokens{REDACTED, "", boolPtr(false))
REDACTED
	require.Equal(t, string(BillingModePerRequest), cost.BillingMode)
	require.InDelta(t, 0.01, cost.TotalCost, 1e-12)
	...
	cost, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"REDACTED, 1.0, 1.0, 1.0, 1.0, UsageTokens{REDACTED, "", boolPtr(false))
REDACTED
	...
	_, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"REDACTED, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10REDACTED, "", boolPtr(false))
REDACTED
REDACTED
