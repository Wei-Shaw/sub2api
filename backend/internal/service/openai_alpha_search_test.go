...
func alphaSearchResponsesSSE(output string) string {
	return "event: response.output_text.delta\n" +
		`data: {"type":"response.output_text.delta","delta":` + strconv.Quote(output) + `REDACTED` + "\n\n" +
		"event: response.output_text.annotation.added\n" +
		`data: {"type":"response.output_text.annotation.added","annotation":{"type":"url_citation","url":"https://example.com/news","title":"Example News"REDACTEDREDACTED` + "\n\n" +
		"event: response.completed\n" +
		`data: {"type":"response.completed","response":{"output":[{"type":"message","content":[{"type":"output_text","text":` + strconv.Quote(output) + `REDACTED]REDACTED]REDACTEDREDACTED` + "\n\n"
REDACTED
...
func TestForwardAlphaSearchOAuthPreservesWire(t *testing.T) {
	...
	body := []byte(`{
		...
		"input":[{"type":"message","role":"user","content":[{"type":"input_text","text":"latest news"REDACTED]REDACTED],
		"commands":{"search_query":[{"q":"OpenAI news","recency":1REDACTED]REDACTED,
		"settings":{"allowed_callers":["direct"],"external_web_access":trueREDACTED,
		"max_output_tokens":2000,
		"future_field":{"keep":trueREDACTED
REDACTED`)
	...
	c.Request.Header.Set("X-Codex-Turn-Metadata", `{"session_id":"search-session","turn_id":"search-turn"REDACTED`)

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"REDACTEDREDACTED,
		Body:       io.NopCloser(strings.NewReader(`{"encrypted_output":"ciphertext","output":"search result"REDACTED`)),
REDACTEDREDACTED
...
REDACTED
	...
	require.JSONEq(t, `{"encrypted_output":"ciphertext","output":"search result"REDACTED`, recorder.Body.String())
...
func TestForwardAlphaSearchPATUsesResponsesWebSearchFallback(t *testing.T) {
	...
	body := []byte(`{
		"id":"search-session",
		"model":"gpt-5.6-sol",
		"commands":{"search_query":[{"q":"OpenAI news"REDACTED]REDACTED,
...
REDACTED`)
	...
	c.Request.Header.Set("X-Codex-Turn-Metadata", `{"turn_id":"turn-1"REDACTED`)

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"REDACTED, "x-request-id": []string{"req-search"REDACTEDREDACTED,
...
REDACTED
	...
	require.Equal(t, http.StatusOK, recorder.Code)
	require.JSONEq(t, `{"output":"search result","results":[{"type":"text_result","ref_id":"turn0search0","url":"https://example.com/news","title":"Example News"REDACTED]REDACTED`, recorder.Body.String())
...
func TestForwardAlphaSearchPATBackfillsMissingChatGPTAccountMetadata(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"id":"search-session","model":"gpt-5.6-sol","commands":{"search_query":[{"q":"OpenAI news"REDACTED]REDACTEDREDACTED`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/alpha/search", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
...
REDACTED))
	...
	defer func() { openAICodexPATWhoamiURL = oldWhoamiURL REDACTED()

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"REDACTEDREDACTED,
		Body:       io.NopCloser(strings.NewReader(`{"output":"search result"REDACTED`)),
REDACTEDREDACTED
...
func TestForwardAlphaSearchAPIKeyMapsModelAndPassesThroughError(t *testing.T) {
	...
	upstreamBody := `{"error":{"type":"invalid_request_error","message":"bad search"REDACTEDREDACTED`
	upstream := &httpUpstreamRecorder{resp: &http.Response{
...
func TestForwardAlphaSearchUnauthorizedDoesNotMarkAccountError(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Content-Type": []string{"application/json"REDACTEDREDACTED,
		Body:       io.NopCloser(strings.NewReader(`{"detail":"Unauthorized"REDACTED`)),
REDACTEDREDACTED
	repo := &alphaSearchAccountStateRepo{REDACTED
	cfg := &config.Config{REDACTED
	service := &OpenAIGatewayService{
		cfg:              cfg,
		httpUpstream:     upstream,
...
func TestForwardAlphaSearchOAuthNotFoundPassesThrough(t *testing.T) {
	...
	upstreamBody := `{"detail":"Not Found"REDACTED`
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusNotFound,
		Header:     http.Header{"Content-Type": []string{"application/json"REDACTEDREDACTED,
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
