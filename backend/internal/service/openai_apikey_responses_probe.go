...
func (s *AccountTestService) ProbeOpenAIAPIKeyResponsesSupport(ctx context.Context, accountID int64) {
	...
	if account.IsCNProvider() {
		// Explicit Responses and adaptive accounts on providers with native
		// Responses support use the configured endpoint without a network probe.
		// Chat-only providers remain on the existing conversion path.
