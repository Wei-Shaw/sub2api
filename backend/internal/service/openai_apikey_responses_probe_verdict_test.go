...
REDACTED{
		{
			...
			body: `{"status":"incomplete","incomplete_details":{"reason":"max_output_tokens"REDACTED,` +
				`"output":[{"type":"reasoning","summary":[]REDACTED]REDACTED`,
	REDACTED,
		{
			// HTTP 200 携带的失败响应是上游瞬时故障，不构成工具能力证据。
			name: "failed_status_on_http_200",
			body: `{"status":"failed","error":{"code":"server_error","message":"upstream hiccup"REDACTED,"output":[]REDACTED`,
	REDACTED,
REDACTED
...
REDACTED{
		{
			...
			body:   `{"status":"completed","output":[{"type":"function_call","name":"probe_ping"REDACTED]REDACTED`,
			want:   true,
	REDACTED,
		{
			...
			status: http.StatusOK,
			body:   `{"status":"completed","output":[{"type":"reasoning"REDACTED]REDACTED`,
			want:   false,
	REDACTED,
		{
			...
			status: http.StatusOK,
			body:   `{"status":"incomplete","incomplete_details":{"reason":"content_filter"REDACTED,"output":[]REDACTED`,
			want:   false,
	REDACTED,
		{
			...
			status: http.StatusOK,
			body:   `{"output":[{"type":"function_call","name":"probe_ping"REDACTED]REDACTED`,
			want:   true,
	REDACTED,
		{
			name:   "endpoint_absent_404",
			status: http.StatusNotFound,
			body:   `{"error":{"message":"Not Found"REDACTEDREDACTED`,
			want:   false,
	REDACTED,
		{
...
func TestResponsesProbeVerdictIsConclusive(t *testing.T) {
	cases := []struct {
		...
		want   bool
REDACTED{
		{"200_completed", 200, `{"status":"completed","output":[]REDACTED`, trueREDACTED,
		{"200_incomplete_max_output_tokens", 200,
			`{"status":"incomplete","incomplete_details":{"reason":"max_output_tokens"REDACTEDREDACTED`, falseREDACTED,
		{"200_incomplete_content_filter", 200,
			`{"status":"incomplete","incomplete_details":{"reason":"content_filter"REDACTEDREDACTED`, trueREDACTED,
		{"200_incomplete_without_reason", 200, `{"status":"incomplete"REDACTED`, trueREDACTED,
		{"200_failed", 200, `{"status":"failed"REDACTED`, falseREDACTED,
		{"200_no_status_field", 200, `{"output":[]REDACTED`, trueREDACTED,
		{"200_non_json", 200, `not-json`, trueREDACTED,
		{"200_empty_body", 200, ``, trueREDACTED,
		// 非 2xx 只看状态码，不读 body。
		{"404_ignores_body_status", 404, `{"status":"failed"REDACTED`, trueREDACTED,
		{"500_ignores_body_status", 500, `{"status":"incomplete","incomplete_details":{"reason":"max_output_tokens"REDACTEDREDACTED`, trueREDACTED,
REDACTED
