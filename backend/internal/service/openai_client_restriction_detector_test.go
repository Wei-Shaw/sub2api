	...
	t.Run("配置 claude_code 白名单且命中真实签名时放行", func(t *testing.T) {
		...
		account := &Account{
			...
			Extra: map[string]any{
				"codex_cli_only":                 true,
				"codex_cli_only_allowed_clients": []any{"claude_code"REDACTED,
		REDACTED,
	REDACTED
...
REDACTED)
	...
	t.Run("全局列表含 claude_code + 非签名 → 403", func(t *testing.T) {
		...
		account := &Account{
			...
			Extra:    map[string]any{"codex_cli_only": trueREDACTED,
	REDACTED
		result := detector.Detect(newCodexDetectorTestContext("curl/8.0", "my_client"), account, []string{"claude_code"REDACTED)
		require.True(t, result.Enabled)
		require.False(t, result.Matched)
		require.Equal(t, CodexClientRestrictionReasonNotMatchedUA, result.Reason)
REDACTED)
