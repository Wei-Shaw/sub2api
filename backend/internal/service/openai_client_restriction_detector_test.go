...
func TestOpenAICodexClientRestrictionDetector_Detect(t *testing.T) {
	...
	t.Run("未开启开关时绕过", func(t *testing.T) {
		...
		account := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{REDACTEDREDACTED

		result := detector.Detect(newCodexDetectorTestContext("curl/8.0", ""), account, CodexRestrictionPolicy{REDACTED, nil)
...
REDACTED)
	...
	REDACTED
		...
		result := detector.Detect(newCodexDetectorTestContext("codex_cli_rs/0.99.0", ""), account, CodexRestrictionPolicy{REDACTED, nil)
...
REDACTED)
	...
	REDACTED
		...
		result := detector.Detect(newCodexDetectorTestContext("codex_vscode/1.0.0", ""), account, CodexRestrictionPolicy{REDACTED, nil)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
		require.Equal(t, CodexClientRestrictionReasonMatchedUA, result.Reason)
REDACTED)
	...
	REDACTED
		...
		result := detector.Detect(newCodexDetectorTestContext("codex_app/2.1.0", ""), account, CodexRestrictionPolicy{REDACTED, nil)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
		require.Equal(t, CodexClientRestrictionReasonMatchedUA, result.Reason)
REDACTED)
	...
	REDACTED
		...
		result := detector.Detect(newCodexDetectorTestContext("myterm/0.141.0", "codex_chatgpt_desktop"), account, CodexRestrictionPolicy{REDACTED, nil)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
...
REDACTED)
	...
	REDACTED
		...
		result := detector.Detect(newCodexDetectorTestContext("curl/8.0", "my_client"), account, CodexRestrictionPolicy{REDACTED, nil)
		require.True(t, result.Enabled)
		require.False(t, result.Matched)
...
REDACTED)
	...
	t.Run("开启 ForceCodexCLI 时允许通过", func(t *testing.T) {
		detector := NewOpenAICodexClientRestrictionDetector(&config.Config{
			Gateway: config.GatewayConfig{ForceCodexCLI: trueREDACTED,
	REDACTED)
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeOAuth,
...
func TestOpenAICodexClientRestrictionDetector_Detect_AllowedClients(t *testing.T) {
	...
	REDACTED
		...
		result := detector.Detect(newCodexDetectorTestContext(claudeCodeUA, claudeCodeOriginator), account, CodexRestrictionPolicy{REDACTED, nil)
		require.True(t, result.Enabled)
		require.False(t, result.Matched)
		require.Equal(t, CodexClientRestrictionReasonNotMatchedUA, result.Reason)
REDACTED)
...
REDACTED)
	...
	REDACTED
		result := detector.Detect(
			...
			account,
			CodexRestrictionPolicy{Whitelist: []openai.AllowedClientEntry{{Originator: "Claude Code", UAContains: []string{"Claude Code/"REDACTEDREDACTEDREDACTEDREDACTED,
			nil,
		)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
...
REDACTED)
	...
	REDACTED
		result := detector.Detect(
			...
			account,
			CodexRestrictionPolicy{REDACTED,
			nil,
		)
		require.True(t, result.Enabled)
		require.False(t, result.Matched)
...
func TestDetect_V3_AppServerAndSkipAndVersionScope(t *testing.T) {
	gin.SetMode(gin.TestMode)
	acc := func() *Account {
	REDACTEDPlatform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"codex_cli_only": trueREDACTEDREDACTED
REDACTED

	t.Run("AppServer OFF：未列名客户端拒", func(t *testing.T) {
		d := NewOpenAICodexClientRestrictionDetector(nil)
		r := d.Detect(newCodexDetectorTestContext("opencode/1.0", "opencode"), acc(), CodexRestrictionPolicy{REDACTED, nil)
...
REDACTED)
	...
	t.Run("AppServer ON + 引擎头 → 放行(app_server)", func(t *testing.T) {
		...
		r := d.Detect(c, acc(), CodexRestrictionPolicy{AllowAppServerClients: true, EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsREDACTED, nil)
...
REDACTED)
	...
	t.Run("AppServer ON + 无引擎头 + strict → 拒", func(t *testing.T) {
		d := NewOpenAICodexClientRestrictionDetector(nil)
		r := d.Detect(newCodexDetectorTestContext("opencode/1.0", "opencode"), acc(), CodexRestrictionPolicy{AllowAppServerClients: true, EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsREDACTED, nil)
		require.False(t, r.Matched)
...
REDACTED)
	...
	t.Run("白名单 skip=true + 无引擎头 + strict → 放行", func(t *testing.T) {
		...
		pol := CodexRestrictionPolicy{
			Whitelist: []openai.AllowedClientEntry{{Originator: "opencode", UAContains: []string{"opencode/"REDACTED, SkipEngineFingerprint: trueREDACTEDREDACTED,
	REDACTED
...
REDACTED)
	...
	t.Run("白名单 skip=false + 无引擎头 + strict → 拒", func(t *testing.T) {
		...
		pol := CodexRestrictionPolicy{
			...
			Whitelist:                []openai.AllowedClientEntry{{Originator: "opencode", UAContains: []string{"opencode/"REDACTEDREDACTEDREDACTED,
	REDACTED
		r := d.Detect(newCodexDetectorTestContext("opencode/1.0", "opencode"), acc(), pol, nil)
		require.False(t, r.Matched)
		require.Equal(t, CodexClientRestrictionReasonMissingEngineFingerprint, r.Reason)
...
REDACTED)
	...
	t.Run("版本门仍卡官方：官方 originator 无版本 → VersionUndetectable", func(t *testing.T) {
		d := NewOpenAICodexClientRestrictionDetector(nil)
		r := d.Detect(newCodexDetectorTestContext("noversion", "codex_cli_rs"), acc(), CodexRestrictionPolicy{REDACTED, nil)
		require.False(t, r.Matched)
...
REDACTED
	...
	t.Run("版本太低：携带 DetectedVersion + MinCodexVersion", func(t *testing.T) {
		...
		r := d.Detect(c, acc(), CodexRestrictionPolicy{MinCodexVersion: "0.42.0"REDACTED, nil)
		require.False(t, r.Matched)
...
REDACTED)
	...
	t.Run("版本太高：携带 DetectedVersion + MaxCodexVersion", func(t *testing.T) {
		...
		r := d.Detect(c, acc(), CodexRestrictionPolicy{MaxCodexVersion: "0.42.0"REDACTED, nil)
		require.False(t, r.Matched)
...
func TestDetect_EngineFingerprintSignals(t *testing.T) {
	gin.SetMode(gin.TestMode)
	det := NewOpenAICodexClientRestrictionDetector(&config.Config{REDACTED)
	acct := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"codex_cli_only": trueREDACTEDREDACTED
	...
	t.Run("body通道: 勾body_path后 仅body命中 → 放行", func(t *testing.T) {
		bodyPolicy := CodexRestrictionPolicy{
			EngineFingerprintSignals: []openai.EngineFingerprintSignal{
				{Type: openai.FingerprintSignalBodyPath, Match: []string{"client_metadata.x-codex-window-id"REDACTED, Required: trueREDACTED,
		REDACTED,
	REDACTED
		c := newCodexDetectorTestContext(officialUA, "")
		got := det.Detect(c, acct, bodyPolicy, []byte(`{"client_metadata":{"x-codex-window-id":"c3"REDACTEDREDACTED`))
		require.True(t, got.Matched)
REDACTED)
REDACTED
...
func TestDetect_AccountAppServerToggle(t *testing.T) {
	...
	d := NewOpenAICodexClientRestrictionDetector(nil)
	acctOn := func() *Account {
	REDACTEDPlatform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"codex_cli_only": true, "codex_cli_only_allow_app_server": trueREDACTEDREDACTED
REDACTED
...
REDACTED
	withFP := func(ua, originator string) *gin.Context {
		...
		return c
REDACTED
	defaultSignals := CodexRestrictionPolicy{EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsREDACTED
...
REDACTED)
	...
	t.Run("账号 app-server OFF + 全局 ON → 放行(OR)", func(t *testing.T) {
		pol := CodexRestrictionPolicy{AllowAppServerClients: true, EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsREDACTED
