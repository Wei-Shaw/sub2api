...
func TestIsRetryableCodexModelsManifestTransportError(t *testing.T) {
	tests := []struct {
		...
		retryable bool
REDACTED{
		{name: "nil", err: nilREDACTED,
		{name: "configuration error", err: errors.New("invalid proxy URL")REDACTED,
		{name: "upstream configuration error", err: errors.New("upstream error: invalid proxy")REDACTED,
		{name: "proxy connection configuration error", err: errors.New("proxy connection error: invalid configuration")REDACTED,
		{name: "canceled request", err: context.CanceledREDACTED,
		{
		...
		REDACTED,
	REDACTED,
		{name: "deadline exceeded", err: context.DeadlineExceeded, retryable: trueREDACTED,
		{name: "unexpected EOF", err: io.ErrUnexpectedEOF, retryable: trueREDACTED,
		{name: "closed connection", err: net.ErrClosed, retryable: trueREDACTED,
		{
	...
	REDACTED,
		{
			name:      "DNS error",
			err:       &net.DNSError{Err: "temporary failure", Name: "upstream.example"REDACTED,
			retryable: true,
	REDACTED,
		{
			name:      "typed HTTP2 GOAWAY",
			err:       http2.GoAwayError{ErrCode: http2.ErrCodeNoREDACTED,
			retryable: true,
	REDACTED,
		{
...
REDACTED))
	...
	if err != nil {
		t.Fatalf("FetchCodexModelsManifest returned error: %v", err)
REDACTED
	if string(manifest.Body) != `{"models":[]REDACTED` {
...
REDACTED
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(r.URL.Path, "/task/register") {
			registerCalls++
			_, _ = w.Write([]byte(`{"task_id":"task-models-new"REDACTED`))
			return
	REDACTED
		modelsCalls++
...
REDACTED))
	...
	chatgptCodexModelsURL = server.URL
	t.Cleanup(func() { chatgptCodexModelsURL = originalModelsURL REDACTED)
	originalAuthBase := openAIAgentIdentityAuthAPIBaseURL
	openAIAgentIdentityAuthAPIBaseURL = server.URL
	t.Cleanup(func() { openAIAgentIdentityAuthAPIBaseURL = originalAuthBase REDACTED)

	s := &OpenAIGatewayService{accountRepo: repoREDACTED
	manifest, err := s.FetchCodexModelsManifest(context.Background(), account, "0.137.0", "")
REDACTED
	require.Equal(t, `{"models":[]REDACTED`, string(manifest.Body))
...
func TestFetchCodexModelsManifestRejectsInvalidEnvelope(t *testing.T) {
	tests := []struct {
		...
		body string
REDACTED{
		{name: "OpenAI models list", body: `{"object":"list","data":[]REDACTED`REDACTED,
		{name: "invalid JSON", body: `{"models":`REDACTED,
		{name: "non-object", body: `[]`REDACTED,
		{name: "null object", body: `null`REDACTED,
		{name: "missing models", body: `{REDACTED`REDACTED,
		{name: "models object", body: `{"models":{REDACTEDREDACTED`REDACTED,
		{name: "models null", body: `{"models":nullREDACTED`REDACTED,
REDACTED
...
func TestFetchCodexModelsManifestAPIKeyDoesNotCacheInvalidEnvelope(t *testing.T) {
	...
	upstream := &codexModelsHTTPUpstreamStub{do: func(_ *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
		body := `{"object":"list","data":[]REDACTED`
		if calls.Add(1) > 1 {
			body = `{"models":[{"slug":"gpt-5.6"REDACTED]REDACTED`
	REDACTED
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
...
REDACTED
	if got, want := string(manifest.Body), `{"models":[{"slug":"gpt-5.6"REDACTED]REDACTED`; got != want {
...
func TestFetchCodexModelsManifestAPIKeyCacheBoundsEntriesAndBodySize(t *testing.T) {
	...
	upstream := &codexModelsHTTPUpstreamStub{do: func(req *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
		...
		if strings.Contains(req.URL.Host, "large") {
			body = `{"models":[],"padding":"` + strings.Repeat("x", (1<<20)+1) + `"REDACTED`
	REDACTED
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
...
func newCodexModels401TestService(repo AccountRepository) *OpenAIGatewayService {
	...
	s := &OpenAIGatewayService{rateLimitService: rateLimitServiceREDACTED
...
func TestFetchCodexModelsManifestOAuth401MarksAccountUnschedulable(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"detail":{"message":"invalid token"REDACTEDREDACTED`))
REDACTED))
	defer server.Close()
	...
	repo := &codexModelsAccountStateRepo{REDACTED
...
func TestFetchCodexModelsManifestOAuth401TokenRevokedDisablesAccount(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"error":{"code":"token_revoked","message":"token has been revoked"REDACTEDREDACTED`))
REDACTED))
	defer server.Close()
...
REDACTED
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"detail":"some non-task 401"REDACTED`))
REDACTED))
	defer server.Close()
...
func TestFetchCodexModelsManifestAPIKey401KeepsNoFailoverAndNoDisable(t *testing.T) {
	upstream := &codexModelsHTTPUpstreamStub{do: func(_ *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
		return &http.Response{
			...
			Header:     make(http.Header),
			Body:       io.NopCloser(strings.NewReader(`{"error":"invalid api key"REDACTED`)),
	REDACTED, nil
REDACTEDREDACTED

	repo := &codexModelsAccountStateRepo{REDACTED
	s := newCodexModelsAPIKeyTestService(upstream)
	s.rateLimitService = NewRateLimitService(repo, nil, &config.Config{REDACTED, nil, nil)
