...
REDACTED))
	...
	defer func() { openAICodexPATWhoamiURL = originalURL REDACTED()
...
REDACTED
	credentials := map[string]any{
		...
		"model_mapping":               map[string]any{"gpt-5": "gpt-5-codex"REDACTED,
		"chatgpt_account_is_fedramp":  true,
		"subscription_expires_at":     "2026-12-31T00:00:00Z",
		"openai_usage_channel_fields": []any{"custom"REDACTED,
REDACTED
	...
	require.Equal(t, map[string]any{"gpt-5": "gpt-5-codex"REDACTED, got["model_mapping"])
	require.Equal(t, true, got["chatgpt_account_is_fedramp"])
	require.Equal(t, "2026-12-31T00:00:00Z", got["subscription_expires_at"])
	require.Equal(t, []any{"custom"REDACTED, got["openai_usage_channel_fields"])
REDACTED
