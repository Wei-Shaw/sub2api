...
func TestEnsureCodexReasoningInclude(t *testing.T) {
	...
	body := map[string]any{"reasoning": map[string]any{"effort": "medium"REDACTEDREDACTED
	require.True(t, ensureCodexReasoningInclude(body))
	require.Equal(t, []any{"reasoning.encrypted_content"REDACTED, body["include"])
	// 幂等：再次调用不重复
	require.False(t, ensureCodexReasoningInclude(body))

	// 无 reasoning → 不动
	body2 := map[string]any{REDACTED
	...
	body3 := map[string]any{
		"reasoning": map[string]any{"effort": "high"REDACTED,
		"include":   []any{"foo"REDACTED,
REDACTED
	require.True(t, ensureCodexReasoningInclude(body3))
	require.Equal(t, []any{"foo", "reasoning.encrypted_content"REDACTED, body3["include"])
REDACTED
...
func TestApplyCodexClientMetadata(t *testing.T) {
	...
	acc := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"openai_device_id": "dev-xyz"REDACTEDREDACTED
	...
	body2 := map[string]any{REDACTED
	require.False(t, applyCodexClientMetadata(body2, &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED))
	_, ok = body2["client_metadata"]
	require.False(t, ok)

	// 既有 client_metadata（如 turn metadata）保留，仅补 installation 键
	body3 := map[string]any{"client_metadata": map[string]any{"x-codex-turn-metadata": "t"REDACTEDREDACTED
