...
REDACTED
	if apiKeyID > 0 {
		c.Set("api_key", &APIKey{ID: apiKeyIDREDACTED)
REDACTED
	return c, rec
REDACTED
...
func TestRelayOpenAICodexTurnState_SetsHeaderAndRecordsProvenance(t *testing.T) {
	...
	account := &Account{ID: 42REDACTED
	c, _ := newTurnStateTestContext(t, 7, "sess-relay")

	upstream := http.Header{REDACTED
...
func TestRelayOpenAICodexTurnState_ClearsStaleValueWhenUpstreamAbsent(t *testing.T) {
	...
	svc.relayOpenAICodexTurnState(c, &Account{ID: 43REDACTED, http.Header{REDACTED)
...
func TestStageOpenAICodexTurnState_StagedHeaders(t *testing.T) {
	...
	// 真正提交时才记录
	svc.noteStagedOpenAICodexTurnStateCommitted(c, &Account{ID: 44REDACTED, staged)
	...
	stageOpenAICodexTurnState(&staged, http.Header{REDACTED)
	require.Empty(t, staged.Get("X-Codex-Turn-State"))
	var nilStaged http.Header
	stageOpenAICodexTurnState(&nilStaged, http.Header{REDACTED)
...
func TestNoteStagedOpenAICodexTurnStateCommitted_NoopWithoutState(t *testing.T) {
	...
	svc.noteStagedOpenAICodexTurnStateCommitted(c, &Account{ID: 60REDACTED, nil)
	svc.noteStagedOpenAICodexTurnStateCommitted(c, &Account{ID: 60REDACTED, http.Header{"X-Request-Id": []string{"rid"REDACTEDREDACTED)
...
REDACTED
	...
	t.Run("same_account_keeps_echo", func(t *testing.T) {
		...
		upstream := http.Header{REDACTED
		upstream.Set("x-codex-turn-state", "blob-A")
		svc.relayOpenAICodexTurnState(c, &Account{ID: 42REDACTED, upstream)

		h := newOutbound("blob-A")
		svc.guardOpenAICodexTurnStateEcho(c, &Account{ID: 42REDACTED, h)
		require.Equal(t, "blob-A", h.Get("x-codex-turn-state"))
REDACTED)
	...
	t.Run("foreign_account_strips_echo", func(t *testing.T) {
		...
		h := newOutbound("blob-A")
		svc.guardOpenAICodexTurnStateEcho(c, &Account{ID: 43REDACTED, h)
...
func TestWriteOpenAIPassthroughResponseHeaders_RelaysAndClearsTurnState(t *testing.T) {
	...
	writeOpenAIPassthroughResponseHeaders(dst, http.Header{"Content-Type": []string{"application/json"REDACTEDREDACTED, nil)
...
func TestWriteOpenAIPassthroughResponseHeaders_RelaysReasoningIncluded(t *testing.T) {
	...
	writeOpenAIPassthroughResponseHeaders(
		dst,
		src,
		responseheaders.CompileHeaderFilter(config.ResponseHeaderConfig{REDACTED),
	)
...
REDACTED)
	...
	t.Run("multi_line_values_merged_single_line", func(t *testing.T) {
		...
		ensureOpenAIRemoteCompactionV2BetaFeature(h)
		require.Equal(t, []string{"feature_a,feature_b,remote_compaction_v2"REDACTED, h.Values("x-codex-beta-features"))
REDACTED)
REDACTED
...
func TestApplyOpenAICodexBetaFeatures(t *testing.T) {
	oauthAccount := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	apiKeyAccount := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyREDACTED
...
REDACTED"chatgpt_account_id": "test-account"REDACTED,
REDACTED
	...
	require.Equal(t, []string{"some_other_feature"REDACTED, declared.Values("x-codex-beta-features"),
		"客户端已声明时原样保留")

	apiKeyHeaders := build(t, &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKeyREDACTED, "")
