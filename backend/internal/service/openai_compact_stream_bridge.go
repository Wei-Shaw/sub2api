...
REDACTED
	for _, item := range gjson.GetBytes(response, "output").Array() {
		if !item.IsObject() {
			continue
	REDACTED
		event, err := sjson.SetBytes([]byte(`{"type":"response.output_item.done"REDACTED`), "output_index", outputIndex)
		if err != nil {
			return nil, false
	REDACTED
...
REDACTED
	...
	completed, err := sjson.SetRawBytes([]byte(`{"type":"response.completed"REDACTED`), "response", response)
	if err != nil {
		return nil, false
REDACTED
...
func openAICompactUsageParsableByCodex(usage gjson.Result) bool {
	if !usage.IsObject() {
		return false
REDACTED
	for _, field := range []string{"input_tokens", "output_tokens", "total_tokens"REDACTED {
