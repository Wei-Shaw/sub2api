...
// writeOpenAICompactSSEBridgeResult is the error-aware form used by response
// paths that must not commit turn-state provenance until the downstream body
// has actually been written. The bool retains the bridge's handled contract.
...
// writeOpenAICompactSSEFailureResult 从上游错误 body 提取错误消息后，以
...
// writeOpenAINonStreamingData preserves gin.Context.Data semantics while
// exposing renderer write failures to callers that gate committed state.
func writeOpenAINonStreamingData(c *gin.Context, statusCode int, contentType string, body []byte) error {
	if c == nil {
		return nil
	}
