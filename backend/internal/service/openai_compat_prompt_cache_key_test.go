...
func TestDeriveAnthropicCompatPromptCacheKey_StableAcrossLaterTurns(t *testing.T) {
	base := &apicompat.AnthropicRequest{
		...
		Messages: []apicompat.AnthropicMessage{
			{Role: "user", Content: mustRawJSON(t, `"Open repo"`)REDACTED,
	REDACTED,
REDACTED
	extended := &apicompat.AnthropicRequest{
		...
		Messages: []apicompat.AnthropicMessage{
			{Role: "user", Content: mustRawJSON(t, `"Open repo"`)REDACTED,
			{Role: "assistant", Content: mustRawJSON(t, `"Opened."`)REDACTED,
			{Role: "user", Content: mustRawJSON(t, `"Run tests"`)REDACTED,
	REDACTED,
REDACTED
...
func TestDeriveAnthropicCompatPromptCacheKey_UsesCacheControlAnchors(t *testing.T) {
	base := &apicompat.AnthropicRequest{
		...
		System: mustRawJSON(t, `[
			{"type":"text","text":"project instructions","cache_control":{"type":"ephemeral"REDACTEDREDACTED
		]`),
		Messages: []apicompat.AnthropicMessage{
			{Role: "user", Content: mustRawJSON(t, `[
				{"type":"text","text":"repo anchor","cache_control":{"type":"ephemeral"REDACTEDREDACTED
			]`)REDACTED,
	REDACTED,
REDACTED
	extended := &apicompat.AnthropicRequest{
		...
		Messages: []apicompat.AnthropicMessage{
			base.Messages[0],
			{Role: "assistant", Content: mustRawJSON(t, `[{"type":"text","text":"Opened."REDACTED]`)REDACTED,
			{Role: "user", Content: mustRawJSON(t, `[{"type":"text","text":"Run tests"REDACTED]`)REDACTED,
	REDACTED,
REDACTED
