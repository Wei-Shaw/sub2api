...
func TestBuildOpenAIEmbeddingsURL(t *testing.T) {
	...
	tests := []struct {
		...
		want string
REDACTED{
		{"bare domain", "https://api.openai.com", "https://api.openai.com/v1/embeddings"REDACTED,
		{"bare /v1", "https://api.openai.com/v1", "https://api.openai.com/v1/embeddings"REDACTED,
		{"already embeddings", "https://api.openai.com/v1/embeddings", "https://api.openai.com/v1/embeddings"REDACTED,
		{"third-party versioned path", "https://open.bigmodel.cn/api/paas/v4", "https://open.bigmodel.cn/api/paas/v4/embeddings"REDACTED,
REDACTED
...
REDACTED`)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header: http.Header{
			...
			"X-Request-Id": []string{"emb-rid"REDACTED,
	REDACTED,
		Body: io.NopCloser(strings.NewReader(`{
			"object":"list",
			"data":[
				{"object":"embedding","index":0,"embedding":[0.1,0.2]REDACTED,
				{"object":"embedding","index":1,"embedding":[0.3,0.4]REDACTED
			],
			"model":"jina-embeddings-v5-text-small",
			"usage":{"prompt_tokens":13,"total_tokens":13REDACTED
	REDACTED`)),
REDACTEDREDACTED
...
func TestForwardEmbeddings_AccessStateUsesTypedFailover(t *testing.T) {
	...
	reqBody := []byte(`{"model":"text-embedding-3-small","input":"hello"REDACTED`)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/embeddings", bytes.NewReader(reqBody))

	upstreamBody := []byte(`{"error":{"code":"deactivated_workspace","message":"Workspace is deactivated"REDACTEDREDACTED`)
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusForbidden,
		Header: http.Header{
			"Content-Type": []string{"application/json"REDACTED,
			"X-Request-Id": []string{"req_embeddings_access_state"REDACTED,
	REDACTED,
