...
func TestWSResponseCreate_DefaultPassesPriorityAndNormalizesFast(t *testing.T) {
	...
	frame := []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"priority","input":[{"type":"input_text","text":"hi"REDACTED]REDACTED`)
...
REDACTED
	...
	frame = []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"fast"REDACTED`)
	updated, blocked, err = svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-5.5", frame)
REDACTED
	require.Nil(t, blocked)
...
REDACTED
	...
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKeyREDACTED
	frame := []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"priority"REDACTED`)
...
REDACTED
	...
	for _, tier := range []string{"flex", "auto", "default", "scale", "fast", "priority"REDACTED {
		frame := []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"` + tier + `"REDACTED`)
		updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-5.5", frame)
	REDACTED
		require.Nil(t, blocked)
...
func TestWSResponseCreate_FlexPassThrough(t *testing.T) {
	...
	frame := []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"flex"REDACTED`)
	updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-5.5", frame)
REDACTED
	require.Nil(t, blocked)
...
func TestWSResponseCreate_NoServiceTierUntouched(t *testing.T) {
	...
	frame := []byte(`{"type":"response.create","model":"gpt-5.5","input":[]REDACTED`)
	updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-5.5", frame)
REDACTED
	require.Nil(t, blocked)
...
REDACTED
	...
	frame := []byte(`{"type":"response.cancel","service_tier":"priority"REDACTED`)
	updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-5.5", frame)
REDACTED
	require.Nil(t, blocked)
	require.Equal(t, string(frame), string(updated))
...
REDACTED
	...
	frame := []byte(`{"service_tier":"priority","model":"gpt-5.5"REDACTED`)
	updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-5.5", frame)
REDACTED
	require.Nil(t, blocked)
	...
	frame = []byte(`{"type":"","service_tier":"priority","model":"gpt-5.5"REDACTED`)
	updated, blocked, err = svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-5.5", frame)
REDACTED
	require.Nil(t, blocked)
	require.Equal(t, string(frame), string(updated))
...
func TestBuildOpenAIFastPolicyBlockedWSEvent_HasEventIDAndCode(t *testing.T) {
	bytes := buildOpenAIFastPolicyBlockedWSEvent(&OpenAIFastBlockedError{Message: "blocked because reasons"REDACTED)
	...
	other := buildOpenAIFastPolicyBlockedWSEvent(&OpenAIFastBlockedError{Message: "second"REDACTED)
...
func gpt55WhitelistFastPolicy() *OpenAIFastPolicySettings {
	return &OpenAIFastPolicySettings{
		Rules: []OpenAIFastPolicyRule{{
			...
			Action:         BetaPolicyActionFilter,
			Scope:          BetaPolicyScopeAll,
			ModelWhitelist: []string{"gpt-5.5", "gpt-5.5*"REDACTED,
			FallbackAction: BetaPolicyActionPass,
REDACTED
REDACTED
REDACTED
...
func TestPolicyEnforcingFrameConn_FollowupFrameWithoutModelUsesCapturedModel(t *testing.T) {
	...
	firstFrame := []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"priority"REDACTED`)
	...
	followupFrame := []byte(`{"type":"response.create","service_tier":"priority"REDACTED`)

	inner := &fakePassthroughFrameConn{
		reads: [][]byte{followupFrameREDACTED,
REDACTED
...
REDACTED
			"model_mapping": map[string]any{"public-model": "private-model"REDACTED,
	REDACTED,
		Extra: map[string]any{"openai_passthrough": trueREDACTED,
REDACTED

	responseCreate := []byte(`{"type":"response.create","model":"public-model"REDACTED`)
	require.Equal(t, "public-model", openAIWSPassthroughPolicyModelForFrame(account, responseCreate))

	sessionUpdate := []byte(`{"type":"session.update","session":{"model":"public-model"REDACTEDREDACTED`)
...
func TestPolicyEnforcingFrameConn_WithoutCapturedFallbackPolicyMisses(t *testing.T) {
	...
	followupFrame := []byte(`{"type":"response.create","service_tier":"priority"REDACTED`)
	inner := &fakePassthroughFrameConn{reads: [][]byte{followupFrameREDACTEDREDACTED
	wrapper := &openAIWSPolicyEnforcingFrameConn{
		inner: inner,
		filter: func(msgType coderws.MessageType, payload []byte) ([]byte, *OpenAIFastBlockedError, error) {
...
func TestWSResponseCreate_IngressFiltersServiceTierBeforeUpstream(t *testing.T) {
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_ws_filter_1","model":"gpt-5.5","usage":{"input_tokens":1,"output_tokens":1REDACTEDREDACTEDREDACTED`),
	REDACTED,
REDACTED
...
REDACTED
	...
	require.NoError(t, clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.5","stream":false,"service_tier":"fast"REDACTED`)))
	cancelWrite()
...
REDACTED
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), 3*time.Second)
	require.NoError(t, clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.5","stream":false,"service_tier":"priority"REDACTED`)))
	cancelWrite()
...
REDACTED
	...
	body := []byte(`{"model":"gpt-5.5","service_tier":"priority","input":[]REDACTED`)
...
func TestForwardAsAnthropicMessages_BetaFastModePassesOpenAIFastPolicyByDefault(t *testing.T) {
	...
	anthropicBody := []byte(`{"model":"gpt-5.5","max_tokens":64,"messages":[{"role":"user","content":"hi"REDACTED]REDACTED`)
...
func TestPolicyEnforcingFrameConn_SessionUpdateRotatesCapturedModel(t *testing.T) {
	...
	first := []byte(`{"type":"response.create","model":"gpt-4o","service_tier":"priority"REDACTED`)
	// Frame 2: session.update rotates the session model to gpt-5.5.
	rotate := []byte(`{"type":"session.update","session":{"model":"gpt-5.5"REDACTEDREDACTED`)
	// Frame 3: response.create WITHOUT model — must inherit gpt-5.5.
	followup := []byte(`{"type":"response.create","service_tier":"priority"REDACTED`)

	inner := &fakePassthroughFrameConn{reads: [][]byte{first, rotate, followupREDACTEDREDACTED
...
func TestPolicyModelFromSessionFrame_OnlySessionUpdate(t *testing.T) {
	...
	created := []byte(`{"type":"session.created","session":{"model":"gpt-5.5"REDACTEDREDACTED`)
	...
	notSession := []byte(`{"type":"response.create","session":{"model":"gpt-9"REDACTEDREDACTED`)
	...
	noModel := []byte(`{"type":"session.update","session":{"voice":"alloy"REDACTEDREDACTED`)
...
REDACTED
	...
	body := []byte(`{"model":"gpt-4","service_tier":"fast"REDACTED`)
...
REDACTED
	...
	body = []byte(`{"model":"gpt-4","service_tier":"turbo"REDACTED`)
	updated, err = svc.applyOpenAIFastPolicyToBody(context.Background(), account, "gpt-4", body)
REDACTED
	require.Equal(t, string(body), string(updated))
REDACTED
...
func TestPassthroughBilling_PostFilterServiceTier(t *testing.T) {
	...
	raw := []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"priority"REDACTED`)
...
func TestApplyOpenAIFastPolicyToBody_NonStringServiceTier(t *testing.T) {
	...
	cases := [][]byte{
		[]byte(`{"model":"gpt-5.5","service_tier":1REDACTED`),
		[]byte(`{"model":"gpt-5.5","service_tier":nullREDACTED`),
		[]byte(`{"model":"gpt-5.5","service_tier":{"nested":"priority"REDACTEDREDACTED`),
		[]byte(`{"model":"gpt-5.5","service_tier":["priority"]REDACTED`),
		[]byte(`{"model":"gpt-5.5","service_tier":trueREDACTED`),
REDACTED
...
REDACTED
	...
	turn2 := []byte(`{"type":"response.create","model":"gpt-5.5","service_tier":"flex"REDACTED`)
	...
	cancelFrame := []byte(`{"type":"response.cancel","service_tier":"priority"REDACTED`)
	...
	turn3 := []byte(`{"type":"response.create","model":"gpt-5.5"REDACTED`)
...
func TestPassthroughUsageMeta_TracksReasoningEffortAcrossTurns(t *testing.T) {
	...
	firstFrame := []byte(`{"type":"response.create","model":"gpt-5.5","reasoning":{"effort":"medium"REDACTED,"service_tier":"priority"REDACTED`)
...
REDACTED
	...
	_, blockedSession, errSession := process([]byte(`{"type":"session.update","session":{"model":"gpt-5-high"REDACTEDREDACTED`))
	...
	_, blockedCancel, errCancel := process([]byte(`{"type":"response.cancel","reasoning_effort":"x-high"REDACTED`))
	require.NoError(t, errCancel)
	require.Nil(t, blockedCancel)
	require.NotNil(t, meta.reasoningEffort.Load())
	require.Equal(t, "medium", *meta.reasoningEffort.Load(), "非 response.create 帧不能污染当前 turn metadata")

	_, blockedFlat, errFlat := process([]byte(`{"type":"response.create","reasoning_effort":"x-high"REDACTED`))
	...
	_, blockedClear, errClear := process([]byte(`{"type":"response.create","model":"gpt-4o"REDACTED`))
