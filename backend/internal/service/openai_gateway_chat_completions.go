	...
	REDACTED
		...
		REDACTED
			chatReq, err := apicompat.ResponsesToChatCompletionsRequestWithOptions(
				&responsesReq,
				&apicompat.ResponsesToChatOptions{ReasoningContentByID: s.reasoningContentByIDREDACTED,
			)
			if err != nil {
...
REDACTED
	...
	REDACTED
		if effort := gjson.GetBytes(responsesBody, "reasoning.effort").String(); effort != "" {
			responsesReq.Reasoning = &apicompat.ResponsesReasoning{Effort: effortREDACTED
	REDACTED
REDACTED else {
...
REDACTED
	...
	REDACTED
REDACTED else if handleErr == nil && account.IsShadow() && account.ParentAccountID != nil {
...
REDACTED
	...
	if strings.TrimSpace(finalResponse.Status) == "failed" {
		payload, _ := json.Marshal(gin.H{"type": "response.failed", "response": finalResponseREDACTED)
...
REDACTED)
	if err != nil {
		return "data: {\"error\":{\"type\":\"invalid_request_error\",\"code\":\"" + code + "\",\"message\":\"upstream error\"REDACTEDREDACTED\n\n"
REDACTED
