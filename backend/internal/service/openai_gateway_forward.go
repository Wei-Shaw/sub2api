...
REDACTED else if toolSchemaSanitized {
...
REDACTED
	if imageIntent && !imageGenerationAllowed {
		MarkOpsClientBusinessLimited(c, OpsClientBusinessLimitedReasonLocalFeatureGate)
		c.JSON(http.StatusForbidden, gin.H{"error": gin.H{"type": "permission_error", "message": ImageGenerationPermissionMessage()REDACTEDREDACTED)
...
REDACTED
	...
	REDACTED
		if err := validateOpenAIResponsesImageModel(decoded, upstreamModel); err != nil {
			setOpsUpstreamError(c, http.StatusBadRequest, err.Error(), "")
			c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{"type": "invalid_request_error", "message": err.Error(), "param": "model"REDACTEDREDACTED)
			return nil, err
	REDACTED
	...
	REDACTED
REDACTED else if imageGenerationAllowed && imageIntent && openAIRequestBodyHasImageGenerationDeclaration(body) {
...
REDACTED
	...
	REDACTED
		if err := validateCodexSparkInput(decoded, upstreamModel); err != nil {
			setOpsUpstreamError(c, http.StatusBadRequest, err.Error(), "")
			c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{"type": "invalid_request_error", "message": err.Error(), "param": "input"REDACTEDREDACTED)
			return nil, err
	REDACTED
REDACTED
...
REDACTED
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if decodeErr != nil {
			return nil, decodeErr
	REDACTED
		codexResult := codexTransformResult{REDACTED
		if compatMessagesBridge {
			codexResult = applyCodexOAuthTransformWithOptions(decoded, codexOAuthTransformOptions{IsCodexCLI: isCodexCLI, IsCompact: isCompactRequest, SkipDefaultInstructions: true, PreserveToolCallIDs: trueREDACTED)
	...
	REDACTED
		if codexResult.Error != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{"type": "invalid_request_error", "message": codexResult.Error.Error()REDACTEDREDACTED)
	...
	REDACTED else if currentPromptCacheKey, ok := decoded["prompt_cache_key"].(string); ok && currentPromptCacheKey != "" {
	...
	REDACTED else if codexResult.PromptCacheKey != "" {
...
REDACTED
	...
	REDACTED
		for _, unsupportedField := range []string{"prompt_cache_retention", "safety_identifier", "prompt_cache_options"REDACTED {
...
REDACTED
	...
	if rawTier := requestView.ServiceTier; rawTier != "" {
		if normTier := normalizedOpenAIServiceTierValue(rawTier); normTier != "" {
			...
			REDACTED
				blocked := &OpenAIFastBlockedError{Message: msgREDACTED
...
REDACTED
	...
	REDACTED
		if imageCfgErr != nil {
			...
			c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{"type": "invalid_request_error", "message": imageCfgErr.Error(), "param": "size"REDACTEDREDACTED)
...
REDACTED
	...
	REDACTED
		if headerGuard != nil {
			resp.Body = &openAIRequestContextReadCloser{ReadCloser: resp.Body, cleanup: headerGuard.closeREDACTED
	REDACTED
		...
		REDACTED else if changed && rejectedFieldRetryState.Allow(retryBody) {
				body = retryBody
				requestView = newOpenAIRequestView(body)
				reqBody = nil
