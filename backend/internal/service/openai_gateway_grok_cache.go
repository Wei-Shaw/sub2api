...
REDACTED
	if matches := claudeCodeSessionSuffixPattern.FindStringSubmatch(userID); len(matches) >= 2 {
		return matches[1]
REDACTED
	// Claude Code may embed JSON: {"session_id":"..."REDACTED
	...
	if billing, err := grokBillingSnapshotFromExtra(account.Extra); err == nil && billing != nil {
		if tier := strings.TrimSpace(billing.Plan); tier != "" {
			if isGrokFreeSubscriptionTier(tier) {
				freeSignal = true
		REDACTED else if !isGrokUnknownSubscriptionTier(tier) {
				paidSignal = true
		REDACTED
	REDACTED
...
REDACTED
	...
	REDACTED
REDACTED
	for _, key := range []string{"subscription_tier", "plan_type"REDACTED {
		...
		REDACTED
			...
			if (name == "web_search" || name == "x_search") && allowFunctionSearch {
				if present[name] {
					continue
			REDACTED
				raw, err := json.Marshal(map[string]string{"type": nameREDACTED)
				if err != nil {
					return nil, err
			REDACTED
...
REDACTED
	for _, toolType := range []string{"web_search", "x_search"REDACTED {
		if present[toolType] {
			continue
	REDACTED
		raw, err := json.Marshal(map[string]string{"type": toolTypeREDACTED)
		if err != nil {
			return nil, err
	REDACTED
		merged = append(merged, raw)
