...
REDACTED
	...
	REDACTED else if sessionSeed := promptCacheKeyFromAnthropicMetadataSession(&anthropicReq); sessionSeed != "" {
			promptCacheKey = sessionSeed
			compatPromptCacheInjected = true
	REDACTED
REDACTED
...
REDACTED
	...
	REDACTED
		return &apicompat.ResponsesResponse{
			Status: "failed",
			Error:  &apicompat.ResponsesError{Code: event.Code, Message: messageREDACTED,
	REDACTED
	default:
		return nil
REDACTED
...
REDACTED
	...
	go func() {
		...
		for scanner.Scan() {
			select {
			case events <- scanEvent{line: scanner.Text()REDACTED:
			case <-done:
				return
		REDACTED
	REDACTED
		if err := scanner.Err(); err != nil {
			select {
			case events <- scanEvent{err: errREDACTED:
			case <-done:
		REDACTED
	REDACTED
REDACTED()
...
REDACTED)
	if err != nil {
		return "event: error\ndata: {\"type\":\"error\",\"error\":{\"type\":\"" + errType + "\",\"message\":\"upstream error\"REDACTEDREDACTED\n\n"
REDACTED
	return "event: error\ndata: " + string(payload) + "\n\n"
REDACTED
