	...
	for {
		...
		if reqStream {
			...
			if imageIntent && s.shouldBufferOpenAIImagesForUpscale(ctx, imageGroup, imageInputSize) {
				result, handleErr = s.handleStreamingResponsePassthroughBufferedForImageUpscale(ctx, resp, c, account, startTime, reqModel, upstreamPassthroughModel, imageGroup, imageInputSize)
			} else {
			...
			imageResponseQuality = result.imageQuality
			imageOutputSizes = result.imageOutputSizes
			imageOutputBase64s = result.imageOutputBase64s
		} else {
			result, handleErr := s.handleNonStreamingResponsePassthrough(ctx, resp, c, account, reqModel, upstreamPassthroughModel, imageGroup, imageInputSize)
			if handleErr != nil {
				if retryBody, fallbackModel, retry := s.applyOpenAIPassthroughCompactFallbackFromSignal(
					c, account, requestedModel, body, handleErr, compactModelFallbackRetried, resp,
				); retry {
...
// handleStreamingResponsePassthroughBufferedForImageUpscale 与主路径同理：缓冲整段上游
// SSE，先放大并原地改写 base64，再交给正常的直通流式处理器一次性回吐。
func (s *OpenAIGatewayService) handleStreamingResponsePassthroughBufferedForImageUpscale(
	...
	return s.handleStreamingResponsePassthrough(ctx, resp, c, account, startTime, originalModel, mappedModel)
}
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthrough(
	...
	if isEventStreamResponse(resp.Header) {
		return s.handlePassthroughSSEToJSON(ctx, resp, c, account, body, originalModel, mappedModel, group, requestedImageSize)
	}
	...
	if imageCount > 0 {
		...
		body, imageCount, imageOutputSizes, imageOutputBase64s, _ = s.prepareOpenAIResponsesImageBody(ctx, group, requestedImageSize, body, false)
	}
...
func (s *OpenAIGatewayService) handlePassthroughSSEToJSON(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, body []byte, originalModel string, mappedModel string, group *Group, requestedImageSize string) (*openaiNonStreamingResultPassthrough, error) {
	...
	var imageOutputBase64s []string
	if ok {
		// 终端事件已被重建为 JSON 响应体，按 JSON 形状提取/放大。
		imageCount = countOpenAIResponseImageOutputsFromJSONBytes(body)
		imageQuality = collectOpenAIResponseImageQualityFromJSONBytes(body)
		imageOutputSizes = collectOpenAIResponseImageOutputSizesFromJSONBytes(body)
		imageOutputBase64s = collectOpenAIResponseImageOutputBase64sFromJSONBytes(body)
		...
		if imageCount > 0 {
			body, imageCount, imageOutputSizes, imageOutputBase64s, _ = s.prepareOpenAIResponsesImageBody(ctx, group, requestedImageSize, body, true)
		}
	}
