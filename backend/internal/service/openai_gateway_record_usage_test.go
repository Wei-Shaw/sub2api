...
func TestOpenAIGatewayServiceRecordUsage_RejectsNilInput(t *testing.T) {
	...
	require.Error(t, svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{REDACTED))
REDACTED
...
func TestRecordCyberPolicyUsageLog_BillsRealUpstreamTokens(t *testing.T) {
	...
	usage := OpenAIUsage{InputTokens: 1200, OutputTokens: 300REDACTED
	...
	svc.RecordCyberPolicyUsageLog(context.Background(), CyberPolicyUsageInput{
		APIKey:       &APIKey{ID: 2, User: &User{ID: 1REDACTEDREDACTED,
		Account:      &Account{ID: 3REDACTED,
...
func TestRecordCyberPolicyUsageLog_SkipsWhenIncomplete(t *testing.T) {
	...
	acct := &Account{ID: 3REDACTED
	svc.RecordCyberPolicyUsageLog(context.Background(), CyberPolicyUsageInput{Account: acct, Model: "gpt-5"REDACTED)                              // APIKey nil
	svc.RecordCyberPolicyUsageLog(context.Background(), CyberPolicyUsageInput{APIKey: &APIKey{ID: 2REDACTED, Account: acct, Model: "gpt-5"REDACTED)      // User nil
	svc.RecordCyberPolicyUsageLog(context.Background(), CyberPolicyUsageInput{APIKey: &APIKey{ID: 2, User: &User{ID: 1REDACTEDREDACTED, Model: "gpt-5"REDACTED) // Account nil
	svc.RecordCyberPolicyUsageLog(context.Background(), CyberPolicyUsageInput{APIKey: &APIKey{ID: 2, User: &User{ID: 1REDACTEDREDACTED, Account: acctREDACTED)  // Model 空
...
func TestOpenAIGatewayServiceRecordUsage_MissingPricingRecordsZeroCostUsageLog(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		...
		REDACTED,
			...
			Duration: time.Second,
	REDACTED,
		APIKey:        &APIKey{ID: 1002, Quota: 100, Group: &Group{RateMultiplier: 1REDACTEDREDACTED,
		User:          &User{ID: 2002REDACTED,
		Account:       &Account{ID: 3002, Type: AccountTypeAPIKeyREDACTED,
		APIKeyService: quotaSvc,
REDACTED)
...
REDACTED)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			...
			Model:     "gpt-5.1",
			Usage:     OpenAIUsage{InputTokens: 1000, OutputTokens: 500REDACTED,
	REDACTED,
...
REDACTEDREDACTED)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		...
		REDACTED,
			...
			Duration: time.Second,
	REDACTED,
		APIKey:  &APIKey{ID: 1056REDACTED,
		User:    &User{ID: 2056REDACTED,
		Account: &Account{ID: 3056REDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_GrokLongContextFollowsGroupToggleOnly(t *testing.T) {
	...
	REDACTEDID: id, Platform: PlatformGrok, Type: AccountTypeOAuthREDACTED
REDACTED
	...
	t.Run("group on applies the official ladder", func(t *testing.T) {
		...
		err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
			Result: &OpenAIForwardResult{
				...
				Usage:     OpenAIUsage{InputTokens: 250000, OutputTokens: 1000REDACTED,
				Model:     "grok-4.5",
				Duration:  time.Second,
		REDACTED,
			APIKey:  openAIRecordUsageAPIKeyWithGroup(svc, 1030, true),
			User:    &User{ID: 2030REDACTED,
...
func TestOpenAIGatewayServiceRecordUsage_SparkShadowUsesCurrentParentBillingSetting(t *testing.T) {
	tests := []struct {
		...
		parentEnabled bool
REDACTED{
		{name: "parent opt out overrides stale enabled shadow", parentEnabled: falseREDACTED,
		{name: "parent opt in overrides stale disabled shadow", parentEnabled: trueREDACTED,
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			accountRepo := &openAIRecordUsageAccountRepoStub{account: &Account{
				...
				Extra:    map[string]any{openAILongContextBillingEnabledKey: tt.parentEnabledREDACTED,
		REDACTEDREDACTED
			svc := newOpenAIRecordUsageServiceForTest(
				usageRepo,
				&openAIRecordUsageUserRepoStub{REDACTED,
				&openAIRecordUsageSubRepoStub{REDACTED,
				nil,
			)
			...
			err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
				Result: &OpenAIForwardResult{
					...
					Usage:     OpenAIUsage{InputTokens: 300000, OutputTokens: 2000REDACTED,
					Model:     "gpt-5.4-2026-03-05",
					Duration:  time.Second,
			REDACTED,
...
func TestOpenAIGatewayServiceRecordUsage_EmptyImageSizeDefaultsBeforeBillingAndPersistence(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 21201REDACTED,
		Account: &Account{ID: 31201REDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_OutputImageSizeWinsBeforeBillingAndPersistence(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			...
			ImageOutputSizes: []string{"3840x2160"REDACTED,
			Duration:         time.Second,
	REDACTED,
		APIKey: &APIKey{
			ID:      11202,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 21202REDACTED,
		Account: &Account{ID: 31202REDACTED,
REDACTED)
...
REDACTED
	...
	require.Equal(t, map[string]int{ImageBillingSize4K: 1REDACTED, usageRepo.lastLog.ImageSizeBreakdown)
...
func TestOpenAIGatewayServiceRecordUsage_ImageSharedMultiplierPreservesExistingBehavior(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20121REDACTED,
		Account: &Account{ID: 30121REDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_ImageSharedMultiplierUsesUserGroupOverride(t *testing.T) {
	...
	svc := newOpenAIRecordUsageServiceForTest(
		...
		&openAIRecordUsageUserRepoStub{REDACTED,
		&openAIRecordUsageSubRepoStub{REDACTED,
		&openAIUserGroupRateRepoStub{rate: &userRateREDACTED,
	)
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20125REDACTED,
		Account: &Account{ID: 30125REDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_ImageIndependentMultiplierUsesImageRate(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20122REDACTED,
		Account: &Account{ID: 30122REDACTED,
REDACTED)
...
func TestGrokVideoBillingUsesSeparateVideoRateMultiplier(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20126REDACTED,
		Account: &Account{ID: 30126, Platform: PlatformGrokREDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_GroupImagePriceOverridesChannelImagePrice(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20127REDACTED,
		Account: &Account{ID: 30127, Platform: PlatformGrokREDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_GroupVideoPriceOverridesChannelImagePrice(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20128REDACTED,
		Account: &Account{ID: 30128, Platform: PlatformGrokREDACTED,
REDACTED)
		...
		APIKey: &APIKey{
			...
			Group: &Group{
				...
				VideoModelPrices: map[string]map[string]float64{
					VideoPriceFamilyGrokImagineVideo15: {VideoBillingResolution720P: modelVideoPrice720PREDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
		User:    &User{ID: 20129REDACTED,
		Account: &Account{ID: 30129, Platform: PlatformGrokREDACTED,
REDACTED)
...
REDACTEDREDACTEDREDACTED
	...
	refreshed := svc.apiKeyWithFreshGroupMediaPricing(context.Background(), &APIKey{GroupID: i64p(groupID), Group: &Group{ID: groupIDREDACTEDREDACTED)
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20130REDACTED,
		Account: &Account{ID: 30130, Platform: PlatformGrokREDACTED,
REDACTED)
...
REDACTEDREDACTEDREDACTED
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20131REDACTED,
		Account: &Account{ID: 30131, Platform: PlatformGrokREDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_GrokVideoWithTokenChannelPricingKeepsVideoMetadata(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			...
			Usage:                OpenAIUsage{InputTokens: 100, OutputTokens: 200REDACTED,
			Duration:             time.Second,
	REDACTED,
		APIKey: &APIKey{
			ID:      10132,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20132REDACTED,
		Account: &Account{ID: 30132, Platform: PlatformGrokREDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_ChannelImageBillingUsesImageCountAndSharedMultiplier(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20123REDACTED,
		Account: &Account{ID: 30123REDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_ChannelImageBillingUsesImageCountAndIndependentMultiplier(t *testing.T) {
	...
	REDACTED,
		...
		REDACTED,
	REDACTED,
		User:    &User{ID: 20124REDACTED,
		Account: &Account{ID: 30124REDACTED,
REDACTED)
...
REDACTED
	...
	cache.pricingByGroupModel[channelModelKey{groupID: groupID, model: modelREDACTED] = &ChannelModelPricing{
		BillingMode:     BillingModeImage,
		PerRequestPrice: &price,
REDACTED
	cache.channelByGroupID[groupID] = &Channel{ID: groupID, Status: StatusActiveREDACTED
	cache.groupPlatform[groupID] = ""
	cache.loadedAt = time.Now()
	cs := &ChannelService{REDACTED
	cs.cache.Store(cache)
	return NewModelPricingResolver(cs, NewBillingService(&config.Config{REDACTED, nil))
REDACTED
...
REDACTED
	...
	require.True(t, ok)
	cached.pricingByGroupModel[channelModelKey{groupID: groupID, model: modelREDACTED].TimePricing = timePricing
	return resolver
REDACTED
...
func TestGatewayServiceCalculateRecordUsageCost_ChannelImageBillingUsesImageCount(t *testing.T) {
	groupID := int64(126)
	billingService := NewBillingService(&config.Config{REDACTED, nil)
	svc := &GatewayService{
...
REDACTED
	...
	cost := svc.calculateRecordUsageCost(
		context.Background(),
		&ForwardResult{Model: "gemini-image", ImageCount: 2, ImageSize: "1K"REDACTED,
		&APIKey{GroupID: i64p(groupID), Group: &Group{ID: groupIDREDACTEDREDACTED,
		"gemini-image",
		0.15,
		1.0,
		time.Time{REDACTED,
...
func TestGatewayServiceCalculateRecordUsageCost_ChannelImageBillingUsesSizeTier(t *testing.T) {
	...
	price4K := 0.40
	cache := newEmptyChannelCache()
	cache.pricingByGroupModel[channelModelKey{groupID: groupID, model: "gemini-image"REDACTED] = &ChannelModelPricing{
		BillingMode:     BillingModeImage,
...
REDACTED
	cache.channelByGroupID[groupID] = &Channel{ID: groupID, Status: StatusActiveREDACTED
	cache.loadedAt = time.Now()
	channelService := &ChannelService{REDACTED
	channelService.cache.Store(cache)

	svc := &GatewayService{
		billingService: NewBillingService(&config.Config{REDACTED, nil),
		resolver:       NewModelPricingResolver(channelService, NewBillingService(&config.Config{REDACTED, nil)),
REDACTED

	cost := svc.calculateRecordUsageCost(
		context.Background(),
		&ForwardResult{Model: "gemini-image", ImageCount: 2, ImageSize: "4K"REDACTED,
		&APIKey{GroupID: i64p(groupID), Group: &Group{ID: groupIDREDACTEDREDACTED,
		"gemini-image",
		1.0,
		1.0,
...
REDACTED
	...
	cost := svc.calculateRecordUsageCost(
		context.Background(),
		&ForwardResult{Model: "gemini-image", ImageCount: 2, ImageSize: ImageBillingSize2KREDACTED,
		&APIKey{
			GroupID: i64p(groupID),
			Group: &Group{
				ID:           groupID,
...
func TestRecordUsageMarksCyberRequestType(t *testing.T) {
	logStub := &openAIRecordUsageLogRepoStub{inserted: trueREDACTED
	userStub := &openAIRecordUsageUserRepoStub{REDACTED
	subStub := &openAIRecordUsageSubRepoStub{REDACTED
	rateStub := &openAIUserGroupRateRepoStub{REDACTED
	...
	in := &OpenAIRecordUsageInput{
		...
		Result: &OpenAIForwardResult{
			Model:    "gpt-5",
			Duration: time.Second,
			Usage:    OpenAIUsage{InputTokens: 100, OutputTokens: 0REDACTED,
	REDACTED,
		APIKey:  &APIKey{ID: 2, Group: &Group{RateMultiplier: 1REDACTEDREDACTED,
...
REDACTED
	...
	cost := svc.calculateRecordUsageCost(
		context.Background(),
		&ForwardResult{Model: "gemini-image", ImageCount: 2, ImageSize: ""REDACTED,
		&APIKey{GroupID: i64p(groupID), Group: &Group{ID: groupIDREDACTEDREDACTED,
		"gemini-image",
		1.0,
		1.0,
...
func TestOpenAIGatewayServiceRecordUsage_ServiceTierDowngradedByUpstreamResponse(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			...
			Duration:                    time.Second,
	REDACTED,
		APIKey:  &APIKey{ID: 1017REDACTED,
		User:    &User{ID: 2017REDACTED,
		Account: &Account{ID: 3017REDACTED,
REDACTED)
...
func TestOpenAIGatewayServiceRecordUsage_ServiceTierNeverRaisedByUpstreamResponse(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			...
			Duration:                    time.Second,
	REDACTED,
		APIKey:  &APIKey{ID: 1018REDACTED,
		User:    &User{ID: 2018REDACTED,
		Account: &Account{ID: 3018REDACTED,
REDACTED)
