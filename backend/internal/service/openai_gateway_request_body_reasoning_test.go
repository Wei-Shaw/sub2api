...
func TestFilterOpenAIResponsesNoneReasoningEffortForAccount(t *testing.T) {
	tests := []struct {
		...
		{
			name: "forced chat completions preserves explicit none",
			...
			body:          `{"model":"qwen3.5-flash","reasoning":{"effort":"none"},"reasoning_effort":"none"}`,
			wantNested:    true,
			wantFlat:      true,
			wantReasoning: true,
		},
		{
			name: "probed chat completions preserves explicit none",
			account: &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey,
				Credentials: map[string]any{"base_url": "https://compat.example/v1"},
