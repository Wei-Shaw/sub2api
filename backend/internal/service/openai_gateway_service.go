...
REDACTED
	...
	if account.Type == AccountTypeOAuth {
		...
		// AI SDK sends: {"content": [{"type": "input_text", "text": "..."REDACTED]REDACTED
		// Codex API expects: {"content": "..."REDACTED
		...
		REDACTED
			...
			REDACTED
				...
				if _, err := fmt.Fprintf(w, "%s\n", line); err != nil {
					...
					return &openaiStreamingResult{usage: usage, firstTokenMs: firstTokenMsREDACTED, err
			REDACTED
				flusher.Flush()
...
//
//	{"content": [{"type": "input_text", "text": "hello"REDACTED]REDACTED
//
...
REDACTED
	...
	REDACTED
REDACTED else if use5hFromSecondary {
		if snapshot.SecondaryUsedPercent != nil {
...
REDACTED
	...
	REDACTED
REDACTED else if use7dFromSecondary {
		if snapshot.SecondaryUsedPercent != nil {
