...
REDACTED)
	...
	t.Run("empty updatedAt uses fallback", func(t *testing.T) {
		got := codexSnapshotBaseTime(&OpenAICodexUsageSnapshot{REDACTED, fallback)
		if !got.Equal(fallback) {
			t.Fatalf("got %v, want fallback %v", got, fallback)
	REDACTED
REDACTED)
	...
	t.Run("valid updatedAt wins", func(t *testing.T) {
		got := codexSnapshotBaseTime(&OpenAICodexUsageSnapshot{UpdatedAt: "2026-02-16T10:00:00Z"REDACTED, fallback)
...
REDACTED)
	...
	t.Run("invalid updatedAt uses fallback", func(t *testing.T) {
		got := codexSnapshotBaseTime(&OpenAICodexUsageSnapshot{UpdatedAt: "invalid"REDACTED, fallback)
		if !got.Equal(fallback) {
			t.Fatalf("got %v, want fallback %v", got, fallback)
	REDACTED
REDACTED)
...
REDACTED
	...
	updates := buildCodexUsageExtraUpdates(snapshot, time.Time{REDACTED)
	if updates == nil {
		t.Fatal("expected non-nil updates")
REDACTED
