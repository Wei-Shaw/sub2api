...
REDACTED else if result.ImageCount > 0 && (cost == nil || cost.BillingMode != string(BillingModeToken)) {
...
REDACTED
	...
	candidates := append([]string{REDACTED, billingModels...)
...
REDACTED
	gid := apiKey.Group.ID
	resolved := s.resolver.Resolve(ctx, PricingInput{Model: billingModel, GroupID: &gid, Group: apiKey.GroupREDACTED)
...
func ParseCodexRateLimitHeaders(headers http.Header) *OpenAICodexUsageSnapshot {
	snapshot := &OpenAICodexUsageSnapshot{REDACTED
	hasData := false
