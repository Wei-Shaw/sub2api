...
func TestOpenAIGatewayServiceForwardImages_OAuthUsesDecodedOutputDimensions(t *testing.T) {
	...
	require.Equal(t, []string{"1672x941"REDACTED, run.result.ImageOutputSizes)
...
REDACTED
	...
	body := []byte(fmt.Sprintf(`{"model":"gpt-image-2","prompt":"draw a test chart","size":"3840x2160","quality":"low","output_format":"png","stream":%tREDACTED`, stream))
	...
	upstreamBody := fmt.Sprintf(
		"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000000,\"tools\":[{\"type\":\"image_generation\",\"model\":\"gpt-image-2\",\"size\":\"auto\",\"quality\":\"auto\",\"output_format\":\"png\"REDACTED]REDACTEDREDACTED\n\n"+
			"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000000,\"tools\":[{\"type\":\"image_generation\",\"model\":\"gpt-image-2\",\"size\":\"auto\",\"quality\":\"auto\",\"output_format\":\"png\"REDACTED],\"output\":[{\"id\":\"ig_actual_size\",\"type\":\"image_generation_call\",\"result\":%qREDACTED]REDACTEDREDACTED\n\n"+
			"data: [DONE]\n\n",
		encoded,
	)
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header: http.Header{
			...
			"X-Request-Id": []string{"req_img_actual_size"REDACTED,
	REDACTED,
...
REDACTED
	...
	return openAIOAuthImageActualSizeTestRun{result: result, recorder: rec, upstream: upstreamREDACTED
REDACTED

func encodeOpenAIImageTestPNG(t *testing.T, width, height int) string {
REDACTED
	img := image.NewNRGBA(image.Rect(0, 0, width, height))
	img.SetNRGBA(0, 0, color.NRGBA{R: 0xff, A: 0xffREDACTED)
...
REDACTED
	img := image.NewNRGBA(image.Rect(0, 0, width, height))
	img.SetNRGBA(0, 0, color.NRGBA{G: 0xff, A: 0xffREDACTED)
	var buf bytes.Buffer
