...
func (s *OpenAIGatewayService) handleOpenAICodexImagesResponse(
	...
	// The upstream image bytes are authoritative. The Codex backend can
	// normalize a requested option while still returning a valid image; do not
	// discard that image solely because its metadata differs from the request.
...
// auditOpenAICodexImagesResponseMismatch records an upstream normalization
// without turning an otherwise valid generated image into a gateway error.
// The public Images API returns the generated bytes; callers can inspect the
// returned metadata and the image itself instead of losing a billable result.
func auditOpenAICodexImagesResponseMismatch(err *OpenAIImagesUpstreamError) {
	if err == nil {
		return
	}
	logger.LegacyPrintf(
		"service.openai_gateway",
		"[OpenAI] Images upstream normalized requested parameter without blocking response param=%s message=%q",
		err.Param,
		err.Message,
	)
}
