...
func (b *openAIImagesReadErrorBody) Read([]byte) (int, error) { return 0, b.err REDACTED
func (b *openAIImagesReadErrorBody) Close() error             { return nil REDACTED
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_NormalizesOfficialAndCustomSizes(t *testing.T) {
	...
	tests := []struct {
		...
		wantTier string
REDACTED{
		{size: "1024x1024", wantTier: "1K"REDACTED,
		{size: "1536x1024", wantTier: "2K"REDACTED,
		{size: "1024x1536", wantTier: "2K"REDACTED,
		{size: "2048x2048", wantTier: "2K"REDACTED,
		{size: "2048x1152", wantTier: "2K"REDACTED,
		{size: "3840x2160", wantTier: "4K"REDACTED,
		{size: "2160x3840", wantTier: "4K"REDACTED,
		{size: "1024X768", wantTier: "1K"REDACTED,
		{size: "1280x768", wantTier: "2K"REDACTED,
		{size: "2560x1440", wantTier: "4K"REDACTED,
		{size: "2560x1600", wantTier: "4K"REDACTED,
		{size: "auto", wantTier: "2K"REDACTED,
REDACTED

	svc := &OpenAIGatewayService{REDACTED
	for _, tt := range tests {
		t.Run(tt.size, func(t *testing.T) {
			body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","size":"` + tt.size + `"REDACTED`)
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_UnknownSizesDoNotBlockPassthrough(t *testing.T) {
	...
	tests := []struct {
		...
		wantTier string
REDACTED{
		{size: "2048x1153", wantTier: "2K"REDACTED,
		{size: "4096x1024", wantTier: "4K"REDACTED,
		{size: "3840x1024", wantTier: "4K"REDACTED,
		{size: "512x512", wantTier: "1K"REDACTED,
		{size: "invalid", wantTier: "2K"REDACTED,
		{size: "999999999999999999999999999x2", wantTier: "2K"REDACTED,
REDACTED
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_LegacyImageModelUnknownSizePassthrough(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-1.5","prompt":"draw a cat","size":"2048x1152"REDACTED`)
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_AllowsGrokImageModels(t *testing.T) {
	...
	for _, model := range []string{"grok-imagine", "grok-imagine-image", "grok-imagine-image-quality", "grok-imagine-edit"REDACTED {
		t.Run(model, func(t *testing.T) {
			body := []byte(fmt.Sprintf(`{"model":%q,"prompt":"draw a cat","response_format":"b64_json"REDACTED`, model))
			req := httptest.NewRequest(http.MethodPost, "/v1/images/generations", bytes.NewReader(body))
			req.Header.Set("Content-Type", "application/json")
			rec := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(rec)
...
REDACTED, openAIUpstreamErrorBodyReadLimit)
REDACTED
...
func TestNewOpenAIImageStatusError_UsesProvidedReadLimit(t *testing.T) {
	...
	body := fmt.Sprintf(`{"error":{"padding":"%s","message":"diagnostic-marker"REDACTEDREDACTED`, padding)
...
REDACTED)
	...
	t.Run("alpha search 允许 OpenAI OAuth/PAT 与 APIKey 账号，拒绝 Grok", func(t *testing.T) {
		// OAuth/PAT 走 chatgpt.com Codex 端点，APIKey 走 {base_urlREDACTED/v1/alpha/search，
...
REDACTED)
	...
	t.Run("responses 能力：仍需通过 chat_completions 配置集校验", func(t *testing.T) {
		...
		account := &Account{
			...
			Type:     AccountTypeAPIKey,
	REDACTED
				"openai_capabilities": []any{"embeddings"REDACTED,
		REDACTED,
	REDACTED
...
func findOpenAIImageTestSSEEvent(events []openAIImageTestSSEEvent, name string) (openAIImageTestSSEEvent, bool) {
	...
	REDACTED
REDACTED
	return openAIImageTestSSEEvent{REDACTED, false
REDACTED

func TestOpenAIGatewayServiceForwardImages_OAuthPassesNAndReturnsAllImages(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","size":"1024x1024","quality":"high","n":3REDACTED`)
	...
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				...
				"X-Request-Id": []string{"req_img_badreq"REDACTED,
		REDACTED,
			Body: io.NopCloser(strings.NewReader(
				`{"error":{"message":"Invalid value for 'size': expected one of 1024x1024, 1536x1024.","type":"invalid_request_error","param":"size","code":"unknown_parameter"REDACTEDREDACTED`,
			)),
	REDACTED,
REDACTED
...
func TestOpenAIGatewayServiceForwardImages_OAuthNonStreamModerationBlockedReturnsClientError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw blocked image","response_format":"b64_json"REDACTED`)
...
REDACTED
	...
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"REDACTED,
				"X-Request-Id": []string{"req_img_blocked"REDACTED,
		REDACTED,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000020REDACTEDREDACTED\n\n" +
					"data: {\"type\":\"error\",\"error\":{\"type\":\"image_generation_user_error\",\"code\":\"moderation_blocked\",\"message\":\"Your request was rejected by the safety system. safety_violations=[sexual].\"REDACTEDREDACTED\n\n" +
					"data: {\"type\":\"response.failed\",\"response\":{\"id\":\"resp_blocked\",\"status\":\"failed\",\"error\":{\"type\":\"image_generation_user_error\",\"code\":\"moderation_blocked\",\"message\":\"Your request was rejected by the safety system. safety_violations=[sexual].\"REDACTEDREDACTEDREDACTED\n\n",
			)),
	REDACTED,
REDACTED
	...
	svc := &OpenAIGatewayService{
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"REDACTED,
					"X-Request-Id": []string{"req_img_server_error"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000021REDACTEDREDACTED\n\n" +
						"data: {\"type\":\"error\",\"error\":{\"type\":\"server_error\",\"code\":\"server_error\",\"message\":\"The image service is temporarily unavailable.\"REDACTEDREDACTED\n\n",
				)),
		REDACTED,
	REDACTED,
REDACTED
...
func TestOpenAIGatewayServiceForwardImages_OAuth429CarriesSameAccountRetryWindow(t *testing.T) {
	...
	svc := &OpenAIGatewayService{httpUpstream: &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Retry-After": []string{"1"REDACTED, "X-Request-Id": []string{"req_img_oauth_429"REDACTEDREDACTED,
...
REDACTEDREDACTEDREDACTED
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
REDACTED
	account := &Account{ID: 22, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{"access_token": "token-123"REDACTEDREDACTED
	startedAt := time.Now()
...
func TestOpenAIImagesOAuthBodyReadTransportErrorFailover(t *testing.T) {
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header: http.Header{
			"X-Request-Id": []string{"req_h2_read_failure"REDACTED,
			"X-Upstream":   []string{"preserved"REDACTED,
	REDACTED,
		Body: &openAIImagesReadErrorBody{err: errors.New("stream error: stream ID 11; INTERNAL_ERROR; received from peer")REDACTED,
REDACTED
	account := &Account{ID: 5400, Name: "openai-oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	svc := &OpenAIGatewayService{REDACTED
	...
	require.ErrorAs(t, err, &failoverErr)
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
	require.JSONEq(t, `{"error":{"type":"upstream_error","code":"upstream_http2_stream_error","message":"Upstream HTTP/2 stream failed"REDACTEDREDACTED`, string(failoverErr.ResponseBody))
...
func TestOpenAIImagesOAuthBodyReadErrorsNotMisclassified(t *testing.T) {
	tests := []struct {
		...
		err  error
REDACTED{
		{name: "context canceled", err: context.CanceledREDACTED,
		{name: "response too large", err: fmt.Errorf("%w: limit=1", ErrUpstreamResponseBodyTooLarge)REDACTED,
		{name: "semantic error", err: &OpenAIImagesUpstreamError{StatusCode: http.StatusBadRequest, ErrorType: "invalid_request_error", Code: "invalid_value", Message: "bad image request"REDACTEDREDACTED,
REDACTED
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c, _ := gin.CreateTestContext(rec)
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/images/generations", nil)
			resp := &http.Response{StatusCode: http.StatusOK, Header: make(http.Header)REDACTED
			err := tt.err
		...
		REDACTED
			...
			got := (&OpenAIGatewayService{REDACTED).handleOpenAIImagesOAuthResponseError(context.Background(), c, &Account{Platform: PlatformOpenAIREDACTED, "gpt-image-2", "", resp, OpenAIImagesJSONKeepaliveAdjustedWrittenSize(c), err)
			var failoverErr *UpstreamFailoverError
...
func TestOpenAIImagesOAuthTransportErrorAfterDownstreamWriteDoesNotFailover(t *testing.T) {
	...
	account := &Account{ID: 5401, Name: "openai-oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	resp := &http.Response{Header: http.Header{"X-Request-Id": []string{"req_after_write"REDACTEDREDACTEDREDACTED

	err := (&OpenAIGatewayService{REDACTED).handleOpenAIImagesOAuthResponseError(context.Background(), c, account, "gpt-image-2", "", resp, before, classifiedErr)
...
func TestShouldClassifyOpenAIUpstreamStreamReadErrorTransportStrings(t *testing.T) {
	for _, message := range []string{"unexpected EOF", "connection reset by peer", "broken pipe", "use of closed network connection"REDACTED {
	...
	svc := &OpenAIGatewayService{
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"REDACTED,
					"X-Request-Id": []string{"req_img_server_error_after_partial"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"response.image_generation_call.partial_image\",\"partial_image_b64\":\"cGFydGlhbA==\",\"partial_image_index\":0,\"output_format\":\"png\"REDACTED\n\n" +
						"data: {\"type\":\"error\",\"error\":{\"type\":\"server_error\",\"code\":\"server_error\",\"message\":\"The image service failed after partial output.\"REDACTEDREDACTED\n\n",
				)),
		REDACTED,
	REDACTED,
REDACTED
...
REDACTED{
		{
			...
			payload:    `{"type":"error","error":{"type":"invalid_request_error","code":"invalid_value","message":"bad size"REDACTEDREDACTED`,
			wantStatus: http.StatusBadRequest,
	REDACTED,
		{
			name:       "content policy",
			payload:    `{"type":"error","error":{"type":"image_generation_user_error","code":"content_policy_violation","message":"blocked"REDACTEDREDACTED`,
			wantStatus: http.StatusBadRequest,
	REDACTED,
		{
			name:       "rate limit remains distinct from server error",
			payload:    `{"type":"error","error":{"type":"rate_limit_exceeded","code":"rate_limit_exceeded","message":"try again"REDACTEDREDACTED`,
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"application/json"REDACTED,
					"X-Request-Id": []string{"req_img_apikey"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(`{"created":1710000007,"data":[{"b64_json":"aGVsbG8=","revised_prompt":"draw a cat"REDACTED]REDACTED`)),
		REDACTED,
	REDACTED,
REDACTED
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
...
func TestOpenAIGatewayServiceForwardImages_APIKeyAccessStateUsesTypedFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-1","prompt":"draw a cat"REDACTED`)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/images/generations", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")

	upstreamBody := []byte(`{"error":{"code":"organization_suspended","message":"Organization has been suspended"REDACTEDREDACTED`)
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusForbidden,
		Header: http.Header{
			"Content-Type": []string{"application/json"REDACTED,
			"X-Request-Id": []string{"req_images_access_state"REDACTED,
	REDACTED,
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"application/json"REDACTED,
					"X-Request-Id": []string{"req_img_stream_json"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(`{"created":1710000008,"usage":{"input_tokens":12,"output_tokens":21,"output_tokens_details":{"image_tokens":9REDACTEDREDACTED,"data":[{"b64_json":"aGVsbG8=","revised_prompt":"draw a cat"REDACTED]REDACTED`)),
		REDACTED,
	REDACTED,
REDACTED
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"REDACTED,
					"X-Request-Id": []string{"req_img_stream_json_mislabeled"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(`{"created":1710000009,"usage":{"input_tokens":10,"output_tokens":18,"output_tokens_details":{"image_tokens":8REDACTEDREDACTED,"data":[{"b64_json":"ZmluYWw="REDACTED]REDACTED`)),
		REDACTED,
	REDACTED,
REDACTED
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"REDACTED,
					"X-Request-Id": []string{"req_img_stream_multiline"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"image_generation.completed\",\n" +
						"data: \"usage\":{\"input_tokens\":10,\"output_tokens\":18,\"output_tokens_details\":{\"image_tokens\":8REDACTEDREDACTED,\n" +
						"data: \"b64_json\":\"ZmluYWw=\",\"output_format\":\"png\"REDACTED\n\n" +
						"data: [DONE]\n\n",
				)),
		REDACTED,
	REDACTED,
...
func TestExtractOpenAIImagesBillableCountFromJSONBytes_CompletedEvent(t *testing.T) {
	body := []byte(`{"type":"image_generation.completed","b64_json":"ZmluYWw=","usage":{"input_tokens":10,"output_tokens":18REDACTEDREDACTED`)
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"application/json"REDACTED,
					"X-Request-Id": []string{"req_img_edit_apikey"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(`{"created":1710000008,"data":[{"b64_json":"ZWRpdGVk","revised_prompt":"replace background"REDACTED]REDACTED`)),
		REDACTED,
	REDACTED,
REDACTED
	parsed, err := svc.ParseOpenAIImagesRequest(c, body.Bytes())
...
func TestOpenAIGatewayServiceForwardImages_APIKeyStreamingDrainsAfterClientDisconnect(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","stream":trueREDACTED`)
	...
	c, _ := gin.CreateTestContext(rec)
	c.Request = req
	c.Writer = &failingOpenAIImageWriter{ResponseWriter: c.Writer, failAfter: 1REDACTED
	...
	REDACTED,
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"REDACTED,
					"X-Request-Id": []string{"req_img_stream_disconnect_apikey"REDACTED,
			REDACTED,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"image_generation.partial_image\",\"b64_json\":\"cGFydGlhbA==\"REDACTED\n\n" +
						"data: {\"type\":\"image_generation.completed\",\"usage\":{\"input_tokens\":3,\"output_tokens\":4,\"output_tokens_details\":{\"image_tokens\":2REDACTEDREDACTED,\"b64_json\":\"ZmluYWw=\",\"output_format\":\"png\"REDACTED\n\n" +
						"data: [DONE]\n\n",
				)),
		REDACTED,
	REDACTED,
...
REDACTED
	...
	upstream := &httpUpstreamRecorder{
		...
		REDACTED,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000003,\"tools\":[{\"type\":\"image_generation\",\"model\":\"gpt-image-2\",\"background\":\"transparent\",\"output_format\":\"webp\",\"quality\":\"high\",\"size\":\"1024x1024\"REDACTED]REDACTEDREDACTED\n\n" +
					"data: {\"type\":\"response.image_generation_call.partial_image\",\"partial_image_b64\":\"cGFydGlhbA==\",\"partial_image_index\":0,\"output_format\":\"webp\",\"background\":\"transparent\"REDACTED\n\n" +
					"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000003,\"usage\":{\"input_tokens\":7,\"output_tokens\":10,\"output_tokens_details\":{\"image_tokens\":5REDACTEDREDACTED,\"tool_usage\":{\"image_gen\":{\"images\":1REDACTEDREDACTED,\"tools\":[{\"type\":\"image_generation\",\"model\":\"gpt-image-2\",\"background\":\"transparent\",\"output_format\":\"webp\",\"quality\":\"high\",\"size\":\"1024x1024\"REDACTED],\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZWRpdGVk\",\"revised_prompt\":\"replace background with aurora\",\"output_format\":\"webp\"REDACTED]REDACTEDREDACTED\n\n" +
					"data: [DONE]\n\n",
			)),
	REDACTED,
REDACTED
...
func TestCollectOpenAIImagesFromResponsesBody_MultilineSSE(t *testing.T) {
	body := []byte(
		"data: {\"type\":\"response.completed\",\n" +
			"data: \"response\":{\"created_at\":1710000010,\"usage\":{\"input_tokens\":5,\"output_tokens\":9,\"output_tokens_details\":{\"image_tokens\":4REDACTEDREDACTED,\"tool_usage\":{\"image_gen\":{\"images\":1REDACTEDREDACTED,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZmluYWw=\",\"output_format\":\"png\"REDACTED]REDACTEDREDACTED\n\n" +
			"data: [DONE]\n\n",
	)
...
REDACTED
	...
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"REDACTED,
				"X-Request-Id": []string{"req_img_stream_multiline_oauth"REDACTED,
		REDACTED,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.completed\",\n" +
					"data: \"response\":{\"created_at\":1710000011,\"usage\":{\"input_tokens\":6,\"output_tokens\":10,\"output_tokens_details\":{\"image_tokens\":5REDACTEDREDACTED,\"tool_usage\":{\"image_gen\":{\"images\":1REDACTEDREDACTED,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"TXVsdGlsaW5l\",\"output_format\":\"png\"REDACTED]REDACTEDREDACTED\n\n" +
					"data: [DONE]\n\n",
			)),
	REDACTED,
REDACTED
...
REDACTED
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"REDACTED,
				"X-Request-Id": []string{"req_img_stream_disconnect_oauth"REDACTED,
		REDACTED,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.image_generation_call.partial_image\",\"partial_image_b64\":\"cGFydGlhbA==\",\"partial_image_index\":0,\"output_format\":\"png\"REDACTED\n\n" +
					"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000009,\"usage\":{\"input_tokens\":5,\"output_tokens\":9,\"output_tokens_details\":{\"image_tokens\":4REDACTEDREDACTED,\"tool_usage\":{\"image_gen\":{\"images\":1REDACTEDREDACTED,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZmluYWw=\",\"output_format\":\"png\"REDACTED]REDACTEDREDACTED\n\n" +
					"data: [DONE]\n\n",
			)),
	REDACTED,
REDACTED
