...
func (s *OpenAIGatewayService) createUpstreamLiveCall(
	...
	resp, err := s.httpUpstream.DoWithTLS(upstreamReq, resolveAccountProxyURL(account), account.ID, account.Concurrency, s.tlsFPProfileService.ResolveTLSProfile(account))
	if err != nil {
