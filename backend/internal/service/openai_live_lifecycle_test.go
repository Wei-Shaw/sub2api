...
func TestWaitForLiveObserverRetryTreatsStoreErrorAsRetryable(t *testing.T) {
	record := &LiveCallRecord{
		...
		ExpiresAt:  time.Now().Add(time.Hour),
REDACTED
	store := &liveTestStore{getControllerErr: errors.New("redis: connection refused")REDACTED
	require.NoError(t, store.SaveLiveCall(context.Background(), record, time.Hour))
	svc := &OpenAIGatewayService{cache: storeREDACTED
	...
	require.False(t, (&OpenAIGatewayService{cache: &liveTestStore{REDACTEDREDACTED).waitForLiveObserverRetry(record))
REDACTED
...
func TestObserveLiveCallStoreOutageFallsBackToExpiryFinalize(t *testing.T) {
	...
	t.Cleanup(func() { liveObserverStoreRetryInterval = restore REDACTED)

	cases := []struct {
		name   string
		inject func(*liveTestStore)
REDACTED{
		{"GetLiveCall 持续报错", func(s *liveTestStore) { s.getCallErr = errors.New("redis: i/o timeout") REDACTEDREDACTED,
		{"ClaimLiveController 报错", func(s *liveTestStore) { s.claimErr = errors.New("redis: i/o timeout") REDACTEDREDACTED,
REDACTED
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			record := &LiveCallRecord{
...
REDACTED
	store := &liveTestStore{REDACTED
	require.NoError(t, store.SaveLiveCall(context.Background(), record, time.Hour))
	usageRepo := &liveTestBestEffortUsageRepo{bestEffortErr: errors.New("usage log queue dropped")REDACTED
	svc := &OpenAIGatewayService{
		cache:              store,
		concurrencyService: NewConcurrencyService(&liveTestConcurrencyCache{REDACTED),
		usageLogRepo:       usageRepo,
REDACTED
