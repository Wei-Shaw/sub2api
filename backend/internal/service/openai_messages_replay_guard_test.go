...
func TestApplyAnthropicCompatFullReplayGuard_TrimsOldMessages(t *testing.T) {
	...
	req := &apicompat.AnthropicRequest{Messages: make([]apicompat.AnthropicMessage, 0, openAICompatAnthropicReplayMaxTailMessages+3)REDACTED
...
func TestApplyAnthropicCompatFullReplayGuard_KeepsToolBoundaryIntact(t *testing.T) {
	...
	for i := 0; i < openAICompatAnthropicReplayMaxTailMessages+3; i++ {
		...
		if i == 1 {
			role = "assistant"
			content = json.RawMessage(`[{"type":"tool_use","id":"toolu_keep","name":"Read","input":{"file_path":"main.go"REDACTEDREDACTED]`)
	REDACTED
		if i == 3 {
			content = json.RawMessage(`[{"type":"tool_result","tool_use_id":"toolu_keep","content":"ok"REDACTED]`)
	REDACTED
		req.Messages = append(req.Messages, apicompat.AnthropicMessage{
			Role:    role,
			Content: content,
