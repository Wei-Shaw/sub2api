...
func hasChatGPTAccountDeactivatedMarker(obj map[string]any) bool {
	for _, key := range []string{"deactivated", "is_deactivated", "disabled", "is_disabled"REDACTED {
		if value, ok := obj[key].(bool); ok && value {
			return true
	REDACTED
REDACTED
	for _, key := range []string{"deactivated_at", "disabled_at", "deleted_at"REDACTED {
		if value, ok := obj[key].(string); ok && strings.TrimSpace(value) != "" {
			return true
	REDACTED
REDACTED
	for _, key := range []string{"status", "state"REDACTED {
