...
func TestProfitControl_GateReuseKeepsThresholdAcrossFailover(t *testing.T) {
	...
	reCtx := svc.withOpenAIProfitControlGate(ctx, &groupID)
	reGate, ok := reCtx.Value(openAIProfitControlGateCtxKey{REDACTED).(*openAIProfitControlGate)
	require.True(t, ok)
	...
	otherGate, _ := otherCtx.Value(openAIProfitControlGateCtxKey{REDACTED).(*openAIProfitControlGate)
...
func TestProfitControl_PricingAtFixesDownstreamPeakFactor(t *testing.T) {
	...
	ctx := context.WithValue(profitControlTestCtx(group), openAIPricingAtCtxKey{REDACTED, pricingAt)
...
func TestProfitControl_UsesAccountRateInsteadOfProbeSnapshot(t *testing.T) {
	gate := &openAIProfitControlGate{threshold: 0.5, pricingAt: time.Now().Add(-12 * time.Hour)REDACTED
	ctx := context.WithValue(context.Background(), openAIProfitControlGateCtxKey{REDACTED, gate)
...
func TestProfitControl_ResponsesCapabilityUsesTextGateAtScheduler(t *testing.T) {
	...
	svc := &OpenAIGatewayService{
		accountRepo:        stubOpenAIAccountRepo{accounts: []Account{*expensiveREDACTEDREDACTED,
...
func TestProfitControl_AccountRateSemantics(t *testing.T) {
	...
	base := context.WithValue(profitControlTestCtx(group), openAIPricingAtCtxKey{REDACTED, now)
	gate := (&OpenAIGatewayService{REDACTED).resolveOpenAIProfitControlGate(base, &group.ID)
	require.NotNil(t, gate)
	gateCtx := context.WithValue(base, openAIProfitControlGateCtxKey{REDACTED, gate)
...
func TestOpenAIUsagePricingAt(t *testing.T) {
	...
	require.Equal(t, fixed, openAIUsagePricingAt(&OpenAIRecordUsageInput{PricingAt: fixedREDACTED))
	fallback := openAIUsagePricingAt(&OpenAIRecordUsageInput{REDACTED)
...
func TestOpenAIProfitControlStickyBindingOccursOnlyAfterTerminalAdmission(t *testing.T) {
	...
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{cacheKey: expensiveIDREDACTED,
REDACTED
...
REDACTED)
	...
	t.Run("keeps scheduled group of the existing gate", func(t *testing.T) {
		...
		gate, ok := turnCtx.Value(openAIProfitControlGateCtxKey{REDACTED).(*openAIProfitControlGate)
		require.True(t, ok)
		require.NotNil(t, gate)
