...
type openAIProxyStreamQuarantineBypassKey struct{REDACTED

func withOpenAIProxyStreamQuarantineBypass(ctx context.Context) context.Context {
	return context.WithValue(ctx, openAIProxyStreamQuarantineBypassKey{REDACTED, true)
REDACTED
...
func openAIProxyStreamQuarantineBypassed(ctx context.Context) bool {
	if ctx == nil {
		return false
REDACTED
	bypassed, _ := ctx.Value(openAIProxyStreamQuarantineBypassKey{REDACTED).(bool)
	return bypassed
REDACTED
