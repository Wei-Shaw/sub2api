...
REDACTED)
	...
	t.Run("开启时补齐两个百分百阈值并剥离运行态", func(t *testing.T) {
		extra, err := normalizeOpenAIAutoResetCreditExtra(PlatformOpenAI, AccountTypeOAuth, false, map[string]any{
			...
			OpenAIAutoResetCreditStateExtraKey:   map[string]any{"status": "success"REDACTED,
	REDACTED)
	REDACTED
...
REDACTED
	...
	t.Run("卡状态未知时暂停并触发异步查询", func(t *testing.T) {
		account := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: cloneOpenAIAutoResetExtra(baseExtra)REDACTED
...
REDACTED)
	...
	REDACTED
		account := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: extraREDACTED
...
REDACTED)
	...
	t.Run("达到用卡阈值后即使有卡也退出调度", func(t *testing.T) {
		...
		extra[OpenAIAutoResetCreditStateExtraKey] = OpenAIAutoResetCreditState{
			Status: OpenAIAutoResetStatusAvailable, AvailableCount: 1, CheckedAt: now.Format(time.RFC3339),
	REDACTED
		account := &Account{ID: 3, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: extraREDACTED
		paused, decision := shouldAutoPauseOpenAIAccountByQuota(context.Background(), account)
		require.True(t, paused)
...
REDACTED)
	...
	REDACTED
		account := &Account{ID: 4, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: extraREDACTED
		paused, _ := shouldAutoPauseOpenAIAccountByQuota(context.Background(), account)
		require.False(t, paused)
REDACTED)
REDACTED
...
func TestSelectOpenAIAutoResetCandidate_FailsClosed(t *testing.T) {
	candidates := []openAIAutoResetCreditCandidate{
		{ID: "later", ExpiresAt: "2026-09-02T00:00:00Z"REDACTED,
		{ID: "earlier", ExpiresAt: "2026-09-01T00:00:00Z"REDACTED,
REDACTED
...
REDACTED
	...
	_, err = selectOpenAIAutoResetCandidate([]openAIAutoResetCreditCandidate{
		{ExpiresAt: "2026-09-01T00:00:00Z"REDACTED,
REDACTED, 1, nil, "cycle-a")
REDACTED
...
func TestOpenAIQuotaAutoResetService_AssessesIndependentWindows(t *testing.T) {
	service := &OpenAIQuotaAutoResetService{REDACTED
	account := &Account{Extra: map[string]any{
		"auto_pause_5h_disabled": true,
		"auto_pause_7d_disabled": true,
REDACTEDREDACTED
	config := OpenAIAutoResetCreditConfig{Enabled: true, Threshold5h: 0.8, Threshold7d: 0.9REDACTED
	tests := []struct {
		name       string
		fiveHour   float64
		sevenDay   float64
		wantWindow string
REDACTED{
		{name: "5h", fiveHour: 0.8, sevenDay: 0.2, wantWindow: "5h"REDACTED,
		{name: "7d", fiveHour: 0.2, sevenDay: 0.9, wantWindow: "7d"REDACTED,
		{name: "同时触发", fiveHour: 0.95, sevenDay: 0.95, wantWindow: "5h+7d"REDACTED,
REDACTED
...
type autoResetTestQuota struct {
	...
	resetEntered chan struct{REDACTED
	releaseReset chan struct{REDACTED
	enterOnce    sync.Once
	mu           sync.Mutex
	resetArgs    [][2]string
	failFirst    bool
...
REDACTED
	...
	q.mu.Lock()
	q.resetArgs = append(q.resetArgs, [2]string{creditID, redeemRequestIDREDACTED)
	q.mu.Unlock()
	if q.failFirst && call == 1 {
		return nil, context.DeadlineExceeded
REDACTED
	if q.resetEntered != nil {
		q.enterOnce.Do(func() { close(q.resetEntered) REDACTED)
REDACTED
	if q.releaseReset != nil {
		<-q.releaseReset
REDACTED
	return &OpenAIQuotaResetResult{Code: "ok", WindowsReset: 2REDACTED, nil
REDACTED

type autoResetTestRecoverer struct{REDACTED
...
func (autoResetTestRecoverer) RecoverAccountState(context.Context, int64, AccountRecoveryOptions) (*SuccessfulTestRecoveryResult, error) {
	return &SuccessfulTestRecoveryResult{ClearedRateLimit: trueREDACTED, nil
REDACTED
...
func TestOpenAIQuotaAutoResetService_ConcurrentInstancesConsumeOnce(t *testing.T) {
	...
	REDACTED,
REDACTED
	repo := &autoResetTestAccountRepo{account: accountREDACTED
	usage := &OpenAIQuotaUsage{
		FetchedAt: now.Unix(),
		RateLimit: &OpenAIRateLimit{
			PrimaryWindow:   &OpenAIRateLimitWindow{UsedPercent: 100, LimitWindowSeconds: 5 * 60 * 60, ResetAfterSeconds: 3600, ResetAt: now.Add(time.Hour).Unix()REDACTED,
			SecondaryWindow: &OpenAIRateLimitWindow{UsedPercent: 10, LimitWindowSeconds: 7 * 24 * 60 * 60, ResetAfterSeconds: 86400, ResetAt: now.Add(24 * time.Hour).Unix()REDACTED,
	REDACTED,
		RateLimitResetCredits: &OpenAIRateLimitResetCredits{
			AvailableCount: 1,
			Credits:        []OpenAIRateLimitResetCreditDetail{{ExpiresAt: now.Add(48 * time.Hour).Format(time.RFC3339)REDACTEDREDACTED,
	REDACTED,
		autoResetCandidates: []openAIAutoResetCreditCandidate{{ID: "credit-sensitive-id", ExpiresAt: now.Add(48 * time.Hour).Format(time.RFC3339)REDACTEDREDACTED,
REDACTED
	quota := &autoResetTestQuota{usage: usage, resetEntered: make(chan struct{REDACTED), releaseReset: make(chan struct{REDACTED)REDACTED
	...
	serviceA := NewOpenAIQuotaAutoResetService(repo, quota, autoResetTestRecoverer{REDACTED, NewIdempotencyCoordinator(idempotencyRepo, config), nil, nil, nil)
	serviceB := NewOpenAIQuotaAutoResetService(repo, quota, autoResetTestRecoverer{REDACTED, NewIdempotencyCoordinator(idempotencyRepo, config), nil, nil, nil)
...
REDACTED
	...
	quota := &autoResetTestQuota{
		...
		REDACTED,
			RateLimitResetCredits: &OpenAIRateLimitResetCredits{
				AvailableCount: 1,
				Credits:        []OpenAIRateLimitResetCreditDetail{{ExpiresAt: expiresAtREDACTEDREDACTED,
		REDACTED,
			autoResetCandidates: []openAIAutoResetCreditCandidate{{ID: "retry-credit", ExpiresAt: expiresAtREDACTEDREDACTED,
	REDACTED,
REDACTED
	...
	service := NewOpenAIQuotaAutoResetService(
		repo,
		quota,
		autoResetTestRecoverer{REDACTED,
