...
func (s *OpenAIQuotaService) QueryUsage(ctx context.Context, accountID int64) (*OpenAIQuotaUsage, error) {
	...
	if payload.RateLimitResetCredits != nil && payload.RateLimitResetCredits.AvailableCount > 0 {
		payload.RateLimitResetCredits.Credits = s.queryResetCreditDetails(callCtx, proxyURL, account, router, accessToken, chatGPTAccountID, fedRAMP, accountID)
	}
	return &payload, nil
}

// queryResetCreditDetails fetches the sanitized reset-credit metadata. Best-effort:
// failures only log a warning and return nil. Like QueryUsage/ResetCredit it goes
// through doQuotaRequest so the router/account TLS fingerprint and UA are applied.
func (s *OpenAIQuotaService) queryResetCreditDetails(ctx context.Context, proxyURL string, account *Account, router *model.TLSFingerprintRouter, accessToken, chatGPTAccountID string, fedRAMP bool, accountID int64) []OpenAIRateLimitResetCreditDetail {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, chatGPTRateLimitCreditsURL, nil)
	if err != nil {
	...
	credits, err := parseOpenAIRateLimitResetCreditDetails(body)
	if err != nil {
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID int64) (accessToken, chatGPTAccountID, proxyURL string, fedRAMP bool, account *Account, err error) {
	...
	if account.IsShadow() {
		...
		if rerr != nil {
			return "", "", "", false, nil, infraerrors.Newf(http.StatusBadGateway, "OPENAI_QUOTA_SHADOW_RESOLVE_FAILED", "failed to resolve shadow account: %v", rerr)
		}
		account = resolved
	}
