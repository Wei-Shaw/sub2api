...
REDACTED
	...
	if details != nil {
		...
		if payload.RateLimitResetCredits == nil {
			payload.RateLimitResetCredits = &OpenAIRateLimitResetCredits{REDACTED
	REDACTED
...
REDACTED
	...
	REDACTED
		...
		resp, err := client.R().
			SetContext(callCtx).
			SetHeaders(headers).
			SetBody(map[string]string{"redeem_request_id": redeemRequestIDREDACTED).
			SetSuccessResult(&payload).
...
REDACTED
	if account.IsShadow() {
		if resolved, resolveErr := resolveCredentialAccount(ctx, s.accountRepo, account); resolveErr == nil && resolved != nil {
			account = resolved
	REDACTED else if strings.TrimSpace(accessToken) == "" {
...
REDACTED
	...
	snap := &OpenAICodexUsageSnapshot{REDACTED
