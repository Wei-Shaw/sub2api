...
func (s *OpenAIQuotaService) QueryUsage(ctx context.Context, accountID int64) (*OpenAIQuotaUsage, error) {
	authorization, chatGPTAccountID, proxyURL, fedRAMP, err := s.prepareUpstreamCall(ctx, accountID)
	if err != nil {
		return nil, err
	}
	...
	resp, err := client.R().
		SetContext(callCtx).
		SetHeaders(buildCodexCommonHeaders(authorization, chatGPTAccountID, fedRAMP)).
	...
	details := s.queryResetCreditDetails(callCtx, client, authorization, chatGPTAccountID, fedRAMP, accountID)
	if details != nil {
...
func (s *OpenAIQuotaService) queryResetCreditDetails(ctx context.Context, client *req.Client, authorization, chatGPTAccountID string, fedRAMP bool, accountID int64) *openAIRateLimitResetCreditDetails {
	resp, err := client.R().
		SetContext(ctx).
		SetHeaders(buildCodexCommonHeaders(authorization, chatGPTAccountID, fedRAMP)).
...
func (s *OpenAIQuotaService) ResetCredit(ctx context.Context, accountID int64) (*OpenAIQuotaResetResult, error) {
	...
	headers := buildCodexCommonHeaders(authorization, chatGPTAccountID, fedRAMP)
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID int64) (authorization, chatGPTAccountID, proxyURL string, fedRAMP bool, err error) {
	...
	authResult, err := NewOpenAIRequestAuthProvider(s.accountRepo, s.tokenProvider).Build(ctx, account)
	if err != nil {
		return "", "", "", false, infraerrors.Newf(http.StatusBadGateway, "OPENAI_QUOTA_TOKEN_UNAVAILABLE", "failed to acquire request authentication: %v", err)
	}
	authorization = strings.TrimSpace(authResult.Headers.Get("Authorization"))
	if authorization == "" {
		return "", "", "", false, infraerrors.New(http.StatusBadGateway, "OPENAI_QUOTA_TOKEN_UNAVAILABLE", "request authorization is empty")
	}
	...
	return authorization, chatGPTAccountID, proxyURL, fedRAMP, nil
}
...
func buildCodexCommonHeaders(authorization, chatGPTAccountID string, fedRAMP bool) map[string]string {
