...
// quotaRedirectingUpstream 实现 HTTPUpstream，将请求重定向到 httptest.Server。
// （配额服务出站已统一走 httpUpstream.DoWithTLS，测试桩相应从 req.Client 工厂换成 HTTPUpstream。）
...
func (u *quotaRedirectingUpstream) Do(r *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
	r.URL.Scheme = u.target.Scheme
	r.URL.Host = u.target.Host
	return http.DefaultClient.Do(r)
}

func (u *quotaRedirectingUpstream) DoWithTLS(r *http.Request, proxyURL string, accountID int64, concurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func TestPrepareUpstreamCallShadowResolve(t *testing.T) {
	...
	// prepareUpstreamCall 在返回前不发上游请求；传零值 upstream 仅满足非 nil 校验
	svc := NewOpenAIQuotaService(repo, nil, tokenProvider, &quotaRedirectingUpstream{}, nil, nil)

	_, chatGPTAccountID, _, _, _, err := svc.prepareUpstreamCall(ctx, 200)
...
func TestQueryUsageIncludesResetCreditExpirations_EndToEnd(t *testing.T) {
	...
	svc := NewOpenAIQuotaService(repo, nil, tokenProvider, newQuotaRedirectingUpstream(srv), nil, nil)
