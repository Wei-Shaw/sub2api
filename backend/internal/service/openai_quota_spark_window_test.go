...
func (c *stubQuotaTokenCache) DeleteAccessToken(_ context.Context, _ string) error { return nil REDACTED

func (c *stubQuotaTokenCache) AcquireRefreshLock(_ context.Context, _ string, _ time.Duration) (bool, error) {
	return true, nil
REDACTED

func (c *stubQuotaTokenCache) ReleaseRefreshLock(_ context.Context, _ string) error { return nil REDACTED
...
func TestBuildCodexSparkWindowExtraUpdates_NoBengalfox(t *testing.T) {
	usage := &OpenAIQuotaUsage{
		AdditionalRateLimits: []OpenAIAdditionalRateLimit{
			{MeteredFeature: "other_feature", RateLimit: &OpenAIRateLimit{REDACTEDREDACTED,
	REDACTED,
REDACTED
...
REDACTED
	repo := &stubQuotaAccountRepo{
		accounts: map[int64]*Account{200: shadowREDACTED,
REDACTED
...
REDACTED
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(r.URL.Path, "/task/register") {
			registerCalls++
			_, _ = w.Write([]byte(`{"task_id":"task-reset-new"REDACTED`))
			return
	REDACTED
		resetCalls++
		...
		if resetCalls == 1 {
			...
			return
	REDACTED
		_, _ = w.Write([]byte(`{"code":"ok","windows_reset":2REDACTED`))
REDACTED))
	defer srv.Close()
...
REDACTED
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(r.URL.Path, "/task/register") {
			registerCalls++
			_, _ = w.Write([]byte(`{"task_id":"task-reset-unexpected"REDACTED`))
			return
	REDACTED
		resetCalls++
		assertions = append(assertions, r.Header.Get("authorization"))
...
REDACTED
	...
	REDACTED,
REDACTED
	repo := &stubQuotaAccountRepo{accounts: map[int64]*Account{200: shadow, 100: parentREDACTEDREDACTED
...
REDACTED{
		{
			name: "credits",
			body: `{"credits":[{"id":"secret-id","expires_at":"2026-07-03T04:05:06Z"REDACTED]REDACTED`,
			want: []string{"2026-07-03T04:05:06Z"REDACTED,
	REDACTED,
		{
			name: "rate limit reset credits",
			body: `{"rate_limit_reset_credits":[{"expiresAt":"2026-07-04T04:05:06Z"REDACTED]REDACTED`,
			want: []string{"2026-07-04T04:05:06Z"REDACTED,
	REDACTED,
		{
			name: "items",
			body: `{"items":[{"expires_at":"2026-07-05T04:05:06Z"REDACTED]REDACTED`,
			want: []string{"2026-07-05T04:05:06Z"REDACTED,
	REDACTED,
		{
			name: "data",
			body: `{"data":[{"expires_at":"2026-07-06T04:05:06Z"REDACTED]REDACTED`,
			want: []string{"2026-07-06T04:05:06Z"REDACTED,
	REDACTED,
		{
			name: "array",
			body: `[{"expires_at":"2026-07-07T04:05:06Z"REDACTED]`,
			want: []string{"2026-07-07T04:05:06Z"REDACTED,
	REDACTED,
REDACTED
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/backend-api/wham/usage":
			_ = json.NewEncoder(w).Encode(OpenAIQuotaUsage{
				RateLimitResetCredits: &OpenAIRateLimitResetCredits{AvailableCount: 2REDACTED,
		REDACTED)
		case "/backend-api/wham/rate-limit-reset-credits":
			detailCalls++
			capturedBeta = r.Header.Get("OpenAI-Beta")
			require.Equal(t, "org-parent123", r.Header.Get("ChatGPT-Account-ID"))
			_, _ = w.Write([]byte(`{"credits":[{"id":"secret-credit-id","expires_at":"2026-07-03T04:05:06Z"REDACTED,{"expiresAt":"2026-07-04T04:05:06Z"REDACTED]REDACTED`))
		default:
			http.NotFound(w, r)
	REDACTED
REDACTED))
...
REDACTED, usage.RateLimitResetCredits.Credits)
	...
	require.Equal(t, &OpenAIRateLimitResetCredits{
		...
		Credits: []OpenAIRateLimitResetCreditDetail{
			...
			{ExpiresAt: "2026-07-04T04:05:06Z"REDACTED,
	REDACTED,
REDACTED, repo.extraUpdates[100][openaiQuotaResetCreditsKey])
...
REDACTEDREDACTED
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/backend-api/wham/usage":
			_ = json.NewEncoder(w).Encode(OpenAIQuotaUsage{
				RateLimitResetCredits: &OpenAIRateLimitResetCredits{AvailableCount: 1REDACTED,
		REDACTED)
		case "/backend-api/wham/rate-limit-reset-credits":
			detailCalls++
			w.WriteHeader(http.StatusUnauthorized)
			_, _ = w.Write([]byte(`{"error":"unauthorized","id":"secret-error-id"REDACTED`))
		default:
			http.NotFound(w, r)
	REDACTED
REDACTED))
...
func TestCacheResetCreditsSnapshot(t *testing.T) {
	...
	t.Run("zero count allows an empty expiration list", func(t *testing.T) {
		repo := &stubQuotaAccountRepo{REDACTED
		svc := &OpenAIQuotaService{accountRepo: repoREDACTED
		credits := &OpenAIRateLimitResetCredits{AvailableCount: 0REDACTED
...
REDACTED)
	...
	t.Run("missing expiration list preserves the cache", func(t *testing.T) {
		...
		err := svc.CacheResetCreditsSnapshot(ctx, 100, &OpenAIRateLimitResetCredits{AvailableCount: 1REDACTED)
...
REDACTED)
	...
	t.Run("empty expiration list with a positive count preserves the cache", func(t *testing.T) {
		...
		err := svc.CacheResetCreditsSnapshot(ctx, 100, &OpenAIRateLimitResetCredits{
			AvailableCount: 2,
			Credits:        []OpenAIRateLimitResetCreditDetail{REDACTED,
	REDACTED)
...
REDACTED)
	...
	t.Run("repository errors are returned", func(t *testing.T) {
		repo := &stubQuotaAccountRepo{extraUpdateErr: errors.New("database unavailable")REDACTED
		svc := &OpenAIQuotaService{accountRepo: repoREDACTED

		err := svc.CacheResetCreditsSnapshot(ctx, 100, &OpenAIRateLimitResetCredits{
			AvailableCount: 1,
			Credits:        []OpenAIRateLimitResetCreditDetail{{ExpiresAt: "2026-07-03T04:05:06Z"REDACTEDREDACTED,
	REDACTED)
...
func TestResetCreditGetByIDError_FailsClosed(t *testing.T) {
	...
	repo := &stubQuotaAccountRepo{accounts: map[int64]*Account{REDACTEDREDACTED
...
REDACTED"chatgpt_account_id": "org-e2e-parent"REDACTED,
REDACTED
	repo := &stubQuotaAccountRepo{accounts: map[int64]*Account{200: shadow, 100: parentREDACTEDREDACTED
...
REDACTEDREDACTED
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("content-type", "application/json")
		_ = json.NewEncoder(w).Encode(OpenAIQuotaUsage{REDACTED)
REDACTED))
	defer srv.Close()
