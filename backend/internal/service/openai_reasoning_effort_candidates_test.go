...
func TestExtractOpenAIReasoningEffortFromBodyModelCandidates(t *testing.T) {
	bodyWithoutEffort := []byte(`{"model":"whatever","input":"hello"REDACTED`)
	bodyWithMax := []byte(`{"model":"sol","reasoning":{"effort":"max"REDACTED,"input":"hello"REDACTED`)
...
REDACTED{
		{
			...
			candidates: []string{"gpt-5.4", "gpt-5.4", "gpt-5.4-xhigh"REDACTED,
			want:       "xhigh",
	REDACTED,
		{
			name:       "GPT-5.6 后缀 max 经原始模型推导保留",
			body:       bodyWithoutEffort,
			candidates: []string{"gpt-5.6-sol", "gpt-5.6-sol", "gpt-5.6-sol-max"REDACTED,
			want:       "max",
	REDACTED,
		{
			name:       "显式 max 用第一个非空候选（映射后模型）判定",
			body:       bodyWithMax,
			candidates: []string{"gpt-5.6-sol", "sol"REDACTED,
			want:       "max",
	REDACTED,
		{
			name:       "显式 max 非 5.6 首候选仍折叠为 xhigh",
			body:       bodyWithMax,
			candidates: []string{"gpt-5.4", "sol"REDACTED,
			want:       "xhigh",
	REDACTED,
		{
			name:       "所有候选均无后缀时返回 nil",
			body:       bodyWithoutEffort,
			candidates: []string{"gpt-5.4", "gpt-5.4", "gpt-5.4"REDACTED,
			want:       "",
	REDACTED,
REDACTED
...
func TestExtractOpenAIReasoningEffortModelCandidates(t *testing.T) {
	reqBody := map[string]any{"model": "gpt-5.3-codex-high", "input": "hello"REDACTED
...
REDACTED
	...
	body := []byte(`{"model":"gpt-5.3-codex-xhigh","instructions":"suffix-test","input":"hello","stream":falseREDACTED`)
