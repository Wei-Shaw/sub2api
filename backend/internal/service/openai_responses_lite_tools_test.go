...
func TestNormalizeOpenAIResponsesLiteTools_RejectsConflictingAdditionalTool(t *testing.T) {
	reqBody := map[string]any{
		"tools": []any{map[string]any{
			"type":  "namespace",
			"name":  "collaboration",
			"tools": []any{map[string]any{"type": "function", "name": "spawn_agent"REDACTEDREDACTED,
REDACTED
		"input": []any{map[string]any{
			...
			"tools": []any{map[string]any{
				"type":  "namespace",
				"name":  "collaboration",
				"tools": []any{map[string]any{"type": "function", "name": "send_message"REDACTEDREDACTED,
	REDACTED
REDACTED
REDACTED
...
REDACTED
	reqBody := map[string]any{
		"tools": []any{namespaceREDACTED,
		"input": []any{
			map[string]any{
				"type":  "additional_tools",
				"tools": []any{map[string]any{"type": "custom", "name": "exec"REDACTEDREDACTED,
		REDACTED,
			map[string]any{
				"type":  "additional_tools",
				"tools": []any{namespaceREDACTED,
...
REDACTED{
		{
			...
			body: map[string]any{
				"tools":               []any{map[string]any{"type": "function", "name": "shell"REDACTEDREDACTED,
	...
	REDACTED,
		{
			...
			body: map[string]any{
				"input": []any{map[string]any{
					"type":  "additional_tools",
					"tools": []any{map[string]any{"type": "namespace", "name": "collaboration"REDACTEDREDACTED,
		REDACTED
		REDACTED,
	REDACTED,
REDACTED
...
func TestNormalizeOpenAIResponsesLiteTools_RejectsNonBooleanParallelToolCalls(t *testing.T) {
	for _, value := range []any{"false", float64(0), nil, map[string]any{REDACTEDREDACTED {
		reqBody := map[string]any{
			"tools":               []any{map[string]any{"type": "function", "name": "shell"REDACTEDREDACTED,
...
REDACTED
	...
	reqBody := map[string]any{"parallel_tool_calls": []any{REDACTEDREDACTED
	changed, err := normalizeOpenAIResponsesLiteTools(reqBody)
	require.ErrorContains(t, err, "parallel_tool_calls to be a boolean")
	require.False(t, changed)
REDACTED
...
func TestNormalizeOpenAIResponsesLiteTools_RejectsUnsupportedTools(t *testing.T) {
	tests := []struct {
		...
		want string
REDACTED{
		{name: "hosted web search", tool: map[string]any{"type": "web_search"REDACTED, want: `top-level tool type "web_search"`REDACTED,
		{name: "hosted image generation", tool: map[string]any{"type": "image_generation"REDACTED, want: `top-level tool type "image_generation"`REDACTED,
REDACTED

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			reqBody := map[string]any{"tools": []any{tt.toolREDACTEDREDACTED
			changed, err := normalizeOpenAIResponsesLiteTools(reqBody)
...
func TestOpenAIGatewayServiceForward_NormalizesResponsesLiteToolsForOAuth(t *testing.T) {
	...
	REDACTED
		t.Run(name, func(t *testing.T) {
			...
			upstream := &httpUpstreamRecorder{resp: &http.Response{
				...
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_lite\",\"usage\":{\"input_tokens\":1,\"output_tokens\":1REDACTEDREDACTEDREDACTED\n\n" +
						"data: [DONE]\n\n",
				)),
		REDACTEDREDACTED
		...
		REDACTED
			...
			badUpstream := &httpUpstreamRecorder{REDACTED
			svc.httpUpstream = badUpstream

			result, err = svc.Forward(context.Background(), badCtx, account, []byte(`{"model":"gpt-5.6-terra","tools":[{"type":"function","name":"shell"REDACTED],"parallel_tool_calls":"false"REDACTED`))
			...
			for _, malformed := range []struct {
				...
				wantParam string
		REDACTED{
				{body: `{"model":"gpt-5.6-terra","tools":{REDACTEDREDACTED`, wantParam: "tools"REDACTED,
				{body: `{"model":"gpt-5.6-terra","reasoning":[]REDACTED`, wantParam: "reasoning"REDACTED,
		REDACTED {
				rec := httptest.NewRecorder()
