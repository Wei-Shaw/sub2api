...
REDACTED
	...
	sort.Slice(hits, func(i, j int) bool { return hits[i].offset < hits[j].offset REDACTED)
...
REDACTED
	...
	REDACTED
		...
		for _, suffix := range []string{"parameters", "function.parameters"REDACTED {
...
REDACTED
	if !bytes.Equal(body[typ.Index:end], []byte(typ.Raw)) {
		return
REDACTED
	*hits = append(*hits, openAIResponsesToolSchemaNullType{offset: typ.Index, length: len(typ.Raw)REDACTED)
REDACTED
