...
REDACTED))
	...
	t.Cleanup(func() { chatGPTSubscriptionsURL = oldURL REDACTED)
...
REDACTED))
	...
	t.Cleanup(func() { chatGPTAccountsCheckURL = oldURL REDACTED)

	got := fetchChatGPTAccountInfo(context.Background(), func(proxyURL string) (*req.Client, error) {
		return req.C().SetTimeout(5 * time.Second), nil
REDACTED, "access-token", "", "org-expired-workspace")
...
REDACTED))
	...
	got := fetchChatGPTAccountInfo(context.Background(), func(proxyURL string) (*req.Client, error) {
		return req.C().SetTimeout(5 * time.Second), nil
REDACTED, "access-token", "", "org-deactivated-workspace")
...
func TestChatGPTAccountInfoBelongsToTokenAccount(t *testing.T) {
	require.False(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{ChatGPTAccountID: "personal-a"REDACTED, &ChatGPTAccountInfo{AccountID: "workspace-b"REDACTED))
	require.True(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{ChatGPTAccountID: "personal-a"REDACTED, &ChatGPTAccountInfo{AccountID: "PERSONAL-A"REDACTED))
	// 任一侧缺 ID 时无法区分，保持既有行为（采用 accounts/check 的值）。
	require.True(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{REDACTED, &ChatGPTAccountInfo{AccountID: "workspace-b"REDACTED))
	require.True(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{ChatGPTAccountID: "personal-a"REDACTED, &ChatGPTAccountInfo{REDACTED))
REDACTED
			...
			"accounts": map[string]any{
				"default": map[string]any{
					"account": map[string]any{
						...
						"is_default": true,
				REDACTED,
					"entitlement": map[string]any{"expires_at": futureAtREDACTED,
			REDACTED,
		REDACTED,
	REDACTED)
REDACTED))
			...
			"accounts": map[string]any{
				workspaceAccountID: map[string]any{
					"account": map[string]any{
						...
						"is_default": true,
				REDACTED,
					"entitlement": map[string]any{"expires_at": workspaceExpiresAtREDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
		onSubscription: func(accountID string) map[string]any {
			...
			return map[string]any{"plan_type": "pro", "active_until": personalActiveUntil, "will_renew": trueREDACTED
	REDACTED,
REDACTED)
	defer server.Close()
...
REDACTED
	svc := &OpenAIOAuthService{privacyClientFactory: newTestPrivacyClientFactory()REDACTED
			...
			"accounts": map[string]any{
				personalAccountID: map[string]any{
					"account": map[string]any{
						...
						"is_default": true,
				REDACTED,
					"entitlement": map[string]any{"expires_at": entitlementExpiresAtREDACTED,
			REDACTED,
		REDACTED,
	REDACTED,
