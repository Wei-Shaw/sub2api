...
// {type:"upstream_error", message:"Upstream request failed"REDACTED 会把这些信息全部抹掉。
//
...
func writeOpenAIUpstreamClientError(c *gin.Context, statusCode int, body []byte, upstreamMsg string) {
	errorPayload := gin.H{"type": openAIUpstreamClientErrorFallbackTypeREDACTED
...
REDACTED
	...
	c.JSON(statusCode, gin.H{"error": errorPayloadREDACTED)
REDACTED
