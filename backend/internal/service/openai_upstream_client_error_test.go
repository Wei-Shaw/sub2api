...
const openAIInvalidFunctionParametersBody = `{"error":{` +
	...
	`"code":"invalid_function_parameters"REDACTEDREDACTED`
...
REDACTEDID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Name: "acct"REDACTED
REDACTED
...
func TestHandleErrorResponse_Deterministic400WithoutUpstreamMetadata(t *testing.T) {
	...
	_, err := svc.handleErrorResponse(
		context.Background(),
		newOpenAIUpstreamErrorResponse(http.StatusBadRequest, `{"error":{"message":"Invalid 'input': expected an array."REDACTEDREDACTED`),
		c, newOpenAIUpstreamErrorTestAccount(), nil,
	)
...
REDACTED{
		...
		{"not_found", http.StatusNotFound, `{"error":{"message":"Unknown request URL"REDACTEDREDACTED`,
			http.StatusBadGateway, "upstream_error", "Upstream request failed"REDACTED,
		{"unprocessable", http.StatusUnprocessableEntity, `{"error":{"message":"Invalid schema for field messages"REDACTEDREDACTED`,
			http.StatusBadGateway, "upstream_error", "Upstream request failed"REDACTED,
		// 401/402/403 是网关运营方的凭据/账单问题，必须继续对客户端屏蔽上游账号状态。
		{"unauthorized", http.StatusUnauthorized, `{"error":{"message":"Incorrect API key provided: sk-abc"REDACTEDREDACTED`,
			http.StatusBadGateway, "upstream_error", "Upstream authentication failed, please contact administrator"REDACTED,
		{"forbidden", http.StatusForbidden, `{"error":{"message":"Your account is deactivated"REDACTEDREDACTED`,
			http.StatusBadGateway, "upstream_error", "Upstream access forbidden, please contact administrator"REDACTED,
		// 429 保持独立映射。
		{"rate_limited", http.StatusTooManyRequests, `{"error":{"message":"Rate limit reached"REDACTEDREDACTED`,
			http.StatusTooManyRequests, "rate_limit_error", "Upstream rate limit exceeded, please retry later"REDACTED,
REDACTED
	...
	REDACTED,
		{
			...
			body:        `{"error":{"type":"invalid_prompt","message":"blocked"REDACTEDREDACTED`,
			upstreamMsg: "blocked",
	...
	REDACTED,
		{
			...
			body:        `{"error":{"message":"failed for key=secret123"REDACTEDREDACTED`,
