...
var openAITransportFailoverBody = []byte(`{"error":{"type":"upstream_error","message":"Upstream request failed"REDACTEDREDACTED`)
...
func classifyOpenAITransportError(err error) openAITransportErrorClass {
	if err == nil {
		return openAITransportErrorClass{REDACTED
REDACTED
	...
	if errors.Is(err, syscall.ECONNREFUSED) ||
		...
		return openAITransportErrorClass{Persistent: trueREDACTED
REDACTED
