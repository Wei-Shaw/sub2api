...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if account.Platform == PlatformGrok && strings.TrimSpace(token) == "" {
	...
	wsHeaders, _, buildHdrErr := s.buildOpenAIWSHeaders(ctx, c, account, wsDecision, isCodexCLI, turnState, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), firstPayload.promptCacheKey)
	if buildHdrErr != nil {
	...
	for {
		...
		if nextPayload.promptCacheKey != "" {
			...
			updatedHeaders, _, updHdrErr := s.buildOpenAIWSHeaders(ctx, c, account, wsDecision, isCodexCLI, turnState, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), nextPayload.promptCacheKey)
			if updHdrErr != nil {
