...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	if shouldClearStickySessionForContext(ctx, account, requestedModel) || !account.IsOpenAI() || !isOpenAIAccountSchedulableForContext(ctx, account) {
		_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
		return 0, nil, "", nil
	}
	...
	if s.schedulerSnapshot != nil && s.accountRepo != nil {
		...
		if shouldClearStickySessionForContext(ctx, latest, requestedModel) || !latest.IsOpenAI() || !isOpenAIAccountSchedulableForContext(ctx, latest) {
			_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
			return 0, nil, "", nil
		}
