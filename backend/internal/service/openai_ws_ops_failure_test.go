...
func TestMarkOpenAIWSClientVisibleFailure_ResponseFailedNestedError(t *testing.T) {
	...
	markOpenAIWSClientVisibleFailure(c, "response.failed", []byte(`{"type":"response.failed","response":{"error":{"type":"invalid_request_error","code":"context_length_exceeded","message":"too long","status_code":400REDACTEDREDACTEDREDACTED`))
...
func TestMarkOpenAIWSClientVisibleFailure_ErrorAndSuccessBoundary(t *testing.T) {
	t.Run("error", func(t *testing.T) {
		c, _ := gin.CreateTestContext(nil)
		markOpenAIWSClientVisibleFailure(c, "error", []byte(`{"type":"error","error":{"type":"rate_limit_error","code":"rate_limit_exceeded","message":"slow down"REDACTEDREDACTED`))
		got, ok := GetOpsStreamError(c)
		require.True(t, ok)
...
REDACTED)
	...
	t.Run("completed does not mark", func(t *testing.T) {
		c, _ := gin.CreateTestContext(nil)
		markOpenAIWSClientVisibleFailure(c, "response.completed", []byte(`{"type":"response.completed"REDACTED`))
