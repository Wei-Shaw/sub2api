...
func TestOpenAIWSSessionPreemptRegistryCancelsSameScopedSessionOnly(t *testing.T) {
	...
	key := openAIWSSessionPreemptKey{groupID: 7, apiKeyID: 11, sessionHash: "sess"REDACTED
	other := openAIWSSessionPreemptKey{groupID: 7, apiKeyID: 12, sessionHash: "sess"REDACTED
...
func TestOpenAIWSSessionPreemptContextEligibilityAndLocalCancellation(t *testing.T) {
	...
	svc := &OpenAIGatewayService{openaiWSStateStore: stateStoreREDACTED
	oauth := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	apiKey := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyREDACTED
	grok := &Account{ID: 3, Platform: PlatformGrok, Type: AccountTypeOAuthREDACTED
...
func TestOpenAIWSIngressSessionPreemptionSurvivesNestedForwardCleanup(t *testing.T) {
	...
	newContext := func() *gin.Context {
		...
		c.Set("api_key", &APIKey{ID: 11, GroupID: &groupIDREDACTED)
		return c
REDACTED

	svc := &OpenAIGatewayService{REDACTED
	account := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
	firstMessage := []byte(`{"type":"response.create","prompt_cache_key":"session-1","input":"hello"REDACTED`)
...
func TestOpenAIWSSessionPreemptRemoteClaimAndStaleReleaseAreAtomic(t *testing.T) {
	cache := &openAIWSSessionPreemptCacheStub{REDACTED
