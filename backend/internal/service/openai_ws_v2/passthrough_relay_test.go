...
func TestRelay_OnTurnComplete_BareErrorWithoutIDBeforeLaterCompleted(t *testing.T) {
	...
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"error","usage":{"input_tokens":5,"output_tokens":1REDACTED,"error":{"message":"first turn failed"REDACTEDREDACTED`),
	REDACTED,
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"response.completed","response":{"id":"resp_next","usage":{"input_tokens":3,"output_tokens":2REDACTEDREDACTEDREDACTED`),
	REDACTED,
REDACTED, true)
	...
	result, relayExit := Relay(
		...
		clientConn,
		upstreamConn,
		[]byte(`{"type":"response.create","model":"gpt-5.6-sol","input":[]REDACTED`),
		RelayOptions{OnTurnComplete: func(turn RelayTurnResult) { turns = append(turns, turn) REDACTEDREDACTED,
	)
	...
	require.Equal(t, Usage{InputTokens: 5, OutputTokens: 1REDACTED, turns[0].Usage)
	require.Equal(t, "response.completed", turns[1].TerminalEventType)
	require.Equal(t, "resp_next", turns[1].RequestID)
	require.Equal(t, Usage{InputTokens: 3, OutputTokens: 2REDACTED, turns[1].Usage)
	require.Equal(t, Usage{InputTokens: 8, OutputTokens: 3REDACTED, result.Usage)
REDACTED
...
func TestRelay_OnTurnComplete_AuxiliaryFrameDoesNotSettleBareErrorBeforeFailed(t *testing.T) {
	...
	clientConn := newPassthroughTestFrameConn(nil, false)
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
		{msgType: coderws.MessageText, payload: []byte(`{"type":"error","error":{"code":"server_error","message":"failed"REDACTEDREDACTED`)REDACTED,
		{msgType: coderws.MessageText, payload: []byte(`{"type":"rate_limits.updated","rate_limits":[{"name":"requests","remaining":1REDACTED]REDACTED`)REDACTED,
		{msgType: coderws.MessageText, payload: []byte(`{"type":"response.failed","response":{"id":"resp_authoritative","usage":{"input_tokens":7,"output_tokens":2REDACTED,"error":{"code":"server_error","message":"failed"REDACTEDREDACTEDREDACTED`)REDACTED,
REDACTED, true)

	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	turns := make([]RelayTurnResult, 0, 1)
	result, relayExit := Relay(ctx, clientConn, upstreamConn, []byte(`{"type":"response.create","model":"gpt-5.6-sol","input":[]REDACTED`), RelayOptions{
		OnTurnComplete: func(turn RelayTurnResult) { turns = append(turns, turn) REDACTED,
REDACTED)
	...
	require.Equal(t, Usage{InputTokens: 7, OutputTokens: 2REDACTED, turns[0].Usage)
...
func TestRelay_OnTurnComplete_UsesResponseCreateTimeAcrossPricingBoundary(t *testing.T) {
	...
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"response.completed","response":{"id":"resp_boundary","usage":{"input_tokens":1,"output_tokens":1REDACTEDREDACTEDREDACTED`),
	REDACTED,
REDACTED, true)
...
REDACTED
	...
	_, relayExit := Relay(
		...
		clientConn,
		upstreamConn,
		[]byte(`{"type":"response.create","model":"gpt-5.3-codex","input":[]REDACTED`),
		RelayOptions{
			Now:            nowFn,
			OnTurnComplete: func(current RelayTurnResult) { turn = current REDACTED,
	REDACTED,
	)
...
func TestRelay_OnTurnComplete_UsesExplicitFirstTurnStartedAt(t *testing.T) {
	...
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"response.completed","response":{"id":"resp_initial_boundary","usage":{"input_tokens":1,"output_tokens":1REDACTEDREDACTEDREDACTED`),
	REDACTED,
REDACTED, true)
	...
	_, relayExit := Relay(
		...
		RelayOptions{
			...
			Now:                func() time.Time { return relayStartedAt REDACTED,
			OnTurnComplete:     func(current RelayTurnResult) { turn = current REDACTED,
	REDACTED,
	)
...
func TestRelay_OnTurnComplete_UsesSubsequentResponseCreateTimeAcrossPricingBoundary(t *testing.T) {
	...
	go func() {
		...
		_, _ = Relay(
			...
			[]byte(`{"type":"response.create","model":"gpt-5.3-codex","input":[]REDACTED`),
			RelayOptions{
				Now: func() time.Time { return time.Unix(0, clock.Load()).UTC() REDACTED,
				OnTurnComplete: func(current RelayTurnResult) {
					turns <- current
			REDACTED,
		REDACTED,
...
REDACTED()
	...
	require.Eventually(t, func() bool { return len(upstreamConn.Writes()) == 1 REDACTED, time.Second, time.Millisecond)
	upstreamConn.readCh <- passthroughTestFrame{
		msgType: coderws.MessageText,
		payload: []byte(`{"type":"response.completed","response":{"id":"resp_first","usage":{"input_tokens":1,"output_tokens":1REDACTEDREDACTEDREDACTED`),
REDACTED
	select {
	case <-turns:
	case <-time.After(time.Second):
...
REDACTED
	...
	clientConn.readCh <- passthroughTestFrame{
		msgType: coderws.MessageText,
		payload: []byte(`{"type":"response.create","model":"gpt-5.3-codex","input":[]REDACTED`),
REDACTED
	require.Eventually(t, func() bool { return len(upstreamConn.Writes()) == 2 REDACTED, time.Second, time.Millisecond)
	clock.Store(secondResponseAt.UnixNano())
	upstreamConn.readCh <- passthroughTestFrame{
		msgType: coderws.MessageText,
		payload: []byte(`{"type":"response.completed","response":{"id":"resp_second","usage":{"input_tokens":1,"output_tokens":1REDACTEDREDACTEDREDACTED`),
REDACTED
...
func TestRelay_NoSemanticOutputTerminalSequence_FirstTokenMsNil(t *testing.T) {
	...
	for _, terminalEvent := range []string{"response.completed", "response.done"REDACTED {
		...
		t.Run(terminalEvent, func(t *testing.T) {
			...
			upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
				{
					msgType: coderws.MessageText,
					payload: []byte(`{"type":"response.created","response":{"id":"resp_no_output"REDACTEDREDACTED`),
			REDACTED,
				{
					msgType: coderws.MessageText,
					payload: []byte(`{"type":"response.in_progress","response":{"id":"resp_no_output"REDACTEDREDACTED`),
			REDACTED,
				{
					msgType: coderws.MessageText,
					payload: []byte(`{"type":"response.content_part.done","response_id":"resp_no_output"REDACTED`),
			REDACTED,
				{
					msgType: coderws.MessageText,
					payload: []byte(`{"type":"response.output_item.done","response_id":"resp_no_output"REDACTED`),
			REDACTED,
				{
					msgType: coderws.MessageText,
					payload: []byte(`{"type":"` + terminalEvent + `","response":{"id":"resp_no_output","usage":{"input_tokens":2,"output_tokens":0REDACTEDREDACTEDREDACTED`),
			REDACTED,
		REDACTED, true)
			...
			result, relayExit := Relay(
				...
				upstreamConn,
				[]byte(`{"type":"response.create","model":"gpt-5.3-codex","input":[]REDACTED`),
				RelayOptions{OnTurnComplete: func(current RelayTurnResult) { turn = current REDACTEDREDACTED,
			)
...
REDACTED{
		{
			...
			donePayload: `{"type":"response.output_text.done","response_id":"resp_done","text":"hello"REDACTED`,
	REDACTED,
		{
			name:        "function call arguments done",
			donePayload: `{"type":"response.function_call_arguments.done","response_id":"resp_done","arguments":"{\"city\":\"Paris\"REDACTED"REDACTED`,
	REDACTED,
REDACTED
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			clientConn := newPassthroughTestFrameConn(nil, false)
			upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
				{msgType: coderws.MessageText, payload: []byte(`{"type":"response.created","response":{"id":"resp_done"REDACTEDREDACTED`)REDACTED,
				{msgType: coderws.MessageText, payload: []byte(tt.donePayload)REDACTED,
				{msgType: coderws.MessageText, payload: []byte(`{"type":"response.completed","response":{"id":"resp_done","usage":{"input_tokens":2,"output_tokens":1REDACTEDREDACTEDREDACTED`)REDACTED,
		REDACTED, true)
