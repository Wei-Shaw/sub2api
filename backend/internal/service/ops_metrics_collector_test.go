...
func TestWriteOpenAIFastPolicyBlockedResponseMarksBusinessLimited(t *testing.T) {
	...
	writeOpenAIFastPolicyBlockedResponse(c, &OpenAIFastBlockedError{Message: "custom fast policy block"REDACTED)
...
REDACTED
	...
	collector := &OpsMetricsCollector{db: dbREDACTED
	...
	mock.ExpectQuery(`(?s)FROM ops_error_logs\s+WHERE created_at >= \$1 AND created_at < \$2\s+AND is_count_tokens = FALSE`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"upstream_429",
			"upstream_529",
	REDACTED).AddRow(int64(5), int64(2), int64(3), int64(1), int64(1), int64(1)))
