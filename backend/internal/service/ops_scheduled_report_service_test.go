...
REDACTED
	overview := &OpsDashboardOverview{
		...
		Duration:                     OpsPercentiles{P50: &latencyP50, P99: &latencyP99REDACTED,
		TTFT:                         OpsPercentiles{P50: &ttftP50, P99: &ttftP99REDACTED,
		QPS:                          OpsRateSummary{Current: 0, Peak: 1.2, Avg: 0REDACTED,
		TPS:                          OpsRateSummary{Current: 0, Peak: 133421.2, Avg: 1406.8REDACTED,
REDACTED
	...
	legacyTemplate, err := renderNotificationEmail(
		...
		"Report",
		`<section>{{report_htmlREDACTEDREDACTED</section>`,
		variables,
		map[string]string{"report_html": `<h2>generated summary</h2>`REDACTED,
	)
REDACTED
...
func TestOpsScheduledReportLegacyTemplateReceivesSummaryHTML(t *testing.T) {
	...
	_, err := notificationService.UpdateTemplate(
		...
		NotificationEmailEventOpsScheduledReport,
		"en",
		"Legacy report {{report_nameREDACTEDREDACTED",
		`<section data-template="legacy">{{report_htmlREDACTEDREDACTED</section>`,
	)
REDACTED

	svc := &OpsScheduledReportService{
		opsService:   &OpsService{opsRepo: &opsRepoMock{REDACTEDREDACTED,
...
REDACTED
	report := &opsScheduledReport{
		...
		ReportType: "daily_summary",
		TimeRange:  24 * time.Hour,
		Recipients: []string{"ops@example.com"REDACTED,
REDACTED
