...
REDACTED
	...
	detail := `{"authorization":"Bearer secret-token"REDACTED`
	entries := []*OpsInsertErrorLogInput{
		{
			ErrorBody:            `{"error":"bad","access_token":"secret"REDACTED`,
...
REDACTED
	...
	err := svc.RecordErrorBatch(context.Background(), []*OpsInsertErrorLogInput{
		{ErrorMessage: "first"REDACTED,
		{ErrorMessage: "second"REDACTED,
REDACTED)
REDACTED
...
REDACTED
	...
	err := svc.RecordError(context.Background(), &OpsInsertErrorLogInput{
		...
		UpstreamErrors: []*OpsUpstreamErrorEvent{
			{Stage: string(GatewayFailureStageInference), UpstreamStatusCode: 403, Message: staleMessage, Detail: staleDetailREDACTED,
			{
