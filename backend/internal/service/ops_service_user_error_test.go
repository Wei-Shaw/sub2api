...
func TestListUserErrorRequests_ForcesScopeAndRedacts(t *testing.T) {
	stub := &stubOpsRepoForUserErr{REDACTED
	svc := &OpsService{opsRepo: stubREDACTED
	uid := int64(42)
	kid := int64(7)
	in := &OpsErrorLogFilter{UserID: nil, View: "errors", Phase: "upstream", APIKeyID: &kidREDACTED
...
func TestGetUserErrorRequestDetail_OwnershipEnforced(t *testing.T) {
	...
	detail := &OpsErrorLogDetail{
		OpsErrorLog: OpsErrorLog{
			...
			UserID:          &ownerUID,
	REDACTED,
		ErrorBody:          `{"error":"upstream"REDACTED`,
		UpstreamStatusCode: &upstreamStatus,
REDACTED

	stub := &stubOpsRepoForUserErr{detailToReturn: detailREDACTED
	svc := &OpsService{opsRepo: stubREDACTED
...
REDACTED
	if got2.ErrorBody != `{"error":"upstream"REDACTED` {
		t.Errorf("want ErrorBody=%q, got %q", `{"error":"upstream"REDACTED`, got2.ErrorBody)
REDACTED
...
func TestGetUserErrorRequestDetail_NotFound(t *testing.T) {
	stub := &stubOpsRepoForUserErr{detailErrToReturn: sql.ErrNoRowsREDACTED
	svc := &OpsService{opsRepo: stubREDACTED
...
func TestListUserErrorRequests_EnablesMatchDeletedKeyOwner(t *testing.T) {
	...
	if _, err := svc.ListUserErrorRequests(context.Background(), uid, &OpsErrorLogFilter{REDACTED); err != nil {
		t.Fatal(err)
REDACTED
...
REDACTED
	...
	svcOwner := &OpsService{opsRepo: &stubOpsRepoForUserErr{detailToReturn: mk()REDACTEDREDACTED
...
REDACTED
	...
	// 他人 → NotFound,不泄露存在性
	svcOther := &OpsService{opsRepo: &stubOpsRepoForUserErr{detailToReturn: mk()REDACTEDREDACTED
