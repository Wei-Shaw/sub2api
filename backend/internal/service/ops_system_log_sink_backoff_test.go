...
func TestOpsSystemLogSinkFlushBackoffFor(t *testing.T) {
	sink := &OpsSystemLogSink{flushBackoff: time.Second, flushBackoffMax: 8 * time.SecondREDACTED
	...
	cases := []struct {
		...
		want     time.Duration
REDACTED{
		{"first_failure", 1, time.SecondREDACTED,
		{"second_failure", 2, 2 * time.SecondREDACTED,
		{"third_failure", 3, 4 * time.SecondREDACTED,
		{"fourth_failure", 4, 8 * time.SecondREDACTED,
		{"capped", 9, 8 * time.SecondREDACTED,
		{"large_streak_does_not_overflow", 1000, 8 * time.SecondREDACTED,
		{"zero_streak_uses_base", 0, time.SecondREDACTED,
		{"negative_streak_uses_base", -1, time.SecondREDACTED,
REDACTED
...
func TestOpsSystemLogSinkFlushBackoffForFallbacks(t *testing.T) {
	zero := &OpsSystemLogSink{REDACTED
...
REDACTED
	...
	inverted := &OpsSystemLogSink{flushBackoff: 5 * time.Second, flushBackoffMax: time.SecondREDACTED
...
REDACTED
	...
	if !waitForOpsSystemLogCondition(t, 2*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 1 REDACTED) {
...
REDACTED
	...
	if !waitForOpsSystemLogCondition(t, 3*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 3 REDACTED) {
...
REDACTED
	...
	if !waitForOpsSystemLogCondition(t, 3*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 2 REDACTED) {
...
REDACTED
	...
	if !waitForOpsSystemLogCondition(t, 3*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 10 REDACTED) {
