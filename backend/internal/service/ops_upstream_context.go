...
func MarkResponseCommitted(c *gin.Context) { c.Set(ResponseCommittedKey, true) REDACTED
...
REDACTED
	...
	c.Set(OpsUpstreamErrorsKey, []*OpsUpstreamErrorEvent{REDACTED)
...
func GetOpsStreamError(c *gin.Context) (OpsStreamError, bool) {
	if c == nil {
		return OpsStreamError{REDACTED, false
REDACTED
...
REDACTED
	if streamErr, ok := GetOpsStreamError(c); ok {
		return []OpsStreamError{streamErrREDACTED
REDACTED
	return nil
REDACTED
...
func ParseOpsUpstreamErrors(raw string) ([]*OpsUpstreamErrorEvent, error) {
	...
	if raw == "" {
		return []*OpsUpstreamErrorEvent{REDACTED, nil
REDACTED
