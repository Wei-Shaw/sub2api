...
const (
	// defaultInputOutputRatio is the default input token ratio (90% input, 10% output)
	// used when a group does not have an explicit input_output_ratio configured.
	defaultInputOutputRatio = 0.90
)
...
// computeAmountLedgerFromCredits derives the ledger amount from a token credit count
// and the group's rate_multiplier + token_price_per_million (inverse of computeActualCreditsFromLedger):
//
//	amount_ledger = actual_credits / 1,000,000 × token_price_per_million × rate_multiplier
func computeAmountLedgerFromCredits(actualCredits int64, grp *dbent.Group) float64 {
	if actualCredits <= 0 || grp == nil || grp.RateMultiplier <= 0 {
		return 0
	}
	tokenPrice := getGroupTokenPricePerMillion(grp)
	if tokenPrice <= 0 {
	...
	amount := float64(actualCredits) / 1_000_000 * tokenPrice * grp.RateMultiplier
