...
// createPaymentProviderFromInstance is a seam so tests can supply a fake
...
// resolveUniqueLegacyOrderProviderInstance handles orders written before the
// provider snapshot existed. It resolves only when exactly one instance could
// have produced the order — guessing between two merchants would attribute a
