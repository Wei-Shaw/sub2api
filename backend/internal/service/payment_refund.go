...
REDACTED
	ok := []string{OrderStatusCompleted, OrderStatusRefundRequested, OrderStatusRefundPending, OrderStatusRefundFailedREDACTED
...
REDACTED
	...
	if u.Balance < p.RefundAmount && !force {
		return &RefundResult{Success: false, Warning: "user balance is insufficient for deduction, use force", RequireForce: trueREDACTED
REDACTED
...
REDACTED else if o.OrderType == payment.OrderTypeSubscription {
...
REDACTED
	...
	case payment.ProviderStatusPending:
		s.writeAuditLog(ctx, oid, "REFUND_QUERY_PENDING", "admin", map[string]any{"refundID": resp.RefundIDREDACTED)
		return &RefundResult{Success: false, Warning: "gateway refund is still pending confirmation"REDACTED, nil
	default:
...
func (s *PaymentService) finalizeRefundFailed(ctx context.Context, o *dbent.PaymentOrder, gErr error) (*RefundResult, error) {
	...
	s.writeAuditLog(ctx, o.ID, "REFUND_FAILED", "admin", map[string]any{"detail": psErrMsg(gErr)REDACTED)
	return &RefundResult{Success: false, Warning: "gateway refund failed: " + psErrMsg(gErr)REDACTED, nil
REDACTED
...
func (s *PaymentService) latestRefundPendingDetail(ctx context.Context, oid int64) refundPendingAuditDetail {
	...
	if err != nil || logEntry == nil {
		return refundPendingAuditDetail{DeductionRollbackOK: trueREDACTED
REDACTED
	detail := refundPendingAuditDetail{DeductionRollbackOK: trueREDACTED
...
REDACTED
	...
	if err != nil {
		return nil, fmt.Errorf("mark refund: %w", err)
REDACTED
	detail, err := json.Marshal(map[string]any{"refundAmount": p.RefundAmount, "reason": p.Reason, "balanceDeducted": p.BalanceToDeduct, "force": p.ForceREDACTED)
	if err != nil {
