...
// A SePay callback carries the order code in its transfer description, so the
// order's own instance is normally resolvable before verification; the registry
// fallback only covers single-instance deployments.
