...
func TestEvaluatePluginCompatibility(t *testing.T) {
	...
	host := PluginHostInfo{Version: "0.1.179", BuildType: "release"REDACTED
	...
	manifest.Requires.TestedSub2APIVersions = []string{"0.1.178"REDACTED
	result = EvaluatePluginCompatibility(manifest, host)
	require.True(t, result.Compatible)
...
func TestEvaluatePluginCompatibilityRejectsProtocolMismatch(t *testing.T) {
	...
	result := EvaluatePluginCompatibility(manifest, PluginHostInfo{Version: "0.1.179"REDACTED)
