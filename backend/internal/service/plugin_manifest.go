...
REDACTED
	for path, hash := range m.Files {
		if !safePluginRelativePath(path) || !regexp.MustCompile(`^[a-f0-9]{64REDACTED$`).MatchString(hash) {
...
func (m PluginManifest) SortedCapabilities() []PluginCapability {
	...
	sort.Slice(out, func(i, j int) bool { return out[i].ID < out[j].ID REDACTED)
	return out
REDACTED
