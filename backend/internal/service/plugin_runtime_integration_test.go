...
type pluginFailingRequestBody struct{REDACTED

func (pluginFailingRequestBody) Read([]byte) (int, error) {
	return 0, errors.New("测试请求体读取失败")
REDACTED
func (pluginFailingRequestBody) Close() error { return nil REDACTED
...
REDACTED
	defer func() { _ = packageFile.Close() REDACTED()
...
REDACTED
	...
	require.NoError(t, runtime.validateAndApplyConfig(context.Background(), []byte(`{
		...
		"tls_min_version":"1.2",
		"proxy_mode":"disabled",
		"extra_headers":{"X-Plugin-Test":"enabled"REDACTED
REDACTED`)))
...
REDACTED
	...
	account := &Account{ID: 42, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 1REDACTED
...
REDACTED
	...
	failingRequest, err := http.NewRequestWithContext(context.Background(), http.MethodPost, upstream.URL, pluginFailingRequestBody{REDACTED)
REDACTED
