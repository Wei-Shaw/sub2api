...
func PreviewProfitAdmission(inputs []ProfitPreviewGroupInput, evalAt time.Time) []ProfitPreviewGroupReport {
	...
	REDACTED
		sort.Slice(report.Verdicts, func(i, j int) bool { return report.Verdicts[i].AccountID < report.Verdicts[j].AccountID REDACTED)
...
REDACTED else if account.RateMultiplier != nil && *account.RateMultiplier == 1 {
...
func profitPreviewProbeWarnings(account *Account, evalAt time.Time) []string {
	...
	if snapshot == nil {
		return []string{ProfitPreviewWarningProbeMissingREDACTED
REDACTED
	switch snapshot.Status {
	case UpstreamBillingProbeStatusFailed:
		return []string{ProfitPreviewWarningProbeFailedREDACTED
	case UpstreamBillingProbeStatusUnsupported:
		return []string{ProfitPreviewWarningProbeUnsupportedREDACTED
	case UpstreamBillingProbeStatusOK:
		if snapshot.FreshUntil == nil || !evalAt.Before(*snapshot.FreshUntil) {
			return []string{ProfitPreviewWarningProbeStaleREDACTED
	REDACTED
REDACTED
	return nil
REDACTED
