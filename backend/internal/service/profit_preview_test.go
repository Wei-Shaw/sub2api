...
func TestPreviewProfitAdmissionUsesAccountRatesAndPreinitializesModels(t *testing.T) {
	...
	cheap.Credentials = map[string]any{"model_mapping": map[string]any{"gpt-sol": "gpt-sol"REDACTEDREDACTED
...
REDACTEDREDACTED
	...
	reports := PreviewProfitAdmission([]ProfitPreviewGroupInput{{
		Group:         group,
		Accounts:      []*Account{cheap, boundary, expensive, invalidREDACTED,
		UserOverrides: map[int64]float64{40: 0.5REDACTED,
		Models:        []string{"gpt-sol", "gpt-luna", "gpt-no-account"REDACTED,
REDACTEDREDACTED, now)
	...
	byID := map[int64]ProfitPreviewAccountVerdict{REDACTED
...
func TestPreviewProfitAdmissionAssumeEnabled(t *testing.T) {
	...
	withoutAssume := PreviewProfitAdmission([]ProfitPreviewGroupInput{{
		Group:    group,
		Accounts: []*Account{cheapAccount, expensiveAccountREDACTED,
		Models:   []string{"gpt-test"REDACTED,
REDACTEDREDACTED, now)[0]
