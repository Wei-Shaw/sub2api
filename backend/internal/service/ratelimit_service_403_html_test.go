...
REDACTED
	...
	counter := &countingOpenAI403CounterCache{openAI403CounterCacheStub: openAI403CounterCacheStub{counts: countsREDACTEDREDACTED
...
func (h *openAI403TestHarness) handle(body string) bool {
	return h.svc.HandleUpstreamError(
		context.Background(), h.account, http.StatusForbidden, http.Header{REDACTED, []byte(body),
	)
REDACTED
...
func TestHandleUpstreamError_OpenAIHTML403DoesNotPenalizeAccount(t *testing.T) {
	cases := []struct {
		...
		body string
REDACTED{
		{"doctype_prefixed", openAI403HTMLBodyREDACTED,
		{"bare_html_tag", "<html><body>403 Forbidden</body></html>"REDACTED,
		{"leading_whitespace_and_uppercase", "\n\t  <!DOCTYPE HTML><html><body>Forbidden</body></html>"REDACTED,
REDACTED
...
func TestHandleUpstreamError_OpenAIStructured403StillPenalizes(t *testing.T) {
	t.Run("first_hit_temp_unschedulable", func(t *testing.T) {
		...
		require.True(t, h.handle(`{"error":{"message":"Your account is not authorized"REDACTEDREDACTED`))
...
REDACTED)
	...
	t.Run("threshold_disables", func(t *testing.T) {
		...
		require.True(t, h.handle(`{"error":{"message":"workspace forbidden by policy"REDACTEDREDACTED`))
...
func TestHandleUpstreamError_HTML403OnOtherPlatformsUnchanged(t *testing.T) {
	for _, platform := range []string{PlatformAnthropic, PlatformGeminiREDACTED {
		t.Run(platform, func(t *testing.T) {
			repo := &rateLimitAccountRepoStub{REDACTED
			svc := NewRateLimitService(repo, nil, &config.Config{REDACTED, nil, nil)
			account := &Account{ID: 506, Platform: platform, Type: AccountTypeAPIKeyREDACTED

			shouldDisable := svc.HandleUpstreamError(
				context.Background(), account, http.StatusForbidden, http.Header{REDACTED, []byte(openAI403HTMLBody),
			)
...
func TestIsHTMLResponse(t *testing.T) {
	cases := []struct {
		...
		want bool
REDACTED{
		{"doctype_lower", "<!doctype html><html></html>", trueREDACTED,
		{"doctype_upper", "<!DOCTYPE HTML>", trueREDACTED,
		{"bare_html", "<html lang=\"en\">", trueREDACTED,
		{"leading_whitespace", "\n\n   <html>", trueREDACTED,
		{"json_error", `{"error":{"message":"forbidden"REDACTEDREDACTED`, falseREDACTED,
		{"plain_text", "Forbidden", falseREDACTED,
		{"empty", "", falseREDACTED,
		// XML/SVG 之类不是 HTML，不在放行范围内。
		{"xml_declaration", `<?xml version="1.0"?><error/>`, falseREDACTED,
REDACTED
