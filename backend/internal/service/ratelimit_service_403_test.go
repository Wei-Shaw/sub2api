...
func TestRateLimitService_HandleUpstreamError_XAI403SubscriptionTempUnschedulable(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	blocker := &runtimeBlockRecorder{}
	service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	service.SetAccountRuntimeBlocker(blocker)
	...
	shouldDisable := service.HandleUpstreamError(
		...
		http.StatusForbidden,
		http.Header{},
		[]byte(`{"error":"SuperGrok Heavy subscription required"}`),
	)

	require.True(t, shouldDisable)
	require.Equal(t, 0, repo.setErrorCalls)
	require.Equal(t, 1, repo.tempCalls)
	require.Contains(t, repo.lastTempReason, "SuperGrok Heavy subscription required")
	require.Len(t, blocker.accounts, 1)
	require.Equal(t, account.ID, blocker.accounts[0].ID)
	require.Equal(t, "xai_quota_temp", blocker.reasons[0])
	require.True(t, blocker.until[0].After(time.Now()))
}

func TestRateLimitService_HandleUpstreamError_XAI403GenericDisables(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	account := &Account{
		ID:       402,
	...
	shouldDisable := service.HandleUpstreamError(
		...
		http.StatusForbidden,
		http.Header{},
		[]byte(`{"error":"account suspended by policy"}`),
	)

	require.True(t, shouldDisable)
	require.Equal(t, 1, repo.setErrorCalls)
	require.Equal(t, 0, repo.tempCalls)
	require.Contains(t, repo.lastErrorMsg, "account suspended by policy")
}
