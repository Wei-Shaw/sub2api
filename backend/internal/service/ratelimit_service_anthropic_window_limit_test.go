...
func TestHandleUpstreamError_AnthropicWindowLimitPreemptsTempUnschedRule(t *testing.T) {
	...
	repo := &anthropicWindowLimitRepo{REDACTED
...
REDACTED
			...
			"temp_unschedulable_rules": []any{
				map[string]any{
					...
					"keywords":         []any{"rate limit"REDACTED,
...
REDACTED
	...
	svc.HandleUpstreamError(
		...
		headers,
		[]byte(`{"type":"error","error":{"type":"rate_limit_error","message":"This request would exceed your account's rate limit. Please try again later."REDACTEDREDACTED`),
	)
...
func TestHandleUpstreamError_Anthropic5hWindowStillWinsOver7dOi(t *testing.T) {
	...
	repo := &anthropicWindowLimitRepo{REDACTED
	svc := NewRateLimitService(repo, nil, nil, nil, nil)
	account := &Account{ID: 42, Type: AccountTypeOAuth, Platform: PlatformAnthropicREDACTED
