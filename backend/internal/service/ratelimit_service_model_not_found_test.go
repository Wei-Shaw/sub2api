	...
	for _, model := range []string{"gpt-image-1", "gpt-image-1.5", "gpt-image-2"REDACTED {
		t.Run(model, func(t *testing.T) {
			...
			handled := svc.HandleUpstreamError(
				...
				http.StatusBadRequest,
				http.Header{REDACTED,
				[]byte(`{"detail":"The '`+model+`' model is not supported when using Codex with a ChatGPT account."REDACTED`),
				model,
			)
...
func TestRateLimitService_HandleUpstreamError_CodexPlanGatedImageModelKeepsCooldownOnImagesEndpoint(t *testing.T) {
	...
	handled := svc.HandleUpstreamError(
		...
		http.StatusBadRequest,
		http.Header{REDACTED,
		[]byte(`{"detail":"The 'gpt-image-2' model is not supported when using Codex with a ChatGPT account."REDACTED`),
		"gpt-image-2",
	)
...
func TestRateLimitService_HandleUpstreamError_CodexPlanGatedImageModelSkipsCooldownViaModelMapping(t *testing.T) {
	...
	svc := &RateLimitService{accountRepo: repoREDACTED
	account := openAICodexPlanGatedOAuthAccount()
	account.Credentials["model_mapping"] = map[string]any{"my-draw-alias": "gpt-image-2"REDACTED
...
func TestRateLimitService_HandleUpstreamError_ModelNotFoundImageModelStillCoolsDown(t *testing.T) {
	...
	handled := svc.HandleUpstreamError(
		...
		http.StatusNotFound,
		http.Header{REDACTED,
		[]byte(`{"error":{"message":"The model 'gpt-image-2' does not exist","code":"model_not_found"REDACTEDREDACTED`),
		"gpt-image-2",
	)
