...
REDACTED
	body := []byte(`{"error":{"type":"usage_limit_reached","message":"limit reached","plan_type":"free","resets_at":1777283883REDACTEDREDACTED`)

	svc.handle429(context.Background(), account, http.Header{REDACTED, body)
...
func TestHandle429_SkipsSparkShadow(t *testing.T) {
	...
	parentID := int64(900)
	shadowRepo := &openAI429SnapshotRepo{REDACTED
...
REDACTED
	...
	normalRepo := &openAI429SnapshotRepo{REDACTED
	normalSvc := NewRateLimitService(normalRepo, nil, nil, nil, nil)
	normal := &Account{ID: 902, Platform: PlatformOpenAI, Type: AccountTypeOAuthREDACTED
...
REDACTED
	...
	shouldDisable := service.HandleUpstreamError(
		...
		[]byte(`{"error":{"message":"workspace forbidden by policy","type":"invalid_request_error"REDACTEDREDACTED`),
	)
...
REDACTED
	...
	shouldDisable := service.HandleUpstreamError(
		...
		403,
		http.Header{REDACTED,
		[]byte(`{"error":{"type":"access_denied","details":{"reason":"ip_blocked"REDACTEDREDACTEDREDACTED`),
	)
