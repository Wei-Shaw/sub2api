...
func TestRateLimitService_ApplyAccountSchedulingThreshold_SetsTempUnschedulable(t *testing.T) {
	...
	settingsRepo.data[SettingKeyAccountSchedulingThresholds] = `{"openai":80REDACTED`

	accountRepo := &rateLimitAccountRepoStub{REDACTED
	rl := NewRateLimitService(accountRepo, nil, &config.Config{REDACTED, nil, nil)
	rl.SetSettingService(NewSettingService(settingsRepo, &config.Config{REDACTED))
