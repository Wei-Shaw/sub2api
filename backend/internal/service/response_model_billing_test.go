...
REDACTED{
		{
			...
			responseModel: func(cheaper string) string { return cheaper REDACTED,
			conflict:      true,
			source:        BillingModelSourceResponse,
	REDACTED,
		{
			name:          "empty_response_model_falls_back_to_baseline",
			responseModel: func(string) string { return "" REDACTED,
			source:        BillingModelSourceResponse,
	REDACTED,
		{
			name:          "unpriced_response_model_falls_back_to_baseline",
			responseModel: func(string) string { return "zz-unpriced-response-model" REDACTED,
			source:        BillingModelSourceResponse,
	REDACTED,
		{
...
func TestResponseModelBillingAdoptable(t *testing.T) {
	...
	cost := func(total float64) *CostBreakdown {
		return &CostBreakdown{TotalCost: total, ActualCost: totalREDACTED
REDACTED
	tests := []struct {
		name                  string
...
REDACTED{
		// 1. 不得更贵
		{name: "cheaper_adopted", baseline: cost(1), response: cost(0.5), want: trueREDACTED,
		{name: "equal_adopted", baseline: cost(1), response: cost(1), want: trueREDACTED,
		{name: "float_noise_within_epsilon_adopted", baseline: cost(1), response: cost(1 + 1e-13), want: trueREDACTED,
		{name: "pricier_rejected", baseline: cost(1), response: cost(1.0001)REDACTED,

		// 2. 不得把一笔本应计费的请求归零（价格表里有显式写 0 的条目，能通过确定性识别）
		{name: "zeroing_a_billable_request_rejected", baseline: cost(1), response: cost(0)REDACTED,
		{name: "negative_cost_rejected_as_zeroing", baseline: cost(1), response: cost(-1)REDACTED,
		{name: "already_zero_baseline_unaffected", baseline: cost(0), response: cost(0), want: trueREDACTED,

		// 3. 不得从渠道定价跨到全局价格表（否则渠道加价被带日期的自报模型名绕过）
		{name: "channel_priced_baseline_to_global_rejected", baseline: cost(1), response: cost(0.5), baselineChannelPriced: trueREDACTED,
		{name: "channel_priced_on_both_sides_adopted", baseline: cost(1), response: cost(0.5), baselineChannelPriced: true, responseChannelPriced: true, want: trueREDACTED,
		{name: "global_baseline_to_channel_priced_adopted", baseline: cost(1), response: cost(0.5), responseChannelPriced: true, want: trueREDACTED,

		{name: "nil_baseline_rejected", response: cost(0.5)REDACTED,
		{name: "nil_response_rejected", baseline: cost(1)REDACTED,
REDACTED
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
