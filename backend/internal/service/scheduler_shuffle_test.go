...
func TestShuffleWithinSortGroups_Empty(t *testing.T) {
	...
	shuffleWithinSortGroups([]accountWithLoad{REDACTED)
REDACTED

func TestShuffleWithinSortGroups_SingleElement(t *testing.T) {
	accounts := []accountWithLoad{
		{account: &Account{ID: 1, Priority: 1REDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
REDACTED
...
func TestShuffleWithinSortGroups_DifferentGroups_OrderPreserved(t *testing.T) {
	...
	accounts := []accountWithLoad{
		{account: &Account{ID: 1, Priority: 1, LastUsedAt: &earlierREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
		{account: &Account{ID: 2, Priority: 1, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 20REDACTEDREDACTED,
		{account: &Account{ID: 3, Priority: 2, LastUsedAt: &earlierREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
REDACTED
...
func TestShuffleWithinSortGroups_SameGroup_Shuffled(t *testing.T) {
	...
	accounts := []accountWithLoad{
		{account: &Account{ID: 1, Priority: 1, LastUsedAt: &sameSecondREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
		{account: &Account{ID: 2, Priority: 1, LastUsedAt: &sameSecond2REDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
		{account: &Account{ID: 3, Priority: 1, LastUsedAt: &sameSecondREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
REDACTED

	// 多次执行，验证所有 ID 都出现在第一个位置（说明确实被打乱了）
	seen := map[int64]bool{REDACTED
	for i := 0; i < 100; i++ {
		cpy := make([]accountWithLoad, len(accounts))
		copy(cpy, accounts)
		shuffleWithinSortGroups(cpy)
...
func TestShuffleWithinSortGroups_NilLastUsedAt_SameGroup(t *testing.T) {
	accounts := []accountWithLoad{
		{account: &Account{ID: 1, Priority: 1, LastUsedAt: nilREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 0REDACTEDREDACTED,
		{account: &Account{ID: 2, Priority: 1, LastUsedAt: nilREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 0REDACTEDREDACTED,
		{account: &Account{ID: 3, Priority: 1, LastUsedAt: nilREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 0REDACTEDREDACTED,
REDACTED
...
func TestShuffleWithinSortGroups_MixedGroups(t *testing.T) {
	...
	accounts := []accountWithLoad{
		{account: &Account{ID: 1, Priority: 1, LastUsedAt: &earlierREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
		{account: &Account{ID: 2, Priority: 1, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 20REDACTEDREDACTED,
		{account: &Account{ID: 3, Priority: 1, LastUsedAt: &sameAsNowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 20REDACTEDREDACTED,
		{account: &Account{ID: 4, Priority: 2, LastUsedAt: &earlierREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED,
REDACTED
	...
	for i := 0; i < 20; i++ {
		...
		mid := map[int64]bool{cpy[1].account.ID: true, cpy[2].account.ID: trueREDACTED
...
func TestShuffleWithinPriorityAndLastUsed_Empty(t *testing.T) {
	...
	shuffleWithinPriorityAndLastUsed([]*Account{REDACTED, false)
REDACTED

func TestShuffleWithinPriorityAndLastUsed_SingleElement(t *testing.T) {
	accounts := []*Account{{ID: 1, Priority: 1REDACTEDREDACTED
...
func TestShuffleWithinPriorityAndLastUsed_SameGroup_Shuffled(t *testing.T) {
	accounts := []*Account{
		{ID: 1, Priority: 1, LastUsedAt: nilREDACTED,
		{ID: 2, Priority: 1, LastUsedAt: nilREDACTED,
...
func TestShuffleWithinPriorityAndLastUsed_DifferentPriority_OrderPreserved(t *testing.T) {
	accounts := []*Account{
		{ID: 1, Priority: 1, LastUsedAt: nilREDACTED,
		{ID: 2, Priority: 2, LastUsedAt: nilREDACTED,
		{ID: 3, Priority: 3, LastUsedAt: nilREDACTED,
REDACTED
...
func TestShuffleWithinPriorityAndLastUsed_DifferentLastUsedAt_OrderPreserved(t *testing.T) {
	...
	accounts := []*Account{
		{ID: 1, Priority: 1, LastUsedAt: nilREDACTED,
		{ID: 2, Priority: 1, LastUsedAt: &earlierREDACTED,
		{ID: 3, Priority: 1, LastUsedAt: &nowREDACTED,
REDACTED
...
func TestSameAccountWithLoadGroup(t *testing.T) {
	...
	t.Run("same group", func(t *testing.T) {
		a := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED
		b := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: &sameSecondREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED
		require.True(t, sameAccountWithLoadGroup(a, b))
REDACTED)

	t.Run("different priority", func(t *testing.T) {
		a := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED
		b := accountWithLoad{account: &Account{Priority: 2, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED
		require.False(t, sameAccountWithLoadGroup(a, b))
REDACTED)

	t.Run("different load rate", func(t *testing.T) {
		a := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED
		b := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 20REDACTEDREDACTED
		require.False(t, sameAccountWithLoadGroup(a, b))
REDACTED)
	...
	t.Run("different last used at", func(t *testing.T) {
		...
		a := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: &nowREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED
		b := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: &laterREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 10REDACTEDREDACTED
		require.False(t, sameAccountWithLoadGroup(a, b))
REDACTED)

	t.Run("both nil LastUsedAt", func(t *testing.T) {
		a := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: nilREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 0REDACTEDREDACTED
		b := accountWithLoad{account: &Account{Priority: 1, LastUsedAt: nilREDACTED, loadInfo: &AccountLoadInfo{LoadRate: 0REDACTEDREDACTED
		require.True(t, sameAccountWithLoadGroup(a, b))
REDACTED)
REDACTED
...
func TestSameAccountGroup(t *testing.T) {
	...
	t.Run("same group", func(t *testing.T) {
		a := &Account{Priority: 1, LastUsedAt: nilREDACTED
		b := &Account{Priority: 1, LastUsedAt: nilREDACTED
		require.True(t, sameAccountGroup(a, b))
REDACTED)

	t.Run("different priority", func(t *testing.T) {
		a := &Account{Priority: 1, LastUsedAt: nilREDACTED
		b := &Account{Priority: 2, LastUsedAt: nilREDACTED
		require.False(t, sameAccountGroup(a, b))
REDACTED)

	t.Run("different LastUsedAt", func(t *testing.T) {
		later := now.Add(1 * time.Second)
		a := &Account{Priority: 1, LastUsedAt: &nowREDACTED
		b := &Account{Priority: 1, LastUsedAt: &laterREDACTED
		require.False(t, sameAccountGroup(a, b))
REDACTED)
REDACTED
...
REDACTED)
	...
	t.Run("different priorities still sorted correctly", func(t *testing.T) {
		now := time.Now()
		accounts := []*Account{
			{ID: 3, Priority: 3, LastUsedAt: &nowREDACTED,
			{ID: 1, Priority: 1, LastUsedAt: &nowREDACTED,
			{ID: 2, Priority: 2, LastUsedAt: &nowREDACTED,
	REDACTED
