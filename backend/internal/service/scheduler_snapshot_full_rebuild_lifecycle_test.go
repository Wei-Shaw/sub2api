...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Len(t, cache.tokens(), 2*groupLifecycleBucketCount(), "full rebuild and the following group event must each run fresh authority")
	_, reopenHeld := cache.lifecycleMutationLeaseStates()
	require.Len(t, reopenHeld, 2*groupLifecycleBucketCount())
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	require.Equal(t, 3*groupLifecycleAccountQueryCount(), accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, 2*groupLifecycleAccountQueryCount()+1, accounts.callCount())
	require.Equal(t, groupLifecycleAccountQueryCount()+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, groupLifecycleAccountQueryCount(), accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Equal(t, 3*groupLifecycleBucketCount()+3, len(cache.retiredBuckets()))
	require.Equal(t, groupLifecycleAccountQueryCount(), accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Equal(t, groupLifecycleAccountQueryCount()+3, accounts.callCount())
	require.Equal(t, groupLifecycleAccountQueryCount()+3, accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, 2*groupLifecycleAccountQueryCount(), accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 4*groupLifecycleAccountQueryCount(), accounts.callCount())
