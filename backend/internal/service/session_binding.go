...
type sessionBindingCtxKey struct{REDACTED
...
func WithSessionBinding(ctx context.Context, binding *SessionBinding) context.Context {
	if binding == nil {
		return ctx
REDACTED
	return context.WithValue(ctx, sessionBindingCtxKey{REDACTED, binding)
REDACTED
...
func SessionBindingFromContext(ctx context.Context) *SessionBinding {
	if ctx == nil {
		return nil
REDACTED
	binding, _ := ctx.Value(sessionBindingCtxKey{REDACTED).(*SessionBinding)
	return binding
REDACTED
