...
func (s *SettingService) PasskeyConfiguration() (configured bool, rpID string, origins []string) {
	if s == nil || s.cfg == nil {
		return false, "", []string{REDACTED
REDACTED
	origins = append([]string{REDACTED, s.cfg.WebAuthn.RPOrigins...)
...
REDACTED)
	if err != nil {
		return CaptchaProviderConfig{REDACTED, fmt.Errorf("read captcha provider settings: %w", err)
REDACTED
...
func (s *SettingService) GetTencentCaptchaConfig(ctx context.Context) TencentCaptchaConfig {
	...
	if err != nil {
		return TencentCaptchaConfig{REDACTED
REDACTED
...
func (s *SettingService) SetRateLimit429CooldownSettings(ctx context.Context, settings *RateLimit429CooldownSettings) error {
	if settings == nil {
		return fmt.Errorf("settings cannot be nil")
REDACTED
	if settings.Strategy == "" { settings.Strategy = "cooldown" REDACTED
	if settings.RetryIntervalMs == 0 { settings.RetryIntervalMs = 500 REDACTED
	if settings.RetryMaxDurationSeconds == 0 { settings.RetryMaxDurationSeconds = 120 REDACTED
	if settings.MaxAccountSwitches < 0 { settings.MaxAccountSwitches = 0 REDACTED
...
func (s *SettingService) GetDefaultPlatformQuotas(ctx context.Context) (map[string]*DefaultPlatformQuotaSetting, error) {
	...
	for _, platform := range AllowedQuotaPlatforms {
		out[platform] = &DefaultPlatformQuotaSetting{REDACTED
REDACTED
	raw, err := s.settingRepo.GetValue(ctx, SettingKeyDefaultPlatformQuotas)
	if err != nil || raw == "" {
		return out, nil // 无配置 = 全部不限制
REDACTED
	parsed := map[string]*DefaultPlatformQuotaSetting{REDACTED
...
func (s *SettingService) GetAuthSourcePlatformQuotas(ctx context.Context, source string) map[string]*DefaultPlatformQuotaSetting {
	out := map[string]*DefaultPlatformQuotaSetting{REDACTED
...
REDACTED
	if err := json.Unmarshal([]byte(raw), &out); err != nil {
		...
		return map[string]*DefaultPlatformQuotaSetting{REDACTED
REDACTED
