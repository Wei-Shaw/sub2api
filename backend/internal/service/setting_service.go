...
REDACTED
	if cfg.AppID == "" {
		return WeChatConnectOAuthConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "wechat oauth app id not configured")
REDACTED
	if cfg.AppSecret == "" {
		return WeChatConnectOAuthConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "wechat oauth app secret not configured")
REDACTED
	if cfg.RedirectURL == "" {
		return WeChatConnectOAuthConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "wechat oauth redirect url not configured")
REDACTED
	if cfg.FrontendRedirectURL == "" {
		return WeChatConnectOAuthConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "wechat oauth frontend redirect url not configured")
REDACTED
...
REDACTED
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		if cached, ok := gatewayForwardingCache.Load().(*cachedGatewayForwardingSettings); ok && cached != nil {
			if time.Now().UnixNano() < cached.expiresAt {
				return gwfResult{cached.fingerprintUnification, cached.metadataPassthrough, cached.cchSigningREDACTED, nil
		REDACTED
	REDACTED
	...
	REDACTED)
		...
		REDACTED)
			return gwfResult{true, false, falseREDACTED, nil
	REDACTED
		fp := true
	...
	REDACTED
		...
		gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
			...
			expiresAt:              time.Now().Add(gatewayForwardingCacheTTL).UnixNano(),
	REDACTED)
		return gwfResult{fp, mp, cchREDACTED, nil
REDACTED)
