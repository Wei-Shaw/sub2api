...
func (s *SettingService) IsUserErrorViewAllowed(ctx context.Context) bool {
	vals, err := s.settingRepo.GetMultiple(ctx, []string{SettingKeyAllowUserViewErrorRequestsREDACTED)
	if err != nil {
...
var legacyClaudeCodeCodexWhitelistEntry = openai.AllowedClientEntry{
	...
	UAContains: []string{"Claude Code/"REDACTED,
REDACTED
...
REDACTED
	...
	s.codexRestrictionPolicyCache.Store(&cachedCodexRestrictionPolicy{expiresAt: 0REDACTED)
	return nil
REDACTED
...
REDACTED
	...
	if v, err := s.settingRepo.GetValue(dbCtx, SettingKeyCodexCLIOnlyEngineFingerprintSignals); err == nil && strings.TrimSpace(v) != "" {
		return nil // 已配置/已迁移
REDACTED else if err != nil && !errors.Is(err, ErrSettingNotFound) {
...
REDACTED else if !errors.Is(err, ErrSettingNotFound) {
...
func normalizedCodexClientMarkers(markers []string) map[string]struct{REDACTED {
	normalized := make(map[string]struct{REDACTED, len(markers))
	for _, marker := range markers {
		marker = strings.TrimSpace(marker)
		if marker == "" {
			continue
	REDACTED
		normalized[strings.ToLower(marker)] = struct{REDACTED{REDACTED
REDACTED
	return normalized
REDACTED
...
REDACTED
	...
	REDACTED
		...
		pol := CodexRestrictionPolicy{EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsREDACTED // 安全默认：默认种子指纹信号
...
REDACTED)
	if pol, ok := result.(CodexRestrictionPolicy); ok {
		return pol
REDACTED
	return CodexRestrictionPolicy{EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsREDACTED
REDACTED
...
REDACTED
	...
	if err := json.Unmarshal([]byte(trimmed), &entries); err != nil {
		return fmt.Errorf("must be empty or a valid JSON array of {originator, ua_containsREDACTED")
REDACTED
	return nil
REDACTED
...
func validateDefaultPlatformQuotaMap(m map[string]*DefaultPlatformQuotaSetting) error {
	...
	REDACTED
		if pq == nil {
			continue
	REDACTED
		for _, v := range []*float64{pq.DailyLimitUSD, pq.WeeklyLimitUSD, pq.MonthlyLimitUSDREDACTED {
...
REDACTED{
		{"email", settings.Email.PlatformQuotasREDACTED,
		{"linuxdo", settings.LinuxDo.PlatformQuotasREDACTED,
		{"oidc", settings.OIDC.PlatformQuotasREDACTED,
		{"wechat", settings.WeChat.PlatformQuotasREDACTED,
		{"github", settings.GitHub.PlatformQuotasREDACTED,
		{"google", settings.Google.PlatformQuotasREDACTED,
		{"dingtalk", settings.DingTalk.PlatformQuotasREDACTED,
REDACTED {
		if pgs.pq != nil {
...
REDACTED
	...
	REDACTED)
		...
		REDACTED)
			return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, rewriteMessageCacheControl: s.defaultRewriteMessageCacheControl(), clientDatelineNormalization: trueREDACTED, nil
	REDACTED
		fp := true
...
REDACTED)
	if r, ok := val.(gatewayForwardingSettingsResult); ok {
		return r
REDACTED
	return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, clientDatelineNormalization: trueREDACTED
REDACTED
...
REDACTED
	...
	dingTalkBase := config.DingTalkConnectConfig{REDACTED
	if s.cfg != nil {
...
func (s *SettingService) GetDingTalkConnectOAuthConfig(ctx context.Context) (config.DingTalkConnectConfig, error) {
	if s == nil || s.cfg == nil {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.ServiceUnavailable("CONFIG_NOT_READY", "config not loaded")
REDACTED
...
REDACTED
	settings, err := s.settingRepo.GetMultiple(ctx, keys)
	if err != nil {
		return config.DingTalkConnectConfig{REDACTED, fmt.Errorf("get dingtalk connect settings: %w", err)
REDACTED
...
REDACTED
	...
	if !effective.Enabled {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.NotFound("OAUTH_DISABLED", "dingtalk oauth login is disabled")
REDACTED

	// 基础健壮性校验（避免把用户重定向到一个必然失败或不安全的 OAuth 流程里）。
	if strings.TrimSpace(effective.ClientID) == "" {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth client id not configured")
REDACTED
	if strings.TrimSpace(effective.AuthorizeURL) == "" {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth authorize url not configured")
REDACTED
	if strings.TrimSpace(effective.TokenURL) == "" {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth token url not configured")
REDACTED
	if strings.TrimSpace(effective.UserInfoURL) == "" {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth userinfo url not configured")
REDACTED
	if strings.TrimSpace(effective.RedirectURL) == "" {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth redirect url not configured")
REDACTED
	if strings.TrimSpace(effective.FrontendRedirectURL) == "" {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth frontend redirect url not configured")
REDACTED

	if err := config.ValidateAbsoluteHTTPURL(effective.AuthorizeURL); err != nil {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth authorize url invalid")
REDACTED
	if err := config.ValidateAbsoluteHTTPURL(effective.TokenURL); err != nil {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth token url invalid")
REDACTED
	if err := config.ValidateAbsoluteHTTPURL(effective.UserInfoURL); err != nil {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth userinfo url invalid")
REDACTED
	if err := config.ValidateAbsoluteHTTPURL(effective.RedirectURL); err != nil {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth redirect url invalid")
REDACTED
	if err := config.ValidateFrontendRedirectURL(effective.FrontendRedirectURL); err != nil {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth frontend redirect url invalid")
REDACTED
	if strings.TrimSpace(effective.ClientSecret) == "" {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "dingtalk oauth client secret not configured")
REDACTED
...
REDACTED
	...
	if err := config.ValidateDingTalkConfig(effective); err != nil {
		return config.DingTalkConnectConfig{REDACTED, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", err.Error())
REDACTED
