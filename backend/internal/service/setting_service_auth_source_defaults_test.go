...
REDACTED
	...
	require.Equal(t, []DefaultSubscriptionSetting{{GroupID: 11, ValidityDays: 30REDACTEDREDACTED, got.Email.Subscriptions)
	...
	require.Equal(t, []DefaultSubscriptionSetting{REDACTED, got.LinuxDo.Subscriptions)
...
func TestSettingService_UpdateAuthSourceDefaultSettings_PersistsAllKeys(t *testing.T) {
	repo := &authSourceDefaultsRepoStub{REDACTED
	svc := NewSettingService(repo, &config.Config{REDACTED)
	...
	err := svc.UpdateAuthSourceDefaultSettings(context.Background(), &AuthSourceDefaultSettings{
		Email: ProviderDefaultGrantSettings{
			Balance:          1.25,
			Concurrency:      3,
			Subscriptions:    []DefaultSubscriptionSetting{{GroupID: 21, ValidityDays: 14REDACTEDREDACTED,
			GrantOnSignup:    false,
	...
	REDACTED,
		LinuxDo: ProviderDefaultGrantSettings{
			Balance:          2,
			Concurrency:      4,
			Subscriptions:    []DefaultSubscriptionSetting{{GroupID: 22, ValidityDays: 30REDACTEDREDACTED,
			GrantOnSignup:    true,
	...
	REDACTED,
		OIDC: ProviderDefaultGrantSettings{
			Balance:          3,
			Concurrency:      5,
			Subscriptions:    []DefaultSubscriptionSetting{{GroupID: 23, ValidityDays: 60REDACTEDREDACTED,
			GrantOnSignup:    true,
			GrantOnFirstBind: true,
	REDACTED,
		WeChat: ProviderDefaultGrantSettings{
			Balance:          4,
			Concurrency:      6,
			Subscriptions:    []DefaultSubscriptionSetting{{GroupID: 24, ValidityDays: 90REDACTEDREDACTED,
			GrantOnSignup:    false,
			GrantOnFirstBind: false,
	REDACTED,
...
REDACTED
	...
	require.Equal(t, []DefaultSubscriptionSetting{{GroupID: 24, ValidityDays: 90REDACTEDREDACTED, got)
REDACTED
