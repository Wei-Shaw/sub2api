...
func TestGetOIDCConnectOAuthConfig_ResolvesEndpointsFromIssuerDiscovery(t *testing.T) {
	...
	REDACTED`, baseURL, baseURL, baseURL, baseURL)))
REDACTED))
	defer srv.Close()
	baseURL = srv.URL
...
REDACTED
	...
	repo := &settingOIDCRepoStub{values: map[string]string{REDACTEDREDACTED
...
func TestSettingService_ParseSettings_PreservesOptionalOIDCCompatibilityFlags(t *testing.T) {
	svc := NewSettingService(&settingOIDCRepoStub{values: map[string]string{REDACTEDREDACTED, &config.Config{REDACTED)
...
func TestSettingService_ParseSettings_DefaultsOIDCSecurityFlagsToSafeConfigValues(t *testing.T) {
	svc := NewSettingService(&settingOIDCRepoStub{values: map[string]string{REDACTEDREDACTED, &config.Config{
		OIDC: config.OIDCConnectConfig{
			UsePKCE:         true,
			ValidateIDToken: true,
	REDACTED,
