...
REDACTED
	ten := 10.0
	patch := DefaultPlatformQuotaSetting{DailyLimitUSD: &tenREDACTED
...
func TestMergePlatformQuotaDefaults_ZeroIsExplicitDisable(t *testing.T) {
	five := 5.0
	base := DefaultPlatformQuotaSetting{DailyLimitUSD: &fiveREDACTED
	zero := 0.0
	patch := DefaultPlatformQuotaSetting{DailyLimitUSD: &zeroREDACTED
...
func floatPtrPQ(v float64) *float64 { return &v REDACTED
...
func TestGetDefaultPlatformQuotas_ReturnsFourPlatforms(t *testing.T) {
	...
	svc := newSettingServiceForPlatformQuotaTest(map[string]string{
		...
		SettingKeyDefaultPlatformQuotas: `{"anthropic":{"daily":10.5REDACTED,"openai":{"monthly":0REDACTEDREDACTED`,
REDACTED)
...
REDACTED
	...
	for _, platform := range []string{"anthropic", "openai", "gemini", "antigravity"REDACTED {
...
func TestGetAuthSourcePlatformQuotas_OnlyConfiguredReturned(t *testing.T) {
	...
	svc := newSettingServiceForPlatformQuotaTest(map[string]string{
		SettingKeyAuthSourcePlatformQuotas(source): `{"anthropic":{"daily":5,"monthly":100REDACTED,"openai":{"weekly":0REDACTEDREDACTED`,
REDACTED)
...
func TestGetAuthSourcePlatformQuotas_AllNegativeOrEmpty_NoEntry(t *testing.T) {
	...
	svc := newSettingServiceForPlatformQuotaTest(map[string]string{
		SettingKeyAuthSourcePlatformQuotas(source): `{REDACTED`,
REDACTED)
	got := svc.GetAuthSourcePlatformQuotas(context.Background(), source)
...
func TestSystemPlatformQuotas_WriteReadRoundTrip(t *testing.T) {
	...
	ss := &SystemSettings{
		DefaultPlatformQuotas: map[string]*DefaultPlatformQuotaSetting{
			"anthropic": {DailyLimitUSD: &ten, WeeklyLimitUSD: nil, MonthlyLimitUSD: nilREDACTED,
	REDACTED,
REDACTED
...
REDACTED
	...
	for _, p := range []string{"anthropic", "openai", "gemini", "antigravity"REDACTED {
		if _, ok := got[p]; !ok {
...
// 写入 DefaultPlatformQuotas={REDACTED 后，GetDefaultPlatformQuotas 返回 4 个平台、所有字段均为 nil，
...
func TestSystemPlatformQuotas_EmptyMapClearsAll(t *testing.T) {
	...
	if err := svc.UpdateSettings(ctx, &SystemSettings{
		DefaultPlatformQuotas: map[string]*DefaultPlatformQuotaSetting{
			"anthropic": {DailyLimitUSD: &tenREDACTED,
	REDACTED,
REDACTED); err != nil {
...
REDACTED
	...
	if err := svc.UpdateSettings(ctx, &SystemSettings{
		DefaultPlatformQuotas: map[string]*DefaultPlatformQuotaSetting{REDACTED,
REDACTED); err != nil {
...
func TestUpdateSettingsWithAuthSourceDefaults_PlatformQuotaRoundTrip(t *testing.T) {
	svc := newSettingServiceForPlatformQuotaTest(nil)
	systemSettings := &SystemSettings{REDACTED
...
func TestUpdateSettingsWithAuthSourceDefaults_NilPlatformQuotaPreservesExisting(t *testing.T) {
	svc := newSettingServiceForPlatformQuotaTest(map[string]string{
		SettingKeyAuthSourcePlatformQuotas("email"): `{"anthropic":{"daily":5,"weekly":null,"monthly":nullREDACTEDREDACTED`,
REDACTED)
	// authDefaults 不携带 Email 的 PlatformQuotas（nil）——应保留既有配置
	authDefaults := &AuthSourceDefaultSettings{
		Email: ProviderDefaultGrantSettings{PlatformQuotas: nilREDACTED,
REDACTED
	if err := svc.UpdateSettingsWithAuthSourceDefaults(context.Background(), &SystemSettings{REDACTED, authDefaults); err != nil {
		t.Fatalf("UpdateSettingsWithAuthSourceDefaults: %v", err)
REDACTED
...
func TestGetAuthSourcePlatformQuotas_JSON(t *testing.T) {
	svc := newSettingServiceForPlatformQuotaTest(map[string]string{
		SettingKeyAuthSourcePlatformQuotas("email"): `{"openai":{"daily":null,"weekly":null,"monthly":20REDACTEDREDACTED`,
REDACTED)
	got := svc.GetAuthSourcePlatformQuotas(context.Background(), "email")
...
func TestUpdateSettingsWithAuthSourceDefaults_NegativeQuotaRejected(t *testing.T) {
	...
	authDefaults := &AuthSourceDefaultSettings{
		Email: ProviderDefaultGrantSettings{
			PlatformQuotas: map[string]*DefaultPlatformQuotaSetting{
				"anthropic": {DailyLimitUSD: &negREDACTED,
		REDACTED,
	REDACTED,
REDACTED
	err := svc.UpdateSettingsWithAuthSourceDefaults(context.Background(), &SystemSettings{REDACTED, authDefaults)
