...
func newStubGenRepo() *stubGenRepo {
	return &stubGenRepo{gens: make(map[int64]*SoraGeneration), nextID: 1REDACTED
REDACTED
...
REDACTED
	var count int64
	statusSet := make(map[string]struct{REDACTED)
	for _, s := range statuses {
		statusSet[s] = struct{REDACTED{REDACTED
REDACTED
	for _, gen := range r.gens {
...
func newStubUserRepoForQuota() *stubUserRepoForQuota {
	return &stubUserRepoForQuota{users: make(map[int64]*User)REDACTED
REDACTED
...
REDACTED
	...
	return nil
REDACTED
func (r *stubUserRepoForQuota) Create(context.Context, *User) error { return nil REDACTED
func (r *stubUserRepoForQuota) GetByEmail(context.Context, string) (*User, error) {
	return nil, nil
REDACTED
func (r *stubUserRepoForQuota) GetFirstAdmin(context.Context) (*User, error) { return nil, nil REDACTED
func (r *stubUserRepoForQuota) Delete(context.Context, int64) error          { return nil REDACTED
...
func (r *stubUserRepoForQuota) ListWithFilters(context.Context, pagination.PaginationParams, UserListFilters) ([]User, *pagination.PaginationResult, error) {
	return nil, nil, nil
REDACTED
func (r *stubUserRepoForQuota) UpdateBalance(context.Context, int64, float64) error { return nil REDACTED
func (r *stubUserRepoForQuota) DeductBalance(context.Context, int64, float64) error { return nil REDACTED
func (r *stubUserRepoForQuota) UpdateConcurrency(context.Context, int64, int) error { return nil REDACTED
...
func newS3StorageWithCDN(cdnURL string) *SoraS3Storage {
	storage := &SoraS3Storage{REDACTED
...
REDACTED
	...
	storage.client = s3.New(s3.Options{REDACTED)
	return storage
REDACTED
...
func newS3StorageFailingDelete() *SoraS3Storage {
	return &SoraS3Storage{REDACTED // settingService 为 nil → getConfig 返回 error
REDACTED
...
func TestMarkGenerating_Success(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusPendingREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestMarkCompleted_Success(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusGeneratingREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)

	err := svc.MarkCompleted(context.Background(), 1,
		"https://cdn.example.com/video.mp4",
		[]string{"https://cdn.example.com/video.mp4"REDACTED,
		SoraStorageTypeS3,
		[]string{"sora/1/2024/01/01/uuid.mp4"REDACTED,
		1048576,
	)
REDACTED
	gen := repo.gens[1]
	...
	require.Equal(t, []string{"https://cdn.example.com/video.mp4"REDACTED, gen.MediaURLs)
	require.Equal(t, SoraStorageTypeS3, gen.StorageType)
	require.Equal(t, []string{"sora/1/2024/01/01/uuid.mp4"REDACTED, gen.S3ObjectKeys)
...
func TestMarkCancelled_Completed(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusCompletedREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestMarkCancelled_Failed(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusFailedREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestMarkCancelled_AlreadyCancelled(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusCancelledREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestGetByID_Success(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusCompleted, Model: "sora2-landscape-10s"REDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestGetByID_WrongUser(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 2, Status: SoraGenStatusCompletedREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestList_Success(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusCompleted, MediaType: "video"REDACTED
	repo.gens[2] = &SoraGeneration{ID: 2, UserID: 1, Status: SoraGenStatusPending, MediaType: "image"REDACTED
	repo.gens[3] = &SoraGeneration{ID: 3, UserID: 2, Status: SoraGenStatusCompleted, MediaType: "video"REDACTED
	svc := NewSoraGenerationService(repo, nil, nil)

	gens, total, err := svc.List(context.Background(), SoraGenerationListParams{UserID: 1, Page: 1, PageSize: 20REDACTED)
REDACTED
...
func TestList_DefaultPagination(t *testing.T) {
	...
	_, _, err := svc.List(context.Background(), SoraGenerationListParams{UserID: 1REDACTED)
REDACTED
REDACTED
...
func TestList_MaxPageSize(t *testing.T) {
	...
	_, _, err := svc.List(context.Background(), SoraGenerationListParams{UserID: 1, Page: 1, PageSize: 200REDACTED)
REDACTED
REDACTED
...
func TestDelete_Success(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusCompleted, StorageType: SoraStorageTypeUpstreamREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestDelete_S3Cleanup_NilS3(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, StorageType: SoraStorageTypeS3, S3ObjectKeys: []string{"key1"REDACTEDREDACTED
	svc := NewSoraGenerationService(repo, nil, nil)

	err := svc.Delete(context.Background(), 1, 1)
REDACTED // s3Storage 为 nil，跳过清理
REDACTED

func TestDelete_QuotaRelease_NilQuota(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, StorageType: SoraStorageTypeS3, FileSizeBytes: 1024REDACTED
	svc := NewSoraGenerationService(repo, nil, nil)

	err := svc.Delete(context.Background(), 1, 1)
REDACTED // quotaService 为 nil，跳过释放
REDACTED

func TestDelete_NonS3NoCleanup(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, StorageType: SoraStorageTypeLocal, FileSizeBytes: 1024REDACTED
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestDelete_DeleteRepoError(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, StorageType: SoraStorageTypeUpstreamREDACTED
...
func TestCountActiveByUser_Success(t *testing.T) {
	repo := newStubGenRepo()
	repo.gens[1] = &SoraGeneration{ID: 1, UserID: 1, Status: SoraGenStatusPendingREDACTED
	repo.gens[2] = &SoraGeneration{ID: 2, UserID: 1, Status: SoraGenStatusGeneratingREDACTED
	repo.gens[3] = &SoraGeneration{ID: 3, UserID: 1, Status: SoraGenStatusCompletedREDACTED // 不算
	svc := NewSoraGenerationService(repo, nil, nil)
...
func TestResolveMediaURLs_NonS3(t *testing.T) {
	svc := NewSoraGenerationService(newStubGenRepo(), nil, nil)
	gen := &SoraGeneration{StorageType: SoraStorageTypeUpstream, MediaURL: "https://original.com/v.mp4"REDACTED
...
func TestResolveMediaURLs_S3NilStorage(t *testing.T) {
	svc := NewSoraGenerationService(newStubGenRepo(), nil, nil)
	gen := &SoraGeneration{StorageType: SoraStorageTypeS3, S3ObjectKeys: []string{"key1"REDACTEDREDACTED
	require.NoError(t, svc.ResolveMediaURLs(context.Background(), gen))
REDACTED

func TestResolveMediaURLs_Local(t *testing.T) {
	svc := NewSoraGenerationService(newStubGenRepo(), nil, nil)
	gen := &SoraGeneration{StorageType: SoraStorageTypeLocal, MediaURL: "/video/2024/01/01/file.mp4"REDACTED
	require.NoError(t, svc.ResolveMediaURLs(context.Background(), gen))
...
REDACTED
	...
	// 3. 标记 completed
	err = svc.MarkCompleted(context.Background(), gen.ID, "https://s3.com/video.mp4", nil, SoraStorageTypeS3, []string{"key"REDACTED, 1024)
REDACTED
...
REDACTED
	...
	userRepo.users[1] = &User{ID: 1, SoraStorageUsedBytes: 2097152REDACTED // 2MB
...
func TestDelete_S3Cleanup_And_QuotaRelease(t *testing.T) {
	...
	repo.gens[1] = &SoraGeneration{
		ID: 1, UserID: 1,
		StorageType:   SoraStorageTypeS3,
		S3ObjectKeys:  []string{"key1"REDACTED,
		FileSizeBytes: 512,
REDACTED
...
REDACTED
	...
	userRepo := newStubUserRepoForQuota()
	userRepo.users[1] = &User{ID: 1, SoraStorageUsedBytes: 2048REDACTED
	quotaService := NewSoraQuotaService(userRepo, nil, nil)
...
func TestResolveMediaURLs_S3_CDN_SingleKey(t *testing.T) {
	...
	gen := &SoraGeneration{
		StorageType:  SoraStorageTypeS3,
		S3ObjectKeys: []string{"sora/1/2024/01/01/video.mp4"REDACTED,
		MediaURL:     "original",
REDACTED
...
REDACTED
	err := svc.ResolveMediaURLs(context.Background(), gen)
REDACTED // 第一个 key 的 GetAccessURL 就会失败
REDACTED
