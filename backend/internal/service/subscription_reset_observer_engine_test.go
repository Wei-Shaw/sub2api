...
func TestSubscriptionResetObserverEarlyDropsStayReviewAndKeepDistinctSequences(t *testing.T) {
	...
	observerApply(t, p, state, observerTestSample(1, at.Add(time.Minute), reset, 60))
	alias := observerTestSample(3, at.Add(time.Minute+time.Second), reset, 60)
	alias.Subject.UserID = "user-1"
	observerApply(t, p, state, alias)
	require.Len(t, state.Events, 1)
	observerApply(t, p, state, observerTestSample(2, at.Add(2*time.Minute), reset, 60))
	require.Equal(t, "needs_review", state.Events[0].Status)
	require.Zero(t, state.Events[0].ConfirmedCount)
	observerApply(t, p, state, observerTestSample(1, at.Add(3*time.Minute), reset, 10))
...
func TestSubscriptionResetPolicyValidationRejectsAutoAndUnsupportedSource(t *testing.T) {
