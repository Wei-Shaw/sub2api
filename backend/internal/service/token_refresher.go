...
// XAITokenRefresher 处理 xAI OAuth token刷新
type XAITokenRefresher struct {
	xaiOAuthService *XAIOAuthService
}

// NewXAITokenRefresher 创建 xAI token刷新器
func NewXAITokenRefresher(xaiOAuthService *XAIOAuthService) *XAITokenRefresher {
	return &XAITokenRefresher{
		xaiOAuthService: xaiOAuthService,
	}
}

// CacheKey 返回用于分布式锁的缓存键
func (r *XAITokenRefresher) CacheKey(account *Account) string {
	return XAITokenCacheKey(account)
}

// CanRefresh 检查是否能处理此账号
func (r *XAITokenRefresher) CanRefresh(account *Account) bool {
	return account.Platform == PlatformXAI && account.Type == AccountTypeOAuth
}

// NeedsRefresh 检查token是否需要刷新
// expires_at 缺失且处于限流状态时需要刷新，保持与 OpenAI OAuth 的请求恢复语义一致。
func (r *XAITokenRefresher) NeedsRefresh(account *Account, refreshWindow time.Duration) bool {
	expiresAt := account.GetCredentialAsTime("expires_at")
	if expiresAt == nil {
		return account.IsRateLimited()
	}
...
// Refresh 执行token刷新
// 保留原有credentials中的所有字段，只更新token相关字段
func (r *XAITokenRefresher) Refresh(ctx context.Context, account *Account) (map[string]any, error) {
	tokenInfo, err := r.xaiOAuthService.RefreshAccountToken(ctx, account)
	if err != nil {
		return nil, err
	}

	newCredentials := r.xaiOAuthService.BuildAccountCredentials(tokenInfo)
	newCredentials = MergeCredentials(account.Credentials, newCredentials)
