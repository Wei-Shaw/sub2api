...
REDACTED {
		...
		require.True(t, isUpstreamBillingProbeAccount(&Account{Platform: platform, Type: AccountTypeAPIKeyREDACTED), platform)
REDACTED
...
func TestUpstreamBillingProbeNonOpenAIWithoutBaseURLIsUnsupportedWithoutRequest(t *testing.T) {
	account := &Account{
		...
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
REDACTED"api_key": "sk-grok-official"REDACTED,
REDACTED
	repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: accountREDACTEDREDACTED
...
func TestUpstreamBillingProbeOfficialAPIBaseURLIsUnsupportedWithoutRequest(t *testing.T) {
	cases := []struct {
		...
		baseURL  string
REDACTED{
		{PlatformAnthropic, "https://api.anthropic.com"REDACTED,
		{PlatformAnthropic, "https://api.anthropic.com:443"REDACTED,
		{PlatformAnthropic, "https://api.anthropic.com./"REDACTED,
		{PlatformAnthropic, "HTTPS://API.ANTHROPIC.COM/"REDACTED,
		{PlatformGemini, "https://generativelanguage.googleapis.com"REDACTED,
		{PlatformAntigravity, "https://cloudcode-pa.googleapis.com"REDACTED,
		{PlatformGrok, "https://api.x.ai/v1"REDACTED,
		{PlatformGrok, "https://us-east-1.api.x.ai/v1"REDACTED,
		{PlatformGrok, "https://eu-west-1.api.x.ai/v1"REDACTED,
		{PlatformGrok, "https://cli-chat-proxy.grok.com/v1"REDACTED,
		// 跨平台官方域同样必无该端点，一并拦截。
		{PlatformAnthropic, "https://api.x.ai/v1"REDACTED,
		{PlatformGrok, "https://api.openai.com"REDACTED,
		// Ollama Cloud 是本仓一等支持配置（platform openai/anthropic +
		// base_url https://ollama.com/v1），同为官方 API，不能拿 Key 去空探。
		{PlatformAnthropic, "https://ollama.com/v1"REDACTED,
		{PlatformAnthropic, "https://ollama.com"REDACTED,
		{PlatformAnthropic, "https://www.ollama.com/v1"REDACTED,
		// 国产供应商官方域（含各协议端点）同样是官方 API，创建即开探测也不发请求。
		{PlatformKimi, "https://api.moonshot.cn/v1"REDACTED,
		{PlatformKimi, "https://api.moonshot.cn/anthropic"REDACTED,
		{PlatformKimi, "https://api.kimi.com/coding"REDACTED,
		{PlatformZhipu, "https://open.bigmodel.cn/api/paas/v4"REDACTED,
		{PlatformZhipu, "https://open.bigmodel.cn/api/anthropic"REDACTED,
		{PlatformDeepseek, "https://api.deepseek.com"REDACTED,
		{PlatformDeepseek, "https://api.deepseek.com/anthropic"REDACTED,
REDACTED
	for i, tc := range cases {
		account := &Account{
			...
			Type:        AccountTypeAPIKey,
			Status:      StatusActive,
	REDACTED"api_key": "sk-official", "base_url": tc.baseURLREDACTED,
	REDACTED
		repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: accountREDACTEDREDACTED
		upstream := &httpUpstreamRecorder{REDACTED
		svc := newUpstreamBillingProbeTestService(repo, upstream, &upstreamBillingProbeSettingRepo{REDACTED)
...
func TestUpstreamBillingProbeSetAccountEnabledAcceptsGrokAPIKey(t *testing.T) {
	grokAPIKey := &Account{
		...
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
REDACTED"api_key": "sk", "base_url": "https://relay.example"REDACTED,
REDACTED
	grokOAuth := &Account{ID: 152, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActiveREDACTED
