...
var (
	...
	upstreamResponseModelBenchmarkTerminal = []byte(`{"type":"response.completed","response":{"id":"resp_123","model":"gpt-5.5-2026-04-23"REDACTED,"usage":{"input_tokens":128,"output_tokens":64REDACTEDREDACTED`)
	upstreamResponseModelBenchmarkDelta    = []byte(`{"type":"response.output_text.delta","delta":"hello"REDACTED`)
	upstreamResponseModelBenchmarkWrapper  = []byte(`{"response":{"response":{"modelVersion":"gemini-3-pro","candidates":[]REDACTEDREDACTEDREDACTED`)
)
...
func BenchmarkUpstreamResponseModelOpenAI(b *testing.B) {
	tests := []struct {
		...
		payload []byte
REDACTED{
		{name: "terminal_with_model", payload: upstreamResponseModelBenchmarkTerminalREDACTED,
		{name: "delta_without_model", payload: upstreamResponseModelBenchmarkDeltaREDACTED,
REDACTED
