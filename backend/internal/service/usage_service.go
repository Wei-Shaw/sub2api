...
func (s *UsageService) GetDailyStats(ctx context.Context, userID int64, days int) ([]map[string]interface{REDACTED, error) {
	endTime := time.Now()
...
REDACTED
	...
	for _, log := range logs {
		...
		if _, exists := dailyStats[dateKey]; !exists {
			dailyStats[dateKey] = &UsageStats{REDACTED
	REDACTED
...
REDACTED
	...
	// 计算平均值并转换为数组
	result := make([]map[string]interface{REDACTED, 0, len(dailyStats))
	for date, stats := range dailyStats {
		if stats.TotalRequests > 0 {
			stats.AverageDurationMs /= float64(stats.TotalRequests)
	REDACTED

		result = append(result, map[string]interface{REDACTED{
			"date":                  date,
