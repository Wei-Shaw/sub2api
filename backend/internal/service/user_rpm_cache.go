...
//
// 与账号级 RPMCache 的区别：
//   - RPMCache    —— 按外部 AI provider 账号聚合（key: rpm:{accountIDREDACTED:{minREDACTED）。
//   - UserRPMCache —— 按用户或 (用户, 分组) 聚合，杜绝"同一用户创建多个 API Key 绕过 RPM"的路径。
//     key 形如 rpm:ug:{userIDREDACTED:{groupIDREDACTED:{minREDACTED 或 rpm:u:{userIDREDACTED:{minREDACTED。
