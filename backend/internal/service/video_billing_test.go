...
func TestNormalizeAndLookupVideoModelPrices(t *testing.T) {
	...
	raw := map[string]map[string]float64{
		"grok-imagine-video-1.5-preview": {"480p": 0.08, "720p": 0.14REDACTED,
		"grok-imagine-video":             {"480p": 0.05REDACTED,
		"grok-imagine-video-2":           {"1080p": 0.4REDACTED,
REDACTED
...
func TestNormalizeVideoModelPricesDropsUnknownResolutions(t *testing.T) {
	...
	norm := NormalizeVideoModelPrices(map[string]map[string]float64{
		"grok-imagine-video": {"480p": 0.05, "4k": 0.50, "1080i": 0.30REDACTED,
REDACTED)
	require.NotNil(t, norm)
	require.Equal(t, map[string]float64{VideoBillingResolution480P: 0.05REDACTED, norm[VideoPriceFamilyGrokImagineVideo])
	...
	require.Nil(t, NormalizeVideoModelPrices(map[string]map[string]float64{
		"grok-imagine-video": {"4k": 0.50REDACTED,
REDACTED))
REDACTED
...
func TestNormalizeVideoModelPricesIsDeterministicAcrossAliasConflicts(t *testing.T) {
	...
	raw := map[string]map[string]float64{
		"grok-imagine-video-1.5":         {"480p": 0.08REDACTED,
		"grok-imagine-video-1.5-preview": {"480p": 0.11REDACTED,
		"grok-video-1.5":                 {"480p": 0.09REDACTED,
REDACTED
...
REDACTED
	...
	aliased := map[string]map[string]float64{
		"grok-imagine-video": {"720": 0.12, "720p": 0.13, "hd": 0.14REDACTED,
REDACTED
...
func TestLookupVideoBillingResolutionReportsUnknownTiers(t *testing.T) {
	t.Parallel()
	for _, in := range []string{"480", "480p", "SD", "720", "hd", "1080", "full-hd", " fhd "REDACTED {
		normalized, ok := LookupVideoBillingResolution(in)
		require.True(t, ok, "input=%q", in)
		require.NotEmpty(t, normalized)
REDACTED
	for _, in := range []string{"", "4k", "1080i", "2160p", "potato"REDACTED {
		normalized, ok := LookupVideoBillingResolution(in)
