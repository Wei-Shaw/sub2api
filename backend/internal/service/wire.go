...
// ProvidePlazaAccountSource exposes the account repository through the
// read-only subset required by PlazaService. Wire does not implicitly convert
// one interface type to another, even when the source satisfies the target.
