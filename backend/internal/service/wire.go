...
// ProvidePricingService constructs PricingService without synchronizing remote data.
func ProvidePricingService(cfg *config.Config, remoteClient PricingRemoteClient) *PricingService {
	return NewPricingService(cfg, remoteClient)
}
...
func ProvideOpenAICodexVersionSyncService(
	...
	return NewOpenAICodexVersionSyncService(settingRepo, settingService, githubClient, openAICodexVersionSyncInterval)
}
...
func ProvideProxyExpiryService(proxyRepo ProxyRepository) *ProxyExpiryService {
	return NewProxyExpiryService(proxyRepo, time.Minute)
}
...
// ProvideConcurrencyService creates ConcurrencyService.
...
// ProvideUserMessageQueueService creates UserMessageQueueService.
...
func ProvideOpsAlertEvaluatorService(
	...
	return NewOpsAlertEvaluatorService(opsService, opsRepo, emailService, redisClient, cfg, proxyRepo)
}
...
func ProvideAuditLogService(repo AuditLogRepository, settingService *SettingService) *AuditLogService {
	return NewAuditLogService(repo, settingService)
}
...
func ProvideUserPlatformQuotaUsageFlusher(cfg *config.Config, cache BillingCache, quotaRepo UserPlatformQuotaRepository, tw *TimingWheelService) *UserPlatformQuotaUsageFlusher {
	return NewUserPlatformQuotaUsageFlusher(cfg, cache, quotaRepo, tw)
}
