...
func (s *FrontendServer) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if cleanPath == "index.html" || !s.fileExists(cleanPath) {
			if cleanPath != "index.html" && !isKnownFrontendRoute(path) {
...
func (s *FrontendServer) serveIndexHTMLWithStatus(c *gin.Context, status int) {
	...
	if cached != nil {
		...
		if match := c.GetHeader("If-None-Match"); status == http.StatusOK && match == cached.ETag {
	...
	if err != nil {
		...
		c.Data(status, "text/html; charset=utf-8", s.baseHTML)
		c.Abort()
		return
	}
...
func injectSiteTitle(html, settingsJSON []byte) []byte {
	...
	currentTitle := string(html[titleStart+len("<title>") : titleEnd])
	if !shouldReplaceInjectedTitle(currentTitle) {
		return html
	}
...
func shouldReplaceInjectedTitle(title string) bool {
	normalized := strings.TrimSpace(title)
	return normalized == "Sub2API" || strings.Contains(normalized, "Sub2API - AI API Gateway")
}
...
func isKnownFrontendRoute(path string) bool {
	trimmed := strings.TrimSpace(path)
	if trimmed == "" {
		trimmed = "/"
	}
	...
	if _, ok := exactRoutes[trimmed]; ok {
		return true
	}

	return strings.HasPrefix(trimmed, "/admin") ||
		strings.HasPrefix(trimmed, "/auth/") ||
		strings.HasPrefix(trimmed, "/custom/")
}

func serveIndexHTML(c *gin.Context, fsys fs.FS, status int) {
