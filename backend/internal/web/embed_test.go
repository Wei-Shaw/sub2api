...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"MyCustomSite"REDACTED`)
...
REDACTED)
	...
	t.Run("returns_unchanged_when_site_name_empty", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{"site_name":""REDACTED`)
...
REDACTED)
	...
	t.Run("returns_unchanged_when_site_name_missing", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{"other_field":"value"REDACTED`)
...
REDACTED)
	...
	t.Run("returns_unchanged_when_invalid_json", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{invalid jsonREDACTED`)
...
REDACTED)
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"NewSite"REDACTED`)
...
REDACTED)
	...
	t.Run("escapes_html_in_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{"site_name":"</title><script>alert(1)</script><title>"REDACTED`)
...
REDACTED)
	...
	t.Run("escapes_ampersand_in_site_name", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"A&B"REDACTED`)
...
REDACTED)
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"TestSite"REDACTED`)
...
func TestInjectSiteFavicon(t *testing.T) {
	t.Run("replaces_favicon_with_site_logo", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_logo":"https://example.com/custom-logo.png"REDACTED`)
...
REDACTED)
	...
	t.Run("supports_relative_and_data_image_urls", func(t *testing.T) {
		...
		assert.Contains(t, string(injectSiteFavicon(html, []byte(`{"site_logo":"/uploads/logo.svg"REDACTED`))), `/uploads/logo.svg`)
		assert.Contains(t, string(injectSiteFavicon(html, []byte(`{"site_logo":"data:image/png;base64,abc"REDACTED`))), `data:image/png;base64,abc`)
REDACTED)

	t.Run("rejects_unsafe_logo_urls", func(t *testing.T) {
		html := []byte(`<link rel="icon" href="/logo.png" />`)

		result := injectSiteFavicon(html, []byte(`{"site_logo":"javascript:alert(1)"REDACTED`))
...
REDACTED)
	...
	t.Run("escapes_logo_url_for_html", func(t *testing.T) {
		...
		result := injectSiteFavicon(html, []byte(`{"site_logo":"https://example.com/logo.png?a=1&b=2"REDACTED`))
...
func TestFrontendServer_InjectSettings(t *testing.T) {
	t.Run("injects_settings_with_nonce_placeholder", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"key": "value"REDACTED,
	REDACTED

		server, err := NewFrontendServer(provider)
	REDACTED

		settingsJSON := []byte(`{"test":"data"REDACTED`)
		...
		assert.Contains(t, string(result), `window.__APP_CONFIG__={"test":"data"REDACTED;`)
...
REDACTED)
	...
	REDACTED
		...
		settingsJSON := []byte(`{"nested":{"array":[1,2,3]REDACTED,"special":"<>&"REDACTED`)
		result := server.injectSettings(settingsJSON)

		assert.Contains(t, string(result), `window.__APP_CONFIG__={"nested":{"array":[1,2,3]REDACTED,"special":"<>&"REDACTED;`)
REDACTED)
REDACTED
...
func TestFrontendServer_ServeIndexHTML(t *testing.T) {
	t.Run("serves_html_with_nonce", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"REDACTED,
	REDACTED
...
func TestFrontendServer_InvalidateCache(t *testing.T) {
	...
	REDACTED
		...
		// Update settings
		provider.settings = map[string]string{"test": "new_value"REDACTED
...
REDACTED)
	...
	t.Run("handles_nil_cache", func(t *testing.T) {
		server := &FrontendServer{REDACTED
		// Should not panic
		assert.NotPanics(t, func() {
			server.InvalidateCache()
	REDACTED)
...
func TestOverrideFilesNeverReceiveImmutableCacheHeaders(t *testing.T) {
	...
	t.Run("frontend_server_override", func(t *testing.T) {
		...
		server := &FrontendServer{overrideDir: overrideDirREDACTED
...
REDACTED)
	...
	REDACTED
		...
		router.POST("/responses/compact", func(c *gin.Context) {
			nextCalled = true
			c.String(http.StatusOK, `{"ok":trueREDACTED`)
	REDACTED)

		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/responses/compact", strings.NewReader(`{"model":"gpt-5"REDACTED`))
		req.Header.Set("Content-Type", "application/json")
		router.ServeHTTP(w, req)

		assert.True(t, nextCalled, "next handler should be called for compact API route")
		assert.Equal(t, http.StatusOK, w.Code)
		assert.JSONEq(t, `{"ok":trueREDACTED`, w.Body.String())
REDACTED)
	...
	REDACTED)
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/alpha/search", strings.NewReader(`{"model":"gpt-5.6-sol"REDACTED`))
		req.Header.Set("Content-Type", "application/json")
		router.ServeHTTP(w, req)
...
REDACTED)
	...
	t.Run("serves_index_html_for_spa_routes", func(t *testing.T) {
		...
		spaPaths := []string{"/dashboard", "/users/123", "/settings"REDACTED
...
REDACTED)
	...
	t.Run("set_and_get", func(t *testing.T) {
		...
		settings := []byte(`{"key":"value"REDACTED`)
...
REDACTED)
	...
	t.Run("etag_changes_with_settings", func(t *testing.T) {
		...
		cache.Set(html, []byte(`{"v":1REDACTED`))
		etag1 := cache.Get().ETag

		cache.Invalidate()
		cache.Set(html, []byte(`{"v":2REDACTED`))
...
REDACTED)
	...
	t.Run("etag_format", func(t *testing.T) {
		...
		cache.Set([]byte("<html></html>"), []byte(`{REDACTED`))
		result := cache.Get()
...
func BenchmarkReplaceNoncePlaceholder(b *testing.B) {
	html := []byte(`<!DOCTYPE html><html><head><script nonce="__CSP_NONCE_VALUE__">window.__APP_CONFIG__={"test":"data"REDACTED;</script></head><body></body></html>`)
