...
-- 2. model_mapping: 从扁平 {"src":"dst"REDACTED 迁移为嵌套 {"anthropic":{"src":"dst"REDACTEDREDACTED
-- 仅迁移非空、非 '{REDACTED' 的旧格式数据（通过检查第一个 value 是否为字符串来判断是否为旧格式）
UPDATE channels
SET model_mapping = jsonb_build_object('anthropic', model_mapping)
WHERE model_mapping IS NOT NULL
  AND model_mapping::text NOT IN ('{REDACTED', 'null', '')
  AND NOT EXISTS (
