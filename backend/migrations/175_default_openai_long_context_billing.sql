...
BEGIN
    ...
    NEW.extra := COALESCE(NEW.extra, '{REDACTED'::jsonb);
    IF NEW.parent_account_id IS NOT NULL AND NEW.quota_dimension = 'spark' THEN
        SELECT CASE
            ...
            WHEN NOT (COALESCE(parent.extra, '{REDACTED'::jsonb) ? 'openai_long_context_billing_enabled') THEN 'false'::jsonb
        ...
        NEW.extra := jsonb_set(
            NEW.extra,
            '{openai_long_context_billing_enabledREDACTED',
...
BEGIN
    WITH updated_shadows AS (
        ...
        SET extra = jsonb_set(
            COALESCE(shadow.extra, '{REDACTED'::jsonb),
            '{openai_long_context_billing_enabledREDACTED',
...
WHERE platform = 'openai'
  AND COALESCE(extra, '{REDACTED'::jsonb) ? 'openai_long_context_billing_enabled'
...
WHERE platform = 'openai'
  ...
  AND NOT (COALESCE(extra, '{REDACTED'::jsonb) ? 'openai_long_context_billing_enabled');
