...
`{remote_hostREDACTED` forwarding lines unchanged behind a CDN: all clients would be
...
and derive upstream headers from Caddy's parsed `{client_ipREDACTED`. For example:
...
api.example.com {
	reverse_proxy 127.0.0.1:8080 {
		header_up X-Real-IP {client_ipREDACTED
		header_up X-Forwarded-For {client_ipREDACTED
REDACTED
REDACTED
```
