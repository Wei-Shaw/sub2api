# AI2API Deployment Files

This directory contains files for deploying AI2API on Linux servers.
...
| `ai2api.service` | Systemd service unit file |
| `ai2api-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/Usersxiao/ai2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs ai2api | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/ai2api-datamanagement.sock`
...
# Restart AI2API only
docker compose -f docker-compose.local.yml restart ai2api
...
| `ADMIN_EMAIL` | No | `admin@ai2api.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf ai2api-complete.tar.gz deployment/

# Transfer to new server
scp ai2api-complete.tar.gz user@new-server:/path/to/destination/
...
AI2API supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/Usersxiao/ai2api/releases)
2. Extract and copy the binary to `/opt/ai2api/`
3. Copy `ai2api.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/ai2api.service
   ```
...
The main config file is at `/etc/ai2api/config.yaml` (created by Setup Wizard).
...
/opt/ai2api/
├── ai2api              # Main binary
├── ai2api.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 ai2api
...
# View detailed logs
docker compose logs --tail=100 ai2api
...
# View recent logs
sudo journalctl -u ai2api -n 50

# Check config file
sudo cat /etc/ai2api/config.yaml
...
AI2API supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.ai2api.org](https://tls.ai2api.org/)** to get TLS fingerprint information for different devices and browsers.
