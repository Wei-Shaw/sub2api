# Nub2API Deployment Files

This directory contains files for deploying Nub2API on Linux servers.
...
| `nub2api.service` | Systemd service unit file |
| `nub2api-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/Nub2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs nub2api | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/nub2api-datamanagement.sock`
...
# Restart Nub2API only
docker compose -f docker-compose.local.yml restart nub2api
...
# Restart Nub2API only
docker compose restart nub2api
...
| `ADMIN_EMAIL` | No | `admin@nub2api.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf nub2api-complete.tar.gz deployment/

# Transfer to new server
scp nub2api-complete.tar.gz user@new-server:/path/to/destination/
...
Nub2API supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/Wei-Shaw/Nub2api/releases)
2. Extract and copy the binary to `/opt/nub2api/`
3. Copy `nub2api.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/nub2api.service
   ```
...
The main config file is at `/etc/nub2api/config.yaml` (created by Setup Wizard).
...
/opt/nub2api/
├── nub2api              # Main binary
├── nub2api.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 nub2api
...
# View detailed logs
docker compose logs --tail=100 nub2api
...
# View recent logs
sudo journalctl -u nub2api -n 50

# Check config file
sudo cat /etc/nub2api/config.yaml
...
Nub2API supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.nub2api.org](https://tls.nub2api.org/)** to get TLS fingerprint information for different devices and browsers.
