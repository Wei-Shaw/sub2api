...
print_warning() {
    echo -e "${YELLOWREDACTED[WARNING]${NCREDACTED $1"
REDACTED
...
main() {
    ...
    if command_exists curl; then
        curl -sSL "${GITHUB_RAW_URLREDACTED/docker-compose.local.yml" -o docker-compose.local.yml
    elif command_exists wget; then
        wget -q "${GITHUB_RAW_URLREDACTED/docker-compose.local.yml" -O docker-compose.local.yml
    else
    ...
    if command_exists curl; then
        curl -sSL "${GITHUB_RAW_URLREDACTED/.env.example" -o .env.example
    else
        wget -q "${GITHUB_RAW_URLREDACTED/.env.example" -O .env.example
    fi
    ...
    if sed --version >/dev/null 2>&1; then
        # GNU sed (Linux)
        sed -i "s/^JWT_SECRET=.*/JWT_SECRET=${JWT_SECRETREDACTED/" .env
        sed -i "s/^TOTP_ENCRYPTION_KEY=.*/TOTP_ENCRYPTION_KEY=${TOTP_ENCRYPTION_KEYREDACTED/" .env
        sed -i "s/^POSTGRES_PASSWORD=.*/POSTGRES_PASSWORD=${POSTGRES_PASSWORDREDACTED/" .env
    else
        # BSD sed (macOS)
        sed -i '' "s/^JWT_SECRET=.*/JWT_SECRET=${JWT_SECRETREDACTED/" .env
        sed -i '' "s/^TOTP_ENCRYPTION_KEY=.*/TOTP_ENCRYPTION_KEY=${TOTP_ENCRYPTION_KEYREDACTED/" .env
    ...
    echo "  POSTGRES_PASSWORD:     ${POSTGRES_PASSWORDREDACTED"
    echo "  JWT_SECRET:            ${JWT_SECRETREDACTED"
    echo "  TOTP_ENCRYPTION_KEY:   ${TOTP_ENCRYPTION_KEYREDACTED"
    echo ""
