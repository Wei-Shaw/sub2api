...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/nub2api/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/nub2api"
DATA_DIR="/var/lib/nub2api/datamanagement"
SERVICE_FILE_NAME="nub2api-datamanagementd.service"
...
参数:
  ...
  --source  指定 nub2api 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/nub2api-src
EOF
}
...
if ! id nub2api >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 nub2api..."
  useradd --system --no-create-home --shell /usr/sbin/nologin nub2api
else
  echo "[2/6] 系统用户 nub2api 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R nub2api:nub2api /var/lib/nub2api
chmod 0750 "$DATA_DIR"
...
systemctl enable --now nub2api-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status nub2api-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u nub2api-datamanagementd -f
2. 在 nub2api（容器部署时）挂载 socket:
