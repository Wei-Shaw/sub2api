...
configure_server() {
    ...
    read -p "$(msg 'server_host_prompt') [${SERVER_HOSTREDACTED]: " input_host
    ...
    while true; do
        read -p "$(msg 'server_port_prompt') [${SERVER_PORTREDACTED]: " input_port
...
print_completion() {
    local ip_addr
    ip_addr=$(hostname -I 2>/dev/null | awk '{print $1REDACTED' || echo "YOUR_SERVER_IP")
