...
if [ -z "${BASH_VERSION:-REDACTED" ]; then
...
BASH_MAJOR_VERSION="${BASH_VERSION%%.*REDACTED"
...
msg() {
    ...
    if [ "$LANG_CHOICE" = "en" ]; then
        echo "${MSG_EN[$key]REDACTED"
    else
        echo "${MSG_ZH[$key]REDACTED"
    fi
REDACTED

# Print functions
print_info() {
    echo -e "${BLUEREDACTED[$(msg 'info')]${NCREDACTED $1"
REDACTED

print_success() {
    echo -e "${GREENREDACTED[$(msg 'success')]${NCREDACTED $1"
REDACTED

print_warning() {
    echo -e "${YELLOWREDACTED[$(msg 'warning')]${NCREDACTED $1"
REDACTED

print_error() {
    echo -e "${REDREDACTED[$(msg 'error')]${NCREDACTED $1"
REDACTED
...
configure_server() {
    ...
    if ! is_interactive; then
        print_info "$(msg 'server_config_summary'): ${SERVER_HOSTREDACTED:${SERVER_PORTREDACTED (default)"
        return
    fi
    ...
    echo "==============================================${NCREDACTED"
    echo ""
    echo -e "${BLUEREDACTED$(msg 'server_config_desc')${NCREDACTED"
    echo ""

    # Server host
    echo -e "${YELLOWREDACTED$(msg 'server_host_hint')${NCREDACTED"
    read -p "$(msg 'server_host_prompt') [${SERVER_HOSTREDACTED]: " input_host < /dev/tty
    ...
    # Server port
    echo -e "${YELLOWREDACTED$(msg 'server_port_hint')${NCREDACTED"
    while true; do
        read -p "$(msg 'server_port_prompt') [${SERVER_PORTREDACTED]: " input_port < /dev/tty
    ...
    echo ""
    print_info "$(msg 'server_config_summary'): ${SERVER_HOSTREDACTED:${SERVER_PORTREDACTED"
    echo ""
REDACTED
...
detect_platform() {
    ...
    print_info "$(msg 'detected_platform'): ${OSREDACTED_${ARCHREDACTED"
REDACTED
...
check_dependencies() {
    ...
    if [ ${#missing[@]REDACTED -gt 0 ]; then
        print_error "$(msg 'missing_deps'): ${missing[*]REDACTED"
...
github_api_curl() {
    ...
    if [ -n "${UPDATE_GITHUB_TOKEN:-REDACTED" ]; then
...
get_latest_version() {
    ...
    LATEST_VERSION=$(github_api_curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOREDACTED/releases/latest" 2>/dev/null | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
...
list_versions() {
    ...
    local versions
    versions=$(github_api_curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOREDACTED/releases" 2>/dev/null | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/' | head -20)
...
validate_version() {
    ...
    local http_code
    http_code=$(github_api_curl -s -o /dev/null -w "%{http_codeREDACTED" --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOREDACTED/releases/tags/${versionREDACTED" 2>/dev/null)
...
download_and_extract() {
    local version_num=${LATEST_VERSION#vREDACTED
    local archive_name="sub2api_${version_numREDACTED_${OSREDACTED_${ARCHREDACTED.tar.gz"
    local download_url="https://github.com/${GITHUB_REPOREDACTED/releases/download/${LATEST_VERSIONREDACTED/${archive_nameREDACTED"
    local checksum_url="https://github.com/${GITHUB_REPOREDACTED/releases/download/${LATEST_VERSIONREDACTED/checksums.txt"

    print_info "$(msg 'downloading') ${archive_nameREDACTED..."
    ...
    if curl -sL "$checksum_url" -o "$TEMP_DIR/checksums.txt" 2>/dev/null; then
        local expected_checksum=$(grep "$archive_name" "$TEMP_DIR/checksums.txt" | awk '{print $1REDACTED')
        local actual_checksum=$(sha256sum "$TEMP_DIR/$archive_name" | awk '{print $1REDACTED')
...
Environment=SERVER_HOST=${SERVER_HOSTREDACTED
Environment=SERVER_PORT=${SERVER_PORTREDACTED
...
get_public_ip() {
    ...
    PUBLIC_IP=$(hostname -I 2>/dev/null | awk '{print $1REDACTED' || echo "YOUR_SERVER_IP")
    return 1
REDACTED
...
print_completion() {
    ...
    local display_host="${PUBLIC_IP:-YOUR_SERVER_IPREDACTED"
    ...
    echo "$(msg 'server_config_summary'): ${SERVER_HOSTREDACTED:${SERVER_PORTREDACTED"
    echo ""
    echo "=============================================="
    echo "  $(msg 'step4_open_wizard')"
    echo "=============================================="
    echo ""
    print_info "     http://${display_hostREDACTED:${SERVER_PORTREDACTED"
    echo ""
...
install_version() {
    ...
    if [ "$current_version" = "$target_version" ] || [ "$current_version" = "${target_version#vREDACTED" ]; then
    ...
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="sub2api.backup.${current_versionREDACTED"
        else
...
uninstall() {
    ...
    if ! is_interactive; then
        if [ "${FORCE_YES:-REDACTED" != "true" ]; then
...
main() {
    ...
    case "${1:-REDACTED" in
        ...
        rollback)
            ...
            if [ -z "$target_version" ] && [ -n "${2:-REDACTED" ]; then
                target_version="$2"
            fi
            if [ -z "$target_version" ]; then
