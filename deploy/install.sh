...
get_latest_version() {
    ...
    LATEST_VERSION=$(curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOREDACTED/releases/latest" 2>/dev/null | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
...
list_versions() {
    ...
    local versions
    versions=$(curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOREDACTED/releases" 2>/dev/null | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/' | head -20)
...
validate_version() {
    ...
    local http_code
    http_code=$(curl -s -o /dev/null -w "%{http_codeREDACTED" --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOREDACTED/releases/tags/${versionREDACTED" 2>/dev/null)
