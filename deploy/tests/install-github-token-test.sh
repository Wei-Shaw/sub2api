...
ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]REDACTED")/../.." && pwd)
TEMP_DIR=$(mktemp -d)
...
#!/bin/bash
printf '%s\n' "$@" > "$CURL_ARGS_LOG"
env > "${CURL_ARGS_LOGREDACTED.env"
cat > "${CURL_ARGS_LOGREDACTED.stdin"
EOF
...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-REDACTED" \
