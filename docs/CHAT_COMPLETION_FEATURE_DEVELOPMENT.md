...
- 新增 `ChatCompletionView.vue`：
  ...
  - 支持在输入框粘贴截图或选择本地图片，图片作为临时附件随本次对话请求发送；当前版本不持久化图片文件到会话历史。
  - 多模态请求按平台转换为 OpenAI Responses、Chat Completions、Gemini 或 Anthropic Messages 的图片输入格式；带图发送会对模型名做保守的视觉能力判断。
