...
| **AppID** | 支付宝应用 AppID | 是 |
| **应用私钥** | RSA2 应用私钥 | 是 |
| **支付宝公钥** | 支付宝公钥 | 普通公钥模式必填 |

支付宝支持两种接口加签方式，必须与支付宝开放平台中对应 AppID 的设置一致：

- **普通公钥**（默认，兼容已有配置）：填写应用私钥和支付宝公钥。这里的支付宝公钥不是应用公钥，也不是证书文件。
- **公钥证书**：选择此模式后，填写应用私钥，并粘贴以下三个文件的完整 PEM 内容：

| 配置项 | 支付宝开放平台下载的文件 | 配置键 |
| --- | --- | --- |
| 应用公钥证书 | `appCertPublicKey_*.crt` | `appCertPublicKey` |
| 支付宝公钥证书 | `alipayCertPublicKey_RSA2.crt` | `alipayCertPublicKey` |
| 支付宝根证书 | `alipayRootCert.crt`（保留所有证书段） | `alipayRootCert` |

配置键 `authMode` 为 `public_key`（缺省值）或 `certificate`。应用私钥支持 RSA PKCS#1 和 PKCS#8。公钥证书模式无需另外提取或填写普通支付宝公钥；系统自动计算证书 SN 并加入请求签名，同时使用支付宝证书验证响应及通知。

启用证书模式时，保存会检查证书格式及应用证书与私钥是否匹配。编辑时证书内容与密钥一样不会回显，留空保留已保存内容。有待处理订单时不能更换加签方式或证书，需先完成或关闭这些订单。
