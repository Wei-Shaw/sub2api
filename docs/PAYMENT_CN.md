...
系统通过 createPaymentSession 创建托管会话，并自动传入通知地址、用户标识及支付完成回跳地址。会话创建成功、浏览器回跳以及银行卡授权成功都不直接入账；卡支付等待全额扣款成功。普通查单在五分钟通知等待期后兜底，显式取消前仍立即查单。

支持退款、退款查询及取消；退款处理中或结果未知时仅查询原退款，不能重新发起。确认失败后重试使用新的退款尝试标识；查询校验含手续费的网关退款金额，但不额外扣减用户余额。存在待支付订单时禁止更换商户、密钥、网关及币种。停用实例后仍可接收历史订单的扣款通知，不要提前删除实例或轮换验签公钥。
