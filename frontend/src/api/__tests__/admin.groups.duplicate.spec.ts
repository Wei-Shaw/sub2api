...
import { duplicate REDACTED from '@/api/admin/groups'
...
describe('admin group duplicate API', () => {
  beforeEach(() => {
    ...
    post.mockReset()
    post.mockResolvedValue({ data: { id: 43, name: 'primary (Copy)', status: 'inactive' REDACTED REDACTED)
  ...
  it('sends a stable idempotency key with the duplicate request', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/groups/42/duplicate', undefined, {
      ...
      REDACTED
    REDACTED)
    expect(group).toEqual({ id: 43, name: 'primary (Copy)', status: 'inactive' REDACTED)
  ...
  it('reuses the operation key after a page reload', async () => {
    ...
    const { duplicate: duplicateAfterReload REDACTED = await import('@/api/admin/groups')
  ...
  it('does not reuse an operation key across administrators for the same group', async () => {
    ...
    post.mockResolvedValueOnce({ data: { id: 56, name: 'second admin copy' REDACTED REDACTED)
    await duplicate(55)
  ...
  it('does not persist or reuse keys when the current user cannot be parsed', async () => {
    ...
    post.mockResolvedValueOnce({ data: { id: 67, name: 'fallback copy' REDACTED REDACTED)
    await duplicate(66)
