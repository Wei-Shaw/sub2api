...
import type { AxiosInstance REDACTED from 'axios'
...
describe('API Client', () => {
  ...
  beforeEach(async () => {
    localStorage.clear()
    window.history.replaceState({REDACTED, '', '/')
    // 每次测试重新导入以获取干净的模块状态
    vi.resetModules()
  ...
  describe('请求拦截器', () => {
    ...
    it('自动附加 Authorization 头', async () => {
      ...
      const adapter = vi.fn().mockResolvedValue({
        status: 200,
        data: { code: 0, data: {REDACTED REDACTED,
        headers: {REDACTED,
        config: {REDACTED,
        statusText: 'OK',
      REDACTED)
    ...
    it('POST 请求不附加 timezone 参数', async () => {
      ...
      await apiClient.post('/test', { foo: 'bar' REDACTED)
    ...
    it('管理页面调用共享 API 时带 Admin UI 标记', async () => {
      window.history.replaceState({REDACTED, '', '/admin/dashboard')
      const adapter = vi.fn().mockResolvedValue({
        status: 200,
        data: { code: 0, data: {REDACTED REDACTED,
        headers: {REDACTED,
    ...
    it('支付用户 API 带 User UI 标记，公开支付 API 不带', async () => {
      ...
      await apiClient.post('/payment/public/orders/verify', {REDACTED)
  ...
  describe('响应拦截器', () => {
    it('code=0 时解包 data 字段', async () => {
      const adapter = vi.fn().mockResolvedValue({
        status: 200,
        data: { code: 0, data: { name: 'test' REDACTED, message: 'ok' REDACTED,
        headers: {REDACTED,
        config: {REDACTED,
        statusText: 'OK',
      REDACTED)
      ...
      expect(response.data).toEqual({ name: 'test' REDACTED)
    REDACTED)

    it('code!=0 时拒绝并返回结构化错误', async () => {
      const adapter = vi.fn().mockResolvedValue({
        status: 200,
        data: { code: 1001, message: '参数错误', data: null REDACTED,
        headers: {REDACTED,
        config: {REDACTED,
        statusText: 'OK',
      REDACTED)
    ...
    it('部署与运营合规未确认时广播事件且保留登录态', async () => {
      ...
      const adapter = vi.fn().mockRejectedValue({
        ...
        config: {
          url: '/admin/users',
          headers: { Authorization: 'Bearer admin-token' REDACTED,
        REDACTED,
  ...
  describe('401 Token 刷新', () => {
    it('无 refresh_token 时 401 清除 localStorage', async () => {
      ...
      Object.defineProperty(window, 'location', {
        value: { ...originalLocation, pathname: '/dashboard', href: '/dashboard' REDACTED,
        writable: true,
      REDACTED)
      ...
      const adapter = vi.fn().mockRejectedValue({
        response: {
          status: 401,
          data: { code: 'TOKEN_EXPIRED', message: 'Token expired' REDACTED,
        REDACTED,
        config: {
          url: '/test',
          headers: { Authorization: 'Bearer expired-token' REDACTED,
        REDACTED,
        code: 'ERR_BAD_REQUEST',
      REDACTED)
      apiClient.defaults.adapter = adapter
    ...
    it('有 refresh_token 时刷新并重试原请求', async () => {
      ...
      localStorage.setItem('auth_user', JSON.stringify({ id: 7 REDACTED))
      ...
      const adapter = vi.fn()
        ...
        .mockResolvedValueOnce({
          status: 200,
          data: { code: 0, data: { ok: true REDACTED REDACTED,
          headers: {REDACTED,
          config: {REDACTED,
          statusText: 'OK',
        REDACTED)
      ...
      await expect(apiClient.get('/test')).resolves.toMatchObject({ data: { ok: true REDACTED REDACTED)
    ...
    it('刷新期间换号时旧请求不会清除新会话', async () => {
      ...
      apiClient.defaults.adapter = vi.fn().mockRejectedValueOnce({
        ...
        config: {
          url: '/test',
          headers: { Authorization: 'Bearer user-a-access' REDACTED,
        REDACTED,
        code: 'ERR_BAD_REQUEST',
      REDACTED)
      ...
      localStorage.setItem('auth_user', JSON.stringify({ id: 8 REDACTED))
      rejectRefresh(new Error('stale refresh failed'))

      await expect(staleRequest).rejects.toMatchObject({ code: 'AUTH_SESSION_CHANGED' REDACTED)
      expect(localStorage.getItem('auth_token')).toBe('user-b-access')
      expect(localStorage.getItem('refresh_token')).toBe('user-b-refresh')
      expect(localStorage.getItem('auth_user')).toBe(JSON.stringify({ id: 8 REDACTED))
  ...
  describe('请求取消', () => {
    it('取消的请求保持原始取消错误', async () => {
      ...
      await expect(
        apiClient.get('/test', { cancelToken: source.token REDACTED)
      ).rejects.toBeDefined()
    REDACTED)
  REDACTED)
REDACTED)
