...
const { get, post, patch, remove, credentialGet, credentialCreate REDACTED = vi.hoisted(() => ({
  get: vi.fn(),
  post: vi.fn(),
  patch: vi.fn(),
  remove: vi.fn(),
...
describe('passkey api', () => {
  beforeEach(() => {
    ...
    Object.defineProperty(navigator, 'credentials', {
      configurable: true,
      value: { get: credentialGet, create: credentialCreate REDACTED
    REDACTED)
  REDACTED)
  ...
  it('sends Tencent captcha proof only with the passkey begin request', async () => {
    post
      .mockResolvedValueOnce({
        ...
        REDACTED
      REDACTED)
      .mockResolvedValueOnce({ data: { access_token: 'access', token_type: 'Bearer', user: { id: 1 REDACTED REDACTED REDACTED)
    credentialGet.mockResolvedValue(new FakePublicKeyCredential())
      ...
      .mockResolvedValueOnce({
        data: {
          ...
          options: {
            publicKey: {
              challenge: 'AQID',
              user: { id: 'BAU', name: 'user@example.com', displayName: 'user' REDACTED
            REDACTED
          REDACTED
        REDACTED
      REDACTED)
      .mockResolvedValueOnce({
        data: { id: 3, name: 'Laptop', created_at: '2026-07-28T00:00:00Z', backup: false REDACTED
      REDACTED)
