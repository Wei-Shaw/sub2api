...
function duplicateOperationStorageKey(id: number): string {
  return `sub2api:admin:account-duplicate:${idREDACTED`
REDACTED
...
export async function duplicate(id: number): Promise<Account> {
  ...
  if (!idempotencyKey) {
    ...
    idempotencyKey = `account-duplicate-${idREDACTED-${requestIDREDACTED`
  REDACTED
  duplicateOperationKeys.set(id, idempotencyKey)
  storeDuplicateOperationKey(id, idempotencyKey)
  const { data REDACTED = await apiClient.post<Account>(`/admin/accounts/${idREDACTED/duplicate`, undefined, {
...
export async function getBatchUsage(accountIds: number[], force?: boolean): Promise<BatchAccountUsageResponse> {
  const { data REDACTED = await apiClient.post<BatchAccountUsageResponse>('/admin/accounts/usage/batch', {
...
export async function createOpenAICodexPAT(payload: OpenAICodexPATCreateRequest): Promise<Account> {
  const { data REDACTED = await apiClient.post<Account>('/admin/openai/create-from-codex-pat', payload)
  return data
REDACTED
...
export async function refreshOpenAIQuota(id: number): Promise<OpenAIQuotaRefreshResult> {
  const { data REDACTED = await apiClient.post<OpenAIQuotaRefreshResult>(
    `/admin/openai/accounts/${idREDACTED/quota/refresh`
  )
  return data
REDACTED
...
export async function resetOpenAIQuota(id: number): Promise<OpenAIQuotaResetResult> {
  const { data REDACTED = await apiClient.post<OpenAIQuotaResetResult>(
    `/admin/openai/accounts/${idREDACTED/reset-quota`,
    undefined,
    { timeout: 90_000 REDACTED
  )
  return data
REDACTED
...
export async function createSparkShadow(parentId: number, payload: SparkShadowCreatePayload): Promise<Account> {
  const { data REDACTED = await apiClient.post<Account>(`/admin/accounts/${parentIdREDACTED/shadow`, payload)
  return data
REDACTED
...
export async function getOllamaCloudUsageSettings(): Promise<OllamaCloudUsageSettings> {
  const { data REDACTED = await apiClient.get<OllamaCloudUsageSettings>('/admin/accounts/ollama-cloud-usage/settings')
  return data
REDACTED
...
export async function updateOllamaCloudUsageSettings(
  ...
  const { data REDACTED = await apiClient.put<OllamaCloudUsageSettings>(
...
export async function getOllamaCloudUsage(id: number): Promise<OllamaCloudUsageState> {
  const { data REDACTED = await apiClient.get<OllamaCloudUsageState>(`/admin/accounts/${idREDACTED/ollama-cloud-usage`)
  return data
REDACTED

export async function saveOllamaCloudUsageSession(id: number, session: string): Promise<OllamaCloudUsageState> {
  const { data REDACTED = await apiClient.put<OllamaCloudUsageState>(`/admin/accounts/${idREDACTED/ollama-cloud-usage/session`, {
    session
  REDACTED)
  return data
REDACTED
...
export async function deleteOllamaCloudUsageSession(id: number): Promise<OllamaCloudUsageState> {
  const { data REDACTED = await apiClient.delete<OllamaCloudUsageState>(`/admin/accounts/${idREDACTED/ollama-cloud-usage/session`)
  return data
REDACTED

export async function setOllamaCloudUsageAutoRefresh(id: number, enabled: boolean): Promise<OllamaCloudUsageState> {
  const { data REDACTED = await apiClient.put<OllamaCloudUsageState>(`/admin/accounts/${idREDACTED/ollama-cloud-usage/auto-refresh`, {
    enabled
  REDACTED)
  return data
REDACTED
...
export async function refreshOllamaCloudUsage(id: number): Promise<OllamaCloudUsageState> {
  const { data REDACTED = await apiClient.post<OllamaCloudUsageState>(`/admin/accounts/${idREDACTED/ollama-cloud-usage/refresh`)
  return data
REDACTED
