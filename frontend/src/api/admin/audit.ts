...
export async function list(params: AuditLogQuery): Promise<AuditLogListResponse> {
  const { data REDACTED = await apiClient.get('/admin/audit-logs', { params REDACTED)
  return data
REDACTED
...
export async function get(id: number): Promise<AuditLog> {
  const { data REDACTED = await apiClient.get(`/admin/audit-logs/${idREDACTED`)
  return data
REDACTED
 ...
 */
export async function clear(totpCode: string): Promise<{ deleted: number REDACTED> {
  const { data REDACTED = await apiClient.post('/admin/audit-logs/clear', { totp_code: totpCode REDACTED)
  return data
REDACTED
