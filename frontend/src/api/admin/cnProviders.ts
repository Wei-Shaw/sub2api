...
export async function queryQuota(id: number): Promise<CNProviderQuotaProbeResult> {
  const { data REDACTED = await apiClient.get<CNProviderQuotaProbeResult>(
    `/admin/cn-providers/accounts/${idREDACTED/quota`
  )
  return data
REDACTED
...
export async function queryBalance(id: number): Promise<CNProviderBalanceResult> {
  const { data REDACTED = await apiClient.get<CNProviderBalanceResult>(
    `/admin/cn-providers/accounts/${idREDACTED/balance`
  )
  return data
REDACTED
