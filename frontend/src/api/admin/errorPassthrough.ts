...
export async function list(): Promise<ErrorPassthroughRule[]> {
  const { data REDACTED = await apiClient.get<ErrorPassthroughRule[]>('/admin/error-passthrough-rules')
  return data
REDACTED
...
export async function getById(id: number): Promise<ErrorPassthroughRule> {
  const { data REDACTED = await apiClient.get<ErrorPassthroughRule>(`/admin/error-passthrough-rules/${idREDACTED`)
  return data
REDACTED
...
export async function create(ruleData: CreateRuleRequest): Promise<ErrorPassthroughRule> {
  const { data REDACTED = await apiClient.post<ErrorPassthroughRule>('/admin/error-passthrough-rules', ruleData)
  return data
REDACTED
...
export async function update(id: number, updates: UpdateRuleRequest): Promise<ErrorPassthroughRule> {
  const { data REDACTED = await apiClient.put<ErrorPassthroughRule>(`/admin/error-passthrough-rules/${idREDACTED`, updates)
  return data
REDACTED
 ...
 */
export async function deleteRule(id: number): Promise<{ message: string REDACTED> {
  const { data REDACTED = await apiClient.delete<{ message: string REDACTED>(`/admin/error-passthrough-rules/${idREDACTED`)
  return data
REDACTED
...
export async function toggleEnabled(id: number, enabled: boolean): Promise<ErrorPassthroughRule> {
  return update(id, { enabled REDACTED)
REDACTED
