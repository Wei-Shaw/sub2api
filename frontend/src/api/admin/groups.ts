...
function duplicateOperationScope(id: number): DuplicateOperationScope | null {
  ...
  return {
    adminID,
    key: `sub2api:admin:group-duplicate:${adminIDREDACTED:${idREDACTED`
  REDACTED
REDACTED
...
export async function duplicate(id: number): Promise<AdminGroup> {
  ...
  if (!idempotencyKey) {
    ...
    idempotencyKey = `group-duplicate-${scope?.adminID ?? 'unknown-admin'REDACTED-${idREDACTED-${requestIDREDACTED`
  REDACTED
  if (scope) {
  ...
  const { data REDACTED = await apiClient.post<AdminGroup>(`/admin/groups/${idREDACTED/duplicate`, undefined, {
    headers: { 'Idempotency-Key': idempotencyKey REDACTED
  REDACTED)
...
export async function listCompositeRoutes(id: number): Promise<CompositeModelRoute[]> {
  const { data REDACTED = await apiClient.get<CompositeModelRoute[]>(`/admin/groups/${idREDACTED/composite-routes`)
  return data
REDACTED
...
export async function createCompositeRoute(
  ...
  const { data REDACTED = await apiClient.post<CompositeModelRoute>(
    `/admin/groups/${idREDACTED/composite-routes`,
    route
  )
  return data
REDACTED
...
export async function updateCompositeRoute(
  ...
  const { data REDACTED = await apiClient.put<CompositeModelRoute>(
    `/admin/groups/${idREDACTED/composite-routes/${routeIdREDACTED`,
    route
  )
  return data
REDACTED
...
export async function previewCompositeRoute(
  ...
  const { data REDACTED = await apiClient.post<CompositeRouteDecision>(
    `/admin/groups/${idREDACTED/composite-routes/preview`,
    request
  )
  return data
REDACTED
...
export async function getUsageSummary(): Promise<
  { group_id: number; today_cost: number; yesterday_cost: number; total_cost: number REDACTED[]
> {
  const { data REDACTED = await apiClient.get<
    { group_id: number; today_cost: number; yesterday_cost: number; total_cost: number REDACTED[]
