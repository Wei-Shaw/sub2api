...
export async function getConcurrencyStats(platform?: string, groupId?: number | null): Promise<OpsConcurrencyStatsResponse> {
  const params: Record<string, any> = {REDACTED
  if (platform) {
  ...
  const { data REDACTED = await apiClient.get<OpsConcurrencyStatsResponse>('/admin/ops/concurrency', { params REDACTED)
  return data
REDACTED

export async function getUserConcurrencyStats(): Promise<OpsUserConcurrencyStatsResponse> {
  const { data REDACTED = await apiClient.get<OpsUserConcurrencyStatsResponse>('/admin/ops/user-concurrency')
  return data
REDACTED
...
export async function getAccountAvailabilityStats(platform?: string, groupId?: number | null): Promise<OpsAccountAvailabilityStatsResponse> {
  ...
  if (typeof groupId === 'number' && groupId > 0) {
    params.group_id = groupId
  REDACTED
  const { data REDACTED = await apiClient.get<OpsAccountAvailabilityStatsResponse>('/admin/ops/account-availability', { params REDACTED)
  return data
REDACTED
...
export interface SubscribeQPSOptions {
   ...
   */
  onReconnectScheduled?: (info: { attempt: number, delayMs: number REDACTED) => void
  wsBaseUrl?: string
  /**
...
export function subscribeQPS(onMessage: (data: any) => void, options: SubscribeQPSOptions = {REDACTED): () => void {
  ...
  const scheduleReconnect = () => {
    ...
    reconnectTimer = setTimeout(() => {
      reconnectAttempts++
      connect()
    REDACTED, delay + jitter)
    options.onReconnectScheduled?.({ attempt: reconnectAttempts + 1, delayMs: delay + jitter REDACTED)
  REDACTED
  ...
  const connect = () => {
    ...
    if (rawToken) protocols.push(`jwt.${rawTokenREDACTED`)
...
export async function getDashboardSnapshotV2(
  ...
  const { data REDACTED = await apiClient.get<OpsDashboardSnapshotV2Response>('/admin/ops/dashboard/snapshot-v2', {
    params,
    signal: options.signal
  REDACTED)
  return data
...
export async function getThroughputTrend(
  ...
  const { data REDACTED = await apiClient.get<OpsThroughputTrendResponse>('/admin/ops/dashboard/throughput-trend', {
    params,
    signal: options.signal
  REDACTED)
  return data
...
export async function getLatencyHistogram(
  ...
  const { data REDACTED = await apiClient.get<OpsLatencyHistogramResponse>('/admin/ops/dashboard/latency-histogram', {
    params,
    signal: options.signal
  REDACTED)
  return data
...
export async function getErrorTrend(
  ...
  const { data REDACTED = await apiClient.get<OpsErrorTrendResponse>('/admin/ops/dashboard/error-trend', {
    params,
    signal: options.signal
  REDACTED)
  return data
...
export async function listErrorLogs(params: OpsErrorListQueryParams): Promise<OpsErrorLogsResponse> {
  const { data REDACTED = await apiClient.get<OpsErrorLogsResponse>('/admin/ops/errors', { params REDACTED)
  return data
REDACTED

export async function getErrorLogDetail(id: number): Promise<OpsErrorDetail> {
  const { data REDACTED = await apiClient.get<OpsErrorDetail>(`/admin/ops/errors/${idREDACTED`)
  return data
REDACTED

export async function retryErrorRequest(id: number, req: OpsRetryRequest): Promise<OpsRetryResult> {
  const { data REDACTED = await apiClient.post<OpsRetryResult>(`/admin/ops/errors/${idREDACTED/retry`, req)
  return data
REDACTED

export async function listRetryAttempts(errorId: number, limit = 50): Promise<OpsRetryAttempt[]> {
  const { data REDACTED = await apiClient.get<OpsRetryAttempt[]>(`/admin/ops/errors/${errorIdREDACTED/retries`, { params: { limit REDACTED REDACTED)
  return data
REDACTED

export async function updateErrorResolved(errorId: number, resolved: boolean): Promise<void> {
  await apiClient.put(`/admin/ops/errors/${errorIdREDACTED/resolve`, { resolved REDACTED)
REDACTED

// New split endpoints
export async function listRequestErrors(params: OpsErrorListQueryParams): Promise<OpsErrorLogsResponse> {
  const { data REDACTED = await apiClient.get<OpsErrorLogsResponse>('/admin/ops/request-errors', { params REDACTED)
  return data
REDACTED

export async function listUpstreamErrors(params: OpsErrorListQueryParams): Promise<OpsErrorLogsResponse> {
  const { data REDACTED = await apiClient.get<OpsErrorLogsResponse>('/admin/ops/upstream-errors', { params REDACTED)
  return data
REDACTED

export async function getRequestErrorDetail(id: number): Promise<OpsErrorDetail> {
  const { data REDACTED = await apiClient.get<OpsErrorDetail>(`/admin/ops/request-errors/${idREDACTED`)
  return data
REDACTED

export async function getUpstreamErrorDetail(id: number): Promise<OpsErrorDetail> {
  const { data REDACTED = await apiClient.get<OpsErrorDetail>(`/admin/ops/upstream-errors/${idREDACTED`)
  return data
REDACTED

export async function retryRequestErrorClient(id: number): Promise<OpsRetryResult> {
  const { data REDACTED = await apiClient.post<OpsRetryResult>(`/admin/ops/request-errors/${idREDACTED/retry-client`, {REDACTED)
  return data
REDACTED

export async function retryRequestErrorUpstreamEvent(id: number, idx: number): Promise<OpsRetryResult> {
  const { data REDACTED = await apiClient.post<OpsRetryResult>(`/admin/ops/request-errors/${idREDACTED/upstream-errors/${idxREDACTED/retry`, {REDACTED)
  return data
REDACTED

export async function retryUpstreamError(id: number): Promise<OpsRetryResult> {
  const { data REDACTED = await apiClient.post<OpsRetryResult>(`/admin/ops/upstream-errors/${idREDACTED/retry`, {REDACTED)
  return data
REDACTED

export async function updateRequestErrorResolved(errorId: number, resolved: boolean): Promise<void> {
  await apiClient.put(`/admin/ops/request-errors/${errorIdREDACTED/resolve`, { resolved REDACTED)
REDACTED

export async function updateUpstreamErrorResolved(errorId: number, resolved: boolean): Promise<void> {
  await apiClient.put(`/admin/ops/upstream-errors/${errorIdREDACTED/resolve`, { resolved REDACTED)
REDACTED

export async function listRequestErrorUpstreamErrors(
  id: number,
  params: OpsErrorListQueryParams = {REDACTED,
  options: { include_detail?: boolean REDACTED = {REDACTED
): Promise<PaginatedResponse<OpsErrorDetail>> {
  const query: Record<string, any> = { ...params REDACTED
  if (options.include_detail) query.include_detail = '1'
  const { data REDACTED = await apiClient.get<PaginatedResponse<OpsErrorDetail>>(`/admin/ops/request-errors/${idREDACTED/upstream-errors`, { params: query REDACTED)
  return data
REDACTED

export async function listRequestDetails(params: OpsRequestDetailsParams): Promise<OpsRequestDetailsResponse> {
  const { data REDACTED = await apiClient.get<OpsRequestDetailsResponse>('/admin/ops/requests', { params REDACTED)
  return data
REDACTED

// Alert rules
export async function listAlertRules(): Promise<AlertRule[]> {
  const { data REDACTED = await apiClient.get<AlertRule[]>('/admin/ops/alert-rules')
  return data
REDACTED

export async function createAlertRule(rule: AlertRule): Promise<AlertRule> {
  const { data REDACTED = await apiClient.post<AlertRule>('/admin/ops/alert-rules', rule)
  return data
REDACTED

export async function updateAlertRule(id: number, rule: Partial<AlertRule>): Promise<AlertRule> {
  const { data REDACTED = await apiClient.put<AlertRule>(`/admin/ops/alert-rules/${idREDACTED`, rule)
  return data
REDACTED

export async function deleteAlertRule(id: number): Promise<void> {
  await apiClient.delete(`/admin/ops/alert-rules/${idREDACTED`)
REDACTED
...
export async function listAlertEvents(params: AlertEventsQuery = {REDACTED): Promise<AlertEvent[]> {
  const { data REDACTED = await apiClient.get<AlertEvent[]>('/admin/ops/alert-events', { params REDACTED)
  return data
REDACTED

export async function getAlertEvent(id: number): Promise<AlertEvent> {
  const { data REDACTED = await apiClient.get<AlertEvent>(`/admin/ops/alert-events/${idREDACTED`)
  return data
REDACTED

export async function updateAlertEventStatus(id: number, status: 'resolved' | 'manual_resolved'): Promise<void> {
  await apiClient.put(`/admin/ops/alert-events/${idREDACTED/status`, { status REDACTED)
REDACTED
...
export async function getEmailNotificationConfig(): Promise<EmailNotificationConfig> {
  const { data REDACTED = await apiClient.get<EmailNotificationConfig>('/admin/ops/email-notification/config')
  return data
REDACTED

export async function updateEmailNotificationConfig(config: EmailNotificationConfig): Promise<EmailNotificationConfig> {
  const { data REDACTED = await apiClient.put<EmailNotificationConfig>('/admin/ops/email-notification/config', config)
  return data
REDACTED

// Runtime settings (DB-backed)
export async function getAlertRuntimeSettings(): Promise<OpsAlertRuntimeSettings> {
  const { data REDACTED = await apiClient.get<OpsAlertRuntimeSettings>('/admin/ops/runtime/alert')
  return data
REDACTED

export async function updateAlertRuntimeSettings(config: OpsAlertRuntimeSettings): Promise<OpsAlertRuntimeSettings> {
  const { data REDACTED = await apiClient.put<OpsAlertRuntimeSettings>('/admin/ops/runtime/alert', config)
  return data
REDACTED
...
export async function updateAdvancedSettings(config: OpsAdvancedSettings): Promise<OpsAdvancedSettings> {
  const { data REDACTED = await apiClient.put<OpsAdvancedSettings>('/admin/ops/advanced-settings', config)
  return data
REDACTED
