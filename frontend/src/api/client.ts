...
import { ADMIN_UI_REQUEST_HEADER, shouldMarkAdminUIRequest REDACTED from './adminUIRequest'
...
apiClient.interceptors.response.use(
  ...
  async (error: AxiosError<ApiResponse<unknown>>) => {
    ...
    const originalRequest = error.config as InternalAxiosRequestConfig & { _retry?: boolean REDACTED
    ...
    if (error.response) {
      ...
      const apiData = (typeof data === 'object' && data !== null ? data : {REDACTED) as Record<string, any>
      ...
      if (status === 423 && apiData.code === 'ADMIN_COMPLIANCE_ACK_REQUIRED') {
        try {
          window.dispatchEvent(new CustomEvent('admin-compliance-required', {
            detail: apiData.metadata || {REDACTED
          REDACTED))
        REDACTED catch {
          // ignore event failures
        REDACTED
        ...
        if (refreshToken && !isAuthEndpoint) {
              ...
              subscribeTokenRefresh((newToken: string) => {
                if (newToken) {
                  ...
                  if (originalRequest.headers) {
                    originalRequest.headers.Authorization = `Bearer ${newTokenREDACTED`
                  REDACTED
          ...
          try {
            ...
            const refreshResponse = await axios.post(
              ...
              { headers: { 'Content-Type': 'application/json' REDACTED, timeout: 30000 REDACTED
            )
            ...
            if (refreshData.code === 0 && refreshData.data) {
              const { access_token, refresh_token: newRefreshToken, expires_in REDACTED = refreshData.data
              ...
              if (originalRequest.headers) {
                originalRequest.headers.Authorization = `Bearer ${access_tokenREDACTED`
              REDACTED
