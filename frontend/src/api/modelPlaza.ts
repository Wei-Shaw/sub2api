...
import type { UserPricingInterval, UserSupportedModelPricing REDACTED from './channels'
...
export async function getModelPlaza(options?: { signal?: AbortSignal REDACTED): Promise<ModelPlazaResponse> {
  const { data REDACTED = await apiClient.get<ModelPlazaResponse>('/model-plaza', {
    signal: options?.signal
  REDACTED)
  return data
REDACTED

export const modelPlazaAPI = { getModelPlaza REDACTED
