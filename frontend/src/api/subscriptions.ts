...
import type { UserSubscription, SubscriptionProgress REDACTED from '@/types';
...
export async function getSubscriptionProgress(subscriptionId: number): Promise<SubscriptionProgress> {
  const response = await apiClient.get<SubscriptionProgress>(`/subscriptions/${subscriptionIdREDACTED/progress`);
  return response.data;
REDACTED
