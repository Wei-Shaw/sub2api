<template>
  <div v-if="sceneId && prefix" class="aliyun-captcha-wrapper">
    ...
    >
      ...
      <span>{{ buttonText REDACTEDREDACTED</span>
    </button>
...
interface AliyunCaptchaInitOptions {
  ...
  slideStyle?: { width: number; height: number REDACTED
  language?: string
REDACTED
...
const buttonId = `aliyun-captcha-button-${uidREDACTED`
const elementId = `aliyun-captcha-element-${uidREDACTED`
...
let pending: { resolve: (value: string | null) => void REDACTED | null = null
...
const loadScript = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    window.AliyunCaptchaConfig = { region: props.region, prefix: props.prefix REDACTED
...
function initCaptcha(): void {
  ...
  window.initAliyunCaptcha({
    ...
    prefix: props.prefix,
    mode: 'popup',
    element: `#${elementIdREDACTED`,
    button: `#${buttonIdREDACTED`,
    ...
    captchaVerifyCallback: (captchaVerifyParam: string) => {
      ...
      return { captchaResult: true REDACTED
    REDACTED,
    onBizResultCallback: () => {REDACTED,
    getInstance: () => {REDACTED,
    slideStyle: { width: 360, height: 40 REDACTED,
...
function startPopupWatch(): void {
  ...
  REDACTED, POPUP_WATCH_INTERVAL_MS)
REDACTED
