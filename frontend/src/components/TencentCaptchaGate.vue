...
import { computed, nextTick, onBeforeUnmount, onMounted, ref REDACTED from 'vue'
...
const props = withDefaults(defineProps<{ appId: string; region?: string REDACTED>(), { region: 'cn' REDACTED)
    ...
    void nextTick()
      ...
      .then((TencentCaptcha) => {
        ...
        const handleResult = (result: TencentCaptchaResult): void => {
          ...
          finish(() => resolve({ ticket, randstr REDACTED), region === 'intl')
        REDACTED
        ...
        if (region === 'intl') {
          ...
          REDACTED)
        REDACTED else {
          instance = new TencentCaptcha(props.appId, handleResult, { userLanguage REDACTED)
        REDACTED
        instance.show()
      REDACTED)
