...
interface CapturedInitOptions {
  ...
  element: string
  button: string
  captchaVerifyCallback: (param: string) => { captchaResult: boolean REDACTED
  language?: string
REDACTED

const i18nStub = {
  install(app: { config: { globalProperties: Record<string, unknown> REDACTED REDACTED) {
    app.config.globalProperties.$t = (key: string) => key
  REDACTED
REDACTED

vi.mock('vue-i18n', () => ({
  useI18n: () => ({ locale: { value: 'zh-CN' REDACTED, t: (key: string) => key REDACTED)
REDACTED))
...
describe('AliyunCaptchaWidget', () => {
  ...
  beforeEach(() => {
    ...
    window.initAliyunCaptcha = vi.fn((options: CapturedInitOptions) => {
      initOptions = options
    REDACTED) as unknown as typeof window.initAliyunCaptcha
  REDACTED)
  ...
  function mountWidget() {
    return mount(AliyunCaptchaWidget, {
      props: { sceneId: 'scene-1', prefix: 'prefix-1', region: 'cn' as const REDACTED,
      attachTo: document.body,
      global: { plugins: [i18nStub] REDACTED
    REDACTED)
  REDACTED
  ...
  it('渲染可见验证按钮并以 popup 模式初始化，全局配置就位', async () => {
    ...
    expect(window.AliyunCaptchaConfig).toEqual({ region: 'cn', prefix: 'prefix-1' REDACTED)
  ...
  it('用户点击按钮进入验证中，验证完成后 emit verify 并置已通过', async () => {
    ...
    expect(result).toEqual({ captchaResult: true REDACTED)
  ...
  it('verify() 在已预验证时直接复用缓存的 captchaVerifyParam', async () => {
    ...
    const vm = wrapper.vm as unknown as { verify: () => Promise<string | null> REDACTED
...
declare global {
  interface Window {
    ...
    AliyunCaptchaConfig?: { region: string; prefix: string REDACTED
  REDACTED
REDACTED
