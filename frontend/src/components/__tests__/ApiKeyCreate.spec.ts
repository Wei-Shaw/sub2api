...
import { defineComponent, ref, reactive REDACTED from 'vue'
...
const ApiKeyCreateTestComponent = defineComponent({
  setup() {
    ...
    return { formData, loading, createdKey, handleCreate REDACTED
  REDACTED,
  template: `
    <div>
      ...
      <div v-if="createdKey" class="created-key">{{ createdKey REDACTEDREDACTED</div>
    </div>
  `,
REDACTED)
...
describe('ApiKey 创建流程', () => {
  ...
  it('创建过程中按钮被禁用', async () => {
    ...
    mockCreate.mockImplementation(
      () => new Promise((resolve) => { resolveCreate = resolve REDACTED)
    )
    ...
    resolveCreate!({ id: 1, key: 'sk-test', name: 'Test Key' REDACTED)
    await flushPromises()
