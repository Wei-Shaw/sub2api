...
const sessionLimitTooltip = computed(() => {
  ...
  if (current >= max) return t('admin.accounts.capacity.sessions.full', { idle REDACTED)
...
const rpmTooltip = computed(() => {
  ...
  const buffer = rpmBuffer.value
  if (rpmStrategy.value === 'tiered') {
    if (current >= base + buffer) return t('admin.accounts.capacity.rpm.tieredBlocked', { buffer REDACTED)
    if (current >= base) return t('admin.accounts.capacity.rpm.tieredStickyOnly', { buffer REDACTED)
