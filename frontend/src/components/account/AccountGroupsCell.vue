<template>
  <div v-if="groups && groups.length > 0" class="relative max-w-56">
    ...
    <div class="flex flex-wrap gap-1 max-h-14 overflow-hidden">
      ...
      >
        <span>+{{ hiddenCount REDACTEDREDACTED</span>
      </button>
    </div>
        ...
        >
          <div class="mb-2 flex items-center justify-between">
            <span class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.groupCountTotal', { count: groups.length REDACTED) REDACTEDREDACTED
            </span>
            <button
...
const popoverStyle = computed(() => {
  if (!moreButtonRef.value) return {REDACTED
