<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="isRateLimited" class="flex flex-col items-center gap-1">
      <span class="badge text-xs badge-warning">{{ t('admin.accounts.status.rateLimited') REDACTEDREDACTED</span>
      <span class="text-[11px] text-gray-400 dark:text-gray-500">{{ rateLimitResumeText REDACTEDREDACTED</span>
    </div>

    <!-- Overload Display (529) - Two-line layout -->
    <div v-else-if="isOverloaded" class="flex flex-col items-center gap-1">
      <span class="badge text-xs badge-danger">{{ t('admin.accounts.status.overloaded') REDACTEDREDACTED</span>
      <span class="text-[11px] text-gray-400 dark:text-gray-500">{{ overloadCountdown REDACTEDREDACTED</span>
    </div>
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      ...
      >
        <div class="whitespace-pre-wrap break-words leading-relaxed text-gray-300">
          {{ account.error_message REDACTEDREDACTED
        </div>
        <!-- 上方小三角 -->
        <div
    ...
    <div v-if="isRateLimited" class="group relative">
      ...
      >
        {{ t('admin.accounts.status.rateLimitedUntil', { time: formatDateTime(account.rate_limit_reset_at) REDACTED) REDACTEDREDACTED
        <div
    ...
    >
      <div v-for="item in activeModelStatuses" :key="`${item.kindREDACTED-${item.modelREDACTED`" class="group relative mb-1 break-inside-avoid">
        <!-- 积分已用尽 -->
        <span
    ...
    <div v-if="isOverloaded" class="group relative">
      ...
      <div
        class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-56 -translate-x-1/2 whitespace-normal rounded bg-gray-900 px-3 py-2 text-center text-xs leading-relaxed text-white opacity-0 transition-opacity group-hover:opacity-100 dark:bg-gray-700"
      >
        {{ t('admin.accounts.status.overloadedUntil', { time: formatTime(account.overload_until) REDACTED) REDACTEDREDACTED
        <div
          class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-gray-900 dark:border-t-gray-700"
        ></div>
      </div>
...
const activeModelStatuses = computed<AccountModelStatusItem[]>(() => {
  ...
  const modelLimits = extra?.model_rate_limits as
    | Record<string, { rate_limited_at: string; rate_limit_reset_at: string REDACTED>
    | undefined
  const now = new Date()
  ...
  for (const [model, info] of Object.entries(modelLimits)) {
    ...
    if (model === 'AICredits') {
      // AICredits key → 积分已用尽
      items.push({ kind: 'credits_exhausted', model, reset_at: info.rate_limit_reset_at REDACTED)
    REDACTED else if (allowOverages && !hasActiveAICredits) {
      // 普通模型限流 + overages 启用 + 积分可用 → 正在走积分
      items.push({ kind: 'credits_active', model, reset_at: info.rate_limit_reset_at REDACTED)
    REDACTED else {
      // 普通模型限流
      items.push({ kind: 'rate_limit', model, reset_at: info.rate_limit_reset_at REDACTED)
    REDACTED
  REDACTED
...
const rateLimitResumeText = computed(() => {
  ...
  return t('admin.accounts.status.rateLimitedAutoResume', { time: rateLimitCountdown.value REDACTED)
REDACTED)
