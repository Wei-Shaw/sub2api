...
const activeModelStatuses = computed<AccountModelStatusItem[]>(() => {
  ...
  if (modelLimits) {
    items.push(...Object.entries(modelLimits)
      ...
      .map(([model, info]) => ({ kind: 'rate_limit' as const, model, reset_at: info.rate_limit_reset_at REDACTED)))
  REDACTED

  const overagesStates = extra?.antigravity_credits_overages as
    | Record<string, { activated_at?: string; active_until: string REDACTED>
    | undefined
  if (overagesStates) {
    items.push(...Object.entries(overagesStates)
      .filter(([, info]) => new Date(info.active_until) > now)
      .map(([model, info]) => ({ kind: 'overages' as const, model, reset_at: info.active_until REDACTED)))
  REDACTED
