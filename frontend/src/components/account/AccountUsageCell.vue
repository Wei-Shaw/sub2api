  ...
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    <template v-else-if="account.platform === 'openai' && account.type === 'oauth'">
      <div v-if="hasOpenAIUsageFallback" class="space-y-1">
        ...
        -->
        <OpenAIQuotaResetCell v-if="showActiveQuery" :account="account">
    ...
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <GrokQuotaProbeCell v-if="showActiveQuery" :account="account" @probed="handleGrokProbed" />
      </div>
      <div v-else class="text-xs text-gray-400">-</div>
    </template>
...
const handleGrokProbed = (result: GrokQuotaProbeResult) => {
  ...
  _usageCache.set(usageCacheKey(props.account.id, defaultUsageSource.value), { data: merged, ts: Date.now() })
}
