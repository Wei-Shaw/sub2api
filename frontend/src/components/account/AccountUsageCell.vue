  ...
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    >
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <div v-if="usageInfo.error" class="text-xs text-amber-600 dark:text-amber-400 truncate max-w-[200px]" :title="usageInfo.error">
          {{ usageInfo.error REDACTEDREDACTED
        </div>
        <!-- 5h Window -->
        <UsageProgressBar
        ...
        <div class="flex items-center gap-1.5 mt-0.5">
          ...
          >
            {{ t('admin.accounts.usageWindow.passiveSampled') REDACTEDREDACTED
          </span>
          <button
            type="button"
          ...
          >
            <svg
              class="h-2.5 w-2.5"
              :class="{ 'animate-spin': activeQueryLoading REDACTED"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            {{ t('admin.accounts.usageWindow.activeQuery') REDACTEDREDACTED
          </button>
        </div>
      </div>
    ...
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-if="antigravityTierLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          {{ antigravityTierLabel REDACTEDREDACTED
        </span>
        <!-- 不合格账户警告图标 -->
        <span
        ...
        >
          ...
          >
            {{ t('admin.accounts.ineligibleWarning') REDACTEDREDACTED
          </span>
        </span>
      </div>
      ...
      <div v-if="isForbidden" class="space-y-1">
        ...
        >
          {{ forbiddenLabel REDACTEDREDACTED
        </span>
        <div v-if="validationURL" class="flex items-center gap-1">
          ...
          >
            {{ t('admin.accounts.openVerification') REDACTEDREDACTED
          </a>
          <button
            type="button"
          ...
          >
            {{ linkCopied ? t('admin.accounts.linkCopied') : t('admin.accounts.copyLink') REDACTEDREDACTED
          </button>
        </div>
      </div>
      ...
      <div v-else-if="needsReauth" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300">
          {{ t('admin.accounts.needsReauth') REDACTEDREDACTED
        </span>
      </div>
      ...
      <div v-else-if="usageInfo?.error" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
          {{ usageErrorLabel REDACTEDREDACTED
        </span>
      </div>
      ...
      <div v-else-if="hasAntigravityQuotaFromAPI" class="space-y-1">
        ...
        <div v-if="aiCreditsDisplay" class="mt-1 text-[10px] text-gray-500 dark:text-gray-400">
          💳 {{ t('admin.accounts.aiCreditsBalance') REDACTEDREDACTED: {{ aiCreditsDisplay REDACTEDREDACTED
        </div>
      </div>
    ...
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="isForbidden" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300">
          {{ grokEntitlementLabel || t('admin.accounts.forbidden') REDACTEDREDACTED
        </span>
      </div>
      <div v-else-if="usageInfo" class="space-y-1">
        <div v-if="grokEntitlementLabel" class="mb-0.5">
          <span class="inline-block rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] font-medium text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200">
            {{ grokEntitlementLabel REDACTEDREDACTED
          </span>
        </div>
        <div v-if="showGrokStandaloneLocalStats && grokLocalUsage" class="mb-0.5 flex items-center">
          <div class="flex items-center gap-1.5 text-[9px] text-gray-500 dark:text-gray-400">
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatWindowRequests(grokLocalUsage) REDACTEDREDACTED req
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatWindowTokens(grokLocalUsage) REDACTEDREDACTED
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800" :title="t('usage.accountBilled')">
              A ${{ formatWindowCost(grokLocalUsage) REDACTEDREDACTED
            </span>
            <span
            ...
            >
              U ${{ formatWindowUserCost(grokLocalUsage) REDACTEDREDACTED
            </span>
          </div>
        </div>
        <UsageProgressBar
        ...
        >
          ...
          >
            {{ t('admin.accounts.usageWindow.grokPrepaid') REDACTEDREDACTED ${{ grokBillingMoneySummary.prepaid REDACTEDREDACTED
          </span>
          <span :title="t('admin.accounts.usageWindow.grokMonthlyLimit')">
            {{ t('admin.accounts.usageWindow.grokUsed') REDACTEDREDACTED
            {{ grokBillingMoneySummary.used REDACTEDREDACTED/{{ grokBillingMoneySummary.limit REDACTEDREDACTED
          </span>
          <span
            v-if="grokBillingMoneySummary.usedPercent != null"
            class="rounded bg-gray-100 px-1 py-0.5 dark:bg-gray-800"
          >
            {{ grokBillingMoneySummary.usedPercent REDACTEDREDACTED%
          </span>
          <span
          ...
          >
            {{ t('admin.accounts.usageWindow.grokOverageShort') REDACTEDREDACTED
            {{ grokBillingMoneySummary.onDemandUsed REDACTEDREDACTED/{{ grokBillingMoneySummary.onDemandCap REDACTEDREDACTED
          </span>
        </div>
        <UsageProgressBar
        ...
        <UsageProgressBar
          ...
          label="24h"
          :title="t('admin.accounts.usageWindow.grokFreeQuota24hHint', { limit: formatCompactNumber(grokFreeTokenBar.limit) REDACTED)"
          :utilization="grokFreeTokenBar.utilization"
          :show-now-when-idle="true"
          color="emerald"
        />
        <div v-if="grokRetryAfterLabel" class="text-[10px] text-amber-600 dark:text-amber-400">
          {{ t('admin.accounts.usageWindow.grokRetryAfter', { time: grokRetryAfterLabel REDACTED) REDACTEDREDACTED
        </div>
        <div v-if="grokQuotaUnknown" class="text-[10px] text-gray-500 dark:text-gray-400">
          {{ grokQuotaUnknownLabel REDACTEDREDACTED
        </div>
        ...
        <div v-if="grokQuotaStatusLine" class="text-[10px] text-gray-500 dark:text-gray-400">
          {{ grokQuotaStatusLine REDACTEDREDACTED
        </div>
    ...
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-if="geminiAuthTypeLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          {{ geminiAuthTypeLabel REDACTEDREDACTED
        </span>
        <!-- Help icon -->
        <span
          class="group relative cursor-help"
        >
          ...
          <span
            class="pointer-events-none absolute left-0 top-full z-50 mt-1 w-80 whitespace-normal break-words rounded bg-gray-900 px-3 py-2 text-xs leading-relaxed text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 dark:bg-gray-700"
          >
            <div class="font-semibold mb-1">{{ t('admin.accounts.gemini.quotaPolicy.title') REDACTEDREDACTED</div>
            <div class="mb-2 text-gray-300">{{ t('admin.accounts.gemini.quotaPolicy.note') REDACTEDREDACTED</div>
            <div class="space-y-1">
              <div><strong>{{ geminiQuotaPolicyChannel REDACTEDREDACTED:</strong></div>
              <div class="pl-2">• {{ geminiQuotaPolicyLimits REDACTEDREDACTED</div>
              <div class="mt-2">
      ...
      <div class="space-y-1">
        ...
        >
          <div class="flex items-center gap-1.5 text-[9px] text-gray-500 dark:text-gray-400">
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatKeyRequests REDACTEDREDACTED req
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatKeyTokens REDACTEDREDACTED
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800" :title="t('usage.accountBilled')">
              A ${{ formatKeyCost REDACTEDREDACTED
            </span>
            <span
              ...
              :title="t('usage.userBilled')"
            >
              U ${{ formatKeyUserCost REDACTEDREDACTED
            </span>
          </div>
        </div>
        <div
        ...
        <div v-else-if="geminiUsageAvailable" class="space-y-1">
          ...
          <p class="mt-1 text-[9px] leading-tight text-gray-400 dark:text-gray-500 italic">
            * {{ t('admin.accounts.gemini.quotaPolicy.simulatedNote') || 'Simulated quota' REDACTEDREDACTED
          </p>
        </div>
...
import type { GrokQuotaProbeResult REDACTED from '@/api/admin/grok'
...
import { formatCompactNumber, formatRelativeTime REDACTED from '@/utils/format'
...
const props = withDefaults(
  defineProps<{
    ...
    requestBatchedUsage?: ((account: Account, options?: { force?: boolean REDACTED) => void) | null
  REDACTED>(),
  {
    todayStats: null,
...
const geminiAuthTypeLabel = computed(() => {
  ...
  return geminiUserLevel.value ? `${geminiChannelShort.valueREDACTED ${geminiUserLevel.valueREDACTED` : geminiChannelShort.value
REDACTED)
...
const makeGrokQuotaBar = (quota?: { limit?: number | null; remaining?: number | null; reset_at?: string | null REDACTED | null): GrokQuotaBarInfo | null => {
...
const grokBillingMoneySummary = computed(() => {
  ...
  REDACTED else if (limit != null && limit > 0 && used != null) {
...
const grokFreeTokenBar = computed(() => {
  ...
  return { utilization: Math.min(100, (used / limit) * 100), limit REDACTED
REDACTED)
...
const grokQuotaStatusLine = computed(() => {
  ...
  if (status) {
    parts.push(t('admin.accounts.usageWindow.grokLastStatus', { status REDACTED))
  REDACTED
...
const grokRetryAfterLabel = computed(() => {
  ...
  return `${minutesREDACTEDm`
REDACTED)

const formatWindowRequests = (stats: WindowStats) => formatCompactNumber(stats.requests, { allowBillions: false REDACTED)
...
const copyValidationURL = async () => {
  ...
  try {
    ...
    setTimeout(() => { linkCopied.value = false REDACTED, 2000)
  REDACTED catch {
    // fallback: ignore
  REDACTED
REDACTED
...
const requestParentBatchUsage = (options?: { force?: boolean REDACTED) => {
...
const loadUsage = async (options?: { source?: 'passive' | 'active'; bypassCache?: boolean REDACTED) => {
  ...
  if (isBatchManaged.value) {
    requestParentBatchUsage({ force: options?.bypassCache === true REDACTED)
    return
  REDACTED
...
const handleGrokProbed = (result: GrokQuotaProbeResult) => {
  ...
  _usageCache.set(props.account.id, { data: merged, ts: Date.now() REDACTED)
REDACTED
...
const makeQuotaBar = (
  ...
  if (startKey) {
    ...
    REDACTED
  REDACTED
  return { utilization, resetsAt REDACTED
REDACTED
...
const formatKeyRequests = computed(() => {
  ...
  return formatCompactNumber(props.todayStats.requests, { allowBillions: false REDACTED)
REDACTED)
...
watch(openAIUsageRefreshKey, (nextKey, prevKey) => {
  ...
  if (isBatchManaged.value) {
    requestParentBatchUsage({ force: true REDACTED)
    return
  REDACTED
