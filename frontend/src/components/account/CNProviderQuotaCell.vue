  ...
  <div v-if="visible" class="space-y-1">
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      >
        ...
        {{ t('admin.accounts.cnProviders.window5h') REDACTEDREDACTED/{{ t('admin.accounts.cnProviders.windowWeekly') REDACTEDREDACTED
      </button>
    </div>
    ...
    <div v-if="data?.success && data.tiers?.length" class="space-y-1">
      <div v-for="tier in data.tiers" :key="tier.window" class="flex items-center gap-1.5 text-[10px]">
        <span class="w-10 shrink-0 text-gray-500 dark:text-gray-400">{{ windowLabel(tier.window) REDACTEDREDACTED</span>
...
import type { CNProviderQuotaProbeResult REDACTED from '@/api/admin/cnProviders'
import type { Account REDACTED from '@/types'
import { platformTextClass REDACTED from '@/utils/platformColors'
import { cnQuotaCellVisible REDACTED from './credentialsBuilder'
...
const snapshotData = computed<CNProviderQuotaProbeResult | null>(() => {
  ...
  const used5h = readExtraNumber(`${platformREDACTED_5h_used_percent`)
  const usedWeekly = readExtraNumber(`${platformREDACTED_weekly_used_percent`)
  if (used5h == null && usedWeekly == null) return null
  const tiers: CNProviderQuotaProbeResult['tiers'] = []
  if (used5h != null) {
    tiers.push({ window: '5h', used_percent: used5h, reset_at: readExtraString(`${platformREDACTED_5h_reset_at`) || undefined REDACTED)
  REDACTED
  if (usedWeekly != null) {
    tiers.push({ window: 'weekly', used_percent: usedWeekly, reset_at: readExtraString(`${platformREDACTED_weekly_reset_at`) || undefined REDACTED)
  REDACTED
  return { success: true, tiers REDACTED as CNProviderQuotaProbeResult
REDACTED)

// 快照是否过期（无更新时间或超过 staleness 窗口）→ 挂载时需要自动探测。
const snapshotIsStale = computed(() => {
  const updatedAt = readExtraString(`${props.account.platformREDACTED_usage_updated_at`)
...
const formatReset = (iso: string) => {
  ...
  if (diffMs < 3_600_000) return `${Math.max(1, Math.round(diffMs / 60_000))REDACTEDm`
  const hours = Math.round(diffMs / 3_600_000)
  if (hours < 48) return `${hoursREDACTEDh`
  const mm = String(d.getMonth() + 1).padStart(2, '0')
  const dd = String(d.getDate()).padStart(2, '0')
  return `${mmREDACTED-${ddREDACTED`
REDACTED
