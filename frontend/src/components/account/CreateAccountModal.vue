      ...
      <div v-if="form.platform === 'antigravity'">
        ...
        <div class="mt-2 grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.accounts.types.upstream') REDACTEDREDACTED</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.types.upstreamDesc') REDACTEDREDACTED</span>
            </div>
          </button>
        </div>
      </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                <p class="text-sm font-medium text-blue-800 dark:text-blue-300">
                  {{ t('admin.accounts.gemini.modelPassthrough') REDACTEDREDACTED
                </p>
                <p class="mt-1 text-xs text-blue-700 dark:text-blue-400">
                  {{ t('admin.accounts.gemini.modelPassthroughDesc') REDACTEDREDACTED
                </p>
              </div>
            </div>
          </div>
      ...
      <div v-if="form.platform === 'openai' && accountCategory === 'oauth-based'" class="mb-4">
        <label class="flex items-center justify-between rounded-lg border border-gray-200 p-3 dark:border-dark-600">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">
                {{ t('admin.accounts.openai.enableSora') REDACTEDREDACTED
              </span>
              <span class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.accounts.openai.enableSoraHint') REDACTEDREDACTED
              </span>
            </div>
          </div>
          <label :class="['switch', { 'switch-active': enableSoraOnOpenAIOAuth REDACTED]">
...
import { claudeModels, getPresetMappingsByPlatform, getModelsByPlatform, commonErrorCodes, buildModelMappingObject REDACTED from '@/composables/useModelWhitelist'
...
import type { Proxy, AdminGroup, AccountPlatform, AccountType REDACTED from '@/types'
...
const mixedChannelWarningDetails = ref<{ groupName: string; currentPlatform: string; otherPlatform: string REDACTED | null>(null)
...
const doCreateAccount = async (payload: any) => {
  ...
  REDACTED catch (error: any) {
    ...
    if (error.response?.status === 409 && error.response?.data?.error === 'mixed_channel_warning') {
      const details = error.response.data.details || {REDACTED
