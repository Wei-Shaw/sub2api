  ...
  >
    ...
    >
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity' && form.platform !== 'kiro'" class="space-y-4">
        ...
        <div v-if="(!isCNPlatform && form.platform !== 'fal') || apiProtocol !== 'adaptive'">
          <label class="input-label">{{ t('admin.accounts.baseUrl') }}</label>
          <input
            v-model="apiKeyBaseUrl"
            type="text"
      ...
      <div
        v-if="form.platform === 'openai' && (form.type === 'oauth' || form.type === 'setup-token' || form.type === 'apikey')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            data-testid="create-openai-responses-image-generation-toggle"
...
// supportsUpstreamBillingProbe：判定"平台 + 账号类型"是否可参与上游倍率探测。
// 媒体平台（fal / atlascloud / apiz）按量计费、没有可探测的倍率，必须排除，
// 否则创建后会白发一次探测请求并弹出失败告警。
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (openAIResponsesImageGenerationEnabled.value) {
    delete extra.openai_responses_image_generation_enabled
  } else {
    extra.openai_responses_image_generation_enabled = false
  }
...
const handleSubmit = async () => {
  ...
  // fal 接入域名固定，不写入 base_url（避免被当作上游域名覆盖默认值）
  const credentials: Record<string, unknown> = {
    api_key: apiKeyValue.value.trim()
  }
  if (form.platform !== 'fal') {
