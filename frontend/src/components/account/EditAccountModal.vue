  ...
  >
    ...
    >
      ...
      >
        ...
        <div v-for="window in (['5h', '7d'] as const)" :key="window" class="flex items-center justify-between gap-4">
          <label class="input-label mb-0" :for="`auto-reset-credit-${window}`">
            {{ t(`admin.accounts.autoResetCredit.enable${window === '5h' ? '5h' : '7d'}`) }}
          </label>
          <button
            :id="`auto-reset-credit-${window}`"
            type="button"
            role="switch"
            :aria-checked="window === '5h' ? autoResetCredit5hEnabled : autoResetCredit7dEnabled"
            :data-testid="`auto-reset-credit-${window}-enabled`"
            @click="window === '5h' ? (autoResetCredit5hEnabled = !autoResetCredit5hEnabled) : (autoResetCredit7dEnabled = !autoResetCredit7dEnabled)"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              (window === '5h' ? autoResetCredit5hEnabled : autoResetCredit7dEnabled) ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span :class="[
              'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
              (window === '5h' ? autoResetCredit5hEnabled : autoResetCredit7dEnabled) ? 'translate-x-5' : 'translate-x-0'
            ]" />
          </button>
        </div>
      </div>
...
const autoResetCredit5hEnabled = ref(false)
const autoResetCredit7dEnabled = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  allowOverages.value = false
	...
	autoResetCredit5hEnabled.value = extra?.auto_reset_credit_5h_enabled === true
	autoResetCredit7dEnabled.value = typeof extra?.auto_reset_credit_7d_enabled === 'boolean'
		? extra.auto_reset_credit_7d_enabled : extra?.auto_reset_credit_enabled === true
