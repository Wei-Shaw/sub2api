<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="(account?.platform === 'anthropic' && (account?.type === 'oauth' || account?.type === 'setup-token')) || (account?.platform === 'openai' && account?.type === 'oauth')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4"
      >
        <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          <div class="flex items-center justify-between">
...
function loadQuotaControlSettings(account: Account) {
  ...
  // Remaining quota control settings apply to Anthropic OAuth/SetupToken and OpenAI OAuth accounts
  if (account.platform !== 'anthropic' && !(account.platform === 'openai' && account.type === 'oauth')) {
    return
  }
