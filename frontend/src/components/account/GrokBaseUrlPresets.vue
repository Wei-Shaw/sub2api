<template>
  <div class="flex flex-wrap gap-2">
    ...
    >
      {{ presetLabel(preset) REDACTEDREDACTED ({{ displayUrl(preset.url) REDACTEDREDACTED)
    </button>
  </div>
</template>
...
import { GROK_BASE_URL_PRESETS, type GrokBaseUrlPreset REDACTED from './credentialsBuilder'
...
const presetLabel = (preset: GrokBaseUrlPreset) =>
  preset.label ?? t(`admin.accounts.grokCustomBaseUrl.presets.${preset.labelKeyREDACTED`)
