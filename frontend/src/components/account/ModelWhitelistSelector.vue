<template>
  <div>
    ...
    <div class="relative mb-3">
      ...
      >
        <div class="grid grid-cols-2 gap-1.5">
          ...
          >
            <span class="flex items-center gap-1 truncate">
              ...
              <span class="truncate">{{ model REDACTEDREDACTED</span>
            </span>
            <button
              type="button"
        ...
        <div class="mt-2 flex items-center justify-between border-t border-gray-200 pt-2 dark:border-dark-600">
          <span class="text-xs text-gray-400">{{ t('admin.accounts.modelCount', { count: modelValue.length REDACTED) REDACTEDREDACTED</span>
        ...
        <div class="max-h-52 overflow-auto">
          ...
          >
            ...
            >
              ...
              <span class="truncate text-gray-900 dark:text-white">{{ model.value REDACTEDREDACTED</span>
            </button>
            <button
              type="button"
              ...
              :title="`${t('common.copy')REDACTED ${model.valueREDACTED`"
              :aria-label="`${t('common.copy')REDACTED ${model.valueREDACTED`"
          ...
          <div v-if="filteredModels.length === 0" class="px-3 py-4 text-center text-sm text-gray-500">
            {{ t('admin.accounts.noMatchingModels') REDACTEDREDACTED
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mb-4 flex flex-wrap gap-2">
      ...
      >
        {{ t('admin.accounts.fillRelatedModels') REDACTEDREDACTED
      </button>
      <button
      ...
      >
        {{ isSyncingUpstream ? t('admin.accounts.syncUpstreamModelsLoading') : t('admin.accounts.syncUpstreamModels') REDACTEDREDACTED
      </button>
      <button
        type="button"
        @click="clearAll"
      ...
      >
        {{ t('admin.accounts.clearAllModels') REDACTEDREDACTED
      </button>
    </div>

    <!-- Custom Model Input -->
    <div class="mb-3">
      <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.accounts.customModelName') REDACTEDREDACTED</label>
      <div class="flex gap-2">
        ...
        >
          {{ t('admin.accounts.addModel') REDACTEDREDACTED
        </button>
      </div>
    </div>
  </div>
...
import { accountsAPI REDACTED from '@/api/admin/accounts'
import type { SyncUpstreamPreviewParams REDACTED from '@/api/admin/accounts'
import { useClipboard REDACTED from '@/composables/useClipboard'
import ModelIcon from '@/components/common/ModelIcon.vue'
import Icon from '@/components/icons/Icon.vue'
import { allModels, getModelsByPlatform REDACTED from '@/composables/useModelWhitelist'
...
const syncUpstreamModels = async () => {
  ...
  try {
    ...
    REDACTED else if (props.syncCredentials) {
    ...
    emit('update:modelValue', newModels)
    if (addedCount > 0) {
      appStore.showSuccess(t('admin.accounts.syncUpstreamModelsSuccess', { count: addedCount, total: upstreamModels.length REDACTED))
    REDACTED else {
      appStore.showInfo(t('admin.accounts.syncUpstreamModelsNoChanges', { count: upstreamModels.length REDACTED))
    REDACTED
  REDACTED catch (error) {
    const message = error instanceof Error ? error.message : t('admin.accounts.syncUpstreamModelsFailed')
    appStore.showError(t('admin.accounts.syncUpstreamModelsError', { message REDACTED))
  REDACTED finally {
