<template>
  ...
  >
      ...
      <div class="flex-1">
        <h4 class="mb-3 font-semibold text-blue-900 dark:text-blue-200">{{ oauthTitle REDACTEDREDACTED</h4>

        <!-- Auth Method Selection -->
        <div v-if="showMethodSelection" class="mb-4">
          <label class="mb-2 block text-sm font-medium text-blue-800 dark:text-blue-300">
            {{ methodLabel REDACTEDREDACTED
          </label>
        ...
        <div v-if="inputMethod === 'refresh_token' || inputMethod === 'mobile_refresh_token'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(getOAuthKey('refreshTokenDesc')) REDACTEDREDACTED
            </p>
            ...
            <div class="mb-4">
              ...
              >
                ...
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedRefreshTokenCount REDACTED) REDACTEDREDACTED
                </span>
              </label>
              <textarea
              ...
              >
                {{ t('admin.accounts.oauth.batchCreateAccounts', { count: parsedRefreshTokenCount REDACTED) REDACTEDREDACTED
              </p>
            </div>
        ...
        <div v-if="inputMethod === 'sso_cookie'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(getOAuthKey('ssoCookieDesc')) REDACTEDREDACTED
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t(getOAuthKey('ssoCookieLabel')) REDACTEDREDACTED
                <span
                  v-if="parsedSSOCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedSSOCount REDACTED) REDACTEDREDACTED
                </span>
              </label>
              <textarea
              ...
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t(getOAuthKey('ssoCookieHint')) REDACTEDREDACTED
              </p>
            </div>
            ...
            >
              ...
              <Icon v-else name="sparkles" size="sm" class="mr-2" />
              {{ loading ? t(getOAuthKey('convertingSSO')) : t(getOAuthKey('convertSSOAndCreate')) REDACTEDREDACTED
            </button>
          </div>
        </div>
        ...
        <div v-if="inputMethod === 'email_password'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(getOAuthKey('emailPasswordDesc')) REDACTEDREDACTED
            </p>
            <div class="mb-4">
              <label
                class="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-300"
              >
                ...
                {{ t(getOAuthKey('emailPasswordInputLabel')) REDACTEDREDACTED
                <span
                  v-if="parsedEmailPasswordCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedEmailPasswordCount REDACTED) REDACTEDREDACTED
                </span>
              </label>
              <textarea
              ...
              ></textarea>
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t(getOAuthKey('emailPasswordHint')) REDACTEDREDACTED
              </p>
            </div>
            <div
              v-if="error"
            ...
            >
              ...
              <Icon v-else name="sparkles" size="sm" class="mr-2" />
              {{ loading ? t(getOAuthKey('validating')) : t(getOAuthKey('validateAndCreate')) REDACTEDREDACTED
            </button>
          </div>
        </div>
        ...
        <div v-if="inputMethod === 'codex_session' || inputMethod === 'agent_identity'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(isAgentIdentityInput ? 'admin.accounts.oauth.openai.agentIdentityDesc' : 'admin.accounts.oauth.openai.codexSessionDesc') REDACTEDREDACTED
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t(isAgentIdentityInput ? 'admin.accounts.oauth.openai.agentIdentityInputLabel' : 'admin.accounts.oauth.openai.codexSessionInputLabel') REDACTEDREDACTED
                <span
                  v-if="parsedCodexSessionCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedCodexSessionCount REDACTED) REDACTEDREDACTED
                </span>
              </label>
              <textarea
              ...
              ></textarea>
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t(isAgentIdentityInput ? 'admin.accounts.oauth.openai.agentIdentityHint' : 'admin.accounts.oauth.openai.codexSessionHint') REDACTEDREDACTED
              </p>
            </div>
        ...
        <div v-if="inputMethod === 'codex_pat'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.openai.codexPatDesc') REDACTEDREDACTED
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t('admin.accounts.oauth.openai.codexPatInputLabel') REDACTEDREDACTED
              </label>
              <textarea
              ...
              ></textarea>
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t('admin.accounts.oauth.openai.codexPatHint') REDACTEDREDACTED
              </p>
            </div>
        ...
        <div v-if="inputMethod === 'cookie'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.cookieAutoAuthDesc') REDACTEDREDACTED
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t('admin.accounts.oauth.sessionKey') REDACTEDREDACTED
                <span
                  v-if="parsedKeyCount > 1 && allowMultiple"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedKeyCount REDACTED) REDACTEDREDACTED
                </span>
                <button
                  v-if="showHelp"
                  type="button"
              ...
              <p
                ...
                class="mt-1 text-xs text-blue-600 dark:text-blue-400"
              >
                {{ t('admin.accounts.oauth.batchCreateAccounts', { count: parsedKeyCount REDACTED) REDACTEDREDACTED
              </p>
            </div>
            ...
            >
              <h5 class="mb-2 font-semibold text-amber-800 dark:text-amber-200">
                {{ t('admin.accounts.oauth.howToGetSessionKey') REDACTEDREDACTED
              </h5>
              <ol
                class="list-inside list-decimal space-y-1 text-xs text-amber-700 dark:text-amber-300"
              >
                <li>{{ t('admin.accounts.oauth.step1') REDACTEDREDACTED</li>
                <li>{{ t('admin.accounts.oauth.step2') REDACTEDREDACTED</li>
                <li>{{ t('admin.accounts.oauth.step3') REDACTEDREDACTED</li>
                <li>{{ t('admin.accounts.oauth.step4') REDACTEDREDACTED</li>
                <li>{{ t('admin.accounts.oauth.step5') REDACTEDREDACTED</li>
                <li>{{ t('admin.accounts.oauth.step6') REDACTEDREDACTED</li>
              </ol>
              <p
        ...
        <div v-if="inputMethod === 'manual'" class="space-y-4">
          <p class="mb-4 text-sm text-blue-800 dark:text-blue-300">
            {{ oauthFollowSteps REDACTEDREDACTED
          </p>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ oauthStep1GenerateUrl REDACTEDREDACTED
                </p>
                <div v-if="showProjectId && platform === 'gemini'" class="mb-3">
                  <label class="input-label flex items-center gap-2">
                    {{ t('admin.accounts.oauth.gemini.projectIdLabel') REDACTEDREDACTED
                    <a
                    ...
                    >
                      ...
                      {{ t('admin.accounts.oauth.gemini.howToGetProjectId') REDACTEDREDACTED
                    </a>
                  </label>
                  <input
                    v-model="projectId"
                  ...
                  <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {{ t('admin.accounts.oauth.gemini.projectIdHint') REDACTEDREDACTED
                  </p>
                </div>
                <button
                  v-if="!authUrl"
                ...
                >
                  ...
                  {{ loading ? t('admin.accounts.oauth.generating') : oauthGenerateAuthUrl REDACTEDREDACTED
                </button>
                <div v-else class="space-y-3">
                  ...
                  >
                    ...
                    {{ t('admin.accounts.oauth.regenerate') REDACTEDREDACTED
                  </button>
                </div>
              </div>
            </div>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ oauthStep2OpenUrl REDACTEDREDACTED
                </p>
                <p class="text-sm text-blue-700 dark:text-blue-300">
                  {{ oauthOpenUrlDesc REDACTEDREDACTED
                </p>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ oauthStep3EnterCode REDACTEDREDACTED
                </p>
                <p
                ...
                <div>
                  <label class="input-label">
                    ...
                    {{ oauthAuthCode REDACTEDREDACTED
                  </label>
                  <textarea
                  ...
                  <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    {{ oauthAuthCodeHint REDACTEDREDACTED
                  </p>
                  ...
                  >
                    <div class="flex items-start gap-2">
                      ...
                      <div class="text-sm text-amber-800 dark:text-amber-300">
                        <p class="font-semibold">{{ $t('admin.accounts.oauth.gemini.stateWarningTitle') REDACTEDREDACTED</p>
                        <p class="mt-1">{{ $t('admin.accounts.oauth.gemini.stateWarningDesc') REDACTEDREDACTED</p>
                      </div>
                    </div>
                  </div>
                </div>
...
const getOAuthKey = (key: string) => {
  if (props.platform === 'openai') return `admin.accounts.oauth.openai.${keyREDACTED`
  if (props.platform === 'gemini') return `admin.accounts.oauth.gemini.${keyREDACTED`
  if (props.platform === 'antigravity') return `admin.accounts.oauth.antigravity.${keyREDACTED`
  if (props.platform === 'grok') return `admin.accounts.oauth.grok.${keyREDACTED`
  return `admin.accounts.oauth.${keyREDACTED`
REDACTED
...
watch(authCodeInput, (newVal) => {
  ...
  if (trimmed.includes('code=')) {
    try {
      // Try to parse as URL
      const url = trimmed.includes('?') ? new URL(trimmed) : new URL(`http://localhost/callback?${trimmed.replace(/^\?/, '')REDACTED`)
