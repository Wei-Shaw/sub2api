  ...
  <section v-if="state?.eligible" class="space-y-4 border-t border-gray-200 pt-4 dark:border-dark-600" data-testid="ollama-cloud-usage-settings">
    <div class="flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">
          {{ t('admin.accounts.ollamaCloud.title') REDACTEDREDACTED
        </h3>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {{ t('admin.accounts.ollamaCloud.sessionSecurityHint') REDACTEDREDACTED
        </p>
      </div>
      <span
      ...
      >
        {{ state.configured ? t('admin.accounts.ollamaCloud.configured') : t('admin.accounts.ollamaCloud.notConfigured') REDACTEDREDACTED
      </span>
    </div>
    ...
    <template v-else>
      <div v-if="!state.encryption_key_configured" class="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-800/50 dark:bg-amber-900/20 dark:text-amber-200">
        {{ t('admin.accounts.ollamaCloud.encryptionKeyRequired') REDACTEDREDACTED
      </div>
      ...
      >
        <div class="grid grid-cols-[minmax(4rem,auto)_minmax(0,1fr)] gap-x-3 gap-y-1.5 text-xs">
          <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.ollamaCloud.plan') REDACTEDREDACTED</span>
          <span class="break-words text-gray-900 dark:text-white">{{ snapshot.data?.plan || '-' REDACTEDREDACTED</span>
          <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.ollamaCloud.fiveHour') REDACTEDREDACTED</span>
          <span class="break-words text-gray-900 dark:text-white">{{ windowSummary(snapshot.data?.five_hour) REDACTEDREDACTED</span>
          <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.ollamaCloud.sevenDay') REDACTEDREDACTED</span>
          <span class="break-words text-gray-900 dark:text-white">{{ windowSummary(snapshot.data?.seven_day) REDACTEDREDACTED</span>
          <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.ollamaCloud.balance') REDACTEDREDACTED</span>
          <span class="break-words text-gray-900 dark:text-white">{{ snapshot.data?.balance || '-' REDACTEDREDACTED</span>
          <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.ollamaCloud.models') REDACTEDREDACTED</span>
          <span class="break-words text-gray-900 dark:text-white">{{ modelSummary REDACTEDREDACTED</span>
          <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.ollamaCloud.status') REDACTEDREDACTED</span>
          <span class="break-words font-medium text-gray-900 dark:text-white">{{ statusLabel REDACTEDREDACTED</span>
          <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.ollamaCloud.updatedAt') REDACTEDREDACTED</span>
          <span class="break-words text-gray-900 dark:text-white">{{ formatDate(snapshot.fetched_at || snapshot.last_attempt_at) REDACTEDREDACTED</span>
        </div>
        <p v-if="snapshot.last_error" class="mt-2 break-words border-t border-gray-100 pt-2 text-xs text-amber-700 dark:border-dark-700 dark:text-amber-300">
          {{ t(`admin.accounts.ollamaCloud.errors.${snapshot.last_errorREDACTED`, snapshot.last_error) REDACTEDREDACTED
        </p>
      </div>

      <div>
        <label class="input-label" for="ollama-cloud-session">{{ t('admin.accounts.ollamaCloud.sessionLabel') REDACTEDREDACTED</label>
        <textarea
        ...
        />
        <p class="input-hint">{{ t('admin.accounts.ollamaCloud.writeOnlyHint') REDACTEDREDACTED</p>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {{ t('admin.accounts.ollamaCloud.deleteSession') REDACTEDREDACTED
        </button>
        <button
          v-if="state.configured"
          type="button"
          ...
          @click="refreshUsage"
        >
          <Icon name="refresh" size="xs" class="mr-1.5" :class="{ 'animate-spin': refreshing REDACTED" />
          {{ t('admin.accounts.ollamaCloud.refreshNow') REDACTEDREDACTED
        </button>
      </div>
      ...
      <div v-if="state.configured" class="flex items-center justify-between gap-4 border-t border-gray-100 pt-4 dark:border-dark-700">
        <div>
          <label class="text-sm font-medium text-gray-900 dark:text-white">
            {{ t('admin.accounts.ollamaCloud.autoRefresh') REDACTEDREDACTED
          </label>
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.accounts.ollamaCloud.autoRefreshHint') REDACTEDREDACTED
          </p>
        </div>
        <Toggle
...
import type { Account, OllamaCloudUsageState, OllamaCloudUsageWindow REDACTED from '@/types'
import ConfirmDialog from '@/components/common/ConfirmDialog.vue'
import Toggle from '@/components/common/Toggle.vue'
import Icon from '@/components/icons/Icon.vue'

const props = defineProps<{ account: Account REDACTED>()
const emit = defineEmits<{ updated: [state: OllamaCloudUsageState] REDACTED>()
...
const modelSummary = computed(() => snapshot.value?.data?.models?.map(model => {
  ...
  return `${windowREDACTED ${model.modelREDACTED: ${model.requestsREDACTED`
REDACTED).join(', ') || '-')

const formatPercent = (value?: number) => typeof value === 'number' && Number.isFinite(value)
  ? `${value.toFixed(value % 1 ? 1 : 0)REDACTED%`
  : '-'
...
const windowSummary = (window?: OllamaCloudUsageWindow) => {
  ...
  return reset
    ? t('admin.accounts.ollamaCloud.windowWithReset', { percent: formatPercent(window.used_percent), reset REDACTED)
