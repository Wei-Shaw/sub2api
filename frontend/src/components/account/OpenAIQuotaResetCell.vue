  ...
  <div v-if="visible" class="space-y-1">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      >
        ...
        {{ t('admin.accounts.openaiQuotaReset.count') REDACTEDREDACTED<span v-if="data"> {{ availableResetCount REDACTEDREDACTED</span>
      </button>
      ...
      >
        <svg
          class="h-2.5 w-2.5"
          :class="{ 'animate-spin': resetting REDACTED"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        {{ t('admin.accounts.openaiQuotaReset.reset') REDACTEDREDACTED
      </button>
    </div>
    ...
    >
      ...
      >
        {{ autoResetStateLabel REDACTEDREDACTED
        <span v-if="autoResetState.trigger_window" class="ml-1 tabular-nums">
          {{ autoResetState.trigger_window REDACTEDREDACTED
        </span>
      </span>
      <span v-if="autoResetState.checked_at" class="text-gray-500 dark:text-gray-400">
        {{ formatResetCreditExpiry(autoResetState.checked_at, 'short') REDACTEDREDACTED
      </span>
      <span
      ...
      >
        {{ autoResetState.error_code REDACTEDREDACTED
      </span>
    </div>
    ...
    <div v-if="primaryResetCreditExpiry" class="space-y-1">
      <div class="flex flex-wrap items-center gap-1">
        <span
          ...
          :title="t('admin.accounts.openaiQuotaReset.expiresAtFull', { time: formatResetCreditExpiry(primaryResetCreditExpiry, 'full') REDACTED)"
        >
          {{ t('admin.accounts.openaiQuotaReset.expiresAt', { time: formatResetCreditExpiry(primaryResetCreditExpiry, 'short') REDACTED) REDACTEDREDACTED
        </span>
        <button
        ...
        >
          +{{ hiddenResetCreditCount REDACTEDREDACTED
        </button>
      </div>
      ...
      >
        <span class="sr-only">{{ t('admin.accounts.openaiQuotaReset.expirationDetails') REDACTEDREDACTED</span>
        <span
          v-for="(expiresAt, index) in resetCreditExpirations"
          :key="`${expiresAtREDACTED-${indexREDACTED`"
          class="flex min-w-0 items-center gap-1 tabular-nums"
          :title="t('admin.accounts.openaiQuotaReset.expiresAtFull', { time: formatResetCreditExpiry(expiresAt, 'full') REDACTED)"
        >
          <span class="h-1 w-1 shrink-0 rounded-full bg-gray-400 dark:bg-dark-500" />
          <span class="truncate">{{ formatResetCreditExpiry(expiresAt, 'short') REDACTEDREDACTED</span>
        </span>
      </div>
    </div>
    ...
    >
      {{ resetWarning REDACTEDREDACTED
    </div>
    <div
      v-else-if="resetMessage"
      class="text-[10px] text-emerald-600 dark:text-emerald-400"
    >
      {{ resetMessage REDACTEDREDACTED
    </div>

    <ConfirmDialog
      :show="showResetConfirm"
      :title="t('admin.accounts.openaiQuotaReset.confirmTitle')"
      :message="t('admin.accounts.openaiQuotaReset.confirmMessage', { count: availableResetCount REDACTED)"
...
const autoResetStateLabel = computed(() => {
  ...
  const keyByStatus: Record<string, string> = {
    ...
    failed: 'failed'
  REDACTED
  return t(`admin.accounts.openaiQuotaReset.autoStatus.${keyByStatus[autoResetState.value.status]REDACTED`)
REDACTED)
...
const readCachedResetCredits = (account: Account): OpenAIQuotaUsage | null => {
  ...
  const { available_count: count, credits: rawCredits REDACTED = cached as {
  ...
  const now = Date.now()
  const credits: { expires_at?: string REDACTED[] = []
  if (Array.isArray(rawCredits)) {
    for (const credit of rawCredits) {
      if (!credit || typeof credit !== 'object') continue
      const expiresAt = (credit as { expires_at?: unknown REDACTED).expires_at
      ...
      credits.push({ expires_at: expiresAt REDACTED)
    REDACTED
  REDACTED
...
const resetCreditDetailsToggleLabel = computed(() => {
  ...
  REDACTED
  return t('admin.accounts.openaiQuotaReset.expandExpirations', { count: hiddenResetCreditCount.value REDACTED)
REDACTED)
...
const extractErrorMessage = (e: unknown): string => {
  ...
  // errors into { status, code, message, reason, ... REDACTED and re-rejects them, so
...
const confirmReset = async () => {
  ...
  try {
    ...
    REDACTED else if (result.warning_code === 'account_state_recovery_failed') {
      resetWarning.value = t('admin.accounts.openaiQuotaReset.resetAccountRecoveryFailed')
    REDACTED else if (result.warning_code === 'account_state_refresh_failed') {
