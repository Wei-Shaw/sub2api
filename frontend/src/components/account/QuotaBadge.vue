...
const tooltip = computed(() => {
  if (props.unlimited) return t('admin.accounts.quotaSource.unlimited')
  if (props.used >= props.limit) {
...
const currencyPrefix = computed(() => !props.unit || props.unit.toUpperCase() === 'USD' ? '$' : '')
const unitSuffix = computed(() => !props.unit || props.unit.toUpperCase() === 'USD' ? '' : ` ${props.unit}`)
</script>
...
<template>
  ...
  >
    ...
    <span v-if="unlimited" class="font-mono">∞</span>
    <template v-else>
      <span class="font-mono">{{ currencyPrefix }}{{ fmt(used) }}</span>
      <span class="text-gray-400 dark:text-gray-500">/</span>
      <span class="font-mono">{{ currencyPrefix }}{{ fmt(limit) }}{{ unitSuffix }}</span>
    </template>
  </span>
</template>
