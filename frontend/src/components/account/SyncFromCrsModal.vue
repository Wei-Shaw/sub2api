  ...
  >
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 gap-4">
        <div>
          <label class="input-label">{{ t('admin.accounts.crsBaseUrl') REDACTEDREDACTED</label>
          <input
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label class="input-label">{{ t('admin.accounts.crsUsername') REDACTEDREDACTED</label>
            <input
          ...
          <div>
            <label class="input-label">{{ t('admin.accounts.crsPassword') REDACTEDREDACTED</label>
            <input
        ...
        <div v-if="errorItems.length" class="mt-2">
          ...
          <div class="mt-2 max-h-48 overflow-auto rounded-lg bg-gray-50 dark:bg-dark-800 p-3 text-xs font-mono">
            <div v-for="(item, idx) in errorItems" :key="idx" class="whitespace-pre-wrap">
              {{ item.kind REDACTEDREDACTED {{ item.crs_account_id REDACTEDREDACTED — {{ item.action REDACTEDREDACTED{{ item.error ? `: ${item.errorREDACTED` : '' REDACTEDREDACTED
            </div>
          </div>
        </div>
      </div>
