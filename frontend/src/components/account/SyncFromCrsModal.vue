  ...
  >
    ...
    >
      <div class="text-sm text-gray-600 dark:text-dark-300">
        {{ t('admin.accounts.syncFromCrsDesc') REDACTEDREDACTED
      </div>
      <div
        class="rounded-lg bg-gray-50 p-3 text-xs text-gray-500 dark:bg-dark-700/60 dark:text-dark-400"
      >
        {{ t('admin.accounts.crsUpdateBehaviorNote') REDACTEDREDACTED
      </div>
      <div
        class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-600 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400"
      >
        {{ t('admin.accounts.crsVersionRequirement') REDACTEDREDACTED
      </div>

      <div class="grid grid-cols-1 gap-4">
        <div>
          <label for="crs-base-url" class="input-label">{{ t('admin.accounts.crsBaseUrl') REDACTEDREDACTED</label>
          <input
            id="crs-base-url"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label for="crs-username" class="input-label">{{ t('admin.accounts.crsUsername') REDACTEDREDACTED</label>
            <input id="crs-username" v-model="form.username" type="text" class="input" required autocomplete="username" />
          </div>
          <div>
            <label for="crs-password" class="input-label">{{ t('admin.accounts.crsPassword') REDACTEDREDACTED</label>
            <input
              id="crs-password"
        ...
        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-dark-300">
          ...
          />
          {{ t('admin.accounts.syncProxies') REDACTEDREDACTED
        </label>
      </div>
    </form>
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      >
        <div class="mb-2 text-sm font-medium text-gray-700 dark:text-dark-300">
          {{ t('admin.accounts.crsExistingAccounts') REDACTEDREDACTED
          <span class="ml-1 text-xs text-gray-400">({{ previewResult.existing_accounts.length REDACTEDREDACTED)</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-gray-500 dark:text-dark-400">
          ...
          >
            ...
            >{{ acc.platform REDACTEDREDACTED / {{ acc.type REDACTEDREDACTED</span>
            <span class="truncate">{{ acc.name REDACTEDREDACTED</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-gray-900 dark:text-white">
            {{ t('admin.accounts.crsNewAccounts') REDACTEDREDACTED
            <span class="ml-1 text-xs text-gray-400">({{ previewResult.new_accounts.length REDACTEDREDACTED)</span>
          </div>
          <div class="flex gap-2">
            <button
              ...
              @click="selectAll"
            >{{ t('admin.accounts.crsSelectAll') REDACTEDREDACTED</button>
            <button
              type="button"
              class="text-xs text-gray-500 hover:text-gray-600 dark:text-gray-400"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') REDACTEDREDACTED</button>
          </div>
        </div>
        <div
        ...
        >
          ...
          >
            ...
            >{{ acc.platform REDACTEDREDACTED / {{ acc.type REDACTEDREDACTED</span>
            <span class="truncate text-sm text-gray-700 dark:text-dark-300">{{ acc.name REDACTEDREDACTED</span>
          </label>
        </div>
        <div class="mt-1 text-xs text-gray-400">
          {{ t('admin.accounts.crsSelectedCount', { count: selectedIds.size REDACTED) REDACTEDREDACTED
        </div>
      </div>

      <!-- Sync options summary -->
      <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-dark-400">
        <span>{{ t('admin.accounts.syncProxies') REDACTEDREDACTED:</span>
        <span :class="form.sync_proxies ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-dark-500'">
          {{ form.sync_proxies ? t('common.yes') : t('common.no') REDACTEDREDACTED
        </span>
      </div>
      ...
      >
        {{ t('admin.accounts.crsNoNewAccounts') REDACTEDREDACTED
        <span v-if="previewResult.existing_accounts.length">
          {{ t('admin.accounts.crsWillUpdate', { count: previewResult.existing_accounts.length REDACTED) REDACTEDREDACTED
        </span>
      </div>
    </div>
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      ...
      >
        <div class="text-sm font-medium text-gray-900 dark:text-white">
          {{ t('admin.accounts.syncResult') REDACTEDREDACTED
        </div>
        <div class="text-sm text-gray-700 dark:text-dark-300">
          {{ t('admin.accounts.syncResultSummary', result) REDACTEDREDACTED
        </div>

        <div v-if="errorItems.length" class="mt-2">
          <div class="text-sm font-medium text-red-600 dark:text-red-400">
            {{ t('admin.accounts.syncErrors') REDACTEDREDACTED
          </div>
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-gray-50 p-3 font-mono text-xs dark:bg-dark-800"
          >
            <div v-for="(item, idx) in errorItems" :key="idx" class="whitespace-pre-wrap">
              {{ item.kind REDACTEDREDACTED {{ item.crs_account_id REDACTEDREDACTED — {{ item.action
              REDACTEDREDACTED{{ item.error ? `: ${item.errorREDACTED` : '' REDACTEDREDACTED
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3">
        ...
        <template v-if="currentStep === 'input'">
          ...
          <button
            ...
            :disabled="previewing"
          >
            {{ previewing ? t('admin.accounts.crsPreviewing') : t('admin.accounts.crsPreview') REDACTEDREDACTED
          </button>
        </template>
        ...
        <template v-else-if="currentStep === 'preview'">
          <button
            ...
            @click="handleBack"
          >
            {{ t('admin.accounts.crsBack') REDACTEDREDACTED
          </button>
          <button
            class="btn btn-primary"
            type="button"
            ...
            @click="handleSync"
          >
            {{ syncing ? t('admin.accounts.syncing') : t('admin.accounts.syncNow') REDACTEDREDACTED
          </button>
        </template>
...
import type { PreviewFromCRSResult REDACTED from '@/api/admin/accounts'
