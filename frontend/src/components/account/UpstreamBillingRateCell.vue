  ...
  <div v-if="eligible" class="flex h-6 min-w-[7rem] items-center gap-1">
    <HelpTooltip class="-ml-1" width-class="w-max max-w-[calc(100vw-2rem)]" data-testid="upstream-billing-details">
      <template #trigger>
        ...
        >
          {{ primaryValue REDACTEDREDACTED
        </span>
      </template>
      <div class="space-y-1">
        <template v-if="hasEffectiveRate && data">
          <p>{{ t('admin.accounts.upstreamBilling.groupRate', { value: data.group_rate_multiplier REDACTED) REDACTEDREDACTED</p>
          <p v-if="data.user_rate_multiplier != null">
            {{ t('admin.accounts.upstreamBilling.userRate', { value: data.user_rate_multiplier REDACTED) REDACTEDREDACTED
          </p>
          <p>
            {{
          ...
          <p>{{ t('admin.accounts.upstreamBilling.effectiveRate', { value: currentEffectiveRate ?? '-' REDACTED) REDACTEDREDACTED</p>
          <p>{{ t('admin.accounts.upstreamBilling.updatedAt', { value: formatDate(snapshot?.received_at) REDACTED) REDACTEDREDACTED</p>
        </template>
        <template v-else-if="stale && lastDetectedRate != null">
          <p data-testid="upstream-billing-last-rate">
            {{ t('admin.accounts.upstreamBilling.lastDetectedRate', { value: lastDetectedRate REDACTED) REDACTEDREDACTED
          </p>
          <p data-testid="upstream-billing-last-time">
            {{ t('admin.accounts.upstreamBilling.lastDetectedAt', { value: formatDate(snapshot?.received_at) REDACTED) REDACTEDREDACTED
          </p>
          <p data-testid="upstream-billing-elapsed">
            {{ t('admin.accounts.upstreamBilling.elapsedSince', { value: elapsedSinceLastSuccess REDACTED) REDACTEDREDACTED
          </p>
        </template>
        <p v-else>{{ statusLabel || '-' REDACTEDREDACTED</p>
        <p
          v-if="probeEnabled && globalProbeEnabled !== false && nextProbeAt"
          data-testid="upstream-billing-next-probe"
        >
          {{ t('admin.accounts.upstreamBilling.nextProbeAt', { value: formatDate(nextProbeAt) REDACTED) REDACTEDREDACTED
        </p>
        <p class="mt-2 border-t border-white/15 pt-2" data-testid="upstream-billing-probe-state">
          {{ t('admin.accounts.upstreamBilling.accountProbeState') REDACTEDREDACTED
          <span :class="probeEnabled ? 'text-emerald-400' : 'text-red-400'">
            {{ probeEnabled ? t('admin.accounts.upstreamBilling.enabled') : t('admin.accounts.upstreamBilling.disabled') REDACTEDREDACTED
          </span>
        </p>
        <p
        ...
        >
          {{ t('admin.accounts.upstreamBilling.globalProbeState') REDACTEDREDACTED
          <span class="text-red-400">{{ t('admin.accounts.upstreamBilling.disabled') REDACTEDREDACTED</span>
        </p>
      </div>
    </HelpTooltip>
    <span v-if="hasEffectiveRate && statusLabel" :class="statusClass" class="whitespace-nowrap text-[10px] font-medium">
      {{ statusLabel REDACTEDREDACTED
    </span>
    <button
      type="button"
      ...
      @click="$emit('probe')"
    >
      <Icon name="refresh" size="xs" :class="{ 'animate-spin': probing REDACTED" />
    </button>
  </div>
...
import type { Account, UpstreamBillingProbeSnapshot REDACTED from '@/types'
...
const parseMinute = (value?: string) => {
  ...
  const match = /^(\d{2REDACTED):(\d{2REDACTED)$/.exec(value)
...
const minuteInTimeZone = (timestamp: number, timeZone?: string) => {
  ...
  try {
    const parts = new Intl.DateTimeFormat('en-GB', {
      ...
      minute: '2-digit',
      hourCycle: 'h23'
    REDACTED).formatToParts(new Date(timestamp))
...
const elapsedSinceLastSuccess = computed(() => {
  ...
  if (elapsedMinutes < 60) return t('admin.accounts.upstreamBilling.minutesAgo', { count: elapsedMinutes REDACTED)
  const elapsedHours = Math.floor(elapsedMinutes / 60)
  if (elapsedHours < 24) return t('admin.accounts.upstreamBilling.hoursAgo', { count: elapsedHours REDACTED)
  return t('admin.accounts.upstreamBilling.daysAgo', { count: Math.floor(elapsedHours / 24) REDACTED)
REDACTED)
const effectiveRate = computed(() => {
  ...
  return value == null ? '-' : `${Number(value.toPrecision(12))REDACTEDx`
REDACTED)
