        ...
        account: makeAccount({
          ...
          ollama_cloud_usage: {
            ...
            snapshot: {
              ...
              data: {
                five_hour: { used_percent: 12 REDACTED,
                seven_day: { used_percent: 34 REDACTED
              REDACTED
            REDACTED
          REDACTED
        REDACTED)
      ...
      global: {
        stubs: {
          OllamaCloudUsageCell: {
            props: ['account'],
            emits: ['updated'],
            template: '<button data-test="embedded-ollama" @click="$emit(\'updated\', { ...account.ollama_cloud_usage, auto_refresh_enabled: false REDACTED)">{{ account.ollama_cloud_usage.snapshot.data.five_hour.used_percent REDACTEDREDACTED</button>'
          REDACTED,
...
REDACTED)
  ...
  it('OpenAI 重置响应更新账号行时不会额外拉取 usage', async () => {
      ...
      global: {
        stubs: {
          ...
          OpenAIQuotaResetCell: {
            ...
            template: '<button data-test="quota-reset-result" @click="$emit(\'account-updated\', { ...account, updated_at: \'2026-03-07T10:01:00Z\' REDACTED)" />'
          REDACTED
        REDACTED
      REDACTED
    REDACTED)
    ...
    await wrapper.setProps({ account: updatedAccount as Account REDACTED)
    await flushPromises()
...
REDACTED)
  ...
  it('Grok OAuth compact UI drops local chips and header quota bars', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 3861, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok paid monthly limits show 30d bar without free 24h', async () => {
    getUsage.mockResolvedValue({
      ...
      grok_token_quota: { limit: 1_000, remaining: 250 REDACTED
    REDACTED)

    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4402, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok OAuth uses the official weekly billing percentage when available', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4201, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
            props: ['label', 'utilization', 'resetsAt', 'remainingCapacity'],
            template: '<div class="usage-bar">{{ label REDACTEDREDACTED|{{ utilization REDACTEDREDACTED|{{ resetsAt REDACTEDREDACTED|{{ remainingCapacity REDACTEDREDACTED</div>'
          REDACTED,
          AccountQuotaInfo: true,
        REDACTED
      REDACTED
  ...
  it.each([
    { tokens: 0, expected: 0 REDACTED,
    { tokens: 500_000, expected: 50 REDACTED,
    { tokens: 1_000_000, expected: 100 REDACTED,
    { tokens: 1_100_000, expected: 100 REDACTED
  ])('Grok Free derives its 1M quota from local tokens: $tokens -> $expected%', async ({ tokens, expected REDACTED) => {
    getUsage.mockResolvedValue({
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4300 + expected, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
    ...
    expect(wrapper.text()).toContain(`24h|${expectedREDACTED`)
  ...
  it('Grok Free uses rolling 24h usage instead of today-only usage', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4398, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED),
        todayStats: {
          requests: 2,
          tokens: 200_000,
          cost: 0,
      ...
      global: {
        stubs: {
          UsageProgressBar: {
            ...
            template: '<div class="usage-bar">{{ label REDACTEDREDACTED|{{ utilization REDACTEDREDACTED|{{ title REDACTEDREDACTED</div>'
          REDACTED,
          AccountQuotaInfo: true,
        REDACTED
      REDACTED
  ...
  it('Grok Free does not substitute today stats when rolling 24h usage is unavailable', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4399, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED),
        todayStats: {
          requests: 4,
          tokens: 1_000_000,
          cost: 0,
  ...
  it('Grok JWT free tier shows 24h bar even when leftover Heavy billing metrics remain', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4404, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok SuperGrok Lite stays on paid 7d bar, not free 24h', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4405, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok credential Free tier keeps the 1M fallback when billing is unavailable', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4403, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok Free 24h bar shows rolling local usage chips', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4410, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok SuperGrok and Heavy bars show period-aligned local 7d and 30d usage', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4411, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok paid bars fall back to official seven_day and thirty_day window_stats', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4412, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: {
  ...
  it('Grok paid hides zero prepaid and hides used/limit when monthly limit is 0', async () => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4413, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: true,
  ...
  it('Grok paid shows used/limit without prepaid, and prepaid without a zero monthly limit', async () => {
    ...
    const usedOnly = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4414, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: true,
    ...
    const prepaidOnly = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 4415, platform: 'grok', type: 'oauth', extra: {REDACTED REDACTED)
      REDACTED,
      global: {
        stubs: {
          UsageProgressBar: true,
