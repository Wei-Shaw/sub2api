...
const { queryBalance REDACTED = vi.hoisted(() => ({
  queryBalance: vi.fn()
REDACTED))

vi.mock('@/api/admin', () => ({
  adminAPI: {
    cnProviders: { queryBalance REDACTED
  REDACTED
REDACTED))
...
const account = {
  ...
  platform: 'kimi',
  type: 'apikey',
  credentials: { account_mode: 'payg' REDACTED,
  extra: {
    kimi_balance: 12.5,
...
describe('CNProviderBalanceCell', () => {
  ...
  it('renders the persisted balance as static text with an explicit query action', async () => {
    const wrapper = mount(CNProviderBalanceCell, { props: { account REDACTED REDACTED)
    await flushPromises()
  ...
  it('shows the low-balance badge from the snapshot marker', () => {
    const lowAccount = {
      ...account,
      extra: { kimi_balance: 0.4, kimi_balance_low: true REDACTED
    REDACTED as Account

    const wrapper = mount(CNProviderBalanceCell, { props: { account: lowAccount REDACTED REDACTED)
  ...
  it('keeps the snapshot balance visible when a query fails', async () => {
    queryBalance.mockResolvedValue({ success: false, error: 'HTTP 401' REDACTED)
    const wrapper = mount(CNProviderBalanceCell, { props: { account REDACTED REDACTED)
