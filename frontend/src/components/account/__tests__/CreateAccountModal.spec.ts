...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    accounts: {
      ...
      checkMixedChannelRisk: vi.fn().mockResolvedValue({ has_risk: false REDACTED),
...
const OAuthAuthorizationFlowStub = defineComponent({
  ...
  REDACTED,
  data: () => ({ inputMethod: 'manual' REDACTED),
...
function mountModal(groups: any[] = []) {
  return mount(CreateAccountModal, {
    props: { show: true, proxies: [], groups REDACTED,
    global: {
      stubs: {
...
async function submitApiKeyAccount(
  ...
  REDACTED
  await wrapper.get('form#create-account-form input[type="text"]').setValue(`${platformREDACTED account`)
...
describe('CreateAccountModal OpenAI long-context billing', () => {
  beforeEach(() => {
    ...
    createAccountMock.mockReset().mockResolvedValue({ id: 42, platform: 'openai', type: 'apikey' REDACTED)
    probeUpstreamBillingMock.mockReset().mockResolvedValue({REDACTED)
    importCodexSessionMock.mockReset().mockResolvedValue({
      ...
      warnings: [],
    REDACTED)
    createOpenAICodexPATMock.mockReset().mockResolvedValue({REDACTED)
  REDACTED)

  it('hides only the redundant account toggle when every selected group enables tier pricing', async () => {
    authIsSimpleMode.value = false
    const wrapper = mountModal([
      { id: 1, long_context_pricing_enabled: true REDACTED,
      { id: 2, long_context_pricing_enabled: true REDACTED,
    ])
  ...
  it('keeps the account toggle when any selected group disables tier pricing', async () => {
    ...
    const wrapper = mountModal([
      { id: 1, long_context_pricing_enabled: true REDACTED,
      { id: 2, long_context_pricing_enabled: false REDACTED,
    ])
  ...
  it.each([
    ['camelCase', { authMode: 'agentIdentity', agentIdentity: { agentRuntimeId: 'runtime' REDACTED REDACTED],
    ['nested identity without auth_mode', { agent_identity: { agent_runtime_id: 'runtime' REDACTED REDACTED],
