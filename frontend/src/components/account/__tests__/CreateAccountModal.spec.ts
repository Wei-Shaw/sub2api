...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('sends the default recharge multiplier and omits an empty New API group', async () => {
    await submitApiKeyAccount('openai')

    const payload = createAccountMock.mock.calls[0]?.[0]
    expect(payload?.upstream_billing_recharge_multiplier).toBe(1)
    expect(payload).not.toHaveProperty('upstream_billing_newapi_group')
  })

  it('trims the configured New API group before creating an API key account', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenAI')
    await selectButtonByText(wrapper, 'API Key')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('OpenAI account')
    await wrapper.get('form#create-account-form input[type="password"]').setValue('test-api-key')
    await wrapper.get('[data-testid="upstream-billing-newapi-group"]').setValue('  pro  ')
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()

    expect(createAccountMock.mock.calls[0]?.[0]?.upstream_billing_newapi_group).toBe('pro')
  })
