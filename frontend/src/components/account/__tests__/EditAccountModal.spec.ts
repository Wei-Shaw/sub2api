...
function buildGrokAPIKeyAccount() {
  return {
    ...
    credentials_status: { has_api_key: true REDACTED,
    concurrency: 2
  REDACTED as any
REDACTED
...
function buildOpenAIOAuthParentAccount() {
  return {
    ...
    credentials: { access_token: 'oauth-token' REDACTED,
    extra: {REDACTED
  REDACTED as any
REDACTED
...
describe('EditAccountModal', () => {
  ...
  it('preserves adaptive GLM endpoints on submit', async () => {
    ...
    checkMixedChannelRiskMock.mockReset().mockResolvedValue({ has_risk: false REDACTED)
  ...
  it('exposes the upstream billing auto-probe toggle for non-OpenAI API-key accounts', async () => {
    ...
    account.credentials = { api_key: 'sk-grok', base_url: 'https://relay.example/v1' REDACTED
    updateAccountMock.mockReset()
    checkMixedChannelRiskMock.mockReset()
    checkMixedChannelRiskMock.mockResolvedValue({ has_risk: false REDACTED)
    updateAccountMock.mockResolvedValue(account)
