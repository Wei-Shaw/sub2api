...
import { refreshOpenAIQuota, resetOpenAIQuota REDACTED from '@/api/admin/accounts'
...
describe('OpenAIQuotaResetCell — 外审 F6:影子禁用重置', () => {
    ...
    const account = makeAccount({
      ...
      extra: {
        codex_reset_credit_snapshot: {
          available_count: 2,
          credits: [
            { expires_at: FUTURE_EXPIRY_LATE REDACTED,
            { expires_at: FUTURE_EXPIRY_EARLY REDACTED,
          ],
        REDACTED,
      REDACTED,
    REDACTED)
  ...
  it('缓存中的重置卡全部过期时视为未知,不点亮重置入口', () => {
    const account = makeAccount({
      ...
      extra: {
        codex_reset_credit_snapshot: {
          available_count: 1,
          credits: [{ expires_at: PAST_EXPIRY REDACTED],
        REDACTED,
      REDACTED,
    REDACTED)
    const wrapper = mount(OpenAIQuotaResetCell, { props: { account REDACTED REDACTED)
    ...
    const account = makeAccount({
      ...
      extra: {
        codex_reset_credit_snapshot: {
          available_count: 3,
          credits: [
            { expires_at: PAST_EXPIRY REDACTED,
            { expires_at: FUTURE_EXPIRY_EARLY REDACTED,
          ],
        REDACTED,
      REDACTED,
  ...
  it('快照持久化失败时仍显示实时次数并给出警告', async () => {
    vi.mocked(refreshOpenAIQuota).mockResolvedValue({
      rate_limit_reset_credits: { available_count: 2 REDACTED,
      fetched_at: 1770000000,
  ...
  it('重置成功后直接使用响应中的最新缓存并回传恢复后的账号', async () => {
    ...
    const account = makeAccount({
      ...
      extra: {
        codex_reset_credit_snapshot: {
          available_count: 1,
          credits: [{ expires_at: FUTURE_EXPIRY_EARLY REDACTED],
        REDACTED,
      REDACTED,
    REDACTED)
    const wrapper = mount(OpenAIQuotaResetCell, { props: { account REDACTED REDACTED)
...
describe('OpenAIQuotaResetCell 自动用卡运行态', () => {
  it.each([
    ...
    expect(state.text()).toContain(`admin.accounts.openaiQuotaReset.autoStatus.${labelKeyREDACTED`)
  ...
  it('开关关闭时不显示历史运行态', () => {
    const account = makeAccount({
      extra: {
        ...
        codex_auto_reset_credit_state: { status: 'success', available_count: 1 REDACTED,
      REDACTED,
    REDACTED)
    const wrapper = mount(OpenAIQuotaResetCell, { props: { account REDACTED REDACTED)
