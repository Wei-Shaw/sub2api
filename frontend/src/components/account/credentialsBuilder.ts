...
export function splitHeaderOverridesObject(record: unknown): HeaderOverrideRow[] {
  ...
  return Object.entries(record as Record<string, unknown>)
    ...
    .map(([name, value]) => ({ name, value: value as string REDACTED))
...
export function parseHeaderOverridesJson(text: string): HeaderOverrideRow[] | null {
  ...
  for (const [rawName, rawValue] of Object.entries(parsed as Record<string, unknown>)) {
    ...
    if (
      ...
      return null
    REDACTED
    rows.push({ name, value: String(rawValue).trim() REDACTED)
  REDACTED
...
export function serializeHeaderOverrideRows(rows: HeaderOverrideRow[]): string {
  const record: Record<string, string> = {REDACTED
  for (const row of rows) {
    const name = row.name.trim()
    if (!name) continue
...
export const GROK_BASE_URL_PRESETS: GrokBaseUrlPreset[] = [
  { labelKey: 'cli', url: 'https://cli-chat-proxy.grok.com/v1' REDACTED,
  { labelKey: 'official', url: 'https://api.x.ai/v1' REDACTED,
  { label: 'us-east-1', url: 'https://us-east-1.api.x.ai/v1' REDACTED,
  { label: 'us-west-2', url: 'https://us-west-2.api.x.ai/v1' REDACTED,
  { label: 'eu-west-1', url: 'https://eu-west-1.api.x.ai/v1' REDACTED
]
...
export const CN_BASE_URL_PRESETS: Record<'kimi' | 'zhipu' | 'deepseek', CnBaseUrlPreset[]> = {
  kimi: [
    { mode: 'payg', protocol: 'chat_completions', label: 'Moonshot', url: 'https://api.moonshot.cn/v1' REDACTED,
    { mode: 'payg', protocol: 'anthropic', label: 'Moonshot Anthropic', url: 'https://api.moonshot.cn/anthropic' REDACTED,
    { mode: 'coding', protocol: 'chat_completions', label: 'Kimi For Coding', url: 'https://api.kimi.com/coding/v1' REDACTED,
    { mode: 'coding', protocol: 'anthropic', label: 'Kimi Coding Anthropic', url: 'https://api.kimi.com/coding' REDACTED
  ],
  zhipu: [
    { mode: 'payg', protocol: 'chat_completions', label: 'GLM PaaS', url: 'https://open.bigmodel.cn/api/paas/v4' REDACTED,
    { mode: 'payg', protocol: 'anthropic', label: 'GLM Anthropic', url: 'https://open.bigmodel.cn/api/anthropic' REDACTED,
    { mode: 'coding', protocol: 'chat_completions', label: 'GLM Coding', url: 'https://open.bigmodel.cn/api/coding/paas/v4' REDACTED,
    { mode: 'coding', protocol: 'anthropic', label: 'GLM Coding Anthropic', url: 'https://open.bigmodel.cn/api/anthropic' REDACTED
  ],
  deepseek: [
    { mode: 'payg', protocol: 'chat_completions', label: 'DeepSeek', url: 'https://api.deepseek.com' REDACTED,
    { mode: 'payg', protocol: 'anthropic', label: 'DeepSeek Anthropic', url: 'https://api.deepseek.com/anthropic' REDACTED,
    { mode: 'payg', protocol: 'responses', label: 'DeepSeek Responses', url: 'https://api.deepseek.com' REDACTED
  ]
REDACTED
...
export function buildPlanTypeOptions(current: string, clearLabel: string): PlanTypeOption[] {
  ...
  const opts: PlanTypeOption[] = [{ value: '', label: clearLabel REDACTED]
  for (const p of presets) {
    if (cur && p.value !== cur.toLowerCase() && p.label === curLabel) {
      // 当前值是该预设的别名：用 canonical 当前值占位，标签仍显示友好名
      opts.push({ value: cur, label: p.label REDACTED)
    REDACTED else {
      opts.push(p)
    REDACTED
  REDACTED
  if (cur && !opts.some(o => o.value.toLowerCase() === cur.toLowerCase())) {
    opts.push({ value: cur, label: planTypeDisplayLabel(cur) REDACTED)
  REDACTED
  return opts
REDACTED
