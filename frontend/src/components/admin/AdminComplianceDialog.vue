    ...
    <div class="space-y-5">
      <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
        <div class="flex gap-3">
          ...
          <div class="space-y-2">
            <p class="font-semibold">{{ t('adminCompliance.blockingNotice') REDACTEDREDACTED</p>
            <p class="leading-6">{{ t('adminCompliance.riskNotice') REDACTEDREDACTED</p>
          </div>
        </div>
      </div>
        ...
        <aside class="space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm dark:border-dark-700 dark:bg-dark-900/60">
          <div>
            <p class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-dark-400">
              {{ t('adminCompliance.version') REDACTEDREDACTED
            </p>
            <p class="mt-1 break-all font-mono text-gray-900 dark:text-white">
              {{ complianceStore.status?.version || 'v2026.06.10' REDACTEDREDACTED
            </p>
          </div>
          <a
            :href="documentUrl"
          ...
          >
            ...
            {{ t('adminCompliance.openDocument') REDACTEDREDACTED
          </a>
          <p class="leading-6 text-gray-600 dark:text-dark-300">
            {{ t('adminCompliance.documentSource') REDACTEDREDACTED
          </p>
        </aside>
      </div>
      ...
      <p class="text-xs leading-5 text-gray-500 dark:text-dark-400">
        {{ t('adminCompliance.legalNote') REDACTEDREDACTED
      </p>
    </div>
    ...
    <template #footer>
      <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
        <button
          ...
          @click="logout"
        >
          {{ t('adminCompliance.logout') REDACTEDREDACTED
        </button>
        <button
          type="button"
          ...
          @click="submit"
        >
          <span v-if="complianceStore.submitting">{{ t('common.submitting') REDACTEDREDACTED</span>
          <span v-else>{{ t('adminCompliance.accept') REDACTEDREDACTED</span>
        </button>
      </div>
    </template>
  </BaseDialog>
...
import { useAdminComplianceStore, useAppStore, useAuthStore REDACTED from '@/stores'
...
async function submit(): Promise<void> {
  ...
  REDACTED catch (error) {
    const message = (error as { message?: string REDACTED)?.message || t('adminCompliance.acceptFailed')
