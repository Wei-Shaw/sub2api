      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <template v-if="(account.type === 'oauth' || account.type === 'setup-token') && !isShadow && !isOpenAIAgentIdentity">
...
const isOpenAIAgentIdentity = computed(() =>
  isOpenAIOAuth.value && [
    ...
    props.account?.credentials?.openai_auth_mode,
  ].some(value => typeof value === 'string' && value.trim().toLowerCase() === 'agentidentity')
)
...
const supportsPrivacy = computed(() => (isAntigravityOAuth.value || (isOpenAIOAuth.value && !isOpenAIAgentIdentity.value)) && !isShadow.value)
