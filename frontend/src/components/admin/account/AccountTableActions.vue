<template>
  <div class="flex max-w-full flex-wrap justify-end gap-3">
    ...
    <button @click="$emit('sync')" class="btn btn-secondary flex-shrink-0">{{ t('admin.accounts.syncFromCrs') REDACTEDREDACTED</button>
    <button @click="$emit('create')" class="btn btn-primary flex-shrink-0">{{ t('admin.accounts.createAccount') REDACTEDREDACTED</button>
  </div>
</template>

<script setup lang="ts">
import { useI18n REDACTED from 'vue-i18n'; defineProps(['loading']); defineEmits(['refresh', 'sync', 'create']); const { t REDACTED = useI18n()
</script>
