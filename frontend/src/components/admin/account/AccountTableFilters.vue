<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.batch_id" class="w-40" :options="batchOpts" @update:model-value="updateBatchID" @change="$emit('change')" />
  </div>
</template>
...
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; batches?: { id: number; name: string }[] }>()
...
const updateBatchID = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, batch_id: value }) }
...
const batchOpts = computed(() => [
  { value: '', label: t('admin.accounts.allBatches', '全部批次') },
  ...(props.batches || []).map(b => ({ value: String(b.id), label: b.name }))
])
</script>
