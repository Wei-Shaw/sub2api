<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        v-if="showAddForm"
        class="rounded-xl border border-primary-200 border border-accent/40 bg-accent-tint/50 p-4 dark:border-primary-800"
      >
        <div class="mb-3 text-sm font-medium text-ink-secondary">
          {{ t('admin.scheduledTests.addPlan') }}
        </div>
        ...
        <div class="mt-3 flex justify-end gap-2">
          <button
            ...
            class="rounded-lg bg-gray-100 px-3 py-1.5 text-sm font-medium text-ink-secondary transition-colors hover:bg-gray-200 dark:bg-dark-600 dark:hover:bg-dark-500"
          >
            {{ t('common.cancel') }}
          </button>
          <button
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-ink-tertiary" :stroke-width="2" />
        <span class="ml-2 text-sm text-ink-secondary">{{ t('common.loading') }}...</span>
      </div>
      ...
      >
        <Icon name="calendar" size="lg" class="mx-auto mb-2 text-ink-tertiary" :stroke-width="1.5" />
        <p class="text-sm text-ink-secondary">
      ...
      <div v-else class="space-y-3">
        ...
        >
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              ...
              <div class="min-w-0">
                ...
                <div class="mt-0.5 font-mono text-xs text-ink-secondary">
              ...
              <span
                ...
                class="inline-flex items-center rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-success/20 dark:text-emerald-400"
              >
                {{ t('admin.scheduledTests.autoRecover') }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-3">
              <!-- Last Run -->
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-ink-secondary sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') }}</div>
                <div>{{ formatDateTime(plan.last_run_at) }}</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-ink-secondary sm:block">
              ...
              <div class="flex items-center gap-1" @click.stop>
                <button
                  ...
                  class="rounded-lg p-1.5 text-ink-tertiary transition-colors hover:bg-blue-50 hover:text-blue-500 dark:hover:bg-blue-900/20"
                ...
                <button
                  ...
                  class="rounded-lg p-1.5 text-ink-tertiary transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"
          ...
          <div
            ...
            class="border-t border-blue-100 border border-info/40 bg-info-tint/50 px-4 py-3 dark:border-blue-900"
            @click.stop
          >
            <div class="mb-2 text-xs font-medium text-ink-secondary">
          ...
          <div
            ...
            class="border-t border-line-subtle px-4 py-3"
          >
            <div class="mb-2 text-xs font-medium text-ink-secondary">
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-ink-tertiary" :stroke-width="2" />
              <span class="ml-2 text-xs text-ink-secondary">{{ t('common.loading') }}...</span>
            </div>

            <!-- No Results -->
            <div
              v-else-if="results.length === 0"
              class="py-4 text-center text-xs text-ink-secondary"
            >
            ...
            <div v-else class="max-h-64 space-y-2 overflow-y-auto">
              <div
                ...
                :key="result.id"
                class="rounded-lg border border-line-subtle bg-gray-50 p-3 dark:bg-dark-900"
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      :class="[
                        ...
                        result.status === 'success'
                          ? 'bg-green-100 text-green-700 dark:bg-success/20 dark:text-green-400'
                          : result.status === 'running'
                            ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400'
                            : 'bg-red-100 text-red-700 dark:bg-danger/20 dark:text-red-400'
                      ]"
                    >
                      {{
                        result.status === 'success'
                    ...
                    <!-- Latency -->
                    <span v-if="result.latency_ms > 0" class="text-xs text-ink-secondary">
                ...
                <div v-if="result.error_message" class="mt-2">
                  ...
                  <pre
                    ...
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded border border-danger/40 bg-danger-tint p-2 text-xs text-danger"
                  >{{ result.error_message }}</pre>
                </div>
                <div v-else-if="result.response_text" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-ink-secondary"
                    @click="toggleResultDetail(result.id)"
                  >
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-gray-100 p-2 text-xs text-ink-secondary dark:bg-dark-800"
