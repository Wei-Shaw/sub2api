<template>
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <template v-if="mode === 'token'">
      <div class="w-20">
        <label class="text-xs text-ink-tertiary">{{ t('admin.channels.form.minTokens') }}</label>
      ...
      <div class="w-20">
        <label class="text-xs text-ink-tertiary">{{ t('admin.channels.form.maxTokens') }} <span class="text-gray-300">{{ t('admin.channels.form.inclusive') }}</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-ink-tertiary">{{ t('admin.channels.form.inputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">$/M</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-ink-tertiary">{{ t('admin.channels.form.outputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">$/M</span></label>
        <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-ink-tertiary">{{ t('admin.channels.form.cacheWritePriceShort') }} <span class="text-gray-300">$/M</span></label>
        <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-ink-tertiary">{{ t('admin.channels.form.cacheReadPriceShort') }} <span class="text-gray-300">$/M</span></label>
    ...
    <template v-else>
      <div class="w-24">
        <label class="text-xs text-ink-tertiary">
      ...
      <div class="flex-1">
        <label class="text-xs text-ink-tertiary">{{ t('admin.channels.form.perRequestPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">$</span></label>
        <input :value="interval.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>

    <button type="button" @click="emit('remove')" class="mt-4 rounded p-0.5 text-ink-tertiary hover:text-red-500">
      <Icon name="x" size="sm" />
    </button>
  </div>
</template>
