<template>
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <!-- Per-request mode: tier label + context range + price -->
    <template v-if="mode === 'per_request'">
      <div class="w-24">
    ...
    <!-- Image mode: pixel area range + price -->
    <template v-else-if="mode === 'image'">
      <div class="w-24">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.resolution') }}</label>
        <input :value="interval.tier_label" @input="emitField('tier_label', ($event.target as HTMLInputElement).value)"
          type="text" class="input mt-0.5 text-xs" placeholder="1K / 2K / 4K" />
      </div>
      <div class="w-28">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.minArea', '最小面积') }}</label>
        <input :value="interval.min_tokens" @input="emitField('min_tokens', toInt(($event.target as HTMLInputElement).value))"
          type="number" min="0" class="input mt-0.5 text-xs" placeholder="0" />
      </div>
      <div class="w-28">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.maxArea', '最大面积') }}</label>
        <input :value="interval.max_tokens ?? ''" @input="emitField('max_tokens', toIntOrNull(($event.target as HTMLInputElement).value))"
          type="number" min="0" class="input mt-0.5 text-xs" placeholder="∞" />
      </div>
      <div class="flex-1">
