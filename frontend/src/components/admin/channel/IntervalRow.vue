<template>
  <div class="flex items-start gap-2 rounded border border-gray-200 bg-white p-2 dark:border-dark-500 dark:bg-dark-700">
    ...
    <template v-if="mode === 'token'">
      <div class="w-20">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.minTokens', 'Min (K)') REDACTEDREDACTED</label>
        <input
      ...
      <div class="w-20">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.maxTokens', 'Max (K)') REDACTEDREDACTED</label>
        <input
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePrice', 'Cache W') REDACTEDREDACTED</label>
        <input
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPrice', 'Cache R') REDACTEDREDACTED</label>
        <input
    ...
    <template v-else>
      ...
      <div class="w-20">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.minTokens', 'Min') REDACTEDREDACTED</label>
        <input
          :value="interval.min_tokens"
          @input="emitField('min_tokens', toInt(($event.target as HTMLInputElement).value))"
          type="number"
      ...
      <div class="w-20">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.maxTokens', 'Max') REDACTEDREDACTED</label>
        <input
          :value="interval.max_tokens ?? ''"
          @input="emitField('max_tokens', toIntOrNull(($event.target as HTMLInputElement).value))"
          type="number"
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.perRequestPrice', 'Price') REDACTEDREDACTED</label>
        <input
