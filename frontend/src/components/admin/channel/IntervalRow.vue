  ...
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <template v-if="mode === 'token'">
      <div class="grid min-w-0 flex-1 grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-6">
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.minTokens') REDACTEDREDACTED</label>
        ...
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.maxTokens') REDACTEDREDACTED <span class="text-gray-300">{{ t('admin.channels.form.inclusive') REDACTEDREDACTED</span></label>
        ...
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputPrice') REDACTEDREDACTED <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">$/M</span></label>
        ...
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPrice') REDACTEDREDACTED <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">$/M</span></label>
          <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePriceShort') REDACTEDREDACTED <span class="text-gray-300">$/M</span></label>
          <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPriceShort') REDACTEDREDACTED <span class="text-gray-300">$/M</span></label>
        ...
        <template v-if="enableMultipliers">
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputMultiplier') REDACTEDREDACTED</label>
          ...
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputMultiplier') REDACTEDREDACTED</label>
            <input :value="interval.output_multiplier" @input="emitField('output_multiplier', ($event.target as HTMLInputElement).value)"
              type="number" step="any" min="0.000001" class="input mt-0.5 text-xs" />
          </div>
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWriteMultiplier') REDACTEDREDACTED</label>
          ...
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadMultiplier') REDACTEDREDACTED</label>
    ...
    <template v-else>
      <div class="w-24">
        <label class="text-xs text-gray-400">
          {{ mode === 'image' ? t('admin.channels.form.resolution') : t('admin.channels.form.tierLabel') REDACTEDREDACTED
        </label>
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.perRequestPrice') REDACTEDREDACTED <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">$</span></label>
...
import type { IntervalFormEntry REDACTED from './types'
...
function emitField(field: keyof IntervalFormEntry, value: string | number | null) {
  emit('update', { ...props.interval, [field]: value === '' ? null : value REDACTED)
REDACTED
