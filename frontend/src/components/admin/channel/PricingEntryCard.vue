<template>
  <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
    ...
    >
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          >
            +{{ entry.models.length - 3 REDACTEDREDACTED
          </span>
          <span
            v-if="entry.models.length === 0"
            class="text-xs italic text-gray-400"
          >
            {{ t('admin.channels.form.noModels') REDACTEDREDACTED
          </span>
        </div>
        ...
        >
          {{ billingModeLabel REDACTEDREDACTED
        </span>
      </div>
      ...
      <div v-else class="flex-1 text-xs font-medium text-gray-500 dark:text-gray-400">
        {{ t('admin.channels.form.pricingEntry') REDACTEDREDACTED
      </div>
    ...
    <div
      ...
      :class="{ 'collapsible-content--collapsed': collapsed REDACTED"
    >
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.models') REDACTEDREDACTED <span class="text-red-500">*</span>
            </label>
            <ModelTagInput
              :models="entry.models"
          ...
          <div class="w-40">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.billingMode') REDACTEDREDACTED
            </label>
            <Select
              ...
              @update:modelValue="emit('update', {
                ...
                intervals: [],
                time_pricing: { ...entry.time_pricing, periods: [] REDACTED,
              REDACTED)"
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultPrices') REDACTEDREDACTED
            <span class="ml-1 font-normal text-gray-400">$/MTok</span>
          </label>
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-6">
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputPrice') REDACTEDREDACTED</label>
            ...
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPrice') REDACTEDREDACTED</label>
              <input :value="entry.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePrice') REDACTEDREDACTED</label>
              <input :value="entry.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPrice') REDACTEDREDACTED</label>
              <input :value="entry.cache_read_price" @input="emitField('cache_read_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.imageInputPrice') REDACTEDREDACTED</label>
              <input :value="entry.image_input_price" @input="emitField('image_input_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.imageTokenPrice') REDACTEDREDACTED</label>
          ...
          <div v-if="enableTierMultipliers" class="mt-3 grid max-w-md grid-cols-2 gap-2">
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.fastMultiplier') REDACTEDREDACTED</label>
            ...
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.flexMultiplier') REDACTEDREDACTED</label>
          ...
          <div v-if="!hideTokenIntervals" class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
                {{ t('admin.channels.form.intervals') REDACTEDREDACTED
              ...
              <button type="button" @click="addInterval" class="text-xs text-primary-600 hover:text-primary-700">
                + {{ t('admin.channels.form.addInterval') REDACTEDREDACTED
              </button>
            </div>
          ...
          <TimePricingSection
            ...
            @update:model-value="emit('update', { ...entry, time_pricing: $event REDACTED)"
          />
        </div>
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultPerRequestPrice') REDACTEDREDACTED
          ...
          <div class="mt-3 flex items-center justify-between">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.requestTiers') REDACTEDREDACTED
            </label>
            <button type="button" @click="addInterval" class="text-xs text-primary-600 hover:text-primary-700">
              + {{ t('admin.channels.form.addTier') REDACTEDREDACTED
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
            <IntervalRow
          ...
          <div v-else class="mt-2 rounded border border-dashed border-gray-300 p-3 text-center text-xs text-gray-400 dark:border-dark-500">
            {{ t('admin.channels.form.noTiersYet') REDACTEDREDACTED
          </div>
        </div>
        ...
        <div v-else-if="entry.billing_mode === 'image' || entry.billing_mode === 'video'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ entry.billing_mode === 'video' ? t('admin.channels.form.defaultVideoPrice') : t('admin.channels.form.defaultImagePrice') REDACTEDREDACTED
            <span class="ml-1 font-normal text-gray-400">$</span>
          </label>
          <div class="mt-1 w-48">
            <input :value="entry.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          ...
          <div class="mt-3 flex items-center justify-between">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ entry.billing_mode === 'video' ? t('admin.channels.form.videoTiers') : t('admin.channels.form.imageTiers') REDACTEDREDACTED
            </label>
...
import { perTokenToMTok, getPlatformTagClass REDACTED from './types'
...
const billingModeOptions = computed(() => [
  { value: 'token', label: t('admin.channels.billingMode.token') REDACTED,
  { value: 'per_request', label: t('admin.channels.billingMode.perRequest') REDACTED,
  { value: 'image', label: t('admin.channels.billingMode.image') REDACTED,
  { value: 'video', label: t('admin.channels.billingMode.video') REDACTED
])
...
async function onModelsUpdate(newModels: string[]) {
  ...
  emit('update', { ...props.entry, models: newModels REDACTED)
