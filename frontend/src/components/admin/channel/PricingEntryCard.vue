    ...
    >
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          >
            {{ t('admin.channels.form.noModels', 'No models added') REDACTEDREDACTED
          </span>
        </div>
      ...
      <div v-else class="flex-1 text-xs font-medium text-gray-500 dark:text-gray-400">
        {{ t('admin.channels.form.pricingEntry', 'Pricing Entry') REDACTEDREDACTED
      </div>
      ...
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.models', 'Models') REDACTEDREDACTED <span class="text-red-500">*</span>
            </label>
            <ModelTagInput
              :models="entry.models"
          ...
          <div class="w-40">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.billingMode', 'Billing Mode') REDACTEDREDACTED
            </label>
            <Select
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultPrices', 'Default prices (fallback when no interval matches)') REDACTEDREDACTED
            <span class="ml-1 font-normal text-gray-400">$/MTok</span>
          </label>
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-5">
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputPrice', 'Input') REDACTEDREDACTED</label>
            ...
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPrice', 'Output') REDACTEDREDACTED</label>
              <input :value="entry.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', 'Default')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePrice', 'Cache Write') REDACTEDREDACTED</label>
              <input :value="entry.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', 'Default')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPrice', 'Cache Read') REDACTEDREDACTED</label>
              <input :value="entry.cache_read_price" @input="emitField('cache_read_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', 'Default')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.imageTokenPrice', 'Image Output') REDACTEDREDACTED</label>
          ...
          <div class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
                {{ t('admin.channels.form.intervals', 'Context intervals (optional)') REDACTEDREDACTED
              ...
              <button type="button" @click="addInterval" class="text-xs text-primary-600 hover:text-primary-700">
                + {{ t('admin.channels.form.addInterval', 'Add interval') REDACTEDREDACTED
              </button>
            </div>
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultPerRequestPrice', 'Default per-request price (fallback when no tier matches)') REDACTEDREDACTED
          ...
          <div class="mt-3 flex items-center justify-between">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.requestTiers', 'Request Tiers') REDACTEDREDACTED
            </label>
            <button type="button" @click="addInterval" class="text-xs text-primary-600 hover:text-primary-700">
              + {{ t('admin.channels.form.addTier', 'Add tier') REDACTEDREDACTED
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
            <IntervalRow
          ...
          <div v-else class="mt-2 rounded border border-dashed border-gray-300 p-3 text-center text-xs text-gray-400 dark:border-dark-500">
            {{ t('admin.channels.form.noTiersYet', 'No tiers yet. Click add to configure per-request pricing.') REDACTEDREDACTED
          </div>
        </div>
        ...
        <div v-else-if="entry.billing_mode === 'image'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultImagePrice', 'Default image price (fallback when no tier matches)') REDACTEDREDACTED
            <span class="ml-1 font-normal text-gray-400">$</span>
          </label>
          <div class="mt-1 w-48">
            <input :value="entry.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          ...
          <div class="mt-3 flex items-center justify-between">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.imageTiers', 'Image tiers (per request)') REDACTEDREDACTED
            </label>
...
const billingModeOptions = computed(() => [
  ...
  { value: 'per_request', label: t('admin.channels.billingMode.perRequest', 'Per Request') REDACTED,
  { value: 'image', label: t('admin.channels.billingMode.image', 'Image (per request)') REDACTED
])
