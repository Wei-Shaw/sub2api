...
import { createDefaultTimePricingForm REDACTED from '../types'
...
describe('TimePricingSection', () => {
  ...
  it('offers every-day and weekdays-only scopes', async () => {
    ...
    const wrapper = mount(TimePricingSection, {
      props: { modelValue: value REDACTED,
      global: { stubs: { Select: SelectStub, Icon: true REDACTED REDACTED,
    REDACTED)
    ...
    expect(selects[1].props('options')).toEqual([
      { value: false, label: 'admin.channels.form.timePricingEveryDay' REDACTED,
      { value: true, label: 'admin.channels.form.timePricingWeekdaysOnly' REDACTED,
    ])
  ...
  it('adds a neutral period immutably', async () => {
    ...
    expect(wrapper.emitted('update:modelValue')?.[0]?.[0]).toEqual({
      ...
      weekdays_only: false,
      periods: [{ start_time: '', end_time: '', multiplier: '1.00' REDACTED],
    REDACTED)
  ...
  it('removes without mutating props', async () => {
    const value = {
      timezone: 'Asia/Shanghai',
      periods: [{ start_time: '09:00:00', end_time: '12:00:00', multiplier: '2.00' REDACTED],
    REDACTED
    const wrapper = mount(TimePricingSection, {
      props: { modelValue: value REDACTED,
      global: { stubs: { Select: SelectStub, Icon: true REDACTED REDACTED,
  ...
  it('updates the timezone and normalizes full-width colons immutably', async () => {
    ...
    expect(wrapper.emitted('update:modelValue')?.[1]?.[0]).toEqual({
      ...value,
      periods: [{ ...value.periods[0], start_time: '10:30:15' REDACTED],
    REDACTED)
  ...
  it('normalizes 24:00:00 to 00:00:00', async () => {
    ...
    expect(wrapper.emitted('update:modelValue')?.[0]?.[0]).toEqual({
      ...value,
      periods: [{ ...value.periods[0], start_time: '00:00:00' REDACTED],
    REDACTED)
  REDACTED)
  ...
  it('formats only a valid positive multiplier on blur', async () => {
    const value = {
      timezone: 'Asia/Shanghai',
      periods: [{ start_time: '09:00:00', end_time: '12:00:00', multiplier: '2' REDACTED],
    REDACTED
    const wrapper = mount(TimePricingSection, {
      props: { modelValue: value REDACTED,
      global: { stubs: { Select: SelectStub, Icon: true REDACTED REDACTED,
    ...
    expect(wrapper.emitted('update:modelValue')?.[0]?.[0]).toEqual({
      ...value,
      periods: [{ ...value.periods[0], multiplier: '2.00' REDACTED],
    REDACTED)
  ...
  it.each(['1.234', '1e2', '.5'])('keeps invalid multiplier %s unchanged on blur', async invalid => {
    const value = {
      timezone: 'Asia/Shanghai',
      periods: [{ start_time: '09:00:00', end_time: '12:00:00', multiplier: invalid REDACTED],
    REDACTED
    const wrapper = mount(TimePricingSection, {
      props: { modelValue: value REDACTED,
      global: { stubs: { Select: SelectStub, Icon: true REDACTED REDACTED,
  ...
  it('uses unique input ids across component instances', () => {
    ...
    REDACTED
    const wrapper = mount({
      components: { TimePricingSection REDACTED,
      data: () => ({ first: value, second: value REDACTED),
      template: `
        <div>
