  ...
  <BaseDialog :show="show" :title="t('admin.groups.rpmOverridesTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-gray-50 px-4 py-2.5 text-sm dark:bg-dark-700">
        ...
        <span class="font-medium text-gray-900 dark:text-white">{{ group.name REDACTEDREDACTED</span>
        <span class="text-gray-400">|</span>
        <span class="text-gray-600 dark:text-gray-400">
          {{ t('admin.groups.groupRpmDefault') REDACTEDREDACTED: {{ group.rpm_limit || 0 REDACTEDREDACTED
        </span>
      </div>
      ...
      <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
        <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
          {{ t('admin.groups.addUserRpm') REDACTEDREDACTED
        </h4>
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            >
              ...
              >
                <span class="text-gray-400">#{{ user.id REDACTEDREDACTED</span>
                <span class="text-gray-900 dark:text-white">{{ user.username || user.email REDACTEDREDACTED</span>
                <span v-if="user.username" class="text-xs text-gray-400">{{ user.email REDACTEDREDACTED</span>
              </button>
            </div>
          </div>
          <div class="w-24">
        ...
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center justify-end border-t border-gray-100 pt-3 dark:border-dark-600">
          ...
          >
            ...
            {{ t('admin.groups.clearAll') REDACTEDREDACTED
          </button>
        </div>
      </div>
      ...
      <div v-else>
        <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
          {{ t('admin.groups.rpmOverrides') REDACTEDREDACTED ({{ localEntries.length REDACTEDREDACTED)
        </h4>

        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-gray-400 dark:text-gray-500">
          {{ t('admin.groups.noRpmOverrides') REDACTEDREDACTED
        </div>
              ...
              <table class="w-full min-w-max text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-600 dark:bg-dark-700">
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userEmail') REDACTEDREDACTED</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userName') REDACTEDREDACTED</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userNotes') REDACTEDREDACTED</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userStatus') REDACTEDREDACTED</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400" :title="t('admin.groups.columns.rpmOverrideHint')">{{ t('admin.groups.columns.rpmOverride') REDACTEDREDACTED</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-600">
                  ...
                  >
                    <td class="px-3 py-2 text-gray-600 dark:text-gray-400">{{ entry.user_email REDACTEDREDACTED</td>
                    <td class="whitespace-nowrap px-3 py-2 text-gray-400 dark:text-gray-500">{{ entry.user_id REDACTEDREDACTED</td>
                    <td class="whitespace-nowrap px-3 py-2 text-gray-900 dark:text-white">{{ entry.user_name || '-' REDACTEDREDACTED</td>
                    <td class="max-w-[160px] truncate px-3 py-2 text-gray-500 dark:text-gray-400" :title="entry.user_notes">{{ entry.user_notes || '-' REDACTEDREDACTED</td>
                    <td class="whitespace-nowrap px-3 py-2">
                      ...
                      >
                        {{ entry.user_status REDACTEDREDACTED
                      </span>
                    </td>
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
      ...
      <div class="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-dark-600">
        <template v-if="isDirty">
          <span class="text-xs text-amber-600 dark:text-amber-400">{{ t('admin.groups.unsavedChanges') REDACTEDREDACTED</span>
          <button
            type="button"
            class="text-xs font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300"
            @click="handleCancel"
          >
            {{ t('admin.groups.revertChanges') REDACTEDREDACTED
          </button>
        </template>
...
import type { GroupRPMOverrideEntry REDACTED from '@/api/admin/groups'
import type { AdminGroup, AdminUser REDACTED from '@/types'
import BaseDialog from '@/components/common/BaseDialog.vue'
import Pagination from '@/components/common/Pagination.vue'
import Icon from '@/components/icons/Icon.vue'
import PlatformIcon from '@/components/common/PlatformIcon.vue'

interface LocalEntry extends GroupRPMOverrideEntry {REDACTED
...
const cloneEntries = (entries: GroupRPMOverrideEntry[]): LocalEntry[] => {
  return entries.map(e => ({ ...e REDACTED))
REDACTED
...
const handleSearchUsers = () => {
  ...
  searchTimeout = setTimeout(async () => {
    try {
      const res = await adminAPI.users.list(1, 10, { search: searchQuery.value.trim() REDACTED)
...
const handleClickOutside = () => { showDropdown.value = false REDACTED
