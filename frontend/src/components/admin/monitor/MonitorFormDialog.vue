<template>
  ...
  >
    <form id="channel-monitor-form" @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.checkMode') REDACTEDREDACTED</label>
        <div class="grid gap-3 sm:grid-cols-3" data-testid="monitor-check-mode">
          <button
            v-for="opt in checkModeOptions"
            :key="opt.value"
            type="button"
            :data-testid="`monitor-check-mode-${opt.valueREDACTED`"
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <button
            ...
            :key="opt.value"
            type="button"
            :data-testid="`monitor-provider-${opt.valueREDACTED`"
      ...
      <div v-if="usesQuotaMode">
        <label class="input-label">
          {{ t('admin.channelMonitor.form.linkedAccount') REDACTEDREDACTED <span class="text-red-500">*</span>
        </label>
        ...
        <p class="mt-1 text-xs text-gray-400">{{ t('admin.channelMonitor.form.linkedAccountHint') REDACTEDREDACTED</p>
        <p v-if="form.provider === PROVIDER_OPENAI" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
          {{ t('admin.channelMonitor.form.openAIQuotaProbeHint') REDACTEDREDACTED
        </p>
        <p v-if="accountHydrationFailed" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
          {{ t('admin.channelMonitor.form.linkedAccountMissing') REDACTEDREDACTED
        </p>
        <p v-if="accountOptions.length === 0 && !accountsLoading && !accountSearchQuery" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
          {{ t('admin.channelMonitor.form.linkedAccountEmpty') REDACTEDREDACTED
        </p>
      </div>
      ...
      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.jitterSeconds') REDACTEDREDACTED</label>
        <input v-model.number="form.jitter_seconds" type="number" min="0" :max="maxJitterSeconds" class="input" />
        <p class="mt-1 text-xs text-gray-400">{{ t('admin.channelMonitor.form.jitterSecondsHint') REDACTEDREDACTED</p>
      </div>
      ...
      <details v-if="usesProbePart" class="rounded-lg border border-gray-200 bg-gray-50/50 p-3 dark:border-dark-700 dark:bg-dark-900/30">
        <summary class="cursor-pointer text-sm font-medium text-gray-700 dark:text-gray-300">
          {{ t('admin.channelMonitor.advanced.section') REDACTEDREDACTED
        </summary>
        <p class="mt-1 text-xs text-gray-400">{{ t('admin.channelMonitor.advanced.sectionHint') REDACTEDREDACTED</p>

        <div class="mt-4 space-y-4">
          <div>
            <label class="input-label">{{ t('admin.channelMonitor.templateField.label') REDACTEDREDACTED</label>
            <Select
              v-model="templateSelectValue"
              :options="templateOptions"
              :placeholder="t('admin.channelMonitor.templateField.placeholder')"
            />
            <p class="mt-1 text-xs text-gray-400">{{ t('admin.channelMonitor.templateField.applyHint') REDACTEDREDACTED</p>
          </div>
...
const templateOptions = computed(() => {
  ...
  REDACTED)
  return [
    { value: '', label: t('admin.channelMonitor.templateField.none') REDACTED,
    ...items.map((t) => ({ value: String(t.id), label: templateOptionLabel(t) REDACTED)),
  ]
REDACTED)
...
function templateOptionLabel(tpl: ChannelMonitorTemplate): string {
  ...
  return `${tpl.nameREDACTED · ${t(labelKey)REDACTED`
REDACTED

function clearRequestSnapshot() {
  form.template_id = null
  form.extra_headers = {REDACTED
...
const accountOptions = computed(() => {
  const opts = linkedAccounts.value.map((a) => ({
    value: String(a.id),
    label: `${a.nameREDACTED (#${a.idREDACTED)`,
  REDACTED))
  const pinned = pinnedAccount.value
  if (pinned && !linkedAccounts.value.some((a) => a.id === pinned.id)) {
    opts.unshift({ value: String(pinned.id), label: `${pinned.nameREDACTED (#${pinned.idREDACTED)` REDACTED)
  REDACTED
  return opts
REDACTED)
...
async function loadLinkedAccounts(search = '') {
  ...
  try {
    const res = await adminAPI.accounts.list(
      1,
      50,
      { platform: form.provider, ...(search ? { search REDACTED : {REDACTED) REDACTED,
      { signal: controller.signal REDACTED,
    )
    if (seq !== accountSearchSeq) return
    linkedAccounts.value = (res.items || []).map((a) => ({ id: a.id, name: a.name REDACTED))
...
async function ensureSelectedAccountHydrated() {
  ...
  try {
    ...
    if (String(account.platform) !== form.provider) {
      ...
      return
    REDACTED
    pinnedAccount.value = { id: account.id, name: account.name REDACTED
  REDACTED catch {
...
function loadFromMonitor(m: ChannelMonitor) {
  ...
  form.extra_headers = { ...(m.extra_headers || {REDACTED) REDACTED
  form.body_override_mode = m.body_override_mode || 'off'
  form.body_override = m.body_override ? { ...m.body_override REDACTED : null
  suppressFormWatchers = false
REDACTED
...
async function handleSubmit() {
  ...
  try {
    ...
    if (target) {
      ...
      const req: UpdateParams = { ...rest REDACTED
