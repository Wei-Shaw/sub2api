  ...
  >
    <div class="space-y-3">
      ...
      <div class="relative">
        ...
        />
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-ink-tertiary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
        </svg>
      </div>

      <div v-if="loading" class="py-6 text-center text-sm text-ink-secondary">
        {{ t('common.loading') }}
      </div>
      <div v-else-if="filteredKeys.length === 0" class="py-6 text-center text-sm text-ink-secondary">
        {{ t('admin.channelMonitor.form.noActiveKey') }}
      </div>
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-line">
        <table class="w-full text-sm">
          <thead class="bg-surface-sunken sticky top-0 z-10">
            <tr class="text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">
          ...
          <tbody class="divide-y divide-line">
            ...
            >
              ...
              <td class="px-3 py-2 font-mono text-xs text-ink-secondary">{{ maskApiKey(k.key) }}</td>
              <td class="px-3 py-2">
                <GroupBadge
                  v-if="k.group"
                  :name="k.group.name"
