<template>
  ...
  >
    ...
    <div v-if="loading" class="py-6 text-center text-sm text-ink-tertiary">
      {{ t('common.loading') }}
    </div>

    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-ink-tertiary">
    ...
    <div v-else>
      ...
      <div class="mb-2 flex items-center gap-3 text-xs">
        ...
        <button
          type="button"
          class="text-ink-secondary hover:underline dark:text-gray-400"
          @click="selectNone"
        >
          {{ t('admin.channelMonitor.template.selectNone') }}
        </button>
        <span class="ml-auto text-ink-secondary">
      ...
      <ul class="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded-lg border border-line dark:divide-dark-700">
        <li
          v-for="m in monitors"
          :key="m.id"
        ...
        >
          ...
          <span class="text-xs text-ink-tertiary">{{ m.provider }}</span>
          <span v-if="m.provider === 'openai'" class="text-xs text-ink-tertiary">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-surface-sunken px-1.5 py-0.5 text-xs text-ink-secondary dark:text-gray-400"
          >
