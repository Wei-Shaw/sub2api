<template>
  ...
  >
      ...
      <div role="tablist" class="flex flex-wrap gap-1">
        ...
        >
          ...
          <span
            ...
            class="ml-1.5 rounded-full bg-surface-sunken px-2 py-0.5 text-xs"
          >
    ...
    <div v-if="!editing" class="space-y-2">
      ...
      <div v-if="loading" class="py-8 text-center text-sm text-ink-tertiary">
        {{ t('common.loading') }}
      </div>

      <div
        v-else-if="templatesForActiveProvider.length === 0"
        class="py-8 text-center text-sm text-ink-tertiary"
      >
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            ...
            <p v-if="tpl.description" class="mt-0.5 text-xs text-ink-secondary">
              {{ tpl.description }}
            </p>
...
function tabClass(value: Provider): string {
  return activeProvider.value === value
    ...
    : 'border-b-2 border-transparent text-ink-secondary hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'
}

function modeBadgeClass(mode: BodyOverrideMode): string {
  switch (mode) {
    case 'merge':
      return 'bg-amber-100 text-warn dark:bg-warn/15'
    case 'replace':
      return 'bg-purple-100 text-purple-700 dark:bg-purple-500/15 dark:text-purple-300'
    default:
      return 'bg-gray-100 text-ink-secondary dark:bg-dark-700 dark:text-gray-300'
  }
}
...
function apiModeButtonClass(mode: APIMode): string {
  ...
  if (active) {
    return 'border-primary-500 bg-white text-accent shadow-sm dark:border-primary-400 dark:bg-primary-500/15'
  }
  return 'border-blue-100 bg-white/70 text-ink-secondary hover:border-primary-300 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-400'
}
...
function apiModeBadgeClass(mode: APIMode): string {
  if (normalizeAPIMode(mode) === API_MODE_RESPONSES) {
    return 'bg-blue-100 text-info dark:bg-blue-500/15'
  }
  return 'bg-emerald-100 text-success dark:bg-success/15'
}
</script>
