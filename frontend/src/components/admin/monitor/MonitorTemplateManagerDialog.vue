...
// --- apply to monitors ---
const confirmApply_ = reactive<{ show: boolean; tpl: ChannelMonitorTemplate | null REDACTED>({
  show: false,
  tpl: null,
REDACTED)
...
async function doApply() {
  ...
  if (!tpl) return
  try {
    const { affected REDACTED = await adminAPI.channelMonitorTemplate.apply(tpl.id)
    appStore.showSuccess(t('admin.channelMonitor.template.applySuccess', { n: affected REDACTED))
    await fetchTemplates()
    emit('updated')
  REDACTED catch (err: unknown) {
    appStore.showError(extractApiErrorMessage(err, t('common.error')))
