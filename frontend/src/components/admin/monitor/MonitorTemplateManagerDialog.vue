    ...
    <div class="mb-4 border-b border-gray-200 dark:border-dark-700">
      <div role="tablist" class="flex flex-wrap gap-1">
        ...
        >
          ...
          >
            {{ countByProvider[tab.value] REDACTEDREDACTED
          </span>
        </button>
      </div>
    </div>
    ...
    <div v-if="!editing" class="space-y-2">
      <div class="flex justify-end">
        <button class="btn btn-primary btn-sm" @click="openCreateForm">
          ...
          {{ t('admin.channelMonitor.template.createButton') REDACTEDREDACTED
        </button>
      </div>
      ...
      >
        {{ t('admin.channelMonitor.template.emptyState') REDACTEDREDACTED
      </div>
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-gray-900 dark:text-white">{{ tpl.name REDACTEDREDACTED</span>
              <span
                class="inline-flex items-center rounded-md px-1.5 py-0.5 text-xs"
                :class="modeBadgeClass(tpl.body_override_mode)"
              >
                {{ modeLabel(tpl.body_override_mode) REDACTEDREDACTED
              </span>
              <span
              ...
              >
                {{ apiModeLabel(tpl.api_mode) REDACTEDREDACTED
              </span>
              <span
                v-if="tpl.associated_monitors > 0"
                class="text-xs text-gray-500 dark:text-gray-400"
              >
                {{ t('admin.channelMonitor.template.associatedCount', { n: tpl.associated_monitors REDACTED) REDACTEDREDACTED
              </span>
            </div>
            <p v-if="tpl.description" class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
              {{ tpl.description REDACTEDREDACTED
            </p>
            <p class="mt-1 text-xs text-gray-400">
              {{ t('admin.channelMonitor.template.headersSummary', {
                n: Object.keys(tpl.extra_headers || {REDACTED).length,
          ...
          <div class="flex flex-shrink-0 gap-2">
            ...
            >
              ...
              {{ t('admin.channelMonitor.template.applyButton') REDACTEDREDACTED
            </button>
    ...
    <div v-else class="space-y-4">
      <div>
        <label class="input-label">
          {{ t('admin.channelMonitor.template.form.name') REDACTEDREDACTED
      ...
      <div v-if="editing === 'new'">
        <label class="input-label">
          {{ t('admin.channelMonitor.form.provider') REDACTEDREDACTED
          <span class="text-red-500">*</span>
        </label>
      ...
      <div v-if="form.provider === PROVIDER_OPENAI" class="rounded-lg border border-blue-100 bg-blue-50/50 p-3 dark:border-blue-500/20 dark:bg-blue-500/10">
        <label class="input-label">{{ t('admin.channelMonitor.form.apiMode') REDACTEDREDACTED</label>
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          >
            <span class="block text-sm font-semibold">{{ opt.label REDACTEDREDACTED</span>
            <span class="mt-0.5 block text-xs opacity-80">{{ opt.hint REDACTEDREDACTED</span>
          </button>
        </div>
      </div>
      ...
      <div>
        <label class="input-label">
          {{ t('admin.channelMonitor.template.form.description') REDACTEDREDACTED
        </label>
        <input
      ...
      <div class="flex w-full items-center justify-between">
        ...
        <div class="flex gap-2">
          ...
          <button v-if="editing" class="btn btn-primary" :disabled="submitting" @click="handleSubmit">
            {{ submitting ? t('common.submitting') : editing === 'new' ? t('common.create') : t('common.update') REDACTEDREDACTED
          </button>
        </div>
      </div>
    </template>
...
import type { ChannelMonitorTemplate REDACTED from '@/api/admin/channelMonitorTemplate'
...
const providerTabs = computed<{ value: Provider; label: string REDACTED[]>(() => [
...
function loadForm(tpl: ChannelMonitorTemplate) {
  ...
  form.extra_headers = { ...(tpl.extra_headers || {REDACTED) REDACTED
  form.body_override_mode = tpl.body_override_mode
  form.body_override = tpl.body_override ? { ...tpl.body_override REDACTED : null
REDACTED
...
async function fetchTemplates() {
  loading.value = true
  try {
    const { items REDACTED = await adminAPI.channelMonitorTemplate.list()
...
async function handleSubmit() {
  ...
  try {
    ...
    REDACTED else if (typeof editing.value === 'number') {
...
const applyPicker = reactive<{ show: boolean; tpl: ChannelMonitorTemplate | null REDACTED>({
  show: false,
  tpl: null,
REDACTED)
...
// --- delete ---
const confirmDelete = reactive<{ show: boolean; tpl: ChannelMonitorTemplate | null REDACTED>({
  show: false,
  tpl: null,
REDACTED)
...
function modeLabel(mode: BodyOverrideMode): string {
  return t(`admin.channelMonitor.advanced.bodyMode${mode.charAt(0).toUpperCase()REDACTED${mode.slice(1)REDACTED`)
REDACTED

const apiModeOptions = computed<{ value: APIMode; label: string; hint: string REDACTED[]>(() => [
  {
