  ...
  <div class="space-y-4">
    ...
    <DataTable :columns="columns" :data="orders" :loading="loading">
      ...
      <template #cell-pay_amount="{ value, row }">
        <div class="text-sm">
          <span class="font-medium text-ink">{{ paymentAmountSymbol(row) }}{{ value.toFixed(2) }}</span>
          <span v-if="row.fee_rate > 0" class="ml-1 text-xs text-ink-tertiary" :title="t('payment.orders.fee') + ': ' + row.fee_rate + '%'">
            ({{ row.fee_rate }}%)
          </span>
          <div v-if="row.amount !== row.pay_amount" class="text-xs text-ink-secondary">
      ...
      <template #cell-created_at="{ value }">
        <span class="text-xs text-ink-secondary">{{ formatDateTime(value) }}</span>
      </template>
      ...
      <template #cell-actions="{ row }">
        <div class="flex items-center gap-2">
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-secondary transition-colors hover:bg-gray-50 hover:text-gray-700 dark:hover:bg-gray-800/50 dark:hover:text-gray-300"
          >
          ...
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-secondary transition-colors hover:bg-yellow-50 hover:text-yellow-600 dark:hover:bg-yellow-900/20 dark:hover:text-yellow-400"
          >
            <Icon name="x" size="sm" />
