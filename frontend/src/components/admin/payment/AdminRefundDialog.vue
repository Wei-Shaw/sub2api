  ...
  >
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      >
        <div class="flex items-center gap-2 text-sm font-medium text-violet-700 dark:text-violet-300">
          ...
          {{ t('payment.admin.refundRequestInfo') REDACTEDREDACTED
        </div>
        <div v-if="order?.refund_requested_at" class="mt-2 flex justify-between text-sm">
          <span class="text-violet-600 dark:text-violet-400">{{ t('payment.admin.refundRequestedAt') REDACTEDREDACTED</span>
          <span class="text-violet-800 dark:text-violet-200">{{ formatDateTime(order.refund_requested_at) REDACTEDREDACTED</span>
        </div>
        <div v-if="order?.refund_request_reason" class="mt-1 text-sm">
          <span class="text-violet-600 dark:text-violet-400">{{ t('payment.admin.refundRequestReason') REDACTEDREDACTED:</span>
          <span class="ml-1 text-violet-800 dark:text-violet-200">{{ order.refund_request_reason REDACTEDREDACTED</span>
        </div>
      </div>
      ...
      <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-700">
        <div class="flex justify-between text-sm">
          ...
          <span class="font-mono text-gray-900 dark:text-white">#{{ order?.id REDACTEDREDACTED</span>
        </div>
        ...
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-red-600 dark:text-red-400">${{ actuallyRefunded.toFixed(2) REDACTEDREDACTED</span>
        </div>
      </div>
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          <label for="deduct-balance" class="text-sm text-gray-700 dark:text-gray-300">
            {{ t('payment.admin.deductBalance') REDACTEDREDACTED
          </label>
          <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.deductBalanceHint') REDACTEDREDACTED</span>
        </div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          ...
          <div class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-700">
            ...
            <div class="mt-1 font-semibold text-gray-900 dark:text-white">${{ order?.pay_amount?.toFixed(2) REDACTEDREDACTED</div>
          </div>
        </div>
        ...
        >
          {{ t('payment.admin.insufficientBalance') REDACTEDREDACTED
        </div>
        ...
        >
          {{ t('payment.admin.noDeduction') REDACTEDREDACTED
        </div>
      </div>
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {{ t('payment.admin.maxRefundable') REDACTEDREDACTED: ${{ maxRefundable.toFixed(2) REDACTEDREDACTED
        </p>
      </div>

      <!-- Reason -->
      <div>
        <label class="input-label">{{ t('payment.admin.refundReason') REDACTEDREDACTED</label>
        <textarea
          v-model="form.reason"
          rows="3"
          class="input"
      ...
      <div v-if="requireForce" class="flex items-center gap-2">
        ...
        <label for="force-refund" class="text-sm font-medium text-red-600 dark:text-red-400">
          {{ t('payment.admin.forceRefund') REDACTEDREDACTED
        </label>
      </div>
    </form>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        >
          {{ submitting ? t('common.processing') : t('payment.admin.confirmRefund') REDACTEDREDACTED
        </button>
      </div>
    </template>
  </BaseDialog>
...
import { reactive, computed, watch REDACTED from 'vue'
