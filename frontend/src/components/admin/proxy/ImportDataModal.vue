  ...
  >
    <form id="import-proxy-data-form" class="space-y-4" @submit.prevent="handleImport">
      ...
      <div>
        ...
        <div
          class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-line bg-surface-sunken px-4 py-3"
        >
          <div class="min-w-0">
            <div class="truncate text-sm text-ink-secondary">
              {{ fileName || t('admin.proxies.dataImportSelectFile') }}
            </div>
            <div class="text-xs text-ink-secondary">JSON (.json)</div>
          </div>
