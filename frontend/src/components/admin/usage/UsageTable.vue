  ...
  <div :class="flat ? '' : 'card overflow-hidden'">
    ...
    >
      <span v-if="pendingIpCount > 0" class="text-xs text-gray-500 dark:text-gray-400">
        {{ t('usage.ipGeo.pending', { count: pendingIpCount REDACTED) REDACTEDREDACTED
      </span>
      <button
        type="button"
      ...
      >
        {{ ipGeoBatchLoading ? t('usage.ipGeo.batchFetching') : t('usage.ipGeo.batchFetch') REDACTEDREDACTED
      </button>
    </div>
      ...
      >
        <template #cell-user="{ row REDACTED">
          <div class="text-sm">
            ...
            >
              {{ row.user.email REDACTEDREDACTED
            </button>
            ...
            <span v-if="row.user?.deleted_at" class="ml-1 inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-rose-100 text-rose-600 ring-1 ring-inset ring-rose-200 dark:bg-rose-500/20 dark:text-rose-400 dark:ring-rose-500/30">
              {{ t('admin.usage.userDeletedBadge') REDACTEDREDACTED
            </span>
          ...
          <div class="space-y-0.5 text-xs">
            <div v-if="row.model_mapping_chain && row.model_mapping_chain.includes('→')" class="space-y-0.5">
              <div v-for="(step, i) in row.model_mapping_chain.split('→')" :key="i"
                   ...
                   :style="i > 0 ? `padding-left: ${i * 0.75REDACTEDrem` : ''">
                <span v-if="i > 0" class="mr-0.5">↳</span>{{ step REDACTEDREDACTED
              </div>
            </div>
            <div v-else-if="row.upstream_model && row.upstream_model !== row.model" class="space-y-0.5">
              <div class="break-all font-medium text-gray-900 dark:text-white">
                {{ row.model REDACTEDREDACTED
              </div>
              <div class="break-all text-gray-500 dark:text-gray-400">
                <span class="mr-0.5">↳</span>{{ row.upstream_model REDACTEDREDACTED
              </div>
            </div>
            <span v-else class="font-medium text-gray-900 dark:text-white">{{ row.model REDACTEDREDACTED</span>
            <div
            ...
            >
              <span class="mr-1">↳ {{ t('usage.upstreamResponseModel') REDACTEDREDACTED:</span>{{ row.upstream_response_model REDACTEDREDACTED
              <span
              ...
              >
                {{ isLikelyModelVariant(row) ? t('usage.modelVariant') : t('usage.modelMismatch') REDACTEDREDACTED
              </span>
            </div>
          </div>
        </template>
            ...
            <div class="space-y-1 text-sm">
              ...
              <div v-if="hasImageInputTokens(row)" class="flex items-center gap-2">
                <div class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-fuchsia-600 dark:text-fuchsia-400">{{ row.image_input_tokens.toLocaleString() REDACTEDREDACTED</span>
                </div>
              </div>
        ...
        <template #cell-latency="{ row REDACTED">
          <div class="flex items-stretch gap-2">
            ...
            <div class="grid grid-cols-[max-content_max-content] items-baseline gap-x-2 gap-y-0.5 text-xs">
              <span class="text-gray-400 dark:text-gray-500">{{ t('usage.latencyFirstToken') REDACTEDREDACTED</span>
              <span v-if="row.first_token_ms != null" class="font-medium tabular-nums" :class="LATENCY_TEXT_CLASSES[firstTokenSeverity(row.first_token_ms)]">{{ formatDuration(row.first_token_ms) REDACTEDREDACTED</span>
              <span v-else class="text-gray-400 dark:text-gray-500">-</span>
              <span class="text-gray-400 dark:text-gray-500">{{ t('usage.latencyDuration') REDACTEDREDACTED</span>
              <span class="font-medium tabular-nums" :class="LATENCY_TEXT_CLASSES[durationSeverity(row.duration_ms ?? 0)]">{{ formatDuration(row.duration_ms) REDACTEDREDACTED</span>
            </div>
          </div>
        </template>
        ...
        <template #cell-ip_address="{ row REDACTED">
          <div v-if="row.ip_address">
            <span class="text-sm font-mono text-gray-600 dark:text-gray-400">{{ row.ip_address REDACTEDREDACTED</span>
        ...
        <div class="space-y-1.5">
          <div>
            ...
            <div v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData) && textInputTokens(tokenTooltipData) > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.inputTokens') REDACTEDREDACTED</span>
              <span class="font-medium text-white">{{ textInputTokens(tokenTooltipData).toLocaleString() REDACTEDREDACTED</span>
            </div>
            <div v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.imageInputTokens') REDACTEDREDACTED</span>
              <span class="font-medium text-fuchsia-300">{{ tokenTooltipData.image_input_tokens.toLocaleString() REDACTEDREDACTED</span>
            </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && hasImageInputCost(tooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.imageInputCost') REDACTEDREDACTED</span>
              <span class="font-medium text-fuchsia-300">${{ tooltipData.image_input_cost.toFixed(6) REDACTEDREDACTED</span>
            </div>
            ...
            <template v-if="tooltipData && !isImageUsage(tooltipData) && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
              <div v-if="tooltipData && textInputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatTokenPricePerMillion(tooltipData.input_cost, textInputTokens(tooltipData)) REDACTEDREDACTED {{ t('usage.perMillionTokens') REDACTEDREDACTED</span>
              </div>
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.imageInputTokenPrice') REDACTEDREDACTED</span>
                <span class="font-medium text-fuchsia-300">{{ formatTokenPricePerMillion(tooltipData.image_input_cost ?? 0, tooltipData.image_input_tokens) REDACTEDREDACTED {{ t('usage.perMillionTokens') REDACTEDREDACTED</span>
              </div>
...
import {
  ...
  durationSeverity,
  firstTokenSeverity,
REDACTED from '@/utils/latencyHealth'
import {
  BILLING_MODE_TOKEN,
  getBillingModeLabel,
...
import { fetchBatch, getEntry REDACTED from '@/utils/ipGeoLookup'
...
const normalizeModelVariant = (model: string): string => model
  ...
  .toLowerCase()
  .replace(/-latest$/, '')
  .replace(/-\d{4REDACTED-\d{2REDACTED-\d{2REDACTED$/, '')
  .replace(/-\d{8REDACTED$/, '')
...
const modelAuditTitle = (row: AdminUsageLog): string => [
  `${t('usage.requestedModel')REDACTED: ${row.model || '-'REDACTED`,
  `${t('usage.sentUpstreamModel')REDACTED: ${sentUpstreamModel(row) || '-'REDACTED`,
  `${t('usage.upstreamResponseModel')REDACTED: ${row.upstream_response_model || '-'REDACTED`,
].join('\n')
...
const formatDuration = (ms: number | null | undefined): string => {
  ...
  if (ms < 60_000) return `${(ms / 1000).toFixed(2)REDACTEDs`
  const totalSec = Math.round(ms / 1000)
  if (totalSec < 3600) return `${Math.floor(totalSec / 60)REDACTEDm ${totalSec % 60REDACTEDs`
  return `${Math.floor(totalSec / 3600)REDACTEDh ${Math.floor((totalSec % 3600) / 60)REDACTEDm`
REDACTED
