<template>
  ...
  <div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-dark-700/50 sm:px-6">
      <p class="text-xs text-ink-tertiary">{{ t('admin.usage.tokenRanking.subtitle') }}</p>
      <div class="flex items-center gap-3">
        <span v-if="!loading && items.length > 0" class="text-xs text-ink-tertiary">
    ...
    <div class="overflow-x-auto">
      <table class="w-full min-w-max divide-y divide-line">
        <thead class="bg-surface-sunken">
          <tr>
            <th class="w-16 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary sm:px-6">#</th>
            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">
              {{ t('admin.usage.tokenRanking.columns.user') }}
            </th>
            <th
              v-for="col in sortableColumns"
              :key="col.key"
              class="cursor-pointer select-none whitespace-nowrap px-4 py-3 text-right text-xs font-medium uppercase tracking-wider transition-colors hover:bg-surface-hover"
              :class="sortBy === col.key ? 'text-primary-600 dark:text-primary-400' : 'text-ink-secondary'"
        ...
        <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
          ...
          <tr v-else-if="items.length === 0">
            <td :colspan="sortableColumns.length + 2" class="py-12 text-center text-sm text-ink-tertiary">
          ...
          <tr
            ...
            v-else
            :key="item.user_id"
            class="cursor-pointer transition-colors hover:bg-surface-hover/40"
            :title="t('admin.usage.tokenRanking.rowHint')"
            @click="$emit('select-user', item.user_id, item.email)"
          >
            <!--
              Rank is a position in a sorted list, not a status, so the top
              three get weight rather than gold/silver/bronze pills. The medals
              also spent three of the app's status hues on decoration.
            -->
            <td class="px-4 py-3 sm:px-6">
              <span
                class="inline-block w-6 text-center font-mono text-sm tabular-nums"
                :class="index < 3 ? 'font-semibold text-ink' : 'text-ink-tertiary'"
                >{{ index + 1 }}</span
              >
            </td>
            <td class="max-w-[260px] truncate px-4 py-3 text-sm font-medium text-ink-secondary" :title="item.email">
              {{ item.email || `User #${item.user_id}` }}
              <span class="ml-1 font-normal text-ink-tertiary">#{{ item.user_id }}</span>
            </td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-ink-secondary">{{ item.requests.toLocaleString() }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-ink-secondary">{{ fmtTokens(item.input_tokens) }}</td>
