...
const mockGroupsList = vi.fn().mockResolvedValue({ items: [] REDACTED)
const mockGetModelStats = vi.fn().mockResolvedValue({ models: [] REDACTED)
const mockAccountsList = vi.fn().mockResolvedValue({ items: [] REDACTED)
...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    ...
    REDACTED,
    groups: { list: (...args: any[]) => mockGroupsList(...args) REDACTED,
    dashboard: { getModelStats: (...args: any[]) => mockGetModelStats(...args) REDACTED,
    accounts: { list: (...args: any[]) => mockAccountsList(...args) REDACTED,
  REDACTED,
REDACTED))
...
function deferred<T>() {
  ...
  return { promise, resolve, reject REDACTED
REDACTED
...
describe('UsageFilters — user search dropdown', () => {
  ...
  it('(a) labels deleted users with the i18n badge and (b) sorts active users before deleted ones, (c) selection sets user_id', async () => {
    ...
    mockSearchUsers.mockResolvedValue([
      { id: 2, email: 'gone@test.com', deleted: true REDACTED,
      { id: 1, email: 'active@test.com', deleted: false REDACTED,
    ])
  ...
  it('keeps results from the latest user search when responses arrive out of order', async () => {
    const firstSearch = deferred<Array<{ id: number; email: string; deleted: boolean REDACTED>>()
    const secondSearch = deferred<Array<{ id: number; email: string; deleted: boolean REDACTED>>()
    mockSearchUsers
    ...
    secondSearch.resolve([{ id: 2, email: 'ab@test.com', deleted: false REDACTED])
    await flushPromises()
    expect(wrapper.text()).toContain('ab@test.com')

    firstSearch.resolve([{ id: 1, email: 'a@test.com', deleted: false REDACTED])
    await flushPromises()
    expect(wrapper.text()).toContain('ab@test.com')
  ...
  it('does not restore stale user results after the search is cleared', async () => {
    const pendingSearch = deferred<Array<{ id: number; email: string; deleted: boolean REDACTED>>()
    ...
    pendingSearch.resolve([{ id: 3, email: 'stale@test.com', deleted: false REDACTED])
    await flushPromises()
...
describe('UsageFilters — model options come from prop (no dup request)', () => {
  ...
  REDACTED)
  afterEach(() => { vi.useRealTimers() REDACTED)
  ...
  it('does not call dashboard.getModelStats on mount and renders model options from prop', async () => {
    const wrapper = mount(UsageFilters, {
      ...
      REDACTED,
      global: { stubs: { Select: true, Teleport: true REDACTED REDACTED,
    REDACTED)
    await flushPromises()

    expect(mockGetModelStats).not.toHaveBeenCalled()

    const opts = (wrapper.vm as any).modelOptions as Array<{ value: string | null; label: string REDACTED>
