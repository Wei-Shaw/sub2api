  ...
  it.each([
    ...
    REDACTED,
    {
      ...
      row: {
        ...
        image_size_breakdown: { '4K': 1 REDACTED,
      REDACTED,
    ...
    REDACTED,
  ])('shows image usage metadata for $name', async ({ row, expected REDACTED) => {
    const wrapper = mount(UsageTable, {
      props: {
        data: [row],
        loading: false,
...
describe('admin UsageTable deleted-user badge', () => {
  it('renders deleted badge for a soft-deleted user row', () => {
    const row = {
      ...
      model: 'claude-3',
      user_id: 2,
      user: { id: 2, email: 'd@test.com', deleted_at: '2026-05-28T00:00:00Z' REDACTED,
      actual_cost: 0,
      total_cost: 0,
      input_cost: 0,
      output_cost: 0,
    ...
    const wrapper = mount(UsageTable, {
      props: {
        data: [row],
        loading: false,
        columns: [{ key: 'user', label: 'User' REDACTED],
      REDACTED,
      global: {
        stubs: {
  ...
  it('does NOT render deleted badge for an active user row', () => {
    const row = {
      ...
      model: 'claude-3',
      user_id: 3,
      user: { id: 3, email: 'active@test.com', deleted_at: null REDACTED,
      actual_cost: 0,
      total_cost: 0,
      input_cost: 0,
      output_cost: 0,
