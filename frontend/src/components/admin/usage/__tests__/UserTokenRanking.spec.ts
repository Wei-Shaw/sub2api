...
const mountRanking = (props: Record<string, unknown> = {REDACTED) =>
  mount(UserTokenRanking, {
    props: {
      ...
      ...props,
    REDACTED,
    global: { stubs: { Select: true, LoadingSpinner: true REDACTED REDACTED,
  REDACTED)

describe('UserTokenRanking', () => {
  beforeEach(() => {
    getUserBreakdown.mockReset()
    getUserBreakdown.mockResolvedValue({ users: [item(1, 100), item(2, 50)] REDACTED)
  REDACTED)

  it('loads on mount with shared filters and emits select-user with id + email on row click', async () => {
    const wrapper = mountRanking({ filters: { group_id: 3 REDACTED, model: 'claude-fable-5' REDACTED)
    await flushPromises()
  ...
  it('reloads when shared filters change', async () => {
    ...
    await wrapper.setProps({ filters: { user_id: 9 REDACTED REDACTED)
    await flushPromises()

    expect(getUserBreakdown).toHaveBeenCalledTimes(2)
    expect(getUserBreakdown).toHaveBeenLastCalledWith(expect.objectContaining({ user_id: 9 REDACTED))
  REDACTED)
REDACTED)
