  ...
  >
    <form id="bulk-edit-user-limits-form" class="space-y-5" @submit.prevent="handleSubmit">
      <p class="text-sm font-medium text-gray-700 dark:text-gray-300">
        {{ t('admin.users.bulkLimits.selectedCount', { count: selectedIds.length REDACTED) REDACTEDREDACTED
      </p>
        ...
        <div class="space-y-3 py-4">
          <div class="flex items-center justify-between gap-4">
            <label for="bulk-concurrency" class="input-label mb-0">
              {{ t('admin.users.columns.concurrency') REDACTEDREDACTED
            </label>
            <Toggle
        ...
        <div class="space-y-3 py-4">
          <div class="flex items-center justify-between gap-4">
            <label for="bulk-rpm-limit" class="input-label mb-0">
              {{ t('admin.users.form.rpmLimit') REDACTEDREDACTED
            </label>
            <Toggle
          ...
          <div v-if="enableRPMLimit">
            ...
            <p v-if="parsedRPMLimit === 0" class="input-hint">
              {{ t('admin.users.bulkLimits.unlimited') REDACTEDREDACTED
            </p>
          </div>
        </div>
      </div>
      ...
      <p v-if="hasInvalidValue" class="text-sm text-red-600 dark:text-red-400">
        {{ t('admin.users.bulkLimits.nonNegativeInteger') REDACTEDREDACTED
      </p>
      <p v-if="selectionTooLarge" class="text-sm text-red-600 dark:text-red-400">
        {{ t('admin.users.bulkLimits.selectionLimit', { max: MAX_BATCH_USER_IDS REDACTED) REDACTEDREDACTED
      </p>
    </form>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <button
          ...
          data-test="submit"
        >
          {{ submitting ? t('admin.users.bulkLimits.applying') : t('admin.users.bulkLimits.apply') REDACTEDREDACTED
        </button>
      </div>
    </template>
  </BaseDialog>
...
import type { BatchUpdateUserLimitsRequest REDACTED from '@/api/admin/users'
...
const handleSubmit = async () => {
  ...
  if (parsedConcurrency.value !== undefined && parsedConcurrency.value !== null) {
    ...
    fields.push(
      t('admin.users.bulkLimits.concurrencyValue', { value: parsedConcurrency.value REDACTED)
    )
  REDACTED
  if (parsedRPMLimit.value !== undefined && parsedRPMLimit.value !== null) {
    ...
    fields.push(
      parsedRPMLimit.value === 0
        ...
        : t('admin.users.bulkLimits.rpmValue', { value: parsedRPMLimit.value REDACTED)
    )
  REDACTED
  ...
  try {
    ...
    appStore.showSuccess(
      t('admin.users.bulkLimits.success', { count: result.affected REDACTED)
    )
