  ...
  <BaseDialog :show="show" :title="t('admin.users.replaceGroupTitle')" width="narrow" @close="$emit('close')">
    <div v-if="oldGroup" class="space-y-4">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400">
        {{ t('admin.users.replaceGroupHint', { old: oldGroup.name REDACTED) REDACTEDREDACTED
      </p>
      ...
      <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-gray-900 dark:text-white">{{ oldGroup.name REDACTEDREDACTED</span>
          ...
          <span v-if="selectedGroupId" class="font-medium text-primary-600 dark:text-primary-400">
            {{ availableGroups.find(g => g.id === selectedGroupId)?.name REDACTEDREDACTED
          </span>
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        ...
        >
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-gray-400">{{ group.platform REDACTEDREDACTED</span>
          </div>
        </label>
      </div>
      ...
      <div v-else class="py-6 text-center text-sm text-gray-400">
        {{ t('admin.users.noOtherGroups') REDACTEDREDACTED
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        >
          ...
          {{ submitting ? t('common.saving') : t('admin.users.replaceGroupConfirm') REDACTEDREDACTED
        </button>
      </div>
    </template>
  </BaseDialog>
...
import type { AdminUser, AdminGroup REDACTED from '@/types'
...
interface Props {
  show: boolean
  user: AdminUser | null
  oldGroup: { id: number; name: string REDACTED | null
...
const handleReplace = async () => {
  ...
  try {
    ...
    appStore.showSuccess(t('admin.users.replaceGroupSuccess', { count: result.migrated_keys REDACTED))
    emit('success')
    emit('close')
