    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
        <div class="flex items-center gap-2">
          ...
          <Icon name="arrowRight" size="sm" class="ml-auto text-ink-tertiary" />
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        ...
        >
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-ink-tertiary">{{ group.platform }}</span>
          </div>
        </label>
      </div>

      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-ink-tertiary">
