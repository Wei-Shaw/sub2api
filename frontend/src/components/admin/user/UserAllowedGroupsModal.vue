  ...
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      ...
      <div class="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-primary-50 to-primary-100 p-5 dark:from-primary-900/30 dark:to-primary-800/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-sm dark:bg-dark-700">
          <span class="text-2xl font-semibold text-primary-600 dark:text-primary-400">{{ user.email.charAt(0).toUpperCase() REDACTEDREDACTED</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ user.email REDACTEDREDACTED</p>
          <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ t('admin.users.groupConfigHint', { email: user.email REDACTED) REDACTEDREDACTED</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ t('admin.users.exclusiveGroups') REDACTEDREDACTED</h4>
            <span class="text-xs text-gray-400">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length REDACTEDREDACTED/{{ exclusiveGroupConfigs.length REDACTEDREDACTED)</span>
          </div>
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-gray-900 dark:text-white">{{ config.groupName REDACTEDREDACTED</span>
                    <span class="inline-flex items-center rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/40 dark:text-purple-300">
                      {{ t('admin.groups.exclusive') REDACTEDREDACTED
                    </span>
                  </div>
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    <span class="inline-flex items-center gap-1 text-gray-500 dark:text-gray-400">
                      <PlatformIcon :platform="config.platform" size="xs" />
                      <span>{{ config.platform REDACTEDREDACTED</span>
                    </span>
                    <span class="text-gray-300 dark:text-dark-500">•</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ t('admin.users.defaultRate') REDACTEDREDACTED: <span class="font-medium text-gray-700 dark:text-gray-300">{{ config.defaultRate REDACTEDREDACTEDx</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('admin.users.customRate') REDACTEDREDACTED</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ t('admin.users.publicGroups') REDACTEDREDACTED</h4>
            <span class="text-xs text-gray-400">({{ publicGroupConfigs.length REDACTEDREDACTED)</span>
          </div>
          <div class="grid gap-3">
            <div
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p class="text-gray-500 dark:text-gray-400">{{ t('common.noGroupsAvailable') REDACTEDREDACTED</p>
        </div>
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <button @click="$emit('close')" class="btn btn-secondary px-5">{{ t('common.cancel') REDACTEDREDACTED</button>
        <button @click="handleSave" :disabled="submitting" class="btn btn-primary px-6">
          ...
          {{ submitting ? t('common.saving') : t('common.save') REDACTEDREDACTED
        </button>
      </div>
    </template>
  </BaseDialog>
...
import type { AdminUser, Group, GroupPlatform REDACTED from '@/types'
...
const originalGroupRates = ref<Record<number, number>>({REDACTED) // 记录原始专属倍率，用于检测删除
...
const load = async () => {
  ...
  try {
    ...
    const userGroupRates = props.user?.group_rates || {REDACTED

    // 保存原始专属倍率，用于检测删除操作
    originalGroupRates.value = { ...userGroupRates REDACTED
...
const handleSave = async () => {
  ...
  try {
    ...
    const groupRates: Record<number, number | null> = {REDACTED
    for (const c of groupConfigs.value) {
      ...
      REDACTED else if (hadOriginalRate) {
