        ...
        <div v-for="key in apiKeys" :key="key.id" class="rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800">
          ...
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-500">
            <div class="flex items-center gap-1">
              <span>{{ t('admin.users.group') REDACTEDREDACTED:</span>
              <button
              ...
              >
                ...
                />
                <span v-else class="text-gray-400 italic">{{ t('admin.users.none') REDACTEDREDACTED</span>
  ...
  <Teleport to="body">
    ...
    >
      <div class="max-h-64 overflow-y-auto p-1.5">
        ...
        >
          <span class="text-gray-500 italic">{{ t('admin.users.none') REDACTEDREDACTED</span>
          <svg
...
import { ref, computed, watch, onMounted, onUnmounted, type ComponentPublicInstance REDACTED from 'vue'
import { useI18n REDACTED from 'vue-i18n'
import { useAppStore REDACTED from '@/stores/app'
import { adminAPI REDACTED from '@/api/admin'
import { formatDateTime REDACTED from '@/utils/format'
import type { AdminUser, AdminGroup, ApiKey REDACTED from '@/types'
...
const changeGroup = async (key: ApiKey, newGroupId: number | null) => {
  ...
  try {
    ...
    if (result.auto_granted_group_access && result.granted_group_name) {
      appStore.showSuccess(t('admin.users.groupChangedWithGrant', { group: result.granted_group_name REDACTED))
    REDACTED else {
