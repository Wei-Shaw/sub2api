  ...
  <BaseDialog :show="show" :title="t('admin.users.balanceHistoryTitle')" width="wide" :close-on-click-outside="true" :z-index="40" @close="$emit('close')">
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-shrink-0 text-right">
            <p class="text-xs text-ink-secondary">{{ t('admin.users.currentBalance') }}</p>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-gray-200/60 pt-2.5 dark:border-dark-600/60">
          <p class="min-w-0 flex-1 truncate text-xs text-ink-secondary" :title="user.notes || ''">
            <template v-if="user.notes">{{ t('admin.users.notes') }}: {{ user.notes }}</template>
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-ink-secondary">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-success">${{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
      ...
      <div v-else-if="history.length === 0" class="py-8 text-center">
        <p class="text-sm text-ink-secondary">{{ t('admin.users.noBalanceHistory') }}</p>
      </div>
