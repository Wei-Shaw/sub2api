  ...
  <BaseDialog :show="show" :title="t('admin.users.balanceHistoryTitle')" width="wide" :close-on-click-outside="true" :z-index="40" @close="$emit('close')">
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex items-center gap-3">
          <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30">
            <span class="text-lg font-medium text-primary-700 dark:text-primary-300">
              {{ user.email.charAt(0).toUpperCase() REDACTEDREDACTED
            </span>
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <p class="truncate font-medium text-gray-900 dark:text-white">{{ user.email REDACTEDREDACTED</p>
            ...
            <p class="text-xs text-gray-400 dark:text-dark-500">
              {{ t('admin.users.createdAt') REDACTEDREDACTED: {{ formatDateTime(user.created_at) REDACTEDREDACTED
            </p>
          </div>
          <!-- Current balance: prominent display on the right -->
          <div class="flex-shrink-0 text-right">
            <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('admin.users.currentBalance') REDACTEDREDACTED</p>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-gray-200/60 pt-2.5 dark:border-dark-600/60">
          <p class="min-w-0 flex-1 truncate text-xs text-gray-500 dark:text-dark-400" :title="user.notes || ''">
            <template v-if="user.notes">{{ t('admin.users.notes') REDACTEDREDACTED: {{ user.notes REDACTEDREDACTED</template>
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-gray-500 dark:text-dark-400">
            {{ t('admin.users.totalRecharged') REDACTEDREDACTED: <span class="font-semibold text-emerald-600 dark:text-emerald-400">${{ totalRecharged.toFixed(2) REDACTEDREDACTED</span>
          </p>
        </div>
      </div>
      ...
      <div v-else-if="history.length === 0" class="py-8 text-center">
        <p class="text-sm text-gray-500">{{ t('admin.users.noBalanceHistory') REDACTEDREDACTED</p>
      </div>
          ...
          <div class="flex items-start justify-between">
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                <p class="text-sm font-medium text-gray-900 dark:text-white">
                  {{ getItemTitle(item) REDACTEDREDACTED
                </p>
                ...
                <p
                  ...
                  :title="item.notes"
                >
                  {{ item.notes.length > 60 ? item.notes.substring(0, 55) + '...' : item.notes REDACTEDREDACTED
                </p>
                <p class="mt-0.5 text-xs text-gray-400 dark:text-dark-500">
                  {{ formatDateTime(item.used_at || item.created_at) REDACTEDREDACTED
                </p>
              </div>
            </div>
            <!-- Right: value -->
            <div class="text-right">
              <p :class="['text-sm font-semibold', getValueColor(item)]">
                {{ formatValue(item) REDACTEDREDACTED
              </p>
              <p
      ...
      <div v-if="totalPages > 1" class="flex items-center justify-center gap-2 pt-2">
        ...
        <span class="text-sm text-gray-500 dark:text-dark-400">
          {{ currentPage REDACTEDREDACTED / {{ totalPages REDACTEDREDACTED
        </span>
        <button
...
import { adminAPI, type BalanceHistoryItem REDACTED from '@/api/admin'
...
const props = defineProps<{ show: boolean; user: AdminUser | null; hideActions?: boolean REDACTED>()
...
const typeOptions = computed(() => [
  { value: '', label: t('admin.users.allTypes') REDACTED,
  { value: 'balance', label: t('admin.users.typeBalance') REDACTED,
  { value: 'affiliate_balance', label: t('admin.users.typeAffiliateBalance') REDACTED,
  { value: 'admin_balance', label: t('admin.users.typeAdminBalance') REDACTED,
  { value: 'concurrency', label: t('admin.users.typeConcurrency') REDACTED,
  { value: 'admin_concurrency', label: t('admin.users.typeAdminConcurrency') REDACTED,
  { value: 'subscription', label: t('admin.users.typeSubscription') REDACTED
])
...
const formatValue = (item: BalanceHistoryItem) => {
  ...
  if (isSubscriptionType(item.type)) {
    ...
    return groupName ? `${daysREDACTEDd - ${groupNameREDACTED` : `${daysREDACTEDd`
  REDACTED
  // concurrency types
  const sign = item.value >= 0 ? '+' : ''
  return `${signREDACTED${item.valueREDACTED`
REDACTED
</script>
