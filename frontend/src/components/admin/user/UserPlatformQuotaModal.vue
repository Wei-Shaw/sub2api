<template>
  ...
  >
    <div v-if="user" class="space-y-4">
      ...
      >
        {{ t('admin.users.platformQuota.subscriptionWarning') REDACTEDREDACTED
      </div>
      <p class="text-sm text-gray-600 dark:text-gray-400">
        {{ t('admin.users.platformQuota.subtitle', { email: user.email REDACTED) REDACTEDREDACTED
      </p>
      <div v-if="loading" class="py-10 text-center text-gray-500">{{ t('common.loading') REDACTEDREDACTED</div>
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 text-gray-700 dark:border-dark-700 dark:text-gray-300">
              <th class="px-3 py-2 text-left font-medium">{{ t('admin.users.platformQuota.columns.platform') REDACTEDREDACTED</th>
              <th class="px-3 py-2 text-left font-medium">{{ t('admin.users.platformQuota.columns.daily') REDACTEDREDACTED</th>
              <th class="px-3 py-2 text-left font-medium">{{ t('admin.users.platformQuota.columns.weekly') REDACTEDREDACTED</th>
              <th class="px-3 py-2 text-left font-medium">{{ t('admin.users.platformQuota.columns.monthly') REDACTEDREDACTED</th>
              <th class="px-3 py-2 text-left font-medium">{{ t('admin.users.platformQuota.columns.usage') REDACTEDREDACTED</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in quotas" :key="row.platform" class="border-b border-gray-100 dark:border-dark-800">
              <td class="px-3 py-2 font-mono text-gray-900 dark:text-white">{{ row.platform REDACTEDREDACTED</td>
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    ...
                    :disabled="!!resetting[`${row.platformREDACTED.daily`]"
              ...
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    class="text-xs text-gray-400 hover:text-amber-500 disabled:opacity-50"
                    :disabled="!!resetting[`${row.platformREDACTED.weekly`]"
                    :title="t('admin.users.platformQuota.reset.button')"
              ...
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    class="text-xs text-gray-400 hover:text-amber-500 disabled:opacity-50"
                    :disabled="!!resetting[`${row.platformREDACTED.monthly`]"
                    :title="t('admin.users.platformQuota.reset.button')"
              ...
              <td class="px-3 py-2 text-xs text-gray-500 dark:text-gray-400">
                {{ formatUsage(row.daily_usage_usd) REDACTEDREDACTED / {{ formatUsage(row.weekly_usage_usd) REDACTEDREDACTED / {{ formatUsage(row.monthly_usage_usd) REDACTEDREDACTED
              </td>
            </tr>
          </tbody>
        </table>
        <p class="mt-3 text-xs text-gray-500">{{ t('admin.users.platformQuota.hint') REDACTEDREDACTED</p>
        <div class="mt-3">
          <button type="button" class="btn btn-secondary text-sm" @click="onClearAll">
            {{ t('admin.users.platformQuota.clearAll') REDACTEDREDACTED
          </button>
        </div>
      </div>
    </div>
    <template #footer>
      <div class="flex justify-end gap-3">
        <button type="button" class="btn btn-secondary" @click="$emit('close')">
          {{ t('admin.users.platformQuota.cancel') REDACTEDREDACTED
        </button>
        <button type="button" class="btn btn-primary" :disabled="submitting || loading" @click="onSave">
          {{ submitting ? t('admin.users.platformQuota.saving') : t('admin.users.platformQuota.save') REDACTEDREDACTED
        </button>
      </div>
    </template>
  </BaseDialog>
...
import { ref, reactive, watch, computed REDACTED from 'vue'
import { useI18n REDACTED from 'vue-i18n'
import { useAppStore REDACTED from '@/stores/app'
import { adminAPI REDACTED from '@/api/admin'
import type { AdminUser, PlatformQuotaItem, PlatformQuotaPlatform, PlatformQuotaWindow REDACTED from '@/types'
...
const resetting = reactive<Record<string, boolean>>({REDACTED)
...
watch(
  () => props.show,
  (s) => { if (s && props.user) load() REDACTED,
)
...
async function onSave() {
  ...
  for (const row of quotas.value) {
    for (const win of ['daily', 'weekly', 'monthly'] as const) {
      const v = row[`${winREDACTED_limit_usd` as const]
      if (typeof v === 'number' && Number.isNaN(v)) {
        invalid.push(`${row.platformREDACTED.${winREDACTED`)
      REDACTED
    REDACTED
  REDACTED
  if (invalid.length > 0) {
    appStore.showError(t('admin.users.platformQuota.invalidNumber', { fields: invalid.join(', ') REDACTED))
    return
  REDACTED
...
async function onReset(platform: PlatformQuotaPlatform, quotaWindow: PlatformQuotaWindow) {
  if (!props.user) return
  const windowLabel = t(`admin.users.platformQuota.window${quotaWindow.charAt(0).toUpperCase() + quotaWindow.slice(1)REDACTED`)
  const confirmed = window.confirm(
    t('admin.users.platformQuota.reset.confirm', { platform, window: windowLabel REDACTED)
  )
  if (!confirmed) return
  const key = `${platformREDACTED.${quotaWindowREDACTED`
  resetting[key] = true
  try {
    const data = await adminAPI.users.resetPlatformQuotaWindow(props.user.id, platform, quotaWindow)
    quotas.value = normalize(data.platform_quotas || [])
    appStore.showSuccess(t('admin.users.platformQuota.reset.success', { platform, window: windowLabel REDACTED))
  REDACTED catch (e: any) {
