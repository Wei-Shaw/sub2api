  ...
  <div v-if="hasProviders" class="space-y-4">
    ...
    <div :class="providerGridClass">
      ...
      >
        ...
        <span class="font-medium">{{ providerLabel(provider) REDACTEDREDACTED</span>
      </button>
    </div>
  </div>
</template>
...
function providerLabel(provider: EmailOAuthProvider): string {
  ...
  return hasMultipleProviders.value ? name : t('auth.emailOAuth.signIn', { providerName: name REDACTED)
REDACTED
