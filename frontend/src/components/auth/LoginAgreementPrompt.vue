<template>
    ...
    <div class="flex items-start gap-2">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-[13px] leading-5 text-gray-600 dark:text-dark-300">
          ...
          >
            {{ t('legal.loginAgreementPrompt.checkboxPrefix') REDACTEDREDACTED
          </label>
          <template v-for="(doc, index) in documents" :key="doc.id || doc.title">
            ...
            >
              {{ doc.title REDACTEDREDACTED
            </RouterLink>
            <span v-if="index < documents.length - 1">{{ t('legal.loginAgreementPrompt.documentSeparator') REDACTEDREDACTED</span>
          </template>
        </p>
      </div>
    </div>
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="font-medium">{{ t('legal.loginAgreementPrompt.noticeTitle') REDACTEDREDACTED</p>
        <p class="mt-1 text-primary-700 dark:text-primary-200/80">
          {{ t('legal.loginAgreementPrompt.noticeDescription') REDACTEDREDACTED
        </p>
      </div>
      <button
        type="button"
        ...
        @click="emit('open')"
      >
        {{ t('legal.loginAgreementPrompt.viewTerms') REDACTEDREDACTED
      </button>
    </div>
  </div>
        ...
        <div class="w-full max-w-[600px] overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/10 dark:bg-dark-900 dark:ring-white/10">
          ...
          <div class="max-h-[58vh] overflow-y-auto px-6 py-5">
            <div class="mb-3 flex items-center justify-between gap-3">
              <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('legal.loginAgreementPrompt.relatedDocuments') REDACTEDREDACTED</p>
            </div>
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              >
                ...
                <span class="min-w-0 flex-1">
                  <span class="block truncate text-sm font-semibold text-gray-950 dark:text-white">{{ doc.title REDACTEDREDACTED</span>
                </span>
          ...
          <div class="border-t border-gray-100 bg-gray-50/80 px-6 py-4 dark:border-dark-800 dark:bg-dark-950/60">
            <div class="grid grid-cols-2 gap-3">
              <button
                ...
                @click="emit('reject')"
              >
                {{ t('legal.loginAgreementPrompt.reject') REDACTEDREDACTED
              </button>
              <button
                type="button"
                ...
                @click="emit('accept')"
              >
                {{ t('legal.loginAgreementPrompt.accept') REDACTEDREDACTED
              </button>
            </div>
          </div>
        </div>
