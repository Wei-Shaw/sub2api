...
describe('PendingOAuthCreateAccountForm', () => {
  ...
  it('acquires separate proofs for pending OAuth send-code and create-account', async () => {
    ...
    verifyAction
      .mockResolvedValueOnce({ token: 'ticket-1', randstr: '@rand-1' REDACTED)
      .mockResolvedValueOnce({ token: 'ticket-2', randstr: '@rand-2' REDACTED)
    const CaptchaChallengeStub = defineComponent({
      setup(_, { expose REDACTED) {
        expose({ verifyAction, reset: turnstileReset REDACTED)
        return () => h('div')
      REDACTED
    REDACTED)
  ...
  it('blocks submit again after sending a verify code consumes the turnstile proof', async () => {
    getPublicSettings.mockResolvedValue({
      ...
      turnstile_site_key: 'site-key'
    REDACTED)
    sendPendingOAuthVerifyCode.mockResolvedValue({ message: 'sent', countdown: 60 REDACTED)
