  ...
  <DataTable :columns="columns" :data="rows" :loading="loading">
    <template #cell-name="{ row REDACTED">
      <div class="flex items-center gap-2">
        <span class="font-medium text-gray-900 dark:text-white">{{ row.name REDACTEDREDACTED</span>
        <span
          v-if="row.platform"
          :class="[
    ...
    <template #cell-supported_models="{ row REDACTED">
...
import { computed, useSlots REDACTED from 'vue'
import DataTable from '@/components/common/DataTable.vue'
import Icon from '@/components/icons/Icon.vue'
import PlatformIcon from '@/components/common/PlatformIcon.vue'
import SupportedModelChip from './SupportedModelChip.vue'
import type { UserSupportedModel REDACTED from '@/api/channels'
import type { ChannelStatus, BillingModelSource REDACTED from '@/constants/channel'
