<template>
  <div class="relative inline-block">
    ...
    >
      ...
      >
        {{ model.platform REDACTEDREDACTED
      </span>
      {{ model.name REDACTEDREDACTED
    </span>
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          <span class="truncate font-semibold">{{ model.name REDACTEDREDACTED</span>
          <span
            v-if="model.platform"
        ...
        <div class="p-3">
          <div v-if="!model.pricing" class="text-gray-500 dark:text-gray-400">
            {{ noPricingLabel REDACTEDREDACTED
          </div>

          <div v-else class="space-y-2 text-gray-700 dark:text-gray-300">
            <div class="flex justify-between">
              <span class="text-gray-500 dark:text-gray-400">{{ t(prefixKey('billingMode')) REDACTEDREDACTED</span>
              <span>{{ billingModeLabel REDACTEDREDACTED</span>
            </div>
            ...
            >
              <div class="mb-1 font-medium text-gray-600 dark:text-gray-400">
                {{ t(prefixKey('intervals')) REDACTEDREDACTED
              </div>
              <div class="space-y-1">
                <div
                ...
                >
                  <span class="text-gray-500 dark:text-gray-400">
                    <template v-if="iv.tier_label">{{ iv.tier_label REDACTEDREDACTED</template>
                    <template v-else>{{ formatRange(iv.min_tokens, iv.max_tokens) REDACTEDREDACTED</template>
                  </span>
                  <span>{{ formatInterval(iv, model.pricing.billing_mode) REDACTEDREDACTED</span>
                </div>
              </div>
            </div>
          </div>
...
import { computed, nextTick, onBeforeUnmount, ref REDACTED from 'vue'
...
import type { UserPricingInterval, UserSupportedModel REDACTED from '@/api/channels'
import PlatformIcon from '@/components/common/PlatformIcon.vue'
import type { GroupPlatform REDACTED from '@/types'
import { platformBadgeClass, platformBorderClass, platformBadgeLightClass REDACTED from '@/utils/platformColors'
...
function prefixKey(k: string): string {
  return `${props.pricingKeyPrefixREDACTED.${kREDACTED`
REDACTED
...
function formatRange(min: number, max: number | null): string {
  ...
  return `(${minREDACTED, ${maxLabelREDACTED]`
REDACTED
...
function formatInterval(iv: UserPricingInterval, mode: BillingMode): string {
  ...
  return `${inputREDACTED / ${outputREDACTED`
REDACTED
...
const popoverStyle = ref<Record<string, string>>({ top: '0px', left: '0px' REDACTED)
...
function updatePosition() {
  ...
  popoverStyle.value = {
    top: `${Math.round(top)REDACTEDpx`,
    left: `${Math.round(left)REDACTEDpx`,
  REDACTED
REDACTED
