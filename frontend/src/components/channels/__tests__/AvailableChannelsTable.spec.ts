    ...
    platforms: [
      {
        ...
        supported_models: [{ name: 'claude-test', platform: 'anthropic', pricing: null REDACTED],
      REDACTED,
    ],
  REDACTED,
]
...
const baseProps = {
  ...
  userGroupRates: { 1: 0.8 REDACTED,
REDACTED

function mountTable(props = {REDACTED) {
  return mount(AvailableChannelsTable, {
    props: { ...baseProps, ...props REDACTED,
    global: {
      plugins: [createPinia()],
      stubs: {
        Icon: { props: ['name'], template: '<i :data-icon="name" />' REDACTED,
        PlatformIcon: { template: '<i data-platform-icon />' REDACTED,
        GroupBadge: {
          props: ['name', 'rateMultiplier', 'userRateMultiplier'],
          template:
            '<span data-group-badge>{{ name REDACTEDREDACTED:{{ rateMultiplier REDACTEDREDACTED:{{ userRateMultiplier REDACTEDREDACTED</span>',
        REDACTED,
        SupportedModelChip: {
          props: ['model', 'noPricingLabel'],
          template: '<span data-model-chip>{{ model.name REDACTEDREDACTED:{{ noPricingLabel REDACTEDREDACTED</span>',
        REDACTED,
      REDACTED,
    REDACTED,
  REDACTED)
...
describe('AvailableChannelsTable responsive surfaces', () => {
      ...
      rows: [
        {
          ...
          description: '',
          platforms: [{ platform: 'openai', groups: [], supported_models: [] REDACTED],
        REDACTED,
      ],
    REDACTED)
    const mobile = wrapper.get('[data-testid="mobile-channels"]')
  ...
  it('provides loading and empty states on both responsive surfaces', async () => {
    const wrapper = mountTable({ loading: true, rows: [] REDACTED)
    ...
    await wrapper.setProps({ loading: false REDACTED)
