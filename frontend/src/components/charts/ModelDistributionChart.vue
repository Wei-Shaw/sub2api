    ...
    <div class="mb-4 flex items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center justify-end gap-2">
        ...
        >
          ...
          >
            {{ t('usage.requestedModel') REDACTEDREDACTED
          </button>
          <button
            type="button"
            class="rounded-md px-2.5 py-1 text-xs font-medium transition-colors"
          ...
          >
            {{ t('usage.upstreamModel') REDACTEDREDACTED
          </button>
          <button
            type="button"
            class="rounded-md px-2.5 py-1 text-xs font-medium transition-colors"
          ...
          >
            {{ t('usage.mapping') REDACTEDREDACTED
          </button>
        </div>
        <div
              ...
              >
                ...
                >
                  <span class="inline-flex items-center gap-1">
                    <svg v-if="enableBreakdown && expandedKey === `model-${model.modelREDACTED`" class="h-3 w-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                ...
                <td v-if="showAccountCost" class="py-1.5 text-right text-orange-500 dark:text-orange-400">
                  ${{ formatCost(model.account_cost) REDACTEDREDACTED
                </td>
                <td class="py-1.5 text-right text-gray-400 dark:text-gray-500">
                  ${{ formatCost(model.cost) REDACTEDREDACTED
                </td>
              </tr>
              <tr v-if="expandedKey === `model-${model.modelREDACTED`">
        ...
        <table class="w-full text-xs">
          ...
          <tbody>
            <tr
              ...
              :key="item.isOther ? 'others' : `${item.user_idREDACTED-${indexREDACTED`"
              class="border-t border-gray-100 transition-colors dark:border-gray-700"
              :class="item.isOther
...
import type { ModelStat, UserSpendingRankingItem, UserBreakdownItem REDACTED from '@/types'
...
type RankingDisplayItem = UserSpendingRankingItem & { isOther?: boolean REDACTED
