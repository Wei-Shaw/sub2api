...
const messages: Record<string, string> = {
  ...
  'admin.redeem.userPrefix': 'User #{idREDACTED',
REDACTED
  ...
  return {
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, unknown>) => {
        ...
        return params
          ? message.replace(/\{(\w+)\REDACTED/g, (_, name: string) => String(params[name]))
          : message
      REDACTED,
    REDACTED),
  REDACTED
...
vi.mock('vue-chartjs', () => ({
  Doughnut: {
    props: ['data'],
    template: '<div class="chart-data">{{ JSON.stringify(data) REDACTEDREDACTED</div>',
  REDACTED,
REDACTED))
...
describe('ModelDistributionChart', () => {
  ...
  it('uses total_tokens and token ordering by default', () => {
    ...
    const label = options.plugins.tooltip.callbacks.label({
      label: 'model-a',
      raw: 1000,
      dataset: { data: [1000, 500] REDACTED,
    REDACTED)
  ...
  it('uses the dashboard user label policy and renders Others with a dedicated chart color', async () => {
    const wrapper = mount(ModelDistributionChart, {
      props: {
        ...
        rankingItems: [
          { user_id: 1, email: 'alpha@example.com', username: 'alpha', actual_cost: 12, requests: 10, tokens: 1000 REDACTED,
          { user_id: 2, email: 'beta@example.com', username: '   ', actual_cost: 8, requests: 6, tokens: 600 REDACTED,
          { user_id: 3, email: '   ', username: '', actual_cost: 0, requests: 0, tokens: 0 REDACTED,
        ],
