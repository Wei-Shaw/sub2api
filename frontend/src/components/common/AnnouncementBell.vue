<template>
  <div>
    ...
    <button
      ...
      :class="{ 'text-blue-600 dark:text-blue-400': unreadCount > 0 REDACTED"
                ...
                <div>
                  <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                      {{ t('announcements.title') REDACTEDREDACTED
                    </h2>
                  </div>
                  <p v-if="unreadCount > 0" class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    <span class="font-medium text-blue-600 dark:text-blue-400">{{ unreadCount REDACTEDREDACTED</span>
                    {{ t('announcements.unread') REDACTEDREDACTED
                  </p>
                </div>
                <div class="flex items-center gap-2">
                  <button
                  ...
                  >
                    {{ t('announcements.markAllRead') REDACTEDREDACTED
                  </button>
                  <button
                    @click="closeModal"
            ...
            <div class="max-h-[65vh] overflow-y-auto">
              ...
              <div v-else-if="announcements.length > 0">
                <div
                  ...
                  :class="{ 'bg-blue-50/30 dark:bg-blue-900/5': !item.read_at REDACTED"
                  ...
                  <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
                    <div class="min-w-0 flex-1">
                      <h3 class="truncate text-sm font-medium text-gray-900 dark:text-white">
                        {{ item.title REDACTEDREDACTED
                      </h3>
                      <div class="mt-1 flex items-center gap-2">
                        <time class="text-xs text-gray-500 dark:text-gray-400">
                          {{ formatRelativeTime(item.created_at) REDACTEDREDACTED
                        </time>
                        <span
                          v-if="!item.read_at"
              ...
              <div v-else class="flex flex-col items-center justify-center py-16">
                ...
                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('announcements.empty') REDACTEDREDACTED</p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('announcements.emptyDescription') REDACTEDREDACTED</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="relative z-10 flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <h2 class="mb-3 text-2xl font-bold leading-tight text-gray-900 dark:text-white">
                    {{ selectedAnnouncement.title REDACTEDREDACTED
                  </h2>
                  ...
                  <div class="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                    <div class="flex items-center gap-1.5">
                      ...
                      <time>{{ formatRelativeWithDateTime(selectedAnnouncement.created_at) REDACTEDREDACTED</time>
                    </div>
                    <div class="flex items-center gap-1.5">
                      <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      ...
                      <span>{{ selectedAnnouncement.read_at ? t('announcements.read') : t('announcements.unread') REDACTEDREDACTED</span>
                    </div>
                  </div>
                </div>
            ...
            <div class="border-t border-gray-100 bg-gray-50/50 px-8 py-5 dark:border-dark-700 dark:bg-dark-900/30">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <span>{{ selectedAnnouncement.read_at ? t('announcements.readStatus') : t('announcements.markReadHint') REDACTEDREDACTED</span>
                </div>
                <div class="flex items-center gap-3">
                  ...
                  >
                    <span class="flex items-center gap-2">
                      ...
                      {{ t('announcements.markRead') REDACTEDREDACTED
                    </span>
                  </button>
                </div>
              </div>
...
function handleEscape(e: KeyboardEvent) {
  if (e.key === 'Escape') {
    if (detailModalOpen.value) {
      closeDetail()
    REDACTED else if (isModalOpen.value) {
      closeModal()
    REDACTED
  REDACTED
REDACTED
