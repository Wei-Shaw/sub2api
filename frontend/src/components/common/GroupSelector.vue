<template>
  <div>
    <label class="input-label">
      {{ t('admin.users.groups') REDACTEDREDACTED
      <span class="font-normal text-gray-400">{{ t('common.selectedCount', { count: modelValue.length REDACTED) REDACTEDREDACTED</span>
    </label>
    <div
      v-if="isSearchable"
    ...
    >
      <label
        ...
        :title="t('admin.groups.rateAndAccounts', { rate: group.rate_multiplier, count: group.account_count || 0 REDACTED)"
      >
        <input
          type="checkbox"
          :value="group.id"
        ...
        />
        <span class="shrink-0 text-xs text-gray-400">{{ group.account_count || 0 REDACTEDREDACTED</span>
      </label>
      <div
        v-if="filteredGroups.length === 0"
        class="col-span-2 py-2 text-center text-sm text-gray-500 dark:text-gray-400"
      >
        {{ t('common.noGroupsAvailable') REDACTEDREDACTED
      </div>
    </div>
  </div>
</template>
