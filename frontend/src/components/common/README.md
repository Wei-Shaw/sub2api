...
- `cell-{keyREDACTED` - Custom cell renderer for specific column (receives `row` and `value`)
...
<DataTable
  :columns="[
    ...
    { key: 'email', label: 'Email' REDACTED,
    { key: 'status', label: 'Status', formatter: (val) => val.toUpperCase() REDACTED
  ]"
  :data="users"
  :loading="isLoading"
>
...
<EmptyState
  ...
  :action-to="{ name: 'users-create' REDACTED"
/>
```
...
```typescript
import { DataTable, Pagination, Modal REDACTED from '@/components/common'
```
