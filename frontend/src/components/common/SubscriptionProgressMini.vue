        ...
        <div class="max-h-64 overflow-y-auto">
          ...
          >
            ...
            <div class="space-y-1.5">
              ...
              <template v-else>
                <div v-if="effectiveLimit(subscription, 'daily')" class="flex items-center gap-2">
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-gray-500">
                    {{
                      formatUsage(subscription.daily_usage_usd, effectiveLimit(subscription, 'daily'))
                    }}
                  </span>
                </div>

                <div v-if="effectiveLimit(subscription, 'weekly')" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-gray-500">{{
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-gray-500">
                    {{
                      formatUsage(subscription.weekly_usage_usd, effectiveLimit(subscription, 'weekly'))
                    }}
                  </span>
                </div>

                <div v-if="effectiveLimit(subscription, 'monthly')" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-gray-500">{{
...
function getMaxUsagePercentage(sub: UserSubscription): number {
  ...
  const dailyLimit = effectiveLimit(sub, 'daily')
  const weeklyLimit = effectiveLimit(sub, 'weekly')
  const monthlyLimit = effectiveLimit(sub, 'monthly')
  if (dailyLimit) {
...
function isUnlimited(sub: UserSubscription): boolean {
  return !hasAnyEffectiveSubscriptionLimit(sub)
}
...
function effectiveLimit(
  ...
  return getEffectiveSubscriptionLimit(sub, window)
}
