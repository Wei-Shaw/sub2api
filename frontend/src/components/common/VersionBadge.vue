      ...
      <transition name="dropdown">
        ...
        >
          ...
          <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-dark-700">
            <span class="text-sm font-medium text-gray-700 dark:text-dark-300">{{ t('version.currentVersion') REDACTEDREDACTED</span>
            <button
            ...
            >
              <svg class="w-4 h-4" :class="{ 'animate-spin': loading REDACTED" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <template v-else>
              ...
              <div class="text-center mb-4">
                <div class="inline-flex items-center gap-2">
                  <span v-if="currentVersion" class="text-2xl font-bold text-gray-900 dark:text-white">v{{ currentVersion REDACTEDREDACTED</span>
                ...
                <p class="text-xs text-gray-500 dark:text-dark-400 mt-1">
                  {{ hasUpdate ? t('version.latestVersion') + ': v' + latestVersion : t('version.upToDate') REDACTEDREDACTED
                </p>
              </div>
              ...
              <div v-if="updateError" class="space-y-2">
                <div class="flex items-center gap-3 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-red-700 dark:text-red-300">{{ t('version.updateFailed') REDACTEDREDACTED</p>
                    <p class="text-xs text-red-600/70 dark:text-red-400/70 truncate">{{ updateError REDACTEDREDACTED</p>
                  </div>
                </div>
              ...
              <div v-else-if="updateSuccess && needRestart" class="space-y-2">
                <div class="flex items-center gap-3 p-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/50">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-green-700 dark:text-green-300">{{ t('version.updateComplete') REDACTEDREDACTED</p>
                    <p class="text-xs text-green-600/70 dark:text-green-400/70">{{ t('version.restartRequired') REDACTEDREDACTED</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <template v-if="restarting">
                    ...
                    <span v-if="restartCountdown > 0" class="tabular-nums">({{ restartCountdown REDACTEDREDACTEDs)</span>
                  </template>
              ...
              <div v-else-if="hasUpdate && !isReleaseBuild" class="space-y-2">
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-amber-700 dark:text-amber-300">{{ t('version.updateAvailable') REDACTEDREDACTED</p>
                    <p class="text-xs text-amber-600/70 dark:text-amber-400/70">v{{ latestVersion REDACTEDREDACTED</p>
                  </div>
                ...
                <div class="flex items-center gap-2 p-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50">
                  ...
                  <p class="text-xs text-blue-600 dark:text-blue-400">{{ t('version.sourceModeHint') REDACTEDREDACTED</p>
                </div>
              </div>
...
import { performUpdate, restartService REDACTED from '@/api/admin/system';
