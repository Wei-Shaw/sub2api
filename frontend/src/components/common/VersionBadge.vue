<template>
  <div class="relative">
    ...
    <template v-if="isAdmin">
      ...
      >
        <span v-if="currentVersion" class="font-medium">v{{ currentVersion REDACTEDREDACTED</span>
        <span
          v-else
          ...
          <div class="p-4">
            ...
            <template v-else>
              ...
              <div class="mb-4 text-center">
                <div class="inline-flex items-center gap-2">
                  <span
                    ...
                    >v{{ currentVersion REDACTEDREDACTED</span
                  >
              ...
              <div v-if="updateError" class="space-y-2">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="text-sm font-medium text-red-700 dark:text-red-300">
                      {{ t('version.updateFailed') REDACTEDREDACTED
                    </p>
                    <p class="truncate text-xs text-red-600/70 dark:text-red-400/70">
                      {{ updateError REDACTEDREDACTED
                    </p>
                  </div>
                </div>
                ...
                >
                  {{ t('version.retry') REDACTEDREDACTED
                </button>
              </div>
              ...
              <div v-else-if="updateSuccess && needRestart" class="space-y-2">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-green-600/70 dark:text-green-400/70">
                      {{ t('version.restartRequired') REDACTEDREDACTED
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <template v-if="restarting">
                    <span>{{ t('version.restarting') REDACTEDREDACTED</span>
                    <span v-if="restartCountdown > 0" class="tabular-nums"
                      >({{ restartCountdown REDACTEDREDACTEDs)</span
                    >
                  </template>
                  <span v-else>{{ t('version.restartNow') REDACTEDREDACTED</span>
                </button>
              </div>
              ...
              <div v-else-if="hasUpdate && !isReleaseBuild" class="space-y-2">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="text-sm font-medium text-amber-700 dark:text-amber-300">
                      {{ t('version.updateAvailable') REDACTEDREDACTED
                    </p>
                    <p class="text-xs text-amber-600/70 dark:text-amber-400/70">
                      v{{ latestVersion REDACTEDREDACTED
                    </p>
                  </div>
                  <svg
                ...
                >
                  ...
                  <p class="text-xs text-blue-600 dark:text-blue-400">
                    {{ t('version.sourceModeHint') REDACTEDREDACTED
                  </p>
                </div>
              </div>
              ...
              <div v-else-if="hasUpdate && isReleaseBuild" class="space-y-2">
                ...
                >
                  ...
                  {{ updating ? t('version.updating') : t('version.updateNow') REDACTEDREDACTED
                </button>
                ...
                >
                  {{ t('version.viewChangelog') REDACTEDREDACTED
              ...
              <div v-else class="space-y-2">
                ...
                >
                  ...
                  {{ t('version.viewRelease') REDACTEDREDACTED
                </a>
                ...
                <div class="border-t border-gray-100 pt-2 dark:border-dark-700">
                  ...
                  >
                    <span class="flex items-center gap-1.5">
                      ...
                      {{ t('version.rollback') REDACTEDREDACTED
                    </span>
                    <Icon
                      name="chevronDown"
                      size="xs"
                      ...
                      :class="{ 'rotate-180': rollbackPanelOpen REDACTED"
                    />
                  </button>
                    ...
                    <div v-if="rollbackPanelOpen" class="mt-2 space-y-2">
                      ...
                      >
                        ...
                        <p class="min-w-0 flex-1 text-xs leading-4 text-blue-600 dark:text-blue-400">
                          {{ t('version.rollbackSourceHint') REDACTEDREDACTED
                        </p>
                      </div>
                      ...
                      <div v-else-if="rollbackVersionsError" class="space-y-2">
                        ...
                        >
                          {{ rollbackVersionsError REDACTEDREDACTED
                        </p>
                        <button
                      ...
                      >
                        {{ t('version.noRollbackVersions') REDACTEDREDACTED
                      </p>

                      <!-- Version list -->
                      <template v-else>
                        <p class="px-0.5 text-[11px] text-gray-400 dark:text-dark-500">
                          {{ t('version.rollbackSelectVersion') REDACTEDREDACTED
                        </p>
                        ...
                        >
                          <span class="flex items-center gap-2">
                            ...
                            <span
                              ...
                              "
                              >v{{ item.version REDACTEDREDACTED</span
                            >
                          </span>
                          <span class="text-[11px] tabular-nums text-gray-400 dark:text-dark-500">
                            {{ formatPublishedAt(item.published_at) REDACTEDREDACTED
                          </span>
                        </button>
                        ...
                        <transition name="rollback">
                          <div v-if="selectedRollbackVersion" class="space-y-2">
                            <p class="px-0.5 text-[11px] text-gray-400 dark:text-dark-500">
                              {{ t('version.manualRollbackCommand') REDACTEDREDACTED
                            </p>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  ...
                                  />
                                  {{ copied ? t('version.copied') : t('version.copyCommand') REDACTEDREDACTED
                                </button>
                              </div>
                              <code
                                ...
                                >{{ activeManualCommand REDACTEDREDACTED</code
                              >
                            </div>
                            ...
                            >
                              ...
                              />
                              {{ t('version.rollbackWarning') REDACTEDREDACTED
                            </p>
                            ...
                            >
                              {{ rollbackError REDACTEDREDACTED
                            </p>
...
REDACTED from '@/api/admin/system'
...
const manualTabs = computed(() => [
  { key: 'script' as const, label: t('version.deployScript') REDACTED,
  { key: 'docker' as const, label: t('version.deployDocker') REDACTED
])

const scriptRollbackCommand = computed(() => {
  if (!selectedRollbackVersion.value) return ''
  const tag = `v${selectedRollbackVersion.valueREDACTED`
  return `curl -sSL https://raw.githubusercontent.com/${GITHUB_REPOREDACTED/${tagREDACTED/deploy/install.sh | sudo bash -s -- rollback ${tagREDACTED`
REDACTED)

const dockerRollbackCommand = computed(() => {
  if (!selectedRollbackVersion.value) return ''
  return [
    `# ${t('version.dockerEditCompose')REDACTED`,
    `image: ${DOCKER_IMAGEREDACTED:${selectedRollbackVersion.valueREDACTED`,
    '',
    `# ${t('version.dockerRecreate')REDACTED`,
    'docker compose up -d'
  ].join('\n')
REDACTED)
...
async function handleUpdate() {
  ...
  REDACTED catch (error: unknown) {
    const err = error as { response?: { data?: { message?: string REDACTED REDACTED; message?: string REDACTED
