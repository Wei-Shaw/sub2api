...
import type { MonitorQuotaSnapshot REDACTED from '@/api/admin/channelMonitor'
...
function makeSnapshot(overrides: Partial<MonitorQuotaSnapshot> = {REDACTED): MonitorQuotaSnapshot {
  return {
    source: 'usage',
    success: true,
...
describe('MonitorQuotaView', () => {
  it('renders nothing without a snapshot', () => {
    const wrapper = mount(MonitorQuotaView, { props: { snapshot: null REDACTED REDACTED)
    ...
    const wrapper = mount(MonitorQuotaView, {
      props: {
        snapshot: makeSnapshot({
          tiers: [
            { window: '5h', used_percent: 42.4 REDACTED,
            { window: '7d', label: 'pro', used_percent: 80 REDACTED,
            { window: 'weekly', label: 'unknown-token', used_percent: 95 REDACTED,
          ],
        REDACTED),
      REDACTED,
    REDACTED)
  ...
  it('clamps the tier bar width into 0-100', () => {
    const wrapper = mount(MonitorQuotaView, {
      props: {
        snapshot: makeSnapshot({ tiers: [{ window: '5h', used_percent: 240 REDACTED] REDACTED),
      REDACTED,
    REDACTED)
    ...
    const wrapper = mount(MonitorQuotaView, {
      props: {
        snapshot: makeSnapshot({
          plan_level: 'Max20',
          balances: [
            { currency: 'CNY', balance: 12.5 REDACTED,
            { currency: 'USD', balance: 0 REDACTED,
          ],
        REDACTED),
      REDACTED,
    REDACTED)
  ...
  it('falls back to the single balance + currency pair', () => {
    const wrapper = mount(MonitorQuotaView, {
      props: { snapshot: makeSnapshot({ balance: 3.2, currency: 'CNY' REDACTED) REDACTED,
    REDACTED)
  ...
  it('renders a truncated error state when the fetch failed', () => {
    ...
    const wrapper = mount(MonitorQuotaView, {
      props: {
        snapshot: makeSnapshot({ success: false, error: longError REDACTED),
      REDACTED,
    REDACTED)

    const error = wrapper.get('[data-testid="monitor-quota-error"]')
    expect(error.text()).toBe(`${'x'.repeat(48)REDACTED…`)
  ...
  it('keeps failed snapshots from rendering tier rows', () => {
    const wrapper = mount(MonitorQuotaView, {
      props: {
        snapshot: makeSnapshot({
          success: false,
          tiers: [{ window: '5h', used_percent: 10 REDACTED],
        REDACTED),
      REDACTED,
    REDACTED)
