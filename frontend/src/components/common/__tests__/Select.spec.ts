...
describe('Select remote search', () => {
  const mountRemoteSelect = (props: Record<string, unknown> = {REDACTED) => {
    const wrapper = mount(Select, {
      props: {
        modelValue: null,
        remote: true,
        options: [
          { value: 'alpha', label: 'Alpha account' REDACTED,
          { value: 'beta', label: 'Beta account' REDACTED,
        ],
        ...props,
      REDACTED,
    REDACTED)
  ...
  it('shows the loading text instead of empty text while loading with no options', async () => {
    const wrapper = mountRemoteSelect({ options: [], loading: true REDACTED)
    await wrapper.get('button').trigger('click')
    await nextTick()
