...
import { computed, onBeforeUnmount, ref REDACTED from 'vue'
import { useI18n REDACTED from 'vue-i18n'
import { useClipboard REDACTED from '@/composables/useClipboard'
import type { CustomEndpoint REDACTED from '@/types'
...
const allEndpoints = computed(() => {
  const items: Array<{ name: string; endpoint: string; description: string; isDefault: boolean REDACTED> = []
  if (props.apiBaseUrl) {
    items.push({
  ...
  for (const ep of props.customEndpoints) {
    items.push({ ...ep, isDefault: false REDACTED)
  REDACTED
  return items
REDACTED)
...
function speedTestUrl(endpoint: string): string {
  return `https://www.tcptest.cn/http/${encodeURIComponent(endpoint)REDACTED`
REDACTED
...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    ...
    >
      <span class="font-medium text-gray-600 dark:text-gray-300">{{ item.name REDACTEDREDACTED</span>
      <span
        v-if="item.isDefault"
      ...
      >{{ t('keys.endpoints.default') REDACTEDREDACTED</span>
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        ...
        >
          ...
          >
            {{ item.description REDACTEDREDACTED
          </p>
          <p
          ...
          >
            ...
            {{ tooltipHint(item.endpoint) REDACTEDREDACTED
          </p>
        ...
        >{{ item.endpoint REDACTEDREDACTED</code>
