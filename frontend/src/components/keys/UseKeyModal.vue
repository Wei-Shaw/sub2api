    ...
    <div class="space-y-4">
      ...
      <div v-if="!platform" class="flex items-start gap-3 p-4 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
        ...
        <div>
          <p class="text-sm font-medium text-yellow-800 dark:text-yellow-200">
            {{ t('keys.useKeyModal.noGroupTitle') REDACTEDREDACTED
          </p>
          <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
            {{ t('keys.useKeyModal.noGroupDescription') REDACTEDREDACTED
          </p>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ platformDescription REDACTEDREDACTED
        </p>
        ...
        >
          <div class="mb-2">
            <p class="text-sm font-medium text-gray-900 dark:text-white">
              {{ t('keys.useKeyModal.openai.authModeTitle') REDACTEDREDACTED
            </p>
            <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
              {{ t('keys.useKeyModal.openai.authModeDescription') REDACTEDREDACTED
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              {{ t('keys.useKeyModal.openai.authModeLegacy') REDACTEDREDACTED
            </button>
            <button
              type="button"
              role="radio"
            ...
            >
              {{ t('keys.useKeyModal.openai.authModeApiKey') REDACTEDREDACTED
            </button>
          </div>
          <div
          ...
          >
            ...
            <p>{{ t('keys.useKeyModal.openai.authModeApiKeyRestartNotice') REDACTEDREDACTED</p>
          </div>
        </div>
          ...
          >
            ...
            <div class="bg-gray-900 dark:bg-dark-900 rounded-xl overflow-hidden">
              ...
              <div class="flex items-center justify-between px-4 py-2 bg-gray-800 dark:bg-dark-800 border-b border-gray-700 dark:border-dark-700">
                <span class="min-w-0 truncate text-xs text-gray-400 font-mono">{{ file.path REDACTEDREDACTED</span>
                <button
                  type="button"
                ...
                >
                  ...
                  {{ copiedIndex === index ? t('keys.useKeyModal.copied') : t('keys.useKeyModal.copy') REDACTEDREDACTED
                </button>
              </div>
              <!-- Code Content -->
...
const AppleIcon = {
  render() {
    return h('svg', {
      ...
      class: 'w-4 h-4'
    REDACTED, [
      h('path', { d: 'M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z' REDACTED)
    ])
  REDACTED
REDACTED
...
const WindowsIcon = {
  render() {
    return h('svg', {
      ...
      class: 'w-4 h-4'
    REDACTED, [
      h('path', { d: 'M3 12V6.75l6-1.32v6.48L3 12zm17-9v8.75l-10 .15V5.21L20 3zM3 13l6 .09v6.81l-6-1.15V13zm7 .25l10 .15V21l-10-1.91v-5.84z' REDACTED)
    ])
  REDACTED
REDACTED
...
const clientTabs = computed((): TabConfig[] => {
  ...
  switch (props.platform) {
    ...
    case 'grok':
      return [
        { id: 'grok', label: t('keys.useKeyModal.cliTabs.grokCli'), icon: TerminalIcon REDACTED,
        { id: 'claude', label: t('keys.useKeyModal.cliTabs.claudeCode'), icon: TerminalIcon REDACTED,
        { id: 'codex', label: t('keys.useKeyModal.cliTabs.codexCli'), icon: TerminalIcon REDACTED,
        { id: 'opencode', label: t('keys.useKeyModal.cliTabs.opencode'), icon: TerminalIcon REDACTED
      ]
...
const shellTabs: TabConfig[] = [
  { id: 'unix', label: 'macOS / Linux', icon: AppleIcon REDACTED,
  { id: 'cmd', label: 'Windows CMD', icon: WindowsIcon REDACTED,
  { id: 'powershell', label: 'PowerShell', icon: WindowsIcon REDACTED
]

// OpenAI tabs (2 OS types)
const openaiTabs: TabConfig[] = [
  { id: 'unix', label: 'macOS / Linux', icon: AppleIcon REDACTED,
  { id: 'windows', label: 'Windows', icon: WindowsIcon REDACTED
]
...
const currentFiles = computed((): FileConfig[] => {
  ...
  const ensureV1 = (value: string) => {
    ...
    return trimmed.endsWith('/v1') ? trimmed : `${trimmedREDACTED/v1`
  REDACTED
  const apiBase = ensureV1(baseRoot)
  const antigravityBase = ensureV1(`${baseRootREDACTED/antigravity`)
  const antigravityGeminiBase = (() => {
    const trimmed = `${baseRootREDACTED/antigravity`.replace(/\/+$/, '')
    return trimmed.endsWith('/v1beta') ? trimmed : `${trimmedREDACTED/v1beta`
  REDACTED)()
  const geminiBase = (() => {
  ...
  switch (props.platform) {
    ...
    case 'antigravity':
      if (activeClientTab.value === 'gemini') {
        return [generateGeminiCliContent(`${baseUrlREDACTED/antigravity`, apiKey)]
      REDACTED
      return generateAnthropicFiles(`${baseUrlREDACTED/antigravity`, apiKey)
    case 'grok':
      if (activeClientTab.value === 'claude') {
  ...
  switch (activeTab.value) {
    case 'unix':
      ...
      content = Object.entries(environment)
        .map(([name, value]) => `export ${nameREDACTED="${valueREDACTED"`)
        .join('\n')
      break
    case 'cmd':
      path = 'Command Prompt'
      content = Object.entries(environment)
        .map(([name, value]) => `set ${nameREDACTED=${valueREDACTED`)
        .join('\n')
      break
    case 'powershell':
      path = 'PowerShell'
      content = Object.entries(environment)
        .map(([name, value]) => `$env:${nameREDACTED="${valueREDACTED"`)
        .join('\n')
      break
    default:
      path = 'Terminal'
...
function generateGeminiCliContent(baseUrl: string, apiKey: string): FileConfig {
  ...
  switch (activeTab.value) {
    case 'unix':
      path = 'Terminal'
      content = `export GOOGLE_GEMINI_BASE_URL="${baseUrlREDACTED"
export GEMINI_API_KEY="${apiKeyREDACTED"
export GEMINI_MODEL="${modelREDACTED"  # ${modelCommentREDACTED`
      highlighted = `${keyword('export')REDACTED ${variable('GOOGLE_GEMINI_BASE_URL')REDACTED${operator('=')REDACTED${string(`"${baseUrlREDACTED"`)REDACTED
${keyword('export')REDACTED ${variable('GEMINI_API_KEY')REDACTED${operator('=')REDACTED${string(`"${apiKeyREDACTED"`)REDACTED
${keyword('export')REDACTED ${variable('GEMINI_MODEL')REDACTED${operator('=')REDACTED${string(`"${modelREDACTED"`)REDACTED  ${comment(`# ${modelCommentREDACTED`)REDACTED`
      break
    case 'cmd':
      path = 'Command Prompt'
...
${comment(`REM ${modelCommentREDACTED`)REDACTED`
    ...
    case 'powershell':
      path = 'PowerShell'
      content = `$env:GOOGLE_GEMINI_BASE_URL="${baseUrlREDACTED"
$env:GEMINI_API_KEY="${apiKeyREDACTED"
$env:GEMINI_MODEL="${modelREDACTED"  # ${modelCommentREDACTED`
      highlighted = `${keyword('$env:')REDACTED${variable('GOOGLE_GEMINI_BASE_URL')REDACTED${operator('=')REDACTED${string(`"${baseUrlREDACTED"`)REDACTED
${keyword('$env:')REDACTED${variable('GEMINI_API_KEY')REDACTED${operator('=')REDACTED${string(`"${apiKeyREDACTED"`)REDACTED
${keyword('$env:')REDACTED${variable('GEMINI_MODEL')REDACTED${operator('=')REDACTED${string(`"${modelREDACTED"`)REDACTED  ${comment(`# ${modelCommentREDACTED`)REDACTED`
      break
    default:
      path = 'Terminal'
      content = ''
...
wire_api = "responses"
${generateCodexProviderAuthConfig()REDACTED
...
function joinConfigPath(dir: string, file: string, windows: boolean): string {
  if (!windows) return `${dirREDACTED/${fileREDACTED`
  return `${dirREDACTED\\${fileREDACTED`
REDACTED
...
function generateGrokFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      ...
      envContent = `set GROK_MODELS_BASE_URL=${baseUrlREDACTED
set XAI_API_KEY=${apiKeyREDACTED`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:GROK_MODELS_BASE_URL="${baseUrlREDACTED"
$env:XAI_API_KEY="${apiKeyREDACTED"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export GROK_MODELS_BASE_URL="${baseUrlREDACTED"
export XAI_API_KEY="${apiKeyREDACTED"`
  REDACTED
  ...
  const modelsListUrl = `${baseUrl.replace(/\/+$/, '')REDACTED/models`
...
[endpoints]
models_base_url = "${baseUrlREDACTED"              # inference base; model list defaults to {baseREDACTED/models
models_list_url = "${modelsListUrlREDACTED"        # optional override (env: GROK_MODELS_LIST_URL)
xai_api_base_url = "${baseUrlREDACTED"             # public xAI API base override for gateway routing
cli_chat_proxy_base_url = "${baseUrlREDACTED"      # CLI chat-proxy base (env: GROK_CLI_CHAT_PROXY_BASE_URL)
...
# base_url = "${baseUrlREDACTED"
env_key = "XAI_API_KEY"                     # or: api_key = "${apiKeyREDACTED"  (not recommended)
...
# lsp_tools = false`
  ...
  return [
    { path: envPath, content: envContent REDACTED,
    {
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set SUB2API_API_KEY=${apiKeyREDACTED`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:SUB2API_API_KEY="${apiKeyREDACTED"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export SUB2API_API_KEY="${apiKeyREDACTED"`
  REDACTED
...
# experimental_bearer_token = "${apiKeyREDACTED"
wire_api = "responses"
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  const grokModels = {
    'grok-4.5': {
      name: 'Grok 4.5',
      limit: { context: 500000, output: 64000 REDACTED
    REDACTED,
    'grok-build-0.1': {
      name: 'Grok Build 0.1',
      limit: { context: 256000, output: 64000 REDACTED
    REDACTED,
    'grok-4.20-multi-agent-0309': {
      name: 'Grok 4.20 Multi Agent (text / web_search)',
      limit: { context: 1000000, output: 64000 REDACTED
    REDACTED,
    'grok-4.3': {
      name: 'Grok 4.3',
      limit: { context: 1000000, output: 64000 REDACTED
  ...
  REDACTED else if (platform === 'anthropic') {
    provider[platform].npm = '@ai-sdk/anthropic'
  REDACTED else if (platform === 'antigravity-claude') {
    provider[platform].npm = '@ai-sdk/anthropic'
    provider[platform].name = 'Antigravity (Claude)'
    provider[platform].models = claudeModels
  REDACTED else if (platform === 'antigravity-gemini') {
    provider[platform].npm = '@ai-sdk/google'
