  ...
  >
    <div class="space-y-4">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400">
        {{ t('keys.useKeyModal.description') REDACTEDREDACTED
      </p>
      ...
      <div class="relative">
        <div class="bg-gray-900 dark:bg-dark-900 rounded-xl overflow-hidden">
          ...
          <div class="flex items-center justify-between px-4 py-2 bg-gray-800 dark:bg-dark-800 border-b border-gray-700 dark:border-dark-700">
            <span class="text-xs text-gray-400 font-mono">{{ activeTabConfig?.filename REDACTEDREDACTED</span>
            <button
              @click="copyConfig"
            ...
            >
              ...
              {{ copied ? t('keys.useKeyModal.copied') : t('keys.useKeyModal.copy') REDACTEDREDACTED
            </button>
          </div>
          <!-- Code Content -->
      ...
      <div class="flex items-start gap-3 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
        ...
        <p class="text-sm text-blue-700 dark:text-blue-300">
          {{ t('keys.useKeyModal.note') REDACTEDREDACTED
        </p>
      </div>
    </div>
...
import { ref, computed, h REDACTED from 'vue'
...
const tabs = [
  { id: 'unix' as const, label: 'macOS / Linux', icon: AppleIcon, filename: 'Terminal' REDACTED,
  { id: 'cmd' as const, label: 'Windows CMD', icon: WindowsIcon, filename: 'Command Prompt' REDACTED,
  { id: 'powershell' as const, label: 'PowerShell', icon: WindowsIcon, filename: 'PowerShell' REDACTED
]
...
const configCode = computed(() => {
  ...
  switch (activeTab.value) {
    case 'unix':
      return `export ANTHROPIC_BASE_URL="${baseUrlREDACTED"
export ANTHROPIC_AUTH_TOKEN="${apiKeyREDACTED"`
    case 'cmd':
      return `set ANTHROPIC_BASE_URL=${baseUrlREDACTED
set ANTHROPIC_AUTH_TOKEN=${apiKeyREDACTED`
    case 'powershell':
      return `$env:ANTHROPIC_BASE_URL="${baseUrlREDACTED"
...
const highlightedCode = computed(() => {
  ...
  switch (activeTab.value) {
    case 'unix':
      return `${keyword('export')REDACTED ${variable('ANTHROPIC_BASE_URL')REDACTED${operator('=')REDACTED${string(`"${baseUrlREDACTED"`)REDACTED
${keyword('export')REDACTED ${variable('ANTHROPIC_AUTH_TOKEN')REDACTED${operator('=')REDACTED${string(`"${apiKeyREDACTED"`)REDACTED`
    case 'cmd':
      return `${keyword('set')REDACTED ${variable('ANTHROPIC_BASE_URL')REDACTED${operator('=')REDACTED${baseUrlREDACTED
${keyword('set')REDACTED ${variable('ANTHROPIC_AUTH_TOKEN')REDACTED${operator('=')REDACTED${apiKeyREDACTED`
    case 'powershell':
      return `${keyword('$env:')REDACTED${variable('ANTHROPIC_BASE_URL')REDACTED${operator('=')REDACTED${string(`"${baseUrlREDACTED"`)REDACTED
