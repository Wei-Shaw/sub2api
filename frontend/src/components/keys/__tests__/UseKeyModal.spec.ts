...
const { copyToClipboardMock REDACTED = vi.hoisted(() => ({
...
describe('UseKeyModal', () => {
  ...
  it('renders copyable Claude Code setup through the Grok Messages gateway', async () => {
    ...
    for (const name of [
      ...
      expect(unixConfig).toContain(`export ${nameREDACTED="grok-4.5"`)
    REDACTED
  ...
  it('renders API Key Mode authorization in OpenAI Codex config', async () => {
    ...
    expect(configToml).toContain('http_headers = { "x-openai-actor-authorization" = "local-image-extension" REDACTED')
    expect(configToml).not.toContain('env_key')
    expect(configToml).not.toContain('image_generation')
    expect(codeBlocks).toContain('{\n  "OPENAI_API_KEY": "sk-test"\nREDACTED')
    expect(wrapper.text()).toContain('auth.json')
  ...
  it('resets Codex authentication mode when the modal reopens or platform changes', async () => {
    ...
    await wrapper.get('[data-testid="codex-auth-mode-api-key"]').trigger('click')
    await wrapper.setProps({ platform: 'gemini' REDACTED)
    await wrapper.setProps({ platform: 'openai' REDACTED)
    await nextTick()
  ...
  it('renders Claude Fable 5 OpenCode config with adaptive thinking', async () => {
    ...
    expect(fable.limit).toEqual({ context: 1048576, output: 128000 REDACTED)
    expect(fable.options.thinking).toEqual({ type: 'adaptive' REDACTED)
