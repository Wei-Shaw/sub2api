<template>
  ...
  >
    <!-- Logo/Brand -->
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed REDACTED">
      ...
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed REDACTED" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <router-link
          :to="homePath"
    ...
    <nav ref="sidebarNavRef" class="sidebar-nav scrollbar-hide">
      ...
      <template v-if="isAdmin">
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              ...
              >
                ...
                <span
                  ...
                  :class="{ 'sidebar-label-collapsed': sidebarCollapsed REDACTED"
                  :aria-hidden="sidebarCollapsed ? 'true' : 'false'"
                >
                  <span class="min-w-0 truncate">{{ item.label REDACTEDREDACTED</span>
                  <ChevronDownIcon
              ...
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="mb-1 ml-4 border-l border-gray-200 pl-2 dark:border-dark-600">
                <router-link
                  ...
                  :class="{ 'sidebar-link-active': route.path === child.path REDACTED"
                  @click="handleMenuItemClick(child.path)"
                >
                  <component :is="child.icon" class="h-4 w-4 flex-shrink-0" />
                  <span>{{ child.label REDACTEDREDACTED</span>
                </router-link>
              </div>
            </template>
            ...
            <router-link
              ...
              :class="{ 'sidebar-link-active': isActive(item.path), 'sidebar-link-collapsed': sidebarCollapsed REDACTED"
              :title="sidebarCollapsed ? item.label : undefined"
              :id="
            ...
            >
              ...
              <span class="sidebar-label" :class="{ 'sidebar-label-collapsed': sidebarCollapsed REDACTED" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">{{ item.label REDACTEDREDACTED</span>
            </router-link>
          </template>
        </div>
        ...
        <div v-if="!authStore.isSimpleMode" class="sidebar-section">
          <div class="sidebar-section-title" :class="{ 'sidebar-section-title-collapsed': sidebarCollapsed REDACTED" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
            <span class="sidebar-section-title-text" :class="{ 'sidebar-section-title-text-collapsed': sidebarCollapsed REDACTED">
    ...
    <div class="mt-auto border-t border-gray-100 p-3 dark:border-dark-800">
      ...
      <button
        ...
        :class="{ 'sidebar-link-collapsed': sidebarCollapsed REDACTED"
        :title="sidebarCollapsed ? (isDark ? t('nav.lightMode') : t('nav.darkMode')) : undefined"
      >
        <SunIcon v-if="isDark" class="h-5 w-5 flex-shrink-0 text-amber-500" />
        <MoonIcon v-else class="h-5 w-5 flex-shrink-0" />
        <span class="sidebar-label" :class="{ 'sidebar-label-collapsed': sidebarCollapsed REDACTED" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">{{
      ...
      >
        ...
        <span class="sidebar-label" :class="{ 'sidebar-label-collapsed': sidebarCollapsed REDACTED" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">{{ t('nav.collapse') REDACTEDREDACTED</span>
      </button>
    </div>
  </aside>
...
function applyFeatureFlags(items: NavItem[]): NavItem[] {
  ...
  for (const item of items) {
    ...
    if (item.children) {
      out.push({ ...item, children: applyFeatureFlags(item.children) REDACTED)
    REDACTED else {
      out.push(item)
    REDACTED
  REDACTED
...
const siteLogo = computed(() => sanitizeUrl(appStore.siteLogo || '', { allowRelative: true, allowDataUrl: true REDACTED))
...
const RechargeSubscriptionIcon = {
  render: () =>
    h(
      'svg',
      { fill: 'currentColor', viewBox: '0 0 1024 1024' REDACTED,
      [
        h('path', {
...
const PluginIcon = {
  render: () => h(Icon, { name: 'cube' REDACTED)
REDACTED
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  const items: NavItem[] = []
  if (withDashboard) {
    items.push({ path: '/dashboard', label: t('nav.dashboard'), icon: DashboardIcon REDACTED)
  REDACTED
  items.push(
    { path: '/keys', label: t('nav.apiKeys'), icon: KeyIcon REDACTED,
    { path: '/batch-image', label: t('nav.batchImage'), icon: BatchImageIcon, hideInSimpleMode: true, featureFlag: flagBatchImageAccess REDACTED,
    { path: '/usage', label: t('nav.usage'), icon: ChartIcon, hideInSimpleMode: true REDACTED,
    { path: '/available-channels', label: t('nav.availableChannels'), icon: ChannelIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels REDACTED,
    { path: '/monitor', label: t('nav.channelStatus'), icon: SignalIcon, featureFlag: flagChannelMonitor REDACTED,
    ...
    { path: '/affiliate', label: t('nav.affiliate'), icon: UsersIcon, hideInSimpleMode: true, featureFlag: flagAffiliate REDACTED,
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/ops', label: t('nav.ops'), icon: ChartIcon, featureFlag: flagOpsMonitoring REDACTED,
    ...
    { path: '/admin/groups', label: t('nav.groups'), icon: FolderIcon, hideInSimpleMode: true REDACTED,
    {
      ...
      children: [
        ...
        { path: '/admin/channels/monitor', label: t('nav.channelMonitor'), icon: SignalIcon, featureFlag: flagChannelMonitor REDACTED,
      ],
    REDACTED,
    ...
    { path: '/admin/plugins', label: t('nav.plugins'), icon: PluginIcon, featureFlag: flagPluginManagement REDACTED,
    ...
    { path: '/admin/proxies', label: t('nav.proxies'), icon: ServerIcon REDACTED,
    {
      ...
      children: [
        { path: '/admin/risk-control', label: t('nav.contentModeration'), icon: ShieldIcon REDACTED,
        { path: '/admin/prompt-audit', label: t('nav.promptAudit'), icon: ShieldIcon REDACTED,
      ],
    REDACTED,
    ...
    { path: '/admin/promo-codes', label: t('nav.promoCodes'), icon: GiftIcon, hideInSimpleMode: true REDACTED,
    {
      ...
      children: [
        { path: '/admin/affiliates/invites', label: t('nav.affiliateInviteRecords'), icon: UsersIcon REDACTED,
        { path: '/admin/affiliates/rebates', label: t('nav.affiliateRebateRecords'), icon: OrderIcon REDACTED,
        { path: '/admin/affiliates/transfers', label: t('nav.affiliateTransferRecords'), icon: CreditCardIcon REDACTED,
      ],
    REDACTED,
    {
      path: '/admin/orders',
      ...
      children: [
        ...
        { path: '/admin/orders/plans', label: t('nav.paymentPlans'), icon: CreditCardIcon REDACTED,
      ],
    REDACTED,
    { path: '/admin/usage', label: t('nav.usage'), icon: ChartIcon REDACTED,
    { path: '/admin/audit-logs', label: t('nav.auditLogs'), icon: ShieldIcon, hideInSimpleMode: true REDACTED
  ]
  ...
  visible.push({ path: '/admin/settings', label: t('nav.settings'), icon: CogIcon REDACTED)
  for (const cm of customMenuItemsForAdmin.value) {
    visible.push({ path: `/custom/${cm.idREDACTED`, label: cm.label, icon: null, iconSvg: cm.icon_svg REDACTED)
  REDACTED
  return visible
REDACTED)
