      ...
      <template v-if="isAdmin">
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-else-if="item.externalUrl">
              <a
...
function sanitizeMenuURI(uri?: string): string | undefined {
  const value = uri?.trim() || ''
  if (value.includes('\\')) return undefined
  return sanitizeUrl(value, { allowRelative: true }) || undefined
}
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    ...customMenuItemsForUser.value.map((item): NavItem => ({
      ...
      externalUrl: sanitizeMenuURI(item.uri),
  ...
  if (authStore.isSimpleMode) {
    ...
    for (const cm of customMenuItemsForAdmin.value) {
      filtered.push({
        ...
        icon: null,
        iconSvg: cm.icon_svg,
        externalUrl: sanitizeMenuURI(cm.uri),
        openInNewTab: cm.open_in_new_tab,
      })
    }
    return filtered
  }
