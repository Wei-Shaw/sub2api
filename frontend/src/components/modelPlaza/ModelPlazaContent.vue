<template>
  <div class="space-y-5">
    ...
    <div v-if="!embedded">
      <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">{{ t('modelPlaza.title') REDACTEDREDACTED</h1>
      <p class="mt-1.5 text-sm text-gray-500 dark:text-dark-400">{{ t('modelPlaza.description') REDACTEDREDACTED</p>
    </div>
    ...
    >
      ...
      {{ t('modelPlaza.anonymousHint') REDACTEDREDACTED
    </p>
    ...
    >
      {{ t('modelPlaza.loadFailed') REDACTEDREDACTED
    </div>
    <template v-else>
      <!-- 筛选区:平台 → 分组 → 倍率 -->
      <PlazaFilterBar
      ...
      >
        {{ searchActive ? t('modelPlaza.noSearchResult') : t('modelPlaza.empty') REDACTEDREDACTED
      </div>
    </template>
  </div>
</template>
...
import type { ModelPlazaGroup, ModelPlazaResponse REDACTED from '@/api/modelPlaza'
...
const filteredGroups = computed(() => {
  ...
  if (q) {
    groups = groups
      .map((g) => ({ ...g, models: g.models.filter((m) => m.name.toLowerCase().includes(q)) REDACTED))
