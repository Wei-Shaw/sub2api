    ...
    <table class="w-full min-w-[1000px] table-fixed border-collapse text-sm tabular-nums">
      ...
      <thead>
        ...
        >
          ...
          >
            {{ t('modelPlaza.table.model') REDACTEDREDACTED
          </th>
          <th colspan="3" class="pz-bg pt-2 text-center">
            <div class="pz-title border-b pb-2 font-semibold">
              {{ t('modelPlaza.table.paidPrice') REDACTEDREDACTED
              <span class="pz-unit ml-1 normal-case font-normal">{{ t('modelPlaza.table.unitPerMillion') REDACTEDREDACTED</span>
            </div>
          </th>
          <th
            colspan="3"
          ...
          >
            <div class="border-b border-gray-200 pb-2 text-gray-400 dark:border-dark-600 dark:text-dark-500">
              {{ t('modelPlaza.table.officialPrice') REDACTEDREDACTED
              <span class="ml-1 normal-case font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.unitPerMillion') REDACTEDREDACTED</span>
            </div>
          </th>
          <th
            rowspan="2"
          ...
          >
            {{ t('modelPlaza.table.rate') REDACTEDREDACTED
          </th>
        </tr>
        <tr
        ...
        >
          <th class="pz-bg px-3 py-2 font-medium">{{ t('modelPlaza.table.input') REDACTEDREDACTED</th>
          <th class="pz-bg px-3 py-2 font-medium">{{ t('modelPlaza.table.output') REDACTEDREDACTED</th>
          <th class="pz-bg px-3 py-2 font-medium">{{ t('modelPlaza.table.cache') REDACTEDREDACTED</th>
          <th class="border-l border-gray-100 px-3 py-2 font-medium dark:border-dark-700/60">
            {{ t('modelPlaza.table.input') REDACTEDREDACTED
          </th>
          <th class="px-3 py-2 font-medium">{{ t('modelPlaza.table.output') REDACTEDREDACTED</th>
          <th class="px-3 py-2 font-medium">{{ t('modelPlaza.table.cache') REDACTEDREDACTED</th>
        </tr>
      </thead>
      <tbody>
        <tr
          v-for="{ model: m, period, key REDACTED in rows"
          :key="key"
        ...
        >
          ...
          <td class="border-r border-gray-100 py-2.5 pl-5 pr-4 align-middle dark:border-dark-700/60">
            <div class="flex flex-wrap items-center gap-1.5">
              <span class="font-medium text-gray-900 dark:text-white">{{ m.name REDACTEDREDACTED</span>
              ...
              <span
                ...
                :title="t('modelPlaza.table.timePricingRowHint', { timezone: m.time_pricing?.timezone REDACTED)"
              >
                {{ formatTimeWindow(period) REDACTEDREDACTED
              </span>
              <span
              ...
              >
                {{ platformLabel(m.platform) REDACTEDREDACTED
              </span>
              <span
              ...
              >
                {{ billingModeLabel(m) REDACTEDREDACTED
              </span>
              <span
              ...
              >
                {{ t('modelPlaza.table.marginalBadge') REDACTEDREDACTED
              </span>
            </div>
          </td>
          ...
          <template v-if="billingMode(m) === BILLING_MODE_TOKEN">
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-gray-900 dark:text-gray-50">
              <template v-if="tokenIntervals(m).length">
                ...
                >
                  <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500" :title="tierHint(m)">{{ tierLabel(iv) REDACTEDREDACTED</span>
                  {{ paidPerMillion(iv.input_price, period) REDACTEDREDACTED
                </div>
              </template>
              <template v-else>{{ paidPerMillion(m.pricing?.input_price, period) REDACTEDREDACTED</template>
            </td>
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-gray-900 dark:text-gray-50">
              <template v-if="tokenIntervals(m).length">
                <div
                  ...
                  :title="tierHint(m)"
                >
                  {{ paidPerMillion(iv.output_price, period) REDACTEDREDACTED
                </div>
              </template>
              <template v-else>{{ paidPerMillion(m.pricing?.output_price, period) REDACTEDREDACTED</template>
            </td>
            <td class="pz-cell px-3 py-2.5 align-middle">
              <template v-if="hasTierCachePricing(tokenIntervals(m))">
                ...
                >
                  <template v-if="iv.cache_write_price != null || iv.cache_read_price != null">
                    <span class="font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheWriteShort') REDACTEDREDACTED</span>
                    {{ paidPerMillion(iv.cache_write_price, period) REDACTEDREDACTED
                    <span class="ml-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheReadShort') REDACTEDREDACTED</span>
                    {{ paidPerMillion(iv.cache_read_price, period) REDACTEDREDACTED
                  </template>
              ...
              >
                <div>
                  <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheWrite') REDACTEDREDACTED</span>
                  {{ paidPerMillion(m.pricing?.cache_write_price, period) REDACTEDREDACTED
                </div>
                <div>
                  <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheRead') REDACTEDREDACTED</span>
                  {{ paidPerMillion(m.pricing?.cache_read_price, period) REDACTEDREDACTED
                </div>
              </div>
              <span v-else class="text-gray-400 dark:text-dark-500">-</span>
            </td>
          ...
          <template v-else>
            <td colspan="3" class="pz-cell px-3 py-2.5 align-middle">
              ...
              >
                ...
                >
                  <span class="font-sans text-gray-400 dark:text-dark-500">{{ tierLabel(iv) REDACTEDREDACTED</span>
                  {{ paidRequestPrice(m, iv.per_request_price)
                  REDACTEDREDACTED<span class="font-sans text-gray-400 dark:text-dark-500">{{ perUnitSuffix(m) REDACTEDREDACTED</span>
                </span>
              </div>
              <template v-else-if="m.pricing?.per_request_price != null">
                <span class="font-mono font-semibold text-gray-900 dark:text-gray-50">
                  {{ paidRequestPrice(m, m.pricing.per_request_price) REDACTEDREDACTED
                </span>
                <span class="ml-1 text-xs text-gray-400 dark:text-dark-500">{{ perUnitSuffix(m) REDACTEDREDACTED</span>
              </template>
              <span v-else class="text-gray-400 dark:text-dark-500">-</span>
            </td>
          </template>
          ...
          >
            <template v-if="officialIntervals(m).length">
              ...
              >
                <span class="mr-1 font-sans text-gray-400 dark:text-dark-500" :title="t('modelPlaza.table.tierHint')">{{ tierLabel(iv) REDACTEDREDACTED</span>
                {{ official(iv.input_price) REDACTEDREDACTED
              </div>
            </template>
            <template v-else>{{ official(m.official_pricing?.input_price) REDACTEDREDACTED</template>
          </td>
          <td class="px-3 py-2.5 align-middle font-mono text-xs text-gray-500 dark:text-dark-400">
            <template v-if="officialIntervals(m).length">
              ...
              >
                {{ official(iv.output_price) REDACTEDREDACTED
              </div>
            </template>
            <template v-else>{{ official(m.official_pricing?.output_price) REDACTEDREDACTED</template>
          </td>
          <td class="px-3 py-2.5 align-middle">
            <template v-if="hasTierCachePricing(officialIntervals(m))">
              ...
              >
                <template v-if="iv.cache_write_price != null || iv.cache_read_price != null">
                  <span class="font-sans text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheWriteShort') REDACTEDREDACTED</span>
                  {{ official(iv.cache_write_price) REDACTEDREDACTED
                  <span class="ml-1 font-sans text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheReadShort') REDACTEDREDACTED</span>
                  {{ official(iv.cache_read_price) REDACTEDREDACTED
                </template>
                <span v-else class="text-gray-400 dark:text-dark-500">-</span>
              </div>
            </template>
            ...
            >
              <div>
                ...
                REDACTEDREDACTED<template v-if="m.official_pricing.cache_write_1h_price != null"
                  ><span class="font-sans text-gray-400 dark:text-dark-500"> (1h </span>{{ official(m.official_pricing.cache_write_1h_price)
                  REDACTEDREDACTED<span class="font-sans text-gray-400 dark:text-dark-500">)</span></template
                >
              </div>
              <div>
                <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheRead') REDACTEDREDACTED</span>
                {{ official(m.official_pricing.cache_read_price) REDACTEDREDACTED
              </div>
            </div>
            <span v-else class="text-gray-400 dark:text-dark-500">-</span>
          </td>
          ...
          >
            <span
              ...
              :title="t('modelPlaza.table.timePricingRateHint', { rate: effectiveRate, multiplier: period.multiplier REDACTED)"
              >{{ periodRate(period) REDACTEDREDACTEDx</span
            >
            <span
              v-else-if="usesIndependentImageRate(m)"
              class="font-bold text-gray-700 dark:text-gray-300"
              >{{ requestRate(m) REDACTEDREDACTEDx</span
            >
            <template v-else-if="hasCustomRate">
              <span class="mr-1 text-gray-400 line-through dark:text-dark-500">{{ rateMultiplier REDACTEDREDACTEDx</span>
              <span class="font-bold text-primary-600 dark:text-primary-400">{{ effectiveRate REDACTEDREDACTEDx</span>
            </template>
            <span v-else class="font-bold text-gray-700 dark:text-gray-300">{{ effectiveRate REDACTEDREDACTEDx</span>
          </td>
        </tr>
      </tbody>
    </table>
...
import { formatScaled REDACTED from '@/utils/pricing'
import { platformAccentColor, platformBadgeLightClass, platformLabel REDACTED from '@/utils/platformColors'
import {
  BILLING_MODE_TOKEN,
  BILLING_MODE_IMAGE,
  type BillingMode
REDACTED from '@/constants/channel'
import type { PlazaModel, PlazaTimePricingPeriod REDACTED from '@/api/modelPlaza'
import type { UserPricingInterval REDACTED from '@/api/channels'
...
const accentStyle = computed(() => ({ '--plaza-accent': platformAccentColor(props.platform ?? '') REDACTED))
...
const rows = computed<PlazaRow[]>(() =>
  sortedModels.value.flatMap((m) => {
    const base: PlazaRow = { model: m, period: null, key: `${m.platformREDACTED:${m.nameREDACTED` REDACTED
    const periodRows = timePeriods(m).map<PlazaRow>((p, idx) => ({
      model: m,
      period: p,
      key: `${m.platformREDACTED:${m.nameREDACTED:${idxREDACTED`
    REDACTED))
...
function formatTimeWindow(p: PlazaTimePricingPeriod): string {
  const clock = (v: string) => v.replace(/^(\d{2REDACTED:\d{2REDACTED):00$/, '$1')
  return `${clock(p.start_time)REDACTED–${clock(p.end_time)REDACTED`
REDACTED
...
function tierLabel(iv: UserPricingInterval): string {
  ...
  const { min_tokens: min, max_tokens: max REDACTED = iv
  return max == null ? `>${formatTokenCount(min)REDACTED` : `≤${formatTokenCount(max)REDACTED`
REDACTED

function formatTokenCount(n: number): string {
  if (n >= 1_000_000) return `${trimZero(n / 1_000_000)REDACTEDM`
  if (n >= 1_000) return `${trimZero(n / 1_000)REDACTEDK`
  return String(n)
REDACTED
