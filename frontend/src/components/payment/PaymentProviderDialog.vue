    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="input-label">
            {{ t('admin.settings.payment.providerName') REDACTEDREDACTED
        ...
        <div>
          <label class="input-label">
            {{ t('admin.settings.payment.providerKey') REDACTEDREDACTED
            <span class="text-red-500">*</span>
          </label>
          <Select
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="supportsPaymentMode" class="flex items-center gap-2">
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.paymentMode') REDACTEDREDACTED</span>
          <div class="flex gap-1.5">
            ...
            >{{ mode.label REDACTEDREDACTED</button>
          </div>
        </div>
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.supportedTypes') REDACTEDREDACTED</span>
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-700">
        <div class="mb-3 flex items-center gap-2">
          <h4 class="text-sm font-semibold text-gray-900 dark:text-white">
            {{ t('admin.settings.payment.providerConfig') REDACTEDREDACTED
          </h4>
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            ...
            <div class="space-y-3">
              <p class="font-medium text-white">{{ paymentGuide.summary REDACTEDREDACTED</p>
              <div
                v-for="item in paymentGuide.items"
                :key="item.title"
                class="space-y-1.5 border-t border-white/10 pt-2 first:border-t-0 first:pt-0"
              >
                <p class="font-medium text-white">{{ item.title REDACTEDREDACTED</p>
                <p><span class="text-gray-300">{{ t('admin.settings.payment.guideOpenLabel') REDACTEDREDACTED</span>{{ item.open REDACTEDREDACTED</p>
                <p><span class="text-gray-300">{{ t('admin.settings.payment.guideCallLabel') REDACTEDREDACTED</span>{{ item.call REDACTEDREDACTED</p>
                <p><span class="text-gray-300">{{ t('admin.settings.payment.guideFallbackLabel') REDACTEDREDACTED</span>{{ item.fallback REDACTEDREDACTED</p>
              </div>
              <p v-if="paymentGuide.note" class="border-t border-white/10 pt-2 text-[11px] text-gray-300">
                {{ paymentGuide.note REDACTEDREDACTED
              </p>
            </div>
          </HelpTooltip>
        </div>
        <p v-if="paymentGuide" class="mb-3 text-xs text-gray-500 dark:text-gray-400">
          {{ paymentGuide.summary REDACTEDREDACTED
        </p>
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label REDACTEDREDACTED
              <span v-if="field.optional" class="text-xs text-gray-400">({{ t('common.optional') REDACTEDREDACTED)</span>
            ...
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-gray-500 dark:text-gray-400">
              {{ t(field.hintKey) REDACTEDREDACTED
            </p>
          </div>
        </div>
        ...
        <div v-if="form.provider_key === 'easypay'" class="mt-4 space-y-3 rounded-lg border border-gray-100 p-3 dark:border-dark-700">
          <div class="flex items-center justify-between gap-3">
            <div>
              <h5 class="text-sm font-medium text-gray-900 dark:text-white">
                {{ t('admin.settings.payment.easypayCustomMethods') REDACTEDREDACTED
              </h5>
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.settings.payment.easypayCustomMethodsHint') REDACTEDREDACTED
              </p>
            </div>
            <button type="button" class="btn btn-secondary btn-sm" @click="addEasyPayCustomMethod">
              {{ t('admin.settings.payment.addCustomMethod') REDACTEDREDACTED
            </button>
          </div>
          <div v-if="easyPayCustomMethods.length" class="space-y-2">
            ...
            >
              <div>
                <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.customMethodType') REDACTEDREDACTED</label>
                <input v-model="method.type" type="text" class="input mt-0.5" placeholder="ldc" />
              </div>
              <div>
                <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.customMethodUpstreamType') REDACTEDREDACTED</label>
                <input v-model="method.upstreamType" type="text" class="input mt-0.5" placeholder="epay" />
              </div>
              <div>
                <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.customMethodDisplayName') REDACTEDREDACTED</label>
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            <label class="input-label">{{ t('admin.settings.payment.field_notifyUrl') REDACTEDREDACTED <span class="text-red-500">*</span></label>
            <div class="flex">
              <input v-model="notifyBaseUrl" type="text" class="input min-w-0 flex-1 !rounded-r-none !border-r-0" :placeholder="defaultBaseUrl" />
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-xs text-gray-500 dark:border-dark-600 dark:bg-dark-700 dark:text-gray-400">{{ callbackPaths.notifyUrl REDACTEDREDACTED</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            <label class="input-label">{{ t('admin.settings.payment.field_returnUrl') REDACTEDREDACTED <span class="text-red-500">*</span></label>
            <div class="flex">
              <input v-model="returnBaseUrl" type="text" class="input min-w-0 flex-1 !rounded-r-none !border-r-0" :placeholder="defaultBaseUrl" />
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-xs text-gray-500 dark:border-dark-600 dark:bg-dark-700 dark:text-gray-400">{{ callbackPaths.returnUrl REDACTEDREDACTED</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="providerWebhookUrl" class="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-800/50 dark:bg-blue-900/20">
          <p class="text-xs text-blue-700 dark:text-blue-300">
            {{ t(providerWebhookHint) REDACTEDREDACTED
          </p>
          <code class="mt-1 block break-all rounded bg-blue-100 px-2 py-1 text-xs text-blue-800 dark:bg-blue-900/40 dark:text-blue-200">
            {{ providerWebhookUrl REDACTEDREDACTED
          </code>
          <p v-if="form.provider_key === 'stripe'" class="mt-2 text-xs leading-relaxed text-blue-700 dark:text-blue-300">
            {{ t('admin.settings.payment.stripeWebhookApiVersionHint', { version: STRIPE_SDK_API_VERSION REDACTED) REDACTEDREDACTED
          </p>
        </div>
      </div>
      ...
      <div v-if="limitableTypes.length" class="border-t border-gray-200 pt-4 dark:border-dark-700">
        <button type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          <h4 class="text-sm font-semibold text-gray-900 dark:text-white">
            {{ t('admin.settings.payment.limitsTitle') REDACTEDREDACTED
          </h4>
        ...
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          ...
          >
            <p class="mb-2 text-xs font-medium text-gray-700 dark:text-gray-300">{{ lt.label REDACTEDREDACTED</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.limitSingleMin') REDACTEDREDACTED</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.limitSingleMax') REDACTEDREDACTED</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.limitDaily') REDACTEDREDACTED</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-gray-400 dark:text-gray-500">{{ t('admin.settings.payment.limitsHint') REDACTEDREDACTED</p>
        </div>
      </div>
    </form>
...
import { reactive, computed, ref REDACTED from 'vue'
...
import type { EasyPayCustomMethod, TypeOption REDACTED from './providerConfig'
import {
...
REDACTED)
const config = reactive<Record<string, string>>({REDACTED)
const limits = reactive<Record<string, Record<string, number>>>({REDACTED)
const notifyBaseUrl = ref('')
const returnBaseUrl = ref('')
const limitsExpanded = ref(false)
const visibleFields = reactive<Record<string, boolean>>({REDACTED)
...
const paymentModeOptions = computed(() => {
  if (form.provider_key === 'alipay') {
    ...
    return [
      { value: '', label: t('admin.settings.payment.modeQRCode') REDACTED,
      { value: PAYMENT_MODE_REDIRECT, label: t('admin.settings.payment.modeRedirect') REDACTED,
    ]
  REDACTED
  return [
    { value: PAYMENT_MODE_QRCODE, label: t('admin.settings.payment.modeQRCode') REDACTED,
    { value: PAYMENT_MODE_POPUP, label: t('admin.settings.payment.modePopup') REDACTED,
  ]
REDACTED)
...
const availableTypes = computed(() => {
  ...
  return base.map(opt =>
    opt.label === opt.value
      ? { ...opt, label: t(`payment.methods.${opt.valueREDACTED`, opt.value) REDACTED
      : opt,
  )
REDACTED)
...
const resolvedFields = computed(() => {
  ...
  return fields.map(f => ({
    ...f,
    label: f.label || t(`admin.settings.payment.field_${f.keyREDACTED`),
  REDACTED))
REDACTED)
...
function addEasyPayCustomMethod() {
  easyPayCustomMethods.push({ type: '', upstreamType: '', displayName: '' REDACTED)
REDACTED
...
function setLimitVal(paymentType: string, field: string, val: string) {
  if (!limits[paymentType]) limits[paymentType] = {REDACTED
...
function serializeLimits(): string {
  const result: Record<string, Record<string, number>> = {REDACTED
  for (const [pt, fields] of Object.entries(limits)) {
    const clean: Record<string, number> = {REDACTED
...
function handleSave() {
  ...
  for (const f of PROVIDER_CONFIG_FIELDS[form.provider_key] || []) {
    ...
    if (!val) {
      const label = f.label || t(`admin.settings.payment.field_${f.keyREDACTED`)
      emitValidationError(t('admin.settings.payment.validationFieldRequired', { field: label REDACTED))
      return
    REDACTED
  REDACTED
  ...
  const filteredConfig: Record<string, string> = {REDACTED
...
defineExpose({ reset, loadProvider REDACTED)
</script>
