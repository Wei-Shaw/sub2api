      ...
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-base font-semibold text-gray-900 dark:text-white">
            {{ t('admin.settings.payment.providerManagement') REDACTEDREDACTED
          </h2>
          <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.settings.payment.providerManagementDesc') REDACTEDREDACTED
          </p>
        </div>
        <div class="flex items-center gap-2">
          ...
          >
            {{ t('admin.settings.payment.createProvider') REDACTEDREDACTED
          </button>
        </div>
      </div>
    </div>
      ...
      <div v-else-if="!loading" class="py-6 text-center">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ canCreate
            ...
            : t('admin.settings.payment.enableTypesFirst') REDACTEDREDACTED
        </p>
        <button
          type="button"
          v-if="canCreate"
...
import { getAvailableTypes REDACTED from './providerConfig'
...
const emit = defineEmits<{
  ...
  reorder: [providers: { id: number; sort_order: number REDACTED[]]
REDACTED>()
