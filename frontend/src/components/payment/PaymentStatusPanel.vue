<template>
  <div class="space-y-4">
    ...
    <template v-if="outcome === 'success'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-gray-900 dark:text-white">{{ props.orderType === 'subscription' ? t('payment.result.subscriptionSuccess') : t('payment.result.success') REDACTEDREDACTED</p>
          <div v-if="paidOrder" class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              ...
              <div v-if="paidOrder.out_trade_no" class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.orderNo') REDACTEDREDACTED</span>
                <span class="font-medium text-gray-900 dark:text-white">{{ paidOrder.out_trade_no REDACTEDREDACTED</span>
              </div>
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium text-gray-900 dark:text-white">{{ formatGatewayAmount(paidOrder.pay_amount, paidOrder.currency) REDACTEDREDACTED</span>
              </div>
            </div>
          </div>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') REDACTEDREDACTED</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="outcome === 'cancelled'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-gray-900 dark:text-white">{{ t('payment.qr.cancelled') REDACTEDREDACTED</p>
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.cancelledDesc') REDACTEDREDACTED</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') REDACTEDREDACTED</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="outcome === 'expired'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-gray-900 dark:text-white">{{ t('payment.qr.expired') REDACTEDREDACTED</p>
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.expiredDesc') REDACTEDREDACTED</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') REDACTEDREDACTED</button>
        </div>
      </div>
    </template>
        ...
        <div class="card p-6">
          <div class="flex flex-col items-center space-y-4 py-4 text-center">
            ...
            <p class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ deepLinkState === 'backgrounded' ? t('payment.qr.alipayContinueInApp') : t('payment.qr.alipayOpening') REDACTEDREDACTED
            </p>
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.alipayWaitingHint') REDACTEDREDACTED</p>
            <button
            ...
            >
              ...
              {{ t('payment.qr.reopenAlipay') REDACTEDREDACTED
            </button>
          </div>
        </div>
        <div class="card p-4 text-center">
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.expiresIn') REDACTEDREDACTED</p>
        ...
        <div data-test="alipay-qr-fallback" class="card p-6">
          <div class="flex flex-col items-center space-y-4">
            <div class="text-center">
              <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.qr.alipayFallbackTitle') REDACTEDREDACTED</p>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.alipayFallbackHint') REDACTEDREDACTED</p>
            </div>
            <div class="w-full space-y-2 border-y border-gray-100 py-3 text-sm dark:border-dark-600">
              <div class="flex items-start justify-between gap-4">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') REDACTEDREDACTED</span>
                <span class="font-semibold text-gray-900 dark:text-white">{{ displayPaymentAmount REDACTEDREDACTED</span>
              </div>
              <div class="flex items-start justify-between gap-4">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.orderNo') REDACTEDREDACTED</span>
                <span class="max-w-[70%] break-all text-right font-mono text-xs text-gray-900 dark:text-white">
                  {{ displayOrderNumber REDACTEDREDACTED
                </span>
              </div>
              <div class="flex items-start justify-between gap-4">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.qr.expiresIn') REDACTEDREDACTED</span>
                <span class="font-semibold tabular-nums text-gray-900 dark:text-white">{{ countdownDisplay REDACTEDREDACTED</span>
              </div>
            </div>
            ...
            <p class="text-center text-sm leading-6 text-gray-600 dark:text-gray-300">
              {{ t('payment.qr.alipaySaveAndScanHint') REDACTEDREDACTED
            </p>
            <div class="grid w-full gap-2 sm:grid-cols-2">
              ...
              >
                ...
                {{ t('payment.qr.saveQRCode') REDACTEDREDACTED
              </button>
            </div>
    ...
    <template v-else-if="showQRCode">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4">
          <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ scanTitle REDACTEDREDACTED</p>
          <div :class="['relative rounded-lg border-2 p-4', qrBorderClass]">
            <canvas ref="qrCanvas" class="mx-auto"></canvas>
            <!-- Brand logo overlay -->
            <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
          ...
          <p v-if="scanHint" class="text-center text-sm text-gray-500 dark:text-gray-400">{{ scanHint REDACTEDREDACTED</p>
    ...
    <template v-else>
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.payInNewWindowHint') REDACTEDREDACTED</p>
          <button v-if="payUrl" class="btn btn-secondary text-sm" @click="reopenPopup">
            {{ t('payment.qr.openPayWindow') REDACTEDREDACTED
          </button>
        </div>
...
import { ref, computed, watch, onMounted, onUnmounted, nextTick REDACTED from 'vue'
...
import { currencySymbol, formatPaymentAmount, normalizePaymentCurrency REDACTED from '@/components/payment/currency'
...
const displayOrderNumber = computed(() => props.outTradeNo || `#${props.orderIdREDACTED`)
...
function updateDeepLinkState(state: AlipayDeepLinkState) {
  ...
  if (state === 'fallback') {
    ...
    renderQR()
  REDACTED else if (state === 'backgrounded') {
...
function saveQRCode() {
  ...
  link.download = `alipay-${props.outTradeNo || props.orderIdREDACTED.png`
...
function handleDone() { cleanup(); emit('done') REDACTED
