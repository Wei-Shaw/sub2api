<template>
  <div class="space-y-4">
    ...
    <div v-else-if="initError" class="card p-6 text-center">
      <p class="text-sm text-red-600 dark:text-red-400">{{ initError REDACTEDREDACTED</p>
      <button class="btn btn-secondary mt-4" @click="$emit('back')">{{ t('payment.result.backToRecharge') REDACTEDREDACTED</button>
    </div>
    <!-- Success -->
          ...
          <div class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium text-gray-900 dark:text-white">#{{ orderId REDACTEDREDACTED</span>
              </div>
              <div v-if="amount > 0" class="flex justify-between">
                ...
                <span class="font-medium text-gray-900 dark:text-white">{{ creditedAmountSymbol REDACTEDREDACTED{{ amount.toFixed(2) REDACTEDREDACTED</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') REDACTEDREDACTED</span>
                <span class="font-medium text-gray-900 dark:text-white">{{ paymentAmountSymbol REDACTEDREDACTED{{ payAmount.toFixed(2) REDACTEDREDACTED</span>
              </div>
            </div>
          </div>
          <button class="btn btn-primary" @click="$emit('done')">{{ t('common.confirm') REDACTEDREDACTED</button>
        </div>
      </div>
    </template>
    <template v-else>
      ...
      <div class="card overflow-hidden">
        <div class="bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-6 py-5 text-center">
          <p class="text-sm font-medium text-indigo-200">{{ t('payment.actualPay') REDACTEDREDACTED</p>
          <p class="mt-1 text-3xl font-bold text-white">{{ paymentAmountSymbol REDACTEDREDACTED{{ payAmount.toFixed(2) REDACTEDREDACTED</p>
        </div>
      </div>
      <!-- Stripe Payment Element -->
      <div class="card p-6">
        <div ref="stripeMount" class="min-h-[200px]"></div>
        <p v-if="error" class="mt-4 text-sm text-red-600 dark:text-red-400">{{ error REDACTEDREDACTED</p>
        <button class="btn btn-stripe mt-6 w-full py-3 text-base" :disabled="submitting || !ready" @click="handlePay">
          ...
          <span v-else>{{ t('payment.stripePay') REDACTEDREDACTED</span>
        </button>
      </div>
      <!-- Cancel order -->
...
import { computed, ref, onMounted, nextTick REDACTED from 'vue'
...
const emit = defineEmits<{ success: []; done: []; back: []; redirect: [orderId: number, payUrl: string] REDACTED>()
...
onMounted(async () => {
  try {
    ...
    if (!stripe) { initError.value = t('payment.stripeLoadFailed'); return REDACTED
    ...
    const elements = stripe.elements({
      ...
      appearance: { theme: isDark ? 'night' : 'stripe', variables: { borderRadius: '8px' REDACTED REDACTED,
    REDACTED)
    ...
    paymentElement.on('ready', () => { ready.value = true REDACTED)
    paymentElement.on('change', (event: { value: { type: string REDACTED REDACTED) => {
...
async function handlePay() {
  ...
  if (POPUP_METHODS.has(selectedType.value)) {
    ...
    const onReady = (event: MessageEvent) => {
      ...
      REDACTED, window.location.origin)
    REDACTED
  ...
  error.value = ''
  try {
    const { error: stripeError REDACTED = await stripeInstance.confirmPayment({
