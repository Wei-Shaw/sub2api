...
describe('PaymentMethodSelector', () => {
  it('wraps large custom method collections without letting labels widen the selector', () => {
    const methods = Array.from({ length: 12 REDACTED, (_, index) => ({
      type: `custom_${indexREDACTED`,
      display_name: `CUSTOM_PAYMENT_METHOD_${indexREDACTED`,
      fee_rate: 0,
      available: true,
    REDACTED))
  ...
  it('uses the generic selected style for custom methods that contain built-in names', () => {
    const wrapper = mount(PaymentMethodSelector, {
      props: {
        ...
        methods: [{ type: 'card_alipay', display_name: 'Card Pay', fee_rate: 0, available: true REDACTED],
      REDACTED,
    REDACTED)
