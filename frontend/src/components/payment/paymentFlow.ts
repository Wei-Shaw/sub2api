...
export function decidePaymentLaunch(
  ...
  if (visibleMethod === 'airwallex' && baseState.clientSecret && baseState.intentId) {
    ...
    REDACTED
    const paymentState = { ...baseState, payUrl: context.airwallexRouteUrl || '' REDACTED
    return { kind: 'airwallex_route', paymentState, recovery: paymentState REDACTED
  REDACTED
  ...
  if (result.result_type === 'oauth_required' && result.oauth?.authorize_url) {
    return { kind: 'wechat_oauth', paymentState: baseState, recovery: baseState, oauth: result.oauth REDACTED
  REDACTED

  const jsapiPayload = result.jsapi ?? result.jsapi_payload
  if (result.result_type === 'jsapi_ready' && jsapiPayload) {
    return { kind: 'wechat_jsapi', paymentState: baseState, recovery: baseState, jsapi: jsapiPayload REDACTED
  REDACTED
  ...
  if (
    ...
    return { kind: 'alipay_deep_link', paymentState: baseState, recovery: baseState REDACTED
  REDACTED
