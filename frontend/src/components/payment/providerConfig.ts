...
export const PAYMENT_CURRENCY_OPTIONS: TypeOption[] = [
  { value: 'CNY', label: 'CNY' REDACTED,
  { value: 'HKD', label: 'HKD' REDACTED,
  { value: 'USD', label: 'USD' REDACTED,
  { value: 'EUR', label: 'EUR' REDACTED,
  { value: 'GBP', label: 'GBP' REDACTED,
  { value: 'AUD', label: 'AUD' REDACTED,
  { value: 'CAD', label: 'CAD' REDACTED,
  { value: 'SGD', label: 'SGD' REDACTED,
  { value: 'JPY', label: 'JPY' REDACTED,
  { value: 'KRW', label: 'KRW' REDACTED,
  { value: 'NZD', label: 'NZD' REDACTED,
]
...
export function getPaymentPopupFeatures(): string {
  ...
  return `width=${widthREDACTED,height=${heightREDACTED,left=${leftREDACTED,top=${topREDACTED,scrollbars=yes,resizable=yes`
REDACTED
...
export const PROVIDER_CALLBACK_PATHS: Record<string, CallbackPaths> = {
  easypay: { notifyUrl: WEBHOOK_PATHS.easypay, returnUrl: RETURN_PATH REDACTED,
  alipay: { notifyUrl: WEBHOOK_PATHS.alipay, returnUrl: RETURN_PATH REDACTED,
  wxpay: { notifyUrl: WEBHOOK_PATHS.wxpay REDACTED,
...
export const PROVIDER_CONFIG_FIELDS: Record<string, ConfigFieldDef[]> = {
  easypay: [
    { key: 'pid', label: 'PID', sensitive: false REDACTED,
    { key: 'pkey', label: 'PKey', sensitive: true REDACTED,
    { key: 'apiBase', label: '', sensitive: false REDACTED,
    { key: 'cidAlipay', label: '', sensitive: false, optional: true REDACTED,
    { key: 'cidWxpay', label: '', sensitive: false, optional: true REDACTED,
  ],
  alipay: [
  ...
  stripe: [
    ...
    { key: 'publishableKey', label: '', sensitive: false REDACTED,
    { key: 'webhookSecret', label: '', sensitive: true REDACTED,
    { key: 'currency', label: '', sensitive: false, defaultValue: 'CNY', hintKey: 'admin.settings.payment.field_paymentCurrencyHint', options: PAYMENT_CURRENCY_OPTIONS REDACTED,
  ],
  airwallex: [
    { key: 'clientId', label: '', sensitive: false REDACTED,
    { key: 'apiKey', label: '', sensitive: true REDACTED,
    { key: 'webhookSecret', label: '', sensitive: true REDACTED,
    { key: 'apiBase', label: '', sensitive: false, defaultValue: 'https://api.airwallex.com/api/v1', hintKey: 'admin.settings.payment.field_airwallexApiBaseHint' REDACTED,
    { key: 'countryCode', label: '', sensitive: false, defaultValue: 'CN' REDACTED,
    { key: 'currency', label: '', sensitive: false, defaultValue: 'CNY', hintKey: 'admin.settings.payment.field_paymentCurrencyHint', options: PAYMENT_CURRENCY_OPTIONS REDACTED,
    { key: 'accountId', label: '', sensitive: false, optional: true, clearable: true, hintKey: 'admin.settings.payment.field_accountIdHint' REDACTED,
  ],
REDACTED
...
export function resolveTypeLabel(
  ...
  return allTypes.find(pt => pt.value === typeVal) || { value: typeVal, label: typeVal REDACTED
REDACTED
...
export function extractBaseUrl(fullUrl: string, path: string): string {
  ...
  try { return new URL(fullUrl).origin REDACTED catch { return fullUrl REDACTED
REDACTED
