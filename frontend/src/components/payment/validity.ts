...
export function planValiditySuffix(
  ...
  if (base === 'month') {
    return days === 1 ? t('payment.perMonth') : `${daysREDACTED${t('payment.months')REDACTED`
  REDACTED
  if (base === 'week') {
    return `${daysREDACTED${t('payment.weeks')REDACTED`
  REDACTED
  // 其余单位（含数据库默认的 day 与未知值）后端一律按天计费，展示保持一致。
  return `${daysREDACTED${t('payment.days')REDACTED`
REDACTED
