<template>
  <div class="sora-creator-bar-wrapper">
    <div class="sora-creator-bar">
      <div class="sora-creator-bar-inner" :class="{ focused: isFocused REDACTED">
        <!-- 模型选择行 -->
        <div class="sora-creator-model-row">
          <div class="sora-model-select-wrapper">
            ...
            >
              <optgroup v-if="videoFamilies.length" :label="t('sora.videoModels')">
                <option v-for="f in videoFamilies" :key="f.id" :value="f.id">{{ f.name REDACTEDREDACTED</option>
              </optgroup>
              <optgroup v-if="imageFamilies.length" :label="t('sora.imageModels')">
                <option v-for="f in imageFamilies" :key="f.id" :value="f.id">{{ f.name REDACTEDREDACTED</option>
              </optgroup>
            </select>
          ...
          <div class="sora-credential-select-wrapper">
            <select v-model="selectedCredentialId" class="sora-model-select">
              <option :value="0" disabled>{{ t('sora.selectCredential') REDACTEDREDACTED</option>
              <optgroup v-if="apiKeyOptions.length" :label="t('sora.apiKeys')">
                <option v-for="k in apiKeyOptions" :key="'k'+k.id" :value="k.id">
                  {{ k.name REDACTEDREDACTED{{ k.group ? ' · ' + k.group.name : '' REDACTEDREDACTED
                </option>
              </optgroup>
              <optgroup v-if="subscriptionOptions.length" :label="t('sora.subscriptions')">
                <option v-for="s in subscriptionOptions" :key="'s'+s.id" :value="-s.id">
                  {{ s.group?.name || t('sora.subscription') REDACTEDREDACTED
                </option>
              </optgroup>
            </select>
            <span class="sora-model-select-arrow">▼</span>
          ...
          <span v-if="soraCredentialEmpty" class="sora-no-storage-badge">
            ⚠ {{ t('sora.noCredentialHint') REDACTEDREDACTED
          </span>
          <!-- 无存储提示 -->
          <span v-if="!hasStorage" class="sora-no-storage-badge">
            ⚠ {{ t('sora.noStorageConfigured') REDACTEDREDACTED
          </span>
        </div>
        ...
        <div v-if="imagePreview" class="sora-image-preview-row">
          ...
          <span class="sora-image-preview-label">{{ t('sora.referenceImage') REDACTEDREDACTED</span>
        </div>
        ...
        <div class="sora-creator-tools-row">
          <div class="sora-creator-tools-left">
            ...
            <template v-if="availableAspects.length > 0">
              <button
                ...
                :key="a.value"
                class="sora-tool-btn"
                :class="{ active: currentAspect === a.value REDACTED"
                @click="currentAspect = a.value"
              >
                <span class="sora-tool-btn-icon">{{ a.icon REDACTEDREDACTED</span> {{ a.label REDACTEDREDACTED
              </button>
            ...
            <template v-if="availableDurations.length > 0">
              <button
                ...
                :key="d"
                class="sora-tool-btn"
                :class="{ active: currentDuration === d REDACTED"
            ...
            <template v-if="availableVideoCounts.length > 0">
              <button
                ...
                :key="count"
                class="sora-tool-btn"
                :class="{ active: currentVideoCount === count REDACTED"
                @click="currentVideoCount = count"
              >
                {{ count REDACTEDREDACTED
              </button>
          ...
          <span v-if="activeTaskCount > 0" class="sora-active-tasks-label">
            ...
            <span>{{ t('sora.generatingCount', { current: activeTaskCount, max: maxConcurrentTasks REDACTED) REDACTEDREDACTED</span>
          </span>

          <!-- 生成按钮 -->
          <button
            class="sora-generate-btn"
            :class="{ 'max-reached': isMaxReached REDACTED"
            :disabled="!canSubmit || generating || isMaxReached"
            @click="submit"
          >
            <span class="sora-generate-btn-icon">✨</span>
            <span>{{ generating ? t('sora.generating') : t('sora.generate') REDACTEDREDACTED</span>
          </button>
        </div>
      </div>
    </div>
    ...
    <!-- 文件大小错误 -->
    <p v-if="imageError" class="sora-image-error">{{ imageError REDACTEDREDACTED</p>
  </div>
</template>
...
import soraAPI, { type SoraModelFamily, type GenerateRequest REDACTED from '@/api/sora'
import keysAPI from '@/api/keys'
import { useSubscriptionStore REDACTED from '@/stores/subscriptions'
import type { ApiKey, UserSubscription REDACTED from '@/types'

const MAX_IMAGE_SIZE = 20 * 1024 * 1024

/** 方向显示配置 */
const ASPECT_META: Record<string, { icon: string; label: string REDACTED> = {
  landscape: { icon: '▬', label: '横屏' REDACTED,
  portrait:  { icon: '▮', label: '竖屏' REDACTED,
  square:    { icon: '◻', label: '方形' REDACTED
REDACTED
...
const availableAspects = computed(() => {
  ...
  return fam.orientations
    .map(o => ({ value: o, ...(ASPECT_META[o] || { icon: '?', label: o REDACTED) REDACTED))
REDACTED)
...
function buildModelID(): string {
  ...
  if (fam.type === 'image') {
    ...
    return currentAspect.value === 'square'
      ? fam.id
      : `${fam.idREDACTED-${currentAspect.valueREDACTED`
  REDACTED
  // 视频模型: "sora2-landscape-10s"
  return `${fam.idREDACTED-${currentAspect.valueREDACTED-${currentDuration.valueREDACTEDs`
REDACTED
...
async function loadSoraCredentials() {
  try {
    ...
    REDACTED else if (subscriptionOptions.value.length > 0) {
...
defineExpose({ fillPrompt REDACTED)
