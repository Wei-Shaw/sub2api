  ...
  <div class="card">
    ...
    <div class="space-y-3 p-4">
      ...
      <button v-if="canUseBatchImage" @click="router.push('/batch-image')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('dashboard.batchImageAgent') REDACTEDREDACTED</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('dashboard.batchImageAgentDesc') REDACTEDREDACTED</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
...
import { useBatchImageAccess REDACTED from '@/composables/useBatchImageAccess'
const router = useRouter()
const { t REDACTED = useI18n()
const { canUseBatchImage, refreshBatchImageAccess REDACTED = useBatchImageAccess()
