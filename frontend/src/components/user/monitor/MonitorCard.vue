    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="mt-0.5 flex items-center gap-1.5 min-w-0">
          ...
          >
            {{ item.group_name REDACTEDREDACTED
          </span>
        </div>
      </div>
      <span
      ...
      >
        {{ statusLabel(item.primary_status) REDACTEDREDACTED
      </span>
    </div>
...
import {
  ...
  providerGradient,
REDACTED from '@/composables/useChannelMonitorFormat'
import { isChannelMonitorQuotaVisible REDACTED from '@/utils/featureFlags'
...
const availabilityLabel = computed(() => {
  const win = t(`channelStatus.windowTab.${props.windowREDACTED`)
  return `${t('monitorCommon.availabilityPrefix')REDACTED · ${winREDACTED`
REDACTED)

const extraModelsCountLabel = computed(() => {
  const count = props.item.extra_models?.length ?? 0
  if (count === 0) return undefined
  return t('monitorCommon.extraModelsCount', { n: count REDACTED)
REDACTED)
</script>
