<template>
  <section class="pt-8 pb-10 md:pb-14">
    <div class="text-xs font-medium tracking-widest uppercase text-gray-400 dark:text-gray-500 mb-4">
      {{ t('channelStatus.hero.breadcrumb') REDACTEDREDACTED
    </div>
    <div class="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
      ...
      <div class="flex flex-col items-start md:items-end gap-2.5">
        ...
        <div class="text-xs text-gray-500 dark:text-gray-400 tabular-nums text-right">
          {{ updatedLabel REDACTEDREDACTED<span v-if="intervalSeconds > 0"> · {{ t('monitorCommon.pollEvery', { n: intervalSeconds REDACTED) REDACTEDREDACTED</span>
        </div>
      </div>
    </div>
  </section>
...
const { formatRelativeTime REDACTED = useChannelMonitorFormat()

const windowOptions = computed<{ value: MonitorWindow; label: string REDACTED[]>(() => [
  { value: '7d', label: t('channelStatus.windowTab.7d') REDACTED,
  { value: '15d', label: t('channelStatus.windowTab.15d') REDACTED,
  { value: '30d', label: t('channelStatus.windowTab.30d') REDACTED,
])

const overallLabel = computed(() => t(`channelStatus.overall.${props.overallStatusREDACTED`))
...
const updatedLabel = computed(() => {
  if (!props.updatedAt) return t('monitorCommon.updatedAt', { time: '--' REDACTED)
  return t('monitorCommon.updatedAt', { time: formatRelativeTime(props.updatedAt) REDACTED)
REDACTED)
</script>
