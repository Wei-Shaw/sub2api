<template>
  <div class="mt-4 pt-3 border-t border-gray-100 dark:border-dark-700/60">
    ...
    >
      <span>{{ t('monitorCommon.history60pts', { n: length REDACTED) REDACTEDREDACTED</span>
      <span class="tabular-nums">{{ t('monitorCommon.nextUpdateIn', { n: countdownSeconds REDACTED) REDACTEDREDACTED</span>
    </div>
    ...
    >
      {{ t('monitorCommon.maintenancePaused') REDACTEDREDACTED
    </div>
    <div v-else class="flex items-end gap-[2px] h-5 w-full">
      <div
        ...
        :style="{ height: bar.heightPct + '%' REDACTED"
        :title="bar.title"
      ></div>
    </div>
    ...
    >
      <span>{{ t('monitorCommon.past') REDACTEDREDACTED</span>
      <span>{{ t('monitorCommon.now') REDACTEDREDACTED</span>
    </div>
  </div>
</template>
...
import type { MonitorTimelinePoint REDACTED from '@/api/channelMonitor'
...
const { statusLabel, formatLatency, formatRelativeTime REDACTED = useChannelMonitorFormat()
...
const displayBars = computed<Bar[]>(() => {
  ...
  for (const point of real) {
    ...
    bars.push({
      colorClass,
      heightPct,
      title: `${relativeREDACTED · ${labelREDACTED · ${latencyREDACTEDms`,
    REDACTED)
  REDACTED
