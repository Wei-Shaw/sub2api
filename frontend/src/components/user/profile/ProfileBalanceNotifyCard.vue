      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          ...
          <div class="flex items-center gap-2">
            ...
            >
              {{ savingThreshold ? t('common.saving') : t('common.save') REDACTEDREDACTED
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <p class="mb-2 text-xs text-yellow-600 dark:text-yellow-400">{{ t('profile.balanceNotify.extraEmailsHint') REDACTEDREDACTED</p>
          ...
          <div v-if="emailEntries.length > 0" class="space-y-2 mb-3">
            <div v-for="(entry, idx) in emailEntries" :key="idx"
              ...
              <div class="flex items-center gap-2 min-w-0 flex-1">
                ...
                <span class="text-sm text-gray-700 dark:text-gray-300 truncate">{{ entry.email REDACTEDREDACTED</span>
              </div>
              <div class="flex items-center gap-2 shrink-0">
                <template v-if="!entry.verified">
                  ...
                  <template v-if="verifyingEmail === entry.email">
                    ...
                    <span v-if="verifyCountdown > 0" class="text-xs text-gray-400">{{ verifyCountdown REDACTEDREDACTEDs</span>
                  ...
                  <template v-else>
                    ...
                    <span class="text-xs text-yellow-500">{{ t('profile.balanceNotify.unverified') REDACTEDREDACTED</span>
                  </template>
                </template>
                <span v-else class="text-xs text-green-500">{{ t('profile.balanceNotify.verified') REDACTEDREDACTED</span>
          ...
          <p v-else class="text-xs text-gray-400">
            {{ t('profile.balanceNotify.maxEmailsReached') REDACTEDREDACTED
          </p>
        </div>
      </template>
    </div>
...
import type { NotifyEmailEntry REDACTED from '@/types'
...
watch(() => props.extraEmails, (val) => { emailEntries.value = [...val] REDACTED)
...
async function verifySavedEmail(email: string) {
  ...
  try {
    ...
    verifyCode.value = ''
    if (verifyTimer) { clearInterval(verifyTimer); verifyTimer = null REDACTED
    appStore.showSuccess(t('profile.balanceNotify.verifySuccess'))
    const updated = await userAPI.getProfile()
    authStore.user = updated
    emailEntries.value = [...updated.balance_notify_extra_emails]
