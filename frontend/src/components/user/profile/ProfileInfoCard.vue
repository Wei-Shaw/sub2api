            ...
            <div class="grid gap-3 sm:grid-cols-3">
              ...
              >
                <p class="text-xs font-medium uppercase tracking-[0.16em] text-gray-400 dark:text-gray-500">
                  {{ t('profile.accountBalance') REDACTEDREDACTED
                </p>
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                  {{ formatCurrency(user?.balance || 0) REDACTEDREDACTED
                </p>
              </div>
              <div
              ...
              >
                <p class="text-xs font-medium uppercase tracking-[0.16em] text-gray-400 dark:text-gray-500">
                  {{ t('profile.concurrencyLimit') REDACTEDREDACTED
                </p>
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                  {{ user?.concurrency || 0 REDACTEDREDACTED
                </p>
              </div>
              <div
              ...
              >
                <p class="text-xs font-medium uppercase tracking-[0.16em] text-gray-400 dark:text-gray-500">
                  {{ t('profile.memberSince') REDACTEDREDACTED
                </p>
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                  {{ memberSinceLabel REDACTEDREDACTED
                </p>
              </div>
            </div>
          </div>
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_320px]">
          ...
          <div class="mb-5 flex items-start justify-between gap-4">
            <div>
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ t('profile.basicsTitle') REDACTEDREDACTED
              </h3>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ t('profile.basicsDescription') REDACTEDREDACTED
              </p>
            </div>
          </div>
      ...
      <aside data-testid="profile-side-column" class="space-y-6">
        <section class="card border border-gray-100 bg-white/90 p-6 dark:border-dark-700 dark:bg-dark-900/50">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
            {{ t('profile.overviewTitle') REDACTEDREDACTED
          </h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {{ t('profile.overviewDescription') REDACTEDREDACTED
          </p>
          ...
          <div class="mt-5 grid gap-3">
            <div class="rounded-2xl border border-gray-100 bg-gray-50/80 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/30">
              ...
              <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">
                {{ user?.username || displayName REDACTEDREDACTED
              </p>
            </div>
            <div class="rounded-2xl border border-gray-100 bg-gray-50/80 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/30">
              <p class="text-xs font-medium uppercase tracking-[0.16em] text-gray-400 dark:text-gray-500">
                {{ t('profile.email') REDACTEDREDACTED
              </p>
              <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">
                {{ user?.email || '-' REDACTEDREDACTED
              </p>
        ...
        >
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
            {{ t('profile.linkedProfileSources') REDACTEDREDACTED
          </h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {{ t('profile.linkedProfileSourcesDescription') REDACTEDREDACTED
          </p>
          ...
          <div class="mt-5 grid gap-3">
            ...
            >
              ...
              <span>{{ hint.text REDACTEDREDACTED</span>
            </div>
          </div>
        </section>
      </aside>
...
): { provider: UserAuthProvider; label: string REDACTED | null {
  if (!rawSource) {
    return null
  REDACTED
...
const sourceHints = computed(() => {
  ...
  const hints: Array<{ key: string; text: string REDACTED> = []
  ...
  if (avatarSource) {
    hints.push({
      key: 'avatar',
      text: t('profile.authBindings.source.avatar', { providerName: avatarSource.label REDACTED)
    REDACTED)
  REDACTED
  ...
  if (usernameSource) {
    hints.push({
      key: 'username',
      text: t('profile.authBindings.source.username', { providerName: usernameSource.label REDACTED)
    REDACTED)
  REDACTED
