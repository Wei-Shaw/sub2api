  ...
  <div class="card">
    <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
      <h2 class="text-lg font-medium text-gray-900 dark:text-white">
        {{ t('profile.totp.title') REDACTEDREDACTED
      </h2>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        {{ t('profile.totp.description') REDACTEDREDACTED
      </p>
    </div>
    <div class="px-6 py-6">
      <!-- Loading state -->
      ...
      <div v-else-if="status && !status.feature_enabled" class="flex items-center gap-4 py-4">
        ...
        <div>
          <p class="font-medium text-gray-700 dark:text-gray-300">
            {{ t('profile.totp.featureDisabled') REDACTEDREDACTED
          </p>
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ t('profile.totp.featureDisabledHint') REDACTEDREDACTED
          </p>
        </div>
      </div>
      ...
      <div v-else-if="status?.enabled" class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="font-medium text-gray-900 dark:text-white">
              {{ t('profile.totp.enabled') REDACTEDREDACTED
            </p>
            <p v-if="status.enabled_at" class="text-sm text-gray-500 dark:text-gray-400">
              {{ t('profile.totp.enabledAt') REDACTEDREDACTED: {{ formatDate(status.enabled_at) REDACTEDREDACTED
            </p>
          </div>
        </div>
        <button
        ...
        >
          {{ t('profile.totp.disable') REDACTEDREDACTED
        </button>
      </div>
      ...
      <div v-else class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="font-medium text-gray-700 dark:text-gray-300">
              {{ t('profile.totp.notEnabled') REDACTEDREDACTED
            </p>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              {{ t('profile.totp.notEnabledHint') REDACTEDREDACTED
            </p>
          </div>
        </div>
        <button
        ...
        >
          {{ t('profile.totp.enable') REDACTEDREDACTED
        </button>
      </div>
    </div>
...
import type { TotpStatus REDACTED from '@/types'
