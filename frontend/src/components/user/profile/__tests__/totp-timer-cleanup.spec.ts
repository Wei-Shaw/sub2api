import { beforeEach, afterEach, describe, expect, it, vi REDACTED from 'vitest'
...
describe('TOTP 弹窗定时器清理', () => {
  ...
  beforeEach(() => {
    ...
    mocks.getVerificationMethod.mockResolvedValue({ method: 'email' REDACTED)
    mocks.sendVerifyCode.mockResolvedValue({ success: true REDACTED)
    mocks.initiateSetup.mockResolvedValue({
      qr_code_url: 'otpauth://totp/Sub2API:test?secret=ABC123',
      secret: 'ABC123',
      setup_token: 'setup-token'
    REDACTED)
    mocks.enable.mockResolvedValue({ success: true REDACTED)
    mocks.disable.mockResolvedValue({ success: true REDACTED)
    ...
    REDACTED) as typeof window.setInterval)
  ...
  it('TotpSetupModal 失败时改用 toast 并不渲染内联错误', async () => {
    mocks.getVerificationMethod.mockResolvedValue({ method: 'password' REDACTED)
    mocks.initiateSetup.mockRejectedValue({
      response: { data: { message: 'setup failed' REDACTED REDACTED
    REDACTED)
  ...
  it('TotpDisableDialog 失败时改用 toast 并不渲染内联错误', async () => {
    ...
    mocks.disable.mockRejectedValue({
      response: { data: { message: 'disable failed' REDACTED REDACTED
    REDACTED)
