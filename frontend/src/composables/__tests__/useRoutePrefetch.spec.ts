...
import type { RouteLocationNormalized, Router, RouteRecordNormalized REDACTED from 'vue-router'

import { useRoutePrefetch, _adminPrefetchMap, _userPrefetchMap REDACTED from '../useRoutePrefetch'
...
const createMockRouter = (): Router => {
  const mockImportFn = vi.fn().mockResolvedValue({ default: {REDACTED REDACTED)

  const routes: Partial<RouteRecordNormalized>[] = [
    { path: '/admin/dashboard', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/admin/accounts', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/admin/users', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/admin/groups', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/admin/subscriptions', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/admin/redeem', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/dashboard', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/keys', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/usage', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/redeem', components: { default: mockImportFn REDACTED REDACTED,
    { path: '/profile', components: { default: mockImportFn REDACTED REDACTED
  ]
...
describe('useRoutePrefetch', () => {
  ...
  describe('_isAdminRoute', () => {
    it('应该正确识别管理员路由', () => {
      const { _isAdminRoute REDACTED = useRoutePrefetch(mockRouter)
  ...
  describe('_getPrefetchConfig', () => {
    it('管理员 dashboard 应该返回正确的预加载配置', () => {
      const { _getPrefetchConfig REDACTED = useRoutePrefetch(mockRouter)
  ...
  describe('triggerPrefetch', () => {
    it('应该在浏览器空闲时触发预加载', async () => {
      const { triggerPrefetch, prefetchedRoutes REDACTED = useRoutePrefetch(mockRouter)
      const route = createMockRoute('/admin/dashboard')
  ...
  describe('cancelPendingPrefetch', () => {
    it('应该取消挂起的预加载任务', () => {
      const { triggerPrefetch, cancelPendingPrefetch, prefetchedRoutes REDACTED = useRoutePrefetch(mockRouter)
      const route = createMockRoute('/admin/dashboard')
  ...
  describe('resetPrefetchState', () => {
    it('应该重置所有预加载状态', async () => {
      const { triggerPrefetch, resetPrefetchState, prefetchedRoutes REDACTED = useRoutePrefetch(mockRouter)
      const route = createMockRoute('/admin/dashboard')
  ...
  describe('requestIdleCallback 超时处理', () => {
    it('超时后仍能正常执行预加载', async () => {
      ...
      vi.stubGlobal('requestIdleCallback', (cb: IdleRequestCallback, options?: IdleRequestOptions) => {
        ...
        return setTimeout(() => cb({ didTimeout: true, timeRemaining: () => 0 REDACTED), timeout)
      REDACTED)
  ...
  describe('预加载失败处理', () => {
    it('预加载失败时应该静默处理不影响页面功能', async () => {
      const { triggerPrefetch REDACTED = useRoutePrefetch(mockRouter)
      const route = createMockRoute('/admin/dashboard')
  ...
  describe('无 router 时的行为', () => {
    it('没有传入 router 时应该正常工作但不执行预加载', async () => {
      const { triggerPrefetch, prefetchedRoutes REDACTED = useRoutePrefetch()
      const route = createMockRoute('/admin/dashboard')
