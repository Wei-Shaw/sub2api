...
import type { CheckMode, MonitorStatus, Provider REDACTED from '@/api/admin/channelMonitor'
import {
  PROVIDER_OPENAI,
  PROVIDER_ANTHROPIC,
  PROVIDER_GEMINI,
...
export function useChannelMonitorFormat() {
  ...
  function statusLabel(s: MonitorStatus | ''): string {
    ...
    return t(`monitorCommon.status.${sREDACTED`)
  REDACTED
  ...
  function providerLabel(p: Provider | string): string {
    if (PROVIDERS.includes(p as Provider)) {
      return t(`monitorCommon.providers.${pREDACTED`)
    REDACTED
    return p || '-'
  REDACTED
  ...
  function checkModeLabel(m: CheckMode | string): string {
    if (m === 'probe' || m === 'quota' || m === 'quota_probe') {
      return t(`monitorCommon.checkMode.${mREDACTED`)
    REDACTED
    return m || '-'
  REDACTED
  ...
  function formatRelativeTime(iso: string | null | undefined): string {
    ...
    if (diffSec < 60) return t('monitorCommon.relativeSecondsAgo', { n: diffSec REDACTED)
    const diffMin = Math.floor(diffSec / 60)
    if (diffMin < 60) return t('monitorCommon.relativeMinutesAgo', { n: diffMin REDACTED)
    const diffHour = Math.floor(diffMin / 60)
    if (diffHour < 24) return t('monitorCommon.relativeHoursAgo', { n: diffHour REDACTED)
    const diffDay = Math.floor(diffHour / 24)
    return t('monitorCommon.relativeDaysAgo', { n: diffDay REDACTED)
  REDACTED
...
export function hslForPct(pct: number | null | undefined): string | undefined {
  ...
  return `hsl(${hueREDACTED ${HSL_SATURATIONREDACTED% ${HSL_LIGHTNESSREDACTED%)`
REDACTED
