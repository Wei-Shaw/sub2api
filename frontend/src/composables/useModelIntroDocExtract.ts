...
- Nested types are supported: use { "properties": { "<child>": <schema> }, ... } for objects and { "items": <schema>, "value": [<default items>], ... } for arrays. Omit the array "value" when no defaults are documented.
- For an array of media URLs, use the matching canonical widget: "ImageUrls" for images, "VideoUrls" for videos, or "AudioUrls" for audio. Example: { "items": { "value": "", "widget": "image" }, "widget": "ImageUrls", "maxItems": N, "description": ..., "extra": { "x-order": N } }. These widgets render one gallery-style multi-media input; set "maxItems" to the documented maximum (omit it when the docs state no limit). "maxItems" may also be used on any other array type to cap the element count.
