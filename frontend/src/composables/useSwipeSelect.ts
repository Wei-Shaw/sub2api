import { ref, onMounted, onUnmounted, type Ref REDACTED from 'vue'
import type { Virtualizer REDACTED from '@tanstack/vue-virtual'
...
export function useSwipeSelect(
  ...
  function onSelectStart(e: Event) { e.preventDefault() REDACTED
  ...
  function removeMarquee() {
    if (marqueeEl) { marqueeEl.remove(); marqueeEl = null REDACTED
  REDACTED
  ...
  function getScrollParent(el: HTMLElement): HTMLElement {
    ...
    while (parent && parent !== document.documentElement) {
      const { overflow, overflowY REDACTED = getComputedStyle(parent)
  ...
  function onThresholdMove(e: MouseEvent) {
    ...
    document.addEventListener('wheel', onWheel, { passive: true REDACTED)
  REDACTED
