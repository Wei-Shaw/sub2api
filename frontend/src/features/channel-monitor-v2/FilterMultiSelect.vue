<template>
  ...
  >
    ...
    >
      ...
      >
        {{ t('channelMonitorV2.filters.labelValue', { label, value: selectionLabel REDACTED) REDACTEDREDACTED
      </span>
      ...
      <Transition name="select-dropdown">
        ...
        >
          <button
            ...
            @click="clear"
          >
            <span>{{ allLabel REDACTEDREDACTED</span>
          ...
          >
            <span class="flex min-w-0 flex-1 items-center gap-2">
              ...
              <span class="min-w-0 flex-1 truncate">{{ option.label REDACTEDREDACTED</span>
            </span>
            <small v-if="option.count != null" class="text-xs text-gray-400">{{ formatCount(option.count) REDACTEDREDACTED</small>
          </button>
          <p v-if="options.length === 0" class="px-4 py-3 text-center text-xs text-gray-400">{{ t('channelMonitorV2.filters.empty') REDACTEDREDACTED</p>
        </div>
      </Transition>
    </Teleport>
  </div>
...
import type { CSSProperties REDACTED from 'vue'
import { useI18n REDACTED from 'vue-i18n'
import { computed, nextTick, onBeforeUnmount, ref, watch REDACTED from 'vue'
import Icon from '@/components/icons/Icon.vue'
import { monitorIntlLocale REDACTED from '@/features/channel-monitor-v2/monitorFormat'
...
const props = withDefaults(
  defineProps<{
    ...
    compact?: boolean
  REDACTED>(),
  { compact: false REDACTED,
)
const emit = defineEmits<{ 'update:modelValue': [value: string[]] REDACTED>()
...
const instanceId = `filter-select-${Math.random().toString(36).slice(2, 9)REDACTED`
...
const selectionLabel = computed(() => {
  ...
  REDACTED
  return t('channelMonitorV2.filters.selectedCount', { count: props.modelValue.length REDACTED)
REDACTED)

const dropdownStyle = computed<CSSProperties>(() => {
  const trigger = triggerRef.value
  if (!trigger) return {REDACTED
...
function positionDropdown() {
  ...
  const preferredMinWidth = Math.max(200, rect.width)
  const minWidth = Math.min(preferredMinWidth, availableWidth)
  dropdown.style.left = `${leftREDACTEDpx`
  dropdown.style.top = `${rect.bottom + 4REDACTEDpx`
  dropdown.style.minWidth = `${minWidthREDACTEDpx`
  dropdown.style.maxWidth = `${availableWidthREDACTEDpx`
REDACTED

function formatCount(value: number) {
  return Intl.NumberFormat(locale.value || monitorIntlLocale(), {
    notation: value >= 10000 ? 'compact' : 'standard',
  REDACTED).format(value)
REDACTED
