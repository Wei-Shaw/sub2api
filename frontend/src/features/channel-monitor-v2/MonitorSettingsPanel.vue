  ...
  <section class="mx-auto w-full max-w-6xl space-y-5 px-1 py-2 sm:px-2">
    ...
    >
      <div class="min-w-0">
        <h2 class="page-title flex items-center gap-2 text-xl font-black text-gray-900 dark:text-white">
          ...
          {{ t('channelMonitorV2.settings.title') REDACTEDREDACTED
        </h2>
        <p class="page-description mt-1.5 text-xs text-gray-500 dark:text-gray-400">
          {{ t('channelMonitorV2.settings.description') REDACTEDREDACTED
        </p>
      </div>
      <button
        type="button"
      ...
      >
        ...
        {{ t('channelMonitorV2.settings.save') REDACTEDREDACTED
      </button>
    </header>
    ...
    >
      {{
        ...
        REDACTED)
      REDACTEDREDACTED
      <router-link class="ml-1 font-medium underline" to="/admin/settings">{{ t('admin.settings.tabs.features') REDACTEDREDACTED</router-link>
    </div>
    ...
    >
      <span class="animate-pulse">{{ t('channelMonitorV2.settings.loading') REDACTEDREDACTED</span>
    </div>
    ...
    <template v-else-if="draft">
      <div class="card divide-y divide-gray-100 !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:divide-dark-700 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
          <div>
            <strong class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('channelMonitorV2.settings.enableTitle') REDACTEDREDACTED</strong>
            <p class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">
              {{ t('channelMonitorV2.settings.enableHint') REDACTEDREDACTED
            </p>
          </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
          <div>
            <strong class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('channelMonitorV2.settings.refreshTitle') REDACTEDREDACTED</strong>
            <p class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">{{ t('channelMonitorV2.settings.refreshHint') REDACTEDREDACTED</p>
          </div>
      ...
      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('channelMonitorV2.settings.platformsTitle') REDACTEDREDACTED</h3>
          <p class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">
            {{ t('channelMonitorV2.settings.platformsHint') REDACTEDREDACTED
          </p>
        </div>
        <div class="divide-y divide-gray-100 dark:divide-dark-700">
          ...
          >
            ...
            <strong class="text-sm font-medium text-gray-900 dark:text-white">{{ platformLabel(platform.platform) REDACTEDREDACTED</strong>
            <input
              class="input"
            ...
            >
              {{ platform.models.length ? t('channelMonitorV2.settings.badgeOther') : t('channelMonitorV2.settings.badgeAllModels') REDACTEDREDACTED
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header flex flex-wrap items-center justify-between gap-2 !py-3">
          <div>
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('channelMonitorV2.settings.groupsTitle') REDACTEDREDACTED</h3>
            <p class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">
              {{
                draft.group_ids.length
                  ? t('channelMonitorV2.settings.groupsSelected', { count: draft.group_ids.length REDACTED)
          ...
          >
            {{ t('channelMonitorV2.settings.groupsAll') REDACTEDREDACTED
          </button>
        </div>
        <div class="max-h-[min(40vh,280px)] overflow-y-auto px-3 py-2 sm:px-4">
          <div class="grid grid-cols-1 gap-1 sm:grid-cols-2">
            ...
            >
              ...
              />
              <span class="min-w-0 flex-1 truncate font-medium text-gray-800 dark:text-gray-100">{{ group.name REDACTEDREDACTED</span>
              <small class="shrink-0 text-xs text-gray-400">{{ platformLabel(group.platform) REDACTEDREDACTED · #{{ group.id REDACTEDREDACTED</small>
            </label>
          </div>
          <p v-if="groups.length === 0" class="empty-state py-8 text-sm text-gray-400">{{ t('channelMonitorV2.settings.groupsEmpty') REDACTEDREDACTED</p>
        </div>
      </div>

      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('channelMonitorV2.settings.errorsTitle') REDACTEDREDACTED</h3>
          <p class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">
            {{ t('channelMonitorV2.settings.errorsHint') REDACTEDREDACTED
          </p>
        </div>
          ...
          <div class="grid grid-cols-1 gap-1 sm:grid-cols-2">
            ...
            >
              ...
              <span class="min-w-0 flex-1 truncate font-medium text-gray-800 dark:text-gray-100">
                {{ categoryLabel(category) REDACTEDREDACTED
              </span>
              <small class="shrink-0 font-mono text-[10px] text-gray-400">{{ category REDACTEDREDACTED</small>
            </label>
          </div>
        </div>
      ...
      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('channelMonitorV2.settings.healthTitle') REDACTEDREDACTED</h3>
          <p class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">
            {{ t('channelMonitorV2.settings.healthHint') REDACTEDREDACTED
          </p>
        </div>
        <div class="grid grid-cols-1 gap-4 px-5 py-4 sm:grid-cols-2 lg:grid-cols-4">
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.minimumSample') REDACTEDREDACTED</span>
            <input v-model.number="draft.health_thresholds.minimum_sample" class="input" type="number" min="1" max="10000" />
          </label>
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.warningError') REDACTEDREDACTED</span>
            <input v-model.number="warningErrorPercent" class="input" type="number" min="0" max="100" step="0.1" />
          </label>
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.criticalError') REDACTEDREDACTED</span>
            <input v-model.number="criticalErrorPercent" class="input" type="number" min="0" max="100" step="0.1" />
          </label>
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.targetTtft') REDACTEDREDACTED</span>
            <input v-model.number="draft.health_thresholds.target_ttft_ms" class="input" type="number" min="1" step="100" />
          </label>
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.warningTtft') REDACTEDREDACTED</span>
            <input v-model.number="draft.health_thresholds.warning_ttft_ms" class="input" type="number" min="1" step="100" />
          </label>
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.criticalTtft') REDACTEDREDACTED</span>
            <input v-model.number="draft.health_thresholds.critical_ttft_ms" class="input" type="number" min="1" step="100" />
          </label>
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.warningCache') REDACTEDREDACTED</span>
            <input v-model.number="warningCachePercent" class="input" type="number" min="0" max="100" step="0.1" />
          </label>
          <label class="block">
            <span class="input-label">{{ t('channelMonitorV2.settings.fields.criticalCache') REDACTEDREDACTED</span>
      ...
      <div class="space-y-2">
        <div class="rounded-2xl border border-primary-200 bg-primary-50/80 px-4 py-3 text-sm text-primary-900 dark:border-primary-800/50 dark:bg-primary-900/20 dark:text-primary-100">
          <template v-if="namedModelCount === 0">
            {{ t('channelMonitorV2.settings.namedModelsEmpty') REDACTEDREDACTED
          </template>
          <template v-else>
            {{ t('channelMonitorV2.settings.namedModelsCount', { count: namedModelCount REDACTED) REDACTEDREDACTED
          </template>
        </div>
        <div class="rounded-2xl border border-gray-200 bg-gray-50/80 px-4 py-3 text-xs text-gray-600 dark:border-dark-600 dark:bg-dark-800/50 dark:text-gray-300">
          <p class="font-medium text-gray-800 dark:text-gray-100">{{ t('channelMonitorV2.settings.userContractTitle') REDACTEDREDACTED</p>
          <ul class="mt-1.5 list-disc space-y-0.5 pl-4">
            <li>{{ t('channelMonitorV2.settings.userContract.health') REDACTEDREDACTED</li>
            <li>{{ t('channelMonitorV2.settings.userContract.trend') REDACTEDREDACTED</li>
            <li>{{ t('channelMonitorV2.settings.userContract.latency') REDACTEDREDACTED</li>
            <li>{{ t('channelMonitorV2.settings.userContract.models') REDACTEDREDACTED</li>
          </ul>
        </div>
      </div>
    </template>
...
import { getChannelMonitorMode, isChannelMonitorV2Mode REDACTED from '@/utils/featureFlags'
import {
  getConfig,
  updateConfig,
...
function categoryLabel(category: string) {
  const key = `channelMonitorV2.errorCategories.${categoryREDACTED`
  ...
  return (
    {
      ...
      composite: 'Composite',
    REDACTED as Record<string, string>
  )[value] || value
REDACTED
...
function normalizeConfig(value: MonitorConfig): MonitorConfig {
  ...
  return {
    ...value,
    health_thresholds: { ...defaultThresholds, ...(value.health_thresholds || {REDACTED) REDACTED,
