              ...
              >
                <span class="pulse-tooltip" role="tooltip">
                  <template v-if="slot.bucket">
                    ...
                    <span v-if="showThroughput" class="pulse-tooltip-line">{{ t('channelMonitorV2.metrics.tpmValue', { value: formatRate(slot.bucket.metrics.tpm) REDACTED) REDACTEDREDACTED</span>
...
function bucketTooltipLines(bucket: MonitorMatrixBucket): string[] {
  ...
  if (props.showThroughput) {
    lines.push(
      t('channelMonitorV2.metrics.rpmValue', { value: formatRate(metrics.rpm) REDACTED),
      t('channelMonitorV2.metrics.tpmValue', { value: formatRate(metrics.tpm) REDACTED),
    )
  REDACTED
  lines.push(
    t('channelMonitorV2.metrics.ttftValue', { value: latencyPrivacy(metrics.ttft) REDACTED),
    t('channelMonitorV2.metrics.durationValue', { value: latencyPrivacy(metrics.duration) REDACTED),
