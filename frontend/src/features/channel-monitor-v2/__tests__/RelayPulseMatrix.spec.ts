const i18nT = (key: string, params?: Record<string, unknown>) => {
  const map: Record<string, string> = {
    ...
    'channelMonitorV2.bucket.minutes': '{countREDACTED分钟',
    ...
    'channelMonitorV2.matrix.scoreLine': '评分 {scoreREDACTED',
    'channelMonitorV2.metrics.successRateValue': '成功率 {valueREDACTED',
    'channelMonitorV2.metrics.errorRateValue': '错误率 {valueREDACTED',
    'channelMonitorV2.metrics.rpmValue': 'RPM {valueREDACTED',
    'channelMonitorV2.metrics.tpmValue': 'TPM {valueREDACTED',
    'channelMonitorV2.metrics.tpsValue': '每秒 Token {valueREDACTED',
    'channelMonitorV2.metrics.ttftValue': '首 Token {valueREDACTED',
    'channelMonitorV2.metrics.durationValue': '时长 {valueREDACTED',
    'channelMonitorV2.metrics.cacheRateValue': '缓存率 {valueREDACTED',
    'channelMonitorV2.matrix.noTrafficAt': '{timeREDACTED 无流量',
  ...
  return template.replace(/\{(\w+)\REDACTED/g, (_, name) => String(params?.[name] ?? ''))
REDACTED
...
import type { MonitorHealth, MonitorMetric REDACTED from '@/api/channelMonitorV2'
...
function metrics(requestCount: number): MonitorMetric {
  return {
    ...
    ttft: { sample_count: requestCount, p50_ms: 100, p95_ms: 300, avg_ms: 150 REDACTED,
    duration: { sample_count: requestCount, p50_ms: 500, p95_ms: 900, avg_ms: 600 REDACTED,
    ...
    const wrapper = mount(RelayPulseMatrix, {
      props: {
        rows: [{
          ...
          health,
          buckets: [
            { bucket_start: '2026-08-01T00:00:00Z', metrics: metrics(10), health REDACTED,
            { bucket_start: '2026-08-01T00:01:00Z', metrics: metrics(0), health REDACTED,
          ],
        REDACTED],
        coverage: {
    ...
    const wrapper = mount(RelayPulseMatrix, {
      props: {
        rows: [{
          ...
          health,
          buckets: [
            { bucket_start: '2026-08-01T00:02:00Z', metrics: metrics(10), health REDACTED,
          ],
        REDACTED],
        coverage: {
          requested_start: '2026-08-01T00:00:00Z',
