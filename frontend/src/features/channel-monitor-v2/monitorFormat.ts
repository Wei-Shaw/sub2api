...
import type { HealthScoreBand, MonitorHealth REDACTED from '@/api/channelMonitorV2'
...
export function formatMonitorNumber(value: number, compactAt = 10000, locale = monitorIntlLocale()): string {
  ...
  REDACTED).format(value || 0)
REDACTED

export function formatMonitorRate(value: number, locale = monitorIntlLocale()): string {
  return Intl.NumberFormat(locale, { maximumFractionDigits: 1 REDACTED).format(value || 0)
REDACTED
...
export function formatMonitorThroughput(value: number | null | undefined): string {
  ...
  if (Math.abs(n) < 1000) {
    return Intl.NumberFormat(monitorIntlLocale(), { maximumFractionDigits: 1 REDACTED).format(n)
  REDACTED
...
export function formatMonitorPercent(value: number, locale = monitorIntlLocale()): string {
  ...
  REDACTED).format((value || 0) * 100)REDACTED%`
REDACTED

export function formatMonitorMs(value: number | null | undefined): string {
  if (value == null) return '-'
  return value >= 1000 ? `${(value / 1000).toFixed(1)REDACTEDs` : `${Math.round(value)REDACTEDms`
REDACTED
...
export function scoreToBand(score: number | null | undefined): HealthScoreBand {
  ...
  return `score${Math.max(0, Math.min(10, band))REDACTED` as HealthScoreBand
REDACTED
...
export function healthScoreClass(
  ...
  REDACTED
  return `health-${scoreToBand(score)REDACTED`
REDACTED

export function healthStateClass(state: string | undefined): string {
  return `health-${state || 'unknown'REDACTED`
REDACTED
...
export function formatLatencyPrivacy(
  ...
  const parts: string[] = []
  if (avg != null) parts.push(`AVG ${formatMonitorMs(avg)REDACTED`)
  if (p50 != null) parts.push(`P50 ${formatMonitorMs(p50)REDACTED`)
  if (p90 != null) parts.push(`P90 ${formatMonitorMs(p90)REDACTED`)
  // p95 only as fallback when p90 missing (older payloads)
  if (p90 == null && p95 != null) parts.push(`P95 ${formatMonitorMs(p95)REDACTED`)
...
export function formatLatencyKpiSecondary(
  ...
  const parts: string[] = []
  if (avg != null && Number.isFinite(avg)) parts.push(`AVG ${formatMonitorMs(avg)REDACTED`)
  if (p90 != null && Number.isFinite(p90)) parts.push(`P90 ${formatMonitorMs(p90)REDACTED`)
  else if (p95 != null && Number.isFinite(p95)) parts.push(`P95 ${formatMonitorMs(p95)REDACTED`)
  return parts.length ? parts.join(' · ') : '-'
REDACTED
