...
import type { PromptAuditDraft, PromptAuditEndpointDraft, PromptAuditEvent, PromptEventFilters REDACTED from '../types'
import { emptyEventFilters, resolveDeleteRangeFilters, SCANNER_CATALOG REDACTED from '../viewModel'
...
const PaginationStub = defineComponent({ props: ['total', 'page', 'pageSize'], emits: ['update:page', 'update:pageSize'], template: '<div data-test="pagination" />' REDACTED)
...
describe('Prompt Audit components', () => {
  ...
  it('edits a saved endpoint with blank-secret keep, explicit clear, replacement, and probe actions', async () => {
    const wrapper = mount(EndpointPool, {
      props: { endpoints: [endpoint()], probeResults: {REDACTED, probingIds: [] REDACTED,
      global: { stubs: { BaseDialog: DialogStub REDACTED REDACTED,
    REDACTED)
    ...
    expect(updated[0]).toMatchObject({ token: 'replacement-canary', clear_token: false REDACTED)
    ...
    expect(wrapper.emitted('probe')?.[0]?.[0]).toMatchObject({ id: 'guard-1' REDACTED)
  REDACTED)

  it('surfaces an undecryptable saved credential and prompts for re-entry', async () => {
    const invalidEndpoint = { ...endpoint(), token_status: 'invalid' REDACTED
    const wrapper = mount(EndpointPool, {
      props: { endpoints: [invalidEndpoint], probeResults: {REDACTED, probingIds: [] REDACTED,
      global: { stubs: { BaseDialog: DialogStub REDACTED REDACTED,
    REDACTED)
  ...
  it('supports group search, stale configured groups, nine scanners, and bounded worker inputs', async () => {
    ...
    const wrapper = mount(PolicyPanel, {
      props: { draft, groups: [{ id: 1, name: 'Alpha', platform: 'openai', status: 'active' REDACTED, { id: 2, name: 'Beta', platform: 'claude', status: 'inactive' REDACTED] REDACTED,
    REDACTED)
    expect(wrapper.text()).toContain('99')
    expect(wrapper.findAll('input[type="checkbox"]').filter((input) => SCANNER_CATALOG.some((scanner) => input.attributes('aria-label') === `admin.promptAudit.scanners.${scanner.idREDACTED`))).toHaveLength(9)
  ...
  it('resolves delete range presets to an epoch start and a cutoff end', () => {
    ...
    const customSource = { ...emptyEventFilters(), start_at: '2026-07-01T00:00', end_at: '2026-07-02T00:00' REDACTED
  ...
  it('drives filter deletion through presets, custom validation, preview, and confirm', async () => {
    const wrapper = mount(FilterDeleteDialog, {
      props: { show: true, initialFilters: emptyEventFilters(), preview: null, previewing: false, deleting: false REDACTED,
      global: { stubs: { BaseDialog: DialogStub REDACTED REDACTED,
    REDACTED)
    ...
    await wrapper.setProps({
      preview: { matched_count: 3, filter_summary: {REDACTED, snapshot_max_id: 9, filter_hash: 'b'.repeat(64), confirmation_token: 'tok', expires_at: '2026-07-16T00:05:00Z' REDACTED,
    REDACTED)
  ...
  it('explains that a zero-match preview leaves nothing to delete', async () => {
    const wrapper = mount(FilterDeleteDialog, {
      props: {
        ...
        preview: { matched_count: 0, filter_summary: {REDACTED, snapshot_max_id: 0, filter_hash: 'c'.repeat(64), confirmation_token: 'tok', expires_at: '2026-07-16T00:05:00Z' REDACTED,
        previewing: false,
        deleting: false,
      REDACTED,
      global: { stubs: { BaseDialog: DialogStub REDACTED REDACTED,
    ...
    await wrapper.setProps({ previewing: true REDACTED)
    expect(wrapper.find('[data-test="confirm-disabled-reason"]').exists()).toBe(false)
    expect(wrapper.get('[data-test="confirm-filter-delete"]').attributes()).toHaveProperty('disabled')
  REDACTED)

  it('inherits an explicit list-filter range as the custom preset', async () => {
    const initialFilters = { ...emptyEventFilters(), start_at: '2026-07-01T00:00', end_at: '2026-07-02T00:00', decision: 'critical' REDACTED
    const wrapper = mount(FilterDeleteDialog, {
      props: { show: true, initialFilters, preview: null, previewing: false, deleting: false REDACTED,
      global: { stubs: { BaseDialog: DialogStub REDACTED REDACTED,
    REDACTED)
  ...
  it('shows the full unredacted prompt and structured guard return on the risks tab', async () => {
    const event: PromptAuditEvent = {
      ...
      scanner_scores: { sexual_content_or_sexual_acts: 1 REDACTED,
      scanner_evidence: { sexual_content_or_sexual_acts: 'Sexual Content or Sexual Acts' REDACTED,
    ...
    const wrapper = mount(EventDetailDialog, {
      props: { show: true, event, loading: false REDACTED,
      global: { stubs: { BaseDialog: DialogStub REDACTED REDACTED,
    REDACTED)
  ...
  it('falls back to the redacted preview for events stored before full prompts were kept', async () => {
    const event: PromptAuditEvent = {
      ...
      categories: ['pii'], matched_scanners: ['pii'], scanner_scores: {REDACTED, scanner_evidence: {REDACTED,
