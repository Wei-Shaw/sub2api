<template>
  <section aria-labelledby="prompt-events-title" class="py-6">
    ...
    <div v-if="error" role="alert" class="mt-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-danger dark:bg-red-950/30">{{ error }}</div>
    <div class="mt-5 overflow-x-auto rounded-xl border border-line/60">
      <table class="min-w-[1120px] w-full text-left text-sm">
        <thead class="bg-gray-50 text-xs uppercase tracking-wide text-ink-secondary dark:bg-dark-900/70 dark:text-dark-400">
          <tr>
        ...
        <tbody class="divide-y divide-gray-100 bg-white dark:divide-dark-700 dark:bg-transparent">
          <tr v-if="loading"><td colspan="8" class="px-4 py-12 text-center text-ink-secondary" aria-busy="true">{{ t('common.loading') }}</td></tr>
          <tr v-else-if="events.length === 0"><td colspan="8" class="px-4 py-12 text-center text-ink-secondary">{{ t('admin.promptAudit.events.empty') }}</td></tr>
          <tr v-for="event in events" v-else :key="event.id" :data-test="`event-${event.id}`" class="align-top hover:bg-gray-50/70 dark:hover:bg-dark-800/70">
            ...
            <td class="whitespace-nowrap px-3 py-3 text-xs text-ink-secondary">{{ formatDate(event.created_at) }}</td>
            <td class="px-3 py-3">
            ...
            <td class="px-3 py-3 text-ink-secondary">{{ event.snapshot.group_name || '—' }}</td>
            <td class="px-3 py-3">
              <p class="font-medium text-ink">{{ event.snapshot.endpoint }}</p>
              <p class="mt-1 text-xs text-ink-secondary">{{ event.snapshot.model }} · {{ event.snapshot.protocol }} · {{ event.snapshot.stage || 'http' }}</p>
            </td>
            <td class="px-3 py-3">
              <span class="rounded-sm px-2 py-0.5 text-xs font-medium" :class="decisionClass(event.decision)">{{ formatDecisionRisk(event.decision, event.risk_level) }}</span>
              <p class="mt-2 max-w-48 truncate text-xs text-ink-secondary" :title="formatCategories(event.categories)">{{ formatCategories(event.categories) }}</p>
            </td>
            <td class="max-w-xs px-3 py-3"><p class="line-clamp-2 break-words text-ink-secondary">{{ event.snapshot.redacted_preview || '—' }}</p></td>
...
const CopyLine = defineComponent({
  ...
  setup(componentProps) {
    return () => h('div', { class: 'flex max-w-56 items-center gap-1 text-xs' }, [
      h('span', { class: 'w-16 flex-none text-ink-secondary' }, componentProps.label),
...
function decisionClass(decision: string): string {
  if (decision === 'critical') return 'bg-red-100 text-danger dark:bg-red-950/50'
  if (decision === 'flag') return 'bg-amber-100 text-warn dark:bg-amber-950/50'
  return 'bg-emerald-100 text-success dark:bg-emerald-950/50'
}
