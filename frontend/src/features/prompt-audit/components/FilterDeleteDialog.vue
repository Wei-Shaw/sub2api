<template>
  <BaseDialog :show="show" :title="t('admin.promptAudit.events.filterDeleteDialogTitle')" width="wide" @close="$emit('close')">
    <div class="space-y-5 text-sm">
      <p class="text-gray-500 dark:text-dark-300">{{ t('admin.promptAudit.events.filterDeleteDialogDesc') REDACTEDREDACTED</p>

      <fieldset>
        <legend class="text-xs font-medium text-gray-600 dark:text-dark-200">{{ t('admin.promptAudit.events.filterTimeRange') REDACTEDREDACTED</legend>
        <div class="mt-2 flex flex-wrap gap-2" role="radiogroup" :aria-label="t('admin.promptAudit.events.filterTimeRange')">
          ...
          >
            <input v-model="preset" type="radio" name="prompt-delete-range" :value="option.id" class="peer sr-only" :data-test="`range-preset-${option.idREDACTED`" @change="criteriaChanged" />
            <span class="inline-flex items-center rounded-full border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 transition-colors peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-700 peer-focus-visible:ring-2 peer-focus-visible:ring-red-500/30 dark:border-dark-600 dark:text-dark-300 dark:peer-checked:border-red-500 dark:peer-checked:bg-red-950/40 dark:peer-checked:text-red-300">
              {{ t(`admin.promptAudit.events.timePresets.${option.idREDACTED`) REDACTEDREDACTED
            </span>
          </label>
        </div>
        <p class="mt-2 text-xs text-gray-500 dark:text-dark-400">{{ t('admin.promptAudit.events.filterTimeRangeHint') REDACTEDREDACTED</p>
        <div v-if="preset === 'custom'" class="mt-3 grid gap-3 sm:grid-cols-2" data-test="custom-range">
          ...
          <p v-if="!canPreview" class="text-xs text-red-600 dark:text-red-400 sm:col-span-2">{{ t('admin.promptAudit.events.customRangeInvalid') REDACTEDREDACTED</p>
        </div>
      </fieldset>
      ...
      <details class="rounded-xl border border-gray-200 px-4 py-3 dark:border-dark-700/60" data-test="more-conditions">
        <summary class="cursor-pointer select-none text-xs font-medium text-gray-600 dark:text-dark-200">{{ t('admin.promptAudit.events.moreConditions') REDACTEDREDACTED</summary>
        <div class="mt-3 grid gap-3 sm:grid-cols-2">
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.endpoint') REDACTEDREDACTED</span>
          ...
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.keyword') REDACTEDREDACTED</span>
          ...
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.groupId') REDACTEDREDACTED</span>
          ...
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.userId') REDACTEDREDACTED</span>
      ...
      <div v-if="preview" class="rounded-xl border border-red-200 bg-red-50/60 px-4 py-3 dark:border-red-900/60 dark:bg-red-950/20" data-test="delete-preview-result">
        <p class="text-sm font-semibold text-red-700 dark:text-red-300">{{ t('admin.promptAudit.events.filterDeleteCount', { count: preview.matched_count REDACTED) REDACTEDREDACTED</p>
        <dl class="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs text-gray-600 dark:text-dark-300">
          <dt>{{ t('admin.promptAudit.events.snapshotMax') REDACTEDREDACTED</dt>
          <dd>{{ preview.snapshot_max_id REDACTEDREDACTED</dd>
          <dt>Filter SHA-256</dt>
          <dd class="break-all font-mono">{{ preview.filter_hash REDACTEDREDACTED</dd>
          <dt>{{ t('admin.promptAudit.events.expiresAt') REDACTEDREDACTED</dt>
          <dd>{{ formatDate(preview.expires_at) REDACTEDREDACTED</dd>
        </dl>
        <p class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:bg-amber-950/30 dark:text-amber-200">{{ t('admin.promptAudit.events.filterDeleteWarning') REDACTEDREDACTED</p>
      </div>
      <p v-else class="rounded-xl border border-dashed border-gray-300 px-4 py-3 text-xs text-gray-500 dark:border-dark-600 dark:text-dark-400" data-test="delete-preview-empty">
        {{ t('admin.promptAudit.events.filterDeleteNeedPreview') REDACTEDREDACTED
      </p>
    </div>
...
import type { PromptDeletePreview, PromptEventFilters REDACTED from '../types'
import {
