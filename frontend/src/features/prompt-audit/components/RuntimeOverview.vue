    ...
    <template v-else-if="runtime">
      <dl class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <div v-for="item in statusItems" :key="item.label" class="rounded-xl border border-gray-100 bg-gray-50/80 px-3 py-3 dark:border-dark-700/60 dark:bg-dark-900/40">
          <dt class="text-xs text-gray-500 dark:text-dark-400">{{ item.label REDACTEDREDACTED</dt>
          <dd class="mt-1.5 flex items-center gap-2 text-sm font-semibold text-gray-900 dark:text-white">
            ...
            <span class="min-w-0 truncate">{{ item.value REDACTEDREDACTED</span>
          </dd>
        </div>
      </dl>
        ...
        <div class="rounded-xl border border-gray-100 px-4 py-3 dark:border-dark-700/60 dark:bg-dark-900/20">
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div v-for="metric in guardMetricItems" :key="metric.label" class="rounded-lg bg-gray-50 px-2.5 py-2 dark:bg-dark-900/60">
              <p class="text-[11px] text-gray-500 dark:text-dark-400">{{ metric.label REDACTEDREDACTED</p>
              <p class="mt-0.5 text-sm font-semibold tabular-nums text-gray-900 dark:text-white">{{ metric.value REDACTEDREDACTED</p>
            </div>
          </div>
...
const props = defineProps<{ runtime: PromptAuditRuntime | null; loading: boolean; error: string REDACTED>()
...
const statusItems = computed(() => {
  ...
  if (!runtime) return []
  return [
    { label: t('admin.promptAudit.runtime.process'), value: t(`admin.promptAudit.status.${runtime.process_statusREDACTED`), dot: statusDot(runtime.process_status) REDACTED,
    { label: t('admin.promptAudit.runtime.mode'), value: t(`admin.promptAudit.mode.${runtime.effective_modeREDACTED`) REDACTED,
    { label: t('admin.promptAudit.runtime.version'), value: `${runtime.active_config_versionREDACTED / ${runtime.expected_config_versionREDACTED` REDACTED,
    { label: t('admin.promptAudit.runtime.workers'), value: `${runtime.worker_activeREDACTED / ${runtime.worker_totalREDACTED` REDACTED,
    { label: t('admin.promptAudit.runtime.queue'), value: `${runtime.queue.activeREDACTED / ${runtime.queue_capacityREDACTED` REDACTED,
    { label: t('admin.promptAudit.runtime.dependencies'), value: `DB ${runtime.database_statusREDACTED · Redis ${runtime.redis_statusREDACTED` REDACTED,
  ]
REDACTED)
...
const guardMetricItems = computed(() => {
  ...
  if (!metrics) return []
  return [
    { label: t('admin.promptAudit.metrics.total'), value: metrics.total REDACTED,
    { label: t('admin.promptAudit.metrics.allowed'), value: metrics.allowed REDACTED,
    { label: t('admin.promptAudit.metrics.flagged'), value: metrics.flagged REDACTED,
    { label: t('admin.promptAudit.metrics.blocked'), value: metrics.blocked REDACTED,
    { label: t('admin.promptAudit.metrics.unavailable'), value: metrics.unavailable REDACTED,
    { label: t('admin.promptAudit.metrics.timeouts'), value: metrics.timeouts REDACTED,
    { label: t('admin.promptAudit.metrics.failovers'), value: metrics.failovers REDACTED,
    { label: 'P95', value: metrics.latency_p95_ms != null ? `${metrics.latency_p95_msREDACTED ms` : '—' REDACTED,
  ]
REDACTED)
