...
export const SCANNER_CATALOG = [
  { id: 'violent', label: 'Violent' REDACTED,
  { id: 'non_violent_illegal_acts', label: 'Non-violent Illegal Acts' REDACTED,
  { id: 'sexual_content_or_sexual_acts', label: 'Sexual Content or Sexual Acts' REDACTED,
  { id: 'pii', label: 'PII' REDACTED,
  { id: 'suicide_and_self_harm', label: 'Suicide & Self-Harm' REDACTED,
  { id: 'unethical_acts', label: 'Unethical Acts' REDACTED,
  { id: 'politically_sensitive_topics', label: 'Politically Sensitive Topics' REDACTED,
  { id: 'copyright_violation', label: 'Copyright Violation' REDACTED,
  { id: 'jailbreak', label: 'Jailbreak' REDACTED,
] as const
...
export function createDefaultEndpoint(index = 1): PromptAuditEndpointDraft {
  return {
    id: `guard-${Date.now()REDACTED-${indexREDACTED`,
    name: `Guard ${indexREDACTED`,
...
export function eventQueryParams(filters: PromptEventFilters): Record<string, string | number> {
  const result: Record<string, string | number> = {REDACTED
...
export const DELETE_RANGE_PRESETS: ReadonlyArray<{ id: DeleteRangePreset; days: number | null REDACTED> = [
  { id: '1d', days: 1 REDACTED,
  { id: '7d', days: 7 REDACTED,
  { id: '30d', days: 30 REDACTED,
  { id: '90d', days: 90 REDACTED,
  { id: 'all', days: null REDACTED,
  { id: 'custom', days: null REDACTED,
]
