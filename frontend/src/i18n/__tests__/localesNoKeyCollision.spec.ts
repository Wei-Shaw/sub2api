...
// locales/{zh,enREDACTED/index.ts 与 admin/index.ts 使用对象展开聚合各域模块，
...
function collisions(modules: Modules): string[] {
  ...
  for (const [name, mod] of Object.entries(modules)) {
    for (const key of Object.keys(mod)) {
      ...
      if (prev) {
        out.push(`"${keyREDACTED" in both ${prevREDACTED and ${nameREDACTED`)
      REDACTED else {
        seen.set(key, name)
      REDACTED
    REDACTED
...
const roots: Record<string, Modules> = {
  zh: { landing: zhLanding, common: zhCommon, dashboard: zhDashboard, misc: zhMisc REDACTED,
  en: { landing: enLanding, common: enCommon, dashboard: enDashboard, misc: enMisc REDACTED
REDACTED
...
describe.each(Object.keys(roots))('locale %s spread assembly', (locale) => {
  ...
  it('root modules do not shadow the explicit "admin" namespace', () => {
    for (const [name, mod] of Object.entries(roots[locale])) {
      expect(Object.keys(mod), `module ${nameREDACTED must not define "admin"`).not.toContain('admin')
    REDACTED
  REDACTED)
