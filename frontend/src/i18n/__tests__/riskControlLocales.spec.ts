...
describe('risk control locale copy', () => {
  ...
  it('keeps pre-block audit key summary aware of async worker load', () => {
    expect(zh.admin.riskControl.preBlockAPIKeyLoadSummary).toContain('worker：{workerActiveREDACTED / {workerTotalREDACTED')
    expect(en.admin.riskControl.preBlockAPIKeyLoadSummary).toContain('worker: {workerActiveREDACTED / {workerTotalREDACTED')
  REDACTED)
