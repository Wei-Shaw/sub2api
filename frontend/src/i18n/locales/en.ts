  ...
  admin: {
    ...
    channels: {
      ...
      form: {
        ...
        imageTiers: 'Image Tiers (Pixel Area)',
        imageTierHint: 'Area = width x height (pixels). Defaults: 1K<2,359,296; 2K<6,553,600; 4K>=6,553,600',
        addTier: 'Add Tier',
    ...
    accounts: {
      ...
      dataImportWarning: 'Import will create new accounts/proxies. To reduce accidental upstream use, defaults are conservative: low concurrency and no scheduling until you review groups, proxies, batches, and quota settings.',
      ...
      dataImportModelSyncResult: 'Model detection succeeded {model_sync_succeeded}, failed {model_sync_failed}',
      dataImportErrors: 'Error Details',
      dataImportSuccess: 'Import completed: accounts {account_created}, failed {account_failed}',
      dataImportCompletedWithErrors: 'Import completed with errors: account failed {account_failed}, proxy failed {proxy_failed}, model detection failed {model_sync_failed}',
      dataImportBatchRequired: 'Please select a batch',
      dataImportConfirm: 'Confirm Import',
      noBatches: 'Please create a batch first in "More Tools → Batch Manager"',
      dataImportSharedSettingsTitle: 'Shared Import Settings',
      dataImportSharedSettingsHint: 'These settings apply to every account in this import. Defaults are conservative so you can import first, review proxies and batches, then enable scheduling gradually.',
      dataImportConcurrencyHint: 'Keep new imports at concurrency 1 until proxy, quota, and request type behavior look stable.',
      dataImportSchedulableOnHint: 'Imported accounts will join scheduling immediately and receive real traffic. Confirm groups, proxy, quota, and model settings first.',
      dataImportSchedulableOffHint: 'Recommended for new or bulk official accounts: accounts are saved but will not enter the pool until you review the batch and enable them.',
      dataImportModelRestrictionHint: 'Apply one model whitelist to all imported accounts. When auto detection is enabled, imported accounts will request upstream model lists after creation and overwrite this whitelist with live supported models.',
      dataImportAutoDetectModelsOnHint: 'After import, each account will request its upstream model list and save it as the whitelist. Detection failures do not delete accounts; details appear in the import result.',
      dataImportAutoDetectModelsOffHint: 'By default, the upstream model list will not be requested. Selected models below are saved as the whitelist; leave empty to avoid model restrictions.',
      dataImportBulkEditHint: 'Hint: After import, you can filter by batch in the account list and use "Bulk Edit" to modify account properties.',
      ...
      tokenPresence: {
        accessTokenPresent: 'AT acquired: access_token exists',
        accessTokenMissing: 'AT missing: access_token is absent',
        refreshTokenPresent: 'RT acquired: refresh_token exists',
        refreshTokenMissing: 'RT missing: refresh_token is absent'
      },
