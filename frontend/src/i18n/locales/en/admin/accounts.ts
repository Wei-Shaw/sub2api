export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: 'This rate is either declared for the current API key by a Sub2API upstream or read from an explicitly selected New API catalogue group. Sub2API cannot verify that it matches actual charges. The upstream site or an intermediary may return forged, stale, or modified data. Verify it against bills, balance changes, and actual usage.',
        ...
        rechargeMultiplierHint: 'Final upstream declared rate = upstream rate × recharge multiplier. Default is 1; must be a finite number greater than 0.',
        newAPIGroup: 'New API group (required for fallback)',
        newAPIGroupHint: 'Required for the New API compatibility fallback. Enter the group actually used by this relay API key; /api/pricing and /api/user/groups are only catalogue sources, and the API key group is never guessed. Enabling automatic sync confirms this group.',
        ...
        failed: 'Failed',
        newAPIPricingAuthRequired: 'New API pricing requires dashboard authentication, and /api/user/groups did not provide a usable anonymous fallback.',
        newAPIGroupRequired: 'Set the New API group before using the compatibility fallback.',
        newAPIGroupNotFound: 'The configured New API group was not returned by the catalogue.',
        notProbed: 'Not probed',
        provider: 'Source: {value}',
        providerSub2API: 'Sub2API',
        providerNewAPI: 'New API',
        upstreamGroup: 'Upstream group: {value}',
        rateFormula: 'Raw base rate {raw} × recharge multiplier {recharge} = current base rate {resolved}',
