    ...
    accounts: {
      ...
      openai: {
        ...
        firstServeSettings: {
          ...
          modeHint: "Reuses the routing ID and rotates the proxy at a fixed interval. With fingerprint convergence enabled, the device fingerprint changes with the IP and remains stable for the same interval.",
          reuseScope: "Routing and proxy reuse scope",
          scopeSession: "One combination per conversation",
          scopeAccount: "Share a combination across this account (default)",
          scopeHint: "HTTP and WebSocket share the account routing ID and proxy. Full convergence also unifies outbound session metadata. Conversation history and previous_response_id remain independent, as do physical WebSocket connections.",
          ...
          rotate_seconds: "IP rotation interval (seconds)",
          ...
          groupPending: "Account {name}: First serve is enabled but needs a proxy group. Select a group with at least two distinct exits before sending requests.",
        ...
        firstServeInactive: "Combination no longer reused",
        ...
        firstServeMetrics: "First token: {latency}s · Next IP rotation: {expires} · Rotations: {rotations}",
        ...
        firstServeReasons: {
          non_stream: "Non-streaming request completed.",
          compact: "Context compaction completed.",
          ...
          observing: "Using the current proxy until the next fixed rotation time.",
          ready: "Current proxy is active until the next fixed rotation time.",
          slow: "First-token timing is recorded for diagnostics only; it does not trigger rotation.",
          context_incomplete: "Context is incomplete; rotation keeps its fixed schedule and preserves previous_response_id for upstream continuation.",
          proxy_unavailable: "No alternative proxy is available. Keeping the current proxy and device fingerprint, and retrying on a later request.",
          cooldown: "Legacy cooldown state; fixed-interval rotation is active.",
        ...
        wsModeFirstServeHint: 'Supports HTTP/SSE streaming and WebSocket. Rotate the proxy every configured interval while reusing the routing ID. Bind at least two distinct proxy exits; response IDs still advance each turn.',
