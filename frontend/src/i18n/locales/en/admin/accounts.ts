export default {
    accounts: {
      ...
      dataImportCodexPolicyOverride: 'Apply one identity policy to imported OpenAI OAuth accounts',
      dataImportCodexPolicyPreserve: 'Off by default. Existing backup policies and stable proxy references are preserved and validated by the server.',
      ...
      status: {
        ...
        provisioningPending: 'Configuring',
        provisioningPendingHint: 'Configuration is not complete. This account cannot be scheduled yet.',
      ...
      codexIdentity: {
        title: 'Codex OS profile device pool',
        description: 'Keep a bounded, stable set of device identities per OAuth account. Connection and WebSocket state remain isolated by API key.',
        offState: 'Disabled. Existing Codex identity behavior is unchanged.',
        profiles: 'Operating-system profiles',
        profilesDesc: 'Enable only the environments this account should accept. Desktop and CLI can be enabled and configured independently within the same OS.',
        fixErrors: 'Complete the identity policy before saving.',
        valid: 'Identity policy is ready to validate on the server.',
        windows: 'Windows',
        windowsDesc: 'Configure Windows Desktop and CLI identities independently.',
        macos: 'macOS',
        macosDesc: 'Configure macOS Desktop and CLI identities independently.',
        linux: 'Linux',
        linuxDesc: 'Configure Linux Desktop and CLI identities independently.',
        generic: 'Generic SDK / third-party',
        genericOS: 'Generic',
        genericDesc: 'Configure SDK and supported third-party clients independently.',
        surface: 'Client surface',
        desktop: 'Desktop',
        desktopDesc: 'Use a dedicated Desktop identity and device pool.',
        cli: 'CLI',
        cliDesc: 'Use a dedicated CLI identity and device pool.',
        sdk: 'SDK',
        sdkDesc: 'Use a dedicated SDK identity and device pool.',
        thirdParty: 'Third-party',
        thirdPartyDesc: 'Use a dedicated third-party identity and device pool.',
        architecture: 'Architecture',
        notApplicable: 'Not applicable',
        deviceSlots: 'Device slots',
        increaseSlots: 'Increase device slots',
        decreaseSlots: 'Decrease device slots',
        profileProxy: 'Profile proxy',
        accountProxyDirect: 'Inherit the account connection (currently direct).',
        accountProxyInherit: 'Inherit account proxy',
        templateProxyInherit: 'Inherits each assigned account connection at runtime; accounts without a proxy use direct routing.',
        inheritAccountProxy: 'Inherit account connection',
        directConnection: 'Direct connection (bypass inherited proxy)',
        slotOverrides: 'Per-slot proxy and version overrides',
        deviceSlot: 'Device slot',
        slotProxyHint: 'Inherits the profile proxy unless overridden.',
        inheritProfileProxy: 'Inherit profile connection',
        clientVersion: 'Codex client version',
        fixedClientVersion: 'Fixed Codex client version',
        clientVersionInherit: 'Automatic (recommended)',
        clientVersionPinned: 'Fixed version',
        effectiveClientVersion: 'Effective version',
        clientProfileVerification: 'Client metadata',
        clientProfileVerified: 'Verified metadata',
        clientProfileUnverified: 'Unverified built-in metadata',
        ...
        conversationIsolatedDesc: 'Recommended. Every API key and conversation keeps its own session identity.',
        apiKeyShared: 'Share within an API key',
        apiKeySharedDesc: 'Conversations using the same downstream API key share one session.',
        sessionPool: 'Fixed session pool',
        sessionPoolDesc: 'Map API key and conversation pairs into a bounded shared session pool per device.',
        sessionPoolRisk: 'Different API keys and conversations can share an upstream session slot. HTTP and WebSocket runtime state remains isolated.',
        deviceShared: 'Share by device',
        deviceSharedDesc: 'All API keys on a device slot use one upstream session.',
        sessionsPerDevice: 'Sessions per device',
        sessionsPerDeviceDesc: 'Each API key and conversation is assigned to one stable slot in the shared pool.',
        increaseSessions: 'Increase session slots',
        decreaseSessions: 'Decrease session slots',
        affinityTtl: 'Conversation affinity',
        affinityTtlDesc: 'Keep a conversation on the same OAuth account and device slot.',
        minutes: 'min',
        deviceSharedRisk: 'High risk: unrelated API keys will present the same upstream session identity.',
        deviceSharedRestriction: 'Concurrent requests can share a stable device slot. Existing account limits still apply; cross-key continuation is disabled.',
        slotConcurrency: 'Local concurrency limit per slot',
        slotConcurrencyHint: '0 adds no slot-specific limit. New conversations try idle slots of the same profile when full; existing conversations stay pinned. Account, user and API key limits still apply.',
        apiKeySharedRisk: 'Conversations within one API key can affect the same upstream session identity.',
        assignmentTitle: 'Codex OS egress template',
        assignmentDescription: 'Enable an egress template managed centrally in System Settings for this OpenAI OAuth account.',
        manageTemplates: 'Manage Codex egress templates',
        assignmentOff: 'Disabled. This account continues using the default Codex identity behavior.',
        loadingTemplates: 'Loading egress templates...',
        loadTemplatesFailed: 'Failed to load Codex egress templates.',
        retryTemplates: 'Retry',
        noTemplates: 'No templates are available. Create a Codex egress template in System Settings first.',
        templateLabel: 'Egress template',
        templatePlaceholder: 'Select a template',
        templateRequired: 'Select a template when Codex egress templates are enabled.',
        templateRevision: 'Revision {revision}',
        templateErrors: {
          CODEX_IDENTITY_TEMPLATE_NOT_FOUND: 'The selected template no longer exists. Refresh and select another template.',
          CODEX_IDENTITY_TEMPLATE_REVISION_CONFLICT: 'The template was updated. Refresh and submit again.',
          INVALID_CODEX_IDENTITY_ASSIGNMENT: 'The template assignment is invalid. Select the template again.',
          CODEX_IDENTITY_TEMPLATE_MANAGED: 'This account is template-managed. Update the template in System Settings or change the account assignment.',
          CODEX_IDENTITY_TEMPLATE_RUNTIME_MISMATCH: 'The same-named template has different runtime settings. Restore was rejected to prevent a wrong egress route.',
          CONFLICTING_CODEX_IDENTITY_CONFIGURATION: 'A template assignment and an account-level identity policy cannot be submitted together.',
        },
        bulkModify: 'Bulk edit Codex egress template',
        bulkModifyDesc: 'Select this field to disable templates for all targets, or enable and assign one template. Leave it unselected to preserve each account.',
        bulkApply: 'Apply Codex OS profile device pool',
        bulkApplyDesc: 'Apply one complete identity-policy snapshot to every selected OpenAI OAuth account.',
        importOverride: 'For safety, this policy replaces identity policies from backup files before import.',
        crsOverrideExisting: 'Also replace existing OpenAI OAuth identity policies',
        crsOverrideExistingDesc: 'Off by default. Credential sync preserves each existing account policy unless you explicitly enable this replacement.',
        lifecycleTitle: 'Device-slot lifecycle',
        lifecycleDesc: 'Saving identity changes rotates affected slots. Previous slots drain until their bindings become idle.',
        refreshSlots: 'Refresh device slots',
        loadingSlots: 'Loading device slots...',
        loadSlotsFailed: 'Failed to load device slots.',
        retry: 'Retry',
        noSlots: 'No device slots have been materialized yet.',
        activeSlot: 'Active',
        drainingSlot: 'Draining',
        slotNumber: 'Slot',
        epoch: 'Epoch',
        bindings: 'Bindings',
        proxy: 'Proxy',
        inheritedProxy: 'Inherited account proxy',
        catalogVersion: 'Identity catalog',
        finalizeDraining: 'Finalize drained slots',
        finalizeConfirm: 'Finalize only slots the server confirms are fully drained?',
        cancel: 'Cancel',
        confirmFinalize: 'Confirm finalize',
        finalizing: 'Finalizing...',
        finalizeSuccess: 'Finalized {count} drained device slot(s).',
        finalizeNone: 'No device slots are ready to finalize yet.',
        finalizeFailed: 'Failed to finalize drained slots.',
        validation: {
          proxyInvalid: 'The selected proxy identifier is invalid.',
          proxyNotFound: 'The selected proxy is unavailable in this deployment.',
          proxyModeInvalid: 'The selected proxy mode is invalid.',
          proxyRequired: 'Select a proxy when using explicit proxy mode.',
          proxyNotAllowed: 'A proxy ID is only valid when explicit proxy mode is selected.',
          surfaceNotAllowed: 'This client surface is not supported by the selected operating system.',
          architectureNotAllowed: 'This architecture is not supported by the selected Profile.',
          slotCountOutOfRange: 'Device slots must be between 1 and 3.',
          slotOverrideOutOfRange: 'A proxy override points to a device slot outside the configured range.',
          duplicateSlotOverride: 'A device slot can only have one proxy override.',
          bindingScopeInvalid: 'Binding scope must include the API key, OS, and client surface.',
          unsupportedPolicyInvalid: 'Unsupported client Profiles must be rejected.',
          offModeHasProfiles: 'A disabled identity policy cannot define OS Profiles.',
          profileRequired: 'Enable at least one operating-system Profile.',
          duplicateProfile: 'Each operating-system and client-surface combination can appear only once.',
          affinityTtlOutOfRange: 'Conversation affinity must be between 1 minute and 24 hours.',
          sessionSlotCountOutOfRange: 'Session slots must be between 1 and 3.',
          sessionSlotCountNotApplicable: 'Session slots are only valid in fixed session-pool mode.',
          deviceSharedRestrictionsInvalid: 'The slot concurrency limit must be an integer from 0 to 1000, with cross-key continuation disabled.',
          clientVersionModeInvalid: 'The Codex client version mode is invalid.',
          clientVersionRequired: 'Enter a Codex client version when fixed version mode is selected.',
          clientVersionInvalid: 'Enter a valid Codex client version such as 0.146.0.',
          clientVersionTooOld: 'Codex client version must be at least 0.144.0.'
        }
      },
      ...
      oauth: {
        ...
        openai: {
          ...
          codexSessionOverrideExisting: 'Also replace identity policies on matched existing accounts',
          codexSessionOverrideExistingDesc: 'Off by default. Existing accounts keep their current device-pool policy while credentials and other selected settings are updated.',
