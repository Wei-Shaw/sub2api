export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: 'This rate is either declared for the current API key by a Sub2API upstream or read from an explicitly selected New API pricing group. Sub2API cannot verify that it matches actual charges. The upstream site or an intermediary may return forged, stale, or modified data. Verify it against bills, balance changes, and actual usage.',
        ...
        newAPIGroupHint: 'Required for the New API compatibility fallback. Enter the group actually used by this relay API key; /api/pricing is only a catalogue and the API key group is never guessed. Enabling automatic sync confirms this group.',
        ...
        failed: 'Failed',
        newAPIPricingAuthRequired: 'New API pricing requires dashboard authentication; this compatibility fallback is not supported for the instance.',
        newAPIGroupRequired: 'Set the New API group before using the compatibility fallback.',
        newAPIGroupNotFound: 'The configured New API group was not returned by the pricing catalogue.',
