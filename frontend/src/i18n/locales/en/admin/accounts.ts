    ...
    accounts: {
      ...
      oauth: {
        ...
        cursor: {
          ...
          step1GenerateUrl: 'Generate the Cursor Deep Control login URL',
          generateAuthUrl: 'Generate Auth URL',
          step2OpenUrl: 'Open the URL in your browser and complete login',
          openUrlDesc: 'Open the login URL, sign in to Cursor, then return here. The admin UI polls once per request until login finishes.',
          importantNotice: 'Closing this dialog cancels authorization. Polling also stops after five minutes.',
          step3EnterCode: 'Waiting for Cursor login',
          authCodeDesc: 'After you generate the URL, click Complete Authorization to start polling. You can also paste a refresh token instead.',
          authCode: 'Optional note',
          authCodePlaceholder: 'Leave empty and complete authorization, or switch to refresh token input',
          authCodeHint: 'Cursor login uses PKCE polling. No callback URL is required.',
          refreshTokenAuth: 'Manual RT Input',
          refreshTokenDesc: 'Paste an existing Cursor refresh token to exchange it for a new access token.',
          refreshTokenPlaceholder: 'Paste your Cursor refresh token',
          validating: 'Validating...',
          validateAndCreate: 'Validate & Save',
          pleaseEnterRefreshToken: 'Please enter Refresh Token',
          failedToGenerateUrl: 'Failed to generate Cursor auth URL',
          missingExchangeParams: 'Missing Cursor OAuth session',
          failedToPoll: 'Failed to poll Cursor login',
          pollTimeout: 'Cursor login timed out. Generate a new auth URL and try again.',
          failedToValidateRT: 'Failed to validate Cursor refresh token',
          errors: {
            CURSOR_OAUTH_SESSION_NOT_FOUND: 'Cursor OAuth session was not found or has expired. Generate a new auth URL.',
            CURSOR_OAUTH_SESSION_REQUIRED: 'Cursor session_id is required.',
            CURSOR_OAUTH_POLL_FAILED: 'Cursor poll failed. Generate a new auth URL and try again.'
          }
        },
        devin: {
          title: 'Devin Account Authorization',
          followSteps: 'Follow these steps to authorize your Devin account:',
          step1GenerateUrl: 'Generate the Devin CLI authorization URL',
          generateAuthUrl: 'Generate Auth URL',
          step2OpenUrl: 'Open the URL in your browser and complete authorization',
          openUrlDesc: 'Open the authorization URL, sign in to Devin, then copy the failed localhost callback URL.',
          importantNotice: 'The official callback is http://127.0.0.1:59653/callback. Paste the full address-bar URL after the browser fails to connect.',
          step3EnterCode: 'Paste callback URL or code',
          authCodeDesc: 'Paste the full callback URL, query string, or authorization code:',
          authCode: 'Callback URL or Code',
          authCodePlaceholder: 'http://127.0.0.1:59653/callback?code=...&state=...',
          authCodeHint: 'Full callback URLs, query strings, and bare codes are accepted.',
          refreshTokenAuth: 'Session token',
          refreshTokenDesc: 'Paste an existing Devin session JWT. The gateway stores it as both access and refresh.',
          refreshTokenPlaceholder: 'Paste your Devin session token',
          validating: 'Importing...',
          validateAndCreate: 'Import & Save',
          pleaseEnterSessionToken: 'Please enter a Devin session token',
          pleaseEnterRefreshToken: 'Please enter a Devin session token',
          failedToGenerateUrl: 'Failed to generate Devin auth URL',
          missingExchangeParams: 'Missing authorization code or OAuth session',
          failedToExchangeCode: 'Failed to exchange Devin authorization code',
          failedToImportToken: 'Failed to import Devin session token',
          errors: {
            DEVIN_OAUTH_SESSION_NOT_FOUND: 'Devin OAuth session was not found or has expired. Generate a new auth URL.',
            DEVIN_OAUTH_INVALID_STATE: 'Devin OAuth state does not match this session. Paste the callback URL from the same generated auth link.',
            DEVIN_OAUTH_CODE_REQUIRED: 'The Devin authorization code is missing. Paste the full callback URL.',
            DEVIN_OAUTH_SESSION_INVALID: 'Devin rejected the session token. Sign in again.'
          }
        },
        // Gemini specific
	        gemini: {
