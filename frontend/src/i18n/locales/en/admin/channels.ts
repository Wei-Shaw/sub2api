export default {
    ...
    riskControl: {
      ...
      cyberPolicyCaptureRequest: 'Store full cyber_policy request',
      cyberPolicyCaptureRequestHint: 'When enabled, cyber_policy blocks persist the full raw request body (unredacted) in a separate table, deleted together with the risk log.',
      requestBodyTitle: 'Full request',
      requestBodyLoad: 'View full request',
      requestBodyLoading: 'Loading',
      requestBodyMissing: 'No request body stored for this record',
      requestBodyFailed: 'Failed to load request body',
      replayToModeration: 'Replay to moderation',
      replaying: 'Replaying',
      replayFailed: 'Replay failed',
      replayResultTitle: 'Replay result',
      replayScannedText: 'Scanned text',
      replayNote: 'The moderation API only returns content category scores and never returns cyber_policy. A "not flagged" result is normal and does not mean upstream would allow it.',
      chunkModerationEnabled: 'Chunked moderation',
      chunkModerationEnabledHint: 'When enabled, long content is split into chunks and each one is moderated, covering the part that was previously truncated and never scored. Off by default: enabling it multiplies moderation requests and latency by the number of chunks, and in pre-block mode that latency is added directly to the user request.',
      chunkTokens: 'Moderation chunk size (tokens)',
      chunkTokensHint: 'The upstream moderation API silently fails for inputs above roughly 4000 tokens (returns not-flagged with no error), so long content is split into chunks of this size and scored separately, taking the highest score per category. Max 3500.',
      chunkConcurrency: 'Chunk concurrency',
      chunkConcurrencyHint: 'How many chunks are moderated in parallel. Higher is faster but issues more requests. Max 8.',
      chunkMaxChunks: 'Max chunks',
      chunkMaxChunksHint: 'Caps how many chunks a single moderation may send, bounding cost and latency for very long sessions. Chunks beyond the cap are not moderated and are recorded in the logs. Max 64.',
      replayChunkInfo: '{tokens} tokens total, sent as {chunks} chunk(s) (limit {limit} tokens each)',
      replayCyberDimensionNote: 'The highest risk here is illicit and no category reached its threshold. Upstream cyber_policy targets cybersecurity risk (e.g. attack/exploit code), a different dimension from the moderation API content-toxicity scores, so a non-hit is expected and does not mean moderation is broken.',
