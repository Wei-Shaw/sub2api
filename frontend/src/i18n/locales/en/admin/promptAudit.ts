export default {
  promptAudit: {
    ...
    actions: { refresh: 'Refresh runtime', retry: 'Retry', Allow: 'Allow', Warn: 'Warn', Block: 'Block', Error: 'Failed' },
    ...
    decisions: { pass: 'Pass', flag: 'Flag', critical: 'Critical', failed: 'Audit failed' },
	...
	metrics: { total: 'Total', allowed: 'Allowed', flagged: 'Flagged', blocked: 'Blocked', unavailable: 'Unavailable', timeouts: 'Timeouts', failovers: 'Failovers', genericRequests: 'Generic LLM requests', sampledOut: 'Sampled out', genericFailures: 'Schema / policy failures', genericTokens: 'Generic LLM tokens', genericLatency: 'Generic LLM average latency' },
    pool: {
      ...
      engineType: 'Audit engine', systemGuidance: 'Additional audit guidance', confidence: 'Confidence threshold', sampleRate: 'Sample rate', maxOutputTokens: 'Maximum output tokens', reasoningEffort: 'Reasoning effort', jsonMode: 'JSON output mode', stage: 'Enforcement stage', failurePolicy: 'Failure policy', compositionMode: 'Keyword composition mode',
		options: { plainJson: 'Compatible JSON (Plain JSON)', jsonSchema: 'Strict JSON Schema', shadow: 'Shadow (record only)', warn: 'Warn (record and allow)', block: 'Block (enforce)', failOpen: 'Allow on failure (fail_open)', failClosed: 'Block on failure (fail_closed)', keywordFirst: 'Keyword first', combined: 'Combined', llmOnly: 'LLM only' },
		help: {
			jsonMode: 'Controls the model response structure. Compatible JSON asks for JSON through the prompt and works with endpoints that lack Structured Outputs, but format failures are more likely. Strict JSON Schema sends response_format/json_schema so the upstream constrains fields and types; it is more reliable but requires provider support.',
			stage: 'Controls whether audit results affect the request. Shadow records and measures the LLM decision but always allows the request. Warn records risk as a warning and allows the request. Block rejects decisions above the policy threshold before account selection, billing, and upstream access; block requires 100% sampling.',
			failurePolicy: 'Controls timeouts, connection failures, and invalid model responses. Allow on failure continues the request for higher availability. Block on failure returns service unavailable for stricter security and can only be used with the block stage.',
			compositionMode: 'Controls how keyword and LLM audits compose. Keyword first stops on an explicit keyword block without disclosing the prompt externally; otherwise it runs the LLM. Combined runs both and uses the more severe result. LLM only skips the keyword result for this audit decision.',
		},
      genericWarning: 'Generic models disclose user prompts to an external service and incur provider cost. Model output may be prompt-injected, so JSON is strictly validated and enforcement remains server-owned. fail_open permits requests during model failures; fail_closed increases availability risk.',
      blockSamplingWarning: 'The block stage requires a 100% sample rate.', failClosedWarning: 'fail_closed is only available in the block stage.',
    ...
    events: {
      ...
      errorCode: 'Error code', errorMessage: 'Failure reason', failedChunk: 'Failed chunk',
      requestId: 'Request ID', promptHash: 'Prompt SHA-256',
      technical: {
        scanner: 'Scanner', policy: 'Policy', guardEndpoint: 'Guard endpoint', config: 'Config',
		chunks: 'Chunks', latency: 'Latency', protocol: 'Protocol', engine: 'Audit engine / model', schemaConfidence: 'Schema / confidence', enforcement: 'Stage / failure policy', usage: 'Input / output / total tokens', shadowComparison: 'Keyword / LLM comparison', agree: 'Agree', disagree: 'Disagree',
      },
