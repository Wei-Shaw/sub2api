export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentPlaceholder: 'Codex Desktop/0.153.4 (Mac OS 15.6.0; arm64) unknown (Codex Desktop; 26.901.41600)',
        openaiCodexUserAgentHint: 'This is the upstream identity generated centrally by the Sub2API server; it does not inherit a visitor browser or an administrator terminal. Leave empty to use the verified complete Codex Desktop artifact profile. A single-version CLI/TUI profile may follow the version setting below, while a Desktop profile with separate Core and frontend versions is preserved as one atomic tuple.',
        openaiCodexClientVersion: 'Codex client version',
        openaiCodexClientVersionPlaceholder: 'Leave empty to follow auto-sync',
        openaiCodexClientVersionHint: 'Core version for single-version CLI/TUI profiles, shared by the User-Agent and version header. Leave empty to follow auto-sync. Complete Desktop tuples are not modified by standalone CLI version sync.',
        openaiCodexVersionAutoSync: 'Auto-sync Codex version',
        openaiCodexVersionAutoSyncHint: 'Fetches the latest stable CLI Core version every 6 hours and updates only single-version CLI/TUI profiles. It never splices a standalone CLI version into a complete Desktop Core/frontend tuple.',
