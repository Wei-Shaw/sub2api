export default {
    settings: {
      ...
      imageInputFallback: {
        title: 'Image Input Fallback',
        description: 'Automatically degrade and retry when the upstream does not support image input (e.g. unknown variant `image_url`)',
        mode: 'Fallback Mode',
        modeOff: 'Off',
        modeStrip: 'Strip Images and Retry',
        modeDescribe: 'Replace with Vision Model Description',
        modeHint: 'Strip: remove image content from the request; Describe: call a vision model to generate text descriptions to replace images',
        models: 'Proactive Models (Multimodal Emulation)',
        modelsHint: 'Comma-separated; supports * wildcards (e.g. deepseek-*) and substring matching. Requests hitting these models are processed before being sent upstream (multimodal emulation); leave empty to only retry after an upstream error',
        modelsPlaceholder: 'e.g. gpt-4, deepseek-* (empty = passive retry only)',
        visionBaseUrl: 'Vision API Base URL',
        visionBaseUrlHint: 'OpenAI-compatible endpoint, e.g. https://api.openai.com/v1',
        visionApiKey: 'Vision API Key',
        visionApiKeyPlaceholderConfigured: 'Configured (leave empty to keep)',
        visionApiKeyPlaceholder: 'Enter vision API key',
        visionApiKeyHint: 'Only updates when a new key is entered; leave empty to keep the current one',
        visionModel: 'Vision Model',
        visionModelHint: 'Model with image understanding, e.g. gpt-4o-mini',
        visionTimeout: 'Describe Timeout (seconds)',
        visionTimeoutHint: 'Timeout for a single image description request; 0 uses the default 60s',
        saved: 'Image input fallback settings saved',
        saveFailed: 'Failed to save image input fallback settings'
      },
      rectifier: {
