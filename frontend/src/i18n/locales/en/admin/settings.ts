export default {
    settings: {
      keyProtection: {
        ...
        description: 'Replace supported credentials in conversation text before moderation and model forwarding. Users keep pasting keys as usual; no per-service credential setup is needed. Disabled by default.',
        enabled: 'Enable automatic key protection',
        mode: 'Mode',
        reversible: 'Reversible replacement (recommended)',
        redact: 'Redact only (cannot restore)',
        scope: 'Restore credentials in',
        textAndTools: 'Reply text and tool arguments',
        toolsOnly: 'Tool arguments only',
        modeHint: 'Reversible mode restores known placeholders when returning the response. Redact-only mode replaces matches with [REDACTED]; generated code will not receive the original key.',
        ...
        boundary: 'Protects recognizable keys in text, not images, audio or arbitrary passwords. Unsupported protected endpoints are rejected. Restored plaintext remains accessible to clients and tools; this does not remove credential permissions or prevent a tool from sending it to the wrong destination.',
        advanced: 'Advanced settings',
        ttl: 'Mapping lifetime (seconds)',
        maxMappings: 'Keys per session',
        maxSessions: 'Saved records per security context',
        ...
        storageHint: 'Mappings are sensitive state with a limited lifetime. Session and response snapshots count toward the record limit for each user / API key / group. Cross-request storage uses platform-managed encryption. Built-in IDs: openai, anthropic, github, gitlab, google, stripe, slack, huggingface, groq, npm.',
        clearMappings: 'Clear all saved mappings',
        clearHint: 'Clearing invalidates saved placeholders and response continuations for every user. Clients must resend full history containing their original credentials to establish a new mapping.',
        confirmClear: 'Confirm clearing all mappings',
        cleared: 'Saved key protection mappings cleared',
        clearFailed: 'Could not clear mappings. Retry after checking the mapping store.',
        ...
        saveFailed: 'Could not save key protection settings. Check rule IDs, expressions, IDs and capacity limits, then retry.',
        platformKeyRequired: 'A deployment administrator must configure a fixed platform TOTP_ENCRYPTION_KEY before enabling reversible protection. Reuse the existing key on every instance; users do not need to configure service credentials.',
