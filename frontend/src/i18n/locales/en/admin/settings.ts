export default {
    settings: {
      ...
      payment: {
        ...
        providerInfini: 'Infini (Crypto)',
        refundUnsupported: 'This provider does not support refunds',
        ...
        field_keyId: 'Key ID (merchant public key)',
        field_forwardPayerEmail: 'Forward payer email to the provider',
        ...
        field_paymentCurrencyHint: 'Default is CNY. Stripe and Airwallex can choose HKD, USD, or another listed currency supported by the account; Infini supports foreign currencies only and defaults to USD; WeChat Pay, Alipay, and EasyPay remain CNY.',
        field_infiniApiBaseHint: 'Use https://openapi.infini.money for production and https://openapi-sandbox.infini.money for the sandbox.',
        field_infiniWebhookSecretHint: 'The secret Infini signs webhooks with. If the merchant console does not issue a separate one, use the same value as the Secret Key.',
        field_forwardPayerEmailHint: 'When on, the payer email is sent to Infini at checkout so it can email a refund claim link for under- or overpayments. When off, no email leaves the system and anomalies need manual handling.',
        toggleEnabled: 'On',
        toggleDisabled: 'Off',
        ...
        infiniWebhookHint: 'Configure the following URL as the webhook endpoint in the Infini merchant console and subscribe to order.completed, order.expired and order.late_payment. Infini has no webhook management API, so this URL cannot be provisioned automatically; a merchant account holds a single webhook URL, so one account can serve only one deployment.',
