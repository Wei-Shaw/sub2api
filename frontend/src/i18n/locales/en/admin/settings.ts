export default {
    settings: {
      ...
      openaiFastPolicy: {
        ...
        description: 'Inject or rewrite service_tier on OpenAI requests, then pass, filter, request priority, or block by account, model, and tier. Applies to the OpenAI gateway only.',
        capabilityNotice: 'Sub2API only processes the request field and creates local billing records from the rewritten request tier; it cannot guarantee that the upstream uses Fast/Flex. Treat the service_tier returned by the upstream and the upstream bill as authoritative.',
        injectDefaultTier: 'Inject a default service_tier when missing',
        injectDefaultTierHint: 'Adds service_tier=default before evaluating the rules below. Combining “All tiers” with “Request priority” sends service_tier=priority upstream and uses that tier for local billing. Explicit client tiers are not overwritten by this step.',
        empty: 'No processing rules configured. Default-tier injection can still work on its own; click below to add a rule.',
        ...
        serviceTier: 'service_tier match condition',
        tierAll: 'All tiers (including injected default)',
        tierPriority: 'priority (request Fast)',
        tierFlex: 'flex',
        action: 'Action',
        actionPass: 'Pass (keep service_tier)',
        actionFilter: 'Filter (remove service_tier)',
        actionForcePriority: 'Rewrite as priority (request Fast)',
