...
decodeSizeOnRspHint: 'When the upstream response omits the size field or returns size=auto, decode the b64_json content to recover the real width and height for billing. URL-only responses are not affected.',
    ...
    modelIntros: {
      ...
      fields: {
        ...
        imageUrlsItemsFixed:
          'The imageUrls widget is selected: each element is a full image URL string, so there is no element schema to configure. The playground renders a gallery-style multi-image input (local upload / pick from library / paste URLs, drag to reorder) and submits an array of image URLs.',
