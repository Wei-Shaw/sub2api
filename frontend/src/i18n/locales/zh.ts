  ...
  admin: {
    ...
    channels: {
      ...
      form: {
        ...
        addInterval: '添加区间',
        requestTiers: '按次计费层级',
        imageTiers: '图片计费层级（按像素面积）',
        imageTierHint: '面积 = 宽 × 高（像素）。默认：1K<2,359,296；2K<6,553,600；4K≥6,553,600',
        addTier: '添加层级',
    ...
    accounts: {
      ...
      dataImportWarning: '导入将创建新账号与代理。为降低误触发上游风险，默认使用低并发、默认不加入调度池，检查分组、代理、批次和额度后再手动开启。',
      dataImportFile: '数据文件',
      ...
      dataImportModelSyncResult: '模型检测成功 {model_sync_succeeded}，失败 {model_sync_failed}',
      dataImportErrors: '失败详情',
      dataImportSuccess: '导入完成：账号 {account_created}，失败 {account_failed}',
      dataImportCompletedWithErrors: '导入完成但有错误：账号失败 {account_failed}，代理失败 {proxy_failed}，模型检测失败 {model_sync_failed}',
      dataImportBatchRequired: '请选择批次标签',
      dataImportConfirm: '确认导入',
      noBatches: '请先在"更多工具 → 批次管理"中创建批次',
      dataImportSharedSettingsTitle: '导入前统一设置',
      dataImportSharedSettingsHint: '以下设置会批量应用到本次导入的所有账号；默认值偏保守，适合先导入、检查代理和批次，再逐步开启调度。',
      dataImportConcurrencyHint: '新导入账号建议先保持 1 并发，确认代理、额度和调用类型稳定后再提高。',
      dataImportSchedulableOnHint: '本次导入的账号会立即加入调度池并承接真实请求，请确认分组、代理、额度和模型配置已准备好。',
      dataImportSchedulableOffHint: '推荐用于新账号或批量导入：账号只保存到系统，不会进入号池承接请求，可按批次检查后再开启。',
      dataImportModelRestrictionHint: '为本次导入的账号统一写入模型白名单；开启自动检测后，会在账号创建成功后请求上游模型列表并覆盖为真实可用模型。',
      dataImportAutoDetectModelsOnHint: '导入后会请求每个账号的上游模型列表并写入模型白名单。检测失败不会删除账号，失败原因会显示在导入结果中。',
      dataImportAutoDetectModelsOffHint: '默认不会主动请求上游模型列表；如果下方选择了模型，会按所选模型写入白名单，留空则不限制模型。',
      dataImportBulkEditHint: '提示：导入完成后，可在账号列表中按批次筛选，使用"批量编辑"功能统一修改账号属性。',
      ...
      tokenPresence: {
        accessTokenPresent: 'AT 已获取：access_token 存在',
        accessTokenMissing: 'AT 未获取：access_token 缺失',
        refreshTokenPresent: 'RT 已获取：refresh_token 存在',
        refreshTokenMissing: 'RT 未获取：refresh_token 缺失'
      },
