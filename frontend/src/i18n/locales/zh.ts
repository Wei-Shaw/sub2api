export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportWarning: '导入将创建新账号与代理，导入前可统一设置分组、代理、调度参数与过期策略；请确认已有数据不会冲突。',
      dataImportFile: '数据文件',
      ...
      dataImportSharedSettingsHint: '以下设置会批量应用到本次导入的所有账号，避免导入后分页逐个修改。',
      dataImportSchedulableOnHint: '本次导入的账号会立即加入调度池，请确认分组、代理与额度配置已准备好。',
      dataImportSchedulableOffHint: '本次导入的账号将不参与调度，导入后可按批次筛选检查再开启。',
      dataImportModelRestrictionHint: '为本次导入的账号统一写入模型白名单；开启自动检测后，会在账号创建成功后从上游模型列表同步并覆盖为真实可用模型。',
      dataImportAutoDetectModelsOnHint: '导入后自动检测每个账号可用模型并写入模型白名单。检测失败不会删除账号，失败原因会显示在导入结果中。',
      dataImportAutoDetectModelsOffHint: '不会主动请求上游模型列表；如果下方选择了模型，会按所选模型写入白名单，留空则不限制模型。',
