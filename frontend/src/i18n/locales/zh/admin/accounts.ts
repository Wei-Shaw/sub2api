    ...
    accounts: {
      ...
      oauth: {
        ...
        openai: {
          ...
          codexSessionDesc: '粘贴 Codex Agent Identity auth.json、OAuth session JSON 或 accessToken，按第一步配置创建账号。',
          codexSessionInputLabel: 'Codex auth.json、OAuth JSON 或 accessToken',
          codexSessionPlaceholder: '支持完整 auth.json；OAuth token/JSON 可按行批量输入',
          codexSessionHint: 'Agent Identity 使用 PKCS#8 私钥动态签名，不需要 access_token 或 refresh_token；OAuth session 未包含 refresh_token 时仍按 accessToken 过期时间处理。',
          codexSessionImportAndCreate: '导入并创建账号',
          codexSessionEmpty: '请输入 Codex auth.json、OAuth JSON 或 accessToken',
