export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '中转倍率由上游站点自行声明，无法独立验证；官方余额来自服务商 API。请结合账单和实际用量核验上游信息。',
        autoProbe: '自动探测上游账务信息',
        autoProbeHint: '启用后按全局周期刷新支持的上游倍率或官方余额；此开关本身不会修改账号倍率。',
        syncRate: '同步上游声明倍率',
        syncRateHint: '成功探测中转倍率后自动更新账号倍率，同步的是不含高峰的基准倍率；探测失败、服务商余额响应或声明超出允许范围时保持不变。开启本项会同时打开“自动探测上游账务信息”。',
        ...
        configuredRate: '账号配置倍率：{value}x',
        deepSeekAvailable: 'DeepSeek API 余额可用',
        deepSeekUnavailable: 'DeepSeek API 余额不可用',
        deepSeekUnavailableShort: '不可用',
        deepSeekTotalBalance: '{currency} 总余额：{value}',
        deepSeekBalanceBreakdown: '赠送余额：{granted}；充值余额：{toppedUp}',
