export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率可能由 Sub2API 上游针对当前 API Key 声明，也可能来自显式选择的 New API pricing 分组。Sub2API 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
        ...
        newAPIGroupHint: 'New API 兼容回退时必须填写。请填写该 relay API Key 实际使用的分组；/api/pricing 只能提供目录，系统不会猜测 API Key 分组。启用自动同步即确认此分组。',
        syncRate: '同步上游声明倍率',
        ...
        unsupported: '不支持',
        failed: '失败',
        newAPIPricingAuthRequired: 'New API pricing 页面需要 dashboard 鉴权；当前实例不支持此兼容回退。',
        newAPIGroupRequired: '请先填写 New API 分组，再使用兼容回退。',
        newAPIGroupNotFound: 'pricing 目录没有返回已配置的 New API 分组。',
        notProbed: '未探测',
        provider: '来源：{value}',
