export default {
    accounts: {
      ...
      dataImportCodexPolicyOverride: '对导入的 OpenAI OAuth 账号应用统一身份策略',
      dataImportCodexPolicyPreserve: '默认关闭。备份中已有的策略和稳定代理引用将由服务端校验并保留。',
      ...
      status: {
        ...
        quotaExceeded: '配额超限',
        unschedulable: '不可调度',
        provisioningPending: '配置中',
        provisioningPendingHint: '账号配置尚未完成，当前不会进入调度。',
      ...
      codexIdentity: {
        title: 'Codex OS Profile 设备池',
        description: '为每个 OAuth 账号维持数量受限且稳定的设备身份；HTTP、WebSocket 和响应状态仍按 API Key 隔离。',
        offState: '已关闭，继续使用现有 Codex 身份处理方式。',
        profiles: '操作系统 Profile',
        profilesDesc: '按需启用该账号允许接收的环境；同一系统的桌面版与 CLI 可同时启用并独立配置。',
        fixErrors: '请先补全身份策略，再保存账号。',
        valid: '身份策略完整，保存时将由服务端再次校验。',
        windows: 'Windows',
        windowsDesc: '分别配置 Windows 桌面版与 CLI 身份。',
        macos: 'macOS',
        macosDesc: '分别配置 macOS 桌面版与 CLI 身份。',
        linux: 'Linux',
        linuxDesc: '分别配置 Linux 桌面版与 CLI 身份。',
        generic: '通用 SDK / 第三方',
        genericOS: '通用',
        genericDesc: '分别配置 SDK 与受支持的第三方客户端。',
        surface: '客户端形态',
        desktop: '桌面版',
        desktopDesc: '使用独立的桌面版身份与设备槽位。',
        cli: 'CLI',
        cliDesc: '使用独立的 CLI 身份与设备槽位。',
        sdk: 'SDK',
        sdkDesc: '使用独立的 SDK 身份与设备槽位。',
        thirdParty: '第三方客户端',
        thirdPartyDesc: '使用独立的第三方客户端身份与设备槽位。',
        architecture: '架构',
        notApplicable: '不适用',
        deviceSlots: '设备槽位',
        increaseSlots: '增加设备槽位',
        decreaseSlots: '减少设备槽位',
        profileProxy: 'Profile 代理',
        accountProxyDirect: '继承账号连接（当前为直连）。',
        accountProxyInherit: '继承账号代理',
        templateProxyInherit: '运行时继承每个已分配账号的连接；账号未配置代理时使用直连。',
        inheritAccountProxy: '继承账号连接',
        directConnection: '直连（绕过继承的代理）',
        slotOverrides: '按槽位覆盖代理与版本',
        deviceSlot: '设备槽位',
        slotProxyHint: '默认继承 Profile 代理，可为此槽位单独覆盖。',
        inheritProfileProxy: '继承 Profile 连接',
        clientVersion: 'Codex 客户端版本',
        fixedClientVersion: '固定 Codex 客户端版本',
        clientVersionInherit: '自动跟随（推荐）',
        clientVersionPinned: '固定版本',
        effectiveClientVersion: '生效版本',
        clientProfileVerification: '客户端资料',
        clientProfileVerified: '资料已验证',
        clientProfileUnverified: '内置资料未验证',
        ...
        conversationIsolatedDesc: '推荐。每个 API Key 下的每个对话都使用独立会话身份。',
        apiKeyShared: '同一 API Key 共享',
        apiKeySharedDesc: '同一个下游 API Key 的多个对话共享一个上游会话。',
        sessionPool: '固定会话池',
        sessionPoolDesc: '每台设备使用有限的共享会话池，并将 API Key 与对话组合稳定映射到槽位。',
        sessionPoolRisk: '不同 API Key 和对话可能共享同一个上游会话槽位；HTTP 与 WebSocket 运行状态仍按 API Key 隔离。',
        deviceShared: '整台设备共享',
        deviceSharedDesc: '同一设备槽位上的全部 API Key 共用一个上游会话。',
        sessionsPerDevice: '每台设备的会话数',
        sessionsPerDeviceDesc: '每个 API Key 与对话组合会稳定分配到共享池中的一个槽位。',
        increaseSessions: '增加会话槽位',
        decreaseSessions: '减少会话槽位',
        affinityTtl: '对话粘性时间',
        affinityTtlDesc: '在此时间内让同一对话固定使用同一个 OAuth 账号和设备槽位。',
        minutes: '分钟',
        deviceSharedRisk: '高风险：无关 API Key 会向上游呈现为同一个会话身份。',
        deviceSharedRestriction: '同一稳定设备槽位可同时处理多个请求；账号原有并发设置仍生效，跨 API Key 的续接保持隔离。',
        slotConcurrency: '每槽位本地并发上限',
        slotConcurrencyHint: '0 表示不额外限制；正整数限制同时请求数。槽位满时，新对话会尝试同类空闲槽位，已有对话保持原槽位。账号、用户和 API Key 原有并发设置仍生效。',
        apiKeySharedRisk: '同一 API Key 内的多个对话会影响同一个上游会话身份。',
        assignmentTitle: 'Codex OS 出站模板',
        assignmentDescription: '为该 OpenAI OAuth 账号启用一个由系统设置集中维护的出站模板。',
        manageTemplates: '管理 Codex 出站模板',
        assignmentOff: '已停用，该账号继续使用默认 Codex 身份处理方式。',
        loadingTemplates: '正在加载出站模板...',
        loadTemplatesFailed: '加载 Codex 出站模板失败。',
        retryTemplates: '重试',
        noTemplates: '尚未创建可用模板，请先前往系统设置创建 Codex 出站模板。',
        templateLabel: '出站模板',
        templatePlaceholder: '请选择模板',
        templateRequired: '启用 Codex 出站模板时必须选择一个模板。',
        templateRevision: '修订 {revision}',
        templateErrors: {
          CODEX_IDENTITY_TEMPLATE_NOT_FOUND: '所选模板已不存在，请刷新后重新选择。',
          CODEX_IDENTITY_TEMPLATE_REVISION_CONFLICT: '模板已被更新，请刷新后重新提交。',
          INVALID_CODEX_IDENTITY_ASSIGNMENT: '模板分配无效，请重新选择模板。',
          CODEX_IDENTITY_TEMPLATE_MANAGED: '该账号由模板管理，请在系统设置修改模板或更换账号模板。',
          CODEX_IDENTITY_TEMPLATE_RUNTIME_MISMATCH: '目标环境中的同名模板运行配置不同，已拒绝恢复以避免错误出口。',
          CONFLICTING_CODEX_IDENTITY_CONFIGURATION: '不能同时提交模板分配和账号级身份策略。',
        },
        bulkModify: '批量修改 Codex 出站模板',
        bulkModifyDesc: '勾选后可统一停用模板，或启用并选择一个模板；不勾选则保留各账号现状。',
        bulkApply: '批量应用 Codex OS Profile 设备池',
        bulkApplyDesc: '向全部选中的 OpenAI OAuth 账号应用同一份完整身份策略。',
        importOverride: '为避免跨部署代理 ID 错指，导入前会使用本策略替换备份文件中的身份策略。',
        crsOverrideExisting: '同时替换既有 OpenAI OAuth 账号的身份策略',
        crsOverrideExistingDesc: '默认关闭。同步凭据时会保留每个既有账号的策略，只有明确开启后才统一替换。',
        lifecycleTitle: '设备槽位生命周期',
        lifecycleDesc: '保存身份策略后，受影响槽位由服务端自动轮换；旧槽位会保持排空状态，直到绑定空闲。',
        refreshSlots: '刷新设备槽位',
        loadingSlots: '正在加载设备槽位...',
        loadSlotsFailed: '加载设备槽位失败。',
        retry: '重试',
        noSlots: '该账号尚未实际创建任何设备槽位。',
        activeSlot: '使用中',
        drainingSlot: '排空中',
        slotNumber: '槽位',
        epoch: '代次',
        bindings: '绑定数',
        proxy: '代理',
        inheritedProxy: '继承账号代理',
        catalogVersion: '身份目录',
        finalizeDraining: '清理已排空槽位',
        finalizeConfirm: '仅清理服务端确认已经完全排空的槽位？',
        cancel: '取消',
        confirmFinalize: '确认清理',
        finalizing: '正在清理...',
        finalizeSuccess: '已清理 {count} 个排空完成的设备槽位。',
        finalizeNone: '暂时没有达到安全清理条件的设备槽位。',
        finalizeFailed: '清理排空槽位失败。',
        validation: {
          proxyInvalid: '所选代理 ID 无效。',
          proxyNotFound: '当前部署中不存在或不可使用所选代理。',
          proxyModeInvalid: '所选代理模式无效。',
          proxyRequired: '显式代理模式必须选择一个代理。',
          proxyNotAllowed: '只有显式代理模式可以携带代理 ID。',
          surfaceNotAllowed: '所选操作系统不支持该客户端形态。',
          architectureNotAllowed: '所选 Profile 不支持该架构。',
          slotCountOutOfRange: '设备槽位数量必须在 1 到 3 之间。',
          slotOverrideOutOfRange: '代理覆盖指向了配置范围之外的设备槽位。',
          duplicateSlotOverride: '同一个设备槽位只能配置一个代理覆盖。',
          bindingScopeInvalid: '绑定范围必须是 API Key、操作系统和客户端形态。',
          unsupportedPolicyInvalid: '不支持的客户端 Profile 必须拒绝调度。',
          offModeHasProfiles: '身份策略关闭时不能保留操作系统 Profile。',
          profileRequired: '请至少启用一个操作系统 Profile。',
          duplicateProfile: '每个操作系统与客户端形态组合只能出现一次。',
          affinityTtlOutOfRange: '对话粘性时间必须在 1 分钟到 24 小时之间。',
          sessionSlotCountOutOfRange: '会话槽位数量必须在 1 到 3 之间。',
          sessionSlotCountNotApplicable: '只有固定会话池模式可以设置会话槽位数量。',
          deviceSharedRestrictionsInvalid: '槽位并发上限须为 0～1000 的整数，并关闭跨 API Key 续接。',
          clientVersionModeInvalid: 'Codex 客户端版本模式无效。',
          clientVersionRequired: '选择固定版本时必须填写 Codex 客户端版本。',
          clientVersionInvalid: '请输入有效的 Codex 客户端版本，例如 0.146.0。',
          clientVersionTooOld: 'Codex 客户端版本不能低于 0.144.0。'
        }
      },
      ...
      oauth: {
        ...
        openai: {
          ...
          codexSessionOverrideExisting: '同时替换命中的既有账号身份策略',
          codexSessionOverrideExistingDesc: '默认关闭。更新凭据及其他所选配置时，既有账号会保留当前设备池策略。',
