export default {
    accounts: {
      ...
      oauth: {
        ...
        cursor: {
          title: 'Cursor 账号授权',
          followSteps: '请按照以下步骤授权 Cursor 账号：',
          step1GenerateUrl: '生成 Cursor Deep Control 登录链接',
          generateAuthUrl: '生成授权链接',
          step2OpenUrl: '在浏览器中打开链接并完成登录',
          openUrlDesc: '打开登录链接并登录 Cursor，然后回到这里。管理端每次只向 Cursor 探测一次，由前端轮询直到登录完成。',
          importantNotice: '关闭此对话框将取消授权。轮询最多持续五分钟。',
          step3EnterCode: '等待 Cursor 登录',
          authCodeDesc: '生成链接后点击完成授权开始轮询。也可以改用 Refresh Token。',
          authCode: '可选备注',
          authCodePlaceholder: '可留空后直接完成授权，或切换到 Refresh Token',
          authCodeHint: 'Cursor 登录使用 PKCE 轮询，不需要 callback URL。',
          refreshTokenAuth: '手动输入 RT',
          refreshTokenDesc: '粘贴已有的 Cursor refresh token，换取新的 access token。',
          refreshTokenPlaceholder: '粘贴 Cursor refresh token',
          validating: '验证中...',
          validateAndCreate: '验证并保存',
          pleaseEnterRefreshToken: '请输入 Refresh Token',
          failedToGenerateUrl: '生成 Cursor 授权链接失败',
          missingExchangeParams: '缺少 Cursor OAuth 会话',
          failedToPoll: '轮询 Cursor 登录失败',
          pollTimeout: 'Cursor 登录超时，请重新生成授权链接。',
          failedToValidateRT: '校验 Cursor Refresh Token 失败',
          errors: {
            CURSOR_OAUTH_SESSION_NOT_FOUND: 'Cursor OAuth 会话不存在或已过期，请重新生成授权链接。',
            CURSOR_OAUTH_SESSION_REQUIRED: '需要 Cursor session_id。',
            CURSOR_OAUTH_POLL_FAILED: 'Cursor 轮询失败，请重新生成授权链接。'
          }
        },
        devin: {
          title: 'Devin 账号授权',
          followSteps: '请按照以下步骤授权 Devin 账号：',
          step1GenerateUrl: '生成 Devin CLI 授权链接',
          generateAuthUrl: '生成授权链接',
          step2OpenUrl: '在浏览器中打开链接并完成授权',
          openUrlDesc: '打开授权链接并登录 Devin，然后复制浏览器无法打开的本地 callback URL。',
          importantNotice: '官方 callback 是 http://127.0.0.1:59653/callback。浏览器连不上时，把地址栏完整 URL 粘贴回来即可。',
          step3EnterCode: '粘贴 callback URL 或授权码',
          authCodeDesc: '粘贴完整 callback URL、查询字符串或授权码：',
          authCode: 'Callback URL 或 Code',
          authCodePlaceholder: 'http://127.0.0.1:59653/callback?code=...&state=...',
          authCodeHint: '支持完整 callback URL、查询字符串或裸 code。',
          refreshTokenAuth: 'Session Token',
          refreshTokenDesc: '粘贴已有的 Devin session JWT，网关会同时作为 access / refresh 保存。',
          refreshTokenPlaceholder: '粘贴 Devin session token',
          validating: '导入中...',
          validateAndCreate: '导入并保存',
          pleaseEnterSessionToken: '请输入 Devin session token',
          pleaseEnterRefreshToken: '请输入 Devin session token',
          failedToGenerateUrl: '生成 Devin 授权链接失败',
          missingExchangeParams: '缺少授权码或 OAuth 会话',
          failedToExchangeCode: '兑换 Devin 授权码失败',
          failedToImportToken: '导入 Devin session token 失败',
          errors: {
            DEVIN_OAUTH_SESSION_NOT_FOUND: 'Devin OAuth 会话不存在或已过期，请重新生成授权链接。',
            DEVIN_OAUTH_INVALID_STATE: 'Devin OAuth state 与当前会话不匹配，请粘贴同一条授权链接产生的 callback URL。',
            DEVIN_OAUTH_CODE_REQUIRED: '缺少 Devin 授权码，请粘贴完整 callback URL。',
            DEVIN_OAUTH_SESSION_INVALID: 'Devin 拒绝了该 session token，请重新登录。'
          }
        },
        // Gemini specific
        gemini: {
