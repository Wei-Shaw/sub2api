export default {
    accounts: {
      ...
      openai: {
        ...
        firstServeSettings: {
          mode: "首服模式",
          modeHint: "开启后复用路由 ID，按固定间隔切换代理 IP；开启指纹收敛时，设备指纹随 IP 一起更换，并保持同样时长。",
          reuseScope: "路由与代理复用范围",
          scopeSession: "按会话独立复用",
          scopeAccount: "同账号所有会话共享组合（默认）",
          scopeHint: "账号共享时，HTTP 与 WebSocket 共用路由 ID 和代理；完全收敛时对外会话标识也统一。对话历史和 previous_response_id 按各会话维护，WebSocket 物理连接仍独立。",
          ...
          groupPending: "账号 {name}：首服已开启，但还没有代理组，请选择包含至少两个不同出口的代理组后再发起请求。",
        ...
        firstServeMetrics: "首 token：{latency} 秒 · 下次 IP 切换：{expires} · 已切换：{rotations} 次",
        ...
        firstServeReasons: {
          ...
          observing: "当前代理保持到下一次固定切换时间。",
          ready: "当前代理有效，等待下一次固定切换。",
          slow: "首 token 仅记录用于诊断，不再触发切换。",
          context_incomplete: "上下文不完整；按固定间隔切换，并保留 previous_response_id 尝试上游续接。",
          proxy_unavailable: "没有可用的替代代理，保留当前代理和设备指纹，并在后续请求重试。",
          cooldown: "兼容旧状态；当前已使用固定间隔切换。",
          connection_failed: "当前组合出现请求失败，请检查该账号的代理和上游错误后重试。",
        },
        wsModeFirstServe: '首服模式',
        wsModeFirstServeHint: '支持 HTTP/SSE 流式及 WebSocket。按配置间隔切换代理 IP，并复用路由 ID。请绑定至少两个不同出口的代理；响应 ID 仍逐轮更新。',
