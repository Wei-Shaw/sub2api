export default {
    ...
    subscriptions: {
      resetObserver: {
        auto: '自动跟随',
        automaticHint: '保存自动跟随后，仅此配置下未来新确认的重置批次可以恢复所选订阅额度。保存会建立新基线，不会执行历史批次；订阅到期时间保持不变，也不会消耗重置次数。首次启用前，请先将所有实例升级，并等待在途请求及待结算任务完成。关闭跟随后仍保留周期记账。',
        allowEarlyResets: '包含提前批量重置',
        earlyResetHint: '默认关闭。提前重置须从至少 20% 用量降至不高于 5%，取得至少两次相隔 30 秒的新鲜低值观测，并满足相同的分组确认比例。本地消耗重置次数的操作不计入；启用此选项不会直接执行历史待核实批次。',
        autoDimensions: '自动恢复的额度维度',
        autoDimensionHint: '重置执行生效时，所选日用量清零，保留下次本地午夜重置；所选周、月窗口从执行生效时刻重新起算 7 天和 30 天。到期时间保持不变，已过期、已撤销、已暂停和尚未生效的订阅不会被重新激活。',
        monthlyAuto: '已选月额度：每次确认的上游 7 天重置都可能同时恢复月额度，因此月额度可能每个上游周恢复一次。',
        autoSaved: '自动跟随配置已保存，需要重新建立基线；只有未来新确认的批次才能恢复额度。',
        confirmedKind: '已确认来源：{kind}',
        executionResult: '生效时间：{time} · 影响 {count} 个订阅',
        groupRevision: '分组额度修订：{revision}',
        executionExpiredHint: '该批次已超过执行有效期，未恢复额度。需要新的重置证据，不会重放旧确认。',
        memberEvidence: '用量 {before}% → {after}% · {count} 次确认观测',
        confirmedKinds: { natural: '自然窗口重置', early: '提前批量重置' },
        unconfirmedSubjects: '未确认主体',
        staleAccounts: '证据已过期的账号',
        unavailableAccounts: '不可用 / 查询失败账号',
        observationOff: '跟随已关闭。选择「仅观察」或「自动跟随」后才会收集新的上游证据。',
        tooManyReferences: '参考账号最多可选择 200 个。',
        reasons: {
          quota_replenished: '已恢复所选订阅额度。',
          confirmation_no_longer_fresh: '确认信息已超过执行所需的新鲜度。',
          early_reset_threshold_not_met: '提前变化不满足 20% → 5% 的用量阈值。',
          early_repeat_sample_required: '至少间隔 30 秒后，还需再次获取新鲜低值观测。',
          repeat_low_sample_required: '该成员还需一次新鲜低值确认。',
          repeated_fresh_low_sample: '已获得相隔至少 30 秒的两次新鲜低值观测。',
          early_low_samples_interrupted: '低值证据因用量回升、窗口变化或观测过期而中断。',
          ambiguous_early_batch: '提前变化无法唯一归入一个重置批次。',
          waiting_for_fresh_sample: '等待新的上游实际观测。',
          baseline_ready: '已取得基线，等待窗口变化。',
          baseline_refreshed_after_gap: '观测中断后已更新基线，需要新的连续证据。',
          missing_quota_fields: '上游响应缺少必要额度字段，不能视为用量归零。',
          invalid_quota_fields: '上游额度字段无法校验。',
          unverified_subject: '无法确认上游额度主体身份，不计确认票。',
          stale_sample: '观测已过期，不能作为新的重置证据。',
          replayed_window: '返回了旧窗口，需要新的窗口变化证据。',
          subject_or_plan_changed: '额度主体或套餐已变化，请核对参考账号并重新保存配置以建立新基线。',
          local_reset_excluded: '本地账号重置已排除，不能用于分组重置确认。',
          early_reset_unverified: '用量提前下降，不能据此确认自然重置，需要核实。',
          waiting_for_reset: '等待新的上游窗口变化。',
          repeat_sample_required: '需要再次获取新鲜观测以核实变化。',
          repeated_fresh_sample: '已获得重复的新鲜观测支持。',
          natural_window_advanced: '新的实际观测确认了自然窗口前进。',
          outside_aggregation_window: '证据到达时已超过本批次聚合期限。',
          single_subject_requires_opt_in: '只有一个独立主体，当前配置至少需要两个。',
          quorum_not_met: '尚未达到确认比例。',
          observation_quorum_met: '已达到确认比例。',
          account_disabled: '账号已停用，仍保留在已固定的分母中。',
          account_removed: '账号已移出分组，审计仍保留原固定成员。',
          query_failed: '上游查询失败，仍保留在已固定的分母中。'
        },
        entry: '上游重置跟随',
        title: '重置跟随 · {group}',
        observationOnly: '当前仅观察：记录上游 7 天窗口的重置证据与候选批次，不会恢复订阅额度、消耗重置次数或改变订阅到期时间。',
        settings: '观察设置',
        observations: '观察与审计',
        mode: '跟随模式',
        off: '关闭',
        observe: '仅观察',
        source: '观察窗口',
        source7d: 'OpenAI / Codex · 7 天',
        references: '参考账号',
        selected: '已选 {count} 个账号',
        referenceHint: '请明确选择属于此分组的普通全局 OpenAI OAuth 账号。重复导入按一个独立额度主体计数；暂不可用的账号仍保留在候选批次的固定分母中。',
        searchPlaceholder: '按账号名或套餐筛选，如 Pro / Team',
        unavailable: '暂不可用',
        noAccounts: '没有符合筛选条件的参考账号。',
        missingReferences: '以下已保存的账号已不符合条件或不再属于此分组。启用观察前请取消勾选；已有审计批次仍保留原先固定的成员。',
        quorum: '确认比例（%）',
        aggregation: '聚合窗口（分钟）',
        quorumHint: '默认在 10 分钟内，至少 80% 的独立额度主体确认，且至少包含 2 个主体。这是主体数量比例，不是美元容量比例；离线、过期证据或身份未知的成员不会让已固定的分母缩小。',
        dimensions: '未来恢复维度 · 仅作预览',
        dimensionHint: '未启用自动跟随时，这些选项只保存为预览配置。启用后，日用量清零并保留下次本地午夜重置；周、月窗口从执行生效时刻重新起算。订阅到期时间保持不变。',
        monthlyPreview: '已选月额度：跟随上游 7 天重置时，月额度也可能每个上游周恢复一次。当前观察模式不会恢复任何额度。',
        loadFailed: '加载观察配置或参考账号失败。',
        retry: '重试',
        saveFailed: '保存观察配置失败，已保留当前编辑内容。',
        saveConflict: '配置已被其他操作更新。请重新加载最新配置后再编辑。',
        reloadPolicy: '重新加载配置并放弃当前编辑',
        saved: '观察配置已保存，未重置任何订阅额度。',
        invalidReferences: '请至少选择 2 个符合条件的参考账号；服务端还会检查它们是否属于独立额度主体。',
        invalidSettings: '确认比例须为 1–100，聚合窗口须为 1–60 分钟，并至少选择一个额度维度。',
        lastObserved: '最近上游观测：{time}',
        statusFailed: '刷新观察状态失败，仍显示上次获取的状态。',
        confirmedCoverage: '已确认 / 固定分母',
        verifiedSubjects: '新鲜已核实 / 全部主体',
        unknownSubjects: '身份未知的额度主体',
        evidenceHint: '只有新鲜的上游实际观测可用于确认重置。缺失值、请求失败、倒计时到期和本地消耗重置次数都不计票；提前批量重置须明确启用，并取得重复低值证据。',
        notReady: '观察条件尚未齐备，请查看下方账号的基线、观测可用性和身份识别情况。',
        accountStatus: '参考账号证据',
        accountEvidence: '最近已用：{percent} · 观测时间：{time}',
        duplicate: '与 {account} 共用额度主体，只计一次。',
        noReferences: '尚未配置参考账号。',
        batches: '候选批次与审计',
        noEvents: '尚无候选重置批次，需要先获得基线和新的上游窗口变化证据。',
        confirmation: '已确认 {count} / {total} 个主体',
        batchDetail: '配置 v{version} · 需要 {required} 个主体 · 截止 {deadline}',
        batchSource: '7d · {kind} · 所选维度：{dimensions}',
        memberConfirmed: '已确认',
        memberUnconfirmed: '未确认',
        accountFallback: '账号 #{id}',
        never: '尚未观测',
        probeFailed: '上游查询失败或账号暂不可用，该账号仍保留在批次的固定分母中。',
        evidencePending: '仍需新鲜且可核实的上游观测证据。',
        states: {
          waiting_baseline: '等待基线',
          observing: '观察中',
          unknown: '证据不足',
          needs_review: '需要核实',
          local_reset: '本地重置已排除'
