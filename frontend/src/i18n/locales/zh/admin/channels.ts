export default {
    ...
    riskControl: {
      ...
      cyberPolicyCaptureRequest: '留存 cyber_policy 完整请求',
      cyberPolicyCaptureRequestHint: '开启后，cyber_policy 拦截会把完整原始请求体存入独立表（不脱敏），随风控记录一起过期删除。',
      requestBodyTitle: '完整请求',
      requestBodyLoad: '查看完整请求',
      requestBodyLoading: '加载中',
      requestBodyMissing: '该记录未留存请求体',
      requestBodyFailed: '加载请求体失败',
      replayToModeration: '重放到审核接口',
      replaying: '重放中',
      replayFailed: '重放失败',
      replayResultTitle: '重放结果',
      replayScannedText: '送审文本',
      replayNote: '审核接口只返回内容分类分数，不会返回 cyber_policy；未命中属正常情况，不代表上游会放行。',
      chunkModerationEnabled: '分块审核',
      chunkModerationEnabledHint: '开启后，超长内容会按分片大小切开逐片送审，覆盖此前被截断、从未审核的部分。默认关闭：开启会按分片数成倍放大审核请求量与耗时，前置拦截模式下该耗时直接叠加在用户请求上。',
      chunkTokens: '审核分片大小（token）',
      chunkTokensHint: '上游审核接口在单次输入超过约 4000 token 时会静默失效（返回未命中且无报错），因此超长内容按此大小分片后分别送审、取各分类最高分。上限 3500。',
      chunkConcurrency: '分片并发数',
      chunkConcurrencyHint: '同时送审的分片数量，越大越快但请求量越高。上限 8。',
      chunkMaxChunks: '最大分片数',
      chunkMaxChunksHint: '单次审核最多送审的分片数，用于给超长会话的成本和延迟封顶。超出的分片不送审，会记入日志。上限 64。',
      replayChunkInfo: '共 {tokens} token，分 {chunks} 片送审（每片上限 {limit} token）',
      replayCyberDimensionNote: '本条最高风险为 illicit（违规），且各分类均未达阈值。上游 cyber_policy 关注的是网络安全风险（如攻击/渗透代码），与审核接口的内容毒性维度不同，因此未命中属正常，不代表审核失灵。',
      queueDelay: '排队 {ms} ms',
      allGroups: '全部分组',
