export default {
    ...
    subscriptions: {
      resetObserver: {
        ...
        observationOff: '观察已关闭。启用「仅观察」后才会收集新的上游证据。',
        ...
        reasons: {
          ...
          observation_quorum_met: '已达到观察确认比例，未重置订阅额度。',
        ...
        dimensionHint: '这些选项目前只保存为预览配置。未来若启用自动跟随，日用量将清零并保留下次本地午夜重置；周、月窗口从确认时刻重新起算。订阅到期时间保持不变。',
        ...
        invalidSettings: '确认比例须为 1–100，聚合窗口须为 1–60 分钟，并至少选择一个预览维度。',
        ...
        evidenceHint: '只有新鲜的上游实际观测可用于确认重置。缺失值、请求失败、倒计时到期和本地消耗重置次数都不计票；用量提前下降需进一步核实。',
        ...
        batchSource: '7d · {kind} · 预览维度：{dimensions}',
        memberConfirmed: '已确认',
