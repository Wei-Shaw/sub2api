export default {
    ops: {
      ...
      runtime: {
        ...
        metricThresholdsHint: '超出阈值的指标会在运维看板中以红色标记。',
        slaMinPercent: 'SLA 最低值（%）',
        slaMinPercentHint: '成功率低于该值时标红（默认 99.5）。',
        ttftP99MaxMs: 'TTFT P99 最大值（毫秒）',
        ttftP99MaxMsHint: '首字延迟 P99 高于该值时标红（默认 500）。',
        requestErrorRateMaxPercent: '请求错误率最大值（%）',
        requestErrorRateMaxPercentHint: '请求失败占比高于该值时标红（默认 5）。',
        upstreamErrorRateMaxPercent: '上游错误率最大值（%）',
        upstreamErrorRateMaxPercentHint: '上游失败占比高于该值时标红（默认 5）。',
        validation: {
          title: '请先修正以下问题',
          invalid: '设置不合法',
