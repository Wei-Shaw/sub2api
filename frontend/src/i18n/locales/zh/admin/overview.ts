export default {
    ...
    groups: {
      ...
      form: {
        ...
        maxReasoningEffortHint: '仅限制客户端主动请求的 OpenAI reasoning effort；Composite 分组仅对解析到 OpenAI 的请求生效。超过上限时自动降档，不会为缺省请求主动开启推理。上限优先级高于映射。映射可选填模型名称以精确匹配客户端请求模型，留空则对所有模型生效。',
        reasoningEffortMappings: '推理强度映射',
        addReasoningEffortMapping: '添加映射',
        removeReasoningEffortMapping: '删除映射',
        reasoningEffortModel: '适用模型（可选）',
        reasoningEffortModelPlaceholder: '留空适用于全部模型',
        reasoningEffortFrom: '请求值',
        reasoningEffortTo: '转发值',
        reasoningEffortFromPlaceholder: '选择或输入 A',
        reasoningEffortToPlaceholder: '请选择 B',
        fromRequired: '请选择请求值 A',
        toRequired: '请选择转发值 B',
        unsupportedFrom: '当前平台不支持推理强度映射',
        unsupportedTo: '转发值不受当前平台支持',
        duplicateFrom: '同一模型范围内请求值 A 不能重复',
        modelTooLong: '模型名称不能超过 64 个字符',
        fromTooLong: '请求值不能超过 64 个字符',
        exclusiveLabel: '专属分组',
