export default {
  promptAudit: {
    ...
    actions: { refresh: '刷新运行态', retry: '重试', Allow: '放行', Warn: '警告', Block: '阻止', Error: '失败' },
    ...
    decisions: { pass: '通过', flag: '标记', critical: '严重', failed: '审计失败' },
	...
	metrics: { total: '总计', allowed: '放行', flagged: '标记', blocked: '阻止', unavailable: '不可用', timeouts: '超时', failovers: '故障切换', genericRequests: '通用 LLM 请求', sampledOut: '采样跳过', genericFailures: 'Schema / 策略失败', genericTokens: '通用 LLM Token', genericLatency: '通用 LLM 平均耗时' },
    pool: {
      ...
      engineType: '审计引擎', systemGuidance: '附加审计指引', confidence: '置信度阈值', sampleRate: '采样率', maxOutputTokens: '最大输出 Token', reasoningEffort: '推理强度', jsonMode: 'JSON 输出模式', stage: '执行阶段', failurePolicy: '失败策略', compositionMode: '关键词组合模式',
		...
		help: {
			...
			stage: '控制审计结果是否影响请求。影子：记录和统计 LLM 判断，但始终放行。警告：风险结果按警告记录，同时放行请求。阻止：达到策略阈值时在账号选择、计费和上游请求之前拒绝；阻止阶段必须 100% 采样。',
			failurePolicy: '控制模型超时、连接失败或响应格式无效时的处理。失败放行：审计异常时继续请求，可用性更高。失败阻止：审计异常时返回服务不可用，安全性更严格；只能与阻止阶段搭配。',
			compositionMode: '控制关键词审计与 LLM 审计如何组合。关键词优先：关键词明确阻止时不再向外部 LLM 发送提示词，否则再运行 LLM。并行组合：两者都运行并采用更严重的结果。仅 LLM：跳过关键词结果，仅以 LLM 审计参与本次决策。',
		},
      genericWarning: '通用模型会把用户提示词发送给外部服务并产生调用成本。模型输出可能受到提示注入影响，因此系统会严格校验 JSON，并由服务端决定最终动作。启用 fail_open 会在模型异常时放行；fail_closed 会提高可用性风险。',
      blockSamplingWarning: 'block 阶段必须使用 100% 采样率。', failClosedWarning: 'fail_closed 只能用于 block 阶段。',
    ...
    events: {
      ...
      errorCode: '错误码', errorMessage: '失败原因', failedChunk: '失败分片',
      requestId: 'Request ID', promptHash: 'Prompt SHA-256',
      technical: {
        scanner: '扫描器', policy: '策略', guardEndpoint: 'Guard 节点', config: '配置版本',
		chunks: '分片数', latency: '耗时', protocol: '协议', engine: '审计引擎 / 模型', schemaConfidence: 'Schema / 置信度', enforcement: '阶段 / 失败策略', usage: '输入 / 输出 / 总 Token', shadowComparison: '关键词 / LLM 对比', agree: '一致', disagree: '不一致',
      },
