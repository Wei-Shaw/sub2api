export default {
    scheduledTests: {
      ...
      running: '运行中',
      schedule: '定时测试',
      createMissing: '一键创建定时测试',
      createMissingTitle: '为缺少计划的账号创建定时测试',
      createMissingConfirm: '系统将只为尚无定时测试计划的账号创建计划，每 30 分钟使用低成本模型检测一次，并开启自动恢复。首次检测会分散执行，是否继续？',
      createMissingSuccess: '已创建 {count} 个定时测试计划',
      createMissingFailed: '批量创建定时测试失败',
