export default {
    ...
    proxies: {
      ...
      subscriptionUrlHint: '可填写 Clash/Mihomo 订阅 URL；也可以留空并直接粘贴内容。',
      subscriptionContent: '订阅内容',
      subscriptionContentPlaceholder: '粘贴 Clash/Mihomo YAML、Base64 订阅，或每行一个 http/socks 代理 URL',
      subscriptionContentHint: 'URL 和内容至少填写一项；同时填写时优先使用粘贴内容。',
      subscriptionNamePrefix: '名称前缀',
      subscriptionNamePrefixPlaceholder: '例如：sub',
      subscriptionQualityPolicy: '质量策略',
      qualityPolicyApply: '应用质量策略',
      qualityPolicyTitle: '应用代理质量策略',
      qualityPolicy: '策略',
      qualityPolicyHint: '执行质量检测，并按所选等级阈值禁用不达标代理。',
      qualityPolicySelectedScope: '将对已选中的 {count} 个代理执行检测并应用策略。',
      qualityPolicyFilteredScope: '将对当前筛选条件下的全部代理执行检测并应用策略。',
      qualityPolicyConfirm: '应用策略',
      qualityPolicyApplying: '应用中...',
      qualityPolicyEmpty: '没有可检测的代理',
      qualityPolicyDone: '质量策略已应用：检测 {checked}/{count}，禁用 {disabled}',
      qualityPolicyPartial: '质量策略已应用但有问题：检测 {checked}/{count}，禁用 {disabled}，失败 {failed}',
      qualityPolicyFailed: '应用质量策略失败',
      subscriptionQualityPolicyHint: '导入后可自动质量检测，并按所选等级阈值禁用不达标代理。',
      subscriptionQualityDisableD: '启用 C 及以上（禁用 D/F）',
      subscriptionQualityDisableCOrBelow: '启用 B 及以上（禁用 C/D/F）',
      subscriptionQualityDisableBOrBelow: '仅启用 A（禁用 B/C/D/F）',
      subscriptionQualityNone: '跳过质量过滤',
      subscriptionSupportedHint: '当前仅会导入 http、https、socks5、socks5h 节点；SS/VLESS/Hysteria/Trojan 等节点请通过本地 Clash/Mihomo SOCKS 桥接后使用。',
      subscriptionImportButton: '导入订阅',
      subscriptionImportRequired: '请填写订阅 URL 或粘贴订阅内容',
      subscriptionImportSuccess: '订阅导入完成：创建 {created}，跳过 {skipped}，不支持 {unsupported}，质量检测 {quality_checked}，禁用 {quality_disabled}',
      subscriptionImportPartial: '订阅导入完成但有问题：创建 {created}，跳过 {skipped}，不支持 {unsupported}，无效 {invalid}，失败 {failed}，质量失败 {quality_failed}，禁用 {quality_disabled}',
