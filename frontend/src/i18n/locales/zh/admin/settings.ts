export default {
    settings: {
      ...
      payment: {
        ...
        field_keyId: 'Key ID（商户公钥）',
        field_forwardPayerEmail: '向服务商透传付款人邮箱',
        ...
        field_paymentCurrencyHint: '默认 CNY。Stripe 和 Airwallex 可按账户支持从下拉项选择 HKD、USD 等币种；Infini 只支持外币，默认 USD；微信、支付宝、易支付仍按 CNY。',
        field_infiniApiBaseHint: '生产环境填 https://openapi.infini.money，沙箱环境填 https://openapi-sandbox.infini.money。',
        field_infiniWebhookSecretHint: 'Infini 商户后台用于 Webhook 验签的密钥；若后台未单独提供，填与 Secret Key 相同的值。',
        field_forwardPayerEmailHint: '开启后下单时把用户邮箱传给 Infini。少付或多付时 Infini 会向该邮箱发送退款认领链接；关闭则不外发邮箱，异常款项只能走人工处理。',
        toggleEnabled: '开启',
        toggleDisabled: '关闭',
        ...
        infiniWebhookHint: '请在 Infini 商户后台把以下地址配置为 Webhook 接收地址，并订阅 order.completed、order.expired、order.late_payment 事件。Infini 没有 Webhook 管理 API，该地址无法自动下发；同一个商户号只能配置一个地址，因此一个商户号只能对接一个部署。',
