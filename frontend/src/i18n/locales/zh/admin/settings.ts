export default {
    settings: {
      ...
      gatewayRuntime: {
        title: '出站隐私与连接隔离',
        description: '管理 HTTP/TLS 连接池隔离、出站隐私头和 OpenAI WebSocket 账号级资源预算。新请求立即使用新值，空闲连接按后台周期收敛。',
        connectionPoolIsolation: 'HTTP/TLS 连接池隔离',
        connectionPoolIsolationHint: '账号 + 代理隔离适合共享网关；严格账号隔离开启时，纯代理模式会自动提升为账号 + 代理。',
        isolationAccountProxy: '按账号 + 代理隔离（推荐）',
        isolationAccount: '按账号隔离',
        isolationProxy: '仅按代理隔离',
        outboundPrivacy: '清理出站隐私头',
        outboundPrivacyHint: '移除客户端网络地址、链路追踪和逐跳请求头。',
        strictAccountIsolation: 'HTTP/TLS 跨 API Key 严格隔离',
        strictAccountIsolationHint: '同一上游账号下，不同 API Key 使用独立 HTTP/TLS 连接状态；WebSocket 状态始终按 API Key 隔离。',
        preserveHeaders: '保留的请求头',
        preserveHeadersPlaceholder: '例如 X-Request-ID，每行或逗号分隔',
        preserveHeadersHint: '仅填写必须向上游保留的隐私/追踪头；名称不区分大小写。',
        openAIWSBudget: 'OpenAI WebSocket 账号级预算',
        openAIWSBudgetHint: '三个值按上游账号聚合计算，不会为每个 API Key 单独放大。',
        maxConnections: '最大连接数', minIdle: '最小空闲数', maxIdle: '最大空闲数',
        integerRequired: 'WebSocket 预算必须填写整数。', nonNegativeBudget: '最大连接数必须大于 0，空闲连接数不能为负数。',
        minIdleExceedsMaxIdle: '最小空闲数不能大于最大空闲数。', maxIdleExceedsMaxConnections: '最大空闲数不能大于最大连接数。',
        retry: '重新加载', apply: '应用运行时设置', saved: '出站隐私与连接隔离设置已生效',
        loadFailed: '加载出站隐私与连接隔离设置失败', saveFailed: '保存出站隐私与连接隔离设置失败'
      },
      gatewayForwarding: {
        title: '请求转发行为',
