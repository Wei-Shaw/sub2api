export default {
    settings: {
      ...
      imageInputFallback: {
        title: '图片输入降级处理',
        description: '当上游不支持图片输入（如报错 unknown variant `image_url`）时，自动降级后重试',
        mode: '降级处理方式',
        modeOff: '不处理',
        modeStrip: '删除图片后重试',
        modeDescribe: '用视觉模型描述替换后重试',
        modeHint: '删除图片：直接移除请求中的图片内容；视觉描述：调用视觉模型生成描述文本替换图片',
        models: '主动处理的目标模型（多模态模拟）',
        modelsHint: '逗号分隔；支持 * 通配符（如 deepseek-*）与子串匹配。命中这些模型的请求会在发送到上游前自动处理图片输入（多模态模拟）；留空表示仅在上游报错后被动重试',
        modelsPlaceholder: '例如 gpt-4, deepseek-*（留空=仅被动重试）',
        visionBaseUrl: '视觉模型接口地址（Base URL）',
        visionBaseUrlHint: 'OpenAI 兼容端点，例如 https://api.openai.com/v1',
        visionApiKey: '视觉模型 API Key',
        visionApiKeyPlaceholderConfigured: '已配置（留空表示保持不变）',
        visionApiKeyPlaceholder: '请输入视觉模型 API Key',
        visionApiKeyHint: '仅填写新的 Key 时才会更新，留空保留原值',
        visionModel: '视觉模型名称',
        visionModelHint: '支持图片理解的模型，例如 gpt-4o-mini',
        visionTimeout: '描述请求超时（秒）',
        visionTimeoutHint: '单张图片描述请求的超时时间，0 表示使用默认 60 秒',
        saved: '图片输入降级设置保存成功',
        saveFailed: '保存图片输入降级设置失败'
      },
      rectifier: {
        title: '请求整流器',
