export default {
    settings: {
      ...
      codexProfiles: {
        title: 'Codex OS 出站模板',
        description: '集中维护可复用的操作系统、客户端形态、设备槽位与代理策略。模板保存独立于本页其他系统设置。',
        create: '新建模板',
        createTitle: '新建 Codex 出站模板',
        editTitle: '编辑 Codex 出站模板',
        edit: '编辑',
        copy: '复制',
        delete: '删除',
        deleteTitle: '删除 Codex 出站模板',
        deleteConfirm: '确定删除模板“{name}”吗？当前引用账号数：{count}。被账号引用的模板无法删除。',
        updateImpactTitle: '确认更新已分配模板',
        updateImpactMessage: '该模板正被 {count} 个账号使用。保存后会立即更新这些账号的运行配置，并轮换发生变化的设备槽位。确认继续吗？',
        confirmUpdate: '确认更新并轮换',
        loading: '正在加载 Codex 出站模板...',
        loadFailed: '加载 Codex 出站模板失败。',
        retry: '重试',
        emptyTitle: '尚未创建出站模板',
        emptyDescription: '新建模板后，可统一维护 Windows、macOS、Linux 及通用客户端的身份槽位和代理策略。',
        ...
        save: '保存模板',
        saving: '正在保存...',
        created: 'Codex 出站模板已创建。',
        updated: 'Codex 出站模板已更新。',
        deleted: 'Codex 出站模板已删除。',
        saveFailed: '保存 Codex 出站模板失败。',
        deleteFailed: '删除 Codex 出站模板失败。',
        form: {
          name: '模板名称',
          namePlaceholder: '例如：亚洲桌面与 CLI 出站',
          description: '说明',
          descriptionPlaceholder: '说明该模板的适用账号或出口策略',
          profilesTitle: '操作系统与客户端形态',
          profilesDescription: '每个系统的桌面版和 CLI 可同时启用，并分别配置架构、槽位与代理。',
        },
        validation: {
          nameRequired: '请输入模板名称。',
          profilesInvalid: '请至少启用一个客户端形态，并修正模板中的配置错误。',
        },
        errors: {
          CODEX_IDENTITY_TEMPLATE_NOT_FOUND: '该模板已不存在，请刷新列表。',
          CODEX_IDENTITY_TEMPLATE_NAME_EXISTS: '已有同名模板，请更换名称。',
          CODEX_IDENTITY_TEMPLATE_REVISION_CONFLICT: '模板已被其他操作更新（当前修订：{current_revision}），请重新打开后再编辑。',
          CODEX_IDENTITY_TEMPLATE_IN_USE: '模板仍被 {assigned_account_count} 个账号引用，不能删除。',
          CODEX_IDENTITY_TEMPLATE_UPDATE_CONFIRMATION_REQUIRED: '该模板已分配给账号，请确认更新会影响运行配置并轮换变化槽位。',
          INVALID_CODEX_IDENTITY_TEMPLATE: '模板内容不符合要求，请检查名称、会话策略和 Profile 配置。',
          INVALID_CODEX_IDENTITY_TEMPLATE_PROXY: '模板引用了不存在的代理。',
          INVALID_CODEX_IDENTITY_TEMPLATE_ID: '模板 ID 无效。',
          INVALID_CODEX_IDENTITY_TEMPLATE_REVISION: '模板修订号无效，请重新打开模板。',
          CODEX_IDENTITY_TEMPLATE_CONFLICT: '模板与现有数据冲突。',
          VALIDATION_ERROR: '模板字段校验失败，请检查必填项与长度限制。',
        },
      },
      features: {
        channelMonitor: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexClientVersionHint: '网关对上游声明的 Codex 客户端版本号，User-Agent 与 version 头同源使用。留空表示使用自动同步到的官方最新稳定版；填写后固定为该版本，不再跟随同步。最低支持 0.144.0。',
        openaiCodexVersionAutoSync: '自动同步 Codex 版本号',
        openaiCodexVersionAutoSyncHint: '每 6 小时从官方仓库获取最新稳定版客户端版本号，无需为了跟版本而升级本服务。关闭后停止后续同步；若上方留空，则沿用最近一次有效同步值，尚无有效同步值时使用内置版本。',
