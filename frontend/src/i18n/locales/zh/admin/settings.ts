export default {
    settings: {
      ...
      overloadCooldown: {
        title: '上游过载冷却',
        description: '配置 HTTP 529 与 OpenAI OAuth 连续容量错误触发的账号调度暂停策略',
        enabled: '启用 529 过载冷却',
        enabledHint: '收到 529 错误时暂停该账号调度，冷却后自动恢复',
        cooldownMinutes: '529 冷却时长（分钟）',
        cooldownMinutesHint: 'HTTP 529 后暂停账号调度的持续时间（1-120 分钟）',
        oauthCapacityEnabled: '启用 OpenAI OAuth 容量冷却',
        oauthCapacityEnabledHint: '同一 OAuth 账号连续出现 server_is_overloaded 或 slow_down 后，仅临时排除该账号的新请求',
        oauthCapacityWindowMinutes: '检测窗口（分钟）',
        oauthCapacityWindowMinutesHint: '按 OAuth 账号统计滚动窗口内的容量错误（1-60 分钟）',
        oauthCapacityFailureThreshold: '错误阈值',
        oauthCapacityFailureThresholdHint: '检测窗口内达到该错误次数后触发冷却（1-10000）',
        oauthCapacityCooldownMinutes: 'OAuth 冷却时长（分钟）',
        oauthCapacityCooldownMinutesHint: '冷却期间新请求避开该账号，到期后自动恢复（1-120 分钟）',
        saved: '过载冷却设置保存成功',
