export default {
    settings: {
      ...
      openaiFastPolicy: {
        ...
        description: '可为 OpenAI 请求注入或改写 service_tier，并按账号、模型和 tier 执行透传、过滤、请求 priority 或拦截；仅作用于 OpenAI 网关。',
        capabilityNotice: 'Sub2API 仅负责处理请求字段，并按改写后的请求档位生成本地计费记录；不能保证上游实际启用 Fast/Flex。实际处理档位及费用请以上游响应中的 service_tier 和上游账单为准。',
        injectDefaultTier: '请求缺少时注入默认 service_tier',
        injectDefaultTierHint: '开启后先补充 service_tier=default，再执行下方规则；配合“全部 tier + 请求 priority”会向上游发送 service_tier=priority，并按该档位进行本地计费。客户端已传入的 tier 不会被此步骤覆盖。',
        empty: '尚未配置处理规则。注入开关仍可单独生效；点击下方按钮新增规则。',
        ruleHeader: '规则 #{index}',
        removeRule: '删除规则',
        addRule: '新增规则',
        saveHint: '保存时随系统设置一起提交（点击页面底部「保存」按钮）。',
        serviceTier: 'service_tier 匹配条件',
        tierAll: '全部 tier（含注入的 default）',
        tierPriority: 'priority（请求 Fast）',
        tierFlex: 'flex',
        action: '处理方式',
        actionPass: '透传（保留 service_tier）',
        actionFilter: '过滤（移除 service_tier）',
        actionForcePriority: '改写为 priority（请求 Fast）',
        actionBlock: '拦截（拒绝请求）',
        scope: '生效范围',
        scopeAll: '全部账号',
        scopeOAuth: '仅 OAuth 账号',
