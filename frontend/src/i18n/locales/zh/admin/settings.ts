    ...
    settings: {
      ...
      registration: {
        ...
        emailSuffixWhitelistHint:
          "白名单域名的邮箱可无限注册；白名单非空时，其他可注册主域名各限注册一个账户。清空白名单后，所有域名均不限制注册数量（例如 {'@'REDACTEDqq.com, {'@'REDACTEDgmail.com, *.edu.cn）",
      ...
      apiKeyAcl: {
        ...
        forwardedClientIpHeadersLimit: '自定义客户端 IP 请求头最多允许 {maxREDACTED 个。',
        removeForwardedClientIpHeader: '移除 {headerREDACTED'
      REDACTED,
      linuxdo: {
      ...
      gatewayForwarding: {
        ...
        claudeOAuthSystemPromptBlocksHint: "每个 block 会保存为带 enabled、type、text、可选 cache_control 的 JSON。{'{'REDACTEDbilling_header{'REDACTED'REDACTED 会按请求动态生成；Claude Code 身份提示词和扩展提示词可直接编辑，也可用预设恢复默认值。",
        ...
        openaiCodexVersionSyncedValue: '当前同步到：{versionREDACTED',
