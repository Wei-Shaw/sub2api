export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentHint: '这是 Sub2API 站点服务端统一生成的上游身份，不继承访问者浏览器或管理员本机终端。留空使用已核验的 Codex Desktop 完整制品画像；填写单版本 CLI/TUI 画像时版本可随下方配置同步，填写 Core 与 frontend 不同的 Desktop 完整元组时会整体保留，禁止局部拼接。',
        openaiCodexClientVersion: 'Codex 客户端版本号',
        openaiCodexClientVersionPlaceholder: '留空则跟随自动同步',
        openaiCodexClientVersionHint: '用于单版本 CLI/TUI 画像的 Core 版本，同时写入 User-Agent 与 version 头。留空跟随自动同步；Desktop 双版本完整元组不受独立 CLI 版本同步影响。',
        openaiCodexVersionAutoSync: '自动同步 Codex 版本号',
        openaiCodexVersionAutoSyncHint: '每 6 小时从官方仓库获取最新稳定 CLI Core 版本，仅更新单版本 CLI/TUI 画像；不会把独立 CLI 版本塞进 Desktop 的 Core/frontend 完整元组。',
