export default {
    settings: {
      keyProtection: {
        title: '自动密钥保护',
        description: '在内容审核和模型转发前，替换对话文本中可识别的密钥。用户仍可照常粘贴密钥，无须逐个登记外部服务凭据。默认关闭。',
        enabled: '启用自动密钥保护',
        mode: '运行模式',
        reversible: '可逆替换（推荐）',
        redact: '仅遮盖（无法还原）',
        scope: '密钥还原范围',
        textAndTools: '回复文本和工具参数',
        toolsOnly: '仅工具参数',
        modeHint: '可逆替换会在响应返回时还原已授权的占位符。仅遮盖将命中内容替换为 [REDACTED]，生成的代码无法自动获得原始密钥。',
        users: '适用用户',
        groups: '适用 API 密钥分组',
        targetHint: '两项均为空时适用于所有用户；否则，命中所选用户或 API 密钥分组即可生效。按住 Ctrl / Command 可多选或取消选择分组。',
        boundary: '保护可识别文本中的密钥，不涵盖图片、音频或任意密码。受保护请求遇到不支持的入口会明确拒绝。恢复后的明文仍由客户端及工具处理；本功能不会取消密钥权限，也无法阻止工具将密钥发送至错误目标。',
        advanced: '高级设置',
        ttl: '映射有效期（秒）',
        maxMappings: '每会话密钥上限',
        maxSessions: '每安全上下文记录上限',
        rules: '内置规则编号（逗号分隔，留空使用全部默认规则）',
        customRules: '附加规则（含 name / pattern 的 JSON 对象数组）',
        storageHint: '映射属于有期限的敏感状态。会话及响应快照均计入对应用户 / API 密钥 / 分组的记录上限。跨请求存储由平台统一管理加密材料。内置编号：openai、anthropic、github、gitlab、google、stripe、slack、huggingface、groq、npm。',
        clearMappings: '清理全部已保存映射',
        clearHint: '清理会使所有用户已有的占位符和响应续接失效。客户端需重新发送含原始凭据的完整历史以建立新映射。',
        confirmClear: '确认清理全部映射',
        cleared: '已清理保存的密钥保护映射',
        clearFailed: '清理映射失败，请检查映射存储后重试。',
        save: '保存密钥保护设置',
        saved: '密钥保护设置已保存',
        loadFailed: '无法读取密钥保护设置。在成功读取前不可保存。',
        retry: '重新读取设置',
        saveFailed: '保存密钥保护设置失败，请检查规则编号、表达式、用户或分组编号及容量限制后重试。',
        platformKeyRequired: '开启可逆保护前，需要部署管理员配置固定的平台 TOTP_ENCRYPTION_KEY。已有部署应沿用原值，各实例保持一致；用户无须配置外部服务凭据。',
