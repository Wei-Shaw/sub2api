export default {
    settings: {
      ...
      features: {
        ...
        availableChannels: {
          title: '模型广场与可用渠道',
          description: '向已登录用户展示账号可访问的渠道、模型与定价。默认关闭。',
          configureLink: '前往 渠道管理 > 渠道定价 配置模型价格',
          enabled: '启用模型广场与可用渠道',
          enabledHint: '关闭后用户端的模型广场和可用渠道入口会隐藏，接口返回空数组。',
        },
        riskControl: {
          title: '风控中心',
      ...
      gatewayForwarding: {
        ...
        openAIImagesResponsesReasoningEffortHint: '仅影响 OAuth 图片桥接调用 Responses API 时的 reasoning.effort。可选：low、medium、high、xhigh；默认 medium。',
