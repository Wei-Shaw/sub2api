export default {
  ...
  auth: {
    ...
    completeCaptchaVerification: '请完成验证',
  ...
  keys: {
    ...
    groupLabel: '分组',
    selectGroup: '选择分组',
    fallbackGroupsLabel: '备用分组',
    fallbackGroupsHint: '主分组不可用时按顺序尝试，仅可选择同平台分组，可跨订阅与按量计费类型。',
    selectFallbackGroup: '添加同平台备用分组',
    fallbackGroupsSelectPrimary: '请先选择主分组，再配置同平台备用分组。',
    fallbackGroupsEmpty: '当前没有其他可用的同平台分组。',
    fallbackGroupsLimit: '最多可配置 5 个备用分组。',
    moveFallbackUp: '上移备用分组',
    moveFallbackDown: '下移备用分组',
    removeFallbackGroup: '移除备用分组',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
    noKeysYet: '暂无 API 密钥',
  ...
  admin: {
    ...
    users: {
      ...
      username: '用户名',
      emailNotBound: '未绑定邮箱',
      copyIAMPrincipal: '复制 IAM 登录账号',
      iamPrincipalCopied: 'IAM 登录账号已复制',
      enterpriseIdentity: {
        owner: '主账号',
        member: '子账号',
        unknown: '企业账号'
    ...
    groups: {
      ...
      imagePricing: {
        ...
        notConfigured: '未配置',
        matrixTitle: '档位 × 质量 二维定价矩阵',
        matrixHint: '按请求短边命中首个阈值档位，每格为单张美元价；空格回退到下方兼容价。',
        matrixTierHeader: '档位',
        matrixResolutionHeader: '分辨率阈值',
...
decodeSizeOnRspHint: '开启后上游不返回 size 字段或返回 size=auto 时，系统会解码 b64_json 内容识别真实宽高用于计费；URL 模式不受影响。',
    ...
    channels: {
      ...
      form: {
        ...
        tierLabel: '层级',
        resolution: '分辨率',
        resolutionThreshold: '分辨率阈值',
        imageTierValidation: '图片档位仅支持 1K、2K、4K，分辨率须为 WIDTHxHEIGHT；同档多质量须使用相同阈值，且各档阈值递增',
        modelMapping: '模型映射',
        ...
        responsesDefaultImageQuality: 'Responses 接口默认图片质量',
        responsesDefaultImageQualityHint: '当 /responses 请求和回包都未提供图片质量时，用此质量记录使用量并匹配图片计价档位。默认 medium。',
    ...
    modelIntros: {
      ...
      detailTitle: '模型详情',
      deleteTitle: '删除模型介绍',
      deleteConfirm: '确定要删除该模型的介绍吗？此操作不可撤销。',
      ...
      detail: {
        curlTitle: 'curl 调用示例',
        curlHint: '示例假设网关部署在当前域名。请将 YOUR_API_KEY 替换为实际 API Key；<your prompt> 等占位符按业务替换。'
      },
      columns: {
        cover: '封面',
        modelKey: '模型 / 标题',
      ...
      fields: {
        modelKey: '模型标识 (model_key)',
        modelKeyPlaceholder: '例如：gpt-4o 或 bytedance/seedance-2.5/text-to-video',
        modelKeyHint: '与对外模型名一致。创建后不可修改；如需改名请删除后重建。',
        modelKeyUseCustom: '使用自定义',
        modelKeySearchPlaceholder: '搜索或输入自定义模型名…',
        modelKeyEmpty: '暂无匹配的上游模型，可直接输入自定义',
        candidatesHint: '共 {n} 个上游候选，可下拉选择',
        candidatesLoading: '加载上游候选中…',
        title: '展示标题',
        titlePlaceholder: '留空则使用 model_key',
        description: '模型介绍',
        descriptionPlaceholder: '介绍模型的能力、适用场景等（不解析 Markdown 或 HTML）',
        descriptionPlaceholderEn: 'Introduce the model capabilities, use cases, etc. (plain text)',
        descriptionLangZh: '中文',
        descriptionLangEn: 'English',
        coverUrl: '封面图 URL',
        coverUrlPlaceholder: 'https://... 或 站内静态资源相对路径',
        sortOrder: '排序 (小值靠前)',
        enabled: '启用（供未来用户端展示）',
        defaultParams: '默认参数（支持为每个字段声明元信息）',
        defaultParamsHint: '每一行表示一个参数：可声明其默认值、是否必填、字段描述，以及是否为枚举（枚举时补充候选值）。声明后的字段会自动在演练台表单中渲染；type=json 时 value 会按 JSON 解析，解析失败则当作字符串。',
        defaultParamsEmpty: '当前没有参数；如无默认参数可留空。',
        addParam: '添加参数',
        paramKey: '键',
        paramValue: '值',
        paramRequired: '必填',
        paramAdvanced: '高级参数（默认收起在演练台“高级参数”折叠区中）',
        paramDescription: '字段描述（将展示在表单字段下方作为说明）',
        paramDescriptionEn: '字段英文描述（可选）',
        paramEnum: '枚举字段（限定可选值）',
        paramEnumPlaceholder: '每行一个，或用逗号 / 分号 / 顿号分隔。例如：\nauto, 21:9, 16:9, 4:3, 1:1, 3:4, 9:16',
        paramEnumHint: '分隔符：换行、英文逗号、中文逗号、顿号、分号、竖线 都可以混用（冒号不作为分隔，确保 “21:9” 不被拆开）。候选值会按当前字段类型尝试还原（number → 数字、json → JSON.parse），并以 select 形式展现。',
        outputFields: '输出字段',
        schema: 'Schema',
        schemaHint: '声明本模型的输入/输出字段以及主结果展示方式，课供演练台自动渲染表单与结果。',
        inputParams: '输入参数',
        inputParamsIntro: '这里声明模型接受的请求体字段。演练台会以此为依据自动渲染表单：required 字段会强制校验，description 会展示在字段旁作为提示，enum 字段会改为下拉选择。default 值同时作为演练台预填与 curl 示例的默认入参。',
        outputParams: '输出参数',
        outputParamsIntro: '这里声明模型任务完成后返回的字段结构，写法与"输入参数"完全一致：type 支持 string / number / boolean / object / array，其中 object 可展开子字段、array 可展开元素 schema，任意深度递归；每一层都可以填 description / required / enum。演练台会按这份 schema 渲染结果。视频 / 图片的主结果由下方"结果字段"和"结果类型"决定。',
        outputFieldsHint: '声明本模型任务完成后返回的字段结构；顶层 key 支持属性链 / 数组下标 / 通配（如 video.url、images[0].url、images[*]），复合类型（object / array）可继续向内嵌套子字段。主结果字段不再在行内勾选，而是在下方统一在"结果字段"中选择。',
        addOutputField: '添加输出字段',
        outputFieldsEmpty: '未配置输出字段；演练台完成后仅展示原始 payload。',
        outputKeyPlaceholder: '字段路径，例如 video.url / images[0].url / seed',
        outputLabelPlaceholder: '展示名（可选），例如：生成的视频',
        outputDescriptionPlaceholder: '字段说明（可选），会显示在演练台字段旁',
        outputDefaultPlaceholder: '默认值（可选，仅用于文档展示）',
        resultField: '结果字段',
        resultFieldNone: '(未选择，默认取第一个 video/image)',
        resultFieldHint: '选择一个输出字段作为主结果，演练台将以大尺寸媒体展示该字段。',
        resultFieldBadge: '主结果',
        resultType: '结果类型',
        resultTypeVideo: '视频 (video)',
        resultTypeImage: '图片 (image)',
        resultTypeHint: '控制主结果字段在演练台中用 <video> 还是 <img> 渲染。',
        schemaCopy: '复制 JSON',
        schemaDownload: '下载 JSON',
        schemaImport: '导入 / 链接生成',
        schemaCopied: 'Schema 已复制到剪贴板',
        schemaCopyFailed: '复制失败，请手动选择内容后复制',
        schemaExportedFile: 'Schema 文件已开始下载',
        schemaImportTitle: '导入 Schema / 从文档链接生成',
        schemaImportHint: '粘贴 JSON 或从本地选择 .json 文件；点击“应用导入”后将覆盖 JSON 中出现的字段（输入参数 / 输出参数 / 结果字段 / 结果类型，以及标题和中英文介绍）。JSON 里没有的字段保持不变。',
        schemaImportPickFile: '从文件选择…',
        schemaImportPlaceholder: '在此粘贴 Schema JSON…',
        schemaImportApply: '应用导入',
        schemaImportEmpty: '导入内容为空',
        schemaImportShape: 'JSON 结构不正确：顶层应为对象，可含 default_params、output_fields、result_field、result_type 等键',
        schemaImportParseFailed: 'JSON 解析失败：{msg}',
        schemaImportReadFailed: '文件读取失败',
        schemaImportedSuccess: 'Schema 已导入到当前表单，可继续编辑后保存',
        // ---- 从文档链接生成（抓取上游模型文档页 + 大模型解析成表单 JSON）----
        docSection: '从文档链接生成（推荐）',
        docSectionHint: '填入上游平台的模型文档页地址，系统会抓取页面正文并用大模型解析成下方 JSON（标题、中英文介绍、输入参数、输出参数、结果字段）。生成后请先审阅，再点“应用导入”回填到表单继续修改。需要先在编辑弹窗底部选好 API Key 与模型。',
        docUrlPlaceholder: '模型文档页地址，例如 https://fal.ai/models/fal-ai/xxx',
        docHintPlaceholder: '补充说明（可选），例如：只取文生视频的参数、时长单位为秒',
        docGenerate: '抓取并生成',
        docGenerating: '生成中…',
        docFetching: '正在抓取页面…',
        docParsing: '已抓到 {chars} 字，正在用大模型解析…',
        docGenerated: '已生成，请审阅下方 JSON 后点“应用导入”',
        docGeneratedTruncated: '已生成（页面内容过长已截断，请重点检查参数是否完整）',
        docFailed: '生成失败：{msg}',
        docUrlRequired: '请先填写文档页地址',
        // 行编辑器每列上方的简短字段名标签（避免只有 placeholder 时看不清列含义）
        labelKey: '键 (key)',
        labelType: '类型',
        labelDefault: '默认值',
        labelDisplayName: '展示名',
        labelDescription: '字段说明',
        labelDescriptionZh: '中文',
        labelDescriptionEn: 'English',
        // ---- 字段说明翻译按钮（每个 label 旁边）+ 编辑弹窗底部翻译工具区 ----
        translateBtn: '翻译',
        translateBtnTitle: '用大模型从另一语言翻译过来',
        translateNotReady: '请先在下方选择 API Key 与目标模型',
        translateSourceEmpty: '另一语言字段为空，无内容可翻译',
        translateSuccess: '翻译完成',
        translateFailed: '翻译失败：{msg}',
        translationTools: '翻译工具',
        translationKeyLabel: 'API Key：',
        translationModelLabel: '模型：',
        translationSelectKey: '选择 API Key',
        translationNoKey: '暂无可用 API Key',
        translationModelPlaceholder: '选择模型',
        translationModelHint: '用于翻译的 chat 模型，需你选择的 API Key 有权访问',
        ...
        moveUp: '上移',
        moveDown: '下移',
        jsonTreeHint: '（树形嵌套编辑，可添加对象键 / 数组项 / 切换叶子类型）'
      },
      errors: {
        modelKeyRequired: '请填写模型标识 (model_key)',
        paramJsonInvalid: '字段 “{key}” 的默认值不是合法 JSON，无法保存'
      }
    },
    ...
    settings: {
      ...
      turnstile: {
        ...
        enableTurnstile: '启用验证',
        enableTurnstileHint: '登录和注册时需要完成验证',
        provider: '验证方式',
        siteKey: '站点密钥',
        secretKey: '私密密钥',
      ...
      captcha: {
        ...
        description: '登录和注册的机器人防护',
        enable: '启用验证',
        enableHint: '登录和注册时需要完成验证',
        provider: '验证方式',
        turnstileSiteKey: 'Cloudflare Turnstile 站点密钥',
        turnstileSecretKey: 'Cloudflare Turnstile 私密密钥',
        turnstileSiteKeyHint: '从 Cloudflare Dashboard 获取',
        cloudflareDashboard: 'Cloudflare Dashboard',
        hcaptchaSiteKey: 'hCaptcha 站点密钥',
        hcaptchaSecretKey: 'hCaptcha 私密密钥',
        ...
        tencentSecretId: 'API密钥 SecretId',
        ...
        tencentSecretKey: 'API密钥 SecretKey',
      ...
      payment: {
        ...
        promo: {
          section: '充值赠送活动',
          sectionHint: '配置后，用户支付金额命中档位时会按 bonus_rate 在 credited_balance（pay_amount × balance_recharge_multiplier）基础上额外赠送余额；自定义金额只要落在赠送区间内同样按对应档位赠送。',
          enabled: '启用赠送活动',
          enabledHint: '关闭后该活动立即失效；保存后会重置前端红点',
          validFrom: '活动开始时间',
          validFromHint: '留空则不限制开始时间',
          validUntil: '活动截止时间',
          validUntilHint: '留空则不限制截止时间',
          tiers: '档位（升序）',
          tiersHint: '按 min_amount 升序填写；用户实付金额 ≥ 该档时按 bonus_rate 计算赠送',
          tierMinAmount: '最低金额',
          tierBonusRate: '赠送比例（0~1）',
          addTier: '新增档位',
          removeTier: '删除',
          errorEmptyTiers: '启用活动后至少需要一档',
          errorMinAmountAscending: '档位 min_amount 必须严格递增',
          errorBonusRateRange: '赠送比例必须在 [0, 1)',
          errorWindowOrder: '开始时间必须早于截止时间',
        },
        helpText: '帮助文本',
        helpImageUrl: '帮助图片链接',
