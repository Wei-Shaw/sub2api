export default {
  ...
  keys: {
    ...
    editGroupPriority: '编辑分组优先级',
    groupPriorityTitle: '分组优先级',
    groupPriorityHint: '请求默认使用第一个分组；仅当主分组没有可用的对应模型账号时，才会按顺序使用备用分组。',
    groupPrioritySelected: '已选择 {count} 个分组，选择顺序即优先级。',
    groupPrioritySaved: '分组优先级已保存',
