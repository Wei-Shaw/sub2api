export default {
  ...
  subscriptionProgress: {
    title: '我的订阅',
    viewDetails: '查看订阅详情',
    activeCount: '{countREDACTED 个有效订阅',
    daily: '每日',
    weekly: '每周',
    monthly: '每月',
    daysRemaining: '剩余 {daysREDACTED 天',
    expired: '已过期',
    expiresToday: '今天到期',
  ...
  version: {
    ...
    rollbackConfirm: '回退到 {versionREDACTED',
  ...
  userSubscriptions: {
    ...
    monthly: '每月',
    daysRemaining: '剩余 {daysREDACTED 天',
    expiresOn: '{dateREDACTED 到期',
    resetIn: '{timeREDACTED 后重置',
    quotaEndsIn: '额度将在 {timeREDACTED 后结束',
    windowNotActive: '等待首次使用',
    usageOf: '已用 {usedREDACTED / {limitREDACTED'
  REDACTED,
  ...
  payment: {
    ...
    amountTooLow: '最低金额为 {minREDACTED',
    amountTooHigh: '最高金额为 {maxREDACTED',
    amountNoMethod: '该金额没有可用的支付方式',
    rechargeRatePreview: '当前倍率：1 CNY = {usdREDACTED USD',
    refundReason: '退款原因',
    ...
    errors: {
      tooManyPending: '待支付订单过多（最多 {maxREDACTED 个），请先完成或取消现有订单',
      ...
      TOO_MANY_PENDING: '待支付订单过多（最多 {maxREDACTED 个），请先完成或取消现有订单',
      DAILY_LIMIT_EXCEEDED: '今日充值已达上限，剩余额度 {remainingREDACTED',
      PAYMENT_GATEWAY_ERROR: '支付方式不可用',
      NO_AVAILABLE_INSTANCE: '暂无可用的支付通道',
      PAYMENT_PROVIDER_MISCONFIGURED: '支付通道配置错误，请联系管理员',
      WXPAY_CONFIG_MISSING_KEY: '微信支付配置缺少必填项：{keyREDACTED',
      WXPAY_CONFIG_INVALID_KEY_LENGTH: '微信支付 {keyREDACTED 长度错误，应为 {expectedREDACTED 字节（实际 {actualREDACTED）',
      WXPAY_CONFIG_INVALID_KEY: '微信支付 {keyREDACTED 格式错误，请确认复制了完整的 PEM 内容',
    ...
    admin: {
      ...
      subscriptionCnyPayPreview: 'CNY 通道实扣预览：{amountREDACTED',
      subscriptionCnyPayPreviewWithFee: '（含 {feeRateREDACTED% 手续费：{totalREDACTED）',
      validity: '有效期',
      validityUnit: '有效期单位',
      sortOrder: '排序',
      forSale: '上架状态',
