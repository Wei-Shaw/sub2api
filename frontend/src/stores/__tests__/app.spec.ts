...
function createPublicSettings(overrides: Partial<PublicSettings> = {REDACTED): PublicSettings {
  return {
...
describe('useAppStore', () => {
  ...
  describe('公开设置加载', () => {
    it('并发调用复用并等待同一个请求，包括 force 调用', async () => {
      ...
      const settings = createPublicSettings({ payment_enabled: true REDACTED)
      const store = useAppStore()
    ...
    it('force 在无活动请求时绕过缓存，刷新期间的普通调用等待刷新结果', async () => {
      const initial = createPublicSettings({ site_name: 'Initial Site' REDACTED)
      vi.mocked(getPublicSettings).mockResolvedValueOnce(initial)
      const store = useAppStore()
      await store.fetchPublicSettings()

      const deferred = createDeferred<PublicSettings>()
      const updated = createPublicSettings({ site_name: 'Updated Site' REDACTED)
    ...
    it('clearPublicSettingsCache 清除缓存', () => {
      const windowAny = window as any
      windowAny.__APP_CONFIG__ = { site_name: 'Test' REDACTED
      const store = useAppStore()
