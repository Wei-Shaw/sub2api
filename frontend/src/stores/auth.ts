...
import { authAPI, isTotp2FARequired, type LoginResponse REDACTED from '@/api'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  function checkAuth(): void {
    ...
    if (savedToken && savedUser) {
      ...
      REDACTED catch (error) {
        ...
        clearAuth({ preservePendingAuthSession: true REDACTED)
      REDACTED
    REDACTED
  REDACTED
  ...
  function startAutoRefresh(): void {
    ...
    refreshIntervalId = setInterval(() => {
      if (token.value) {
        ...
        REDACTED)
      REDACTED
    REDACTED, AUTO_REFRESH_INTERVAL)
  REDACTED
  ...
  async function login(credentials: LoginRequest): Promise<LoginResponse> {
    ...
    REDACTED catch (error) {
      ...
      clearAuth({ preservePendingAuthSession: pendingAuthSession.value !== null REDACTED)
      throw error
    REDACTED
  REDACTED
  ...
  async function login2FA(tempToken: string, totpCode: string): Promise<User> {
    try {
      const response = await authAPI.login2FA({ temp_token: tempToken, totp_code: totpCode REDACTED)
      setAuthFromResponse(response)
      return user.value!
    REDACTED catch (error) {
      clearAuth({ preservePendingAuthSession: pendingAuthSession.value !== null REDACTED)
  ...
  function setAuthFromResponse(response: AuthResponse): void {
    ...
    REDACTED
    const { run_mode: _run_mode, ...userData REDACTED = response.user
    user.value = userData
  ...
  async function refreshUser(): Promise<User> {
    ...
    try {
      ...
      REDACTED
      const { run_mode: _run_mode, ...userData REDACTED = response.data
      user.value = userData
    ...
    REDACTED catch (error) {
      ...
      if ((error as { status?: number REDACTED).status === 401) {
        clearAuth({ preservePendingAuthSession: pendingAuthSession.value !== null REDACTED)
      REDACTED
      throw error
    REDACTED
   ...
   * Internal helper function
   */
  function clearAuth(options?: { preservePendingAuthSession?: boolean REDACTED): void {
    // Stop auto-refresh
    stopAutoRefresh()
    // Stop token refresh
    stopTokenRefresh()
