...
import type { PaymentConfig, PaymentOrder, SubscriptionPlan, CreateOrderRequest REDACTED from '@/types/payment'
...
export const usePaymentStore = defineStore('payment', () => {
  ...
  async function fetchPlans(): Promise<SubscriptionPlan[]> {
    try {
      ...
      plans.value = (response.data || []).map((p: Omit<SubscriptionPlan, 'features'> & { features: string | string[] REDACTED) => ({
        ...p,
