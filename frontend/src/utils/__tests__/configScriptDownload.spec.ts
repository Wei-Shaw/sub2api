...
describe('configScriptDownload', () => {
  it('builds a macOS Codex shell script with look2eye config values', () => {
    ...
    expect(script.content).toContain('ensure_codex_client')
    expect(script.content).toContain('npm install -g @openai/codex')
  })
  ...
  it('builds a Windows Codex batch script with embedded setup payload', () => {
    ...
    expect(script.content).toContain('setlocal EnableDelayedExpansion')
    ...
    expect(script.content).toContain('if /i "!LOOK2EYE_INSTALL_NODE!"=="Y"')
    expect(script.content).not.toContain('if /i "%LOOK2EYE_INSTALL_NODE%"=="Y"')
    expect(script.content).toContain('where winget.exe')
    expect(script.content).toContain('winget install --id OpenJS.NodeJS.LTS --source winget')
    expect(script.content).toContain('npm install -g @openai/codex')
    expect(script.content).toContain('where codex.cmd')
  ...
  it('builds a macOS Claude Code shell script that merges settings.json', () => {
    ...
    expect(script.content).toContain('ensure_claude_client')
    expect(script.content).toContain('npm install -g @anthropic-ai/claude-code')
  })
  ...
  it('builds a Windows Claude Code batch script from the marker-based template', () => {
    ...
    expect(script.content).toContain('winget install --id OpenJS.NodeJS.LTS --source winget')
    expect(script.content).toContain('npm install -g @anthropic-ai/claude-code')
    expect(script.content).toContain('where claude.cmd')
    expect(script.content).toContain('https://nodejs.org/')
    expect(script.content).toContain('where npm.cmd')
  ...
  it('builds a macOS OpenCode shell script that merges opencode.json', () => {
    ...
    expect(script.content).toContain('ensure_opencode_client')
    expect(script.content).toContain('npm install -g opencode-ai')
  })
  ...
  it('builds a Windows OpenCode batch script from the marker-based template', () => {
    ...
    expect(script.content).toContain('winget install --id OpenJS.NodeJS.LTS --source winget')
    expect(script.content).toContain('npm install -g opencode-ai')
    expect(script.content).toContain('where opencode.cmd')
    expect(script.content).toContain('https://nodejs.org/')
    expect(script.content).toContain('where npm.cmd')
