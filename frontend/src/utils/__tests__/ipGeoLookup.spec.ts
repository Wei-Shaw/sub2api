...
import { isPrivateIp, getEntry, formatGeoLabel, fetchOne, fetchBatch REDACTED from '../ipGeoLookup'
...
describe('getEntry', () => {
  it('returns an idle entry for an IP that has never been fetched', () => {
    expect(getEntry('203.0.113.9')).toEqual({ status: 'idle' REDACTED)
  REDACTED)
REDACTED)

describe('formatGeoLabel', () => {
  it('joins country/region/city with a separator', () => {
    expect(formatGeoLabel({ countryCode: 'CN', region: 'Guangdong', city: 'Shenzhen' REDACTED)).toBe('CN · Guangdong · Shenzhen')
  REDACTED)

  it('skips missing fields', () => {
    expect(formatGeoLabel({ countryCode: 'CN' REDACTED)).toBe('CN')
    expect(formatGeoLabel({ countryCode: 'US', region: 'Massachusetts' REDACTED)).toBe('US · Massachusetts')
  REDACTED)
REDACTED)
...
describe('fetchOne', () => {
  ...
  it('marks a private IP without making a network request', async () => {
    ...
    expect(getEntry('192.168.50.1')).toEqual({ status: 'private' REDACTED)
  ...
  it('marks the entry as error when the response has no country_code', async () => {
    (global.fetch as any).mockResolvedValue({
      ok: true,
      json: async () => ({ ip: '192.0.2.55', organization: 'AS64512 Unknown' REDACTED),
    REDACTED)
  ...
  it('does not re-fetch a cached successful IP unless forced', async () => {
    (global.fetch as any).mockResolvedValue({
      ok: true,
      json: async () => ({ ip: '8.8.8.8', country_code: 'US', region: 'California', city: 'Mountain View' REDACTED),
    REDACTED)
...
describe('fetchBatch', () => {
  ...
  it('deduplicates IPs and skips private addresses without a network call', async () => {
    (global.fetch as any).mockResolvedValue({
      ok: true,
      json: async () => [{ ip: '203.0.113.10', country_code: 'US', region: 'Texas', city: 'Dallas' REDACTED],
    REDACTED)
  ...
  it('splits more than 50 IPs into multiple chunk requests', async () => {
    const ips = Array.from({ length: 61 REDACTED, (_, i) => `203.0.${Math.floor(i / 250)REDACTED.${(i % 250) + 1REDACTED`)
    ;(global.fetch as any).mockImplementation(async (url: string) => ({
      ok: true,
      json: async () => {
        const queried = new URL(url).searchParams.get('ip')!.split(',')
        return queried.map((ip) => ({ ip, country_code: 'US' REDACTED))
      REDACTED,
    REDACTED))
  ...
  it('marks individual IPs as error when they are missing from the batch response', async () => {
    (global.fetch as any).mockResolvedValue({
      ok: true,
      json: async () => [{ ip: '203.0.113.20', country_code: 'US' REDACTED],
    REDACTED)
  ...
  it('skips IPs that already have a cached success entry', async () => {
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => [{ ip: '203.0.113.40', country_code: 'CN' REDACTED],
    REDACTED)
    ...
    ;(global.fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => [{ ip: '203.0.113.41', country_code: 'CN' REDACTED],
    REDACTED)
...
describe('ipGeoLookup localStorage persistence', () => {
  ...
  it('hydrates the in-memory cache from a non-expired localStorage entry on module load', async () => {
    localStorage.setItem(
      ...
      JSON.stringify({
        '121.35.47.43': { label: 'CN · Guangdong · Shenzhen', fetchedAt: Date.now() REDACTED,
      REDACTED)
    )
    ...
    expect(mod.getEntry('121.35.47.43')).toEqual(
      expect.objectContaining({ status: 'success', label: 'CN · Guangdong · Shenzhen' REDACTED)
    )
  REDACTED)
  ...
  it('ignores expired localStorage entries on module load', async () => {
    ...
    localStorage.setItem(
      'sub2api:ip-geo-cache:v1',
      JSON.stringify({
        '8.8.8.8': { label: 'US · California', fetchedAt: twentyFiveHoursAgo REDACTED,
      REDACTED)
    )

    const mod = await import('../ipGeoLookup')

    expect(mod.getEntry('8.8.8.8')).toEqual({ status: 'idle' REDACTED)
  REDACTED)

  it('persists a successful fetch result to localStorage', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ip: '1.2.4.8', country_code: 'CN' REDACTED),
    REDACTED)
    const mod = await import('../ipGeoLookup')

    await mod.fetchOne('1.2.4.8')

    const stored = JSON.parse(localStorage.getItem('sub2api:ip-geo-cache:v1') || '{REDACTED')
    expect(stored['1.2.4.8']).toEqual(expect.objectContaining({ label: 'CN' REDACTED))
  REDACTED)
  ...
  it('expires a hydrated in-memory entry after the TTL elapses', async () => {
    ...
    localStorage.setItem(
      'sub2api:ip-geo-cache:v1',
      JSON.stringify({
        '8.8.4.4': { label: 'US · California', fetchedAt: now.getTime() REDACTED,
      REDACTED)
    )

    const mod = await import('../ipGeoLookup')
    expect(mod.getEntry('8.8.4.4')).toEqual(expect.objectContaining({ status: 'success' REDACTED))

    vi.setSystemTime(new Date(now.getTime() + 25 * 60 * 60 * 1000))

    expect(mod.getEntry('8.8.4.4')).toEqual({ status: 'idle' REDACTED)
  REDACTED)
  ...
  it('re-fetches a successful in-memory cache entry after the TTL elapses', async () => {
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ip: '8.8.8.8', country_code: 'US' REDACTED),
    REDACTED)
    const mod = await import('../ipGeoLookup')
