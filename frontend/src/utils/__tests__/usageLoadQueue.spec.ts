...
import { enqueueUsageRequest REDACTED from '../usageLoadQueue'
...
function makeAccount(
  platform: string,
  type: string = 'oauth',
  proxy?: { host: string; port: number; username?: string | null REDACTED | null
): Account {
  return {
    ...
    proxy: proxy
      ? { id: 1, name: 'p', protocol: 'http', host: proxy.host, port: proxy.port, username: proxy.username ?? null, status: 'active', created_at: '', updated_at: '' REDACTED
      : undefined,
    credentials: {REDACTED,
    created_at: '',
    updated_at: ''
  REDACTED as unknown as Account
REDACTED
...
describe('usageLoadQueue', () => {
  ...
  it('Anthropic 同代理出口串行执行，间隔 >= 1s', async () => {
    ...
    const acc = makeAccount('anthropic', 'oauth', { host: '1.2.3.4', port: 8080, username: 'u1' REDACTED)
  ...
  it('Anthropic 不同代理出口并行执行', async () => {
    const timestamps: Record<string, number> = {REDACTED
    const makeTracked = (key: string) => async () => {
      timestamps[key] = Date.now()
      return key
    REDACTED

    const acc1 = makeAccount('anthropic', 'oauth', { host: '1.2.3.4', port: 8080, username: 'u1' REDACTED)
    const acc2 = makeAccount('anthropic', 'oauth', { host: '5.6.7.8', port: 3128, username: 'u2' REDACTED)
  ...
  it('Anthropic 相同代理连接信息的不同账号归为同一队列', async () => {
    ...
    const acc1 = makeAccount('anthropic', 'oauth', { host: '10.0.0.1', port: 3128, username: 'admin' REDACTED)
    const acc2 = makeAccount('anthropic', 'setup-token', { host: '10.0.0.1', port: 3128, username: 'admin' REDACTED)
  ...
  it('Anthropic 请求失败时 reject，后续任务继续执行', async () => {
    ...
    const acc = makeAccount('anthropic', 'oauth', { host: '99.99.99.99', port: 1234 REDACTED)
  ...
  it('非 Anthropic 平台直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('gemini', 'oauth', { host: '1.2.3.4', port: 8080 REDACTED)
    const acc2 = makeAccount('gemini', 'oauth', { host: '1.2.3.4', port: 8080 REDACTED)
  ...
  it('Antigravity 平台直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('antigravity', 'oauth', { host: '1.2.3.4', port: 8080 REDACTED)
    const acc2 = makeAccount('antigravity', 'oauth', { host: '1.2.3.4', port: 8080 REDACTED)
  ...
  it('OpenAI 平台直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('openai', 'oauth', { host: '1.2.3.4', port: 8080 REDACTED)
    const acc2 = makeAccount('openai', 'oauth', { host: '1.2.3.4', port: 8080 REDACTED)
  ...
  it('Anthropic apikey 类型直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('anthropic', 'apikey', { host: '1.2.3.4', port: 8080 REDACTED)
    const acc2 = makeAccount('anthropic', 'apikey', { host: '1.2.3.4', port: 8080 REDACTED)
  ...
  it('返回值正确透传', async () => {
    ...
    const result = await enqueueUsageRequest(acc, async () => {
      return { usage: 42 REDACTED
    REDACTED)
    expect(result).toEqual({ usage: 42 REDACTED)
  REDACTED)

  it('非 Anthropic 返回值正确透传', async () => {
    const acc = makeAccount('gemini', 'oauth')
    const result = await enqueueUsageRequest(acc, async () => {
      return { quota: 100 REDACTED
    REDACTED)
    expect(result).toEqual({ quota: 100 REDACTED)
  REDACTED)
REDACTED)
