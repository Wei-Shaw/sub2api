 ...
 *
 * The API client interceptor rejects with a plain object: { status, code, message, error REDACTED
...
type TranslateWithExistsFn = TranslateFn & { te?: (key: string) => boolean REDACTED
...
function localizeMetadata(metadata: Record<string, unknown>, t: TranslateFn): Record<string, unknown> {
  const out: Record<string, unknown> = { ...metadata REDACTED
  if (typeof out.key === 'string') {
    out.key = tryTranslate(t, `admin.settings.payment.field_${out.keyREDACTED`, out.key)
  REDACTED
  if (typeof out.keys === 'string') {
    out.keys = out.keys
      .split('/')
      .map(k => tryTranslate(t, `admin.settings.payment.field_${kREDACTED`, k))
      .join(' / ')
  REDACTED
  return out
REDACTED
...
export function extractI18nErrorMessage(
  ...
  if (code) {
    const key = `${namespaceREDACTED.${codeREDACTED`
    const rawMetadata = extractApiErrorMetadata(err) ?? {REDACTED
...
export function extractApiErrorMessage(
  ...
  if (typeof err === 'object' && err !== null) {
    const e = err as ApiErrorLike
    // Interceptor shape: { message, error REDACTED
    if (e.message) return e.message
    if (e.error) return e.error
    // Legacy axios shape: { response.data.detail REDACTED
