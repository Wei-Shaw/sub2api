...
export const BILLING_MODE_MIXED = 'mixed'
...
export function getBillingModeLabel(mode: string | null | undefined, t: (key: string) => string): string {
  switch (mode) {
    ...
    case BILLING_MODE_MIXED: return t('admin.usage.billingModeMixed')
...
export function getBillingModeBadgeClass(mode: string | null | undefined): string {
  switch (mode) {
    ...
    case BILLING_MODE_MIXED: return 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300'
...
export function isImageUsage(row: Pick<ImageBillingRow, 'image_count' | 'billing_mode'> | null | undefined): boolean {
  return (row?.image_count ?? 0) > 0 && row?.billing_mode !== BILLING_MODE_TOKEN && row?.billing_mode !== BILLING_MODE_VIDEO && row?.billing_mode !== BILLING_MODE_MIXED
}
...
export function imageUnitPrice(row: Pick<ImageBillingRow, 'image_count' | 'total_cost' | 'image_generation_cost'> | null): number {
  if (!row || row.image_count <= 0) return 0
  const imageCost = row.image_generation_cost ?? 0
  const total = imageCost > 0 ? imageCost : (row.total_cost ?? 0)
