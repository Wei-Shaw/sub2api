import type { CodexUsageSnapshot REDACTED from '@/types'
...
function resolveLegacy5h(snapshot: Record<string, unknown>): { used: number | null; resetAfterSeconds: number | null REDACTED {
  ...
  if (primaryWindow != null && primaryWindow <= 360) {
    return { used: primaryUsed, resetAfterSeconds: primaryReset REDACTED
  REDACTED
  if (secondaryWindow != null && secondaryWindow <= 360) {
    return { used: secondaryUsed, resetAfterSeconds: secondaryReset REDACTED
  REDACTED
  return { used: secondaryUsed, resetAfterSeconds: secondaryReset REDACTED
REDACTED

function resolveLegacy7d(snapshot: Record<string, unknown>): { used: number | null; resetAfterSeconds: number | null REDACTED {
  const primaryWindow = asNumber(snapshot.codex_primary_window_minutes)
  const secondaryWindow = asNumber(snapshot.codex_secondary_window_minutes)
  const primaryUsed = asNumber(snapshot.codex_primary_used_percent)
  const secondaryUsed = asNumber(snapshot.codex_secondary_used_percent)
...
function applyExpiredRule(window: ResolvedCodexUsageWindow, now: Date): ResolvedCodexUsageWindow {
  ...
  if (resetDate.getTime() <= now.getTime()) {
    return { usedPercent: 0, resetAt: resetDate.toISOString() REDACTED
  REDACTED
  return window
REDACTED
...
export function resolveCodexUsageWindow(
  ...
  if (!snapshot) {
    return { usedPercent: null, resetAt: null REDACTED
  REDACTED
  ...
  return applyExpiredRule({ usedPercent, resetAt REDACTED, now)
REDACTED
