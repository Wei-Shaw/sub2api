...
import { i18n, getLocale REDACTED from '@/i18n'
...
export function formatRelativeTime(date: string | Date | null | undefined): string {
  ...
  if (diffDays > 0) return i18n.global.t('common.time.daysAgo', { n: diffDays REDACTED)
  if (diffHours > 0) return i18n.global.t('common.time.hoursAgo', { n: diffHours REDACTED)
  if (diffMins > 0) return i18n.global.t('common.time.minutesAgo', { n: diffMins REDACTED)
...
export function formatDateTimeLocalInput(timestampSeconds: number | null): string {
  ...
  return `${yearREDACTED-${monthREDACTED-${dayREDACTEDT${hoursREDACTED:${minutesREDACTED`
REDACTED
...
export function formatTokensK(tokens: number): string {
  if (tokens >= 1_000_000) return `${(tokens / 1_000_000).toFixed(1)REDACTEDM`
...
export function formatCompactNumber(
  ...
  options?: { allowBillions?: boolean REDACTED
): string {
  if (num === null || num === undefined) return '0'
  ...
  if (allowBillions && abs >= 1_000_000_000) return `${(num / 1_000_000_000).toFixed(1)REDACTEDB`
  if (abs >= 1_000_000) return `${(num / 1_000_000).toFixed(1)REDACTEDM`
  if (abs >= 1_000) return `${(num / 1_000).toFixed(1)REDACTEDK`
  return num.toString()
REDACTED
...
export function formatCountdown(targetDate: string | Date | null | undefined): string | null {
  ...
  if (diffDays > 0) {
    // 超过1天：显示 "Xd Yh"
    return i18n.global.t('common.time.countdown.daysHours', { d: diffDays, h: remainingHours REDACTED)
  REDACTED
  if (diffHours > 0) {
    // 小于1天：显示 "Xh Ym"
    return i18n.global.t('common.time.countdown.hoursMinutes', { h: diffHours, m: remainingMins REDACTED)
  REDACTED
  // 小于1小时：显示 "Ym"
  return i18n.global.t('common.time.countdown.minutes', { m: diffMins REDACTED)
REDACTED
...
export function formatCountdownWithSuffix(targetDate: string | Date | null | undefined): string | null {
  ...
  return i18n.global.t('common.time.countdown.withSuffix', { time: countdown REDACTED)
REDACTED
...
export function formatRelativeWithDateTime(date: string | Date | null | undefined): string {
  ...
  return `${relativeTimeREDACTED · ${dateTimeREDACTED`
REDACTED
