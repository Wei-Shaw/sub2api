...
export function serverTimezoneLabel(utcOffset?: string | null): string {
  return utcOffset ? `UTC${utcOffsetREDACTED` : ''
REDACTED
...
export function formatPeakRateWindow(
  ...
  const base = `${fields.peak_startREDACTED-${fields.peak_endREDACTED ×${fields.peak_rate_multiplier ?? 1REDACTED`
  return tzLabel ? `${baseREDACTED (${tzLabelREDACTED)` : base
REDACTED
