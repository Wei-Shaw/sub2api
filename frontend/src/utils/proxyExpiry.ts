...
export function proxyExpiryLabelKey(
  ...
  status?: string,
): { key: string; params?: { days: number REDACTED REDACTED {
  if (status === 'expired') return { key: 'admin.proxies.expired' REDACTED
  const d = expiresAt ? daysUntil(expiresAt) : Infinity
  if (d < 0) return { key: 'admin.proxies.overdueDays', params: { days: Math.abs(d) REDACTED REDACTED
  if (d <= EXPIRY_WARN_DAYS) return { key: 'admin.proxies.expiringInDays', params: { days: d REDACTED REDACTED
  return { key: 'admin.proxies.remainingDays', params: { days: d REDACTED REDACTED
REDACTED
