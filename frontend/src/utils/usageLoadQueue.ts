...
function buildGroupKey(account: Account): string {
  ...
  const proxyIdentity = proxy
    ? `${proxy.hostREDACTED:${proxy.portREDACTED:${proxy.username || ''REDACTED`
    : 'direct'
  return `anthropic:${proxyIdentityREDACTED`
REDACTED
...
export function enqueueUsageRequest<T>(
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    REDACTED
    queue.push({ fn, resolve, reject REDACTED as Task<unknown>)
    drain(key)
  REDACTED)
REDACTED
