    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          ...
          <span class="text-lg font-semibold tracking-tight text-gray-900 dark:text-white">{{ siteName REDACTEDREDACTED</span>
        </router-link>
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-gray-900 dark:text-white">
          {{ t('keyUsage.title') REDACTEDREDACTED
        </h1>
        <p class="text-gray-500 dark:text-dark-400 text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') REDACTEDREDACTED
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          ...
          >
            ...
            {{ isQuerying ? t('keyUsage.querying') : t('keyUsage.query') REDACTEDREDACTED
          </button>
        </div>
        <p class="text-xs text-gray-400 dark:text-dark-500 mt-3 text-center">
          {{ t('keyUsage.privacyNote') REDACTEDREDACTED
        </p>

        <!-- Date Range Picker -->
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-gray-500 dark:text-dark-400">{{ t('keyUsage.dateRange') REDACTEDREDACTED</span>
            <button
            ...
            >{{ range.label REDACTEDREDACTED</button>
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              ...
              >{{ t('keyUsage.apply') REDACTEDREDACTED</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-gray-200 bg-white/90 shadow-sm backdrop-blur-sm dark:border-dark-700 dark:bg-dark-900/90">
              ...
              ></span>
              <span class="text-sm font-medium text-gray-900 dark:text-white">{{ statusInfo.label REDACTEDREDACTED</span>
              <span class="text-xs text-gray-400 dark:text-dark-500">|</span>
              <span class="text-xs text-gray-500 dark:text-dark-400">{{ statusInfo.statusText REDACTEDREDACTED</span>
            </div>
          </div>
                ...
                <div class="relative">
                  <svg class="w-44 h-44" viewBox="0 0 160 160">
                    ...
                    <circle
                      ...
                      :stroke="`url(#ring-grad-${iREDACTED)`"
                      stroke-width="10" stroke-linecap="round"
                      :stroke-dasharray="CIRCUMFERENCE.toFixed(2)"
                      :stroke-dashoffset="getRingOffset(ring)"
                    />
                    <defs>
                      <linearGradient :id="`ring-grad-${iREDACTED`" x1="0%" y1="0%" x2="100%" y2="100%">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    <template v-if="ring.isBalance">
                      <span class="text-2xl font-bold tabular-nums" :style="{ color: RING_GRADIENTS[i % 4].from REDACTED">
                        {{ ring.amount REDACTEDREDACTED
                      </span>
                    </template>
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-gray-900 dark:text-white">
                        {{ displayPcts[i] ?? 0 REDACTEDREDACTED%
                      </span>
                      <span class="text-xs text-gray-500 dark:text-dark-400 mt-0.5">{{ t('keyUsage.used') REDACTEDREDACTED</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from REDACTED"
                      >{{ ring.amount REDACTEDREDACTED</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 tabular-nums">
                        ⟳ {{ formatResetTime(ring.resetAt) REDACTEDREDACTED
                      </p>
                    </template>
                  </div>
                </div>
          ...
          >
            <div class="px-8 py-5 border-b border-gray-200 dark:border-dark-700">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.detailInfo') REDACTEDREDACTED</h3>
            </div>
            <div class="divide-y divide-gray-100 dark:divide-dark-800">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-gray-700 dark:text-dark-200">{{ row.label REDACTEDREDACTED</span>
                </div>
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-gray-900 dark:text-white'">
                  {{ row.value REDACTEDREDACTED
                </span>
              </div>
            </div>
          </div>
          ...
          >
            <div class="px-8 py-5 border-b border-gray-200 dark:border-dark-700">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.tokenStats') REDACTEDREDACTED</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-gray-100 dark:bg-dark-800">
              ...
              >
                <div class="text-xs text-gray-500 dark:text-dark-400 mb-1">{{ cell.label REDACTEDREDACTED</div>
                <div class="text-sm font-semibold tabular-nums text-gray-900 dark:text-white">{{ cell.value REDACTEDREDACTED</div>
              </div>
            </div>
          </div>
          ...
          >
            <div class="px-8 py-5 border-b border-gray-200 dark:border-dark-700">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.modelStats') REDACTEDREDACTED</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-950">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.model') REDACTEDREDACTED</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.requests') REDACTEDREDACTED</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.inputTokens') REDACTEDREDACTED</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.outputTokens') REDACTEDREDACTED</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.cacheCreationTokens') REDACTEDREDACTED</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.cacheReadTokens') REDACTEDREDACTED</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.totalTokens') REDACTEDREDACTED</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">{{ t('keyUsage.cost') REDACTEDREDACTED</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-gray-100 last:border-b-0 dark:border-dark-800"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-gray-900 dark:text-white">{{ m.model || '-' REDACTEDREDACTED</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtNum(m.requests) REDACTEDREDACTED</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtNum(m.input_tokens) REDACTEDREDACTED</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtNum(m.output_tokens) REDACTEDREDACTED</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtNum(m.cache_creation_tokens) REDACTEDREDACTED</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtNum(m.cache_read_tokens) REDACTEDREDACTED</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtNum(m.total_tokens) REDACTEDREDACTED</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) REDACTEDREDACTED</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-gray-200/50 px-6 py-8 dark:border-dark-800/50">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4 text-center sm:flex-row sm:text-left">
        <p class="text-sm text-gray-500 dark:text-dark-400">
          &copy; {{ currentYear REDACTEDREDACTED {{ siteName REDACTEDREDACTED. {{ t('home.footer.allRightsReserved') REDACTEDREDACTED
        </p>
        <div class="flex items-center gap-4">
          ...
          >{{ t('home.docs') REDACTEDREDACTED</a>
          <a
            :href="githubUrl"
            target="_blank"
            rel="noopener noreferrer"
...
import { ref, computed, onMounted, onUnmounted, nextTick REDACTED from 'vue'
...
const siteLogo = computed(() => sanitizeUrl(appStore.cachedPublicSettings?.site_logo || appStore.siteLogo || '', { allowRelative: true, allowDataUrl: true REDACTED))
...
const RING_GRADIENTS = [
  { from: '#14b8a6', to: '#5eead4' REDACTED,
  { from: '#6366F1', to: '#A5B4FC' REDACTED,
  { from: '#10B981', to: '#6EE7B7' REDACTED,
  { from: '#F59E0B', to: '#FCD34D' REDACTED,
]
...
const ringItems = computed<RingItem[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      ...
      items.push({ title: t('keyUsage.totalQuota'), pct, amount: `${usd(data.quota.used)REDACTED / ${usd(data.quota.limit)REDACTED`, iconType: 'dollar' REDACTED)
    REDACTED
    if (data.rate_limits) {
      const windowLabels: Record<string, string> = { '5h': t('keyUsage.limit5h'), '1d': t('keyUsage.limitDaily'), '7d': t('keyUsage.limit7d') REDACTED
      const windowIcons: Record<string, 'clock' | 'calendar'> = { '5h': 'clock', '1d': 'calendar', '7d': 'calendar' REDACTED
      for (const rl of data.rate_limits) {
        ...
        items.push({
          ...
          pct,
          amount: `${usd(rl.used)REDACTED / ${usd(rl.limit)REDACTED`,
  ...
  REDACTED else {
    if (data.subscription) {
      ...
      const limits = [
        { label: t('keyUsage.limitDaily'), usage: sub.daily_usage_usd, limit: sub.daily_limit_usd REDACTED,
        { label: t('keyUsage.limitWeekly'), usage: sub.weekly_usage_usd, limit: sub.weekly_limit_usd REDACTED,
        { label: t('keyUsage.limitMonthly'), usage: sub.monthly_usage_usd, limit: sub.monthly_limit_usd REDACTED,
      ]
      for (const l of limits) {
        if (l.limit != null && l.limit > 0) {
          const pct = Math.min(Math.round((l.usage / l.limit) * 100), 100)
          items.push({ title: l.label, pct, amount: `${usd(l.usage)REDACTED / ${usd(l.limit)REDACTED`, iconType: 'calendar' REDACTED)
        REDACTED
      REDACTED
    REDACTED
    if (!data.subscription && data.balance != null) {
      items.push({ title: t('keyUsage.walletBalance'), pct: 0, amount: usd(data.balance), isBalance: true, iconType: 'dollar' REDACTED)
    REDACTED
  REDACTED
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.expires_at) {
      ...
      if (daysLeft != null) {
        expiryStr += daysLeft > 0 ? ` ${t('keyUsage.daysLeft', { days: daysLeft REDACTED)REDACTED` : daysLeft === 0 ? ` ${t('keyUsage.todayExpires')REDACTED` : ''
      REDACTED
      rows.push({
    ...
    REDACTED
    if (data.rate_limits) {
      const windowMap: Record<string, string> = { '5h': '5H', '1d': locale.value === 'zh' ? '日' : 'D', '7d': '7D' REDACTED
      for (const rl of data.rate_limits) {
        const pct = rl.limit > 0 ? (rl.used / rl.limit) * 100 : 0
        let valueStr = `${usd(rl.used)REDACTED / ${usd(rl.limit)REDACTED`
        const resetStr = formatResetTime(rl.reset_at)
        if (resetStr) {
          valueStr += ` (⟳ ${resetStrREDACTED)`
        REDACTED
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')REDACTED (${windowMap[rl.window] || rl.windowREDACTED)`,
          value: valueStr,
  ...
  REDACTED else {
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')REDACTED (${locale.value === 'zh' ? '日' : 'D'REDACTED)`, value: `${usd(sub.daily_usage_usd)REDACTED / ${usd(sub.daily_limit_usd)REDACTED`, valueClass: getUsageColor(pct),
        REDACTED)
      REDACTED
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')REDACTED (${locale.value === 'zh' ? '周' : 'W'REDACTED)`, value: `${usd(sub.weekly_usage_usd)REDACTED / ${usd(sub.weekly_limit_usd)REDACTED`, valueClass: getUsageColor(pct),
        REDACTED)
      REDACTED
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')REDACTED (${locale.value === 'zh' ? '月' : 'M'REDACTED)`, value: `${usd(sub.monthly_usage_usd)REDACTED / ${usd(sub.monthly_limit_usd)REDACTED`, valueClass: getUsageColor(pct),
        REDACTED)
      REDACTED
      if (sub.expires_at) {
        rows.push({
...
const usageStatCells = computed<StatCell[]>(() => {
  ...
  const today = usage.today || {REDACTED
  const total = usage.total || {REDACTED

  return [
    { label: t('keyUsage.todayRequests'), value: fmtNum(today.requests) REDACTED,
    { label: t('keyUsage.todayInputTokens'), value: fmtNum(today.input_tokens) REDACTED,
    { label: t('keyUsage.todayOutputTokens'), value: fmtNum(today.output_tokens) REDACTED,
    { label: t('keyUsage.todayTokens'), value: fmtNum(today.total_tokens) REDACTED,
    { label: t('keyUsage.todayCacheCreation'), value: fmtNum(today.cache_creation_tokens) REDACTED,
    { label: t('keyUsage.todayCacheRead'), value: fmtNum(today.cache_read_tokens) REDACTED,
    { label: t('keyUsage.todayCost'), value: usd(today.actual_cost) REDACTED,
    { label: t('keyUsage.rpmTpm'), value: `${usage.rpm || 0REDACTED / ${usage.tpm || 0REDACTED` REDACTED,
    { label: t('keyUsage.totalRequests'), value: fmtNum(total.requests) REDACTED,
    { label: t('keyUsage.totalInputTokens'), value: fmtNum(total.input_tokens) REDACTED,
    { label: t('keyUsage.totalOutputTokens'), value: fmtNum(total.output_tokens) REDACTED,
    { label: t('keyUsage.totalTokensLabel'), value: fmtNum(total.total_tokens) REDACTED,
    { label: t('keyUsage.totalCacheCreation'), value: fmtNum(total.cache_creation_tokens) REDACTED,
    { label: t('keyUsage.totalCacheRead'), value: fmtNum(total.cache_read_tokens) REDACTED,
    { label: t('keyUsage.totalCost'), value: usd(total.actual_cost) REDACTED,
    { label: t('keyUsage.avgDuration'), value: usage.average_duration_ms ? `${Math.round(usage.average_duration_ms)REDACTED ms` : '-' REDACTED,
  ]
REDACTED)
...
async function fetchUsage(key: string) {
  ...
  if (!res.ok) {
    ...
    const msg = body?.error?.message || body?.message || `${t('keyUsage.queryFailed')REDACTED (${res.statusREDACTED)`
    throw new Error(msg)
  REDACTED
...
function formatResetTime(resetAt: string | null | undefined): string {
  ...
  if (days > 0) return `${daysREDACTEDd ${hoursREDACTEDh`
  if (hours > 0) return `${hoursREDACTEDh ${minsREDACTEDm`
  return `${minsREDACTEDm`
REDACTED
...
onMounted(() => {
  ...
  REDACTED
  resetTimer = setInterval(() => { now.value = new Date() REDACTED, 60000)
REDACTED)
...
@keyframes shimmer-kv {
  0%   { background-position: -200% 0; REDACTED
  100% { background-position: 200% 0; REDACTED
REDACTED
.skeleton {
...
@keyframes fade-up-kv {
  from { opacity: 0; transform: translateY(16px); REDACTED
  to { opacity: 1; transform: translateY(0); REDACTED
REDACTED
.fade-up {
  animation: fade-up-kv 0.5s cubic-bezier(0.4, 0, 0.2, 1) forwards;
REDACTED
.fade-up-delay-1 { animation-delay: 0.1s; opacity: 0; REDACTED
.fade-up-delay-2 { animation-delay: 0.2s; opacity: 0; REDACTED
.fade-up-delay-3 { animation-delay: 0.3s; opacity: 0; REDACTED
.fade-up-delay-4 { animation-delay: 0.4s; opacity: 0; REDACTED

/* Pulse dot */
@keyframes pulse-dot-kv {
  0%, 100% { opacity: 1; box-shadow: 0 0 0 0 currentColor; REDACTED
  50% { opacity: 0.6; box-shadow: 0 0 8px 2px currentColor; REDACTED
REDACTED
.pulse-dot {
