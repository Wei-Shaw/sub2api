import { describe, expect, it, beforeEach, afterEach, vi REDACTED from 'vitest'
...
const { showInfo, showSuccess, showError, fetchPublicSettings REDACTED = vi.hoisted(() => ({
  showInfo: vi.fn(),
  showSuccess: vi.fn(),
  showError: vi.fn(),
...
describe('KeyUsageView daily detail', () => {
  beforeEach(() => {
    ...
    Object.defineProperty(window, 'matchMedia', {
      configurable: true,
      value: vi.fn().mockReturnValue({ matches: false REDACTED),
    REDACTED)
  ...
  it('renders daily usage detail rows after a successful query', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: { Authorization: 'Bearer sk-test-key' REDACTED,
      REDACTED)
    )
