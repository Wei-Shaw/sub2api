                ...
                <Teleport to="body">
                  ...
                  >
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      <div class="px-2 py-2">
                        <div class="text-xs font-semibold uppercase tracking-wide text-ink-tertiary">
                      ...
                      <div class="px-2 py-2">
                        <div class="flex items-center justify-between gap-3">
                          ...
                          <Icon name="grid" size="sm" class="text-ink-tertiary" />
                        </div>
                      </div>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-name="{ row, value }">
            <div class="flex flex-col">
              ...
              <span
                ...
                class="text-xs text-ink-secondary truncate max-w-[200px]"
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-ink-secondary">{{ value }}</span>
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              <div v-if="row.proxy" class="flex items-center gap-2">
                <span class="text-sm text-ink-secondary">{{ row.proxy.name }}</span>
                <span v-if="row.proxy.country_code" class="text-xs text-ink-secondary">
                  ({{ row.proxy.country_code }})
                </span>
              </div>
              <span v-else class="text-sm text-ink-tertiary">-</span>
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-ink-secondary">{{ formatDateTime(row.proxy.expires_at) }}</span>
          ...
          <template #cell-rate_multiplier="{ row }">
            <span class="inline-flex items-center gap-1 text-sm font-mono text-ink-secondary">
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="flex min-w-[7rem] flex-col gap-0.5 font-mono text-[11px] leading-4">
              <div
                ...
                class="flex items-center gap-1 whitespace-nowrap text-ink-secondary"
                :title="`${formatSchedulerScoreGroup(score)} / ${formatSchedulerScore(score.base_score)} / ${formatStickySchedulerScore(score)}`"
              >
                <span class="max-w-[4.75rem] truncate text-ink-secondary">{{ formatSchedulerScoreGroup(score) }}</span>
          ...
          <template #cell-last_used_at="{ value }">
            <span class="text-sm text-ink-secondary">{{ formatRelativeTime(value) }}</span>
          </template>
          ...
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-ink-secondary">{{ formatExpiresAt(value) }}</span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button @click="handleDelete(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-secondary transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400">
...
function getAntigravityTierClass(row: any): string {
  const tier = getAntigravityTierFromRow(row)
  switch (tier) {
    case 'free-tier': return 'bg-gray-100 text-ink-secondary dark:bg-dark-700 dark:text-gray-300'
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm text-ink-secondary transition-colors hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-dark-700;
}
