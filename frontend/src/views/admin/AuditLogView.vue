<template>
  <AppLayout>
      ...
      <template #table>
        <DataTable :columns="columns" :data="logs" :loading="loading" row-key="id">
          <template #cell-created_at="{ value }">
            <span class="whitespace-nowrap text-ink-secondary">{{ formatTime(value) }}</span>
          </template>

          <template #cell-actor="{ row }">
            <div class="min-w-0 max-w-[220px]">
              <div class="truncate font-medium text-ink" :title="row.actor_email">
                {{ row.actor_email || '—' }}
              </div>
              <div class="mt-0.5 truncate text-xs text-ink-tertiary">
          ...
          <template #cell-action="{ row }">
            <div class="min-w-0 max-w-xs">
              <div class="truncate font-mono text-sm text-ink" :title="row.action">
                {{ row.action }}
              </div>
              <div class="mt-0.5 truncate font-mono text-xs text-ink-tertiary" :title="`${row.method} ${row.path}`">
                {{ row.method }} {{ row.path }}
              </div>
            </div>
          </template>
          ...
          <template #cell-latency_ms="{ value }">
            <span class="whitespace-nowrap text-ink-secondary">{{ value }} ms</span>
          </template>

          <template #cell-client_ip="{ value }">
            <span class="whitespace-nowrap font-mono text-ink-secondary">{{ value || '—' }}</span>
          </template>
          ...
          <template #empty>
            <div class="flex flex-col items-center py-8">
              ...
              <p class="text-sm font-medium text-ink-secondary">{{ t('admin.audit.empty') }}</p>
            </div>
          </template>
        </DataTable>
      </template>
    ...
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-16">
        <div class="flex flex-col items-center gap-3">
          ...
          <div class="text-sm font-medium text-ink-secondary">{{ t('common.loading') }}</div>
        </div>
      </div>

      <div v-else-if="detail" class="space-y-5 py-2">
        <!-- Hero: action + result at a glance -->
        <div class="rounded-2xl border border-line bg-gray-50/60 p-5 dark:bg-dark-900/60">
          <div class="flex flex-wrap items-center gap-3">
            ...
            <span class="break-all font-mono text-base font-semibold text-ink">
              {{ detail.action }}
            </span>
          </div>

          <div class="mt-3 flex items-center gap-2 rounded-lg bg-surface px-3 py-2 ring-1 ring-gray-200 dark:ring-dark-600">
            <span class="rounded bg-surface-sunken px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-secondary">
              {{ detail.method }}
            </span>
            <span class="break-all font-mono text-xs text-ink-secondary">{{ detail.path }}</span>
          </div>

          <div class="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs text-ink-secondary">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
            <div class="text-xs font-bold uppercase tracking-wider text-ink-tertiary">
              {{ t('admin.audit.columns.actor') }}
            </div>
            <div class="mt-1 break-all text-sm font-medium text-ink">
              {{ detail.actor_email || '—' }}
            </div>
            <div class="mt-0.5 text-xs text-ink-tertiary">{{ detail.actor_role }}</div>
          </div>
          ...
          <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
            ...
            <div v-if="detail.credential_masked" class="mt-0.5 break-all font-mono text-xs text-ink-tertiary">
        ...
        <!-- User-Agent -->
        <section>
          <h4 class="mb-1.5 text-xs font-bold uppercase tracking-wider text-ink-tertiary">
            {{ t('admin.audit.detail.userAgent') }}
          </h4>
          <div class="break-all rounded-xl bg-gray-50 p-3 font-mono text-xs leading-relaxed text-ink-secondary dark:bg-dark-900 dark:text-gray-400">
            {{ detail.user_agent || '—' }}
          </div>
        </section>
        ...
        <section v-if="detail.request_body">
          ...
          <pre class="max-h-72 overflow-auto rounded-xl bg-gray-50 p-4 font-mono text-xs leading-relaxed text-ink-secondary dark:bg-dark-900 dark:text-gray-400">{{ prettyBody(detail.request_body) }}</pre>
        </section>
        ...
        <section v-if="detail.extra && Object.keys(detail.extra).length">
          ...
          <pre class="max-h-48 overflow-auto rounded-xl bg-gray-50 p-4 font-mono text-xs leading-relaxed text-ink-secondary dark:bg-dark-900 dark:text-gray-400">{{ JSON.stringify(detail.extra, null, 2) }}</pre>
        </section>
      </div>
    </BaseDialog>
    ...
    >
      <div class="py-2">
        <p class="text-sm text-ink-secondary">{{ t('admin.audit.clearConfirm.totpHint') }}</p>
        <input
...
function statusBadgeClass(status: number): string {
  const base = 'inline-flex items-center gap-1.5 rounded-sm px-2.5 py-0.5 text-xs font-semibold'
  if (status >= 500) return base + 'bg-red-100 text-danger dark:bg-red-900/30'
  if (status >= 400) return base + 'bg-amber-100 text-warn dark:bg-amber-900/30'
  return base + 'bg-green-100 text-success dark:bg-green-900/30'
}

function statusDotClass(status: number): string {
  if (status >= 500) return 'bg-danger'
  if (status >= 400) return 'bg-warn'
  return 'bg-success'
}
