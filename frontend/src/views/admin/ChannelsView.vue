      ...
      <template #table>
        <DataTable :columns="columns" :data="channels" :loading="loading">
          ...
          <template #cell-status="{ value REDACTED">
            ...
            >
              {{ value === 'active' ? t('admin.channels.statusActive', 'Active') : t('admin.channels.statusDisabled', 'Disabled') REDACTEDREDACTED
            </span>
          </template>
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                ...
                <div class="relative" v-if="availablePlatformsToAdd.length > 0">
                  <button type="button" @click="showPlatformMenu = !showPlatformMenu" class="btn btn-secondary btn-sm">
                    ...
                    {{ t('admin.channels.form.addPlatform', '添加平台') REDACTEDREDACTED
                  </button>
                  <div
              ...
              >
                {{ t('admin.channels.form.noPlatforms', '点击"添加平台"开始配置渠道') REDACTEDREDACTED
              </div>
                ...
                >
                  <div class="flex items-center gap-2">
                    ...
                    <span :class="['text-sm font-medium', getPlatformTextColor(section.platform)]">
                      {{ t('admin.groups.platforms.' + section.platform, section.platform) REDACTEDREDACTED
                    </span>
                    <span class="text-xs text-gray-400">
                      {{ section.group_ids.length REDACTEDREDACTED {{ t('admin.channels.groupsUnit', 'groups') REDACTEDREDACTED
                    </span>
                    <span v-if="Object.keys(section.model_mapping).length > 0" class="text-xs text-gray-400">
                      · {{ Object.keys(section.model_mapping).length REDACTEDREDACTED {{ t('admin.channels.form.mappingCount', 'mappings') REDACTEDREDACTED
                    </span>
                    <span v-if="section.model_pricing.length > 0" class="text-xs text-gray-400">
                      · {{ section.model_pricing.length REDACTEDREDACTED {{ t('admin.channels.pricingUnit', 'pricing rules') REDACTEDREDACTED
                    </span>
                  </div>
                  <button
                    type="button"
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                {{ t('admin.channels.form.groups', 'Associated Groups') REDACTEDREDACTED
...
import { mTokToPerToken, perTokenToMTok, apiIntervalsToForm, formIntervalsToAPI REDACTED from '@/components/admin/channel/types'
