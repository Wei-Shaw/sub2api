      ...
      <form id="channel-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div>
          <label class="input-label">
            ...
            <span v-if="selectedGroupCount > 0" class="ml-1 text-xs font-normal text-gray-400">
              ({{ t('admin.channels.form.selectedCount', { count: selectedGroupCount REDACTED, `已选 ${selectedGroupCountREDACTED 个`) REDACTEDREDACTED)
            </span>
          </label>
          ...
          >
            ...
            <div v-else-if="groupsByPlatform.length === 0" class="py-4 text-center text-sm text-gray-500">
              {{ t('admin.channels.form.noGroupsMatch', 'No groups match your search') REDACTEDREDACTED
            </div>
            <template v-else>
              <div
        ...
        <div>
          <div class="mb-2 flex items-center justify-between">
            <label class="input-label mb-0">{{ t('admin.channels.form.modelMapping', 'Model Mapping') REDACTEDREDACTED</label>
          ...
          <p class="mb-2 text-xs text-gray-400">
            {{ t('admin.channels.form.modelMappingHint', 'Map request model names to actual model names. Runs before account-level mapping.') REDACTEDREDACTED
          </p>
          <div
        ...
        <div>
          <div class="mb-2 flex items-center justify-between">
            <label class="input-label mb-0">{{ t('admin.channels.form.modelPricing', 'Model Pricing') REDACTEDREDACTED <span class="text-red-500">*</span></label>
...
const form = reactive({
  ...
  model_mapping: {REDACTED as Record<string, string>,
...
const groupsByPlatform = computed(() => {
  ...
  const result: Array<{ platform: GroupPlatform; groups: typeof groups REDACTED> = []
  for (const p of platformOrder) {
    const list = grouped.get(p)
    if (list && list.length > 0) {
      result.push({ platform: p, groups: list REDACTED)
    REDACTED
  REDACTED
...
function openEditDialog(channel: Channel) {
  ...
  form.model_mapping = { ...(channel.model_mapping || {REDACTED) REDACTED
