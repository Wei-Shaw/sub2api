    ...
    >
      <div class="channel-dialog-body">
        ...
        <div class="flex items-center border-b border-gray-200 dark:border-dark-700 flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          ...
          >
            {{ t('admin.channels.form.basicSettings') REDACTEDREDACTED
          </button>
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <!-- Platform Management -->
            <div class="space-y-3">
              <label class="input-label mb-0">{{ t('admin.channels.form.platformConfig') REDACTEDREDACTED</label>
            ...
            <div>
              <label class="input-label text-xs">
                ...
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-gray-400">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length REDACTED) REDACTEDREDACTED)
                </span>
              </label>
...
import { apiIntervalsToForm, apiTimePricingToForm, createDefaultTimePricingForm, findModelConflict, formIntervalsToAPI, formTimePricingToAPI, isValidPositiveMultiplier, mTokToPerToken, perTokenToMTok, validateIntervals, validateTimePricing REDACTED from '@/components/admin/channel/types'
...
function formToAPI(): { group_ids: number[], model_pricing: ChannelModelPricing[], model_mapping: Record<string, Record<string, string>>, features_config: Record<string, unknown> REDACTED {
  ...
  return { group_ids: uniqueGroupIds, model_pricing, model_mapping, features_config: featuresConfig REDACTED
REDACTED
...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(t('admin.channels.noGroupsSelected', { platform: platformLabel REDACTED))
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel REDACTED))
        activeTab.value = section.platform
        return
      REDACTED
    REDACTED
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of section.model_pricing) {
      if (!isValidPositiveMultiplier(entry.fast_multiplier) ||
        ...
        appStore.showError(`${platformLabelREDACTED - ${modelLabelREDACTED: ${t('admin.channels.form.multiplierPositive')REDACTED`)
        activeTab.value = section.platform
        return
      REDACTED
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of section.model_pricing) {
      ...
      if (timePricingError) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        const modelLabel = entry.models.join(', ') || t('admin.channels.form.unnamed')
        appStore.showError(`${platformLabelREDACTED - ${modelLabelREDACTED: ${timePricingErrorREDACTED`)
        activeTab.value = section.platform
        return
      REDACTED
    REDACTED
