  ...
  <AppLayout>
    ...
    >
      ...
      >
        ...
        <!-- 账号过滤控制 (OpenAI/Antigravity/Anthropic/Gemini/xAI) -->
        <div
          v-if="
            ['openai', 'antigravity', 'anthropic', 'gemini', 'xai'].includes(
              createForm.platform,
            )
          "
...
watch(
  ...
  (newVal) => {
    ...
    if (!["openai", "antigravity", "anthropic", "gemini", "xai"].includes(newVal)) {
      createForm.require_oauth_only = false;
      createForm.require_privacy_set = false;
    }
    resetModelsListState(createModelsListState);
