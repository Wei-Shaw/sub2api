  ...
  <AppLayout>
      ...
      <template #table>
        <DataTable :columns="columns" :data="groups" :loading="loading">
              ...
              >
                ...
                >
                  <span v-if="row.daily_limit_usd"
                    >${{ row.daily_limit_usd REDACTEDREDACTED/{{ t('admin.groups.limitDay') REDACTEDREDACTED</span
                  >
                  <span
                    v-if="row.daily_limit_usd && (row.weekly_limit_usd || row.monthly_limit_usd)"
                  ...
                  <span v-if="row.weekly_limit_usd"
                    >${{ row.weekly_limit_usd REDACTEDREDACTED/{{ t('admin.groups.limitWeek') REDACTEDREDACTED</span
                  >
                  <span
                  ...
                  <span v-if="row.monthly_limit_usd"
                    >${{ row.monthly_limit_usd REDACTEDREDACTED/{{ t('admin.groups.limitMonth') REDACTEDREDACTED</span
                  >
                </template>
          ...
          <template #cell-rate_multiplier="{ value REDACTED">
            <span class="text-sm text-gray-700 dark:text-gray-300">{{ value REDACTEDREDACTEDx</span>
          </template>

          <template #cell-is_exclusive="{ value REDACTED">
            <span :class="['badge', value ? 'badge-primary' : 'badge-gray']">
              {{ value ? t('admin.groups.exclusive') : t('admin.groups.public') REDACTEDREDACTED
            </span>
          </template>
          ...
          <template #cell-actions="{ row REDACTED">
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <span class="text-xs">{{ t('admin.groups.rateMultipliers') REDACTEDREDACTED</span>
              </button>
              <button
    ...
    >
      <form id="create-group-form" @submit.prevent="handleCreateGroup" class="space-y-5">
        ...
        <div>
          <label class="input-label">{{ t('admin.groups.form.description') REDACTEDREDACTED</label>
          <textarea
        ...
        <div>
          <label class="input-label">{{ t('admin.groups.form.platform') REDACTEDREDACTED</label>
          <Select
        ...
        <div v-if="copyAccountsGroupOptions.length > 0">
          ...
          <div v-if="createForm.copy_accounts_from_group_ids.length > 0" class="flex flex-wrap gap-1.5 mb-2">
            ...
            >
              {{ copyAccountsGroupOptions.find(o => o.value === groupId)?.label || `#${groupIdREDACTED` REDACTEDREDACTED
              <button
                type="button"
          ...
          <select
            ...
            REDACTED"
          >
            <option value="">{{ t('admin.groups.copyAccounts.selectPlaceholder') REDACTEDREDACTED</option>
            <option
        ...
        <div>
          <label class="input-label">{{ t('admin.groups.form.rateMultiplier') REDACTEDREDACTED</label>
          <input
        ...
        <div v-if="createForm.subscription_type !== 'subscription'" data-tour="group-form-exclusive">
          <div class="mb-1.5 flex items-center gap-1">
            ...
            <div class="group relative inline-flex">
              ...
              <div class="pointer-events-none absolute bottom-full left-0 z-50 mb-2 w-72 opacity-0 transition-all duration-200 group-hover:pointer-events-auto group-hover:opacity-100">
                <div class="rounded-lg bg-gray-900 p-3 text-white shadow-lg dark:bg-gray-800">
                  <p class="mb-2 text-xs font-medium">{{ t('admin.groups.exclusiveTooltip.title') REDACTEDREDACTED</p>
                  ...
                  <div class="rounded bg-gray-800 p-2 dark:bg-gray-700">
                    <p class="text-xs leading-relaxed text-gray-300">
                      <span class="inline-flex items-center gap-1 text-primary-400"><Icon name="lightbulb" size="xs" /> {{ t('admin.groups.exclusiveTooltip.example') REDACTEDREDACTED</span>
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ createForm.is_exclusive ? t('admin.groups.exclusive') : t('admin.groups.public') REDACTEDREDACTED
            </span>
          </div>
        </div>
        ...
        <div class="mt-4 border-t pt-4">
          <div>
            <label class="input-label">{{ t('admin.groups.subscription.type') REDACTEDREDACTED</label>
            <Select v-model="createForm.subscription_type" :options="subscriptionTypeOptions" />
            <p class="input-hint">{{ t('admin.groups.subscription.typeHint') REDACTEDREDACTED</p>
          </div>
          ...
          >
            <div>
              <label class="input-label">{{ t('admin.groups.subscription.dailyLimit') REDACTEDREDACTED</label>
              <input
            ...
            <div>
              <label class="input-label">{{ t('admin.groups.subscription.weeklyLimit') REDACTEDREDACTED</label>
              <input
            ...
            <div>
              <label class="input-label">{{ t('admin.groups.subscription.monthlyLimit') REDACTEDREDACTED</label>
              <input
        ...
        <div v-if="createForm.platform === 'sora'" class="border-t pt-4">
          <label class="block mb-2 font-medium text-gray-700 dark:text-gray-300">
            {{ t('admin.groups.soraPricing.title') REDACTEDREDACTED
          </label>
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">
            {{ t('admin.groups.soraPricing.description') REDACTEDREDACTED
          </p>
          <div class="grid grid-cols-2 gap-3 mb-4">
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.image360') REDACTEDREDACTED</label>
              <input
            ...
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.image540') REDACTEDREDACTED</label>
              <input
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.video') REDACTEDREDACTED</label>
              <input
            ...
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.videoHd') REDACTEDREDACTED</label>
              <input
          ...
          <div class="mt-3">
            <label class="input-label">{{ t('admin.groups.soraPricing.storageQuota') REDACTEDREDACTED</label>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.groups.soraPricing.storageQuotaHint') REDACTEDREDACTED
            </p>
          </div>
        </div>
        ...
        <div v-if="createForm.platform === 'antigravity'" class="border-t pt-4">
          ...
          <div class="space-y-2">
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{ t('admin.groups.supportedScopes.claude') REDACTEDREDACTED</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                ...
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{ t('admin.groups.supportedScopes.geminiText') REDACTEDREDACTED</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                ...
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{ t('admin.groups.supportedScopes.geminiImage') REDACTEDREDACTED</span>
            </label>
          </div>
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.groups.supportedScopes.hint') REDACTEDREDACTED</p>
        </div>
        ...
        <div v-if="createForm.platform === 'antigravity'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ createForm.mcp_xml_inject ? t('admin.groups.mcpXml.enabled') : t('admin.groups.mcpXml.disabled') REDACTEDREDACTED
            </span>
          </div>
        </div>
        ...
        <div v-if="createForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ createForm.claude_code_only ? t('admin.groups.claudeCode.enabled') : t('admin.groups.claudeCode.disabled') REDACTEDREDACTED
            </span>
          </div>
          <!-- 降级分组选择（仅当启用 claude_code_only 时显示） -->
          <div v-if="createForm.claude_code_only" class="mt-3">
            <label class="input-label">{{ t('admin.groups.claudeCode.fallbackGroup') REDACTEDREDACTED</label>
            <Select
              v-model="createForm.fallback_group_id"
              :options="fallbackGroupOptions"
              :placeholder="t('admin.groups.claudeCode.noFallback')"
            />
            <p class="input-hint">{{ t('admin.groups.claudeCode.fallbackHint') REDACTEDREDACTED</p>
          </div>
        </div>

        <!-- OpenAI Messages 调度配置（仅 openai 平台） -->
        <div v-if="createForm.platform === 'openai'" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4">
          <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{{ t('admin.groups.openaiMessages.title') REDACTEDREDACTED</h4>

          <!-- 允许 Messages 调度开关 -->
          <div class="flex items-center justify-between">
            <label class="text-sm text-gray-600 dark:text-gray-400">{{ t('admin.groups.openaiMessages.allowDispatch') REDACTEDREDACTED</label>
            <button
              type="button"
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ t('admin.groups.openaiMessages.allowDispatchHint') REDACTEDREDACTED</p>

          <!-- 默认映射模型（仅当开关打开时显示） -->
          <div v-if="createForm.allow_messages_dispatch" class="mt-3">
            <label class="input-label">{{ t('admin.groups.openaiMessages.defaultModel') REDACTEDREDACTED</label>
            <input
              ...
              class="input"
            />
            <p class="input-hint">{{ t('admin.groups.openaiMessages.defaultModelHint') REDACTEDREDACTED</p>
          </div>
        </div>
        ...
        <div v-if="['openai', 'antigravity', 'anthropic', 'gemini'].includes(createForm.platform)" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4 space-y-4">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ createForm.require_oauth_only ? '已启用 — 排除 API Key 类型账号' : '未启用' REDACTEDREDACTED
              </p>
            </div>
            <button
              type="button"
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ createForm.require_privacy_set ? '已启用 — Privacy 未设置的账号将被排除' : '未启用' REDACTEDREDACTED
              </p>
            </div>
            <button
              type="button"
        ...
        <div
          ...
          class="border-t pt-4"
        >
          <label class="input-label">{{ t('admin.groups.invalidRequestFallback.title') REDACTEDREDACTED</label>
          <Select
          ...
          />
          <p class="input-hint">{{ t('admin.groups.invalidRequestFallback.hint') REDACTEDREDACTED</p>
        </div>
        ...
        <div v-if="createForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3 mb-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ createForm.model_routing_enabled ? t('admin.groups.modelRouting.enabled') : t('admin.groups.modelRouting.disabled') REDACTEDREDACTED
            </span>
          </div>
            ...
            >
              <div class="flex items-start gap-3">
                <div class="flex-1 space-y-2">
                  <div>
                    <label class="input-label text-xs">{{ t('admin.groups.modelRouting.modelPattern') REDACTEDREDACTED</label>
                    <input
                  ...
                  <div>
                    <label class="input-label text-xs">{{ t('admin.groups.modelRouting.accounts') REDACTEDREDACTED</label>
                    <!-- 已选账号标签 -->
                    ...
                    <div class="relative account-search-container">
                      ...
                      >
                        <button
                          ...
                          :class="{ 'opacity-50': rule.accounts.some(a => a.id === account.id) REDACTED"
                    ...
                    <p class="text-xs text-gray-400 mt-1">{{ t('admin.groups.modelRouting.accountsHint') REDACTEDREDACTED</p>
                  </div>
                </div>
                <button
                  type="button"
      ...
      >
        ...
        <div v-if="copyAccountsGroupOptionsForEdit.length > 0">
          ...
          <div v-if="editForm.copy_accounts_from_group_ids.length > 0" class="flex flex-wrap gap-1.5 mb-2">
            <span
              ...
              class="inline-flex items-center gap-1 rounded-full bg-primary-100 px-2.5 py-1 text-xs font-medium text-primary-700 dark:bg-primary-900/30 dark:text-primary-300"
            >
              {{ copyAccountsGroupOptionsForEdit.find(o => o.value === groupId)?.label || `#${groupIdREDACTED` REDACTEDREDACTED
              <button
                type="button"
          ...
          <p class="input-hint">{{ t('admin.groups.copyAccounts.hintEdit') REDACTEDREDACTED</p>
        </div>
        <div>
          <label class="input-label">{{ t('admin.groups.form.rateMultiplier') REDACTEDREDACTED</label>
          <input
        ...
        <div v-if="editForm.subscription_type !== 'subscription'">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ editForm.is_exclusive ? t('admin.groups.exclusive') : t('admin.groups.public') REDACTEDREDACTED
            </span>
          </div>
        </div>
        <div>
        ...
        <div class="mt-4 border-t pt-4">
          <div>
            ...
            <Select
              ...
              :disabled="true"
            />
            <p class="input-hint">{{ t('admin.groups.subscription.typeNotEditable') REDACTEDREDACTED</p>
          </div>
        ...
        <div v-if="editForm.platform === 'antigravity'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ editForm.mcp_xml_inject ? t('admin.groups.mcpXml.enabled') : t('admin.groups.mcpXml.disabled') REDACTEDREDACTED
            </span>
          </div>
        </div>
        ...
        <div v-if="editForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ editForm.claude_code_only ? t('admin.groups.claudeCode.enabled') : t('admin.groups.claudeCode.disabled') REDACTEDREDACTED
            </span>
          </div>
          <!-- 降级分组选择（仅当启用 claude_code_only 时显示） -->
        ...
        <div v-if="['openai', 'antigravity', 'anthropic', 'gemini'].includes(editForm.platform)" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4 space-y-4">
          ...
          <div class="flex items-center justify-between">
            <div>
              <label class="text-sm text-gray-600 dark:text-gray-400">仅允许 OAuth 账号</label>
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ editForm.require_oauth_only ? '已启用 — 排除 API Key 类型账号' : '未启用' REDACTEDREDACTED
              </p>
            </div>
            <button
              type="button"
          ...
          <div class="flex items-center justify-between">
            <div>
              <label class="text-sm text-gray-600 dark:text-gray-400">仅允许隐私保护已设置的账号</label>
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ editForm.require_privacy_set ? '已启用 — Privacy 未设置的账号将被排除' : '未启用' REDACTEDREDACTED
              </p>
            </div>
            <button
              type="button"
        ...
        <div v-if="editForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3 mb-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ editForm.model_routing_enabled ? t('admin.groups.modelRouting.enabled') : t('admin.groups.modelRouting.disabled') REDACTEDREDACTED
            </span>
          </div>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          >
            ...
            <div class="flex-1">
              <div class="font-medium text-gray-900 dark:text-white">{{ group.name REDACTEDREDACTED</div>
...
const columns = computed<Column[]>(() => [
  ...
  { key: 'platform', label: t('admin.groups.columns.platform'), sortable: true REDACTED,
  { key: 'billing_type', label: t('admin.groups.columns.billingType'), sortable: true REDACTED,
...
const platformOptions = computed(() => [
  ...
  { value: 'sora', label: 'Sora' REDACTED
])
...
const copyAccountsGroupOptions = computed(() => {
  ...
  return eligibleGroups.map((g) => ({
    value: g.id,
    label: `${g.nameREDACTED (${g.account_count || 0REDACTED 个账号)`
  REDACTED))
REDACTED)
...
const getCreateRuleSearchKey = (rule: ModelRoutingRule) => `create-${resolveCreateRuleKey(rule)REDACTED`
const getEditRuleSearchKey = (rule: ModelRoutingRule) => `edit-${resolveEditRuleKey(rule)REDACTED`
...
const deleteConfirmMessage = computed(() => {
  ...
  if (deletingGroup.value.subscription_type === 'subscription') {
    return t('admin.groups.deleteConfirmSubscription', { name: deletingGroup.value.name REDACTED)
  REDACTED
...
const handleCreateGroup = async () => {
  ...
  try {
    // 构建请求数据，包含模型路由配置
    const { sora_storage_quota_gb: createQuotaGb, ...createRest REDACTED = createForm
    const requestData = {
      ...createRest,
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    const { sora_storage_quota_gb: editQuotaGb, ...editRest REDACTED = editForm
    const payload = {
      ...editRest,
