    ...
    >
      ...
      >
        ...
        <div v-if="createForm.platform === 'composite'">
          <label class="input-label">{{
            t("admin.groups.form.routeScheme")
          }}</label>
          <Select
            v-model="createForm.composite_route_scheme_id"
            :options="routeSchemeSelectOptions"
          />
          <p class="input-hint">
            {{ t("admin.groups.routeSchemeHint") }}
            <router-link
              to="/admin/groups/routes"
              class="ml-1 text-primary-600 hover:underline dark:text-primary-400"
            >
              {{ t("admin.groups.manageRouteSchemes") }}
            </router-link>
          </p>
        </div>
        <!-- 从分组复制账号 -->
    ...
    >
      ...
      >
        ...
        <div v-if="editForm.platform === 'composite'">
          ...
          <Select
            v-model="editForm.composite_route_scheme_id"
            :options="routeSchemeSelectOptions"
          />
          <p class="input-hint">
            {{ t("admin.groups.routeSchemeHint") }}
...
const routeSchemeSelectOptions = computed(() => [
  { value: 0, label: t("admin.groups.compositeRoutes.noScheme") },
  ...routeSchemes.value.map((scheme) => ({
    value: scheme.id,
    label: scheme.name,
  })),
]);

const loadRouteSchemes = async () => {
  try {
    if (!adminAPI.routeSchemes?.list) return;
    routeSchemes.value = await adminAPI.routeSchemes.list();
  } catch (error) {
    console.error("Error loading route schemes:", error);
  }
};
...
const routeSchemes = ref<CompositeRouteScheme[]>([]);
...
const createForm = reactive({
  ...
  composite_route_scheme_id: 0 as number | null,
...
const closeCreateModal = () => {
  ...
  createForm.copy_accounts_from_group_ids = [];
  createForm.composite_route_scheme_id = 0;
...
const handleEdit = async (group: AdminGroup) => {
  ...
  editForm.composite_route_scheme_id = group.composite_route_scheme_id ?? 0;
  void loadRouteSchemes();
