  ...
  <AppLayout>
    <TablePageLayout>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
            ...
            <button @click="showCreateDialog = true" class="btn btn-primary">
              ...
              {{ t('admin.promo.createCode') REDACTEDREDACTED
            </button>
          </div>
        </div>
      </template>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-bonus_amount="{ value REDACTED">
          ...
          <template #cell-usage="{ row REDACTED">
            <span class="text-sm text-gray-600 dark:text-gray-300">
              {{ row.used_count REDACTEDREDACTED / {{ row.max_uses === 0 ? '∞' : row.max_uses REDACTEDREDACTED
            </span>
          </template>

          <template #cell-status="{ value, row REDACTED">
            <span
              :class="[
                'badge',
            ...
            >
              {{ getStatusLabel(value, row) REDACTEDREDACTED
            </span>
          </template>

          <template #cell-expires_at="{ value REDACTED">
            <span class="text-sm text-gray-500 dark:text-dark-400">
              {{ value ? formatDateTime(value) : t('admin.promo.neverExpires') REDACTEDREDACTED
            </span>
          </template>
      ...
      <form id="create-promo-form" @submit.prevent="handleCreate" class="space-y-4">
        <div>
          <label class="input-label">
            {{ t('admin.promo.code') REDACTEDREDACTED
            <span class="ml-1 text-xs font-normal text-gray-400">({{ t('admin.promo.autoGenerate') REDACTEDREDACTED)</span>
          </label>
          <input
        ...
        <div>
          <label class="input-label">{{ t('admin.promo.bonusAmount') REDACTEDREDACTED</label>
          <input
        ...
        <div>
          <label class="input-label">
            {{ t('admin.promo.maxUses') REDACTEDREDACTED
            <span class="ml-1 text-xs font-normal text-gray-400">({{ t('admin.promo.zeroUnlimited') REDACTEDREDACTED)</span>
          </label>
          <input
        ...
        <div>
          <label class="input-label">
            {{ t('admin.promo.expiresAt') REDACTEDREDACTED
            <span class="ml-1 text-xs font-normal text-gray-400">({{ t('common.optional') REDACTEDREDACTED)</span>
          </label>
          <input
        ...
        <div>
          <label class="input-label">
            {{ t('admin.promo.notes') REDACTEDREDACTED
            <span class="ml-1 text-xs font-normal text-gray-400">({{ t('common.optional') REDACTEDREDACTED)</span>
          </label>
          <textarea
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          <button type="submit" form="create-promo-form" :disabled="creating" class="btn btn-primary">
            {{ creating ? t('common.creating') : t('common.create') REDACTEDREDACTED
          </button>
        </div>
      </template>
    </BaseDialog>
    ...
    >
      <form id="edit-promo-form" @submit.prevent="handleUpdate" class="space-y-4">
        <div>
          <label class="input-label">{{ t('admin.promo.code') REDACTEDREDACTED</label>
          <input
        ...
        <div>
          <label class="input-label">{{ t('admin.promo.status') REDACTEDREDACTED</label>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          <button type="submit" form="edit-promo-form" :disabled="updating" class="btn btn-primary">
            {{ updating ? t('common.saving') : t('common.save') REDACTEDREDACTED
          </button>
        </div>
      </template>
    </BaseDialog>
    ...
    >
      ...
      <div v-else-if="usages.length === 0" class="py-8 text-center text-gray-500 dark:text-gray-400">
        {{ t('admin.promo.noUsages') REDACTEDREDACTED
      </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-gray-900 dark:text-white">
                {{ usage.user?.email || t('admin.promo.userPrefix', { id: usage.user_id REDACTED) REDACTEDREDACTED
              </p>
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ formatDateTime(usage.used_at) REDACTEDREDACTED
              </p>
            </div>
          </div>
          <div class="text-right">
            <span class="text-sm font-medium text-green-600 dark:text-green-400">
              +${{ usage.bonus_amount.toFixed(2) REDACTEDREDACTED
            </span>
          </div>
        </div>
        <!-- Usages Pagination -->
...
const filterStatusOptions = computed(() => [
  { value: '', label: t('admin.promo.allStatus') REDACTED,
  { value: 'active', label: t('admin.promo.statusActive') REDACTED,
  { value: 'disabled', label: t('admin.promo.statusDisabled') REDACTED
])
...
const columns = computed<Column[]>(() => [
  { key: 'code', label: t('admin.promo.columns.code') REDACTED,
  { key: 'bonus_amount', label: t('admin.promo.columns.bonusAmount'), sortable: true REDACTED,
  { key: 'usage', label: t('admin.promo.columns.usage') REDACTED,
  { key: 'status', label: t('admin.promo.columns.status'), sortable: true REDACTED,
  { key: 'expires_at', label: t('admin.promo.columns.expiresAt'), sortable: true REDACTED,
  { key: 'created_at', label: t('admin.promo.columns.createdAt'), sortable: true REDACTED,
  { key: 'actions', label: t('admin.promo.columns.actions') REDACTED
])
...
const copyRegisterLink = async (code: PromoCode) => {
  ...
  const registerLink = `${baseUrlREDACTED/register?promo=${encodeURIComponent(code.code)REDACTED`
