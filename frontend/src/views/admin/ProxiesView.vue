<template>
  <AppLayout>
        ...
        >
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded p-0.5 text-ink-tertiary hover:text-primary-600 dark:hover:text-primary-400"
                ...
                >
                  <button
                    ...
                    :key="fmt.label"
                    class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs hover:bg-surface-hover"
                    @click.stop="copyFormat(fmt.value)"
                  >
                    <span class="truncate font-mono text-ink-secondary">{{ fmt.label }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-ink-secondary">{{ row.username }}</span>
                <span v-if="row.password" class="font-mono text-ink-secondary">
              ...
              <button
                v-if="row.password"
                type="button"
                class="ml-1 rounded p-0.5 text-ink-tertiary hover:text-ink-secondary"
          ...
          <template #cell-location="{ row }">
            <div class="flex items-center gap-2">
              ...
              />
              <span v-if="formatLocation(row)" class="text-sm text-ink-secondary">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="text-sm text-ink-tertiary">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            ...
            <span
              v-else
              class="inline-flex items-center rounded bg-gray-100 px-2 py-0.5 text-xs font-medium text-ink dark:bg-dark-600 dark:text-gray-300"
            >
          ...
          <template #cell-latency="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div
                ...
                class="flex items-center gap-1 text-xs text-ink-secondary"
          ...
          <template #cell-expiry="{ row }">
            <span v-if="!row.expires_at" class="text-sm text-ink-tertiary">{{ t('admin.proxies.neverExpires') }}</span>
            <div v-else class="flex flex-col text-xs">
              <span class="text-ink-secondary">{{ formatDateTime(row.expires_at) }}</span>
              <span :class="expiryBadgeClass(row)">{{ expiryLabel(row) }}</span>
            </div>
          </template>

          <template #cell-created_at="{ row }">
            <span class="text-xs text-ink-secondary">{{ formatDateTime(row.created_at) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-secondary transition-colors hover:bg-emerald-50 hover:text-emerald-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-emerald-900/20 dark:hover:text-emerald-400"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-secondary transition-colors hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
              >
                <svg
    ...
    >
      <!-- Tab Switch -->
      <div
        class="mb-6 flex items-center justify-between gap-3 border-b border-line"
      >
          ...
          <button
            ...
            :class="[
              ...
              createMode === 'standard'
                ...
                : 'border-transparent text-ink-secondary hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
            ]"
          >
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-ink-tertiary hover:text-ink-secondary"
      ...
      <div v-else class="space-y-5">
        ...
        <div v-if="batchParseResult.total > 0" class="rounded-lg bg-surface-sunken p-4">
            ...
            <div v-if="batchParseResult.duplicate > 0" class="flex items-center gap-1.5">
              <svg
                class="h-4 w-4 text-ink-tertiary"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="2"
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        <div class="rounded-lg border border-line bg-surface-sunken p-4">
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-ink-secondary">
          ...
          <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-dark-700">
            ...
            <tbody class="divide-y divide-gray-200 bg-surface dark:divide-dark-700">
              <tr v-for="item in qualityReport.items" :key="item.target">
                ...
                <td class="whitespace-nowrap px-3 py-2 text-ink-secondary">{{ item.http_status ?? '-' }}</td>
                <td class="whitespace-nowrap px-3 py-2 text-ink-secondary">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_ms}ms` : '-' }}
                </td>
                <td class="px-3 py-2 text-ink-secondary">
                  <span>{{ item.message || '-' }}</span>
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-ink-tertiary">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div v-if="accountsLoading" class="flex items-center justify-center py-8 text-sm text-ink-secondary">
        <Icon name="refresh" size="md" class="mr-2 animate-spin" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="py-6 text-center text-sm text-ink-secondary">
