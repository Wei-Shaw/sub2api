    ...
    <TablePageLayout>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
            ...
            >
              ...
              {{ t('admin.redeem.batchUpdate') REDACTEDREDACTED
            </button>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-type="{ value REDACTED">
            <span
              :class="[
                'badge',
                value === 'balance'
            ...
            >
              {{ t('admin.redeem.types.' + value) REDACTEDREDACTED
            </span>
          </template>

          <template #cell-value="{ value, row REDACTED">
            <span class="text-sm font-medium text-gray-900 dark:text-white">
              <template v-if="row.type === 'balance'">${{ value.toFixed(2) REDACTEDREDACTED</template>
              <template v-else-if="row.type === 'subscription'">
                {{ row.validity_days || 30 REDACTEDREDACTED {{ t('admin.redeem.days') REDACTEDREDACTED
                <span v-if="row.group" class="ml-1 text-xs text-gray-500 dark:text-gray-400"
                  >({{ row.group.name REDACTEDREDACTED)</span
                >
              </template>
              <template v-else>{{ value REDACTEDREDACTED</template>
            </span>
          </template>
          ...
          <template #cell-status="{ value REDACTED">
            ...
            >
              {{ t('admin.redeem.status.' + value) REDACTEDREDACTED
            </span>
          </template>
          ...
          <template #cell-expires_at="{ value, row REDACTED">
            <span
              :class="[
                'text-sm',
                row.status === 'expired'
            ...
            >
              {{ value ? formatDateTime(value) : t('admin.redeem.neverExpires') REDACTEDREDACTED
            </span>
          </template>
      ...
      <template #pagination>
        ...
        >
          <span class="text-sm font-medium text-primary-900 dark:text-primary-100">
            {{ t('admin.redeem.selectedCount', { count: selectedCount REDACTED) REDACTEDREDACTED
          </span>
          <div class="flex flex-wrap items-center gap-2">
            ...
            >
              {{ t('admin.redeem.clearSelection') REDACTEDREDACTED
            </button>
            <button
              type="button"
      ...
      <div v-if="showGenerateDialog" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        >
          <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
            {{ t('admin.redeem.generateCodesTitle') REDACTEDREDACTED
          </h2>
          <form @submit.prevent="handleGenerateCodes" class="space-y-4">
            <div>
              <label class="input-label">{{ t('admin.redeem.codeType') REDACTEDREDACTED</label>
            ...
            <div v-if="generateForm.type === 'invitation'" class="rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20">
              <p class="text-sm text-blue-700 dark:text-blue-300">
                {{ t('admin.redeem.invitationHint') REDACTEDREDACTED
              </p>
            </div>
            <!-- 订阅类型：显示分组选择和有效天数 -->
            <template v-if="generateForm.type === 'subscription'">
              <div>
                <label class="input-label">{{ t('admin.redeem.selectGroup') REDACTEDREDACTED</label>
                <Select
              ...
              <div>
                <label class="input-label">{{ t('admin.redeem.validityDays') REDACTEDREDACTED</label>
                <input
            ...
            <div>
              <label class="input-label">{{ t('admin.redeem.codeExpiry') REDACTEDREDACTED</label>
            ...
            <div>
              <label class="input-label">{{ t('admin.redeem.count') REDACTEDREDACTED</label>
              <input
            ...
            <div class="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" :disabled="generating" class="btn btn-primary">
                {{ generating ? t('admin.redeem.generating') : t('admin.redeem.generate') REDACTEDREDACTED
              </button>
            </div>
          </form>
        </div>
      ...
      >
        ...
        >
          <h2 class="mb-1 text-lg font-semibold text-gray-900 dark:text-white">
            {{ t('admin.redeem.batchUpdateTitle') REDACTEDREDACTED
          </h2>
          ...
          <form data-test="batch-update-form" class="space-y-4" @submit.prevent="handleBatchUpdate">
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                ...
                />
                {{ t('admin.redeem.batchFields.status') REDACTEDREDACTED
              </label>
              <Select
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  ...
                  class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                />
                {{ t('admin.redeem.batchFields.expiresAt') REDACTEDREDACTED
              </label>
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  ...
                  class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                />
                {{ t('admin.redeem.batchFields.notes') REDACTEDREDACTED
              </label>
              <textarea
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  ...
                  class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                />
                {{ t('admin.redeem.batchFields.group') REDACTEDREDACTED
              </label>
              <Select
            ...
            <div class="flex justify-end gap-3 pt-2">
              ...
              >
                {{ batchUpdating ? t('common.submitting') : t('admin.redeem.batchUpdate') REDACTEDREDACTED
              </button>
            </div>
          </form>
        </div>
        ...
        <div class="relative z-10 w-full max-w-lg rounded-xl bg-white shadow-xl dark:bg-dark-800">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">
                  {{ t('admin.redeem.generatedSuccessfully') REDACTEDREDACTED
                </h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                  {{ t('admin.redeem.codesCreated', { count: generatedCodes.length REDACTED) REDACTEDREDACTED
                </p>
              </div>
            </div>
            <button
          ...
          <div class="p-5">
            <div class="relative">
              <textarea
                ...
                :style="{ height: textareaHeight REDACTED"
          ...
          >
            ...
            >
              ...
              {{ copiedAll ? t('admin.redeem.copied') : t('admin.redeem.copyAll') REDACTEDREDACTED
            </button>
            <button @click="downloadGeneratedCodes" class="btn btn-primary flex items-center gap-2">
              <Icon name="download" size="sm" :stroke-width="2" />
              {{ t('admin.redeem.download') REDACTEDREDACTED
            </button>
          </div>
        </div>
      </div>
...
const batchGroupOptions = computed(() => [
  { value: null, label: t('admin.redeem.clearGroup') REDACTED,
...
const textareaHeight = computed(() => {
  ...
  return `${calculatedHeightREDACTEDpx`
REDACTED)
...
const downloadGeneratedCodes = () => {
  const blob = new Blob([generatedCodesText.value], { type: 'text/plain' REDACTED)
  const url = window.URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = `redeem-codes-${new Date().toISOString().split('T')[0]REDACTED.txt`
...
const columns = computed<Column[]>(() => [
  { key: 'select', label: '' REDACTED,
  { key: 'code', label: t('admin.redeem.columns.code') REDACTED,
  { key: 'type', label: t('admin.redeem.columns.type'), sortable: true REDACTED,
  { key: 'value', label: t('admin.redeem.columns.value'), sortable: true REDACTED,
  { key: 'status', label: t('admin.redeem.columns.status'), sortable: true REDACTED,
  { key: 'used_by', label: t('admin.redeem.columns.usedBy') REDACTED,
  { key: 'used_at', label: t('admin.redeem.columns.usedAt'), sortable: true REDACTED,
  { key: 'expires_at', label: t('admin.redeem.columns.expiresAt'), sortable: true REDACTED,
  { key: 'actions', label: t('admin.redeem.columns.actions') REDACTED
])

const typeOptions = computed(() => [
  { value: 'balance', label: t('admin.redeem.balance') REDACTED,
  { value: 'concurrency', label: t('admin.redeem.concurrency') REDACTED,
  { value: 'subscription', label: t('admin.redeem.subscription') REDACTED,
  { value: 'invitation', label: t('admin.redeem.invitation') REDACTED
])

const filterTypeOptions = computed(() => [
  { value: '', label: t('admin.redeem.allTypes') REDACTED,
  { value: 'balance', label: t('admin.redeem.balance') REDACTED,
  { value: 'concurrency', label: t('admin.redeem.concurrency') REDACTED,
  { value: 'subscription', label: t('admin.redeem.subscription') REDACTED,
  { value: 'invitation', label: t('admin.redeem.invitation') REDACTED
...
const filterStatusOptions = computed(() => [
  ...
  { value: 'disabled', label: t('admin.redeem.status.disabled') REDACTED
])

const batchStatusOptions = computed(() => [
  { value: 'unused', label: t('admin.redeem.status.unused') REDACTED,
  { value: 'disabled', label: t('admin.redeem.status.disabled') REDACTED
])

const batchExpiryModeOptions = computed(() => [
  { value: 'clear', label: t('admin.redeem.neverExpires') REDACTED,
  { value: 'custom', label: t('admin.redeem.customExpiry') REDACTED
])
...
const {
  ...
  toggleVisible
REDACTED = useTableSelection<RedeemCode>({
  rows: codes,
  getId: (code) => code.id
REDACTED)
...
const redeemCodeExpiryOptions = computed<{ value: RedeemCodeExpiryOption; label: string REDACTED[]>(() => [
  { value: 'never', label: t('admin.redeem.neverExpires') REDACTED,
  { value: '1', label: t('admin.redeem.expiryPresetDays', { days: 1 REDACTED) REDACTED,
  { value: '3', label: t('admin.redeem.expiryPresetDays', { days: 3 REDACTED) REDACTED,
  { value: '7', label: t('admin.redeem.expiryPresetDays', { days: 7 REDACTED) REDACTED,
  { value: 'custom', label: t('admin.redeem.customExpiry') REDACTED
])
...
watch(
  ...
  (newType) => {
    ...
    REDACTED else if (generateForm.value === 0) {
...
const toDatetimeLocalInputValue = (date: Date) => {
  ...
  return `${date.getFullYear()REDACTED-${pad(date.getMonth() + 1)REDACTED-${pad(date.getDate())REDACTEDT${pad(
    date.getHours()
  )REDACTED:${pad(date.getMinutes())REDACTED`
REDACTED
...
const buildBatchUpdateFields = (): BatchUpdateRedeemCodeFields | null => {
  const fields: BatchUpdateRedeemCodeFields = {REDACTED
...
const handleExportCodes = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `redeem-codes-${new Date().toISOString().split('T')[0]REDACTED.csv`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    window.URL.revokeObjectURL(url)
...
const confirmDeleteUnused = async () => {
  try {
    ...
    const unusedCodesResponse = await adminAPI.redeem.list(1, 1000, { status: 'unused' REDACTED)
    ...
    appStore.showSuccess(t('admin.redeem.codesDeleted', { count: result.deleted REDACTED))
    showDeleteUnusedDialog.value = false
    loadCodes()
  REDACTED catch (error: any) {
...
const handleBatchUpdate = async () => {
  ...
  try {
    ...
    appStore.showSuccess(t('admin.redeem.batchUpdateSuccess', { count: result.updated REDACTED))
    showBatchUpdateDialog.value = false
    clearSelectedCodes()
    loadCodes()
  REDACTED catch (error: any) {
