    ...
    <div class="space-y-6">
      ...
      <template v-else>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 class="text-2xl font-semibold text-ink">{{ t('admin.riskControl.title') }}</h1>
            <p class="mt-1 text-sm text-ink-secondary">{{ t('admin.riskControl.description') }}</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div
            ...
            :key="item.key"
            class="rounded-lg border border-line-subtle bg-surface px-4 py-3 shadow-sm"
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex min-w-0 items-center justify-between gap-2">
                  <p class="truncate text-xs font-medium text-ink-secondary">{{ item.label }}</p>
                  <span
                    v-if="item.badge"
                    class="inline-flex flex-shrink-0 items-center rounded-sm px-2 py-0.5 text-xs font-medium"
                ...
                <div class="mt-1 flex min-w-0 items-baseline gap-2">
                  <p class="truncate text-xl font-semibold leading-7 text-ink">{{ item.value }}</p>
                  <p v-if="item.meta" class="truncate text-xs text-ink-secondary">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          <div data-test="pre-block-sync-card" class="card">
            <div class="flex flex-col gap-4 border-b border-line-subtle px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-ink">{{ t('admin.riskControl.preBlockSyncStatus') }}</h2>
                <p class="mt-1 text-sm text-ink-secondary">{{ t('admin.riskControl.preBlockSyncHint') }}</p>
              </div>
              <span class="inline-flex w-fit items-center rounded-sm bg-surface-sunken px-2.5 py-1 text-xs font-medium text-ink-secondary">
            ...
            <div class="p-6">
              <div data-test="pre-block-metric-grid" class="grid grid-cols-2 gap-3 md:grid-cols-3">
                <div
                  ...
                  :class="item.class"
                >
                  <p class="text-xs text-ink-secondary">{{ item.label }}</p>
                  <p class="mt-2 truncate text-2xl font-semibold leading-8" :class="item.valueClass">{{ item.value }}</p>
                  <p v-if="item.meta" class="mt-1 truncate text-xs text-ink-secondary">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="p-6">
              ...
              >
                ...
                >
                  <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                      <div class="flex min-w-0 items-center gap-2">
                        ...
                        <span class="truncate font-mono text-sm text-ink-secondary">{{ item.masked || '-' }}</span>
                    ...
                    <div class="grid grid-cols-4 gap-2 text-right text-xs text-ink-secondary sm:min-w-[280px]">
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyActiveShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-info">{{ formatNumber(item.active) }}</p>
                      </div>
                      <div>
                  ...
                  <div class="mt-3 h-1 overflow-hidden bg-surface-sunken">
                    <div class="h-full bg-ink-secondary" :style="{ width: preBlockAPIKeyLoadWidth(item.total) }"></div>
                  </div>
                </div>
              </div>
              <p v-else class="rounded-lg bg-gray-50 p-4 text-sm text-ink-secondary dark:bg-dark-700/50 dark:text-gray-400">
        ...
        <div v-if="showWorkerRuntimeCard" class="card">
          <div class="flex flex-col gap-4 border-b border-line-subtle px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 class="text-lg font-semibold text-ink">{{ t('admin.riskControl.workerStatus') }}</h2>
              <p class="mt-1 text-sm text-ink-secondary">{{ t('admin.riskControl.workerStatusHint') }}</p>
            </div>
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              <div class="rounded-lg border border-line-subtle p-4">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.queueUsage') }}</p>
                    <p class="mt-1 text-xs text-ink-secondary">
                      {{ formatNumber(status?.queue_length ?? 0) }} / {{ formatNumber(status?.queue_size ?? configForm.queue_size) }}
                    </p>
                  </div>
                  <span class="text-sm font-semibold text-ink">{{ queueUsagePercent }}</span>
                </div>
                <div class="mt-4 h-1 overflow-hidden bg-surface-sunken">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                  <p class="text-xs text-ink-secondary">{{ t('admin.riskControl.activeWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-ink">{{ status?.active_workers ?? 0 }}</p>
                </div>
                <div class="rounded-lg border border-success/40 bg-success-tint p-4">
                  <p class="text-xs text-ink-secondary">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-success">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                  <p class="text-xs text-ink-secondary">{{ t('admin.riskControl.processed') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-ink">{{ formatNumber(status?.processed ?? 0) }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                  <p class="text-xs text-ink-secondary">{{ t('admin.riskControl.droppedErrors') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-ink">{{ formatNumber((status?.dropped ?? 0) + (status?.errors ?? 0)) }}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.workerPool') }}</p>
                  <p class="mt-1 text-xs text-ink-secondary">
                ...
                <span class="inline-flex items-center rounded-sm bg-surface-sunken px-2.5 py-1 text-xs font-medium text-ink-secondary">
                  {{ modeLabel(status?.mode ?? configForm.mode) }}
                </span>
              </div>
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-line-subtle px-6 py-4">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-ink">{{ t('admin.riskControl.records') }}</h2>
                <p class="mt-1 text-sm text-ink-secondary">{{ t('admin.riskControl.recordsHint') }}</p>
              </div>
            ...
            <div class="flex flex-col gap-2 rounded-lg border border-line-subtle bg-gray-50 px-3 py-2 dark:bg-dark-900/30 sm:flex-row sm:items-center sm:justify-between">
              <div class="flex min-w-0 items-center gap-2 text-sm text-ink-secondary">
                <Icon name="filter" size="sm" class="flex-shrink-0 text-ink-tertiary" />
                <span class="font-medium">{{ t('admin.riskControl.modelFilter') }}</span>
                <span class="truncate text-ink-secondary">{{ modelFilterSummary }}</span>
              </div>
              <div v-if="modelFilterPreviewModels.length > 0" class="flex flex-wrap gap-1.5">
                <span
                  v-for="model in modelFilterPreviewModels"
                  :key="model"
                  class="inline-flex max-w-[180px] items-center truncate rounded-md bg-surface px-2 py-1 font-mono text-xs text-ink-secondary shadow-sm"
                >
                  {{ model }}
                </span>
                <span v-if="hiddenModelFilterModelCount > 0" class="inline-flex rounded-md bg-surface px-2 py-1 text-xs text-ink-secondary shadow-sm dark:text-gray-400">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-line">
              <thead class="bg-surface-sunken">
                <tr>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.time') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.group') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.user') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.apiKey') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.endpoint') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.result') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.highest') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.actionMeta') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.latency') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink-secondary">{{ t('admin.riskControl.table.input') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 bg-surface dark:divide-dark-800">
                <tr v-if="logsLoading">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-ink-secondary">{{ t('common.loading') }}</td>
                </tr>
                <tr v-else-if="logs.length === 0">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-ink-secondary">{{ t('admin.riskControl.emptyLogs') }}</td>
                </tr>
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-surface-hover/60">
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">{{ formatDateTime(row.created_at) }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">{{ row.group_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">
                      <div>{{ row.user_email || '-' }}</div>
                      <div v-if="row.user_id" class="text-xs text-ink-tertiary">UID {{ row.user_id }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">{{ row.api_key_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">
                      <div>{{ row.endpoint || '-' }}</div>
                      <div class="text-xs text-ink-tertiary">{{ row.provider || '-' }} / {{ row.model || '-' }}</div>
                    </td>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">
                      ...
                      <div class="text-xs text-ink-tertiary">{{ percent(row.highest_score) }}</div>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">
                      ...
                      <button
                        ...
                        type="button"
                        class="mt-2 inline-flex items-center gap-1 rounded-md border border-emerald-200 border border-success/40 bg-success-tint px-2 py-1 text-xs font-medium text-success transition-colors hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-emerald-900/60 dark:hover:bg-emerald-900/30"
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-ink-secondary">
                      ...
                      <div v-if="row.queue_delay_ms !== null && row.queue_delay_ms !== undefined" class="text-xs text-ink-tertiary">
                        {{ t('admin.riskControl.queueDelay', { ms: row.queue_delay_ms }) }}
                      </div>
                    </td>
                    <td class="w-[320px] max-w-sm px-5 py-4 text-sm text-ink-secondary">
                      <button
                        type="button"
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          <div class="flex gap-2 overflow-x-auto border-b border-line-subtle pb-3">
            <button
              v-for="tab in settingsTabs"
              :key="tab.id"
              type="button"
              class="inline-flex whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition-colors"
              :class="activeSettingsTab === tab.id ? 'border border-accent/40 bg-accent-tint text-accent' : 'text-ink-secondary hover:bg-surface-hover hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'"
          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              <div class="flex items-center justify-between rounded-lg border border-line-subtle p-4">
                <div>
                  <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.enabled') }}</p>
                  <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.enabledHint') }}</p>
                </div>
              ...
              <div>
                ...
                <p class="mt-2 text-xs leading-5 text-ink-secondary">{{ modeDescription(configForm.mode) }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="mt-2 text-xs leading-5 text-ink-secondary">{{ t('admin.riskControl.proxyHint') }}</p>
              </div>
            </div>

            <div class="overflow-hidden rounded-xl border border-line-subtle bg-surface shadow-sm">
              <div class="flex flex-col gap-4 border-b border-line-subtle bg-gray-50 px-4 py-4 dark:bg-dark-800/60 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg border border-accent/40 bg-accent-tint text-primary-600 dark:text-primary-300">
                    <Icon name="key" size="md" />
                  </span>
                  <div>
                    <label class="text-sm font-semibold text-ink">{{ t('admin.riskControl.apiKeys') }}</label>
                    <p class="mt-1 max-w-3xl text-xs leading-5 text-ink-secondary">
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-lg border border-line-subtle bg-gray-50 p-2 dark:bg-dark-900/30 sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-xs leading-5 text-ink-secondary">
                      <span class="font-medium text-ink-secondary">{{ t('admin.riskControl.apiKeysWriteMode') }}</span>
                    ...
                    <div class="inline-flex rounded-lg bg-surface p-1 shadow-sm">
                      <button
                        ...
                        :class="configForm.api_keys_mode === 'append' ? 'bg-primary-500 text-white shadow-sm' : 'text-ink-secondary hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700'"
                      ...
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'replace' ? 'bg-warn text-white shadow-sm' : 'text-ink-secondary hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700'"
                        :disabled="configForm.clear_api_key"
                  ...
                  <div class="flex flex-wrap items-center gap-2 text-xs text-ink-secondary">
                    <span class="inline-flex rounded-md bg-surface-sunken px-2 py-1">
                      {{ t('admin.riskControl.inputApiKeyCount', { count: inputApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_key_configured" class="inline-flex rounded-md bg-surface-sunken px-2 py-1">
                      {{ t('admin.riskControl.storedApiKeyCount', { count: configForm.api_key_count }) }}
                    </span>
                    <span v-if="configForm.clear_api_key" class="inline-flex rounded-md border border-danger/40 bg-danger-tint px-2 py-1 text-danger">
                      {{ t('admin.riskControl.apiKeyWillClear') }}
                    </span>
                    <span v-else-if="pendingDeletedApiKeyCount > 0" class="inline-flex rounded-md border border-warn/40 bg-warn-tint px-2 py-1 text-warn">
                      {{ t('admin.riskControl.apiKeyPendingDeleteCount', { count: pendingDeletedApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_keys_mode === 'replace'" class="inline-flex rounded-md border border-warn/40 bg-warn-tint px-2 py-1 text-warn">
                      {{ t('admin.riskControl.apiKeysReplaceWarning') }}
                    </span>
                  </div>

                  <div class="rounded-lg border border-line-subtle bg-gray-50 p-3 dark:bg-dark-900/30" @paste="handleModerationImagePaste">
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-ink">{{ t('admin.riskControl.auditTestInput') }}</p>
                        <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.auditTestInputHint') }}</p>
                      </div>
                      <button
                        v-if="moderationTestPrompt || moderationTestImages.length > 0 || moderationTestResult"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-ink-secondary hover:bg-white hover:text-gray-900 dark:text-gray-400 dark:hover:bg-dark-800 dark:hover:text-white"
                    ...
                    >
                      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-start gap-2">
                          <Icon name="upload" size="md" class="mt-0.5 text-ink-tertiary" />
                          <div>
                            <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.auditTestImages') }}</p>
                            <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.auditTestImagesHint') }}</p>
                          </div>
                        </div>
                      ...
                      <div v-if="moderationTestImages.length > 0" class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <div
                          ...
                          class="group relative aspect-square overflow-hidden rounded-lg border border-line-subtle bg-surface-sunken"
                        >
                ...
                <div class="rounded-lg border border-line-subtle bg-gray-50 p-3 dark:bg-dark-900/30">
                  <div class="mb-3 flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="text-sm font-semibold text-ink">{{ t('admin.riskControl.apiKeyHealth') }}</p>
                      <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.apiKeyFreezeRule') }}</p>
                    </div>
                    <span class="inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-surface px-2 py-0.5 text-[11px] font-medium leading-5 text-ink-secondary shadow-sm">
                  ...
                  <div v-if="apiKeyRows.length === 0" class="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed border-line bg-surface px-4 py-6 text-center">
                    ...
                    <p class="mt-2 text-sm font-medium text-ink-secondary">{{ t('admin.riskControl.apiKeyHealthEmpty') }}</p>
                    <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.apiKeyHealthEmptyHint') }}</p>
                  </div>
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      ...
                      >
                        <div class="flex items-start justify-between gap-2">
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              <span class="truncate font-mono text-sm font-semibold text-ink">{{ row.masked || '-' }}</span>
                              <span
                                class="inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-medium"
                                :class="row.configured ? 'border border-accent/40 bg-accent-tint text-accent' : 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300'"
                              >
                            ...
                            <p class="mt-1 text-xs leading-5 text-ink-secondary">{{ apiKeyStatusMeta(row) }}</p>
                          </div>
                          <div class="flex flex-shrink-0 items-center gap-1.5">
                            <span class="inline-flex items-center gap-1.5 whitespace-nowrap rounded-sm px-2 py-0.5 text-xs font-medium" :class="apiKeyStatusBadgeClass(row.status)">
                            ...
                            <button
                              ...
                              type="button"
                              class="inline-flex h-7 w-7 items-center justify-center rounded-md text-ink-tertiary transition-colors hover:bg-surface-hover hover:text-gray-700 dark:hover:text-gray-200"
                        ...
                        <p v-if="row.last_error" class="mt-1.5 rounded-md border border-warn/40 bg-warn-tint px-2 py-1.5 text-xs leading-5 text-warn">
                          {{ row.last_error }}
                        </p>
                      </div>
                    </div>

                    <div v-if="canToggleApiKeyRows" class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-line bg-surface px-3 py-2 text-xs text-ink-secondary dark:text-gray-400">
                      ...
                      <button
                        type="button"
                        class="inline-flex shrink-0 items-center gap-1 rounded-md px-2 py-1 font-medium text-primary-600 transition-colors hover:bg-primary-50 hover:text-accent dark:hover:bg-primary-900/20"
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-line-subtle bg-surface p-3">
                    <div class="flex items-start justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-ink">{{ t('admin.riskControl.auditTestResult') }}</p>
                        <p class="mt-1 text-xs text-ink-secondary">
                      ...
                      <span class="inline-flex rounded-sm px-2 py-1 text-xs font-medium" :class="moderationTestResult.flagged ? 'border border-danger/40 bg-danger-tint text-danger' : 'border border-success/40 bg-success-tint text-success'">
                        {{ moderationTestResult.flagged ? t('admin.riskControl.auditTestFlagged') : t('admin.riskControl.auditTestPassed') }}
                      </span>
                    </div>
                    <div class="mt-3">
                      <div class="mb-2 flex items-center justify-between text-xs text-ink-secondary">
                        <span>{{ t('admin.riskControl.auditTestComposite') }}</span>
                        <span class="font-semibold text-ink">{{ percent(moderationTestResult.composite_score) }}</span>
                      </div>
                      <div class="h-1 overflow-hidden bg-surface-sunken">
                    ...
                    <div class="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
                      <div v-for="score in moderationScoreRows" :key="score.category">
                        <div class="mb-1 flex items-center justify-between gap-3 text-xs">
                          <span class="truncate text-ink-secondary">{{ score.category }}</span>
                          <span class="font-mono text-ink-secondary">{{ percent(score.score) }} / {{ percent(score.threshold) }}</span>
                        </div>
                        <div class="h-1 overflow-hidden bg-surface-sunken-sunken">
                          <div class="h-full rounded-full" :class="score.hit ? 'bg-danger' : 'bg-primary-500'" :style="{ width: percentWidth(score.score) }"></div>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div v-else-if="activeSettingsTab === 'scope'" class="space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-ink">{{ t('admin.riskControl.groupScope') }}</h3>
                <p class="mt-1 text-sm text-ink-secondary">{{ t('admin.riskControl.groupScopeHint') }}</p>
              </div>
              <div class="inline-flex rounded-lg bg-surface-sunken p-1">
                <button
                  type="button"
                  class="rounded-md px-3 py-1.5 text-sm font-medium transition-colors"
                  :class="configForm.all_groups ? 'bg-white text-ink shadow-sm dark:bg-dark-800 dark:text-white' : 'text-ink-secondary'"
            ...
            <div v-if="!configForm.all_groups" class="space-y-4">
              <div class="relative">
                <Icon name="search" size="sm" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink-tertiary" />
              ...
              <div class="grid max-h-[420px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
                <button
                  ...
                  :class="isGroupSelected(group.id) ? 'border-primary-300 border border-accent/40 bg-accent-tint dark:border-primary-700' : 'border-gray-100 hover:bg-gray-50 dark:border-dark-700 dark:hover:bg-dark-700/60'"
                  @click="toggleGroup(group.id)"
                >
                  <span class="min-w-0">
                    <span class="block truncate text-sm font-semibold text-ink">{{ group.name }}</span>
                    <span class="mt-1 inline-flex rounded-md bg-surface-sunken px-2 py-0.5 text-xs text-ink-secondary dark:text-gray-400">{{ group.platform }}</span>
                  </span>
                  <span
                ...
                <p v-if="filteredGroups.length === 0" class="text-sm text-ink-secondary">{{ t('admin.riskControl.noGroups') }}</p>
              </div>
            </div>

            <div class="space-y-4 rounded-lg border border-line-subtle p-4">
              <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h3 class="text-base font-semibold text-ink">{{ t('admin.riskControl.modelFilter') }}</h3>
                  <p class="mt-1 text-sm text-ink-secondary">{{ t('admin.riskControl.modelFilterHint') }}</p>
                </div>
                <span class="inline-flex w-fit rounded-md bg-surface-sunken px-2.5 py-1 text-xs font-medium text-ink-secondary">
                  {{ modelFilterSummary }}
                </span>
              </div>
              ...
              <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                <button
                  ...
                  :class="configForm.model_filter_type === option.value
                    ? 'border-primary-300 border border-accent/40 bg-accent-tint text-primary-900 shadow-sm dark:border-primary-700 dark:text-primary-100'
                ...
                >
                  ...
                  <p class="mt-1 text-xs leading-5 text-ink-secondary">{{ option.description }}</p>
                </button>
              </div>
          ...
          <div v-else-if="activeSettingsTab === 'runtime'" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="flex items-center justify-between rounded-lg border border-line-subtle p-4 lg:col-span-2">
              <div>
                <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.recordNonHits') }}</p>
                <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.recordNonHitsHint') }}</p>
              </div>
              <Toggle v-model="configForm.record_non_hits" />
            </div>
            <div class="space-y-4 rounded-lg border border-line-subtle p-4 lg:col-span-2">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.preHashCheck') }}</p>
                  <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.preHashCheckHint') }}</p>
                </div>
              ...
              <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-900/30">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.flaggedHashHint') }}</p>
                  </div>
                  <button
                    type="button"
                    class="btn btn-secondary inline-flex items-center justify-center gap-2 text-red-600 hover:text-danger"
          ...
          <div v-else-if="activeSettingsTab === 'response'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div class="flex items-center justify-between rounded-lg border border-line-subtle p-4">
                <div>
                  <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.emailOnHit') }}</p>
                  <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.emailOnHitHint') }}</p>
                </div>
                <Toggle v-model="configForm.email_on_hit" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-line-subtle p-4">
                <div>
                  <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.autoBan') }}</p>
                  <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.autoBanHint') }}</p>
                </div>
                <Toggle v-model="configForm.auto_ban_enabled" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-line-subtle p-4 lg:col-span-2">
                <div>
                  <p class="text-sm font-medium text-ink">{{ t('admin.riskControl.cyberPolicyExcludeBan') }}</p>
                  <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.riskControl.cyberPolicyExcludeBanHint') }}</p>
                </div>
          ...
          <div v-else-if="activeSettingsTab === 'riskThresholds'" class="space-y-5">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-ink">{{ t('admin.riskControl.riskThresholds') }}</h3>
                <p class="mt-1 text-sm text-ink-secondary">{{ t('admin.riskControl.riskThresholdsHint') }}</p>
              </div>
              <button
                type="button"
                class="btn btn-secondary inline-flex items-center justify-center gap-2"
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              <div
                ...
                :key="row.category"
                class="rounded-lg border border-line-subtle bg-gray-50 p-4 dark:bg-dark-900/30"
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <label class="block truncate text-sm font-semibold text-ink" :for="`risk-threshold-${row.category}`">
                      {{ row.category }}
                    </label>
                    <p class="mt-1 text-xs text-ink-secondary">
                  ...
                  <span class="inline-flex shrink-0 rounded-md bg-surface px-2 py-1 font-mono text-xs font-medium text-ink-secondary shadow-sm">
          ...
          <div v-else-if="activeSettingsTab === 'keywords'" class="space-y-5">
            ...
            >
              ...
              <div class="text-sm leading-6">
                ...
                <p class="mt-1 text-xs text-ink-secondary">{{ keywordNotice.description }}</p>
              </div>
            </div>
            ...
            <div>
              <div class="mb-2 flex items-center justify-between">
                ...
                <span class="inline-flex rounded-md bg-surface-sunken px-2 py-1 text-xs text-ink-secondary">
          ...
          <div v-else class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="rounded-lg border border-line-subtle p-4 text-sm text-ink-secondary dark:text-gray-400 lg:col-span-2">
              <div class="flex flex-wrap items-center gap-3">
                <Icon name="database" size="md" class="text-ink-tertiary" />
      ...
      >
        <div v-if="inputDetailRow" class="space-y-5">
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-line-subtle bg-gray-50 p-4 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-ink-secondary">{{ t('admin.riskControl.table.time') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-ink">{{ formatDateTime(inputDetailRow.created_at) }}</p>
            </div>
            <div class="rounded-lg border border-line-subtle bg-gray-50 p-4 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-ink-secondary">{{ t('admin.riskControl.table.user') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-ink">{{ inputDetailRow.user_email || '-' }}</p>
            </div>
            <div class="rounded-lg border border-line-subtle bg-gray-50 p-4 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-ink-secondary">{{ t('admin.riskControl.table.result') }}</p>
            ...
            <div class="rounded-lg border border-line-subtle bg-gray-50 p-4 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-ink-secondary">{{ t('admin.riskControl.table.highest') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-ink">
                {{ inputDetailRow.highest_category || '-' }} / {{ percent(inputDetailRow.highest_score) }}
              </p>
            </div>
            <div v-if="inputDetailRow.matched_keyword" class="rounded-lg border border-red-100 border border-danger/40 bg-danger-tint p-4 dark:border-red-900/60">
              <p class="text-xs font-medium text-red-500 dark:text-red-300">{{ t('admin.riskControl.matchedKeyword') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-danger" :title="inputDetailRow.matched_keyword">{{ inputDetailRow.matched_keyword }}</p>
            </div>
          </div>

          <div class="rounded-xl border border-line-subtle bg-surface p-4 shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-semibold text-ink">{{ t('admin.riskControl.inputDetailContent') }}</p>
                <p class="mt-1 text-xs text-ink-secondary">
                  {{ inputDetailRow.endpoint || '-' }} · {{ inputDetailRow.provider || '-' }} / {{ inputDetailRow.model || '-' }}
                </p>
              </div>
              <span v-if="inputDetailRow.group_name" class="inline-flex rounded-md border border-info/40 bg-info-tint px-2.5 py-1 text-xs font-medium text-info">
...
const keywordNoticeTones = {
  info: {
    ...
    toneClass: 'border-primary-100 border border-accent/40 bg-accent-tint/60 dark:border-primary-900/40',
    iconClass: 'mt-0.5 flex-shrink-0 text-primary-500 dark:text-primary-300',
    titleClass: 'text-accent',
  },
  warning: {
    icon: 'exclamationTriangle' as const,
    toneClass: 'border-amber-200 border border-warn/40 bg-warn-tint dark:border-amber-900/40',
...
const overviewItems = computed<OverviewItem[]>(() => [
  {
    ...
    iconClass: configForm.enabled
      ? 'border border-success/40 bg-success-tint text-emerald-600 dark:text-emerald-300'
      : 'bg-gray-100 text-ink-secondary dark:bg-dark-700 dark:text-gray-400',
  ...
  },
  {
    ...
    icon: 'key',
    iconClass: 'border border-info/40 bg-info-tint text-sky-600 dark:text-sky-300',
  },
  {
    key: 'scope',
  ...
  },
  {
    ...
    icon: 'document',
    iconClass: 'border border-warn/40 bg-warn-tint text-amber-600 dark:text-amber-300',
  },
])
...
const preBlockMetricItems = computed(() => [
  {
    ...
    class: 'border border-info/40 bg-info-tint',
  ...
  },
  {
    ...
    class: 'border border-success/40 bg-success-tint',
  ...
  },
  {
    ...
    class: 'border border-danger/40 bg-danger-tint',
  ...
  },
  {
    ...
    class: 'border border-warn/40 bg-warn-tint',
...
const runtimeBadgeClass = computed(() => {
  ...
  return 'border border-success/40 bg-success-tint text-success'
})
...
function resultBadgeClass(row: ContentModerationLog): string {
  if (row.action === 'block' || row.action === 'keyword_block' || row.action === 'cyber_policy') return 'bg-red-100 text-danger dark:bg-red-900/30'
  if (row.action === 'error' || row.error) return 'bg-amber-100 text-warn dark:bg-amber-900/30'
  if (row.flagged) return 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300'
  return 'bg-green-100 text-success dark:bg-green-900/30'
}

function workerSlotClass(state: WorkerSlotState): string {
  if (state === 'active') {
    return 'border-sky-200 border border-info/40 bg-info-tint text-info dark:border-sky-900/60'
  }
  if (state === 'idle') {
    return 'border-emerald-200 border border-success/40 bg-success-tint text-success dark:border-emerald-900/60'
  }
  return 'border-gray-100 bg-white text-ink-tertiary dark:border-dark-700 dark:bg-dark-800 dark:text-gray-500'
}
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ok: 'border border-success/40 bg-success-tint text-success',
    error: 'border border-warn/40 bg-warn-tint text-warn',
    frozen: 'border border-danger/40 bg-danger-tint text-danger',
    unknown: 'bg-gray-100 text-ink-secondary dark:bg-dark-700 dark:text-gray-300',
  }
