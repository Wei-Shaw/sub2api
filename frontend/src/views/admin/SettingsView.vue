  ...
  <AppLayout>
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <!-- Tab: Email -->
        <!-- Tab: Payment (moved to plugin) -->
        <div v-show="activeTab === 'payment'" class="card p-6">
          <p class="mb-4 text-sm text-gray-700 dark:text-gray-300">
            {{ t('admin.settings.payment.movedToPlugin') }}
          </p>
          <button
            type="button"
            class="btn-primary"
            @click="$router.push('/admin/plugins/payment')"
          >
            {{ t('admin.settings.payment.openPluginSettings') }}
          </button>
        </div>
