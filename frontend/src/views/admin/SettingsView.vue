          ...
          <div class="space-y-5 p-6">
            ...
            <template v-if="form.payment_enabled">
              ...
              <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div>
                  <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ tOr('admin.settings.payment.balanceRechargeMultiplier', 'Balance Recharge Multiplier') }}
                  </label>
                  <input
                    v-model.number="form.payment_balance_recharge_multiplier"
                    type="number"
                    min="0.01"
                    step="0.01"
                    class="input"
                  />
                  <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ tOr('admin.settings.payment.balanceRechargeMultiplierHint', 'Recharge 1 USD credits how much balance.') }}
                  </p>
                ...
                <div>
                  <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ tOr('admin.settings.payment.rechargeFeeRate', 'Recharge Fee Rate') }}
                  </label>
                  <div class="relative">
                    <input
                  ...
                  <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ tOr('admin.settings.payment.rechargeFeeRateHint', 'Additional fee percentage charged on recharge orders.') }}
                  </p>
                  <p v-if="(Number(form.payment_recharge_fee_rate) || 0) > 0" class="mt-1 text-xs font-medium text-primary-600 dark:text-primary-400">
...
function togglePaymentType(type: PaymentProviderType) {
  if (form.payment_enabled_types.includes(type)) {
	form.payment_enabled_types = form.payment_enabled_types.filter((item): item is PaymentProviderType => item !== type)
	return
  }
...
async function handleToggleProviderField(provider: ProviderInstance, field: 'enabled' | 'refund_enabled' | 'allow_user_refund') {
	providerInstancesSaving.value = true
	try {
		...
		if (field === 'refund_enabled' && payload.refund_enabled === false) {
...
async function saveSettings() {
  saving.value = true
  try {
    form.balance_low_notify_recharge_url = form.balance_low_notify_recharge_url || currentOrigin
