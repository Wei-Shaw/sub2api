          ...
          <template #cell-expires_at="{ value REDACTED">
            <div v-if="value">
              <span
                ...
                "
              >
                {{ formatDateTimeToMinute(value) REDACTEDREDACTED
              </span>
              <template
              ...
              >
                <div v-if="remainingExpiry" class="text-xs text-gray-500">
                  {{ remainingExpiry REDACTEDREDACTED
                </div>
              </template>
            </div>
...
import { formatDateTimeToMinute REDACTED from '@/utils/format'
...
const formatRemainingExpiry = (expiresAt: string): string | null => {
  ...
  if (duration.unit === 'days') {
    return t('admin.subscriptions.daysRemaining', { days: duration.days REDACTED)
  REDACTED
  if (duration.hours) {
    ...
    REDACTED)
  REDACTED
  return t('admin.subscriptions.minutesRemaining', { minutes: duration.minutes REDACTED)
REDACTED
