...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.rate'), t('usage.imageGenerationCost'), t('usage.imageRate'), t('usage.imageActualCost'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.rate_multiplier?.toPrecision(4) || '1.00', log.image_generation_cost?.toFixed(6) || '0.000000',
        log.image_rate_multiplier?.toPrecision(4) || '0.00', log.image_actual_cost?.toFixed(6) || '0.000000',
