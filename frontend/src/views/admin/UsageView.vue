...
import type { UsageLog, TrendDataPoint, ModelStat REDACTED from '@/types'; import type { AdminUsageStatsResponse, AdminUsageQueryParams REDACTED from '@/api/admin/usage'
...
const loadLogs = async () => {
  ...
  REDACTED catch {REDACTED finally { if(abortController === c) loading.value = false REDACTED
REDACTED
const loadStats = async () => { try { const s = await adminAPI.usage.getStats(filters.value); usageStats.value = s REDACTED catch {REDACTED REDACTED
const loadChartData = async () => {
  chartsLoading.value = true
  try {
    const params = { start_date: filters.value.start_date || startDate.value, end_date: filters.value.end_date || endDate.value, granularity: granularity.value, user_id: filters.value.user_id REDACTED
    const [trendRes, modelRes] = await Promise.all([adminAPI.dashboard.getUsageTrend(params), adminAPI.dashboard.getModelStats({ start_date: params.start_date, end_date: params.end_date, user_id: params.user_id REDACTED)])
    trendData.value = trendRes.trend || []; modelStats.value = modelRes.models || []
  REDACTED catch {REDACTED finally { chartsLoading.value = false REDACTED
REDACTED
const applyFilters = () => { pagination.page = 1; loadLogs(); loadStats(); loadChartData() REDACTED
const resetFilters = () => { startDate.value = formatLD(weekAgo); endDate.value = formatLD(now); filters.value = { start_date: startDate.value, end_date: endDate.value REDACTED; granularity.value = 'day'; applyFilters() REDACTED
...
onMounted(() => { loadLogs(); loadStats(); loadChartData() REDACTED)
