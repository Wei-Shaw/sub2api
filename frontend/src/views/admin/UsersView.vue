  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="card overflow-hidden">
        <DataTable :columns="columns" :data="users" :loading="loading">
          ...
          <template #cell-wechat="{ value REDACTED">
            <span class="text-sm text-gray-700 dark:text-gray-300">{{ value || '-' REDACTEDREDACTED</span>
          </template>
          ...
          <template #cell-created_at="{ value REDACTED">
            <span class="text-sm text-gray-500 dark:text-dark-400">{{ formatDate(value) REDACTEDREDACTED</span>
          </template>
    ...
    >
      <form @submit.prevent="handleCreateUser" class="space-y-5">
        ...
        <div>
          <label class="input-label">{{ t('admin.users.wechat') REDACTEDREDACTED</label>
          <input
    ...
    >
      <form v-if="editingUser" @submit.prevent="handleUpdateUser" class="space-y-5">
        ...
        <div>
          <label class="input-label">{{ t('admin.users.password') REDACTEDREDACTED</label>
          <p class="text-xs text-gray-500 dark:text-dark-400 mb-1">{{ t('admin.users.leaveEmptyToKeep') REDACTEDREDACTED</p>
          <div class="flex gap-2">
            <div class="relative flex-1">
              <input
    ...
    >
      <div v-if="viewingUser" class="space-y-4">
        ...
        <div v-else-if="userApiKeys.length === 0" class="text-center py-8">
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">{{ t('admin.users.noApiKeys') REDACTEDREDACTED</p>
        </div>
          ...
          >
            ...
            <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-500 dark:text-dark-400">
              <div class="flex items-center gap-1">
                ...
                <span>{{ t('admin.users.group') REDACTEDREDACTED: {{ key.group?.name || t('admin.users.none') REDACTEDREDACTED</span>
              </div>
              <div class="flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
                </svg>
                <span>{{ t('admin.users.columns.created') REDACTEDREDACTED: {{ formatDate(key.created_at) REDACTEDREDACTED</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="allowedGroupsUser" class="space-y-4">
        ...
        <div v-else>
          ...
          <div v-if="standardGroups.length === 0" class="text-center py-6">
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">{{ t('admin.users.noStandardGroups') REDACTEDREDACTED</p>
          </div>
          ...
          <div v-else class="space-y-2 max-h-64 overflow-y-auto">
            <label
              ...
              :class="{ 'bg-primary-50 dark:bg-primary-900/20 border-primary-300 dark:border-primary-700': selectedGroupIds.includes(group.id) REDACTED"
            >
              <input
                type="checkbox"
                :value="group.id"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p v-if="group.description" class="text-sm text-gray-500 dark:text-dark-400 truncate">{{ group.description REDACTEDREDACTED</p>
              </div>
              <div class="flex items-center gap-2">
                <span class="badge badge-gray text-xs">{{ group.platform REDACTEDREDACTED</span>
                <span v-if="group.is_exclusive" class="badge badge-purple text-xs">{{ t('admin.groups.exclusive') REDACTEDREDACTED</span>
              </div>
            </label>
          </div>
          ...
          <div class="mt-4 pt-4 border-t border-gray-200 dark:border-dark-600">
            <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-dark-600 hover:bg-gray-50 dark:hover:bg-dark-700 cursor-pointer transition-colors"
              :class="{ 'bg-green-50 dark:bg-green-900/20 border-green-300 dark:border-green-700': selectedGroupIds.length === 0 REDACTED"
            >
              <input
                type="radio"
              ...
              />
              <div class="flex-1">
                <p class="font-medium text-gray-900 dark:text-white">{{ t('admin.users.allowAllGroups') REDACTEDREDACTED</p>
                <p class="text-sm text-gray-500 dark:text-dark-400">{{ t('admin.users.allowAllGroupsHint') REDACTEDREDACTED</p>
              </div>
            </label>
          </div>
        </div>
      ...
      <form v-if="balanceUser" @submit.prevent="handleBalanceSubmit" class="space-y-5">
        ...
        <div>
          <label class="input-label">
            {{ balanceOperation === 'add' ? t('admin.users.depositAmount') : t('admin.users.withdrawAmount') REDACTEDREDACTED
          </label>
          <div class="relative">
...
const columns = computed<Column[]>(() => [
  ...
  { key: 'wechat', label: t('admin.users.columns.wechat'), sortable: false REDACTED,
