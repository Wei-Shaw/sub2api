  ...
  <AppLayout>
    <TablePageLayout>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            <div v-if="visibleFilters.has('role')" class="w-full sm:w-32">
              <Select
                ...
                :options="[
                  { value: '', label: t('admin.users.allRoles') REDACTED,
                  { value: 'admin', label: t('admin.users.admin') REDACTED,
                  { value: 'user', label: t('admin.users.user') REDACTED
                ]"
                @change="applyFilter"
              />
            </div>
            ...
            <div v-if="visibleFilters.has('status')" class="w-full sm:w-32">
              <Select
                ...
                :options="[
                  { value: '', label: t('admin.users.allStatus') REDACTED,
                  { value: 'active', label: t('common.active') REDACTED,
                  { value: 'disabled', label: t('admin.users.disabled') REDACTED
                ]"
                @change="applyFilter"
              />
            </div>
            ...
            <template v-for="(value, attrId) in activeAttributeFilters" :key="attrId">
              <div
                v-if="visibleFilters.has(`attr_${attrIdREDACTED`)"
                ...
                <template v-else-if="['select', 'multi_select'].includes(getAttributeDefinition(Number(attrId))?.type || '')">
                  <div class="w-full">
                    <Select
                      :model-value="value"
                      :options="[
                        { value: '', label: getAttributeDefinitionName(Number(attrId)) REDACTED,
                        ...(getAttributeDefinition(Number(attrId))?.options || [])
                      ]"
                      @update:model-value="(val) => { updateAttributeFilter(Number(attrId), String(val ?? '')); applyFilter() REDACTED"
                    />
                  </div>
                </template>
                <!-- Fallback -->
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                >
                  ...
                  <span class="hidden md:inline">{{ t('admin.users.filterSettings') REDACTEDREDACTED</span>
                </button>
                <!-- Dropdown menu -->
                <div
                ...
                >
                  ...
                  >
                    <span>{{ filter.name REDACTEDREDACTED</span>
                    <Icon
                  ...
                  <button
                    ...
                    class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
                  >
                    <span>{{ attr.name REDACTEDREDACTED</span>
                    <Icon
                      v-if="visibleFilters.has(`attr_${attr.idREDACTED`)"
                      name="check"
                      size="sm"
                      class="text-primary-500"
                      :stroke-width="2"
              ...
              >
                ...
                <span class="hidden md:inline">{{ t('admin.users.attributes.configButton') REDACTEDREDACTED</span>
              </button>
            </div>
            ...
            >
              ...
              {{ t('admin.users.bulkLimits.action', { count: selectedCount REDACTED) REDACTEDREDACTED
            </button>

            <!-- Create User Button (full width on mobile, auto width on desktop) -->
            <button @click="showCreateModal = true" class="btn btn-primary flex-1 md:flex-initial">
              <Icon name="plus" size="md" class="mr-2" />
              {{ t('admin.users.createUser') REDACTEDREDACTED
            </button>
          </div>
        </div>
      </template>
      ...
      <template #table>
        ...
        >
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <span class="text-sm font-medium text-primary-700 dark:text-primary-300">
                  {{ value.charAt(0).toUpperCase() REDACTEDREDACTED
                </span>
              </div>
          ...
          <template #cell-username="{ value REDACTED">
          ...
          <template #cell-notes="{ value REDACTED">
            <div class="max-w-xs">
              ...
              >
                {{ value.length > 30 ? value.substring(0, 25) + '...' : value REDACTEDREDACTED
              </span>
          ...
          <template
            ...
            :key="def.id"
            #[`cell-attr_${def.idREDACTED`]="{ row REDACTED"
          >
            <div class="max-w-xs">
              <span
              ...
              >
                {{ getAttributeValue(row.id, def.id) REDACTEDREDACTED
              </span>
            </div>
          </template>

          <template #cell-role="{ value REDACTED">
            <span :class="['badge', value === 'admin' ? 'badge-purple' : 'badge-gray']">
              {{ t('admin.users.roles.' + value) REDACTEDREDACTED
            </span>
          </template>
          ...
          <template #cell-groups="{ row REDACTED">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                ...
                <span class="font-medium text-purple-600 dark:text-purple-400">{{ getUserGroups(row).exclusive.length REDACTEDREDACTED</span>
                <span class="text-gray-500 dark:text-dark-400">{{ t('admin.users.exclusiveLabel') REDACTEDREDACTED</span>
                ...
                >
                  ...
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
                    <span v-for="g in getUserGroups(row).exclusive" :key="g.id">{{ g.name REDACTEDREDACTED</span>
                  </div>
                </div>
                <!-- 点击展开分组操作菜单 -->
                <div
                ...
                >
                  <div class="border-b border-gray-100 px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-gray-400 dark:border-dark-600 dark:text-dark-400">
                    {{ t('admin.users.clickToReplace') REDACTEDREDACTED
                  </div>
                  <div
                  ...
                  >
                    ...
                    <span class="flex-1">{{ g.name REDACTEDREDACTED</span>
                  </div>
                </div>
              </span>
              <!-- 公开分组行 -->
              ...
              >
                ...
                <span class="font-medium text-gray-600 dark:text-dark-300">{{ getUserGroups(row).publicGroups.length REDACTEDREDACTED</span>
                <span class="text-gray-400 dark:text-dark-500">{{ t('admin.users.publicLabel') REDACTEDREDACTED</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-gray-900 px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100 dark:bg-dark-600">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-gray-900 dark:border-b-dark-600"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
                    <span v-for="g in getUserGroups(row).publicGroups" :key="g.id">{{ g.name REDACTEDREDACTED</span>
                  </div>
                </div>
              </span>
              <!-- 都没有 -->
          ...
          <template #cell-subscriptions="{ row REDACTED">
            <div
            ...
            >
              ...
              <span>{{ t('admin.users.noSubscription') REDACTEDREDACTED</span>
            </span>
          </template>

          <template #cell-balance="{ value, row REDACTED">
            <div class="flex items-center gap-2">
              <div class="group relative">
                ...
                >
                  ${{ value.toFixed(2) REDACTEDREDACTED
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-gray-900 px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100 dark:bg-dark-600">
                  {{ t('admin.users.balanceHistoryTip') REDACTEDREDACTED
              ...
              >
                {{ t('admin.users.deposit') REDACTEDREDACTED
              </button>
            </div>
          </template>

          <template #cell-balance_platform_quota="{ row REDACTED">
            <button
              type="button"
          ...
          <template
            ...
            :key="usageKey"
            #[`header-${usageKeyREDACTED`]="{ column REDACTED"
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="usage-sort-trigger relative">
                <button
                  ...
                  :data-test="`usage-sort-trigger-${usageKeyREDACTED`"
                ...
                >
                  ...
                  >{{ usageSort.metric === 'today' ? t('admin.users.today') : t('admin.users.total') REDACTEDREDACTED</span>
                  <svg
                    v-if="usageSort && usageSort.key === usageKey"
                    class="h-3.5 w-3.5"
                    :class="{ 'rotate-180': usageSort.order === 'desc' REDACTED"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                ...
                >
                  <button
                    ...
                    :data-test="`usage-sort-${usageKeyREDACTED-${metricREDACTED`"
                    @click.stop="toggleUsageSort(usageKey, metric)"
                  >
                    <span>{{ metric === 'today' ? t('admin.users.today') : t('admin.users.total') REDACTEDREDACTED</span>
                    <svg
                      v-if="getUsageSortOrder(usageKey, metric)"
                      class="h-3 w-3"
                      :class="{ 'rotate-180': getUsageSortOrder(usageKey, metric) === 'desc' REDACTED"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
                  ...
                  <div class="mt-1 border-t border-gray-100 px-3 py-1 text-[10px] normal-case tracking-normal text-gray-400 dark:border-dark-700 dark:text-dark-500">
                    {{ t('admin.users.sortCurrentPageOnly') REDACTEDREDACTED
                  </div>
                </div>
              </div>
            </div>
          ...
          <template #cell-usage_anthropic="{ row REDACTED">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'anthropic')" />
          </template>

          <template #cell-usage_openai="{ row REDACTED">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'openai')" />
          </template>

          <template #cell-usage_gemini="{ row REDACTED">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'gemini')" />
          </template>

          <template #cell-usage_antigravity="{ row REDACTED">
          ...
          <template #cell-status="{ value REDACTED">
            <div class="flex items-center gap-1.5">
              ...
              <span class="text-sm text-gray-700 dark:text-gray-300">
                {{ value === 'active' ? t('common.active') : t('admin.users.disabled') REDACTEDREDACTED
              </span>
            </div>
          </template>
          ...
          <template #cell-last_used_at="{ value REDACTED">
            <span class="text-sm text-gray-500 dark:text-dark-400">
              {{ value ? formatDateTime(value) : '-' REDACTEDREDACTED
            </span>
          </template>

          <template #cell-last_active_at="{ value REDACTED">
            <span class="text-sm text-gray-500 dark:text-dark-400">
              {{ value ? formatDateTime(value) : '-' REDACTEDREDACTED
            </span>
          </template>
          ...
          <template #cell-actions="{ row REDACTED">
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <span class="text-xs">{{ row.status === 'active' ? t('admin.users.disable') : t('admin.users.enable') REDACTEDREDACTED</span>
              </button>
    ...
    <Teleport to="body">
      <div
        ...
        :style="{ top: menuPosition.top + 'px', left: menuPosition.left + 'px' REDACTED"
      >
        <div class="py-1">
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              >
                ...
                {{ t('admin.users.apiKeys') REDACTEDREDACTED
              </button>
              ...
              >
                ...
                {{ t('admin.users.withdraw') REDACTEDREDACTED
              </button>
              ...
              >
                ...
                {{ t('admin.users.platformQuota.menuItem') REDACTEDREDACTED
              </button>
              ...
              >
                ...
                {{ t('admin.users.balanceHistory') REDACTEDREDACTED
              </button>
...
import type { AdminUser, AdminGroup, UserAttributeDefinition REDACTED from '@/types'
...
const allColumns = computed<Column[]>(() => [
  ...
  { key: 'id', label: t('admin.users.columns.id'), sortable: true REDACTED,
  ...
  { key: 'groups', label: t('admin.users.columns.groups'), sortable: false REDACTED,
  { key: 'subscriptions', label: t('admin.users.columns.subscriptions'), sortable: false REDACTED,
  { key: 'balance', label: t('admin.users.columns.balance'), sortable: true REDACTED,
  { key: 'balance_platform_quota', label: t('admin.users.columns.balancePlatformQuota'), sortable: false REDACTED,
  { key: 'usage', label: t('admin.users.columns.usage'), sortable: false REDACTED,
  { key: 'usage_anthropic', label: t('admin.users.columns.usageAnthropic'), sortable: false REDACTED,
  { key: 'usage_openai', label: t('admin.users.columns.usageOpenAI'), sortable: false REDACTED,
  { key: 'usage_gemini', label: t('admin.users.columns.usageGemini'), sortable: false REDACTED,
  { key: 'usage_antigravity', label: t('admin.users.columns.usageAntigravity'), sortable: false REDACTED,
...
const hasVisibleAttributeColumns = computed(() =>
  attributeDefinitions.value.some((def) => def.enabled && !hiddenColumns.has(`attr_${def.idREDACTED`))
)
...
const loadInitialSortState = (): { sort_by: string; sort_order: 'asc' | 'desc' REDACTED => {
...
const getUserGroups = (user: AdminUser) => {
  ...
  for (const g of allGroups.value) {
    ...
    REDACTED
  REDACTED
  return { exclusive, publicGroups REDACTED
REDACTED

// Group filter options: "All Groups" + active exclusive groups (value = group name for fuzzy match)
const groupFilterOptions = computed(() => {
  const options: { value: string; label: string REDACTED[] = [
    { value: '', label: t('admin.users.allAuthorizedGroups') REDACTED
  ]
  for (const g of allGroups.value) {
    if (g.status !== 'active' || !g.is_exclusive || g.subscription_type !== 'standard') continue
    options.push({ value: g.name, label: g.name REDACTED)
  REDACTED
  return options
REDACTED)
...
REDACTED)
const activeAttributeFilters = reactive<Record<number, string>>({REDACTED)

// Visible filters tracking (which filters are shown in the UI)
// Keys: 'role', 'status', 'attr_${idREDACTED'
...
const builtInFilters = computed(() => [
  { key: 'role', name: t('admin.users.columns.role'), type: 'select' as const REDACTED,
  { key: 'status', name: t('admin.users.columns.status'), type: 'select' as const REDACTED,
  { key: 'group', name: t('admin.users.authorizedGroupFilter'), type: 'select' as const REDACTED,
  { key: 'apiKeyGroup', name: t('admin.users.apiKeyGroupFilter'), type: 'select' as const REDACTED
])
...
REDACTED
const usageStats = ref<Record<string, BatchUserUsageStats>>({REDACTED)
const platformQuotaStats = ref<Record<number, PlatformQuotaItem[]>>({REDACTED)
...
type UsageSortState = { key: string; metric: UsageMetric; order: 'asc' | 'desc' REDACTED | null
...
const loadInitialUsageSort = (): UsageSortState => {
  try {
    ...
    if (!raw) return null
    const parsed = JSON.parse(raw) as Partial<{ key: string; metric: string; order: string REDACTED>
    ...
    return { key: parsed.key, metric, order REDACTED
  REDACTED catch {
    return null
  REDACTED
REDACTED
...
const toggleUsageSort = (key: string, metric: UsageMetric) => {
  ...
  if (cur && cur.key === key && cur.metric === metric) {
    usageSort.value = cur.order === 'desc' ? { key, metric, order: 'asc' REDACTED : null
  REDACTED else {
    usageSort.value = { key, metric, order: 'desc' REDACTED
  REDACTED
  persistUsageSort()
  openUsageSortMenu.value = null
REDACTED
...
const sortedUsers = computed(() => {
  ...
  return [...users.value]
    .map((row, index) => ({ row, index REDACTED))
    .sort((a, b) => {
...
REDACTED = useTableSelection<AdminUser>({
  rows: sortedUsers,
  getId: (user) => user.id
REDACTED)
...
const getUserSelectionLabel = (user: AdminUser) =>
  t('admin.users.bulkLimits.selectUser', { email: user.email REDACTED)

// User attribute definitions and values
const attributeDefinitions = ref<UserAttributeDefinition[]>([])
const userAttributeValues = ref<Record<number, Record<number, string>>>({REDACTED)
      ...
      (async () => {
        try {
          ...
          for (let i = 0; i < userIds.length; i += CHUNK) {
            ...
            if (signal?.aborted) return
            if (typeof expectedSeq === 'number' && expectedSeq !== secondaryDataSeq) return
            const merged = { ...platformQuotaStats.value REDACTED
...
const groupReplaceOldGroup = ref<{ id: number; name: string REDACTED | null>(null)
...
const loadUsers = async () => {
  ...
  try {
    ...
    const attrFilters: Record<number, string> = {REDACTED
    ...
    usageStats.value = {REDACTED
    userAttributeValues.value = {REDACTED
    platformQuotaStats.value = {REDACTED
...
const toggleAttributeFilter = (attr: UserAttributeDefinition) => {
  const key = `attr_${attr.idREDACTED`
  if (visibleFilters.has(key)) {
    visibleFilters.delete(key)
...
const openGroupReplace = (user: AdminUser, group: { id: number; name: string REDACTED) => {
  expandedGroupUserId.value = null
