...
describe('ChannelMonitorView duplicate action', () => {
  beforeEach(() => {
    ...
    listMonitors.mockResolvedValue({
      ...
      pages: 1,
    REDACTED)
    duplicateMonitor.mockResolvedValue(makeMonitor({ id: 43, name: 'primary (Copy)', enabled: false REDACTED))
  REDACTED)
  ...
  it('ignores repeated clicks while a duplicate request is in flight', async () => {
    ...
    duplicateMonitor.mockImplementationOnce(() => new Promise(resolve => { resolveDuplicate = resolve REDACTED))
    const wrapper = mountView()
    await flushPromises()
    ...
    resolveDuplicate(makeMonitor({ id: 43, name: 'primary (Copy)', enabled: false REDACTED))
    await flushPromises()
  ...
  it('rejects a defensive duplicate event when the API key is unavailable', async () => {
    const unavailable = makeMonitor({ id: 99, api_key_decrypt_failed: true REDACTED)
