...
describe('GroupsView duplicate action', () => {
  beforeEach(() => {
    localStorage.clear()
    vi.spyOn(console, 'error').mockImplementation(() => {REDACTED)
    for (const fn of [
      listGroups,
      duplicateGroup,
      getModelsListCandidates,
    ...
    getLiveCapability.mockResolvedValue({ supported: false REDACTED)
  REDACTED)
  ...
  it('ignores repeated clicks while the duplicate request is in flight', async () => {
    ...
    duplicateGroup.mockImplementationOnce(
      () => new Promise<AdminGroup>((resolve) => { resolveDuplicate = resolve REDACTED)
    )
    const wrapper = mountView()
    await flushPromises()
    ...
    resolveDuplicate({ ...sourceGroup, id: 43, name: 'Primary (Copy)', status: 'inactive' REDACTED)
    await flushPromises()
