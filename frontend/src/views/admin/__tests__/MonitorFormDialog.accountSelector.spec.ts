...
describe('MonitorFormDialog linked account selector', () => {
  ...
  it('restores probe mode and clears the quota placeholder when switching away from antigravity', async () => {
    ...
    // 切换 provider 后旧 key 失效：补一个新 key 再提交。
  ...
  it('creates a group-aggregated quota monitor with group_id only', async () => {
    groupsList.mockResolvedValue({ items: [{ id: 31, name: 'Ag Pool', platform: 'antigravity' }] })
    const wrapper = mountDialog()
    await flushPromises()
    ...
    expect(groupsList).toHaveBeenCalledWith(
      1,
      50,
      { platform: 'antigravity', status: 'active' },
      { signal: expect.any(AbortSignal) },
    )

    await wrapper.findAll('input[type="text"]')[0].setValue('Ag Pool')
    const dropdown = await openGroupDropdown(wrapper)
    clickOption(dropdown, 'Ag Pool (#31)')
    await nextTick()
  ...
  it('hydrates the bound group when editing a group-aggregated monitor', async () => {
    groupsList.mockResolvedValue({ items: [{ id: 1, name: 'other pool', platform: 'antigravity' }] })
    groupsGetById.mockResolvedValue({ id: 77, name: 'hidden pool', platform: 'antigravity' })
    const wrapper = mountDialog(makeMonitor({
      provider: 'antigravity',
      check_mode: 'quota',
      group_id: 77,
  ...
  it('clears the other quota target when switching data source', async () => {
    accountsList.mockResolvedValue({ items: [{ id: 5, name: 'single', platform: 'antigravity' }] })
    groupsList.mockResolvedValue({ items: [{ id: 31, name: 'Ag Pool', platform: 'antigravity' }] })
    const wrapper = mountDialog()
    await flushPromises()
