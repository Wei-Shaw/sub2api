...
import type { DOMWrapper, VueWrapper REDACTED from '@vue/test-utils'

import RiskControlView from '../RiskControlView.vue'
import type { ContentModerationConfig, UpdateContentModerationConfig REDACTED from '@/api/admin/riskControl'
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, string | number>) => {
        if (key === 'admin.riskControl.preBlockAPIKeyLoadSummary') {
          return `同步并发 ${params?.activeREDACTED / 可用 Key ${params?.availableREDACTED，累计 ${params?.totalREDACTED 次，worker：${params?.workerActiveREDACTED / ${params?.workerTotalREDACTED`
        REDACTED
        return key.replace(/\{(\w+)\REDACTED/g, (_, token) => String(params?.[token] ?? `{${tokenREDACTEDREDACTED`))
      REDACTED,
    REDACTED),
  REDACTED
REDACTED)
...
function findButtonByText(wrapper: VueWrapper, text: string): DOMWrapper<HTMLButtonElement> {
  ...
  if (!button) {
    throw new Error(`button not found: ${textREDACTED`)
  REDACTED
  return button
REDACTED
...
describe('admin RiskControlView', () => {
  beforeEach(() => {
    ...
    listLogs.mockResolvedValue({ items: [], total: 0, page: 1, page_size: 20, pages: 1 REDACTED)
