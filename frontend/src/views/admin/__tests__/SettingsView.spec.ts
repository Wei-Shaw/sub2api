...
const localeRef = vi.hoisted(() => ({ value: "zh-CN" REDACTED));
...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.paymentVisibleMethods.methodLabel": "{titleREDACTED 可见方式",
    ...
    "admin.settings.paymentVisibleMethods.sourceRequiredError": "{titleREDACTED 已启用，请先选择支付来源。",
    ...
    "admin.settings.openaiExperimentalScheduler.defaultPlaceholder": "配置/默认：{valueREDACTED",
  ...
  return {
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, string>) =>
        (translations[key] ?? key).replace(/\{(\w+)\REDACTED/g, (_, token) => params?.[token] ?? `{${tokenREDACTEDREDACTED`),
      locale: localeRef,
    REDACTED),
  REDACTED;
REDACTED);
...
const ToggleStub = defineComponent({
  ...
  inheritAttrs: false,
  setup(props, { attrs, emit REDACTED) {
    return () =>
      h("input", {
        ...attrs,
        class: "toggle-stub",
      ...
      h(
        ...
        (props.options as Array<Record<string, unknown>>).map((option) =>
          h(
            "option",
            {
              key: `${String(option.value ?? "")REDACTED:${String(option.label ?? "")REDACTED`,
...
const baseSettingsResponse = {
  ...
  default_platform_quotas: {
    ...
    openai:      { daily: null, weekly: 12.5, monthly: null REDACTED,
    gemini:      { daily: null, weekly: null, monthly: 200 REDACTED,
...
describe("admin SettingsView payment visible method controls", () => {
  beforeEach(() => {
    ...
    getSettings.mockResolvedValue({ ...baseSettingsResponse REDACTED);
  ...
  it("submits the compact home page toggle", async () => {
    ...
    expect(updateSettings).toHaveBeenCalledWith(
      expect.objectContaining({ compact_home_enabled: true REDACTED),
    );
  REDACTED);
  ...
  it("shows valid passkey RP configuration and persists the sign-in toggle", async () => {
    ...
    expect(updateSettings).toHaveBeenCalledWith(
      expect.objectContaining({ passkey_enabled: false REDACTED),
    );
  REDACTED);
  ...
  it("loads, edits, validates, and saves forwarded client-IP headers", async () => {
    ...
    await input.trigger("keydown", { key: "Enter" REDACTED);
    await input.setValue("X-CLIENT-IP");
    await input.trigger("keydown", { key: "Enter" REDACTED);
  ...
  it("submits Claude OAuth system prompt injection gateway settings", async () => {
    const blocks = `[{"type":"text","text":"custom block","cache_control":trueREDACTED]`;
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
  ...
  it("updates provider enablement immediately and reloads providers", async () => {
    ...
    getProviders.mockReset();
    getProviders
      .mockResolvedValueOnce({ data: [provider] REDACTED)
      .mockResolvedValueOnce({ data: [{ ...provider, enabled: true REDACTED] REDACTED);
    updateProvider.mockResolvedValue({ data: { ...provider, enabled: true REDACTED REDACTED);
    ...
    expect(updateProvider).toHaveBeenCalledWith(7, { enabled: true REDACTED);
  ...
  it("normalizes null supported_types from API so provider card stays visible", async () => {
    ...
    REDACTED;
    getProviders.mockReset();
    getProviders.mockResolvedValue({ data: [providerWithNullTypes] REDACTED);
    ...
    const PaymentProviderListCapture = defineComponent({
      ...
      setup(props) {
        ...
        return () => h("div", { class: "provider-list-capture" REDACTED);
      REDACTED,
    REDACTED);
...
describe("admin SettingsView platform quota matrix", () => {
  beforeEach(() => {
    ...
    updateSettings.mockImplementation(async (payload) => ({
      ...
      ...payload,
    REDACTED));
    getWebSearchEmulationConfig.mockResolvedValue({ enabled: false, providers: [] REDACTED);
    updateWebSearchEmulationConfig.mockResolvedValue({ enabled: false, providers: [] REDACTED);
    getAdminApiKey.mockResolvedValue({ exists: false, masked_key: "" REDACTED);
    getOverloadCooldownSettings.mockResolvedValue({REDACTED);
    getRateLimit429CooldownSettings.mockResolvedValue({REDACTED);
    updateRateLimit429CooldownSettings.mockResolvedValue({REDACTED);
    getStreamTimeoutSettings.mockResolvedValue({REDACTED);
    getRectifierSettings.mockResolvedValue({REDACTED);
    getBetaPolicySettings.mockResolvedValue({REDACTED);
    getGroups.mockResolvedValue([]);
    listProxies.mockResolvedValue({ items: [] REDACTED);
    getProviders.mockResolvedValue({ data: [] REDACTED);
  REDACTED);
  ...
  it("加载后 form.default_platform_quotas 含全 5 平台，从嵌套 JSON 正确读取数值", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      default_platform_quotas: {
        anthropic: { daily: 5, weekly: null, monthly: null REDACTED,
        openai:    { daily: null, weekly: 12.5, monthly: null REDACTED,
    ...
    // 缺失平台应补全为 null
    expect(quotas["gemini"]).toEqual({ daily: null, weekly: null, monthly: null REDACTED);
    expect(quotas["antigravity"]).toEqual({ daily: null, weekly: null, monthly: null REDACTED);
  REDACTED);
  ...
  it("空输入（v-model.number 产出 \"\"）在提交时清洗为 null 而非空字符串", async () => {
    ...
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      default_platform_quotas: {
        anthropic: { daily: 10, weekly: null, monthly: null REDACTED,
