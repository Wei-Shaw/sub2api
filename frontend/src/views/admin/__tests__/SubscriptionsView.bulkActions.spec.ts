...
describe('subscription bulk operations', () => {
  ...
  it('assigns multiple users once and retries only failed users', async () => {
    ...
    await form.findAll('input[type="number"]')[1]!.setValue('1.5')
    ...
    expect(bulkAssign).toHaveBeenCalledWith({ user_ids: [11, 22], group_id: 7, validity_days: 30, daily_limit_usd: 1.5 })
    ...
    await form.trigger('submit')
    await flushPromises()
    expect(bulkAssign).toHaveBeenLastCalledWith({ user_ids: [22], group_id: 7, validity_days: 30, daily_limit_usd: 1.5 })
