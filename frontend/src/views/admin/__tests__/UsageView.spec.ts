...
import { defineComponent, ref REDACTED from 'vue'
...
const UsageFiltersStub = defineComponent({
  setup(_, { expose REDACTED) {
    ...
    REDACTED)
    return { userKeyword REDACTED
  REDACTED,
  template: '<div><span data-test="user-filter-label">{{ userKeyword REDACTEDREDACTED</span><slot name="after-reset" /></div>',
REDACTED)
...
describe('admin UsageView route filters', () => {
  ...
  it('shows the routed user while applying user_id to usage requests', async () => {
    ...
    getById.mockResolvedValue({ id: 42, email: 'route-user@test.com' REDACTED)

    const wrapper = mountRouteFilteredUsageView()
    await flushPromises()

    expect(getById).toHaveBeenCalledWith(42, true)
    expect(list).toHaveBeenCalledWith(expect.objectContaining({ user_id: 42 REDACTED), expect.anything())
  ...
  it('does not apply a stale routed user label after user_id changes', async () => {
    routeQuery.user_id = '42'
    let resolveLookup!: (user: { id: number; email: string REDACTED) => void
    getById.mockReturnValue(new Promise((resolve) => { resolveLookup = resolve REDACTED))
    ...
    resolveLookup({ id: 42, email: 'stale-user@test.com' REDACTED)
    await flushPromises()
  ...
  it('does not overwrite newer user input when the routed user lookup succeeds', async () => {
    ...
    resolveLookup({ id: 42, email: 'route-user@test.com' REDACTED)
    await flushPromises()
  ...
  it('does not overwrite newer user input when the routed user lookup fails', async () => {
    ...
    getById.mockReturnValue(new Promise((_, reject) => { rejectLookup = reject REDACTED))
...
describe('admin UsageView distribution metric toggles', () => {
  beforeEach(() => {
    ...
    getSnapshotV2.mockResolvedValue({
      ...
      groups: [],
    REDACTED)
    getModelStats.mockResolvedValue({ models: [] REDACTED)
  REDACTED)
  ...
  it('keeps previous model stats visible during refresh until new data arrives', async () => {
    // 首次加载返回 A
    getModelStats.mockResolvedValueOnce({ models: [{ model: 'A', total_tokens: 10 REDACTED] REDACTED)
    ...
    await flushPromises()
    expect((wrapper.vm as any).requestedModelStats).toEqual([{ model: 'A', total_tokens: 10 REDACTED])

    // 刷新:让第二次 getModelStats 处于 pending,断言旧数据 A 仍在(不被清空成 [])
    let resolveSecond: (v: any) => void = () => {REDACTED
    getModelStats.mockReturnValueOnce(new Promise((res) => { resolveSecond = res REDACTED))
    ;(wrapper.vm as any).refreshData()
    await flushPromises()
    expect((wrapper.vm as any).requestedModelStats).toEqual([{ model: 'A', total_tokens: 10 REDACTED])

    // 新数据到达后替换为 B
    resolveSecond({ models: [{ model: 'B', total_tokens: 20 REDACTED] REDACTED)
    await flushPromises()
    expect((wrapper.vm as any).requestedModelStats).toEqual([{ model: 'B', total_tokens: 20 REDACTED])
  REDACTED)
...
describe('admin UsageView request ID column visibility', () => {
  ...
  it('keeps request ID hidden by default and allows enabling it from column settings', async () => {
    ...
    expect(usageTable.props('columns')).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ key: 'request_id' REDACTED)]),
    )
    ...
    expect(usageTable.props('columns')).toEqual(
      expect.arrayContaining([expect.objectContaining({ key: 'request_id', label: 'Request ID' REDACTED)]),
    )
...
describe('admin UsageView errors tab filter forwarding', () => {
  beforeEach(() => {
    ...
    getSnapshotV2.mockResolvedValue({ trend: [], models: [], groups: [] REDACTED)
    getModelStats.mockResolvedValue({ models: [] REDACTED)
    listErrorLogs.mockResolvedValue({ items: [], total: 0, pages: 0 REDACTED)
  REDACTED)
