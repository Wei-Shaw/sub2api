...
const DataTableStub = {
  ...
  template: `
    <div>
      ...
      <div data-test="selected-keys">{{ (selectedKeys || []).join(',') REDACTEDREDACTED</div>
...
const BulkEditUserModalStub = {
  ...
  template: `
    <div v-if="show" data-test="bulk-modal">
      <span data-test="bulk-modal-ids">{{ selectedIds.join(',') REDACTEDREDACTED</span>
    ...
    listUsers.mockImplementation(async (page: number) => {
      const user = page === 2
        ? createAdminUser({
          ...
          REDACTED)
        : createAdminUser({ id: 42, email: 'page-one@example.com' REDACTED)
      return {
        items: [user],
        total: 2,
        page,
