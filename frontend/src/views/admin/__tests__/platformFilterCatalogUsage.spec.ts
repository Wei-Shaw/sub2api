...
describe('admin platform filters', () => {
  it('uses the group platform catalog on the subscriptions page', () => {
    ...
    expect(source).toContain("import { GROUP_PLATFORM_OPTIONS REDACTED from '@/constants/platforms'")
  ...
  it('uses the concrete platform catalog wherever concrete platforms are selected', () => {
    for (const path of [
      ...
      expect(source).toContain("import { CONCRETE_PLATFORM_OPTIONS REDACTED from '@/constants/platforms'")
