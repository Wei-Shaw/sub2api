...
// Select.vue generates :key from `${typeof valueREDACTED:${String(value ?? '')REDACTED` — using distinct
...
export function buildApiKeyGroupFilterOptions(
  ...
  for (const grp of groups) {
    const item: ApiKeyGroupFilterOption = { value: grp.id, label: grp.name REDACTED
    if (grp.status !== 'active') {
      disabledGroups.push(item)
    REDACTED else if (grp.subscription_type === 'subscription') {
      subscription.push(item)
    REDACTED else if (grp.is_exclusive) {
      exclusive.push(item)
    REDACTED else {
  ...
  const options: ApiKeyGroupFilterOption[] = [{ value: null, label: labels.all REDACTED]
  ...
  for (const [label, headerValue, items] of sections) {
    ...
    options.push({ value: headerValue, label, kind: 'group', disabled: true REDACTED)
    options.push(...items)
  REDACTED
  return options
REDACTED
