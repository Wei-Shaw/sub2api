import type { OpenAIMessagesDispatchModelConfig REDACTED from "@/types";
...
export function messagesDispatchConfigToFormState(
  ...
  const exactMappings = Object.entries(config?.exact_model_mappings || {REDACTED)
    .sort(([left], [right]) => left.localeCompare(right))
    .map(([claude_model, target_model]) => ({ claude_model, target_model REDACTED));
