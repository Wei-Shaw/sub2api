...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p0' || s === 'critical') return 'bg-red-100 text-danger dark:bg-red-900/30'
  if (s === 'p1' || s === 'warning') return 'bg-amber-100 text-warn dark:bg-amber-900/30'
  if (s === 'p2' || s === 'info') return 'bg-blue-100 text-info dark:bg-blue-900/30'
  if (s === 'p3') return 'bg-gray-100 text-ink-secondary dark:bg-dark-700 dark:text-gray-300'
  return 'bg-gray-100 text-ink-secondary dark:bg-dark-700 dark:text-gray-300'
}

function statusBadgeClass(status: string | undefined): string {
  const s = String(status || '').trim().toLowerCase()
  if (s === 'firing') return 'border border-danger/40 bg-danger-tint text-danger ring-red-600/20 dark:ring-red-500/30'
  if (s === 'resolved') return 'border border-success/40 bg-success-tint text-success ring-green-600/20 dark:ring-green-500/30'
  ...
  return 'bg-gray-50 text-ink-secondary ring-gray-600/20 dark:bg-gray-900/30 dark:text-gray-300 dark:ring-gray-500/30'
}
...
<template>
  <div class="rounded-3xl bg-surface p-6 shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-ink">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-ink-secondary">
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-line p-8 text-center text-sm text-ink-secondary dark:text-gray-400">
      ...
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <div v-if="!isDesktopViewport" class="divide-y divide-line-subtle">
          <div
            v-for="row in events"
            :key="row.id"
            class="cursor-pointer space-y-2 p-4 hover:bg-surface-hover/50"
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="ml-auto text-[11px] text-ink-secondary">
                {{ formatDateTime(row.fired_at || row.created_at) }}
              </span>
            </div>
            <div class="text-xs font-semibold text-ink">{{ row.title || '-' }}</div>
            <div v-if="row.description" class="line-clamp-2 text-[11px] text-ink-secondary">
              {{ row.description }}
            </div>
            <div class="flex flex-wrap items-center justify-between gap-2 text-[11px] text-ink-secondary">
            ...
            <div class="text-[11px] text-ink-tertiary">{{ formatDimensionsSummary(row) }}</div>
          </div>
        </div>
        <table v-else class="min-w-full divide-y divide-line">
          <thead class="sticky top-0 z-10 bg-surface-sunken">
            <tr>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-ink-secondary">
              ...
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-ink-secondary">
          ...
          <tbody class="divide-y divide-gray-200 bg-surface dark:divide-dark-700">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-surface-hover/50"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 text-xs text-ink-secondary">
                {{ formatDateTime(row.fired_at || row.created_at) }}
              </td>
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-ink-secondary">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-ink-secondary">
                  {{ row.description }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-ink-secondary">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-ink-secondary">
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-ink-secondary">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-ink-tertiary">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-ink-secondary">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-ink-secondary">
      ...
      <div v-else class="space-y-5">
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-ink-secondary">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-surface px-2 py-1 ring-1 ring-gray-200 dark:ring-dark-700">
                <span class="text-[11px] font-bold text-ink-secondary">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-ink">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-ink">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-ink">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-ink-secondary ring-1 ring-gray-200 hover:bg-surface-sunken dark:ring-dark-700 dark:hover:bg-dark-700"
            ...
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
        ...
        <div class="rounded-xl border border-line bg-surface p-4">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-ink">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-ink-secondary">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-ink-secondary">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-ink-secondary">
          ...
          <div v-else class="overflow-hidden rounded-lg border border-line-subtle">
            <table class="min-w-full divide-y divide-gray-100 dark:divide-dark-700">
              <thead class="bg-surface-sunken">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-ink-secondary">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-ink-secondary">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-ink-secondary">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-surface-hover/50">
                  <td class="px-3 py-2 text-xs text-ink-secondary">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
