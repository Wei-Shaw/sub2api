...
import { opsAPI, type AlertEventsQuery REDACTED from '@/api/admin/ops'
import type { AlertEvent REDACTED from '../types'
import { formatDateTime REDACTED from '../utils/opsFormatters'
...
const severityOptions = computed(() => [
  ...
  { value: 'P0', label: 'P0' REDACTED,
  { value: 'P1', label: 'P1' REDACTED,
  { value: 'P2', label: 'P2' REDACTED,
  { value: 'P3', label: 'P3' REDACTED
])

const status = ref<string>('')
const statusOptions = computed(() => [
  { value: '', label: t('common.all') REDACTED,
  { value: 'firing', label: t('admin.ops.alertEvents.status.firing') REDACTED,
  { value: 'resolved', label: t('admin.ops.alertEvents.status.resolved') REDACTED,
  { value: 'manual_resolved', label: t('admin.ops.alertEvents.status.manualResolved') REDACTED
])

const emailSent = ref<string>('')
const emailSentOptions = computed(() => [
  { value: '', label: t('common.all') REDACTED,
  { value: 'true', label: t('admin.ops.alertEvents.table.emailSent') REDACTED,
  { value: 'false', label: t('admin.ops.alertEvents.table.emailIgnored') REDACTED
])

function buildQuery(overrides: Partial<AlertEventsQuery> = {REDACTED): AlertEventsQuery {
...
async function loadMore() {
  ...
  try {
    const data = await opsAPI.listAlertEvents(
      buildQuery({ before_fired_at: last.fired_at || last.created_at, before_id: last.id REDACTED)
    )
    if (!data.length) {
      hasMore.value = false
      return
...
function formatDurationMs(ms: number): string {
  ...
  if (sec < 60) return `${secREDACTEDs`
  const min = Math.floor(sec / 60)
  if (min < 60) return `${minREDACTEDm`
  const hr = Math.floor(min / 60)
  if (hr < 24) return `${hrREDACTEDh`
...
function formatDurationLabel(event: AlertEvent): string {
  ...
  if (resolvedAtStr) {
    ...
    if (!Number.isNaN(resolvedAt.getTime())) {
      ...
      return `${prefixREDACTED ${formatDurationMs(ms)REDACTED`
    REDACTED
  REDACTED

  const now = Date.now()
  const ms = now - firedAt.getTime()
  return `${t('admin.ops.alertEvents.status.firing')REDACTED ${formatDurationMs(ms)REDACTED`
REDACTED

function formatDimensionsSummary(event: AlertEvent): string {
  const parts: string[] = []
  const platform = getDimensionString(event, 'platform')
  if (platform) parts.push(`platform=${platformREDACTED`)
  const groupId = event.dimensions?.group_id
  if (groupId != null && groupId !== '') parts.push(`group_id=${String(groupId)REDACTED`)
  const region = getDimensionString(event, 'region')
  if (region) parts.push(`region=${regionREDACTED`)
...
async function silenceAlert() {
  ...
  try {
    ...
    await opsAPI.createAlertSilence({
      ...
      reason: `silence from UI (${silenceDuration.valueREDACTED)`
    REDACTED)
...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-gray-900 dark:text-white">{{ t('admin.ops.alertEvents.title') REDACTEDREDACTED</h3>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.alertEvents.description') REDACTEDREDACTED</p>
      </div>
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
      ...
      {{ t('admin.ops.alertEvents.loading') REDACTEDREDACTED
    </div>
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400">
      {{ t('admin.ops.alertEvents.empty') REDACTEDREDACTED
    </div>
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="inline-flex items-center rounded-full px-2 py-1 text-[10px] font-bold ring-1 ring-inset" :class="statusBadgeClass(row.status)">
                {{ formatStatusLabel(row.status) REDACTEDREDACTED
              </span>
              <span class="ml-auto text-[11px] text-gray-500 dark:text-gray-400">
                {{ formatDateTime(row.fired_at || row.created_at) REDACTEDREDACTED
              </span>
            </div>
            <div class="text-xs font-semibold text-gray-900 dark:text-white">{{ row.title || '-' REDACTEDREDACTED</div>
            <div v-if="row.description" class="line-clamp-2 text-[11px] text-gray-500 dark:text-gray-400">
              {{ row.description REDACTEDREDACTED
            </div>
            <div class="flex flex-wrap items-center justify-between gap-2 text-[11px] text-gray-500 dark:text-gray-400">
              <span><span class="font-mono">#{{ row.rule_id REDACTEDREDACTED</span> · {{ formatDurationLabel(row) REDACTEDREDACTED</span>
              <span class="inline-flex items-center gap-1">
                ...
                />
                {{ row.email_sent ? t('admin.ops.alertEvents.table.emailSent') : t('admin.ops.alertEvents.table.emailIgnored') REDACTEDREDACTED
              </span>
            </div>
            <div class="text-[11px] text-gray-400 dark:text-gray-500">{{ formatDimensionsSummary(row) REDACTEDREDACTED</div>
          </div>
        </div>
        <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
          <thead class="sticky top-0 z-10 bg-gray-50 dark:bg-dark-900">
            <tr>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.time') REDACTEDREDACTED
              </th>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.severity') REDACTEDREDACTED
              </th>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.platform') REDACTEDREDACTED
              </th>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.ruleId') REDACTEDREDACTED
              </th>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.title') REDACTEDREDACTED
              </th>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.duration') REDACTEDREDACTED
              </th>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.dimensions') REDACTEDREDACTED
              </th>
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.alertEvents.table.email') REDACTEDREDACTED
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
            ...
            >
              ...
              <td class="whitespace-nowrap px-4 py-3 text-xs text-gray-600 dark:text-gray-300">
                {{ getDimensionString(row, 'platform') || '-' REDACTEDREDACTED
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-gray-600 dark:text-gray-300">
                <span class="font-mono">#{{ row.rule_id REDACTEDREDACTED</span>
              </td>
              <td class="min-w-[260px] px-4 py-3 text-xs text-gray-700 dark:text-gray-200">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' REDACTEDREDACTED</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-gray-500 dark:text-gray-400">
                  {{ row.description REDACTEDREDACTED
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-gray-600 dark:text-gray-300">
                {{ formatDurationLabel(row) REDACTEDREDACTED
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-gray-500 dark:text-gray-400">
                {{ formatDimensionsSummary(row) REDACTEDREDACTED
              </td>
    ...
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-gray-500 dark:text-gray-400">
        {{ t('admin.ops.alertEvents.detail.loading') REDACTEDREDACTED
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-gray-500 dark:text-gray-400">
        {{ t('admin.ops.alertEvents.detail.empty') REDACTEDREDACTED
      </div>
      ...
      <div v-else class="space-y-5">
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div class="flex flex-wrap items-center gap-2">
                <span class="inline-flex items-center rounded-full px-2 py-1 text-[10px] font-bold" :class="severityBadgeClass(String(selected.severity || ''))">
                  {{ selected.severity || '-' REDACTEDREDACTED
                </span>
            ...
            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-white px-2 py-1 ring-1 ring-gray-200 dark:bg-dark-800 dark:ring-dark-700">
                <span class="text-[11px] font-bold text-gray-600 dark:text-gray-300">{{ t('admin.ops.alertEvents.detail.silence') REDACTEDREDACTED</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.firedAt') REDACTEDREDACTED</div>
              <div class="mt-1 text-sm font-medium text-gray-900 dark:text-white">{{ formatDateTime(selected.fired_at || selected.created_at) REDACTEDREDACTED</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.resolvedAt') REDACTEDREDACTED</div>
              <div class="mt-1 text-sm font-medium text-gray-900 dark:text-white">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' REDACTEDREDACTED</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.ruleId') REDACTEDREDACTED</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-gray-900 dark:text-white">#{{ selected.rule_id REDACTEDREDACTED</div>
                <a
                  ...
                  :href="`/admin/ops?open_alert_rules=1&alert_rule_id=${selected.rule_idREDACTED`"
                >
                  <Icon name="externalLink" size="xs" />
                  {{ t('admin.ops.alertEvents.detail.viewRule') REDACTEDREDACTED
                </a>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-gray-700 ring-1 ring-gray-200 hover:bg-gray-50 dark:bg-dark-800 dark:text-gray-200 dark:ring-dark-700 dark:hover:bg-dark-700"
                  :href="`/admin/ops?platform=${encodeURIComponent(getDimensionString(selected,'platform')||'')REDACTED&group_id=${selected.dimensions?.group_id || ''REDACTED&error_type=request&open_error_details=1`"
                >
                  <Icon name="externalLink" size="xs" />
                  {{ t('admin.ops.alertEvents.detail.viewLogs') REDACTEDREDACTED
                </a>
              </div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.dimensions') REDACTEDREDACTED</div>
              <div class="mt-1 text-sm text-gray-900 dark:text-white">
                <div v-if="getDimensionString(selected, 'platform')">platform={{ getDimensionString(selected, 'platform') REDACTEDREDACTED</div>
                <div v-if="selected.dimensions?.group_id">group_id={{ selected.dimensions.group_id REDACTEDREDACTED</div>
                <div v-if="getDimensionString(selected, 'region')">region={{ getDimensionString(selected, 'region') REDACTEDREDACTED</div>
              </div>
            </div>
          </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-gray-900 dark:text-white">{{ t('admin.ops.alertEvents.detail.historyTitle') REDACTEDREDACTED</div>
              <div class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.alertEvents.detail.historyHint') REDACTEDREDACTED</div>
            </div>
          ...
          <div v-if="historyLoading" class="py-6 text-center text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.ops.alertEvents.detail.historyLoading') REDACTEDREDACTED
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') REDACTEDREDACTED
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-gray-100 dark:border-dark-700">
            <table class="min-w-full divide-y divide-gray-100 dark:divide-dark-700">
              <thead class="bg-gray-50 dark:bg-dark-900">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.ops.alertEvents.table.time') REDACTEDREDACTED</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.ops.alertEvents.table.status') REDACTEDREDACTED</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.ops.alertEvents.table.metric') REDACTEDREDACTED</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-gray-50 dark:hover:bg-dark-700/50">
                  <td class="px-3 py-2 text-xs text-gray-600 dark:text-gray-300">{{ formatDateTime(it.fired_at || it.created_at) REDACTEDREDACTED</td>
                  <td class="px-3 py-2 text-xs">
                    <span class="inline-flex items-center rounded-full px-2 py-1 text-[10px] font-bold ring-1 ring-inset" :class="statusBadgeClass(it.status)">
                      {{ formatStatusLabel(it.status) REDACTEDREDACTED
                    </span>
                  </td>
                  <td class="px-3 py-2 text-xs text-gray-600 dark:text-gray-300">
                    <span v-if="typeof it.metric_value === 'number' && typeof it.threshold_value === 'number'">
                      {{ it.metric_value.toFixed(2) REDACTEDREDACTED / {{ it.threshold_value.toFixed(2) REDACTEDREDACTED
                    </span>
                    <span v-else>-</span>
                  </td>
                </tr>
