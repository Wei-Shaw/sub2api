...
function getLoadTextClass(loadPct: number): string {
  ...
  if (loadPct >= 50) return 'text-warn'
  return 'text-success'
}
...
<template>
  <div class="flex h-full flex-col rounded-3xl bg-surface p-6 shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="showByUser
            ...
            : 'bg-gray-100 text-ink-secondary hover:bg-gray-200 hover:text-gray-700 dark:bg-dark-700 dark:text-gray-400 dark:hover:bg-dark-600 dark:hover:text-gray-300'"
        ...
        <!-- 刷新按钮 -->
        <button
          class="flex items-center gap-1 rounded-lg bg-surface-sunken px-2 py-1 text-[11px] font-semibold text-ink-secondary transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-dark-600"
          :disabled="loading"
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="mb-3 shrink-0 rounded-xl border border-danger/40 bg-danger-tint p-2.5 text-xs text-red-600 dark:text-red-400">
      {{ errorMessage }}
    </div>
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-line text-sm text-ink-secondary dark:text-gray-400"
    >
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-line">
      <!-- 维度标题栏 -->
      <div class="flex shrink-0 items-center justify-between border-b border-line bg-gray-50 px-3 py-2 dark:bg-dark-900">
        <span class="text-[10px] font-bold uppercase tracking-wider text-ink-secondary">
          {{ displayTitle }}
        </span>
      ...
      <!-- 空状态 -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-ink-secondary">
        ...
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-2.5 dark:bg-dark-900">
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-bold text-ink" :title="row.username || row.user_email">
                {{ row.username || row.user_email }}
              </span>
              <span v-if="row.username" class="shrink-0 truncate text-[10px] text-ink-tertiary" :title="row.user_email">
                {{ row.user_email }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-ink"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-3 dark:bg-dark-900">
          ...
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="truncate text-[11px] font-bold text-ink" :title="row.name">
                {{ row.name }}
              </div>
              <span v-if="displayDimension === 'group' && row.platform" class="text-[10px] text-ink-tertiary">
                {{ row.platform.toUpperCase() }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-ink"> {{ row.used_concurrency }}/{{ row.total_concurrency }} </span>
              <span :class="['font-bold', getLoadTextClass(row.concurrency_percentage)]"> {{ row.concurrency_percentage }}% </span>
            </div>
          </div>

          <!-- 进度条 -->
          <div class="mb-2 h-1.5 w-full overflow-hidden rounded-full bg-surface-active">
            <div
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              <svg class="h-3 w-3 text-ink-tertiary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
              ...
              <span class="text-ink-secondary">
                <span class="font-bold text-success">{{ row.available_accounts }}</span
                >/{{ row.total_accounts }}
              </span>
              <span class="text-ink-tertiary">{{ row.availability_percentage }}%</span>
            </div>

            <!-- 限流账号 -->
            <span
              v-if="row.rate_limited_accounts > 0"
              class="rounded-sm border border-warn/40 bg-warn-tint px-1.5 py-0.5 font-medium text-warn"
            >
            ...
            <span
              ...
              class="rounded-sm border border-danger/40 bg-danger-tint px-1.5 py-0.5 font-medium text-danger"
            >
        ...
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-2.5 dark:bg-dark-900">
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-ink-tertiary">
                {{ row.group_name }}
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-ink"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <!-- 状态徽章 -->
              <span
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-ink-secondary dark:bg-gray-800 dark:text-gray-400"
              >
