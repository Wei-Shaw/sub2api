...
import { opsAPI, type OpsAccountAvailabilityStatsResponse, type OpsConcurrencyStatsResponse, type OpsUserConcurrencyStatsResponse REDACTED from '@/api/admin/ops'
...
const platformRows = computed((): SummaryRow[] => {
  const concStats = concurrency.value?.platform || {REDACTED
  const availStats = availability.value?.platform || {REDACTED

  const platforms = new Set([...Object.keys(concStats), ...Object.keys(availStats)])

  return Array.from(platforms).map(platform => {
    const conc = concStats[platform] || {REDACTED
    const avail = availStats[platform] || {REDACTED
    ...
    REDACTED
  REDACTED).sort((a, b) => b.concurrency_percentage - a.concurrency_percentage)
REDACTED)

// 分组维度汇总
const groupRows = computed((): SummaryRow[] => {
  const concStats = concurrency.value?.group || {REDACTED
  const availStats = availability.value?.group || {REDACTED

  const groupIds = new Set([...Object.keys(concStats), ...Object.keys(availStats)])

  const rows = Array.from(groupIds)
    .map(gid => {
      const conc = concStats[gid] || {REDACTED
      const avail = availStats[gid] || {REDACTED
      ...
      return {
        key: gid,
        name: String(conc.group_name || avail.group_name || `Group ${gidREDACTED`),
...
const accountRows = computed((): AccountRow[] => {
  const concStats = concurrency.value?.account || {REDACTED
  const availStats = availability.value?.account || {REDACTED

  const accountIds = new Set([...Object.keys(concStats), ...Object.keys(availStats)])

  const rows = Array.from(accountIds)
    .map(aid => {
      const conc = concStats[aid] || {REDACTED
      const avail = availStats[aid] || {REDACTED
      ...
      return {
        key: aid,
        name: String(conc.account_name || avail.account_name || `Account ${aidREDACTED`),
        platform: String(conc.platform || avail.platform || ''),
...
const userRows = computed((): UserRow[] => {
  const userStats = userConcurrency.value?.user || {REDACTED

  return Object.keys(userStats)
    .map(uid => {
      const u = userStats[uid] || {REDACTED
      return {
        key: uid,
        user_id: safeNumber(u.user_id),
        user_email: u.user_email || `User ${uidREDACTED`,
        username: u.username || '',
...
function getLoadBarStyle(loadPct: number): string {
  return `width: ${Math.min(100, Math.max(0, loadPct))REDACTED%`
REDACTED
...
function formatDuration(seconds: number): string {
  ...
  if (seconds < 60) return `${Math.round(seconds)REDACTEDs`
  const minutes = Math.floor(seconds / 60)
  if (minutes < 60) return `${minutesREDACTEDm`
  const hours = Math.floor(minutes / 60)
  return `${hoursREDACTEDh`
REDACTED
  ...
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    ...
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
        ...
        {{ t('admin.ops.concurrency.title') REDACTEDREDACTED
      </h3>
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          @click="loadData"
        >
          <svg class="h-3 w-3" :class="{ 'animate-spin': loading REDACTED" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      {{ t('admin.ops.concurrency.disabledHint') REDACTEDREDACTED
    </div>
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
      ...
      <div class="flex shrink-0 items-center justify-between border-b border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-900">
        ...
        <span class="text-[10px] text-gray-500 dark:text-gray-400">
          {{ t('admin.ops.concurrency.totalRows', { count: displayRows.length REDACTED) REDACTEDREDACTED
        </span>
      </div>

      <!-- 空状态 -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-gray-500 dark:text-gray-400">
        {{ t('admin.ops.concurrency.empty') REDACTEDREDACTED
      </div>
        ...
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-2.5 dark:bg-dark-900">
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-bold text-gray-900 dark:text-white" :title="row.username || row.user_email">
                {{ row.username || row.user_email REDACTEDREDACTED
              </span>
              <span v-if="row.username" class="shrink-0 truncate text-[10px] text-gray-400 dark:text-gray-500" :title="row.user_email">
                {{ row.user_email REDACTEDREDACTED
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-gray-900 dark:text-white"> {{ row.current_in_use REDACTEDREDACTED/{{ row.max_capacity REDACTEDREDACTED </span>
              <span :class="['font-bold', getLoadTextClass(row.load_percentage)]"> {{ Math.round(row.load_percentage) REDACTEDREDACTED% </span>
            </div>
          </div>
          ...
          <div v-if="row.waiting_in_queue > 0" class="mt-1.5 flex justify-end">
            <span class="rounded-full bg-purple-100 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">
              {{ t('admin.ops.concurrency.queued', { count: row.waiting_in_queue REDACTED) REDACTEDREDACTED
            </span>
          </div>
        </div>
      </div>
          ...
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              ...
              <span v-if="displayDimension === 'group' && row.platform" class="text-[10px] text-gray-400 dark:text-gray-500">
                {{ row.platform.toUpperCase() REDACTEDREDACTED
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-gray-900 dark:text-white"> {{ row.used_concurrency REDACTEDREDACTED/{{ row.total_concurrency REDACTEDREDACTED </span>
              <span :class="['font-bold', getLoadTextClass(row.concurrency_percentage)]"> {{ row.concurrency_percentage REDACTEDREDACTED% </span>
            </div>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              ...
              <span class="text-gray-600 dark:text-gray-300">
                <span class="font-bold text-green-600 dark:text-green-400">{{ row.available_accounts REDACTEDREDACTED</span
                >/{{ row.total_accounts REDACTEDREDACTED
              </span>
              <span class="text-gray-400 dark:text-gray-500">{{ row.availability_percentage REDACTEDREDACTED%</span>
            </div>
            ...
            >
              {{ t('admin.ops.concurrency.rateLimited', { count: row.rate_limited_accounts REDACTED) REDACTEDREDACTED
            </span>
            ...
            <template v-if="row.scope_rate_limit_count && Object.keys(row.scope_rate_limit_count).length > 0">
              <span
                ...
                :title="t('admin.ops.concurrency.scopeRateLimitedTooltip', { scope, count REDACTED)"
              >
                {{ formatScopeName(scope as string) REDACTEDREDACTED {{ count REDACTEDREDACTED
              </span>
            </template>
            ...
            >
              {{ t('admin.ops.concurrency.errorAccounts', { count: row.error_accounts REDACTED) REDACTEDREDACTED
            </span>
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-gray-400 dark:text-gray-500">
                {{ row.group_name REDACTEDREDACTED
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-gray-900 dark:text-white"> {{ row.current_in_use REDACTEDREDACTED/{{ row.max_capacity REDACTEDREDACTED </span>
              <!-- 状态徽章 -->
              <span
              ...
              >
                ...
                {{ t('admin.ops.accountAvailability.available') REDACTEDREDACTED
              </span>
              <span
              ...
              >
                ...
                {{ formatDuration(row.rate_limit_remaining_sec || 0) REDACTEDREDACTED
              </span>
              <span
              ...
              >
                ...
                {{ formatDuration(row.overload_remaining_sec || 0) REDACTEDREDACTED
              </span>
              <span
              ...
              >
                ...
                {{ t('admin.ops.accountAvailability.accountError') REDACTEDREDACTED
              </span>
              <span
                v-else
              ...
              >
                {{ t('admin.ops.accountAvailability.unavailable') REDACTEDREDACTED
              </span>
            </div>
          </div>
