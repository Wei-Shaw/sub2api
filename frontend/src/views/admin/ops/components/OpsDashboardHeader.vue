...
const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return 'text-ink-tertiary'
  const score = healthScoreValue.value
...
const dbMiddleClass = computed(() => {
  ...
  if (dbUsagePercent.value != null) {
    ...
    if (dbUsagePercent.value >= 70) return 'text-warn'
    return 'text-success'
  }
...
const redisMiddleClass = computed(() => {
  ...
  if (redisUsagePercent.value != null) {
    ...
    if (redisUsagePercent.value >= 70) return 'text-warn'
    return 'text-success'
  }
...
<template>
  <div :class="['flex flex-col gap-4 rounded-3xl bg-surface shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-line-subtle pb-4">
      <div>
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-ink-secondary">
          <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
            <span class="relative flex h-2 w-2">
              <span class="relative inline-flex h-2 w-2 rounded-full" :class="props.loading ? 'bg-gray-400' : 'bg-success'"></span>
            </span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-surface-active sm:block"></div>
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-ink-secondary transition-colors hover:bg-surface-active dark:text-gray-400 dark:hover:bg-dark-600"
          :disabled="loading"
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-surface-active sm:block"></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-gray-100 px-3 text-xs font-bold text-ink-secondary transition-colors hover:bg-surface-active dark:hover:bg-dark-600"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-ink-secondary transition-colors hover:bg-surface-active dark:hover:bg-dark-600"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-surface-sunken lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          ...
          >
            ...
            >
              <div class="rounded-xl bg-surface p-4 shadow-xl ring-1 ring-black/5 dark:ring-white/10">
                <h4 class="mb-3 border-b border-line-subtle pb-2 text-sm font-bold text-ink flex items-center gap-2">
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-ink">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-ink-secondary">{{ item.impact }}</div>
                      <div v-if="item.action" class="mt-1 text-[11px] text-info flex items-center gap-1">
                ...
                <div class="mt-3 border-t border-line-subtle pt-2 text-[10px] text-ink-tertiary">
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-ink-tertiary']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-ink-secondary">
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <!--
                  A 3px dot under a 12px `animate-ping` halo, forever. The
                  panel is titled "realtime" and its numbers move on their own;
                  an infinite pulse beside them adds motion, not information,
                  and it is exactly the animation this system bans.
                -->
                <span class="h-1.5 w-1.5 shrink-0 rounded-full bg-success" aria-hidden="true"></span>
                <h3 class="text-xs font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.realtime.title') }}</h3>
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-ink-tertiary']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-ink']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-ink-secondary']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-ink']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-ink-secondary']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-ink-tertiary']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-ink-secondary']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-ink-tertiary']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-ink-secondary']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-tertiary">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-ink">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-ink">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-ink">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.avgTps') }}:</span>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-ink-tertiary">{{ t('admin.ops.sla') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.sla')" />
              <span class="h-1.5 w-1.5 rounded-full" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-danger' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-success'"></span>
            </div>
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-blue-500 hover:underline"
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-surface-active">
            <div class="h-full transition-all" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-danger' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-success'" :style="{ width: `${Math.max((slaPercent ?? 0) - 90, 0) * 10}%` }"></div>
          </div>
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.exceptions') }}:</span>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-tertiary">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            ...
            <span class="text-xs font-bold text-ink-tertiary">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-secondary">P95:</span>
              <span class="font-bold text-ink">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-ink-tertiary">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-secondary">P90:</span>
              <span class="font-bold text-ink">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-ink-tertiary">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-secondary">P50:</span>
              <span class="font-bold text-ink">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-ink-tertiary">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-secondary">Avg:</span>
              <span class="font-bold text-ink">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-ink-tertiary">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-secondary">Max:</span>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-tertiary">TTFT</span>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-tertiary">{{ t('admin.ops.requestErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-ink">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-ink">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-tertiary">{{ t('admin.ops.upstreamErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-secondary">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-ink">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-secondary">429/529:</span>
              <span class="font-bold text-ink">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-line-subtle pt-4">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-tertiary">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-ink-secondary">
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.db') }}</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-tertiary">Redis</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-ink-tertiary">{{ t('admin.ops.jobs') }}</div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-ink-secondary">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        <div
          ...
          :key="hb.job_name"
          class="rounded-xl border border-line-subtle bg-surface p-4"
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-ink">{{ hb.job_name }}</div>
          ...
          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-ink-secondary sm:grid-cols-2">
            <div>
          ...
          <div
            v-if="hb.last_error"
            class="mt-3 rounded-lg border border-danger/40 bg-danger-tint p-2 text-xs text-danger"
          >
            {{ hb.last_error }}
          </div>
        </div>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input
            ...
            type="datetime-local"
            class="w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-ink-secondary mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-ink-secondary hover:bg-surface-active dark:hover:bg-dark-600"
