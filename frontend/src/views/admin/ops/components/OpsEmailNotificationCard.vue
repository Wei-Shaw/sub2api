...
<template>
  <div class="rounded-3xl bg-surface p-6 shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-ink">{{ t('admin.ops.email.title') }}</h3>
        <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.ops.email.description') }}</p>
      </div>
      <div class="flex items-center gap-2">
        <button
          class="flex items-center gap-1.5 rounded-lg bg-surface-sunken px-3 py-1.5 text-xs font-bold text-ink-secondary transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-dark-600"
          :disabled="loading"
          @click="loadConfig"
        >
    ...
    <div v-if="!config" class="text-sm text-ink-secondary">
    ...
    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-2 text-sm font-semibold text-ink">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div class="text-xs text-ink-secondary">
            ...
            <span class="ml-1 font-medium text-ink">{{ config.alert.recipients.length }}</span>
          </div>
          <div class="text-xs text-ink-secondary">
          ...
          <div class="text-xs text-ink-secondary">
            ...
            <span class="ml-1 font-medium text-ink">{{ config.alert.rate_limit_per_hour }}</span>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-2 text-sm font-semibold text-ink">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-ink-secondary">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-ink">
          ...
          <div class="text-xs text-ink-secondary">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-ink">{{ config.report.recipients.length }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <BaseDialog :show="showEditor" :title="t('admin.ops.email.title')" width="extra-wide" @close="showEditor = false">
    <div v-if="draft" class="space-y-6">
      <div
        ...
        class="rounded-lg border border-amber-200 border border-warn/40 bg-warn-tint p-3 text-xs text-warn dark:border-amber-900/50"
      >
      ...
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-3 text-sm font-semibold text-ink">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('common.enabled') }}</div>
          ...
          <div>
            <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.minSeverity') }}</div>
            <Select v-model="draft.alert.min_severity" :options="severityOptions" />
          </div>

          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.recipients') }}</div>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              ...
              >
                ...
                <button
                  type="button"
                  class="text-info/80 hover:text-blue-900"
            ...
            <div class="mt-1 text-xs text-ink-secondary">{{ t('admin.ops.email.recipientsHint') }}</div>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.rateLimitPerHour') }}</div>
            <input v-model.number="draft.alert.rate_limit_per_hour" type="number" min="0" max="100000" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.batchWindowSeconds') }}</div>
            <input v-model.number="draft.alert.batching_window_seconds" type="number" min="0" max="86400" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.includeResolved') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-ink-secondary">
      ...
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-3 text-sm font-semibold text-ink">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-ink-secondary">
          ...
          <div class="md:col-span-2">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.dailySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink-secondary">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.weeklySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink-secondary">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.errorDigest') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink-secondary">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.errorDigestMinCount') }}</div>
                <input v-model.number="draft.report.error_digest_min_count" type="number" min="0" max="1000000" class="input" />
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.accountHealth') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink-secondary">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-ink-secondary">{{ t('admin.ops.email.accountHealthThreshold') }}</div>
                <input v-model.number="draft.report.account_health_error_rate_threshold" type="number" min="0" max="100" step="0.1" class="input" />
              </div>
            </div>
            <div class="mt-2 text-xs text-ink-secondary">{{ t('admin.ops.email.reportHint') }}</div>
          </div>
        </div>
      </div>
    </div>
