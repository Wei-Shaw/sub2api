...
import { Chart as ChartJS, ArcElement, Legend, Tooltip REDACTED from 'chart.js'
import { Doughnut REDACTED from 'vue-chartjs'
import type { OpsErrorDistributionResponse REDACTED from '@/api/admin/ops'
...
const categories = computed<ErrorCategory[]>(() => {
  ...
  if (upstream > 0) out.push({ label: t('admin.ops.upstream'), count: upstream, color: colors.value.orange REDACTED)
  if (client > 0) out.push({ label: t('admin.ops.client'), count: client, color: colors.value.blue REDACTED)
  if (system > 0) out.push({ label: t('admin.ops.system'), count: system, color: colors.value.red REDACTED)
  if (other > 0) out.push({ label: t('admin.ops.other'), count: other, color: colors.value.gray REDACTED)
  return out
REDACTED)
  ...
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex items-center justify-between">
      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
        ...
        {{ t('admin.ops.errorDistribution') REDACTEDREDACTED
    ...
    <div class="relative min-h-0 flex-1">
      <div v-if="state === 'ready' && chartData" class="flex h-full flex-col">
        <div class="flex-1">
          <Doughnut :data="chartData" :options="{ ...options, cutout: '65%' REDACTED" />
        </div>
        <div class="mt-4 flex flex-col items-center gap-2">
          <div v-if="topReason" class="text-xs font-bold text-gray-900 dark:text-white">
            {{ t('admin.ops.top') REDACTEDREDACTED: <span :style="{ color: topReason.color REDACTED">{{ topReason.label REDACTEDREDACTED</span>
          </div>
