...
<template>
  <section class="card p-4 md:p-5">
    ...
    <div v-if="errorMessage" class="mb-4 rounded-lg border border-danger/40 bg-danger-tint px-3 py-2 text-xs text-red-600 dark:text-red-400">
      {{ errorMessage }}
    </div>

    <div v-if="loading" class="py-8 text-center text-sm text-ink-secondary">
    ...
    <div v-else class="space-y-3">
      <div class="overflow-hidden rounded-xl border border-line">
        <div class="max-h-[420px] overflow-auto">
          <div v-if="!isDesktopViewport" class="divide-y divide-line-subtle">
            <div v-for="row in items" :key="row.model" class="space-y-2 p-3">
              <div class="break-all text-xs font-medium text-ink">{{ row.model }}</div>
              <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-secondary">{{ t('admin.ops.openaiTokenStats.table.requestCount') }}</span>
                  <span class="text-ink-secondary">{{ formatInt(row.request_count) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-secondary">{{ t('admin.ops.openaiTokenStats.table.avgTokensPerSec') }}</span>
                  <span class="text-ink-secondary">{{ formatRate(row.avg_tokens_per_sec) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-secondary">{{ t('admin.ops.openaiTokenStats.table.avgFirstTokenMs') }}</span>
                  <span class="text-ink-secondary">{{ formatRate(row.avg_first_token_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-secondary">{{ t('admin.ops.openaiTokenStats.table.totalOutputTokens') }}</span>
                  <span class="text-ink-secondary">{{ formatInt(row.total_output_tokens) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-secondary">{{ t('admin.ops.openaiTokenStats.table.avgDurationMs') }}</span>
                  <span class="text-ink-secondary">{{ formatInt(row.avg_duration_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-secondary">{{ t('admin.ops.openaiTokenStats.table.requestsWithFirstToken') }}</span>
                  <span class="text-ink-secondary">{{ formatInt(row.requests_with_first_token) }}</span>
                </div>
              </div>
            </div>
          </div>
          <table v-else class="min-w-full text-left text-xs md:text-sm">
            <thead class="sticky top-0 z-10 bg-surface">
              <tr class="border-b border-line text-ink-secondary dark:text-gray-400">
            ...
            <tbody>
              <tr
                v-for="row in items"
                :key="row.model"
                class="border-b border-gray-100 text-ink-secondary last:border-b-0 dark:border-dark-800"
              >
      ...
      <div v-if="viewMode === 'topn'" class="mt-3 text-xs text-ink-secondary">
