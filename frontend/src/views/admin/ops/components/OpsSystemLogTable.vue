        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-dark-800">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              ...
              <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">{{ row.component || '-' REDACTEDREDACTED</td>
              <td class="max-w-[680px] px-3 py-2 text-xs text-gray-700 dark:text-gray-300">{{ row.message REDACTEDREDACTED</td>
              <td class="px-3 py-2 text-xs text-gray-600 dark:text-gray-400">
                <div>req: {{ row.request_id || '-' REDACTEDREDACTED</div>
                <div>client: {{ row.client_request_id || '-' REDACTEDREDACTED</div>
                <div>user: {{ row.user_id || '-' REDACTEDREDACTED / acc: {{ row.account_id || '-' REDACTEDREDACTED</div>
                <div>{{ row.platform || '-' REDACTEDREDACTED / {{ row.model || '-' REDACTEDREDACTED</div>
              </td>
            </tr>
          </tbody>
        </table>
