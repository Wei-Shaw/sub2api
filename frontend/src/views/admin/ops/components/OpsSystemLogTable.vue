...
const levelBadgeClass = (level: string) => {
  ...
  if (v === 'error' || v === 'fatal') return 'bg-red-100 text-danger dark:bg-red-900/30'
  if (v === 'warn' || v === 'warning') return 'bg-amber-100 text-warn dark:bg-amber-900/30'
...
<template>
  <section class="rounded-2xl border border-line bg-white p-4 shadow-sm dark:bg-dark-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-ink">{{ t('admin.ops.systemLogs.title') }}</h3>
        <p class="mt-1 text-xs text-ink-secondary">{{ t('admin.ops.systemLogs.description') }}</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-surface-sunken px-2 py-1 text-ink-secondary">{{ t('admin.ops.systemLogs.queue') }} {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-surface-sunken px-2 py-1 text-ink-secondary">{{ t('admin.ops.systemLogs.written') }} {{ health.written_count }}</span>
        <span class="rounded-sm border border-warn/40 bg-warn-tint px-1.5 py-0.5 text-warn">{{ t('admin.ops.systemLogs.dropped') }} {{ health.dropped_count }}</span>
        <span class="rounded-md bg-red-100 px-2 py-1 text-danger dark:bg-red-900/30">{{ t('admin.ops.systemLogs.failed') }} {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-line bg-gray-50 p-3 dark:bg-dark-800/70">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-ink-secondary">{{ t('admin.ops.systemLogs.runtimeConfig') }}</div>
        <span v-if="runtimeLoading" class="text-xs text-ink-secondary">{{ t('common.loading') }}</span>
      </div>
    ...
    <div class="overflow-hidden rounded-xl border border-line">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-ink-secondary">{{ t('common.loading') }}</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-ink-secondary">{{ t('admin.ops.systemLogs.empty') }}</div>
      <div v-else-if="!isDesktopViewport" class="divide-y divide-line-subtle">
        <div v-for="row in logs" :key="row.id" class="space-y-1.5 p-3">
          <div class="flex items-center justify-between gap-2">
            <span class="inline-flex rounded-sm px-2 py-0.5 text-xs font-semibold" :class="levelBadgeClass(row.level)">
              {{ row.level }}
            </span>
            <span class="text-xs text-ink-secondary">{{ formatTime(row.created_at) }}</span>
          </div>
          <div v-if="row.host" class="truncate text-xs text-ink-secondary" :title="row.host">
            {{ row.host }}
          </div>
          <div class="whitespace-normal break-all text-xs text-ink-secondary">
      ...
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-line">
          <thead class="bg-surface-sunken">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-ink-secondary">{{ t('admin.ops.systemLogs.time') }}</th>
              <th class="w-[160px] px-3 py-2 text-left text-[11px] font-semibold text-ink-secondary">{{ t('admin.ops.systemLogs.host') }}</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-ink-secondary">{{ t('admin.ops.systemLogs.level') }}</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-ink-secondary">{{ t('admin.ops.systemLogs.logDetails') }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-line-subtle">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              <td class="px-3 py-2 text-xs text-ink-secondary">{{ formatTime(row.created_at) }}</td>
              ...
              <td class="px-3 py-2 text-xs text-ink-secondary whitespace-normal break-all">
                {{ formatSystemLogDetail(row) }}
              </td>
            </tr>
          </tbody>
