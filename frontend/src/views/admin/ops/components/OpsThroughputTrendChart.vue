...
<template>
  <div class="flex h-full min-w-0 flex-col rounded-3xl bg-surface p-6 shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700">
    <div
    ...
    >
      <h3 class="flex min-w-0 items-center gap-2 text-sm font-bold text-ink">
      ...
      <div
        ...
        class="flex w-full min-w-0 flex-wrap items-center gap-2 text-xs text-ink-secondary sm:w-auto sm:justify-end"
      >
        ...
        <template v-if="!props.fullscreen">
          <button
            type="button"
            class="inline-flex shrink-0 items-center rounded-lg border border-line bg-surface px-2 py-1 text-[11px] font-semibold text-ink-secondary hover:bg-gray-50 disabled:opacity-50 dark:hover:bg-dark-800"
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-line bg-surface px-3 py-1 text-[11px] font-semibold text-ink-secondary hover:bg-gray-50 dark:hover:bg-dark-800"
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-ink-tertiary">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-ink-tertiary">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
