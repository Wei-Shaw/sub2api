...
import { Chart as ChartJS, CategoryScale, Filler, Legend, LineElement, LinearScale, PointElement, Title, Tooltip REDACTED from 'chart.js'
import { Line REDACTED from 'vue-chartjs'
import type { ChartComponentRef REDACTED from 'vue-chartjs'
import type { OpsThroughputGroupBreakdownItem, OpsThroughputPlatformBreakdownItem, OpsThroughputTrendPoint REDACTED from '@/api/admin/ops'
import type { ChartState REDACTED from '../types'
import { formatHistoryLabel, sumNumbers REDACTED from '../utils/opsFormatters'
...
const options = computed(() => {
  ...
  return {
    ...
    interaction: { intersect: false, mode: 'index' as const REDACTED,
    plugins: {
      legend: {
        position: 'top' as const,
        align: 'end' as const,
        labels: { color: c.text, usePointStyle: true, boxWidth: 6, font: { size: 10 REDACTED REDACTED
      REDACTED,
      tooltip: {
      ...
      zoom: {
        pan: { enabled: true, mode: 'x' as const, modifierKey: 'ctrl' as const REDACTED,
        zoom: { wheel: { enabled: true REDACTED, pinch: { enabled: true REDACTED, mode: 'x' as const REDACTED
      REDACTED
    REDACTED,
    scales: {
      x: {
        type: 'category' as const,
        grid: { display: false REDACTED,
        ticks: {
          color: c.text,
          font: { size: 10 REDACTED,
          maxTicksLimit: 8,
          autoSkip: true,
          autoSkipPadding: 10
        REDACTED
      ...
      y: {
        ...
        grid: { color: c.grid, borderDash: [4, 4] REDACTED,
        ticks: { color: c.text, font: { size: 10 REDACTED REDACTED
      REDACTED,
      y1: {
        type: 'linear' as const,
        display: true,
        ...
        grid: { display: false REDACTED,
        ticks: { color: c.green, font: { size: 10 REDACTED REDACTED
      REDACTED
    REDACTED
  REDACTED
REDACTED)
...
function downloadChart() {
  ...
  a.href = url
  a.download = `ops-throughput-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')REDACTED.png`
  a.click()
REDACTED
</script>
  ...
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex shrink-0 items-center justify-between">
      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
        ...
        {{ t('admin.ops.throughputTrend') REDACTEDREDACTED
        <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.throughputTrend')" />
      </h3>
      <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
        <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-blue-500"></span>{{ t('admin.ops.qps') REDACTEDREDACTED</span>
        <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-green-500"></span>{{ t('admin.ops.tpsK') REDACTEDREDACTED</span>
        <template v-if="!props.fullscreen">
          ...
          <button
            ...
            @click="resetZoom"
          >
            {{ t('admin.ops.charts.resetZoom') REDACTEDREDACTED
          </button>
          <button
            type="button"
            ...
            @click="downloadChart"
          >
            {{ t('admin.ops.charts.downloadChart') REDACTEDREDACTED
          </button>
        </template>
      </div>
    </div>
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_idREDACTED` REDACTEDREDACTED</span>
        <span class="text-gray-400 dark:text-gray-500">{{ formatNumber(g.request_count) REDACTEDREDACTED</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        <span class="uppercase">{{ p.platform REDACTEDREDACTED</span>
        <span class="text-gray-400 dark:text-gray-500">{{ formatNumber(p.request_count) REDACTEDREDACTED</span>
      </button>
    </div>
    ...
    <div class="min-h-0 flex-1">
      ...
      <div v-else class="flex h-full items-center justify-center">
        <div v-if="state === 'loading'" class="animate-pulse text-sm text-gray-400">{{ t('common.loading') REDACTEDREDACTED</div>
