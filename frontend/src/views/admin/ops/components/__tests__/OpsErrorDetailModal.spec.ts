...
vi.mock('@/stores', () => ({
  useAppStore: () => ({ showError: vi.fn() REDACTED)
REDACTED))
...
describe('OpsErrorDetailModal', () => {
  beforeEach(() => {
    ...
    mocks.listRequestErrorUpstreamErrors.mockResolvedValue({ items: [] REDACTED)
  REDACTED)
  ...
  it('prioritizes upstream root cause and deduplicates diagnostic payloads', async () => {
    mocks.getRequestErrorDetail.mockResolvedValue({
      ...
      error_body: '{"error":"same"REDACTED',
      upstream_error_message: 'provider rate limit exhausted',
      upstream_error_detail: '{"error":"same"REDACTED',
      upstream_errors: '[]',
    ...
    const wrapper = shallowMount(OpsErrorDetailModal, {
      props: { show: true, errorId: 1, errorType: 'request' REDACTED,
      global: {
        stubs: {
