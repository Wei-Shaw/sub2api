...
const TooltipStub = { template: '<div><slot /></div>' REDACTED
const PaginationStub = { template: '<div class="pagination-stub" />' REDACTED
...
function mountTable(row: Partial<OpsErrorLog>) {
  const base = {
    ...
    group_name: '',
    ...row,
  REDACTED as OpsErrorLog

  return mount(OpsErrorLogTable, {
    props: { rows: [base], total: 1, loading: false, page: 1, pageSize: 20 REDACTED,
    global: { stubs: { 'el-tooltip': TooltipStub, Pagination: PaginationStub REDACTED REDACTED,
  REDACTED)
REDACTED
...
describe('OpsErrorLogTable i18n keys exist in the errorLog namespace', () => {
  const locales: Record<string, any> = { zh: zhLocale, en: enLocale REDACTED
  for (const [name, msgs] of Object.entries(locales)) {
    it(`has apiKey & keyDeletedBadge for ${nameREDACTED`, () => {
