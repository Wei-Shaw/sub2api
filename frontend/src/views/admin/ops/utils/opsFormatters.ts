...
import { formatBytes REDACTED from '@/utils/format'
...
export function formatDateTime(dateStr: string): string {
  ...
  return `${String(d.getMonth() + 1).padStart(2, '0')REDACTED-${String(d.getDate()).padStart(2, '0')REDACTED ${String(d.getHours()).padStart(2, '0')REDACTED:${String(d.getMinutes()).padStart(2, '0')REDACTED:${String(d.getSeconds()).padStart(2, '0')REDACTED`
REDACTED
...
export function formatHistoryLabel(date: string | undefined, timeRange: string): string {
  ...
  if (minutes >= 24 * 60) {
    return `${String(d.getMonth() + 1).padStart(2, '0')REDACTED-${String(d.getDate()).padStart(2, '0')REDACTED ${String(d.getHours()).padStart(2, '0')REDACTED:${String(d.getMinutes()).padStart(2, '0')REDACTED`
  REDACTED
  return `${String(d.getHours()).padStart(2, '0')REDACTED:${String(d.getMinutes()).padStart(2, '0')REDACTED`
REDACTED

export function formatByteRate(bytes: number, windowMinutes: number): string {
  const seconds = Math.max(1, (windowMinutes || 1) * 60)
  return `${formatBytes(bytes / seconds, 1)REDACTED/s`
REDACTED
