        ...
        <template #actions="{ row REDACTED">
          <div class="flex items-center gap-1">
            <button @click="showOrderDetail(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-dark-600">
              ...
              {{ t('common.view') REDACTEDREDACTED
            </button>
            <button v-if="row.status === 'PENDING'" @click="handleCancelOrder(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-yellow-600 hover:bg-yellow-50 dark:text-yellow-400 dark:hover:bg-yellow-900/20">
              <Icon name="x" size="sm" />
              {{ t('payment.orders.cancel') REDACTEDREDACTED
            </button>
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">{{ creditedAmountSymbol REDACTEDREDACTED{{ row.refund_amount.toFixed(2) REDACTEDREDACTED</span>
            ...
            <button v-else-if="row.status === 'REFUND_FAILED'" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20">
              <Icon name="refresh" size="sm" />
              {{ t('payment.admin.retryRefund') REDACTEDREDACTED
            </button>
            <button v-else-if="row.status === 'REFUND_PENDING'" :disabled="refundQueryingIds.has(row.id)" @click="handleQueryRefund(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-orange-600 hover:bg-orange-50 disabled:opacity-60 dark:text-orange-400 dark:hover:bg-orange-900/20">
              ...
              {{ t('payment.admin.queryRefundStatus') REDACTEDREDACTED
            </button>
            <button v-else-if="row.status === 'COMPLETED' || row.status === 'PARTIALLY_REFUNDED'" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20">
              ...
              {{ t('payment.admin.refund') REDACTEDREDACTED
            </button>
          </div>
        </template>
      </OrderTable>
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.amount') REDACTEDREDACTED</p><p class="text-sm font-medium text-gray-900 dark:text-white">{{ creditedAmountSymbol REDACTEDREDACTED{{ selectedOrder.amount.toFixed(2) REDACTEDREDACTED</p></div>
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') REDACTEDREDACTED</p><p class="text-sm font-medium text-gray-900 dark:text-white">{{ paymentAmountSymbol(selectedOrder) REDACTEDREDACTED{{ selectedOrder.pay_amount.toFixed(2) REDACTEDREDACTED</p></div>
          ...
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundAmount') REDACTEDREDACTED</p><p class="text-sm font-medium text-red-600 dark:text-red-400">{{ creditedAmountSymbol REDACTEDREDACTED{{ selectedOrder.refund_amount.toFixed(2) REDACTEDREDACTED</p></div>
          ...
          <div v-if="selectedOrder.refund_requested_at" class="col-span-2 border-t border-gray-200 pt-3 dark:border-dark-600">
            <p class="mb-2 text-xs font-medium text-purple-600 dark:text-purple-400">{{ t('payment.admin.refundRequestInfo') REDACTEDREDACTED</p>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundRequestedAt') REDACTEDREDACTED</p>
                <p class="text-sm text-gray-700 dark:text-gray-300">{{ formatDateTime(selectedOrder.refund_requested_at) REDACTEDREDACTED</p>
              </div>
              <div>
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundRequestedBy') REDACTEDREDACTED</p>
                <p class="text-sm text-gray-700 dark:text-gray-300">#{{ selectedOrder.refund_requested_by REDACTEDREDACTED</p>
              </div>
              <div class="col-span-2">
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundRequestReason') REDACTEDREDACTED</p>
                <p class="text-sm text-gray-700 dark:text-gray-300">{{ selectedOrder.refund_request_reason REDACTEDREDACTED</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="orderAuditLogs.length > 0" class="border-t border-gray-200 pt-4 dark:border-dark-600">
          <p class="mb-2 text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('payment.admin.auditLogs') REDACTEDREDACTED</p>
          <div class="max-h-48 space-y-2 overflow-y-auto">
            <div v-for="log in orderAuditLogs" :key="log.id" class="rounded-lg border border-gray-100 bg-gray-50 p-2.5 dark:border-dark-600 dark:bg-dark-800">
              <div class="flex items-center justify-between">
                <span class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ log.action REDACTEDREDACTED</span>
                <span class="text-xs text-gray-400">{{ formatDateTime(log.created_at) REDACTEDREDACTED</span>
              </div>
              <div v-if="log.detail" class="mt-1 break-all text-xs text-gray-500 dark:text-gray-400">{{ log.detail REDACTEDREDACTED</div>
              <div v-if="log.operator" class="mt-1 text-xs text-gray-400">{{ t('payment.admin.operator') REDACTEDREDACTED: {{ log.operator REDACTEDREDACTED</div>
            </div>
          </div>
        </div>
      </div>
...
const orderFilters = reactive({ status: '', payment_type: '', order_type: '' REDACTED)
const orderPagination = reactive({ page: 1, page_size: 20, total: 0 REDACTED)
...
function handleOrderPageChange(page: number) { orderPagination.page = page; loadOrders() REDACTED
function handleOrderPageSizeChange(size: number) { orderPagination.page_size = size; orderPagination.page = 1; loadOrders() REDACTED
...
const statusFilterOptions = computed(() => [
  ...
  { value: 'REFUND_PENDING', label: t('payment.status.refund_pending') REDACTED,
...
async function showOrderDetail(order: PaymentOrder) {
  ...
  REDACTED catch (_err: unknown) { /* keep cached order data */ REDACTED
REDACTED
...
async function handleRefund(data: { amount: number; reason: string; deduct_balance: boolean; force: boolean REDACTED) {
  if (!selectedOrder.value) return
  refundSubmitting.value = true
  try {
    const res = await adminPaymentAPI.refundOrder(selectedOrder.value.id, { amount: data.amount, reason: data.reason, deduct_balance: data.deduct_balance, force: data.force REDACTED)
    if (res.data.success) {
...
async function handleQueryRefund(order: PaymentOrder) {
  ...
  try {
    ...
    if (res.data.success) {
      appStore.showSuccess(t('payment.admin.refundSuccess'))
    REDACTED else if (isRefundPendingWarning(res.data.warning)) {
      appStore.showSuccess(t('payment.admin.refundPending'))
    REDACTED else {
      appStore.showError(res.data.warning || t('common.error'))
    REDACTED
...
function formatDateTime(dateStr: string): string { return formatOrderDateTime(dateStr) REDACTED
