        ...
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              ...
              <button v-if="orderRefundable(row)" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20">
                <Icon name="check" size="sm" />
                {{ t('payment.admin.approveRefund') }}
              </button>
              <span v-else class="text-xs text-gray-400 dark:text-gray-500" data-testid="order-refund-unsupported">{{ t('payment.admin.refundUnsupported') }}</span>
            </template>
            <button v-else-if="row.status === 'REFUND_FAILED' && orderRefundable(row)" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20">
              <Icon name="refresh" size="sm" />
            ...
            <button v-else-if="(row.status === 'COMPLETED' || row.status === 'PARTIALLY_REFUNDED') && orderRefundable(row)" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20">
...
// Providers without a refund API never offer refund actions, whatever the
// order status is.
function orderRefundable(order: PaymentOrder): boolean {
  return providerSupportsRefund(order.provider_key || '')
}
