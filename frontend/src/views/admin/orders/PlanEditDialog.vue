<template>
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <!-- Group Info Preview -->
      <div v-if="selectedGroupInfo" class="rounded-lg border border-line bg-surface-sunken p-3">
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <div><span class="text-ink-secondary">{{ t('payment.admin.dailyLimit') }}:</span> <span class="ml-1 font-medium text-ink-secondary">{{ selectedGroupInfo.daily_limit_usd != null ?'$' + selectedGroupInfo.daily_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-ink-secondary">{{ t('payment.admin.weeklyLimit') }}:</span> <span class="ml-1 font-medium text-ink-secondary">{{ selectedGroupInfo.weekly_limit_usd != null ?'$' + selectedGroupInfo.weekly_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-ink-secondary">{{ t('payment.admin.monthlyLimit') }}:</span> <span class="ml-1 font-medium text-ink-secondary">{{ selectedGroupInfo.monthly_limit_usd != null ?'$' + selectedGroupInfo.monthly_limit_usd : t('payment.admin.unlimited') }}</span></div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="mt-1 text-xs text-ink-secondary">{{ t('payment.admin.currencyHint') }}</p>
        </div>
      </div>
      <div>
        <label class="input-label">{{ t('payment.admin.features') }}</label>
        <textarea v-model="planFeaturesText" rows="3" class="input" :placeholder="t('payment.admin.featuresPlaceholder')"></textarea>
        <p class="mt-1 text-xs text-ink-secondary">{{ t('payment.admin.featuresHint') }}</p>
      </div>
      <div class="flex items-center gap-3">
        <label class="text-sm text-ink-secondary">{{ t('payment.admin.forSale') }}</label>
        <button
          type="button"
          :class="[
