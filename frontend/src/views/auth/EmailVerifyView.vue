      ...
      <form v-else @submit.prevent="handleVerify" class="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="000000"
          />
          <p class="input-hint text-center">{{ t('auth.verificationCodeHint') REDACTEDREDACTED</p>
        </div>
...
import { computed, ref, onMounted, onUnmounted, watch REDACTED from 'vue'
...
onMounted(async () => {
  ...
  if (registerDataStr) {
    ...
    REDACTED
  REDACTED else if (activePendingSession) {
  ...
  try {
    const requestPayload = {
      ...
      tencent_captcha_randstr: tencentCaptchaEnabled.value
        ...
        : undefined
    REDACTED as Parameters<typeof sendVerifyCode>[0]
  ...
  try {
    if (isPendingOAuthFlow()) {
      const payload: Record<string, unknown> = {
        ...
        ...((turnstileEnabled.value || aliyunCaptchaEnabled.value) &&
        createAccountTurnstileToken.value
          ? { turnstile_token: createAccountTurnstileToken.value REDACTED
          : {REDACTED),
    ...
    REDACTED else {
      ...
      await authStore.register({
        ...
        ...(affCode.value ? { aff_code: affCode.value REDACTED : {REDACTED)
      REDACTED)
    REDACTED
