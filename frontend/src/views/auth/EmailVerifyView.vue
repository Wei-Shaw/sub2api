<template>
  <AuthLayout>
      ...
      <form v-else @submit.prevent="handleVerify" class="space-y-5">
        ...
        <div class="text-center">
          <button
            ...
            type="button"
            disabled
            class="cursor-not-allowed text-sm text-gray-400 dark:text-dark-300"
          >
    ...
    <template #footer>
      <button
        @click="handleBack"
        class="flex items-center gap-2 text-gray-500 transition-colors hover:text-gray-700 dark:text-dark-300 dark:hover:text-gray-300"
      >
...
const siteName = ref<string>('起源AI')
...
onMounted(async () => {
  ...
  try {
    ...
    siteName.value = settings.site_name || '起源AI'
