    ...
    <div class="space-y-6">
      ...
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.forgotPasswordTitle') REDACTEDREDACTED
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ t('auth.forgotPasswordHint') REDACTEDREDACTED
        </p>
      </div>
          ...
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <h3 class="text-lg font-semibold text-green-800 dark:text-green-200">
                {{ t('auth.resetEmailSent') REDACTEDREDACTED
              </h3>
              <p class="mt-2 text-sm text-green-700 dark:text-green-300">
                {{ t('auth.resetEmailSentHint') REDACTEDREDACTED
              </p>
            </div>
          </div>
        </div>
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-5">
        ...
        >
          ...
          {{ isLoading ? t('auth.sendingResetLink') : t('auth.sendResetLink') REDACTEDREDACTED
        </button>
      </form>
    </div>
...
import { computed, ref, reactive, onMounted, watch REDACTED from 'vue'
import { useI18n REDACTED from 'vue-i18n'
import { AuthLayout REDACTED from '@/components/layout'
import Icon from '@/components/icons/Icon.vue'
import TurnstileWidget from '@/components/CaptchaChallenge.vue'
import { useAppStore REDACTED from '@/stores'
import { getPublicSettings, forgotPassword REDACTED from '@/api/auth'
...
function validateForm(): boolean {
  ...
  if (!formData.email.trim()) {
    ...
    isValid = false
  REDACTED else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
