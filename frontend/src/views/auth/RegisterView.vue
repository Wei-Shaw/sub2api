    ...
    <div class="space-y-6">
      ...
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.createAccount') REDACTEDREDACTED
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ t('auth.signUpToStart', { siteName REDACTED) REDACTEDREDACTED
        </p>
      </div>
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <p class="text-sm text-amber-700 dark:text-amber-400">
            {{ t('auth.registrationDisabled') REDACTEDREDACTED
          </p>
        </div>
      </div>
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div>
          ...
          <div class="relative">
            ...
            <input
              ...
              class="input pl-11"
              :class="{ 'input-error': errors.email REDACTED"
        ...
        <div>
          <label for="password" class="input-label">
            {{ t('auth.passwordLabel') REDACTEDREDACTED
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
        ...
        <div v-if="invitationCodeEnabled">
          <label for="invitation_code" class="input-label">
            {{ t('auth.invitationCodeLabel') REDACTEDREDACTED
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
        ...
        <div v-else-if="affiliateEnabled" data-testid="affiliate-invitation-field">
          <label for="affiliate_code" class="input-label">
            {{ t('auth.invitationCodeLabel') REDACTEDREDACTED
            <span class="ml-1 text-xs font-normal text-gray-400 dark:text-dark-500">({{ t('common.optional') REDACTEDREDACTED)</span>
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
        ...
        <div v-if="promoCodeEnabled">
          <label for="promo_code" class="input-label">
            {{ t('auth.promoCodeLabel') REDACTEDREDACTED
            <span class="ml-1 text-xs font-normal text-gray-400 dark:text-dark-500">({{ t('common.optional') REDACTEDREDACTED)</span>
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
...
import { extractApiErrorCode, extractI18nErrorMessage REDACTED from '@/utils/apiError'
import {
  clearAffiliateReferralCode,
  loadAffiliateReferralCode,
  resolveAffiliateReferralCode
REDACTED from '@/utils/oauthAffiliate'
import type { LoginAgreementDocument REDACTED from '@/types'
...
REDACTED): void {
  ...
  loginAgreementRevision.value =
    ...
    `${loginAgreementUpdatedAt.valueREDACTED:${documents.map((doc) => `${doc.idREDACTED:${doc.titleREDACTED`).join('|')REDACTED`
...
function hasAcceptedLoginAgreement(revision: string): boolean {
  ...
  try {
    ...
    if (!raw) {
      return false
    REDACTED
    const parsed = JSON.parse(raw) as { revision?: string REDACTED
    ...
    const result = await startOAuthLogin(
      ...
      tencentCaptchaEnabled.value
        ? {
          ...
          REDACTED
        : { turnstile_token: proof.token REDACTED
    )
    ...
    if (emailVerifyEnabled.value) {
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          ...(affCode ? { aff_code: affCode REDACTED : {REDACTED)
        REDACTED)
      )
...
function buildRegistrationErrorMessage(error: unknown, fallback: string): string {
  ...
  REDACTED
  return buildAuthErrorMessage(error, { fallback REDACTED)
REDACTED
</script>
