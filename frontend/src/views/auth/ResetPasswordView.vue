  ...
  <AuthLayout>
    <div class="space-y-6">
      ...
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.resetPasswordTitle') REDACTEDREDACTED
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ t('auth.resetPasswordHint') REDACTEDREDACTED
        </p>
      </div>
      ...
      <div v-if="isInvalidLink" class="space-y-6">
          ...
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <h3 class="text-lg font-semibold text-amber-800 dark:text-amber-200">
                {{ t('auth.invalidResetLink') REDACTEDREDACTED
              </h3>
              <p class="mt-2 text-sm text-amber-700 dark:text-amber-300">
                {{ t('auth.invalidResetLinkHint') REDACTEDREDACTED
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="text-center">
          ...
          >
            {{ t('auth.requestNewResetLink') REDACTEDREDACTED
          </router-link>
        </div>
      </div>
      ...
      <div v-else-if="isSuccess" class="space-y-6">
          ...
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <h3 class="text-lg font-semibold text-green-800 dark:text-green-200">
                {{ t('auth.passwordResetSuccess') REDACTEDREDACTED
              </h3>
              <p class="mt-2 text-sm text-green-700 dark:text-green-300">
                {{ t('auth.passwordResetSuccessHint') REDACTEDREDACTED
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="text-center">
          ...
          >
            ...
            {{ t('auth.signIn') REDACTEDREDACTED
          </router-link>
        </div>
      </div>
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div>
          <label for="email" class="input-label">
            {{ t('auth.emailLabel') REDACTEDREDACTED
          </label>
          <div class="relative">
        ...
        <div>
          <label for="password" class="input-label">
            {{ t('auth.newPassword') REDACTEDREDACTED
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
            ...
            <input
              ...
              :class="{ 'input-error': errors.password REDACTED"
        ...
        <div>
          <label for="confirmPassword" class="input-label">
            {{ t('auth.confirmPassword') REDACTEDREDACTED
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="lock" size="md" class="text-gray-400 dark:text-dark-500" />
            ...
            <input
              ...
              :disabled="isLoading"
              class="input pl-11 pr-11"
              :class="{ 'input-error': errors.confirmPassword REDACTED"
        ...
        >
          ...
          {{ isLoading ? t('auth.resettingPassword') : t('auth.resetPassword') REDACTEDREDACTED
        </button>
      </form>
    </div>
    ...
    <template #footer>
      <p class="text-gray-500 dark:text-dark-400">
        {{ t('auth.rememberedPassword') REDACTEDREDACTED
        <router-link
          to="/login"
...
import { resetPassword REDACTED from '@/api/auth'
...
function validateForm(): boolean {
  ...
  if (!formData.password) {
    ...
    isValid = false
  REDACTED else if (formData.password.length < 6) {
  ...
  if (!formData.confirmPassword) {
    ...
    isValid = false
  REDACTED else if (formData.password !== formData.confirmPassword) {
...
async function handleSubmit(): Promise<void> {
  ...
  REDACTED catch (error: unknown) {
    const err = error as { message?: string; response?: { data?: { detail?: string; code?: string REDACTED REDACTED REDACTED
    ...
    REDACTED else if (err.response?.data?.detail) {
