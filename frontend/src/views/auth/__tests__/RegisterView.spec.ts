...
describe('RegisterView invitation layout', () => {
  ...
  it('keeps the shared promo and referral field before Turnstile', async () => {
    const wrapper = mountRegister()
    await flushPromises()

    const referralInput = wrapper.get('#promo_code')
    const turnstile = wrapper.get('[data-testid="registration-turnstile"]')

    expect(wrapper.find('#affiliate_code').exists()).toBe(false)
    expect(wrapper.get('label[for="promo_code"]').text()).toContain('common.optional')
    expect(
      referralInput.element.compareDocumentPosition(turnstile.element) &
        Node.DOCUMENT_POSITION_FOLLOWING
    ).toBeTruthy()
  })

  it('keeps mandatory invitation and optional referral inputs distinct', async () => {
