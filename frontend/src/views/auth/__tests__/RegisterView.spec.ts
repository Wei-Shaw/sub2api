...
vi.mock('vue-router', () => ({
  useRouter: () => ({ push: vi.fn() REDACTED),
  useRoute: () => ({ query: {REDACTED REDACTED)
REDACTED))
  ...
  return mount(RegisterView, {
    global: {
      stubs: {
        ...
        Icon: true,
        TurnstileWidget: { template: '<div data-testid="turnstile-widget" />' REDACTED,
...
describe('RegisterView invitation layout', () => {
  ...
  it('still submits whitelisted email domains when the domain quota switch is disabled', async () => {
    ...
    expect(registerMock).toHaveBeenCalledWith(
      expect.objectContaining({ email: 'user@allowed.com' REDACTED)
    )
    expect(showErrorMock).not.toHaveBeenCalled()
  REDACTED)
REDACTED)
