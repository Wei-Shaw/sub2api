<template>
  <div class="min-h-screen bg-gray-50 text-gray-900 dark:bg-dark-950 dark:text-white">
    ...
    <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:py-10">
      ...
      >
        <h1 class="text-lg font-semibold">{{ t('legal.loadFailed') REDACTEDREDACTED</h1>
        <p class="mt-2 text-sm">{{ t('legal.retryLater') REDACTEDREDACTED</p>
      </section>
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('legal.notFound') REDACTEDREDACTED</h1>
            <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-dark-300">
              {{ t('legal.notFoundDescription') REDACTEDREDACTED
            </p>
          </div>
        </div>
      </section>
      ...
      <article v-else>
        <div class="mb-8 border-b border-gray-200 pb-6 dark:border-dark-700">
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0">
              <p class="text-sm font-medium text-primary-700 dark:text-primary-300">{{ documentTypeLabel REDACTEDREDACTED</p>
              <h1 class="mt-2 break-words text-2xl font-bold tracking-normal text-gray-950 dark:text-white sm:text-3xl">
                {{ currentDocument.title REDACTEDREDACTED
              </h1>
              <p v-if="updatedAt" class="mt-3 text-sm text-gray-500 dark:text-dark-400">
                {{ t('legal.updatedAt', { date: updatedAt REDACTED) REDACTEDREDACTED
              </p>
            </div>
          </div>
        </div>
        ...
        >
          {{ t('legal.empty') REDACTEDREDACTED
        </div>
      </article>
    </main>
  </div>
...
import type { LoginAgreementDocument, PublicSettings REDACTED from '@/types'
