      ...
      <template #filters>
        <div class="flex flex-col gap-3">
          <div class="flex flex-col gap-3 2xl:flex-row 2xl:items-center 2xl:justify-between">
            ...
            <div class="flex flex-wrap items-center justify-start gap-2 sm:justify-end 2xl:flex-shrink-0">
              ...
              <button type="button" class="btn btn-secondary" @click="showGuideModal = true">
                ...
                {{ t('batchImage.actions.usageGuide') REDACTEDREDACTED
              </button>
              <button type="button" class="btn btn-primary" @click="openCreateModal">
                <Icon name="plus" size="md" class="mr-2" />
                {{ t('batchImage.actions.createJob') REDACTEDREDACTED
              </button>
            </div>
          </div>
          ...
          >
            ...
            >
              <template #count>
                <span class="font-medium text-gray-900 dark:text-white">{{ selectedJobIds.size REDACTEDREDACTED</span>
              </template>
            </i18n-t>
            <div class="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {{ t('batchImage.actions.downloadSelected') REDACTEDREDACTED
              </button>
              <button
                type="button"
              ...
              >
                ...
                {{ t('batchImage.actions.deleteRecords') REDACTEDREDACTED
              </button>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <template #cell-id="{ row REDACTED">
	            <div class="flex w-[220px] items-start gap-1" :class="row.is_child ? 'pl-6' : ''">
	              <button
	                ...
	                :title="expandedParentIds.has(row.id) ? t('batchImage.list.collapseChildren') : t('batchImage.list.expandChildren', { n: row.child_count REDACTED, row.child_count)"
                ...
                >
                  <span class="min-w-0 truncate">{{ row.task_name || defaultTaskName(row.created_at) REDACTEDREDACTED</span>
                  <span v-if="row.child_count > 0 && !row.is_child" class="flex-shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-xs font-normal text-gray-600 dark:bg-dark-700 dark:text-gray-300">
                    {{ t('batchImage.list.childCount', { n: row.child_count REDACTED, row.child_count) REDACTEDREDACTED
                  </span>
          ...
          <template #cell-model="{ row REDACTED">
	            <div class="mx-auto max-w-[180px] text-center">
	              <p class="truncate text-sm text-gray-700 dark:text-gray-300" :title="row.model">{{ row.model REDACTEDREDACTED</p>
	            </div>
	          </template>

          <template #cell-api_key_name="{ value REDACTED">
            <span class="block truncate text-center text-sm text-gray-700 dark:text-gray-300">
              {{ value || t('batchImage.list.keyNotRecorded') REDACTEDREDACTED
            </span>
          </template>
          ...
          <template #cell-status="{ row REDACTED">
            <div class="flex justify-center">
              <span :class="statusBadgeClass(displayJob(row))" class="badge">
                {{ statusLabel(displayJob(row)) REDACTEDREDACTED
              </span>
            </div>
          </template>

          <template #cell-counts="{ row REDACTED">
            <div class="flex items-center justify-center gap-2 text-sm tabular-nums">
              <span class="text-emerald-600 dark:text-emerald-300">{{ displayJob(row).success_count REDACTEDREDACTED</span>
              <span class="text-gray-300 dark:text-dark-500">/</span>
              <span :class="displayJob(row).fail_count > 0 ? 'text-red-600 dark:text-red-300' : 'text-gray-400 dark:text-gray-500'">{{ displayJob(row).fail_count REDACTEDREDACTED</span>
              <span class="text-xs text-gray-400 dark:text-gray-500">{{ t('batchImage.list.totalCount', { n: displayJob(row).item_count REDACTED) REDACTEDREDACTED</span>
            </div>
          </template>

          <template #cell-cost="{ row REDACTED">
            <span class="block text-center text-sm text-gray-700 dark:text-gray-300">
              {{ costLabel(displayJob(row)) REDACTEDREDACTED
            </span>
          </template>

          <template #cell-downloaded="{ row REDACTED">
            <span class="block text-center text-sm" :class="row.downloaded_at ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-500 dark:text-gray-400'">
              {{ row.downloaded_at ? formatDate(row.downloaded_at) : t('batchImage.list.notDownloaded') REDACTEDREDACTED
            </span>
          </template>
              ...
              >
                <Icon
                  :name="isDownloadingJob(row.id) ? 'refresh' : 'download'"
	                ...
	                />
                <span class="text-xs">{{ t('batchImage.actions.download') REDACTEDREDACTED</span>
	              </button>
              <div v-if="canRetry(row) || canDeleteRecord(row)">
                <button
                  ...
                  :class="{ 'bg-gray-100 text-gray-900 dark:bg-dark-700 dark:text-white': openMoreJobId === row.id REDACTED"
                  :title="t('batchImage.actions.moreActions')"
                  @click.stop="toggleMoreMenu(row, $event)"
                >
                  <Icon name="more" size="sm" />
                  <span class="text-xs">{{ t('common.more') REDACTEDREDACTED</span>
                </button>
              </div>
	            </div>
	          </template>
          ...
          <template #empty>
            <div class="flex min-h-[260px] flex-col items-center justify-center py-6 md:min-h-[300px]">
              ...
              <p class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ t('batchImage.list.empty') REDACTEDREDACTED</p>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ t('batchImage.list.emptyHint') REDACTEDREDACTED
              </p>
            </div>
          </template>
        </DataTable>
        ...
        >
          <div class="flex flex-wrap items-center gap-3 text-sm text-gray-700 dark:text-gray-300">
            <i18n-t keypath="batchImage.pagination.pageNumber" tag="span" scope="global">
              <template #page>
                <span class="font-medium">{{ pagination.page REDACTEDREDACTED</span>
              </template>
            </i18n-t>
            <i18n-t keypath="batchImage.pagination.pageItems" tag="span" scope="global">
              <template #count>
                <span class="font-medium">{{ visibleBatchJobs.length REDACTEDREDACTED</span>
              </template>
            </i18n-t>
            <div class="flex items-center gap-2">
              <span>{{ t('pagination.perPage') REDACTEDREDACTED</span>
              <Select
        ...
        <template v-for="job in batchJobs" :key="job.id">
          <template v-if="job.id === openMoreJobId">
            ...
            >
              ...
              {{ t('batchImage.actions.retryFailedItems') REDACTEDREDACTED
            </button>
            <button
    ...
    <Teleport to="body">
      ...
      >
        <div class="mb-2 flex items-center justify-between gap-3">
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('batchImage.promptPopover.title') REDACTEDREDACTED</span>
          <button
            type="button"
        ...
        <p class="max-h-48 overflow-y-auto whitespace-pre-wrap break-words leading-6 selection:bg-primary-100 selection:text-primary-900 dark:selection:bg-primary-900/60 dark:selection:text-primary-100">
          {{ promptPopover.text REDACTEDREDACTED
        </p>
      </div>
    </Teleport>
    ...
    <BaseDialog :show="!!currentJob" :title="t('batchImage.detail.title')" width="extra-wide" @close="closeDetail">
      <div v-if="currentJob" class="space-y-4">
        <div class="rounded-lg border border-gray-200 bg-gray-50/70 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/40">
          <div class="grid gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="min-w-0 text-center">
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('common.status') REDACTEDREDACTED</p>
              <div class="mt-1 flex justify-center">
                <span :class="statusBadgeClass(currentDisplayJob || currentJob)" class="badge whitespace-nowrap">
                  {{ statusLabel(currentDisplayJob || currentJob) REDACTEDREDACTED
                </span>
              </div>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ hasChildJobs(currentJob.id) ? t('batchImage.detail.aggregatedResult') : t('batchImage.detail.result') REDACTEDREDACTED</p>
              <p class="mt-1 flex items-center justify-center gap-2 font-medium tabular-nums">
              <span class="text-emerald-600 dark:text-emerald-300">{{ (currentDisplayJob || currentJob).success_count REDACTEDREDACTED</span>
              <span class="text-gray-300 dark:text-dark-500">/</span>
              <span :class="(currentDisplayJob || currentJob).fail_count > 0 ? 'text-red-600 dark:text-red-300' : 'text-gray-400 dark:text-gray-500'">{{ (currentDisplayJob || currentJob).fail_count REDACTEDREDACTED</span>
            </p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('batchImage.detail.cost') REDACTEDREDACTED</p>
              <p class="mt-1 truncate font-medium text-gray-900 dark:text-white">{{ costLabel(currentDisplayJob || currentJob) REDACTEDREDACTED</p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('batchImage.detail.downloadStatus') REDACTEDREDACTED</p>
              <p class="mt-1 truncate font-medium text-gray-900 dark:text-white">
              {{ currentJob.downloaded_at ? formatDate(currentJob.downloaded_at) : t('batchImage.list.notDownloaded') REDACTEDREDACTED
            </p>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('batchImage.detail.items') REDACTEDREDACTED</h3>
          ...
          <table class="w-full min-w-[860px] table-fixed divide-y divide-gray-200 text-sm dark:divide-dark-700">
            ...
            <thead class="bg-gray-50 dark:bg-dark-800/80">
              <tr>
                ...
                <th class="px-3 py-3 text-center text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('common.status') REDACTEDREDACTED</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('batchImage.detail.preview') REDACTEDREDACTED</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('batchImage.detail.result') REDACTEDREDACTED</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
              ...
              >
                <td class="px-3 py-2.5 text-center">
                  ...
                  >
                    {{ item.custom_id REDACTEDREDACTED
                  </span>
                </td>
                <td class="px-3 py-2.5 text-left" :class="isRecoveredOriginalFailure(item) ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'">
                  ...
                  >
                    {{ item.prompt_preview || '-' REDACTEDREDACTED
                  </div>
                </td>
                <td class="px-3 py-2.5 text-center">
                  <span :class="itemDisplayStatusBadgeClass(item)" class="badge max-w-full truncate whitespace-nowrap" :title="itemDisplayStatusLabel(item)">
                    {{ itemDisplayStatusLabel(item) REDACTEDREDACTED
                  </span>
                </td>
                <td class="px-3 py-2.5 text-center">
                  <div class="mx-auto h-12 w-12 overflow-hidden rounded-md border border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-800">
                    <button
                      ...
                      :title="t('batchImage.detail.previewZoom', { id: item.custom_id REDACTED)"
                ...
                <td class="px-3 py-2.5 text-center">
                  ...
                  >
                    {{ itemResultLabel(item) REDACTEDREDACTED
                  </span>
                </td>
              </tr>
            </tbody>
        ...
        <div v-else class="rounded-lg border border-dashed border-gray-200 py-10 text-center dark:border-dark-700">
          ...
          <p class="text-sm font-medium text-gray-700 dark:text-gray-200">
            {{ loadingItems ? t('batchImage.detail.loadingItems') : t('batchImage.detail.noItems') REDACTEDREDACTED
          </p>
          <p v-if="!loadingItems" class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {{ t('batchImage.detail.noItemsHint') REDACTEDREDACTED
          </p>
        </div>
      </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
	          <button type="button" class="btn btn-secondary" :disabled="!currentJob || !canCancel(currentJob) || cancelling" @click="cancelSelected">
	            ...
	            {{ t('batchImage.actions.cancelJob') REDACTEDREDACTED
	          </button>
	          <button
          ...
          >
            ...
            />
            {{ t('batchImage.actions.downloadZip') REDACTEDREDACTED
          </button>
        </div>
      </template>
    </BaseDialog>
    ...
    <BaseDialog :show="!!previewImageItem" :title="previewImageItem?.custom_id || t('batchImage.imagePreview.title')" width="extra-wide" :z-index="60" @close="closeImagePreview">
      <div class="space-y-3">
        <div class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100">
          {{ t('batchImage.imagePreview.notice') REDACTEDREDACTED
        </div>
      ...
      <form class="space-y-5" @submit.prevent="submitJob">
        <div class="grid gap-4 md:grid-cols-2">
          <div class="md:col-span-2">
            <label class="input-label">{{ t('batchImage.create.taskName') REDACTEDREDACTED</label>
            <input
          ...
          <div class="md:col-span-2">
            ...
            <select v-model.number="form.apiKeyId" class="input" :disabled="loadingKeys">
              <option :value="0">{{ loadingKeys ? t('batchImage.create.loadingKeys') : t('batchImage.create.selectKeyPlaceholder') REDACTEDREDACTED</option>
              <option v-for="key in geminiApiKeys" :key="key.id" :value="key.id">
                {{ key.name REDACTEDREDACTED · {{ key.group?.name || 'Gemini' REDACTEDREDACTED
              </option>
            </select>
            <p v-if="!loadingKeys && geminiApiKeys.length === 0" class="input-hint text-amber-600 dark:text-amber-400">
              {{ t('batchImage.create.noKeysHint') REDACTEDREDACTED
            </p>
          </div>

          <div>
            <label class="input-label">{{ t('batchImage.create.model') REDACTEDREDACTED</label>
            <select v-model="form.model" class="input" :disabled="loadingModels || availableBatchImageModels.length === 0">
              <option v-if="loadingModels" value="">{{ batchImageText('loadingModels') REDACTEDREDACTED</option>
              <option v-else-if="availableBatchImageModels.length === 0" value="">{{ batchImageText('noModels') REDACTEDREDACTED</option>
              <option v-for="model in availableBatchImageModels" :key="model.value" :value="model.value">
                {{ model.label REDACTEDREDACTED
              </option>
            </select>
            <p v-if="modelLoadError" class="input-hint text-amber-600 dark:text-amber-400">
              {{ modelLoadError REDACTEDREDACTED
            </p>
            <p v-else-if="selectedApiKey && !loadingModels && availableBatchImageModels.length === 0" class="input-hint text-amber-600 dark:text-amber-400">
              {{ batchImageText('noModelsHint') REDACTEDREDACTED
            </p>
          </div>

          <div>
            <label class="input-label">{{ t('batchImage.create.imageSize') REDACTEDREDACTED</label>
            <div class="input flex items-center bg-gray-50 text-gray-600 dark:bg-dark-900 dark:text-gray-300">
              1K
            </div>
            <p class="input-hint">{{ t('batchImage.create.imageSizeHint') REDACTEDREDACTED</p>
          </div>

          <div>
            <label class="input-label">{{ t('batchImage.create.outputFormat') REDACTEDREDACTED</label>
          ...
          <div>
            <label class="input-label">{{ t('batchImage.create.estimatedOutput') REDACTEDREDACTED</label>
            <div class="input flex items-center bg-gray-50 text-gray-600 dark:bg-dark-900 dark:text-gray-300">
              {{ t('batchImage.create.estimatedOutputValue', { images: estimatedOutputCount, prompts: promptRows.length REDACTED) REDACTEDREDACTED
            </div>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('batchImage.create.promptAdded', { count: promptRows.length REDACTED) REDACTEDREDACTED</span>
          </div>
          <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-700">
            ...
            <div class="mt-2 grid gap-2 md:grid-cols-[minmax(0,1fr)_112px_132px_112px] md:items-center">
              ...
              >
                <option v-for="count in outputCountOptions" :key="count" :value="count">
                  {{ t('batchImage.create.outputCountOption', { n: count REDACTED, count) REDACTEDREDACTED
                </option>
              </select>
              <label
              ...
              >
                ...
                {{ t('batchImage.create.referenceImage') REDACTEDREDACTED
                <input
                  type="file"
            ...
            <div v-if="referenceImageDrafts.length" class="mt-3 flex flex-wrap gap-2">
              <span
                ...
                :key="`${ref.nameREDACTED-${refIndexREDACTED`"
              ...
              >
                <span class="max-w-[180px] truncate">{{ ref.name REDACTEDREDACTED</span>
            ...
            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {{ t('batchImage.create.limitsHint', { maxPerItem: BATCH_IMAGE_MAX_OUTPUTS_PER_ITEM, maxPerJob: BATCH_IMAGE_MAX_OUTPUTS_PER_JOB, refLimit: selectedModelReferenceLimit REDACTED) REDACTEDREDACTED
            </p>
          </div>
          <div v-if="promptRows.length" class="overflow-hidden rounded-lg border border-gray-200 dark:border-dark-700">
            ...
            >
              <span class="w-20 flex-shrink-0 font-mono text-xs text-gray-500 dark:text-gray-400">{{ row.custom_id REDACTEDREDACTED</span>
              <p class="min-w-0 flex-1 truncate text-sm text-gray-800 dark:text-gray-100">{{ row.prompt REDACTEDREDACTED</p>
              <span v-if="row.output_count > 1" class="flex-shrink-0 text-xs text-gray-500 dark:text-gray-400">
                x{{ row.output_count REDACTEDREDACTED
              </span>
              <span v-if="row.reference_images.length" class="flex-shrink-0 text-xs text-gray-500 dark:text-gray-400">
                {{ t('batchImage.create.referenceCount', { n: row.reference_images.length REDACTED, row.reference_images.length) REDACTEDREDACTED
              </span>
          ...
          <div v-else class="rounded-lg border border-dashed border-gray-200 px-3 py-6 text-center text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400">
            {{ t('batchImage.create.noPrompts') REDACTEDREDACTED
          </div>
        </div>
	        ...
	        <div class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm leading-6 text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100">
	          {{ t('batchImage.create.cancelNotice') REDACTEDREDACTED
	        </div>
	        <div v-if="submitting" class="rounded-lg border border-sky-200 bg-sky-50 p-3 text-sm leading-6 text-sky-800 dark:border-sky-800 dark:bg-sky-950/30 dark:text-sky-100">
	          {{ t('batchImage.create.submittingNotice') REDACTEDREDACTED
	        </div>
	      </form>

      <template #footer>
        <div class="flex justify-end gap-3">
          <button type="button" class="btn btn-secondary" :disabled="submitting" @click="closeCreateModal">{{ t('common.cancel') REDACTEDREDACTED</button>
            ...
            {{ submitting ? t('common.submitting') : t('batchImage.actions.submitJob') REDACTEDREDACTED
          </button>
        </div>
      </template>
    </BaseDialog>
    ...
    <BaseDialog :show="showGuideModal" :title="t('batchImage.guide.title')" width="wide" @close="showGuideModal = false">
	      <div class="space-y-5">
	        <section class="space-y-3">
	          <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('batchImage.guide.uiTitle') REDACTEDREDACTED</h3>
	          <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm leading-6 text-gray-700 dark:border-dark-700 dark:bg-dark-900/50 dark:text-gray-200">
	            <p>{{ t('batchImage.guide.step1') REDACTEDREDACTED</p>
	            <p>{{ t('batchImage.guide.step2') REDACTEDREDACTED</p>
	            <p>{{ t('batchImage.guide.step3') REDACTEDREDACTED</p>
	            <p>{{ t('batchImage.guide.step4') REDACTEDREDACTED</p>
	          </div>
	        </section>
	        <section class="space-y-3">
	          <div class="flex flex-wrap items-center justify-between gap-3">
	            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('batchImage.guide.skillTitle') REDACTEDREDACTED</h3>
	            <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('batchImage.guide.skillDesc') REDACTEDREDACTED</p>
	          </div>
	        <textarea
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <button type="button" class="btn btn-secondary" @click="showGuideModal = false">{{ t('common.close') REDACTEDREDACTED</button>
          <button type="button" class="btn btn-primary" @click="copyInstruction">
            <Icon name="copy" size="sm" class="mr-2" />
            {{ t('batchImage.actions.copyInstruction') REDACTEDREDACTED
          </button>
        </div>
      </template>
    </BaseDialog>
...
import { computed, onBeforeUnmount, onMounted, reactive, ref, watch REDACTED from 'vue'
...
import { getPersistedPageSize, setPersistedPageSize REDACTED from '@/composables/usePersistedPageSize'
import { useAppStore REDACTED from '@/stores/app'
import { keysAPI REDACTED from '@/api'
import {
  cancelBatchImageJob,
...
const outputCountOptions = Array.from({ length: BATCH_IMAGE_MAX_OUTPUTS_PER_ITEM REDACTED, (_, index) => index + 1)
const batchPageSizeOptions: SelectOption[] = [20, 50, 100].map(size => ({ value: size, label: String(size) REDACTED))
...
const columns = computed<Column[]>(() => [
  { key: 'select', label: '', sortable: false, class: 'w-12 text-center' REDACTED,
  { key: 'id', label: t('batchImage.columns.taskName'), sortable: false, class: 'w-[240px] max-w-[240px]' REDACTED,
  { key: 'model', label: t('batchImage.columns.model'), sortable: false, class: 'w-[180px] max-w-[180px] text-center' REDACTED,
  { key: 'api_key_name', label: t('batchImage.columns.apiKey'), sortable: false, class: 'w-40 max-w-40 text-center' REDACTED,
  { key: 'status', label: t('common.status'), sortable: false, class: 'w-28 text-center' REDACTED,
  { key: 'counts', label: t('batchImage.columns.result'), sortable: false, class: 'w-32 text-center' REDACTED,
  { key: 'cost', label: t('batchImage.columns.cost'), sortable: false, class: 'w-36 text-center' REDACTED,
  { key: 'downloaded', label: t('batchImage.columns.downloadStatus'), sortable: false, class: 'w-40 text-center' REDACTED,
  { key: 'actions', label: t('common.actions'), sortable: false, class: 'w-40 text-center' REDACTED,
])

const statusFilterOptions = computed<SelectOption[]>(() => [
  { value: '', label: t('batchImage.filters.allStatuses') REDACTED,
  { value: 'queued', label: t('batchImage.status.queued') REDACTED,
  { value: 'running', label: t('batchImage.status.running') REDACTED,
  { value: 'processing_results', label: t('batchImage.status.processingResults') REDACTED,
  { value: 'settling', label: t('batchImage.status.settling') REDACTED,
  { value: 'completed', label: t('batchImage.status.completed') REDACTED,
  { value: 'failed', label: t('batchImage.status.failed') REDACTED,
  { value: 'cancelled', label: t('batchImage.status.cancelled') REDACTED,
  { value: 'output_deleted', label: t('batchImage.status.outputDeleted') REDACTED,
])

const downloadFilterOptions = computed<SelectOption[]>(() => [
  { value: '', label: t('batchImage.filters.allDownloadStates') REDACTED,
  { value: 'true', label: t('batchImage.filters.downloaded') REDACTED,
  { value: 'false', label: t('batchImage.filters.notDownloaded') REDACTED,
])
...
const itemPreviewUrls = reactive<Record<string, string>>({REDACTED)
...
const availableBatchImageModels = ref<Array<{ value: string; label: string REDACTED>>([])
const modelLoadError = ref('')
const openMoreJobId = ref('')
const moreMenuStyle = ref<Record<string, string>>({REDACTED)
const promptPopover = reactive({
  visible: false,
  text: '',
  style: {REDACTED as Record<string, string>,
REDACTED)
...
const apiKeyFilterOptions = computed<SelectOption[]>(() => [
  { value: '', label: t('batchImage.filters.allApiKeys') REDACTED,
  ...geminiApiKeys.value.map(key => ({
    value: String(key.id),
    label: key.name || `API Key #${key.idREDACTED`,
  REDACTED)),
])
...
const visibleBatchJobs = computed(() => {
  ...
  for (const job of batchJobs.value.filter(item => !item.parent_batch_id)) {
    ...
    if (expandedParentIds.value.has(job.id)) {
      rows.push(...(childrenByParent.value.get(job.id) || []).map(child => ({ ...child, is_child: true REDACTED)))
    REDACTED
  REDACTED
  return rows
REDACTED)
...
const parsedItems = computed<BatchImageSubmitItem[]>(() => {
  ...
  return promptRows.value
    .map((row, index) => {
      const customID = uniqueCustomID(row.custom_id || `img_${String(index + 1).padStart(3, '0')REDACTED`, used, index)
      const item: BatchImageSubmitItem = { custom_id: customID, prompt: row.prompt.trim() REDACTED
...
API 调用规范：
- 模型：GET ${joinEndpointPath(endpointBase.value, '/v1/images/batches/models')REDACTED
- 提交：POST ${joinEndpointPath(endpointBase.value, '/v1/images/batches')REDACTED
- 查询：GET ${joinEndpointPath(endpointBase.value, '/v1/images/batches/{idREDACTED')REDACTED
- 明细：GET ${joinEndpointPath(endpointBase.value, '/v1/images/batches/{idREDACTED/items')REDACTED
- 下载：GET ${joinEndpointPath(endpointBase.value, '/v1/images/batches/{idREDACTED/download')REDACTED
- 取消：POST ${joinEndpointPath(endpointBase.value, '/v1/images/batches/{idREDACTED/cancel')REDACTED
...
function joinEndpointPath(base: string, path: string): string {
  return `${base.replace(/\/+$/, '')REDACTED/${path.replace(/^\/+/, '')REDACTED`
REDACTED

function uniqueCustomID(raw: string, used: Set<string>, index: number): string {
  const base = raw.replace(/[^\w.-]+/g, '_').replace(/^_+|_+$/g, '') || `img_${String(index + 1).padStart(3, '0')REDACTED`
  let candidate = base
  let suffix = 2
  while (used.has(candidate)) {
    candidate = `${baseREDACTED_${suffixREDACTED`
    suffix += 1
  REDACTED
  used.add(candidate)
  return candidate
...
function addPromptRow() {
  ...
  const customID = uniqueCustomID(customIdDraft.value || `img_${String(promptRows.value.length + 1).padStart(3, '0')REDACTED`, used, promptRows.value.length)
  promptRows.value = [
    ...promptRows.value,
    {
      localId: `${Date.now()REDACTED-${Math.random().toString(36).slice(2, 8)REDACTED`,
      custom_id: customID,
      prompt,
      output_count: outputCount,
      reference_images: referenceImageDrafts.value.map(({ name: _name, size: _size, ...ref REDACTED) => ref),
    REDACTED,
  ]
  promptDraft.value = ''
  customIdDraft.value = ''
...
async function handleReferenceImageFiles(event: Event) {
  ...
  if (slots <= 0) {
    appStore.showError(t('batchImage.create.refLimitReached', { limit REDACTED))
    return
  REDACTED
  const accepted = files.slice(0, slots)
  if (accepted.length < files.length) {
    appStore.showError(t('batchImage.create.refLimitExceededIgnored', { limit REDACTED))
  REDACTED
  ...
  for (const file of accepted) {
    ...
    if (file.size > 10 * 1024 * 1024) {
      appStore.showError(t('batchImage.create.refFileTooLarge', { name: file.name REDACTED))
      continue
    REDACTED
...
async function loadApiKeys() {
  ...
  try {
    const response = await keysAPI.list(1, 100, { status: 'active', sort_by: 'created_at', sort_order: 'desc' REDACTED)
  ...
  try {
    ...
    availableBatchImageModels.value = (result.data || [])
      ...
      .filter((model) => {
        ...
        return true
      REDACTED)
      .map(model => ({ value: model, label: model REDACTED))
...
function applyChildCounts(rows: BatchImageJobRow[]) {
  ...
  REDACTED
  return rows.map(row => ({ ...row, child_count: counts.get(row.id) || 0 REDACTED))
REDACTED
...
function toggleMoreMenu(job: BatchImageJobRow, event: MouseEvent) {
  ...
  moreMenuStyle.value = {
    left: `${leftREDACTEDpx`,
    top: `${Math.max(margin, top)REDACTEDpx`,
  REDACTED
...
function upsertJob(job: BatchImageJob) {
  ...
  if (index >= 0) {
    ...
    rows[index] = { ...next, is_child: rows[index].is_child REDACTED
  ...
  if (!key) return
	  ...
	  try {
        ...
        items: parsedItems.value,
	      REDACTED,
	      `sub2api-ui-${Date.now()REDACTED-${Math.random().toString(36).slice(2, 10)REDACTED`,
	    )
...
async function retryFailedJob(job: BatchImageJobRow | BatchImageJob) {
  ...
  try {
    ...
    const failedItems = sourceItems
      ...
      .map(item => ({ custom_id: retryCustomID(item.custom_id), prompt: String(item.prompt_preview || '').trim() REDACTED))
      .filter(item => item.prompt)
    ...
    const retryJob = await submitBatchImageJob(
      ...
      {
        model: job.model,
        task_name: `${job.task_name || defaultTaskName()REDACTED ${t('batchImage.messages.retryTaskNameSuffix')REDACTED`,
        ...
        items: failedItems,
      REDACTED,
      `sub2api-ui-retry-${job.idREDACTED-${Date.now()REDACTED-${Math.random().toString(36).slice(2, 10)REDACTED`,
    )
...
function retryCustomID(customID: string) {
  ...
  return `${baseREDACTED_retry_${Date.now().toString(36)REDACTED`
REDACTED
...
async function downloadJob(job: (BatchImageJobRow | Pick<BatchImageJob, 'id'>)) {
  ...
  try {
    ...
    saveBlob(blob, `${job.idREDACTED.zip`)
...
async function downloadSelectedJobs() {
  ...
  try {
    for (const row of selectedDownloadableRows.value) {
      ...
      saveBlob(blob, `${row.idREDACTED.zip`)
...
function markJobDownloaded(batchId: string) {
  ...
  batchJobs.value = batchJobs.value.map(job => job.id === batchId ? { ...job, downloaded_at: job.downloaded_at || downloadedAt REDACTED : job)
  if (currentJob.value?.id === batchId && !currentJob.value.downloaded_at) {
    currentJob.value = { ...currentJob.value, downloaded_at: downloadedAt REDACTED
  REDACTED
REDACTED
  ...
  previewCacheDBPromise = new Promise((resolve) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(PREVIEW_CACHE_STORE_NAME)) {
        const store = db.createObjectStore(PREVIEW_CACHE_STORE_NAME, { keyPath: 'key' REDACTED)
        store.createIndex('lastAccessedAt', 'lastAccessedAt', { unique: false REDACTED)
      REDACTED
    REDACTED
    request.onsuccess = () => resolve(request.result)
...
async function createThumbnailBlob(blob: Blob): Promise<Blob> {
  ...
  return await new Promise<Blob>((resolve, reject) => {
    ...
    REDACTED, 'image/webp', PREVIEW_THUMBNAIL_QUALITY)
  REDACTED)
REDACTED

async function loadPreviewImageSource(blob: Blob): Promise<{ image: PreviewImageSource, width: number, height: number, close: () => void REDACTED> {
...
function detailSourceName(job: Pick<BatchImageJobRow, 'id' | 'task_name' | 'parent_batch_id'>, rootBatchId: string) {
  ...
  if (job.id === rootBatchId) return t('batchImage.detail.mainTask', { name REDACTED)
  return t('batchImage.detail.childTask', { name REDACTED)
REDACTED
...
function statusLabel(jobOrStatus: BatchImageStatus | Pick<BatchImageJob, 'status' | 'success_count' | 'fail_count'>) {
  ...
  return key ? t(`batchImage.status.${keyREDACTED`) : status
REDACTED
...
function itemStatusLabel(status: string) {
  ...
  REDACTED
  const key = statusKeys[status]
  return key ? t(`batchImage.itemStatus.${keyREDACTED`) : status
REDACTED
...
function formatMoney(value: number | null | undefined) {
  ...
  return `$${Number(value).toFixed(2)REDACTED`
REDACTED
...
function costLabel(job: Pick<BatchImageJob, 'status' | 'hold_amount' | 'actual_cost'>) {
  ...
  return t('batchImage.detail.holdCost', { amount: formatMoney(job.hold_amount) REDACTED)
REDACTED
...
function batchImageText(key: BatchImageTextKey) {
  return t(`batchImage.messages.${keyREDACTED`)
REDACTED
...
function batchImageErrorReference(error: any) {
  ...
  if (code) parts.push(t('batchImage.messages.errorCodeRef', { code REDACTED))
  if (requestId) parts.push(t('batchImage.messages.requestIdRef', { id: requestId REDACTED))
  if (!code && status) parts.push(t('batchImage.messages.httpStatusRef', { status REDACTED))
  return parts.length ? `（${parts.join(isZhLocale() ? '，' : ', ')REDACTED）` : ''
REDACTED

function batchImageAdminError(base: string, error: any) {
  const reference = batchImageErrorReference(error)
  return `${baseREDACTED${reference ? ` ${referenceREDACTED` : ''REDACTED ${batchImageText('adminReference')REDACTED`
REDACTED
...
function batchImageErrorMessage(error: any, fallback: string) {
  ...
  REDACTED
  if (isZhLocale()) {
    const detail = message ? `${batchImageText('errorReference')REDACTED：${messageREDACTED` : batchImageText('adminReference')
    return `${fallbackREDACTED。${detailREDACTED ${batchImageErrorReference(error)REDACTED`
  REDACTED
