        ...
        <div class="flex flex-col gap-3">
          ...
          >
            <span class="text-sm text-gray-600 dark:text-gray-300">
              已选择 <span class="font-medium text-gray-900 dark:text-white">{{ selectedJobIds.size REDACTEDREDACTED</span> 个任务
            </span>
        ...
        >
          ...
          <template #cell-id="{ row REDACTED">
	            <div class="flex w-[220px] items-start gap-1" :class="row.is_child ? 'pl-6' : ''">
	              <button
	                ...
	                :title="expandedParentIds.has(row.id) ? '收起子任务' : `展开 ${row.child_countREDACTED 个子任务`"
                ...
                >
                  ...
                  <span v-if="row.child_count > 0 && !row.is_child" class="flex-shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-xs font-normal text-gray-600 dark:bg-dark-700 dark:text-gray-300">
                    {{ row.child_count REDACTEDREDACTED 子任务
                  </span>
          ...
          <template #cell-counts="{ row REDACTED">
            <div class="flex items-center justify-center gap-2 text-sm tabular-nums">
              ...
              <span class="text-xs text-gray-400 dark:text-gray-500">共 {{ displayJob(row).item_count REDACTEDREDACTED</span>
            </div>
          </template>
          ...
          <template #cell-downloaded="{ row REDACTED">
            <span class="block text-center text-sm" :class="row.downloaded_at ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-500 dark:text-gray-400'">
              {{ row.downloaded_at ? formatDate(row.downloaded_at) : '未下载' REDACTEDREDACTED
            </span>
          </template>
      ...
      <template #pagination>
        ...
        >
          <div class="flex flex-wrap items-center gap-3 text-sm text-gray-700 dark:text-gray-300">
            <span>
              第 <span class="font-medium">{{ pagination.page REDACTEDREDACTED</span> 页
            </span>
            <span>
              本页 <span class="font-medium">{{ visibleBatchJobs.length REDACTEDREDACTED</span> 条
            </span>
      ...
      <div v-if="currentJob" class="space-y-4">
        <div class="rounded-lg border border-gray-200 bg-gray-50/70 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/40">
          <div class="grid gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="min-w-0 text-center">
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ hasChildJobs(currentJob.id) ? '汇总结果' : '结果' REDACTEDREDACTED</p>
            ...
            <div class="min-w-0 text-center">
              ...
              {{ currentJob.downloaded_at ? formatDate(currentJob.downloaded_at) : '未下载' REDACTEDREDACTED
            </p>
            </div>
          </div>
        </div>
                ...
                <td class="px-3 py-2.5 text-center">
                  <div class="mx-auto h-12 w-12 overflow-hidden rounded-md border border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-800">
                    <button
                      ...
                      :title="`放大压缩预览 ${item.custom_idREDACTED`"
        ...
        <div v-else class="rounded-lg border border-dashed border-gray-200 py-10 text-center dark:border-dark-700">
          ...
          <p class="text-sm font-medium text-gray-700 dark:text-gray-200">
            {{ loadingItems ? '正在加载明细...' : '暂无明细' REDACTEDREDACTED
          </p>
      ...
      <form class="space-y-5" @submit.prevent="submitJob">
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            ...
            <select v-model.number="form.apiKeyId" class="input" :disabled="loadingKeys">
              <option :value="0">{{ loadingKeys ? '加载 API Key 中...' : '请选择 Gemini API Key' REDACTEDREDACTED</option>
          ...
          <div>
            ...
            <div class="input flex items-center bg-gray-50 text-gray-600 dark:bg-dark-900 dark:text-gray-300">
              {{ estimatedOutputCount REDACTEDREDACTED 张 / {{ promptRows.length REDACTEDREDACTED 条
            </div>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">已添加 {{ promptRows.length REDACTEDREDACTED 条</span>
          </div>
          <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-700">
            ...
            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
              每条最多 {{ BATCH_IMAGE_MAX_OUTPUTS_PER_ITEM REDACTEDREDACTED 张，整组最多 {{ BATCH_IMAGE_MAX_OUTPUTS_PER_JOB REDACTEDREDACTED 张；当前模型每条最多 {{ selectedModelReferenceLimit REDACTEDREDACTED 张参考图，参考图按生成张数重复消耗输入 token。
            </p>
          </div>
          <div v-if="promptRows.length" class="overflow-hidden rounded-lg border border-gray-200 dark:border-dark-700">
            ...
            >
              ...
              <span v-if="row.reference_images.length" class="flex-shrink-0 text-xs text-gray-500 dark:text-gray-400">
                {{ row.reference_images.length REDACTEDREDACTED 参考图
              </span>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <button type="button" class="btn btn-secondary" :disabled="submitting" @click="closeCreateModal">取消</button>
            ...
            {{ submitting ? '提交中...' : '提交任务' REDACTEDREDACTED
          </button>
        </div>
      </template>
    </BaseDialog>
...
const columns: Column[] = [
  ...
  { key: 'id', label: '任务名称', sortable: false, class: 'w-[240px] max-w-[240px]' REDACTED,
  { key: 'model', label: '模型', sortable: false, class: 'w-[180px] max-w-[180px] text-center' REDACTED,
  { key: 'api_key_name', label: '提交密钥', sortable: false, class: 'w-40 max-w-40 text-center' REDACTED,
  { key: 'status', label: '状态', sortable: false, class: 'w-28 text-center' REDACTED,
  { key: 'counts', label: '结果', sortable: false, class: 'w-32 text-center' REDACTED,
  { key: 'cost', label: '费用', sortable: false, class: 'w-36 text-center' REDACTED,
  { key: 'downloaded', label: '下载状态', sortable: false, class: 'w-40 text-center' REDACTED,
  { key: 'actions', label: '操作', sortable: false, class: 'w-40 text-center' REDACTED,
]

const statusFilterOptions: SelectOption[] = [
  { value: '', label: '全部状态' REDACTED,
  { value: 'queued', label: '排队中' REDACTED,
  { value: 'running', label: '生成中' REDACTED,
  { value: 'processing_results', label: '整理结果' REDACTED,
  { value: 'settling', label: '结算中' REDACTED,
  { value: 'completed', label: '已完成' REDACTED,
  { value: 'failed', label: '失败' REDACTED,
  { value: 'cancelled', label: '已取消' REDACTED,
  { value: 'output_deleted', label: '结果已删除' REDACTED,
]

const downloadFilterOptions: SelectOption[] = [
  { value: '', label: '全部下载状态' REDACTED,
  { value: 'true', label: '已下载' REDACTED,
  { value: 'false', label: '未下载' REDACTED,
]
...
const apiKeyFilterOptions = computed<SelectOption[]>(() => [
  { value: '', label: '全部 API Key' REDACTED,
...
async function handleReferenceImageFiles(event: Event) {
  ...
  if (slots <= 0) {
    appStore.showError(`当前模型每条最多 ${limitREDACTED 张参考图。`)
    return
  REDACTED
  const accepted = files.slice(0, slots)
  if (accepted.length < files.length) {
    appStore.showError(`当前模型每条最多 ${limitREDACTED 张参考图，已忽略超出的文件。`)
  REDACTED
  ...
  for (const file of accepted) {
    ...
    if (file.size > 10 * 1024 * 1024) {
      appStore.showError(`${file.nameREDACTED 超过 10MB，已忽略。`)
      continue
    REDACTED
...
async function retryFailedJob(job: BatchImageJobRow | BatchImageJob) {
  ...
  try {
    ...
    const retryJob = await submitBatchImageJob(
      ...
      {
        model: job.model,
        task_name: `${job.task_name || defaultTaskName()REDACTED 重试失败项`,
...
function detailSourceName(job: Pick<BatchImageJobRow, 'id' | 'task_name' | 'parent_batch_id'>, rootBatchId: string) {
  ...
  if (job.id === rootBatchId) return `主任务：${nameREDACTED`
  return `子任务：${nameREDACTED`
REDACTED
...
function costLabel(job: Pick<BatchImageJob, 'status' | 'hold_amount' | 'actual_cost'>) {
  ...
  return `冻结 ${formatMoney(job.hold_amount)REDACTED`
REDACTED
...
function batchImageErrorReference(error: any) {
  ...
  if (code) parts.push(isZhLocale() ? `错误码：${codeREDACTED` : `code: ${codeREDACTED`)
  if (requestId) parts.push(isZhLocale() ? `请求 ID：${requestIdREDACTED` : `request ID: ${requestIdREDACTED`)
  if (!code && status) parts.push(isZhLocale() ? `HTTP 状态：${statusREDACTED` : `HTTP status: ${statusREDACTED`)
