  ...
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
        {{ qrUrl ? scanTitle : t('payment.qr.payInNewWindow') REDACTEDREDACTED
      </h2>
      ...
      <div v-if="expired" class="text-center">
        ...
        <button class="btn btn-primary mt-4" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') REDACTEDREDACTED</button>
      </div>
      <div v-else class="text-center">
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') REDACTEDREDACTED</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-gray-900 dark:text-white">{{ countdownDisplay REDACTEDREDACTED</p>
        <p class="mt-2 text-sm text-gray-400 dark:text-gray-500">{{ t('payment.qr.waitingPayment') REDACTEDREDACTED</p>
      </div>
...
import { ref, computed, onMounted, onUnmounted, watch, nextTick REDACTED from 'vue'
...
import { isBuiltInAlipayMethod, isBuiltInWxpayMethod REDACTED from '@/components/payment/providerConfig'
...
async function pollStatus() {
  ...
  if (order.status === 'COMPLETED' || order.status === 'PAID') {
    cleanup()
    router.push({ path: '/payment/result', query: { order_id: String(orderId.value), status: 'success' REDACTED REDACTED)
