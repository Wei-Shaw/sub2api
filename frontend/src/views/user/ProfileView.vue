    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="px-6 py-5 bg-gradient-to-r from-primary-500/10 to-primary-600/5 dark:from-primary-500/20 dark:to-primary-600/10 border-b border-gray-100 dark:border-dark-700">
          <div class="flex items-center gap-4">
            ...
            <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-primary-500/20">
              {{ user?.email?.charAt(0).toUpperCase() || 'U' REDACTEDREDACTED
            </div>
            <div class="flex-1 min-w-0">
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white truncate">{{ user?.email REDACTEDREDACTED</h2>
              <div class="flex items-center gap-2 mt-1">
                ...
                >
                  {{ user?.role === 'admin' ? t('profile.administrator') : t('profile.user') REDACTEDREDACTED
                </span>
                <span
                  :class="[
                    'badge',
        ...
        <div class="px-6 py-4">
          <div class="space-y-3">
            <div class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span class="truncate">{{ user?.email REDACTEDREDACTED</span>
            </div>
            <div v-if="user?.username" class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span class="truncate">{{ user.username REDACTEDREDACTED</span>
            </div>
            <div v-if="user?.wechat" class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span class="truncate">{{ user.wechat REDACTEDREDACTED</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="contactInfo" class="card border-primary-200 dark:border-primary-800/40 bg-gradient-to-r from-primary-50 to-primary-100/50 dark:from-primary-900/20 dark:to-primary-800/10">
        <div class="px-6 py-5">
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              <h3 class="text-sm font-semibold text-primary-800 dark:text-primary-200">{{ t('common.contactSupport') REDACTEDREDACTED</h3>
              <p class="mt-1 text-sm font-medium text-primary-600 dark:text-primary-300">{{ contactInfo REDACTEDREDACTED</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-dark-700">
          <h2 class="text-lg font-medium text-gray-900 dark:text-white">{{ t('profile.editProfile') REDACTEDREDACTED</h2>
        </div>
        <div class="px-6 py-6">
          <form @submit.prevent="handleUpdateProfile" class="space-y-4">
            ...
            <div>
              <label for="wechat" class="input-label">
                {{ t('profile.wechat') REDACTEDREDACTED
              </label>
              <input
                id="wechat"
            ...
            <div class="flex justify-end pt-4">
              ...
              >
                {{ updatingProfile ? t('profile.updating') : t('profile.updateProfile') REDACTEDREDACTED
              </button>
            </div>
          </form>
        </div>
      ...
      <div class="card">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-dark-700">
          <h2 class="text-lg font-medium text-gray-900 dark:text-white">{{ t('profile.changePassword') REDACTEDREDACTED</h2>
        </div>
        <div class="px-6 py-6">
          <form @submit.prevent="handleChangePassword" class="space-y-4">
            ...
            <div class="flex justify-end pt-4">
              <button
                ...
                class="btn btn-primary"
              >
                {{ changingPassword ? t('profile.changingPassword') : t('profile.changePasswordButton') REDACTEDREDACTED
              </button>
            </div>
          </form>
        </div>
...
import { ref, computed, h, onMounted REDACTED from 'vue'
import { useI18n REDACTED from 'vue-i18n'
import { useAuthStore REDACTED from '@/stores/auth'
import { useAppStore REDACTED from '@/stores/app'

const { t REDACTED = useI18n()
import { userAPI, authAPI REDACTED from '@/api'
...
const WalletIcon = {
  render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' REDACTED, [
    h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M21 12a2.25 2.25 0 00-2.25-2.25H15a3 3 0 11-6 0H5.25A2.25 2.25 0 003 12m18 0v6a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 18v-6m18 0V9M3 12V9m18 0a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 9m18 0V6a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 6v3' REDACTED)
  ])
REDACTED

const BoltIcon = {
  render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' REDACTED, [
    h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'm3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z' REDACTED)
  ])
REDACTED

const CalendarIcon = {
  render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' REDACTED, [
    h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5' REDACTED)
  ])
REDACTED
...
const formatCurrency = (value: number): string => {
  return `$${value.toFixed(2)REDACTED`
REDACTED
