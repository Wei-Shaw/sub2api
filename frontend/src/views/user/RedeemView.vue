    ...
    <div class="max-w-2xl mx-auto space-y-6">
      ...
      <div class="card overflow-hidden">
        <div class="bg-gradient-to-br from-primary-500 to-primary-600 px-6 py-8 text-center">
          ...
          <p class="text-sm font-medium text-primary-100">{{ t('redeem.currentBalance') REDACTEDREDACTED</p>
          <p class="mt-2 text-4xl font-bold text-white">
            ${{ user?.balance?.toFixed(2) || '0.00' REDACTEDREDACTED
          </p>
          <p class="mt-2 text-sm text-primary-100">
            {{ t('redeem.concurrency') REDACTEDREDACTED: {{ user?.concurrency || 0 REDACTEDREDACTED {{ t('redeem.requests') REDACTEDREDACTED
          </p>
        </div>
      </div>
          ...
          <form @submit.prevent="handleRedeem" class="space-y-5">
            <div>
              <label for="code" class="input-label">
                {{ t('redeem.redeemCodeLabel') REDACTEDREDACTED
              </label>
              ...
              <p class="input-hint">
                {{ t('redeem.redeemCodeHint') REDACTEDREDACTED
              </p>
            </div>
            ...
            >
              ...
              {{ submitting ? t('redeem.redeeming') : t('redeem.redeemButton') REDACTEDREDACTED
            </button>
          </form>
        </div>
      </div>
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <h3 class="text-sm font-semibold text-emerald-800 dark:text-emerald-300">
                  {{ t('redeem.redeemSuccess') REDACTEDREDACTED
                </h3>
                <div class="mt-2 text-sm text-emerald-700 dark:text-emerald-400">
                  <p>{{ redeemResult.message REDACTEDREDACTED</p>
                  <div class="mt-3 space-y-1">
                    <p v-if="redeemResult.type === 'balance'" class="font-medium">
                      {{ t('redeem.added') REDACTEDREDACTED: ${{ redeemResult.value.toFixed(2) REDACTEDREDACTED
                    </p>
                    <p v-else-if="redeemResult.type === 'concurrency'" class="font-medium">
                      {{ t('redeem.added') REDACTEDREDACTED: {{ redeemResult.value REDACTEDREDACTED {{ t('redeem.concurrentRequests') REDACTEDREDACTED
                    </p>
                    <p v-else-if="redeemResult.type === 'subscription'" class="font-medium">
                      {{ t('redeem.subscriptionAssigned') REDACTEDREDACTED
                      <span v-if="redeemResult.group_name"> - {{ redeemResult.group_name REDACTEDREDACTED</span>
                      <span v-if="redeemResult.validity_days"> ({{ t('redeem.subscriptionDays', { days: redeemResult.validity_days REDACTED) REDACTEDREDACTED)</span>
                    </p>
                    <p v-if="redeemResult.new_balance !== undefined">
                      {{ t('redeem.newBalance') REDACTEDREDACTED: <span class="font-semibold">${{ redeemResult.new_balance.toFixed(2) REDACTEDREDACTED</span>
                    </p>
                    <p v-if="redeemResult.new_concurrency !== undefined">
                      {{ t('redeem.newConcurrency') REDACTEDREDACTED: <span class="font-semibold">{{ redeemResult.new_concurrency REDACTEDREDACTED {{ t('redeem.requests') REDACTEDREDACTED</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <h3 class="text-sm font-semibold text-red-800 dark:text-red-300">
                  {{ t('redeem.redeemFailed') REDACTEDREDACTED
                </h3>
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              <h3 class="text-sm font-semibold text-primary-800 dark:text-primary-300">
                {{ t('redeem.aboutCodes') REDACTEDREDACTED
              </h3>
              <ul class="mt-2 text-sm text-primary-700 dark:text-primary-400 space-y-1 list-disc list-inside">
                <li>{{ t('redeem.codeRule1') REDACTEDREDACTED</li>
                <li>{{ t('redeem.codeRule2') REDACTEDREDACTED</li>
                <li>
                  {{ t('redeem.codeRule3') REDACTEDREDACTED
                  <span v-if="contactInfo" class="inline-flex items-center ml-1.5 px-2 py-0.5 rounded-md bg-primary-200/50 dark:bg-primary-800/40 text-primary-800 dark:text-primary-200 font-medium text-xs">
                    {{ contactInfo REDACTEDREDACTED
                  </span>
                </li>
                <li>{{ t('redeem.codeRule4') REDACTEDREDACTED</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div class="card">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-dark-700">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('redeem.recentActivity') REDACTEDREDACTED</h2>
        </div>
        <div class="p-6">
          <!-- Loading State -->
          ...
          <div v-else-if="history.length > 0" class="space-y-3">
            ...
            >
              ...
              <div class="text-right">
                ...
                >
                  {{ formatHistoryValue(item) REDACTEDREDACTED
                </p>
                <p v-if="!isAdminAdjustment(item.type)" class="text-xs text-gray-400 dark:text-dark-500 font-mono">
                  {{ item.code.slice(0, 8) REDACTEDREDACTED...
                </p>
                <p v-else class="text-xs text-gray-400 dark:text-dark-500">
                  {{ t('redeem.adminAdjustment') REDACTEDREDACTED
                </p>
              </div>
            </div>
          </div>
          ...
          <div v-else class="empty-state py-8">
            ...
            <p class="text-sm text-gray-500 dark:text-dark-400">
              {{ t('redeem.historyWillAppear') REDACTEDREDACTED
            </p>
          </div>
        </div>
      </div>
...
const getHistoryItemTitle = (item: RedeemHistoryItem) => {
  ...
  REDACTED else if (item.type === 'admin_balance') {
    return item.value >= 0 ? t('redeem.balanceAddedAdmin') : t('redeem.balanceDeductedAdmin')
  REDACTED else if (item.type === 'concurrency') {
    return t('redeem.concurrencyAddedRedeem')
  REDACTED else if (item.type === 'admin_concurrency') {
...
const formatHistoryValue = (item: RedeemHistoryItem) => {
  if (isBalanceType(item.type)) {
    ...
    return `${signREDACTED$${item.value.toFixed(2)REDACTED`
  REDACTED else if (isSubscriptionType(item.type)) {
    // 订阅类型显示有效天数和分组名称
    const days = item.validity_days || Math.round(item.value)
    const groupName = item.group?.name || ''
    return groupName ? `${daysREDACTED${t('redeem.days')REDACTED - ${groupNameREDACTED` : `${daysREDACTED${t('redeem.days')REDACTED`
  REDACTED else {
    const sign = item.value >= 0 ? '+' : ''
    return `${signREDACTED${item.valueREDACTED ${t('redeem.requests')REDACTED`
  REDACTED
REDACTED
