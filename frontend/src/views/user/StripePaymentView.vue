  ...
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.stripeLoadFailed') REDACTEDREDACTED</h3>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ initError REDACTEDREDACTED</p>
        <button class="btn btn-primary mt-6" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') REDACTEDREDACTED</button>
      </div>
      <template v-else>
        <!-- 金额头部 -->
        <div v-if="order" class="card overflow-hidden">
          <div class="bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-6 py-6 text-center">
            ...
            <p class="mt-1 text-3xl font-bold text-white">{{ formatGatewayAmount(order.pay_amount) REDACTEDREDACTED</p>
          </div>
        </div>
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.qr.scanWxpay') REDACTEDREDACTED</p>
              ...
              <p class="text-center text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.scanWxpayHint') REDACTEDREDACTED</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.waitingPayment') REDACTEDREDACTED</p>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.stripeSuccessProcessing') REDACTEDREDACTED</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <p v-if="stripeError" class="mt-4 text-sm text-red-600 dark:text-red-400">{{ stripeError REDACTEDREDACTED</p>
          ...
          <div class="text-center">
            <button class="btn btn-secondary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') REDACTEDREDACTED</button>
          </div>
        </template>

        <!-- 错误状态 -->
        <div v-if="stripeError && !showPaymentElement" class="card p-4">
          <p class="text-sm text-red-600 dark:text-red-400">{{ stripeError REDACTEDREDACTED</p>
          <button class="btn btn-secondary mt-3 w-full" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') REDACTEDREDACTED</button>
        </div>
      </template>
    </div>
  </component>
...
import { ref, computed, nextTick, onMounted, onUnmounted REDACTED from 'vue'
...
onMounted(async () => {
  ...
  try {
    ...
    if (!publishableKey) { initError.value = t('payment.stripeNotConfigured'); return REDACTED
...
async function confirmAlipay(stripe: Stripe, clientSecret: string, orderId: number) {
  ...
  const { error REDACTED = await stripe.confirmAlipayPayment(clientSecret, { return_url: returnUrl REDACTED)
  if (error) {
...
async function confirmWechatPay(stripe: Stripe, clientSecret: string) {
  ...
  if (qrData) {
    ...
    // 轮询支付完成状态
    startPolling()
  REDACTED else if (paymentIntent?.status === 'succeeded') {
...
function mountPaymentElement(stripe: Stripe, clientSecret: string) {
  ...
  paymentElement.on('ready', () => { stripeReady.value = true REDACTED)
REDACTED
...
async function handleGenericPay() {
  ...
  stripeError.value = ''
  try {
    const { error REDACTED = await stripeInstance.confirmPayment({
...
function scheduleClose() {
  if (window.opener) {
    redirectTimer = setTimeout(() => { window.close() REDACTED, 2000)
  REDACTED else {
    redirectTimer = setTimeout(() => {
      router.push({ path: '/payment/result', query: { order_id: String(route.query.order_id || ''), status: 'success' REDACTED REDACTED)
    REDACTED, 2000)
  REDACTED
REDACTED
