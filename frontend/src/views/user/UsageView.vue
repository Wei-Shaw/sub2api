        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
          <div v-if="activeTab === 'errors'" class="flex flex-1 flex-wrap items-end gap-4">
            <div class="w-full sm:w-auto sm:min-w-[220px]">
              <label class="input-label">{{ t('usage.errors.keyName') REDACTEDREDACTED</label>
              <Select v-model="errorFilter.api_key_id" :options="errorKeyOptions" @change="applyErrorFilters" />
            </div>
            <div class="w-full sm:w-auto sm:min-w-[220px]">
              <label class="input-label">{{ t('usage.errors.model') REDACTEDREDACTED</label>
              <Select
            ...
            <div class="w-full sm:w-auto sm:min-w-[200px]">
              <label class="input-label">{{ t('usage.errors.category') REDACTEDREDACTED</label>
              <Select v-model="errorFilter.category" :options="errorCategoryOptions" @change="applyErrorFilters" />
            </div>
            <div class="w-full sm:w-auto sm:min-w-[180px]">
              <label class="input-label">{{ t('usage.errors.status') REDACTEDREDACTED</label>
          ...
          <div v-else class="flex flex-1 flex-wrap items-end gap-4">
            ...
            <div class="w-full sm:w-auto sm:min-w-[200px]">
              <label class="input-label">{{ t('admin.usage.billingType') REDACTEDREDACTED</label>
              <Select v-model="filters.billing_type" :options="billingTypeOptions" @change="applyFilters" />
            </div>
            <div class="w-full sm:w-auto sm:min-w-[200px]">
              <label class="input-label">{{ t('admin.usage.billingMode') REDACTEDREDACTED</label>
...
import { computed, onMounted, onUnmounted, reactive, ref, watch REDACTED from 'vue'
import { useI18n REDACTED from 'vue-i18n'
import { useAppStore REDACTED from '@/stores/app'
import { keysAPI, usageAPI, userGroupsAPI REDACTED from '@/api'
...
import { getBillingModeLabel, getDisplayBillingMode as resolveDisplayBillingMode REDACTED from '@/utils/billingMode'
...
import { COMMON_ERROR_STATUS_CODES REDACTED from '@/utils/errorBadges'
...
const errorFilter = ref<{ model: string | null; category: string; api_key_id: number | null; status_code: number | null REDACTED>({
  model: '',
  category: '',
  api_key_id: null,
  status_code: null,
...
const errorKeyOptions = computed<SelectOption[]>(() => [
  { value: null, label: t('usage.errors.allKeys') REDACTED,
  ...apiKeys.value.map((k) => ({ value: k.id, label: k.name REDACTED)),
])
...
const errorModelOptions = computed<SelectOption[]>(() => {
  ...
  for (const r of errorRows.value) {
    if (r.model && !seen.has(r.model)) {
      seen.add(r.model)
      opts.push({ value: r.model, label: r.model REDACTED)
    REDACTED
  REDACTED
  return opts
REDACTED)
...
const errorCategoryOptions = computed<SelectOption[]>(() => [
  { value: '', label: t('usage.errors.allCategories') REDACTED,
  ...errorCategoryCodes.map((c) => ({ value: c, label: t('usage.errors.categories.' + c) REDACTED)),
])
...
const errorStatusOptions = computed<SelectOption[]>(() => [
  { value: null, label: t('usage.errors.allStatuses') REDACTED,
  ...COMMON_ERROR_STATUS_CODES.map((c) => ({ value: c, label: String(c) REDACTED)),
])
...
const formatLocalDate = (date: Date): string =>
  `${date.getFullYear()REDACTED-${String(date.getMonth() + 1).padStart(2, '0')REDACTED-${String(date.getDate()).padStart(2, '0')REDACTED`

const getLast24HoursRangeDates = () => {
  const end = new Date()
  const start = new Date(end.getTime() - 24 * 60 * 60 * 1000)
  return { start: formatLocalDate(start), end: formatLocalDate(end) REDACTED
REDACTED
...
const billingTypeOptions = computed<SelectOption[]>(() => [
  ...
  { value: 0, label: t('admin.usage.billingTypeBalance') REDACTED,
  { value: 1, label: t('admin.usage.billingTypeSubscription') REDACTED,
])
...
const apiKeyOptions = computed<SelectOption[]>(() => [
  ...
  ...apiKeys.value.map((key) => ({ value: key.id, label: key.name REDACTED)),
])
const groupOptions = computed<SelectOption[]>(() => [
  { value: null, label: t('admin.usage.allGroups') REDACTED,
  ...groups.value.map((group) => ({ value: group.id, label: group.name REDACTED)),
])
const modelOptions = computed<SelectOption[]>(() => [
  { value: null, label: t('admin.usage.allModels') REDACTED,
  ...modelOptionValues.value.map((model) => ({ value: model, label: model REDACTED)),
])
...
const resetFilters = () => {
  ...
  if (activeTab.value === 'errors') {
    errorFilter.value = { model: '', category: '', api_key_id: null, status_code: null REDACTED
    applyErrorFilters()
  REDACTED
REDACTED
...
const escapeCSVValue = (value: unknown): string => {
  ...
  if (/^[=+\-@\t\r]/.test(str)) return `"\'${escapedREDACTED"`
  if (/[,"\n\r]/.test(str)) return `"${escapedREDACTED"`
  return str
REDACTED
...
const exportToCSV = async () => {
  ...
  try {
    ...
    link.href = url
    link.download = `usage_${startDate.valueREDACTED_to_${endDate.valueREDACTED.csv`
    link.click()
...
const allColumns = computed<Column[]>(() => [
  ...
  { key: 'ip_address', label: 'IP', sortable: false REDACTED,
...
const errAllColumns = computed<Column[]>(() => [
  { key: 'key_name', label: t('usage.errors.keyName') REDACTED,
  { key: 'model', label: t('usage.errors.model') REDACTED,
  { key: 'endpoint', label: t('usage.errors.endpoint') REDACTED,
  { key: 'client_ip', label: 'IP' REDACTED,
  { key: 'group', label: t('admin.usage.group') REDACTED,
  { key: 'type', label: t('usage.type') REDACTED,
  { key: 'platform', label: t('usage.errors.platform') REDACTED,
  { key: 'category', label: t('usage.errors.category') REDACTED,
  { key: 'status', label: t('usage.errors.status') REDACTED,
  { key: 'message', label: t('usage.errors.message') REDACTED,
  { key: 'created_at', label: t('usage.errors.time') REDACTED,
