        ...
        <div class="card p-4">
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400">
                <span>{{ t('usage.in') REDACTEDREDACTED {{ formatTokens(usageStats?.total_input_tokens || 0) REDACTEDREDACTED</span>
                <span> · </span>
                <span>{{ t('usage.out') REDACTEDREDACTED {{ formatTokens(usageStats?.total_output_tokens || 0) REDACTEDREDACTED</span>
                <span> · </span>
                <span class="text-sky-600 dark:text-sky-400">{{ t('usage.cacheHit') REDACTEDREDACTED {{ formatTokens(usageStats?.total_cache_read_tokens || 0) REDACTEDREDACTED</span>
                <span> · </span>
                <span class="text-amber-600 dark:text-amber-400">{{ t('usage.cacheCreate') REDACTEDREDACTED {{ formatTokens(usageStats?.total_cache_creation_tokens || 0) REDACTEDREDACTED</span>
              </p>
              <p class="text-xs text-gray-400 dark:text-gray-500">
                {{ t('usage.cacheHitRate') REDACTEDREDACTED:
                <template v-if="cacheStats.totalInput > 0">
                  <span class="text-sky-600 dark:text-sky-400">{{ formatTokens(cacheStats.cacheRead) REDACTEDREDACTED</span>
                  <span class="text-gray-400">/</span>
                  <span class="text-gray-600 dark:text-gray-300">{{ formatTokens(cacheStats.totalInput) REDACTEDREDACTED</span>
                  <span class="ml-1">{{ cacheStats.ratePercent REDACTEDREDACTED</span>
                </template>
              ...
              <div class="space-y-1.5 text-sm">
                ...
                <div v-if="hasImageOutputTokens(row)" class="flex items-center gap-2">
                  <div class="inline-flex items-center gap-1">
                    ...
                    <span class="font-medium text-pink-600 dark:text-pink-400">{{ row.image_output_tokens.toLocaleString() REDACTEDREDACTED</span>
                  </div>
                </div>
              </div>
          ...
          <template #cell-cost="{ row REDACTED">
            <div class="flex items-center gap-1.5 text-sm">
              <span class="font-medium text-green-600 dark:text-green-400">
                ${{ (row.actual_cost ?? 0).toFixed(6) REDACTEDREDACTED
              </span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
        ...
        <div class="space-y-1.5">
          ...
          <div>
            ...
            <div v-if="tokenTooltipData && hasImageOutputTokens(tokenTooltipData) && textOutputTokens(tokenTooltipData) > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.outputTokens') REDACTEDREDACTED</span>
              <span class="font-medium text-white">{{ textOutputTokens(tokenTooltipData).toLocaleString() REDACTEDREDACTED</span>
            </div>
            <div v-if="tokenTooltipData && hasImageOutputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.imageOutputTokens') REDACTEDREDACTED</span>
              <span class="font-medium text-pink-300">{{ tokenTooltipData.image_output_tokens.toLocaleString() REDACTEDREDACTED</span>
            </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && hasImageOutputCost(tooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.imageOutputCost') REDACTEDREDACTED</span>
              <span class="font-medium text-pink-300">${{ tooltipData.image_output_cost.toFixed(6) REDACTEDREDACTED</span>
            </div>
            ...
            <template v-if="!tooltipData?.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN">
              ...
              <div v-if="tooltipData && tooltipData.output_cost > 0 && textOutputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-violet-300">{{ formatTokenPricePerMillion(tooltipData.output_cost, textOutputTokens(tooltipData)) REDACTEDREDACTED {{ t('usage.perMillionTokens') REDACTEDREDACTED</span>
              </div>
              <div v-if="tooltipData && hasImageOutputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.imageOutputTokenPrice') REDACTEDREDACTED</span>
                <span class="font-medium text-pink-300">{{ formatTokenPricePerMillion(tooltipData.image_output_cost ?? 0, tooltipData.image_output_tokens) REDACTEDREDACTED {{ t('usage.perMillionTokens') REDACTEDREDACTED</span>
              </div>
            </template>
...
const cacheStats = computed(() => {
  ...
  const ratePercent = totalInput > 0 ? `${((cacheRead / totalInput) * 100).toFixed(1)REDACTED%` : '-'
  return { cacheRead, totalInput, ratePercent REDACTED
REDACTED)
