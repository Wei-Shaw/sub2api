...
const { copyToClipboard, getAffiliateDetail REDACTED = vi.hoisted(() => ({
...
vi.mock('@/composables/useClipboard', () => ({
  useClipboard: () => ({ copyToClipboard REDACTED),
REDACTED))
...
describe('AffiliateView', () => {
  ...
  it('stacks long values and copy controls on mobile while retaining desktop rows', async () => {
    const wrapper = mount(AffiliateView, {
      global: {
        stubs: {
          AppLayout: { template: '<main><slot /></main>' REDACTED,
          Icon: true,
        REDACTED,
      REDACTED,
    REDACTED)
    ...
    expect(copyToClipboard).toHaveBeenNthCalledWith(
      2,
      `${window.location.originREDACTED/register?aff=${encodeURIComponent(affiliateCode)REDACTED`,
