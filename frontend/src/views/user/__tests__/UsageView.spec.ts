...
const simpleStub = { template: '<div><slot /></div>' REDACTED
const chartStub = { template: '<div />' REDACTED
...
describe('user UsageView', () => {
  beforeEach(() => {
    ...
    query.mockResolvedValue({ items: [usageLog], total: 1, pages: 1 REDACTED)
    ...
    getDashboardModels.mockResolvedValue({
      models: [{ model: 'gpt-5.4', requests: 1, input_tokens: 10, output_tokens: 20, cache_creation_tokens: 0, cache_read_tokens: 0, total_tokens: 30, cost: 0.1, actual_cost: 0.08 REDACTED],
    ...
    getDashboardSnapshotV2.mockResolvedValue({
      ...
      groups: [],
    REDACTED)
    list.mockResolvedValue({ items: [{ id: 1, name: 'demo-key' REDACTED] REDACTED)
    getAvailable.mockResolvedValue([{ id: 1, name: 'default' REDACTED])
  REDACTED)
