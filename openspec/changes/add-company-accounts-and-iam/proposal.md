...
- Add immutable public account identifiers: one independently generated 16-digit `account_id` for every user, plus a separate immutable `company_id` for each enterprise, backed by cryptographically secure generation, database uniqueness, and a resumable legacy-user backfill.
...
- Resolve the actual wallet consistently across gateway billing, balance RPC, and asynchronous media hold/capture/release flows. An IAM user's allocated balance is used first when it can cover the full charge; shared-balance permission allows the organization's independent balance to cover a charge only when the allocation is insufficient. The owner's personal balance is never the shared company wallet.
...
- Add owner-managed per-member daily and monthly company-sponsored spend limits. An all-member default can be overridden for selected IAM members; allocated member balance is excluded while shared company balance and enterprise subscriptions count toward limits. Threshold crossings enqueue deduplicated email alerts to the affected member and configured additional recipients.
...
- Make the company application, IAM, public-ID readiness, billing-integration, and upgrade-fee configuration editable from the system-settings feature panel while retaining configuration-file defaults; add an optional company-console documentation URL that renders as a new-window help link beside the console navigation item.
