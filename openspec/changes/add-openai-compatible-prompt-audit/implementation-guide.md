...
sequenceDiagram
    ...
    PG-->>E: job_id
    E->>R: SET payload:{job_idREDACTED scan_text EX 1800
    R-->>E: OK
    ...
    W->>R: GET payload:{job_idREDACTED
    loop 每个必要分片
    ...
    W->>R: DEL payload:{job_idREDACTED
```
...
- HTTP/SSE 必须保留各协议 envelope，不能所有协议统一成 Gin `{"error":"..."REDACTED`。
...
- 使用 SecretEncryptor 认证加密 `{filter_hash,snapshot_max_id,admin_id,issued_at,expires_atREDACTED`，返回默认 5 分钟有效的 confirmation_token。
...
请求固定发送到规范化 `{base_urlREDACTED/v1/chat/completions`，默认模型 `sileader/qwen3guard:0.6b`，role=user、temperature=0、max_tokens=64、seed=42。
