# balance-rpc Specification

## Purpose

独立端口的 tRPC-Go 余额账本服务：把"余额扣费 / 回退"抽成一个薄账本，供外部"扣费 app"通过 RPC 调用。只动 `users.balance`，不触碰网关计费的 apikey 配额/限流/账号配额/订阅用量。扣费不透支、幂等；退费支持部分退、凭原流水冲销、双幂等；接入方以无状态 token 鉴权（本地密钥解密 + 启停/版本本地缓存），每笔扣/退落永久流水做审计。
