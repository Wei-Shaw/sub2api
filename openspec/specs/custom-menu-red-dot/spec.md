...
Provide a lightweight promotion channel that surfaces a red dot on custom menu items in `AppSidebar` whenever an admin publishes a new custom-menu configuration. The dot invites users to explore recently added or updated custom pages, and dismisses per-user via a version-scoped localStorage key so it never re-appears until the admin ships a new version. This capability spans an admin toggle, a deterministic version hash, a public settings surface, sidebar rendering, and dismissal triggers.
