# general-inbox Specification

## Purpose

通用信箱（general-inbox）：账号级站内消息系统，支持单播（direct，fan-out on write）
与广播（broadcast，fan-out on read）两类消息。客户端通过 WebSocket 实时接收、以累积
ack 水位标记已读，断线时通过 catchup 拉取补齐；全局单调 seq 保证有序与幂等消费。
工单等业务通过 `inbox.Publisher` 以 namespace 发布事件到本能力。
