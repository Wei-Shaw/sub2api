# openai-image-upscale Specification

## Purpose

OpenAI 出图上游回包分辨率不足时的同步 upscale 交付：当分组开启开关、请求显式目标档位 ≥ 2K、且回包 b64_json 解码出的真实档位更低时，在把图返回客户端（及转存 COS）之前调用 fal `fal-ai/seedvr/upscale/image` 放大到目标档位，使客户端与 COS 拿到同一份放大图；放大失败/超时按原图兜底，计费随交付字节自洽（成功按目标档位、失败按原图档位），upscale 成本由平台吸收。fal 接入参数为系统配置，开关挂在分组上（依赖回包分辨率自检）。
