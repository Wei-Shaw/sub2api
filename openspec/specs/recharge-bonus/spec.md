...
Provide a configurable, tiered, time-bounded **recharge bonus campaign** ("满 X 元加赠 Y%") that adds bonus balance on top of the normally credited balance during user top-ups. The capability covers admin configuration, deterministic bonus resolution and rounding, atomic fulfillment and refund, checkout API surfacing of the active campaign, the user-facing recharge breakdown UI, and the per-version red-dot promotion / dismissal flow.
