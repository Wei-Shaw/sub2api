...
2. Compute its embedding via the embedding service defined by the `support-knowledge-rag` capability. The helper SHALL read its own dedicated credential pair (`support_chat_embedding_base_url` + `support_chat_embedding_api_key`), which is configured independently from the chat LLM credentials (no fallback to `support_chat_llm_*`, no reuse of internal `api_keys`). The upstream protocol SHALL be selected by `support_chat_rag_embed_provider` (`gemini` default → `POST <base_url>/models/<model>:batchEmbedContents` with `x-goog-api-key`; `openai` → `POST <base_url>/embeddings` with `Authorization: Bearer …`), and the model SHALL come from `support_chat_rag_embed_model`.
