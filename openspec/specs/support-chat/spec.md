...
2. Compute its embedding via the embedding service defined by the `support-knowledge-rag` capability. The helper SHALL share the same credential pair (`support_chat_llm_base_url` + `support_chat_llm_api_key`) as the chat endpoint (no separate api_key_id resolution). The upstream protocol SHALL be selected by `support_chat_rag_embed_provider` (`gemini` default → `POST <base_url>/models/<model>:batchEmbedContents` with `x-goog-api-key`; `openai` → `POST <base_url>/embeddings` with `Authorization: Bearer …`), and the model SHALL come from `support_chat_rag_embed_model`.
