<template>
  <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
    ...
    >
      ...
      <button type="button" @click.stop="emit('remove')" class="btn-icon-danger flex-shrink-0 p-1">
      ...
      <div class="collapsible-inner">
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
            <IntervalRow v-for="(iv, idx) in entry.intervals" :key="idx" :interval="iv" :mode="entry.billing_mode"
              @update="updateInterval(idx, $event)" @remove="removeInterval(idx)" />
          </div>
...
import { Select, Icon } from '@sub2api/plugin-sdk'
import IntervalRow from './IntervalRow.vue'
import ModelTagInput from './ModelTagInput.vue'
import PricingTokenSection from './PricingTokenSection.vue'
...
async function onModelsUpdate(newModels: string[]) {
  ...
  try {
    ...
    // query failure does not affect user operation
  }
}
</script>
