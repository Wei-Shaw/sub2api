<template>
  ...
  >
    ...
    <div v-else>
      ...
      <ul
        class="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded-lg border border-gray-200 dark:divide-dark-700 dark:border-dark-700"
      >
        <li
          v-for="m in monitors"
          :key="m.id"
        ...
        >
          ...
          >
            {{ t('admin.channelMonitor.template.pickerDisabledBadge') }}
          </span>
        </li>
      </ul>
    </div>
 ...
 *
 * 端口差异：
 *   - BaseDialog 从 `@sub2api/plugin-sdk` 取（host 原本是 `@/components/common/BaseDialog.vue`）
 *   - API 从本地 `../../../api/admin/channelMonitorTemplate` 取（host 原本用 `adminAPI.channelMonitorTemplate`）
 *   - 错误处理复用 plugin `extractApiErrorMessage` + `sdk.notify`，不依赖 host appStore
 *   - "仅禁用" badge 改用独立 i18n key `template.pickerDisabledBadge`，
 *     避免 host 版本里对 `onlyDisabled` 正则切字符串的耦合写法
 */
...
async function fetchMonitors(id: number) {
  ...
  selectedIds.value = []
  try {
    const { items } = await channelMonitorTemplateAPI.listAssociatedMonitors(id)
...
async function handleApply() {
  ...
  try {
    const { affected } = await channelMonitorTemplateAPI.apply(
      props.templateId,
      [...selectedIds.value],
    )
    sdk.notify.success(t('admin.channelMonitor.template.applySuccess', { n: affected }))
