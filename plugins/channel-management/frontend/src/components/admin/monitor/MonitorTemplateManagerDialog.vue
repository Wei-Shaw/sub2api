...
/**
 * Worker B' — ported from host commit 6925ac25c
 * (frontend/src/components/admin/monitor/MonitorTemplateManagerDialog.vue).
 *
 * 端口差异：
 *   - BaseDialog / ConfirmDialog / Icon 从 `@sub2api/plugin-sdk` 取
 *   - adminAPI.channelMonitorTemplate → 本地 `channelMonitorTemplateAPI`
 *   - appStore → `getSdk().notify`
 *   - PROVIDER_* 常量从 plugin `utils/channelMonitorConstants` 取
 *   - useChannelMonitorFormat() 已有 providerPickerClass 导出
 *   - MonitorAdvancedRequestConfig 为本 commit 新引入的同目录组件
 */
