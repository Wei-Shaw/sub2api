<template>
  <div class="relative inline-block">
    <span
      ...
      tabindex="0"
    >
      <PlatformIcon v-if="effectivePlatform" :platform="effectivePlatform" size="xs" />
      <span
...
/**
 * V5 W9 — 模型徽章 + hover 弹层显示定价. 内嵌 Teleport-to-body popover 保证
 * 不被父级 overflow:hidden 截断. 行为与 host 版完全一致, 仅替换 import:
 *   - PlatformIcon: SDK
 *   - PricingRow: plugin local
 *   - platformColors / pricing utils: plugin local
 *   - DTO: plugin local api/user/availableChannels.ts
 */
