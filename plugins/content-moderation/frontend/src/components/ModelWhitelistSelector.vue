<template>
  <div>
    ...
    <div class="relative mb-3">
      ...
      >
        ...
        <div class="mt-2 flex items-center justify-between border-t border-gray-200 pt-2 dark:border-dark-600">
          <span class="text-xs text-gray-400">{{ t('admin.riskControl.modelSelector.modelCount', { count: modelValue.length }) }}</span>
      ...
      >
        <div class="sticky top-0 border-b border-gray-200 bg-white p-2 dark:border-dark-600 dark:bg-dark-700">
          <input
            ...
            :placeholder="t('admin.riskControl.modelSelector.searchModels')"
            @click.stop
          />
        </div>
        <div class="max-h-52 overflow-auto">
          ...
          <div v-if="filteredModels.length === 0" class="px-3 py-4 text-center text-sm text-gray-500">
            {{ t('admin.riskControl.modelSelector.noMatchingModels') }}
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Custom Model Input -->
    <div class="mb-3">
      <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.riskControl.modelSelector.customModelName') }}</label>
      <div class="flex gap-2">
        <input
          v-model="customModel"
          type="text"
          class="input flex-1"
          :placeholder="t('admin.riskControl.modelSelector.enterCustomModelName')"
        ...
        >
          {{ t('admin.riskControl.modelSelector.addModel') }}
        </button>
      </div>
    </div>
  </div>
...
const addCustom = () => {
  ...
  if (!model) return
  if (props.modelValue.includes(model)) {
    getSdk().notify.info(t('admin.riskControl.modelSelector.modelExists'))
    return
  }
  emit('update:modelValue', [...props.modelValue, model])
  customModel.value = ''
