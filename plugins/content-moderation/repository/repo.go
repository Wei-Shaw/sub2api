// Package repository holds the SQL persistence for content_moderation_logs
// and the flagged-hash Redis cache. It is migrated from the core's content
// moderation repository and hash cache, rebuilt on top of the SDK-provided
// *sql.DB (proxied through the host SQLProxy) and RedisClient.
package repository
...
// NewContentModerationRepository builds the SQL-backed Repository on top of
// the SDK's *sql.DB.
func NewContentModerationRepository(db *sql.DB) service.Repository {
...
LIMIT $`+fmt.Sprint(len(queryArgs)-1)+` OFFSET $`+fmt.Sprint(len(queryArgs)),
	...
	for rows.Next() {
		item, err := scanContentModerationLog(rows)
		if err != nil {
			return nil, nil, err
		}
	...
	return items, pagination.ResultFromTotal(total, params), nil
}

func scanContentModerationLog(rows *sql.Rows) (service.ContentModerationLog, error) {
	...
	if err := rows.Scan(
		...
		&item.ViolationCount, &item.AutoBanned, &item.EmailSent, &item.UserStatus, &queueDelay, &item.CreatedAt,
	); err != nil {
		return item, fmt.Errorf("scan content moderation log: %w", err)
	}
	if userID.Valid {
		v := userID.Int64
		item.UserID = &v
...
// GroupExists checks the host's groups table (read via db.core.read) so config
// validation can reject unknown group IDs. Replaces the core
// GroupRepository.GetByIDLite lookup.
func (r *contentModerationRepository) GroupExists(ctx context.Context, groupID int64) (bool, error) {
	if groupID <= 0 {
		return false, nil
	}
	var exists bool
	if err := r.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM groups WHERE id = $1)`, groupID).Scan(&exists); err != nil {
