...
func (s *ModerationService) GetStatus(ctx context.Context) (*ContentModerationRuntimeStatus, error) {
	if s == nil {
	...
	active := clampWorkerCount(int(s.asyncActive.Load()), cfg.WorkerCount)
	...
	if s.hashCache != nil {
		if n, err := s.hashCache.CountFlaggedInputHashes(ctx); err == nil {
			...
			s.logger.Warn("content_moderation.hash_count_failed", "error", err)
		}
	}
	...
	return &ContentModerationRuntimeStatus{
		Enabled:                      cfg.Enabled,
		RiskControlEnabled:           cfg.Enabled,
		Mode:                         cfg.Mode,
...
func clampWorkerCount(active, workerCount int) int {
	if active < 0 {
		return 0
	}
