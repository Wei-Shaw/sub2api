      ...
      <OrderTable :orders="orders" :loading="ordersLoading" show-user>
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <button v-if="row.status === 'PENDING'" @click="handleCancelOrder(row)" class="btn-ghost-warning inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium">
              <Icon name="x" size="sm" />
              {{ t('payment.orders.cancel') }}
            </button>
            <button v-if="row.status === 'FAILED'" @click="handleRetryOrder(row)" class="btn-ghost-info inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium">
              <Icon name="refresh" size="sm" />
              {{ t('payment.admin.retry') }}
            </button>
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">{{ row.order_type === 'balance' ? '$' : '¥' }}{{ formatMoney(row.refund_amount) }}</span>
            ...
            <button v-else-if="row.status === 'COMPLETED' || row.status === 'PARTIALLY_REFUNDED'" @click="openRefundDialog(row)" class="btn-ghost-danger inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium">
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.amount') }}</p><p class="text-sm font-medium text-gray-900 dark:text-white">{{ selectedOrder.order_type === 'balance' ? '$' : '¥' }}{{ formatMoney(selectedOrder.amount) }}</p></div>
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</p><p class="text-sm font-medium text-gray-900 dark:text-white">¥{{ formatMoney(selectedOrder.pay_amount) }}</p></div>
          ...
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundAmount') }}</p><p class="text-semantic-danger text-sm font-medium">{{ selectedOrder.order_type === 'balance' ? '$' : '¥' }}{{ formatMoney(selectedOrder.refund_amount) }}</p></div>
...
async function handleRefund(data: { amount: string; reason: string; deduct_balance: boolean; force: boolean }) {
