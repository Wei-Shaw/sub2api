      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-actions="{ row }">
          <div class="flex items-center gap-2">
            <button @click="openPlanEdit(row)" class="hover-tint-info flex flex-col items-center gap-0.5 rounded-lg p-1.5">
              <Icon name="edit" size="sm" />
              <span class="text-xs">{{ t('common.edit') }}</span>
            </button>
            <button @click="confirmDeletePlan(row)" class="hover-tint-danger flex flex-col items-center gap-0.5 rounded-lg p-1.5">
...
import PlanEditDialog, { type PlanPayload } from './PlanEditDialog.vue'
...
async function handleSavePlan(payload: PlanPayload) {
  planSaving.value = true
  try {
    if (editingPlan.value) { await adminPaymentAPI.updatePlan(editingPlan.value.id, payload) }
    else { await adminPaymentAPI.createPlan(payload) }
