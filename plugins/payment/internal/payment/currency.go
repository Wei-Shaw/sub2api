...
// MinorUnitToAmount converts a minor-unit integer (e.g. cents) to a
// precise decimal amount in the given currency.
func MinorUnitToAmount(value int64, currency string) decimal.Decimal {
	factor := decimal.New(1, int32(CurrencyMinorUnit(currency)))
	return decimal.NewFromInt(value).Div(factor)
}

// MinorUnitToAmountFloat64 converts a minor-unit integer to a float64.
// Prefer MinorUnitToAmount for precision-sensitive paths.
func MinorUnitToAmountFloat64(value int64, currency string) float64 {
	return MinorUnitToAmount(value, currency).InexactFloat64()
}
