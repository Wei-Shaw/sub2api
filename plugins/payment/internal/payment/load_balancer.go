...
//
// The instanceID parameter stays int64 to match the LoadBalancer interface
// (legacy callers in service/ pass int64 IDs); the plugin's ent client
// uses the default int primary key, hence the explicit cast on the way
// into ent.
func (lb *DefaultLoadBalancer) GetInstanceConfig(ctx context.Context, instanceID int64) (map[string]string, error) {
	inst, err := lb.db.PaymentProviderInstance.Get(ctx, int(instanceID))
	if err != nil {
