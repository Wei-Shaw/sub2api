...
`simulate_recharge_rebate.py` 用于本地验证与管理"**邀请返利 + 通用信箱通知**"链路，提供三个子命令：

| 子命令 | 作用 |
|---|---|
| `simulate`（默认） | 跑通完整链路：造邀请人 A + 被邀请人 B + 充值 → A 返利 + A 收到信箱通知 → 校验 |
| `add` | 给**现有邀请人**增加一个或多个被邀请人，可选 `--recharge` 立即充值触发返利 |
...
1. **后端已在本地运行**（默认 `http://127.0.0.1:8080`）——`simulate` / `add` 需要。
2. **可连后端的 Postgres**——所有子命令都需要（插订单 / 解析邀请人 / 删除账号）。
3. **已有管理员账号**（role=admin）且**未开启 TOTP 两步验证**（脚本不支持 2FA 登录）——`simulate` / `add` 需要。
...
> 不加 `--recharge` 时只创建绑定关系（不充值、不产生返利）。
...
公共（`simulate` / `add`）：`--base-url` `--admin-email` `--admin-password`；
...
| add | `--count` / `--recharge` / `--amount` / `--rebate-rate` | 1 / off / 100 / 15 | 新增数量、是否充值、充值金额与比例 |
...
- `add`：打印新增被邀请人的 `user_id` / 邮箱，以及（若充值）预期返利。
