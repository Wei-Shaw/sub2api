...
function usage() {
  ...
  sub2api-admin.js accounts create --json '{...REDACTED' | --file account.json
  sub2api-admin.js accounts update <id> --json '{...REDACTED' | --file patch.json
  ...
  sub2api-admin.js accounts apply-oauth <id> --json '{...REDACTED' | --file credentials.json
  sub2api-admin.js accounts batch-create --file accounts.json
  sub2api-admin.js accounts batch-update-credentials --json '{...REDACTED' | --file payload.json
  sub2api-admin.js accounts bulk-update --ids 1,2 --json '{...REDACTED' | --file patch.json
  ...
  sub2api-admin.js accounts crs-preview --json '{...REDACTED' | --file payload.json
  sub2api-admin.js accounts crs-sync --json '{...REDACTED' | --file payload.json
  sub2api-admin.js accounts import-codex-session --json '{...REDACTED' | --file payload.json
  ...
  sub2api-admin.js redeem-codes generate --json '{...REDACTED' | --file payload.json [--idempotency-key KEY]
  sub2api-admin.js redeem-codes create-and-redeem --json '{...REDACTED' | --file payload.json [--idempotency-key KEY]
  sub2api-admin.js redeem-codes batch-update --ids 1,2 --json '{...REDACTED' | --file fields.json
  ...
  sub2api-admin.js api <GET|POST|PUT|DELETE> <admin-path> [--json '{...REDACTED' | --file payload.json]
`);
REDACTED

function parseArgs(argv) {
  const positional = [];
  const flags = {REDACTED;
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token.startsWith("--")) {
      ...
      REDACTED else {
        if (flags[key] === undefined) {
          flags[key] = next;
        REDACTED else if (Array.isArray(flags[key])) {
          flags[key].push(next);
        REDACTED else {
    ...
    REDACTED
  REDACTED
  return { positional, flags REDACTED;
REDACTED

function authHeaders() {
  if (!BASE_URL) throw new Error("Missing SUB2API_BASE_URL");
  if (ADMIN_API_KEY) return { "x-api-key": ADMIN_API_KEY REDACTED;
  if (ADMIN_JWT) return { Authorization: `Bearer ${ADMIN_JWTREDACTED` REDACTED;
  throw new Error("Missing SUB2API_ADMIN_API_KEY or SUB2API_JWT");
REDACTED

async function apiRequest(method, pathname, body, extraHeaders = {REDACTED) {
  const headers = {
    ...authHeaders(),
    Accept: "application/json",
    ...extraHeaders,
  REDACTED;
  const options = { method, headers REDACTED;
  if (body !== undefined) {
    headers["Content-Type"] = "application/json";
    options.body = JSON.stringify(body);
  REDACTED
  const res = await fetch(`${BASE_URLREDACTED${pathnameREDACTED`, options);
  const text = await res.text();
  let data;
  try {
    data = JSON.parse(text);
  REDACTED catch {
    data = { raw: text REDACTED;
  REDACTED
  if (!res.ok || (data && data.code !== undefined && data.code !== 0 && data.code !== "0")) {
    const detail = data.message || data.code || res.statusText;
    throw new Error(`${methodREDACTED ${pathnameREDACTED failed: ${detailREDACTED`);
  REDACTED
  return data.data;
REDACTED

async function apiRawRequest(method, pathname, body, extraHeaders = {REDACTED) {
  const headers = {
    ...authHeaders(),
    Accept: "*/*",
    ...extraHeaders,
...
function encodeQuery(params) {
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    pairs.push(`${encodeURIComponent(key)REDACTED=${encodeURIComponent(String(value))REDACTED`);
  REDACTED
  return pairs.length ? `?${pairs.join("&")REDACTED` : "";
REDACTED

function normalizeAdminPath(pathname) {
  if (!pathname.startsWith("/")) pathname = `/${pathnameREDACTED`;
  if (pathname.startsWith("/api/v1/admin/")) return pathname;
  if (pathname.startsWith("/admin/")) return `/api/v1${pathnameREDACTED`;
  return pathname;
REDACTED
...
async function adminRequestWithHeaders(method, adminPath, body, headers = {REDACTED) {
  return apiRequest(method, normalizeAdminPath(adminPath), body, headers);
REDACTED

async function adminRawRequest(method, adminPath, body, headers = {REDACTED) {
  return apiRawRequest(method, normalizeAdminPath(adminPath), body, headers);
REDACTED

async function listAccounts(options = {REDACTED) {
  const params = {
    page: options.page || 1,
  ...
  REDACTED;
  return apiRequest("GET", `/api/v1/admin/accounts${encodeQuery(params)REDACTED`);
REDACTED

async function getAccount(id) {
  return apiRequest("GET", `/api/v1/admin/accounts/${idREDACTED`);
REDACTED

async function deleteAccount(id) {
  return apiRequest("DELETE", `/api/v1/admin/accounts/${idREDACTED`);
REDACTED
...
function parseBool(value, name = "value") {
  ...
  throw new Error(`${nameREDACTED must be true or false`);
REDACTED
...
function parseIds(value) {
  ...
  return String(value)
    ...
    .map((id) => {
      const n = Number(id);
      if (!Number.isFinite(n)) throw new Error(`invalid id: ${idREDACTED`);
      return n;
    REDACTED);
REDACTED

function readJsonPayload(flags, { required = true REDACTED = {REDACTED) {
...
function idempotencyHeaders(flags) {
  if (!flags["idempotency-key"]) return {REDACTED;
  return { "Idempotency-Key": flags["idempotency-key"] REDACTED;
REDACTED
...
async function accountData(flags) {
  ...
  REDACTED
  return adminRequest("GET", `/admin/accounts/data${encodeQuery(params)REDACTED`);
REDACTED
...
async function commandAccounts(args) {
  ...
  if (sub === "export") {
    ...
    if (args.flags.file) {
      ...
      printJson({ file: path.resolve(args.flags.file), count: Array.isArray(data) ? data.length : data.count REDACTED);
    REDACTED else {
      printJson(data);
    REDACTED
    return;
  ...
  if (sub === "update") {
    ...
    printJson(await adminRequest("PUT", `/admin/accounts/${idREDACTED`, readJsonPayload(args.flags)));
    return;
  REDACTED
  ...
  if (sub === "keep-only") {
    ...
    const data = await listAccounts({ pageSize: Number(args.flags["page-size"] || 500) REDACTED);
    const items = data.items || [];
    const keep = items.find((item) => item.name === name);
    if (!keep) throw new Error(`account not found: ${nameREDACTED`);
    const targets = items.filter((item) => item.name !== name);
    const results = [];
    for (const item of targets) {
      const out = await deleteAccount(item.id);
      results.push({ id: item.id, name: item.name, result: out REDACTED);
    REDACTED
    printJson({ kept: { id: keep.id, name: keep.name REDACTED, deleted: results REDACTED);
    return;
  REDACTED
  ...
  if (sub === "usage") {
    ...
    const params = encodeQuery({ source: args.flags.source, force: args.flags.force ? "true" : undefined REDACTED);
    printJson(await adminRequest("GET", `/admin/accounts/${idREDACTED/usage${paramsREDACTED`));
    return;
  REDACTED
  ...
  if (sub === "stats") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idREDACTED/stats${encodeQuery({ days: args.flags.days || 30 REDACTED)REDACTED`));
    return;
  REDACTED
  ...
  if (sub === "today-stats") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idREDACTED/today-stats`));
    return;
  REDACTED

  if (sub === "batch-today-stats") {
    printJson(await adminRequest("POST", "/admin/accounts/today-stats/batch", { account_ids: parseIds(args.flags.ids) REDACTED));
    return;
  REDACTED
  ...
  if (sub === "set-status") {
    ...
    printJson(await adminRequest("PUT", `/admin/accounts/${idREDACTED`, { status REDACTED));
    return;
  REDACTED
  ...
  if (sub === "set-schedulable") {
    ...
    printJson(await adminRequest("POST", `/admin/accounts/${idREDACTED/schedulable`, { schedulable: parseBool(schedulable, "schedulable") REDACTED));
    return;
  REDACTED
  ...
  if (noBodyPostById[sub]) {
    const id = args.positional[2];
    if (!id) throw new Error(`accounts ${subREDACTED requires <id>`);
    printJson(await adminRequest("POST", `/admin/accounts/${idREDACTED/${noBodyPostById[sub]REDACTED`));
    return;
  REDACTED
  ...
  if (sub === "models") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idREDACTED/models`));
    return;
  REDACTED
  ...
  if (sub === "apply-oauth") {
    ...
    printJson(await adminRequest("POST", `/admin/accounts/${idREDACTED/apply-oauth-credentials`, readJsonPayload(args.flags)));
    return;
  REDACTED
  ...
  if (sub === "batch-create") {
    ...
    if (!Array.isArray(accounts)) throw new Error("batch-create payload must be an array or {accounts:[...]REDACTED");
    printJson(await adminRequest("POST", "/admin/accounts/batch", { accounts REDACTED));
    return;
  REDACTED
  ...
  if (sub === "bulk-update") {
    const patch = readJsonPayload(args.flags, { required: false REDACTED) || {REDACTED;
    const payload = args.flags.ids ? { account_ids: parseIds(args.flags.ids), ...patch REDACTED : patch;
  ...
  if (sub === "batch-refresh") {
    printJson(await adminRequest("POST", "/admin/accounts/batch-refresh", { account_ids: parseIds(args.flags.ids) REDACTED));
    return;
  REDACTED

  if (sub === "batch-clear-error") {
    printJson(await adminRequest("POST", "/admin/accounts/batch-clear-error", { account_ids: parseIds(args.flags.ids) REDACTED));
    return;
  REDACTED
  ...
  if (sub === "temp-unschedulable") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idREDACTED/temp-unschedulable`));
    return;
  REDACTED
  ...
  if (sub === "reset-temp-unschedulable") {
    ...
    printJson(await adminRequest("DELETE", `/admin/accounts/${idREDACTED/temp-unschedulable`));
    return;
  REDACTED
  ...
  if (sub === "import-json") {
    ...
    const live = await listAccounts({ pageSize: 500 REDACTED);
    const liveItems = live.items || [];
    const template = liveItems.find((item) => item.name === templateName);
    if (!template) throw new Error(`template account not found in backend: ${templateNameREDACTED`);
    ...
    const templateModelMapping = (template.credentials && template.credentials.model_mapping) || {REDACTED;
    ...
    for (const acc of planned) {
      if (existingNames.has(acc.name)) {
        results.push({ name: acc.name, skipped: true, reason: "already exists" REDACTED);
        continue;
      REDACTED
      const payload = {
        ...acc,
      ...
      results.push({ name: acc.name, id: created.id, skipped: false REDACTED);
    REDACTED
    printJson({ imported: results REDACTED);
    return;
  REDACTED

  throw new Error(`unknown accounts subcommand: ${sub || "(missing)"REDACTED`);
REDACTED
...
async function commandGroups(args) {
  ...
  if (sub === "all") {
    ...
    return;
  REDACTED
  throw new Error(`unknown groups subcommand: ${sub || "(missing)"REDACTED`);
REDACTED
...
async function commandProxies(args) {
  ...
  if (sub === "all") {
    ...
    return;
  REDACTED
  throw new Error(`unknown proxies subcommand: ${sub || "(missing)"REDACTED`);
REDACTED

async function commandRedeemCodes(args) {
  const sub = args.positional[1];
  if (sub === "list") {
    printJson(await adminRequest("GET", `/admin/redeem-codes${redeemCodesQuery(args.flags)REDACTED`));
    return;
  REDACTED
  if (sub === "export") {
    const csv = await adminRawRequest("GET", `/admin/redeem-codes/export${redeemCodesQuery(args.flags)REDACTED`);
    if (args.flags.file) {
      fs.writeFileSync(path.resolve(args.flags.file), csv);
      printJson({ file: path.resolve(args.flags.file) REDACTED);
    REDACTED else {
  ...
  if (sub === "get") {
    ...
    printJson(await adminRequest("GET", `/admin/redeem-codes/${idREDACTED`));
    return;
  REDACTED
  if (sub === "generate") {
  ...
  REDACTED
  if (sub === "batch-update") {
    const fields = readJsonPayload(args.flags, { required: false REDACTED) || {REDACTED;
    const payload = args.flags.ids ? { ids: parseIds(args.flags.ids), fields REDACTED : fields;
  ...
  if (sub === "delete") {
    ...
    printJson(await adminRequest("DELETE", `/admin/redeem-codes/${idREDACTED`));
    return;
  REDACTED
  if (sub === "batch-delete") {
    printJson(await adminRequest("POST", "/admin/redeem-codes/batch-delete", { ids: parseIds(args.flags.ids) REDACTED));
    return;
  REDACTED
  if (sub === "expire") {
    const id = args.positional[2];
    if (!id) throw new Error("redeem-codes expire requires <id>");
    printJson(await adminRequest("POST", `/admin/redeem-codes/${idREDACTED/expire`));
    return;
  REDACTED
  if (sub === "stats") {
    ...
    return;
  REDACTED
  throw new Error(`unknown redeem-codes subcommand: ${sub || "(missing)"REDACTED`);
REDACTED

async function commandCrudResource(args, name, basePath, options = {REDACTED) {
  const sub = args.positional[1];
  const id = args.positional[2];
  if (sub === "list") {
  ...
  REDACTED
  if (sub === "get") {
    if (!id) throw new Error(`${nameREDACTED get requires <id>`);
    printJson(await adminRequest("GET", `${basePathREDACTED/${idREDACTED`));
    return;
  REDACTED
  if (sub === "create") {
  ...
  REDACTED
  if (sub === "update") {
    if (!id) throw new Error(`${nameREDACTED update requires <id>`);
    printJson(await adminRequest("PUT", `${basePathREDACTED/${idREDACTED`, readJsonPayload(args.flags)));
    return;
  REDACTED
  if (sub === "delete") {
    if (!id) throw new Error(`${nameREDACTED delete requires <id>`);
    printJson(await adminRequest("DELETE", `${basePathREDACTED/${idREDACTED`));
    return;
  REDACTED
  if (options.toggle && sub === "toggle") {
    const enabled = args.positional[3];
    if (!id || enabled === undefined) throw new Error(`${nameREDACTED toggle requires <id> <true|false>`);
    printJson(await adminRequest("PUT", `${basePathREDACTED/${idREDACTED`, { enabled: parseBool(enabled, "enabled") REDACTED));
    return;
  REDACTED
  throw new Error(`unknown ${nameREDACTED subcommand: ${sub || "(missing)"REDACTED`);
REDACTED
...
async function commandApi(args) {
  ...
  const body = readJsonPayload(args.flags, { required: false REDACTED);
...
async function main() {
  ...
  REDACTED
  if (root === "error-rules") {
    await commandCrudResource(args, "error-rules", "/admin/error-passthrough-rules", { toggle: true REDACTED);
    return;
  REDACTED
  ...
  if (root === "api") {
    ...
    return;
  REDACTED
  throw new Error(`unknown command: ${rootREDACTED`);
REDACTED
