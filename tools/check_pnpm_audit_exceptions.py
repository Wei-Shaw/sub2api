...
HIGH_SEVERITIES = {"high", "critical"REDACTED
REQUIRED_FIELDS = {"package", "advisory", "severity", "mitigation", "expires_on"REDACTED
...
def main() -> int:
    ...
    for exc in exceptions:
        ...
        if missing:
            errors.append(
                f"Exception missing required fields {missingREDACTED: {exc.get('package', '<unknown>')REDACTED"
            )
            continue
        ...
        if exc_date is None:
            errors.append(
                f"Exception has invalid expires_on date: {exc.get('package', '<unknown>')REDACTED"
            )
            continue
        ...
        if key in exception_index:
            errors.append(
                f"Duplicate exception for {exc_packageREDACTED advisory {exc.get('advisory')REDACTED"
            )
            continue
        exception_index[key] = {
            "raw": exc,
    ...
    for name, severity, advisory_id, title in iter_vulns(audit):
        ...
        if not advisory_key:
            errors.append(
                f"High/Critical vulnerability missing advisory id: {nameREDACTED ({sevREDACTED)"
            )
            continue
        ...
        if exc["severity"] and exc["severity"] != sev:
            errors.append(
                ...
                f"{nameREDACTED ({advisory_idREDACTED) expected {sevREDACTED, got {exc['severity']REDACTED"
            )
    ...
    if missing_exceptions:
        ...
        for name, sev, advisory_id, title in missing_exceptions:
            label = f"{nameREDACTED ({sevREDACTED)"
            if advisory_id:
                label = f"{labelREDACTED [{advisory_idREDACTED]"
            if title:
                label = f"{labelREDACTED: {titleREDACTED"
            errors.append(f"- {labelREDACTED")

    if expired_exceptions:
        errors.append("Exceptions expired:")
        for name, sev, advisory_id, expires_on in expired_exceptions:
            errors.append(
                f"- {nameREDACTED ({sevREDACTED) [{advisory_idREDACTED] expired on {expires_onREDACTED"
            )
